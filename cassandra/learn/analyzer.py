...
from cassandra._subprocess import Popen, run
...
# CASSANDRA_LEARN_CLI_TIMEOUT_SECS for slow networks or large digests.
_CLI_TIMEOUT = 300
...
# analyses run to completion. Override with CASSANDRA_LEARN_CLI_IDLE_TIMEOUT_SECS.
...
def _detect_default_model() -> str:
    ...
    Priority order:
      ...
      2. CASSANDRA_LEARN_CLI env var → use specified CLI backend
    ...
    cli_override = os.environ.get("CASSANDRA_LEARN_CLI")
    if cli_override:
        for cli_name, model, _cmd in _CLI_BACKENDS:
            if cli_name == cli_override:
                logger.info("CASSANDRA_LEARN_CLI=%s — using %s CLI backend", cli_override, cli_name)
                return model
        valid = ", ".join(name for name, _, _ in _CLI_BACKENDS)
        raise ValueError(
            f"CASSANDRA_LEARN_CLI={cli_override!r} is not a supported CLI. Valid values: {valid}"
        )
    ...
    raise RuntimeError(
        "No LLM API key found. cassandra learn needs one of:\n"
        ...
        "Or set CASSANDRA_LEARN_CLI to a coding agent CLI (claude, gemini, codex).\n"
        "Or install one of those CLIs for auto-detection.\n"
        "Or specify a model directly: cassandra learn --model <litellm-model-name>"
    )
...
def _call_cli_llm(digest: str, model: str) -> dict:
    ...
    hard_cap = _resolve_timeout_secs("CASSANDRA_LEARN_CLI_TIMEOUT_SECS", _CLI_TIMEOUT)

    if model == "claude-cli":
        idle_cap = _resolve_timeout_secs("CASSANDRA_LEARN_CLI_IDLE_TIMEOUT_SECS", _CLI_IDLE_TIMEOUT)
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Check network connectivity, raise CASSANDRA_LEARN_CLI_TIMEOUT_SECS, "
    ...
    while eofs < 2:
        ...
        if elapsed > hard_cap:
            ...
            raise RuntimeError(
                ...
                "Raise CASSANDRA_LEARN_CLI_TIMEOUT_SECS for slower networks or "
        ...
        if idle_elapsed > idle_cap:
            ...
            raise RuntimeError(
                ...
                "CASSANDRA_LEARN_CLI_IDLE_TIMEOUT_SECS, or try a different "
...
def _call_llm(digest: str, model: str) -> dict:
    ...
    # the user's local cassandra proxy
    api_base = None
