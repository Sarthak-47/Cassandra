"""Plugin registry for cassandra learn.

Discovers built-in plugins from cassandra.learn.plugins.* and external
plugins registered via the ``cassandra.learn_plugin`` entry point group.

Follows the same pattern as cassandra.storage_backend (storage/__init__.py).
"""
...
def _discover() -> dict[str, LearnPlugin]:
    ...
    # 1. Built-in: scan cassandra.learn.plugins.* submodules
    from cassandra.learn import plugins as plugins_pkg

    for _, mod_name, _ in pkgutil.iter_modules(plugins_pkg.__path__):
        try:
            mod = importlib.import_module(f"cassandra.learn.plugins.{mod_name}")
    ...
    # 2. External: entry_points(group="cassandra.learn_plugin")
    try:
        from importlib.metadata import entry_points

        for ep in entry_points(group="cassandra.learn_plugin"):
            try:
                obj = ep.load()
