...
# Marker delimiters for Cassandra-managed sections
_MARKER_START = "<!-- cassandra:learn:start -->"
_MARKER_END = "<!-- cassandra:learn:end -->"
...
def _build_section(recommendations: list[Recommendation]) -> str:
    ...
    lines = [
        ...
        f"*Auto-generated by `cassandra learn` on {now} — do not edit manually*",
        "",
    ]
...
def extract_marker_block(file_content: str) -> str | None:
    """Return the raw text of the cassandra:learn marker block, or None.
...
def _strip_marker_block(content: str) -> str:
    """Remove the cassandra:learn marker block from text, tidying blank lines.
...
class ClaudeCodeWriter(ContextWriter):
    ...
    def write(
        ...
        if context_recs:
            ...
            # cassandra versions into the new target, then strip it from CLAUDE.md
    ...
    def _migrate_legacy_block(
        ...
        """Move a stale cassandra block out of CLAUDE.md into the new target.
        ...
        if cleaned:
            ...
            result.warnings.append(
                f"Moved Cassandra learnings out of {legacy_path} into {target_path}: "
            ...
            # CLAUDE.md held nothing but the Cassandra block — remove the husk.
            result.warnings.append(
                f"Removed {legacy_path} (it contained only Cassandra learnings) and "
