...
``rtk`` — Cassandra fetches the prebuilt release asset for the current
...
multi-minute compile is the wrong thing to trigger from ``cassandra wrap``.
...
Supply-chain integrity:
    Because ``cassandra wrap`` downloads and then *executes* this binary by
    ...
    code. When ``CASSANDRA_TOKENSAVE_VERSION`` overrides the pinned tag there
    is no pinned digest, so the download is refused unless the operator
    explicitly opts out of verification via
    ``CASSANDRA_TOKENSAVE_ALLOW_UNVERIFIED=1``.

Env vars:
    CASSANDRA_BINARIES_OFFLINE        if set, never reach the network (returns
                                     the already-installed binary or ``None``).
    CASSANDRA_TOKENSAVE_VERSION       override the pinned release tag.
    CASSANDRA_TOKENSAVE_ALLOW_UNVERIFIED  permit installing an asset that has
...
#: Pinned release. Override with CASSANDRA_TOKENSAVE_VERSION.
...
def _pinned_version() -> str:
    return os.environ.get("CASSANDRA_TOKENSAVE_VERSION", "").strip() or TOKENSAVE_VERSION
...
def _verify_asset_digest(filename: str, data: bytes) -> None:
    ...
    ``CASSANDRA_TOKENSAVE_ALLOW_UNVERIFIED``.
    """
    expected = TOKENSAVE_ASSET_DIGESTS.get(filename)
    if expected is None:
        if os.environ.get("CASSANDRA_TOKENSAVE_ALLOW_UNVERIFIED"):
            logger.warning(
                "tokensave asset %s has no pinned digest; installing unverified "
                "(CASSANDRA_TOKENSAVE_ALLOW_UNVERIFIED is set)",
                filename,
            )
            return
        raise RuntimeError(
            f"no pinned SHA-256 digest for tokensave asset {filename!r}; refusing to "
            "install unverified. Set CASSANDRA_TOKENSAVE_ALLOW_UNVERIFIED=1 to override."
        )
...
def download_tokensave(version: str | None = None) -> Path:
    ...
    try:
        from cassandra._subprocess import run as _run
