...
cache_control breakpoints, growing-prefix strategy). If Cassandra compresses
...
# cassandra.cache.anthropic.ANTHROPIC_CACHE_TTL_SECONDS); the others are best-
...
class SessionTrackerStore:
    ...
    Keyed by session ID (from x-cassandra-session-id header or computed hash).
    ...
    def compute_session_id(
        ...
        Priority:
        1. x-cassandra-session-id header (explicit)
