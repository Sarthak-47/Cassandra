...
DEFAULT_CCR_TTL_SECONDS = 1800  # session-scale; override via CASSANDRA_CCR_TTL_SECONDS
CCR_TTL_SECONDS_ENV = "CASSANDRA_CCR_TTL_SECONDS"
...
CCR_MISS_MESSAGE = (
    ...
    "(default 30 minutes; configurable via CASSANDRA_CCR_TTL_SECONDS)."
)
...
_request_ccr_store: ContextVar[CompressionStore | None] = ContextVar(
    "cassandra_request_ccr_store", default=None
)
...
def _create_default_ccr_backend() -> CompressionStoreBackend | None:
    """Create a CCR backend from env (e.g. CASSANDRA_CCR_BACKEND=redis).

    Default (env unset or "sqlite"): SQLiteBackend at
    ~/.cassandra/ccr_store.db — restart-safe and shared across worker
    processes, which the session-scale 30-minute TTL assumes.
    "memory" opts back into the in-process dict. Other values load
    adapters via setuptools entry point 'cassandra.ccr_backend'.
    Returns None to use InMemoryBackend.
    """
    backend_type = (os.environ.get("CASSANDRA_CCR_BACKEND") or "").strip().lower()
    ...
    try:
        ...
        all_eps = entry_points(group="cassandra.ccr_backend")
        ep = next((e for e in all_eps if e.name == backend_type), None)
        if ep is None:
            logger.warning(
                "CASSANDRA_CCR_BACKEND=%s but no entry point cassandra.ccr_backend[%s]",
                backend_type,
                backend_type,
            )
            return None
        fn = ep.load()
        kwargs = {
            "url": os.environ.get("CASSANDRA_REDIS_URL", ""),
            "tenant_prefix": os.environ.get("CASSANDRA_CCR_TENANT_PREFIX", ""),
        }
...
def get_compression_store(
    ...
    explicitly or created from env (CASSANDRA_CCR_BACKEND) when building the global.
    ...
    Args:
        ...
        default_ttl: Default TTL (only used on first call for global store).
            When omitted, CASSANDRA_CCR_TTL_SECONDS overrides the 1800-second default.
