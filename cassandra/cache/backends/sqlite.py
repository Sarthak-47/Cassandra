...
- **Multi-worker sharing.** The database file (WAL mode) is shared
  across worker processes, so a `cassandra_retrieve` call served by a
...
Set ``CASSANDRA_CCR_BACKEND=memory`` to opt back into the in-memory
backend, or ``CASSANDRA_CCR_SQLITE_PATH`` to relocate the database file
(default ``~/.cassandra/ccr_store.db``).
"""
...
def default_db_path() -> Path:
    """Resolve the database path (env override or ~/.cassandra/)."""
    env = os.environ.get("CASSANDRA_CCR_SQLITE_PATH", "").strip()
    if env:
        return Path(env).expanduser()
    return Path.home() / ".cassandra" / "ccr_store.db"
