...
- ``get_compression_store()`` (the proxy path) defaults to SQLite
  (restart-safe, shared across workers); ``CASSANDRA_CCR_BACKEND=memory``
...
Usage:
    from cassandra.cache.backends import SQLiteBackend, CompressionStoreBackend
    from cassandra.cache.compression_store import CompressionStore, get_compression_store

    # Env-driven default (SQLite at ~/.cassandra/ccr_store.db)
