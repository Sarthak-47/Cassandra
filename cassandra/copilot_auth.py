...
from cassandra.copilot_linux_secret import read_copilot_oauth_token as read_linux_secret_token
from cassandra.copilot_macos_keychain import read_copilot_oauth_token as read_macos_keychain_token
...
def cassandra_copilot_auth_path() -> Path:
    """Return the path where Cassandra stores its Copilot OAuth token."""

    override = os.environ.get("CASSANDRA_COPILOT_AUTH_FILE", "").strip()
    if override:
...
def read_cassandra_copilot_oauth_token() -> str | None:
    """Return Cassandra's saved Copilot OAuth token, if one is available."""

    try:
        payload = json.loads(cassandra_copilot_auth_path().read_text(encoding="utf-8"))
    except FileNotFoundError:
        return None
    except Exception as exc:
        logger.debug("Unable to read Cassandra Copilot auth file: %s", exc)
        return None
...
def save_cassandra_copilot_oauth_token(
    token: str,
    *,
    ...
    path = cassandra_copilot_auth_path()
...
def iter_oauth_token_candidates() -> list[CopilotTokenCandidate]:
    ...
    cassandra_copilot_token = read_cassandra_copilot_oauth_token()
    if cassandra_copilot_token:
        candidates.append(
            CopilotTokenCandidate(
                token=cassandra_copilot_token,
                source=f"cassandra-copilot-auth:{cassandra_copilot_auth_path()}",
