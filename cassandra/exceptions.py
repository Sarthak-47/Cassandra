"""Custom exceptions for Cassandra.

This module provides explicit exception classes for better error handling
and debugging. All exceptions inherit from CassandraError, making it easy
to catch all Cassandra-related errors.

Example:
    from cassandra import CassandraClient, CassandraError, ConfigurationError
...
class CassandraError(Exception):
    ...
    All Cassandra exceptions inherit from this class, making it easy
    to catch any Cassandra-related error:

        try:
            client.chat.completions.create(...)
        except CassandraError as e:
            # Handle any Cassandra error
            pass
    """
...
class ConfigurationError(CassandraError):
    """Raised when Cassandra is misconfigured.
...
class StorageError(CassandraError):
...
class CompressionError(CassandraError):
...
class TokenizationError(CassandraError):
...
class CacheError(CassandraError):
...
class ValidationError(CassandraError):
...
class TransformError(CassandraError):
