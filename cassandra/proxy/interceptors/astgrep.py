...
# reused proxy re-synced by ``cassandra wrap`` takes effect without a restart.
def _min_chars_to_rewrite() -> int:
    try:
        return int(runtime_env.getenv("CASSANDRA_INTERCEPT_READ_MIN_CHARS", "500"))
...
OUTLINE_MARKER = "    # ... (body elided by Cassandra; Read a specific line range to see it)\n"
...
def _run_ast_grep(
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="cassandra-sg-"))
    try:
...
def _build_outline(matches: list[dict[str, Any]], source: str) -> str | None:
    ...
    if not outline_chunks:
        return None
    header = (
        "[cassandra: outlined by ast-grep — "
