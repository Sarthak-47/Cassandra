...
Cassandra's transforms compress what goes INTO the model. This module is the
...
_STEERING_SENTINEL = "<cassandra_output_shaping>"
_STEERING_SUFFIX = "</cassandra_output_shaping>"
...
class OutputShaperSettings:
    ...
    Env-driven (like CASSANDRA_INTERCEPT_ENABLED) so the proxy picks it up
    ...
    def from_env(cls) -> OutputShaperSettings:
        enabled = runtime_env.getenv("CASSANDRA_OUTPUT_SHAPER", "").lower() in (
            "1",
            "true",
            "yes",
        )
        try:
            level = int(runtime_env.getenv("CASSANDRA_VERBOSITY_LEVEL", "2"))
        except ValueError:
            level = 2
        level = max(0, min(4, level))
        router = runtime_env.getenv("CASSANDRA_EFFORT_ROUTER", "1").lower() not in (
            "0",
            "false",
            "no",
        )
        mech = runtime_env.getenv("CASSANDRA_MECHANICAL_EFFORT", "low")
...
def resolve_verbosity_level(settings: OutputShaperSettings) -> tuple[int, str]:
    ...
    Precedence:
      1. ``CASSANDRA_VERBOSITY_LEVEL`` set explicitly → manual override.
      2. AIMD controller state (when ``CASSANDRA_VERBOSITY_AUTOTUNE`` is on).
    ...
    """
    if runtime_env.getenv("CASSANDRA_VERBOSITY_LEVEL"):
