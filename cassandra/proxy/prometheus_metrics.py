"""Prometheus-compatible metrics for the Cassandra proxy.
...
if TYPE_CHECKING:
    from cassandra.observability import CassandraOtelMetrics
    from cassandra.proxy.cost import CostTracker

from cassandra import savings_ledger
from cassandra.observability import get_otel_metrics
from cassandra.proxy.savings_tracker import SavingsTracker
...
class PrometheusMetrics:
    ...
    def __init__(
        ...
        otel_metrics: CassandraOtelMetrics | None = None,
        ...
        # Populated via X-Cassandra-Stack header (TS SDK adapters, etc.)
        ...
        # `cassandra_compressions_total{strategy="smart_crusher"}`
        ...
        # Cassandra overhead (optimization time only, excludes LLM)
    ...
    def _get_otel_metrics(self) -> CassandraOtelMetrics:
    ...
    def record_stack(self, stack: str | None) -> None:
        ...
        ``stack`` is the ``X-Cassandra-Stack`` header value (e.g.
        ...
        from cassandra.telemetry.context import MAX_DISTINCT_STACKS, normalize_stack
    ...
    def record_compression(
        ...
        """Implements `cassandra.transforms.observability.CompressionObserver`.
    ...
    async def record_request(
        ...
        async with self._lock:
            ...
            # Track Cassandra overhead separately
            if overhead_ms > 0:
            ...
            # `cassandra savings` reflects proxy traffic alongside MCP-tool usage.
    ...
    async def export(self) -> str:
        ...
        async with self._lock:
            ...
            _append_metric(
                lines,
                name="cassandra_requests_cached_total",
                metric_type="counter",
            ...
            _append_metric(
                lines,
                name="cassandra_requests_rate_limited_total",
                metric_type="counter",
            ...
            _append_metric(
                lines,
                name="cassandra_requests_failed_total",
                metric_type="counter",
            ...
            _append_metric(
                lines,
                name="cassandra_inbound_requests_total",
                metric_type="counter",
            ...
            _append_metric(
                lines,
                name="cassandra_inbound_requests_completed_total",
                metric_type="counter",
            ...
            _append_metric(
                lines,
                name="cassandra_inbound_requests_active",
                metric_type="gauge",
            ...
            _append_metric(
                lines,
                name="cassandra_tokens_input_total",
                metric_type="counter",
            ...
            _append_metric(
                lines,
                name="cassandra_tokens_output_total",
                metric_type="counter",
            ...
            _append_metric(
                lines,
                name="cassandra_tokens_saved_total",
                metric_type="counter",
            ...
            _append_metric(
                lines,
                name="cassandra_latency_ms_count",
                metric_type="counter",
            ...
            _append_metric(
                lines,
                name="cassandra_overhead_ms_sum",
                metric_type="counter",
                help_text="Sum of Cassandra processing overhead in milliseconds",
                value=round(self.overhead_sum_ms, 2),
            )
            _append_metric(
                lines,
                name="cassandra_overhead_ms_count",
                metric_type="counter",
                help_text="Count of observed Cassandra overhead samples",
                value=self.overhead_count,
            )
            _append_metric(
                lines,
                name="cassandra_overhead_ms_min",
                metric_type="gauge",
                help_text="Minimum observed Cassandra overhead in milliseconds",
                value=0 if self.overhead_count == 0 else round(self.overhead_min_ms, 2),
            )
            _append_metric(
                lines,
                name="cassandra_overhead_ms_max",
                metric_type="gauge",
                help_text="Maximum observed Cassandra overhead in milliseconds",
                value=round(self.overhead_max_ms, 2),
            )
            _append_metric(
                lines,
                name="cassandra_ttfb_ms_sum",
                metric_type="counter",
            ...
            _append_metric(
                lines,
                name="cassandra_cache_bust_total",
                metric_type="counter",
            ...
            _append_metric(
                lines,
                name="cassandra_cache_bust_tokens_lost_total",
                metric_type="counter",
            ...
            lines.extend(
                [
                    "# HELP cassandra_requests_by_provider Requests by provider",
                    "# TYPE cassandra_requests_by_provider counter",
                ]
            )
            for provider, count in self.requests_by_provider.items():
                lines.append(f'cassandra_requests_by_provider{{provider="{provider}"}} {count}')
            lines.append("")

            lines.extend(
                [
                    "# HELP cassandra_requests_by_model Requests by model",
                    "# TYPE cassandra_requests_by_model counter",
                ]
            )
            for model, count in self.requests_by_model.items():
                lines.append(f'cassandra_requests_by_model{{model="{model}"}} {count}')
            lines.append("")

            if self.transform_timing_sum:
                lines.extend(
                    [
                        "# HELP cassandra_transform_timing_ms_sum Sum of transform timing in milliseconds",
                        "# TYPE cassandra_transform_timing_ms_sum counter",
                    ]
                )
                for name, total in self.transform_timing_sum.items():
                    lines.append(
                        f'cassandra_transform_timing_ms_sum{{transform="{_escape_label_value(name)}"}} {round(total, 2)}'
                    )
                lines.extend(
                    [
                        "",
                        "# HELP cassandra_transform_timing_ms_count Count of transform timing samples",
                        "# TYPE cassandra_transform_timing_ms_count counter",
                    ]
                )
                for name, count in self.transform_timing_count.items():
                    lines.append(
                        f'cassandra_transform_timing_ms_count{{transform="{_escape_label_value(name)}"}} {count}'
                    )
                lines.extend(
                    [
                        "",
                        "# HELP cassandra_transform_timing_ms_max Maximum transform timing in milliseconds",
                        "# TYPE cassandra_transform_timing_ms_max gauge",
                    ]
                )
                for name, max_value in self.transform_timing_max.items():
                    lines.append(
                        f'cassandra_transform_timing_ms_max{{transform="{_escape_label_value(name)}"}} {round(max_value, 2)}'
                    )
                lines.append("")
            ...
            if stage_timing_sum_snapshot:
                lines.extend(
                    [
                        "# HELP cassandra_stage_timing_ms_sum Sum of per-stage handler timings in milliseconds",
                        "# TYPE cassandra_stage_timing_ms_sum counter",
                    ]
                )
                for (path_label, stage), total in stage_timing_sum_snapshot.items():
                    lines.append(
                        f'cassandra_stage_timing_ms_sum{{path="{_escape_label_value(path_label)}",stage="{_escape_label_value(stage)}"}} {round(total, 2)}'
                    )
                lines.extend(
                    [
                        "",
                        "# HELP cassandra_stage_timing_ms_count Count of per-stage handler timing samples",
                        "# TYPE cassandra_stage_timing_ms_count counter",
                    ]
                )
                for (path_label, stage), count in stage_timing_count_snapshot.items():
                    lines.append(
                        f'cassandra_stage_timing_ms_count{{path="{_escape_label_value(path_label)}",stage="{_escape_label_value(stage)}"}} {count}'
                    )
                lines.extend(
                    [
                        "",
                        "# HELP cassandra_stage_timing_ms_max Maximum per-stage handler timing in milliseconds",
                        "# TYPE cassandra_stage_timing_ms_max gauge",
                    ]
                )
                for (path_label, stage), max_value in stage_timing_max_snapshot.items():
                    lines.append(
                        f'cassandra_stage_timing_ms_max{{path="{_escape_label_value(path_label)}",stage="{_escape_label_value(stage)}"}} {round(max_value, 2)}'
                    )
                lines.append("")
            ...
            lines.extend(
                [
                    "# HELP cassandra_active_ws_sessions Active Codex WebSocket sessions",
                    "# TYPE cassandra_active_ws_sessions gauge",
                    f"cassandra_active_ws_sessions {self.active_ws_sessions}",
                    "",
                    "# HELP cassandra_active_relay_tasks Active Codex WS relay tasks",
                    "# TYPE cassandra_active_relay_tasks gauge",
                    f"cassandra_active_relay_tasks {self.active_relay_tasks}",
                    "",
                ]
            )
            if self.ws_session_duration_sum_ms:
                lines.extend(
                    [
                        "# HELP cassandra_ws_session_duration_ms_sum Sum of Codex WS session durations",
                        "# TYPE cassandra_ws_session_duration_ms_sum counter",
                    ]
                )
                for cause, total in self.ws_session_duration_sum_ms.items():
                    lines.append(
                        f'cassandra_ws_session_duration_ms_sum{{cause="{_escape_label_value(cause)}"}} {round(total, 2)}'
                    )
                lines.extend(
                    [
                        "",
                        "# HELP cassandra_ws_session_duration_ms_count Count of completed Codex WS sessions",
                        "# TYPE cassandra_ws_session_duration_ms_count counter",
                    ]
                )
                for cause, count in self.ws_session_duration_count.items():
                    lines.append(
                        f'cassandra_ws_session_duration_ms_count{{cause="{_escape_label_value(cause)}"}} {count}'
                    )
                lines.extend(
                    [
                        "",
                        "# HELP cassandra_ws_session_duration_ms_max Maximum Codex WS session duration",
                        "# TYPE cassandra_ws_session_duration_ms_max gauge",
                    ]
                )
                for cause, max_value in self.ws_session_duration_max_ms.items():
                    lines.append(
                        f'cassandra_ws_session_duration_ms_max{{cause="{_escape_label_value(cause)}"}} {round(max_value, 2)}'
                    )
                lines.append("")
            ...
            if self.waste_signals_total:
                lines.extend(
                    [
                        "# HELP cassandra_waste_signal_tokens_total Tokens attributed to detected waste signals",
                        "# TYPE cassandra_waste_signal_tokens_total counter",
                    ]
                )
                for signal_name, token_count in self.waste_signals_total.items():
                    lines.append(
                        f'cassandra_waste_signal_tokens_total{{signal="{_escape_label_value(signal_name)}"}} {token_count}'
                    )
                lines.append("")
            ...
            if self.cache_by_provider:
                lines.extend(
                    [
                        "# HELP cassandra_cache_read_tokens_total Provider cache read tokens",
                        "# TYPE cassandra_cache_read_tokens_total counter",
                    ]
                )
                for provider, stats in self.cache_by_provider.items():
                    lines.append(
                        f'cassandra_cache_read_tokens_total{{provider="{provider}"}} {stats["cache_read_tokens"]}'
                    )
                lines.extend(
                    [
                        "",
                        "# HELP cassandra_cache_write_tokens_total Provider cache write tokens",
                        "# TYPE cassandra_cache_write_tokens_total counter",
                    ]
                )
                for provider, stats in self.cache_by_provider.items():
                    lines.append(
                        f'cassandra_cache_write_tokens_total{{provider="{provider}"}} {stats["cache_write_tokens"]}'
                    )
                lines.extend(
                    [
                        "",
                        "# HELP cassandra_cache_write_ttl_tokens_total Provider cache write tokens by observed TTL bucket",
                        "# TYPE cassandra_cache_write_ttl_tokens_total counter",
                    ]
                )
                for provider, stats in self.cache_by_provider.items():
                    lines.append(
                        f'cassandra_cache_write_ttl_tokens_total{{provider="{provider}",ttl="5m"}} {stats["cache_write_5m_tokens"]}'
                    )
                    lines.append(
                        f'cassandra_cache_write_ttl_tokens_total{{provider="{provider}",ttl="1h"}} {stats["cache_write_1h_tokens"]}'
                    )
                lines.extend(
                    [
                        "",
                        "# HELP cassandra_cache_write_ttl_requests_total Provider cache write requests by observed TTL bucket",
                        "# TYPE cassandra_cache_write_ttl_requests_total counter",
                    ]
                )
                for provider, stats in self.cache_by_provider.items():
                    lines.append(
                        f'cassandra_cache_write_ttl_requests_total{{provider="{provider}",ttl="5m"}} {stats["cache_write_5m_requests"]}'
                    )
                    lines.append(
                        f'cassandra_cache_write_ttl_requests_total{{provider="{provider}",ttl="1h"}} {stats["cache_write_1h_requests"]}'
                    )
                lines.extend(
                    [
                        "",
                        "# HELP cassandra_uncached_input_tokens_total Input tokens not served from provider cache",
                        "# TYPE cassandra_uncached_input_tokens_total counter",
                    ]
                )
                for provider, stats in self.cache_by_provider.items():
                    lines.append(
                        f'cassandra_uncached_input_tokens_total{{provider="{provider}"}} {stats["uncached_input_tokens"]}'
                    )
                lines.extend(
                    [
                        "",
                        "# HELP cassandra_provider_cache_requests_total Requests with provider cache observations",
                        "# TYPE cassandra_provider_cache_requests_total counter",
                    ]
                )
                for provider, stats in self.cache_by_provider.items():
                    lines.append(
                        f'cassandra_provider_cache_requests_total{{provider="{provider}"}} {stats["requests"]}'
                    )
                lines.extend(
                    [
                        "",
                        "# HELP cassandra_provider_cache_hit_requests_total Requests with provider cache reads",
                        "# TYPE cassandra_provider_cache_hit_requests_total counter",
                    ]
                )
                for provider, stats in self.cache_by_provider.items():
                    lines.append(
                        f'cassandra_provider_cache_hit_requests_total{{provider="{provider}"}} {stats["hit_requests"]}'
                    )
                lines.extend(
                    [
                        "",
                        "# HELP cassandra_provider_cache_bust_total Provider-specific cache bust count",
                        "# TYPE cassandra_provider_cache_bust_total counter",
                    ]
                )
                for provider, stats in self.cache_by_provider.items():
                    lines.append(
                        f'cassandra_provider_cache_bust_total{{provider="{provider}"}} {stats["bust_count"]}'
                    )
                lines.extend(
                    [
                        "",
                        "# HELP cassandra_provider_cache_bust_write_tokens_total Provider cache write tokens attributed to busts",
                        "# TYPE cassandra_provider_cache_bust_write_tokens_total counter",
                    ]
                )
                for provider, stats in self.cache_by_provider.items():
                    lines.append(
                        f'cassandra_provider_cache_bust_write_tokens_total{{provider="{provider}"}} {stats["bust_write_tokens"]}'
                    )
                lines.append("")
            ...
            # init.
            from cassandra.proxy.request_logger import redactions_total
            ...
            # `cassandra wrap` CLI (cassandra.cli.wrap); the proxy
            ...
            from cassandra.cli.wrap_rtk_metrics import rtk_invocation_counts
