...
This is the input-side analog of :class:`cassandra.proxy.outcome.RequestOutcome`.
...
``x-cassandra-bypass: true`` requests were silently ignored on every
...
class CompressionDecision:
    ...
    #   * ``"bypass_header"``      — user set x-cassandra-bypass or
    ...
    def decide(
        ...
        *,
        ...
        headers
            ...
            Both ``x-cassandra-bypass: true`` and
            ``x-cassandra-mode: passthrough`` trigger bypass — semantics
            mirrored from :func:`cassandra.proxy.helpers._cassandra_bypass_enabled`.
        config
            ``CassandraConfig``-shaped object; only ``optimize: bool``
            is read.
        usage_reporter
