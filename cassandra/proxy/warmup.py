...
``CassandraProxy.startup`` so that request-time use shares one in-process
singleton and concurrent first-use callers do not trigger N parallel loads.

The registry is populated by ``CassandraProxy.startup`` and exposed as
...
class WarmupRegistry:
    """Shared preloaded asset registry populated by ``CassandraProxy.startup``.

    One instance per ``CassandraProxy``. All writes happen on the startup
