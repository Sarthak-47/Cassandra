...
:meth:`CassandraProxy._record_request_outcome`), which owns the four
...
class RequestOutcome:
    ...
    # Response-cache hit (Cassandra's own semantic cache served the
    ...
    #     via :func:`cassandra.proxy.auth_mode.classify_client`. The
    ...
    def cache_hit(self) -> bool:
        ...
        was served from Cassandra's own response cache.
...
async def emit_request_outcome(handler: Any, outcome: RequestOutcome) -> None:
    ...
    Owns the four downstream effects in canonical order:
      ...
      4. structured PERF log line — consumed by ``cassandra perf``

    Takes the handler as a free argument rather than ``self`` so this
    function is callable from:
    * ``CassandraProxy._record_request_outcome`` (production)
    ...
    * any provider handler mixin
    ...
    """
    from cassandra.proxy.cost import _summarize_transforms
    from cassandra.proxy.models import RequestLog
    from cassandra.proxy.project_context import get_current_project
    ...
    # HTTP middleware / WS accept captured from ``X-Cassandra-Project``.
    ...
    #    line unchanged, and gives ``cassandra perf --client X``
