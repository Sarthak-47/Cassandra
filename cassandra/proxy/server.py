"""Cassandra Proxy Server - Production Ready.
...
Usage:
    python -m cassandra.proxy.server --port 8787
...
from cassandra._version import __version__
from cassandra.agent_savings import proxy_pipeline_kwargs
from cassandra.cache.compression_feedback import get_compression_feedback
from cassandra.cache.compression_store import format_retrieval_miss_detail, get_compression_store
from cassandra.ccr import (
    CCR_TOOL_NAME,
    # Batch processing
    CCRResponseHandler,
    CCRToolInjector,
...
from cassandra.dashboard import get_dashboard_html
...
from cassandra.offline import apply_offline_env, is_offline
from cassandra.pipeline import PipelineExtensionManager, PipelineStage
from cassandra.providers.proxy_routes import register_provider_routes
from cassandra.providers.registry import (
...
from cassandra.proxy import runtime_env
from cassandra.proxy.audit import is_auditable_path, record_admin_action
from cassandra.proxy.auth_mode import should_stamp_codex_client
from cassandra.proxy.background_compression import BackgroundCompressor

# =============================================================================
# Extracted modules (re-exported for backward compatibility)
# =============================================================================
from cassandra.proxy.cost import (
...
from cassandra.proxy.loopback_guard import is_loopback_host
from cassandra.proxy.memory_handler import MemoryConfig, MemoryHandler

# Data models (extracted to cassandra/proxy/models.py for maintainability)
from cassandra.proxy.models import CacheEntry, ProxyConfig, RateLimitState, RequestLog  # noqa: F401
from cassandra.proxy.modes import (
    PROXY_MODE_CACHE,
    PROXY_MODE_TOKEN,
    is_token_mode,
    normalize_proxy_mode,
)
from cassandra.proxy.probe_recorder import probe_recorder_from_env
from cassandra.proxy.project_context import (
    classify_project,
    set_current_project,
    strip_project_path_prefix,
)
from cassandra.proxy.prometheus_metrics import PrometheusMetrics  # noqa: F401
from cassandra.proxy.rate_limiter import TokenBucketRateLimiter  # noqa: F401
from cassandra.proxy.request_logger import RequestLogger  # noqa: F401
from cassandra.proxy.savings_tracker import LITELLM_AVAILABLE
from cassandra.proxy.semantic_cache import SemanticCache  # noqa: F401
from cassandra.proxy.ssl_context import build_httpx_verify
from cassandra.proxy.warmup import WarmupRegistry
from cassandra.proxy.ws_session_registry import WebSocketSessionRegistry
from cassandra.subscription.base import get_quota_registry, reset_quota_registry
from cassandra.subscription.codex_rate_limits import get_codex_rate_limit_state
from cassandra.subscription.copilot_quota import get_copilot_quota_tracker
from cassandra.subscription.tracker import (
    configure_subscription_tracker,
    get_subscription_tracker,
)
from cassandra.telemetry import get_telemetry_collector
from cassandra.telemetry.beacon import is_telemetry_enabled
from cassandra.telemetry.toin import get_toin
from cassandra.transforms import (
    CacheAligner,
    CodeAwareCompressor,
...
def _classify_agent_from_log(entry: dict[str, Any]) -> tuple[str, str, str]:
    ...
    for source, candidate in (
        ...
        ("stack", tags.get("stack") or tags.get("cassandra-stack")),
    ):
...
# PyTorch is optional in cassandra; the warning is not actionable for operators.
...
_MULTI_WORKER_CONFIG_ENV = "CASSANDRA_PROXY_CONFIG_JSON"

# Env var that opts out of the Rust core deployment smoke test (Hotfix-A0).
# Default behavior: hard-fail at startup if `cassandra._core` is unimportable
# (Finding #2 in CASSANDRA_PROXY_LOG_FINDINGS_2026_05_03.md — production
...
# fail-loud behavior.
_RUST_CORE_REQUIRED_ENV = "CASSANDRA_REQUIRE_RUST_CORE"
...
def _check_rust_core() -> tuple[str, str | None]:
    """Verify the Rust extension `cassandra._core` is loadable at startup.

    Returns a `(status, error)` tuple:
      - ``("loaded", None)``     — `cassandra._core.hello()` returned the
        expected sentinel.
    ...
    Behavior is gated by the ``CASSANDRA_REQUIRE_RUST_CORE`` env var:
    ...
    try:
        from cassandra._core import hello as _rust_hello
        ...
        msg = (
            f"FATAL: Rust extension `cassandra._core` not loadable.\n"
            f"    error: {reason}\n"
            f"    fix:   `make build-wheel && pip install --force-reinstall "
            f"target/wheels/cassandra_*.whl`\n"
    ...
    if marker != "cassandra-core":
        ...
        if not require:
            ...
            return ("disabled", reason)
        msg = (
            f"FATAL: Rust extension `cassandra._core` is loaded but the "
            f"marker function returned {marker!r}; expected 'cassandra-core'.\n"
            f"    fix:   rebuild: `make build-wheel && pip install "
            f"--force-reinstall target/wheels/cassandra_*.whl`\n"
        )
...
from cassandra.proxy.handlers import (  # noqa: E402
...
def _apply_stateless_persistence(config: ProxyConfig) -> None:
    ...
    Note: setting ``CASSANDRA_TOIN_BACKEND=none`` is NOT sufficient on its own —
    ...
    Concurrency: ``stateless`` is a per-process flag (set once at ``cassandra
    ...
    where two CassandraProxy instances with different ``stateless`` settings live
    ...
    if not getattr(config, "stateless", False):
        return
    from cassandra.telemetry.toin import TOINConfig, get_toin, reset_toin
...
class CassandraProxy(
    ...
    """Production-ready Cassandra optimization proxy."""
    ...
    def __init__(self, config: ProxyConfig):
        ...
        from cassandra import paths as _hr_paths
        ...
        CassandraProxy.ANTHROPIC_API_URL = api_targets.anthropic
        CassandraProxy.OPENAI_API_URL = api_targets.openai
        CassandraProxy.GEMINI_API_URL = api_targets.gemini
        CassandraProxy.CLOUDCODE_API_URL = api_targets.cloudcode
        CassandraProxy.VERTEX_API_URL = api_targets.vertex
        ...
        # `CASSANDRA_LOSSLESS_ONLY=1` routes SmartCrusher through strict
        ...
        if "CASSANDRA_LOSSLESS_ONLY" in os.environ:
            router_config.smart_crusher_lossless_only = _get_env_bool(
                "CASSANDRA_LOSSLESS_ONLY", False
            )
        ...
        if os.environ.get("CASSANDRA_INTERCEPT_ENABLED"):
            from cassandra.proxy.interceptors import ToolResultInterceptorTransform
        ...
        from cassandra.cache.prefix_tracker import PrefixFreezeConfig, SessionTrackerStore
        ...
        self._compression_executor = concurrent.futures.ThreadPoolExecutor(
            ...
            thread_name_prefix="cassandra-compress",
        )
        ...
        self._background_compression_enabled: bool = os.environ.get(
            "CASSANDRA_BACKGROUND_COMPRESSION", ""
        ).strip().lower() in ("1", "true", "yes", "on")
        try:
            self._background_compression_min_tokens: int = int(
                os.environ.get("CASSANDRA_BACKGROUND_COMPRESSION_MIN_TOKENS", "50000")
            )
        except ValueError:
        ...
        self._background_compression_executor = concurrent.futures.ThreadPoolExecutor(
            max_workers=1, thread_name_prefix="cassandra-bg-compress"
        )
        ...
        if config.memory_enabled and config.stateless:
            ...
            from cassandra.proxy.memory_handler import MemoryMode
            ...
            from cassandra.memory.storage_router import MemoryStorageMode
        ...
        if config.license_key and not (config.offline or is_offline()):
            from cassandra.telemetry.reporter import UsageReporter
        ...
        if config.traffic_learning_enabled:
            from cassandra.memory.traffic_learner import TrafficLearner

            self.traffic_learner = TrafficLearner(
                user_id=os.environ.get("CASSANDRA_USER_ID", os.environ.get("USER", "default")),
                agent_type=config.traffic_learning_agent_type,
        ...
        if config.code_graph_watcher:
            from cassandra.graph.watcher import CodeGraphWatcher
    ...
    def _get_compression_cache(self, session_id: str) -> CompressionCache:
        ...
        with self._compression_caches_lock:
            if session_id not in self._compression_caches:
                from cassandra.cache.compression_cache import CompressionCache
        ...
        if config.code_aware_enabled:
            if is_tree_sitter_available():
                ...
                logger.warning(
                    ...
                    "Install with: pip install cassandra-ai[code]"
                )
                return "unavailable"
        else:
            if is_tree_sitter_available():
    ...
    async def startup(self):
        ...
        # CASSANDRA_TLS_STRICT=0, else httpx's default strict verification.
        ...
        logger.info("Cassandra Proxy started")
        ...
        if self._kompress_status == "enabled":
            ...
            logger.info("Kompress: not installed (pip install cassandra-ai[ml] for ML compression)")
        ...
        if self._code_aware_status == "enabled":
            ...
            logger.info("Code-Aware: not installed (pip install cassandra-ai[code])")
        ...
        # only to your own OTEL collector via CASSANDRA_OTEL_METRICS_*.
        if is_telemetry_enabled():
            logger.info(
                "Local telemetry: ENABLED (aggregate stats, local only — nothing sent "
                "externally). Opt out: CASSANDRA_TELEMETRY=off or --no-telemetry"
            )
        else:
            logger.info(
                "Local telemetry: DISABLED (off by default — opt in: "
                "CASSANDRA_TELEMETRY=on or --telemetry)"
            )
    ...
    async def shutdown(self):
        ...
        with contextlib.suppress(Exception):
            from cassandra.models.ml_models import MLModelRegistry
    ...
    def _print_summary(self):
        ...
        m = self.metrics
        logger.info("=" * 70)
        logger.info("CASSANDRA PROXY SESSION SUMMARY")
        logger.info("=" * 70)
    ...
    async def _record_request_outcome(self, outcome: RequestOutcome) -> None:
        ...
        Thin wrapper around :func:`cassandra.proxy.outcome.emit_request_outcome`
        ...
        """
        from cassandra.proxy.outcome import emit_request_outcome
    ...
    async def _retry_request(
        ...
        *,
        ...
        Byte-faithful forwarding (PR-A3, fixes P0-2):
          ...
          * ``CASSANDRA_PROXY_PYTHON_FORWARDER_MODE=legacy_json_kwarg`` is an
...
def _register_memory_components(proxy: CassandraProxy, tracker: MemoryTracker) -> None:
    ...
    Args:
        proxy: The CassandraProxy instance.
...
def _request_is_loopback(request: Request) -> bool:
    ...
    """
    from cassandra.proxy.loopback_guard import is_loopback_host, is_loopback_host_header
...
def create_app(config: ProxyConfig | None = None) -> FastAPI:
    ...
    # Always-on file logging to ~/.cassandra/logs/ for `cassandra perf` analysis.
    # Installed here (not at module import) so importing cassandra.proxy.server
    ...
    if config.offline:
        os.environ.setdefault("CASSANDRA_OFFLINE", "1")
    if is_offline():
        apply_offline_env()
        logger.warning(
    ...
    proxy = CassandraProxy(config)

    # cc-switch reconciler (opt-in: CASSANDRA_CC_SWITCH_RECONCILE=1).
    # Keeps Cassandra in the request path while cc-switch overwrites
    # ~/.claude/settings.json on every provider switch. See
    # cassandra/proxy/cc_switch_reconciler.py for the full rationale.
    from cassandra.proxy.cc_switch_reconciler import (
        CCSwitchReconciler,
        reconciler_enabled,
    )
    ...
    if reconciler_enabled():
        ...
        def _set_anthropic_upstream(url: str) -> None:
            from cassandra.providers.registry import _normalize_api_url

            CassandraProxy.ANTHROPIC_API_URL = _normalize_api_url(
    ...
    async def lifespan(app: FastAPI):  # type: ignore[no-untyped-def]
        ...
        # explicitly opted out with CASSANDRA_REQUIRE_RUST_CORE=false. See
        # Finding #2 in CASSANDRA_PROXY_LOG_FINDINGS_2026_05_03.md.
        ...
        configure_otel_metrics(OTelMetricsConfig.from_env(default_service_name="cassandra-proxy"))
        configure_langfuse_tracing(
            LangfuseTracingConfig.from_env(default_service_name="cassandra-proxy")
        )
        ...
        try:
            try:
                ...
                # and each process would hold its own CassandraProxy.ANTHROPIC_API_URL,
    ...
    def _health_checks() -> dict[str, dict[str, Any]]:
        ...
        return {
            ...
            "upstream": _component_health(
                enabled=os.environ.get("CASSANDRA_SKIP_UPSTREAM_CHECK", "").strip() != "1",
    ...
    def _health_payload(*, include_config: bool) -> dict[str, Any]:
        ...
        deployment_profile = os.environ.get("CASSANDRA_DEPLOYMENT_PROFILE")
        if deployment_profile:
            payload["deployment"] = {
                "profile": deployment_profile,
                "preset": os.environ.get("CASSANDRA_DEPLOYMENT_PRESET"),
                "runtime": os.environ.get("CASSANDRA_DEPLOYMENT_RUNTIME"),
                "supervisor": os.environ.get("CASSANDRA_DEPLOYMENT_SUPERVISOR"),
                "scope": os.environ.get("CASSANDRA_DEPLOYMENT_SCOPE"),
            }
        if include_config:
            profile_kwargs = proxy_pipeline_kwargs(config)
            ...
            payload["config"] = {
                ...
                # Surfaced so `cassandra wrap` can see what a reused proxy is
    ...
    # every /readyz poll.  Set CASSANDRA_SKIP_UPSTREAM_CHECK=1 to opt out (e.g.
    ...
    async def _check_upstream() -> None:
        ...
        """
        if os.environ.get("CASSANDRA_SKIP_UPSTREAM_CHECK", "").strip() == "1":
    ...
    # layer, not in ProxyConfig). Set CASSANDRA_CORS_ORIGINS (comma-separated)
    ...
    _cors_origins_env = os.environ.get("CASSANDRA_CORS_ORIGINS", "").strip()
    ...
    # X-Cassandra-Stack: SDK adapters (TS openai/anthropic/etc.) tag their
    ...
    @app.middleware("http")
    async def _record_cassandra_stack(request, call_next):
        started = time.perf_counter()
        inbound_id = f"inbound-{time.time_ns()}"
        # Project attribution: an explicit X-Cassandra-Project header wins
        ...
        try:
            from cassandra.proxy.helpers import redact_for_wire_debug
        ...
        if request.url.path.startswith("/v1/"):
            stack = request.headers.get("x-cassandra-stack")
            if stack:
                try:
    ...
    _proxy_token = config.proxy_token or os.environ.get("CASSANDRA_PROXY_TOKEN") or None
    ...
    if not _proxy_token and not is_loopback_host(getattr(config, "host", None)):
        logger.warning(
            ...
            "interface with no CASSANDRA_PROXY_TOKEN set; the /v1/* data-plane "
            "routes are reachable WITHOUT authentication. Set CASSANDRA_PROXY_TOKEN "
    ...
    def _extract_proxy_token(headers) -> str | None:
        ...
        raw = headers.get("x-cassandra-proxy-token")
    ...
    # the `cassandra.proxy_extension` entry-point group, but **opt-in only**:
    # only names listed in config.proxy_extensions (CLI: --proxy-extension,
    # env: CASSANDRA_PROXY_EXTENSIONS) actually get installed. Discovery alone
    ...
    from cassandra.proxy.extensions import install_all as _install_extensions
    ...
    from cassandra.proxy.debug_introspection import (
        collect_tasks as _collect_tasks,
    )
    from cassandra.proxy.loopback_guard import require_loopback as _require_loopback
    ...
    async def get_upstream():
        ...
        """
        return {
            "anthropic": CassandraProxy.ANTHROPIC_API_URL,
    ...
    async def admin_runtime_env(request: Request):
        ...
        that ``cassandra wrap`` reused — rather than started — never sees values
    ...
    async def dashboard():
        """Serve the Cassandra dashboard UI."""
    ...
    async def _build_stats_payload() -> dict[str, Any]:
        ...
        async with _throughput_cache_lock:
            ...
            if _throughput_cache["expires_at"] < now or _throughput_cache["value"] is None:
                ...
                def _compute_throughput():
                    from cassandra.perf.analyzer import build_perf_summary, parse_log_files
        ...
        # Calculate Cassandra overhead (optimization time only, excludes pass-through requests)
        avg_overhead_ms = (
        ...
        # Calculate total tokens before Cassandra-side reduction. Proxy
        ...
        try:
            from cassandra.proxy.output_savings import get_recorder
            ...
            "savings": {
                ...
                "by_layer": {
                    ...
                    "compression": {
                        ...
                        "description": (
                            "Tokens removed by Cassandra proxy compression. "
                    ...
                    "prefix_cache": {
                        ...
                        "description": (
                            ...
                            "Cassandra's CacheAligner improves hit rates; "
    ...
    async def stats_history(
        ...
        """
        if format == "csv":
            filename = f"cassandra-stats-history-{series}.csv"
            return Response(
    ...
    async def subscription_window():
        """Current Anthropic subscription window utilisation and Cassandra contribution.
    ...
    # CCR Tool Call Handler - for agent frameworks to call when LLM uses cassandra_retrieve
...
def _proxy_config_from_env() -> ProxyConfig:
    ...
    if raw_config:
        try:
            ...
            logger.warning(
                "Invalid %s; falling back to CASSANDRA_* env vars", _MULTI_WORKER_CONFIG_ENV
            )

    return ProxyConfig(
        host=_get_env_str("CASSANDRA_HOST", "127.0.0.1"),
        port=_get_env_int("CASSANDRA_PORT", 8787),
        ...
        anthropic_buffered_request_timeout_seconds=_get_env_int(
            "CASSANDRA_ANTHROPIC_BUFFERED_REQUEST_TIMEOUT_SECONDS",
            600,
            min_value=1,
        ),
        vertex_api_url=os.environ.get("VERTEX_TARGET_API_URL"),
        backend=_get_env_str("CASSANDRA_BACKEND", "anthropic"),
        bedrock_region=_get_env_str("CASSANDRA_BEDROCK_REGION", "us-west-2"),
        bedrock_profile=os.environ.get("AWS_PROFILE"),
        bedrock_api_url=os.environ.get("BEDROCK_TARGET_API_URL"),
        anyllm_provider=_get_env_str("CASSANDRA_ANYLLM_PROVIDER", "openai"),
        disable_kompress=_get_env_bool("CASSANDRA_DISABLE_KOMPRESS", False),
        disable_kompress_fallback=_get_env_bool("CASSANDRA_DISABLE_KOMPRESS_FALLBACK", False),
        disable_kompress_anthropic=_get_env_optional_bool("CASSANDRA_DISABLE_KOMPRESS_ANTHROPIC"),
        disable_kompress_openai=_get_env_optional_bool("CASSANDRA_DISABLE_KOMPRESS_OPENAI"),
        max_connections=_get_env_int("CASSANDRA_MAX_CONNECTIONS", 500),
        max_keepalive_connections=_get_env_int("CASSANDRA_MAX_KEEPALIVE", 100),
        keepalive_expiry=_get_env_float("CASSANDRA_KEEPALIVE_EXPIRY", 90.0),
        http2=_get_env_bool("CASSANDRA_HTTP2", True),
        periodic_toin_stats_enabled=_get_env_bool("CASSANDRA_PERIODIC_TOIN_STATS", True),
        proxy_token=os.environ.get("CASSANDRA_PROXY_TOKEN") or None,
        offline=_get_env_bool("CASSANDRA_OFFLINE", False),
        mode=normalize_proxy_mode(_get_env_str("CASSANDRA_MODE", PROXY_MODE_TOKEN)),
        read_maturation=_get_env_bool("CASSANDRA_READ_MATURATION", False),
        read_maturation_quiesce_turns=_get_env_int("CASSANDRA_READ_MATURATION_QUIESCE_TURNS", 5),
        read_maturation_max_hold_turns=_get_env_int("CASSANDRA_READ_MATURATION_MAX_HOLD_TURNS", 25),
        read_maturation_min_size_bytes=_get_env_int(
            "CASSANDRA_READ_MATURATION_MIN_SIZE_BYTES", 2048
        ),
    )
...
def _get_code_aware_banner_status(config: ProxyConfig) -> str:
    ...
    if config.code_aware_enabled:
        if is_tree_sitter_available():
            ...
            return "NOT INSTALLED (pip install cassandra-ai[code])"
    else:
        if is_tree_sitter_available():
            return "DISABLED (--code-aware or CASSANDRA_CODE_AWARE_ENABLED=1 to enable)"
        return "DISABLED  (install cassandra-ai[code] to enable)"
...
def run_server(
    ...
    Args:
        ...
        print_banner: When False, skip the legacy ASCII banner. The
            Click CLI (`cassandra proxy`) prints its own startup banner
            before calling this — printing a second banner here is the
            "dual banner" UX issue. Direct `python -m cassandra.proxy.server`
...
""")
    ...
    if workers > 1:
        ...
        # cross-worker backend is configured via CASSANDRA_CCR_BACKEND.
        # See RUST_DEV.md -> "Multi-worker deployment -- CCR fragmentation".
        if os.environ.get("CASSANDRA_CCR_BACKEND", "").strip():
            logger.warning(
                "Cassandra is running with workers=%d. Compression cache, "
            ...
            logger.warning(
                "Cassandra is running with workers=%d. The in-memory CCR store, "
                ...
                "sessions land on different workers. Set CASSANDRA_CCR_BACKEND=sqlite for a "
        ...
        app_target = "cassandra.proxy.server:create_app_from_env"
...
def _parse_exclude_tools(cli_excludes: str | None) -> set[str]:
    ...
    Both --exclude-tools and CASSANDRA_EXCLUDE_TOOLS are comma-separated
    ...
    """
    raw = ",".join(s for s in (cli_excludes, os.environ.get("CASSANDRA_EXCLUDE_TOOLS")) if s)
    names: set[str] = set()
...
def _parse_csv_tools(raw: str | None) -> set[str]:
    """Parse a bare CSV tool-name string without merging CASSANDRA_EXCLUDE_TOOLS."""
    names: set[str] = set()
    if not raw:
        return names
    for entry in raw.split(","):
...
def _parse_tool_profiles(cli_profiles: list[str]) -> dict[str, Any]:
    """Parse tool profiles from CLI args and CASSANDRA_TOOL_PROFILES env var.
    ...
    """
    from cassandra.config import PROFILE_PRESETS, CompressionProfile

    profiles: dict[str, CompressionProfile] = {}
    raw_entries: list[str] = list(cli_profiles)

    # Also check env var
    env_val = os.environ.get("CASSANDRA_TOOL_PROFILES", "")
    if env_val:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Cassandra Proxy Server")
    ...
    parser.add_argument(
        ...
        "Can be specified multiple times. Also settable via CASSANDRA_TOOL_PROFILES env var.",
    )
    parser.add_argument(
        ...
        help=(
            ...
            "Also settable via CASSANDRA_COMPRESS_USER_MESSAGES=1."
        ),
    )
    parser.add_argument(
        "--disable-kompress",
        ...
        help=(
            ...
            "Also settable via CASSANDRA_DISABLE_KOMPRESS=1."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "Also settable via CASSANDRA_DISABLE_KOMPRESS_FALLBACK=1."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "Also settable via CASSANDRA_DISABLE_KOMPRESS_ANTHROPIC=1."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Also settable via CASSANDRA_DISABLE_KOMPRESS_OPENAI=1."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "Also settable via CASSANDRA_EXCLUDE_TOOLS env var.",
    )
    parser.add_argument(
        ...
        "Also settable via CASSANDRA_PROTECT_TOOL_RESULTS env var.",
    )
    ...
    parser.add_argument(
        "--code-aware",
        action="store_true",
        help="Enable AST-based code compression (requires: pip install cassandra-ai[code])",
    )
    parser.add_argument(
        "--no-code-aware",
        action="store_true",
    ...
    # Environment variable defaults (CASSANDRA_* prefix)
    # CLI args override env vars, env vars override ProxyConfig defaults
    env_code_aware = _get_env_bool("CASSANDRA_CODE_AWARE_ENABLED", True)
    env_optimize = _get_env_bool("CASSANDRA_OPTIMIZE", True)
    env_cache = _get_env_bool("CASSANDRA_CACHE_ENABLED", True)
    env_rate_limit = _get_env_bool("CASSANDRA_RATE_LIMIT_ENABLED", True)
    ...
    protect_tool_results = _parse_csv_tools(
        args.protect_tool_results or os.environ.get("CASSANDRA_PROTECT_TOOL_RESULTS")
    )

    config = ProxyConfig(
        host=_get_env_str("CASSANDRA_HOST", args.host),
        port=_get_env_int("CASSANDRA_PORT", args.port),
        ...
        # Backend settings
        backend=_get_env_str("CASSANDRA_BACKEND", args.backend),  # type: ignore[arg-type]
        bedrock_region=_get_env_str("CASSANDRA_BEDROCK_REGION", args.bedrock_region),
        ...
        anyllm_provider=_get_env_str("CASSANDRA_ANYLLM_PROVIDER", args.anyllm_provider),
        optimize=optimize,
        min_tokens_to_crush=_get_env_int("CASSANDRA_MIN_TOKENS", args.min_tokens),
        max_items_after_crush=_get_env_int("CASSANDRA_MAX_ITEMS", args.max_items),
        smart_crusher_with_compaction=(
            _get_env_bool("CASSANDRA_SMART_CRUSHER_COMPACTION", False)
            if "CASSANDRA_SMART_CRUSHER_COMPACTION" in os.environ
            else None
        ),
        cache_enabled=cache_enabled,
        cache_ttl_seconds=_get_env_int("CASSANDRA_CACHE_TTL", args.cache_ttl),
        rate_limit_enabled=rate_limit_enabled,
        rate_limit_requests_per_minute=_get_env_int("CASSANDRA_RPM", args.rpm),
        rate_limit_tokens_per_minute=_get_env_int("CASSANDRA_TPM", args.tpm),
        budget_limit_usd=args.budget,
        budget_period=args.budget_period,
        log_file=_get_env_str("CASSANDRA_LOG_FILE", args.log_file)
        if args.log_file
        else os.environ.get("CASSANDRA_LOG_FILE"),
        log_full_messages=args.log_messages or _get_env_bool("CASSANDRA_LOG_MESSAGES", False),
        ...
        max_connections=_get_env_int("CASSANDRA_MAX_CONNECTIONS", args.max_connections),
        max_keepalive_connections=_get_env_int("CASSANDRA_MAX_KEEPALIVE", args.max_keepalive),
        keepalive_expiry=_get_env_float("CASSANDRA_KEEPALIVE_EXPIRY", args.keepalive_expiry),
        http2=not args.no_http2 and _get_env_bool("CASSANDRA_HTTP2", True),
        read_maturation=_get_env_bool("CASSANDRA_READ_MATURATION", False),
        read_maturation_quiesce_turns=_get_env_int("CASSANDRA_READ_MATURATION_QUIESCE_TURNS", 5),
        read_maturation_max_hold_turns=_get_env_int("CASSANDRA_READ_MATURATION_MAX_HOLD_TURNS", 25),
        read_maturation_min_size_bytes=_get_env_int(
        ...
        or _get_env_bool("CASSANDRA_COMPRESS_USER_MESSAGES", False),
        savings_profile=os.environ.get("CASSANDRA_SAVINGS_PROFILE") or None,
        # Default 0.4 keep-ratio so the Kompress text (prose/code) path compresses
        # meaningfully out of the box; CASSANDRA_TARGET_RATIO overrides.
        target_ratio=(
            float(os.environ["CASSANDRA_TARGET_RATIO"])
            if os.environ.get("CASSANDRA_TARGET_RATIO")
            else 0.4
        ),
        compress_system_messages=(
            _get_env_bool("CASSANDRA_COMPRESS_SYSTEM_MESSAGES", False)
            if "CASSANDRA_COMPRESS_SYSTEM_MESSAGES" in os.environ
            else None
        ),
        protect_recent=(
            int(os.environ["CASSANDRA_PROTECT_RECENT"])
            if os.environ.get("CASSANDRA_PROTECT_RECENT")
            else None
        ),
        protect_analysis_context=(
            _get_env_bool("CASSANDRA_PROTECT_ANALYSIS_CONTEXT", False)
            if "CASSANDRA_PROTECT_ANALYSIS_CONTEXT" in os.environ
            else None
        ),
        accuracy_guard=os.environ.get("CASSANDRA_ACCURACY_GUARD") or None,
    )

    # Get worker and concurrency settings
    workers = _get_env_int("CASSANDRA_WORKERS", args.workers)
    limit_concurrency = _get_env_int("CASSANDRA_LIMIT_CONCURRENCY", args.limit_concurrency)
