"""SSL context builder for the Cassandra upstream httpx client.
...
Strict-mode toggle (``CASSANDRA_TLS_STRICT``):
    ...
    Setting ``CASSANDRA_TLS_STRICT=0`` clears *only* ``VERIFY_X509_STRICT`` from
    every TLS context Cassandra controls (the httpx upstream client AND the
...
TLS_STRICT_ENV = "CASSANDRA_TLS_STRICT"
...
def tls_strict_disabled() -> bool:
    """True when ``CASSANDRA_TLS_STRICT`` opts out of OpenSSL strict mode.
...
def _relax_x509_strict_for_custom_ca(ctx: ssl.SSLContext, *, path: str) -> ssl.SSLContext:
    ...
    strict flag is relaxed here regardless of ``CASSANDRA_TLS_STRICT`` (the
...
def _default_strict_relaxed_context() -> ssl.SSLContext:
    ...
    the OS/default trust store) but ``CASSANDRA_TLS_STRICT=0`` asks us to
...
def build_httpx_verify() -> ssl.SSLContext | bool:
    ...
    2. No bundle, but ``CASSANDRA_TLS_STRICT=0`` → the default trust store with
...
def apply_global_tls_relaxation() -> bool:
    ...
    When ``CASSANDRA_TLS_STRICT=0`` this monkeypatches
    ...
    if getattr(_u3ssl.create_urllib3_context, "_cassandra_strict_relaxed", False):
        return True
    ...
    _relaxed_create_urllib3_context._cassandra_strict_relaxed = True  # type: ignore[attr-defined]
