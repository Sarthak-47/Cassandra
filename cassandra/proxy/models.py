"""Data models for the Cassandra proxy.
...
from cassandra.providers.registry import ProviderApiOverrides
...
class RequestLog:
    ...
    # See ``cassandra.proxy.helpers.compute_turn_id`` for the derivation. None
...
class ProxyConfig:
    ...
    # CLI: --disable-kompress; env: CASSANDRA_DISABLE_KOMPRESS=1.
    ...
    # CLI: --disable-kompress-fallback; env: CASSANDRA_DISABLE_KOMPRESS_FALLBACK=1.
    ...
    # (and -openai); env: CASSANDRA_DISABLE_KOMPRESS_ANTHROPIC / _OPENAI
    # (1 = disable, 0 = enable).
    ...
    # CLI: --compress-user-messages; env: CASSANDRA_COMPRESS_USER_MESSAGES=1.
    compress_user_messages: bool = False
    # Named savings policy shared across Claude/Codex/Cursor proxy handlers.
    # CLI/env: CASSANDRA_SAVINGS_PROFILE=agent-90.
    ...
    # CLI: --exclude-tools <name1,name2>; env: CASSANDRA_EXCLUDE_TOOLS=<name1,name2>
    ...
    # CLI: --protect-tool-results <name1,name2>; env: CASSANDRA_PROTECT_TOOL_RESULTS=<name1,name2>
    ...
    # env: CASSANDRA_READ_MATURATION=1
    ...
    # env: CASSANDRA_READ_MATURATION_QUIESCE_TURNS / _MAX_HOLD_TURNS / _MIN_SIZE_BYTES.
    ...
    # to enable from the `cassandra.proxy_extension` group, or `["*"]` for
    # wildcard. Empty/None means no extensions run, even if installed.
    # CLI: --proxy-extension <name1,name2>; env: CASSANDRA_PROXY_EXTENSIONS.
    ...
    memory_db_path: str = ""  # Empty = auto: {cwd}/.cassandra/memory.db
    ...
    # x-cassandra-user-id only. ``global`` keeps the pre-fix single-DB
    ...
    # Qdrant connection (defaults resolve from CASSANDRA_QDRANT_* env vars)
    ...
    license_cloud_url: str = "https://app.cassandralabs.ai"
    ...
    # Env: CASSANDRA_PERIODIC_TOIN_STATS=0.
    ...
    # ``X-Cassandra-Proxy-Token`` header). Loopback callers are exempt. Closes the
    # gap where a container bound to 0.0.0.0 exposes unauthenticated /v1/* routes
    # to the pod network. Env: CASSANDRA_PROXY_TOKEN.
    ...
    # downloads) for fully offline / regulated deployments. Env: CASSANDRA_OFFLINE=1.
    offline: bool = False
    ...
    # ``CASSANDRA_COMPRESSION_MAX_WORKERS``.
    #
    ...
    # ``CassandraProxy._run_compression_in_executor``.
