"""Top-level helper functions and constants for the Cassandra proxy.
...
_CODEX_WIRE_DEBUG_ENV = "CASSANDRA_CODEX_WIRE_DEBUG"
_CODEX_WIRE_DEBUG_DIR_ENV = "CASSANDRA_CODEX_WIRE_DEBUG_DIR"
...
#     forwards untouched.
#
# Configurable via CASSANDRA_MEMORY_INJECTION_MODE env var. There is no
...
_MEMORY_INJECTION_MODE_ENV = "CASSANDRA_MEMORY_INJECTION_MODE"
...
_PYTHON_FORWARDER_MODE_ENV = "CASSANDRA_PROXY_PYTHON_FORWARDER_MODE"
...
def extract_tags(headers: Any) -> dict[str, str]:
    """Extract ``x-cassandra-*`` tags from inbound headers.
    ...
    ``x-cassandra-`` prefix stripped.
    """
    return {
        k.lower().replace("x-cassandra-", ""): v
        for k, v in headers.items()
        if k.lower().startswith("x-cassandra-")
    }


def _cassandra_bypass_enabled(headers: Any) -> bool:
    """Return True when inbound headers request full Cassandra passthrough.
    ...
    try:
        bypass = str(headers.get("x-cassandra-bypass", "")).strip().lower() == "true"
        passthrough = str(headers.get("x-cassandra-mode", "")).strip().lower() == "passthrough"
...
_RTK_GAIN_SCOPE_ENV = "CASSANDRA_RTK_GAIN_SCOPE"
...
# CASSANDRA_SSE_BUFFER_MAX_BYTES. Guards against pathological huge events
...
_SSE_EVENT_MAX_BYTES_ENV = "CASSANDRA_SSE_BUFFER_MAX_BYTES"
...
# Configurable via CASSANDRA_PROXY_BODY_TOO_LARGE_STATUS for operators who need
# to override (no expected production use; documentation knob).
_BODY_TOO_LARGE_STATUS_ENV = "CASSANDRA_PROXY_BODY_TOO_LARGE_STATUS"
...
# CASSANDRA_COMPRESSION_TIMEOUT_SECONDS env var for slow CPUs or long Claude Code
# conversations (GH #946). Falls back to 30 on an unparseable value.
try:
    COMPRESSION_TIMEOUT_SECONDS = float(
        os.environ.get("CASSANDRA_COMPRESSION_TIMEOUT_SECONDS", "30")
    )
except ValueError:
...
# CASSANDRA_EAGER_PRELOAD_TIMEOUT_SECONDS. Falls back to 120 on an unparseable value.
try:
    EAGER_PRELOAD_TIMEOUT_SECONDS = float(
        os.environ.get("CASSANDRA_EAGER_PRELOAD_TIMEOUT_SECONDS", "120")
    )
except ValueError:
...
# Cassandra thought was large enough to compress, the legacy behaviour was to
...
# upstream (which Cassandra's earlier successful compressions shrunk), then
...
# ``CASSANDRA_WS_FAIL_OPEN_ON_COMPRESSION_FAILURE=1``. The oversize threshold
# below which transient errors still fall through to passthrough is
# configurable via ``CASSANDRA_WS_COMPRESSION_FAIL_THRESHOLD_BYTES``
# (default 256 KiB ≈ 64K tokens).
WS_COMPRESSION_FAIL_OPEN_ENV = "CASSANDRA_WS_FAIL_OPEN_ON_COMPRESSION_FAILURE"
WS_COMPRESSION_OVERSIZE_BYTES_ENV = "CASSANDRA_WS_COMPRESSION_FAIL_THRESHOLD_BYTES"
...
def decide_compression_failure_action(
    ...
    * Codex client compression timeout → forward. Codex currently treats
      ...
      standalone rather than through ``cassandra wrap codex``.
    * exception is :class:`asyncio.TimeoutError` → refuse (the compression
      stage hit its own timeout, which only fires on frames Cassandra
...
def _get_image_compressor():
    ...
    try:
        from cassandra.image import ImageCompressor
        ...
        if _image_compressor_available is None:
            logger.info("Image compression enabled (model: Sarthak-47/technique-router)")
...
# Always-on file logging to the workspace logs directory for `cassandra perf` analysis.
# Resolved lazily so CASSANDRA_WORKSPACE_DIR env-var changes are honored.


def _cassandra_log_dir() -> Path:
    return _paths.log_dir()


def _setup_file_logging() -> None:
    """Add a RotatingFileHandler to the cassandra root logger.

    Writes to ~/.cassandra/logs/proxy.log with automatic rotation:
    - Rotates at 10 MB
    ...
    try:
        ...
        # Attach to the cassandra root logger so all sub-loggers are captured.
        ...
        if not any(isinstance(h, RotatingFileHandler) for h in cassandra_logger.handlers):
            cassandra_logger.addHandler(handler)
        cassandra_logger.propagate = False
    except OSError:
...
def _context_tool_summary_payload(
    *,
    ...
    Cassandra computes session values by subtracting a startup baseline, so
...
def _get_context_tool_stats() -> dict[str, Any] | None:
    """Get context-tool savings for the current Cassandra proxy session.
...
# ---------------------------------------------------------------------------
#
# `x-cassandra-*` request headers (e.g. ``x-cassandra-bypass``,
# ``x-cassandra-mode``, ``x-cassandra-user-id``, ``x-cassandra-stack``,
# ``x-cassandra-base-url``) are internal control flags consumed by the
...
# ``x-cassandra-*``, memory ``x-cassandra-user-id`` lookup) keep using
# the original dict / ``request.headers``. The stripped copy is what
# every upstream-bound forwarder receives.
#
# Note: response-side ``X-Cassandra-*`` injection (e.g.
# ``x-cassandra-tokens-saved``) is unrelated — the proxy is allowed to
# tell its client about its own work. This helper only filters
# request-side headers.

_INTERNAL_HEADER_PREFIX = "x-cassandra-"

# Operator opt-in env var. ``enabled`` (default) strips internal
# ``x-cassandra-*`` headers from every upstream-bound forwarder.
...
_STRIP_INTERNAL_HEADERS_ENV = "CASSANDRA_STRIP_INTERNAL_HEADERS"
...
def _strip_internal_headers(headers: dict[str, str]) -> dict[str, str]:
    """Return a copy of ``headers`` with internal ``x-cassandra-*`` keys stripped.

    Used at every upstream call site to prevent fingerprinting / leakage of
    internal flags like ``x-cassandra-bypass``, ``x-cassandra-mode``,
    ``x-cassandra-user-id``, ``x-cassandra-stack``, ``x-cassandra-base-url``.
    Case-insensitive on the prefix. Returns a NEW dict; never mutates the
    caller's mapping. Pure function. No regex.

    When the operator opt-in ``CASSANDRA_STRIP_INTERNAL_HEADERS=disabled``
...
#      client value already contained a Cassandra-required token.
#
...
#     then Cassandra-required tokens (in the order passed). Dedupe is
...
#
# Operator opt-in `CASSANDRA_BETA_HEADER_STICKY=disabled` short-circuits
...
_BETA_HEADER_STICKY_ENV = "CASSANDRA_BETA_HEADER_STICKY"
BetaHeaderStickyMode = Literal["enabled", "disabled"]
_BETA_HEADER_STICKY_DEFAULT: BetaHeaderStickyMode = "enabled"

_BETA_TRACKER_MAX_SESSIONS_ENV = "CASSANDRA_BETA_TRACKER_MAX_SESSIONS"
...
def _merge_beta_tokens(client_value: str | None, cassandra_required: list[str]) -> str:
    ...
    * Cassandra-required tokens append in the order given, skipping any
    ...
    * Returns ``""`` when both inputs are empty.
    ...
    for token in _split_beta_tokens(client_value):
        ...
        out.append(token)
    for token in cassandra_required:
        if not token:
            continue
        token = token.strip()
        if not token:
...
def merge_anthropic_beta(client_value: str | None, cassandra_required: list[str]) -> str:
    """Merge client `anthropic-beta` value with Cassandra-required tokens.

    See `_merge_beta_tokens` for full semantics. Order is deterministic:
    client tokens first (in their original order), then cassandra tokens
    ...
    Returns ``""`` when both inputs are empty.
    """
    return _merge_beta_tokens(client_value, cassandra_required)


def merge_openai_beta(client_value: str | None, cassandra_required: list[str]) -> str:
    """Merge client `OpenAI-Beta` value with Cassandra-required tokens.
...
class SessionBetaTracker:
    ...
    def record_and_get_sticky_betas(
        ...
        When `CASSANDRA_BETA_HEADER_STICKY=disabled` returns the client
...
def get_session_beta_tracker() -> SessionBetaTracker:
    ...
    Lazily constructed so the env-var bound (`CASSANDRA_BETA_TRACKER_MAX_SESSIONS`)
...
def log_beta_header_merge(
    *,
    ...
    cassandra_added: list[str],
    request_id: str | None,
) -> None:
    """Structured log for every cache-affecting beta-header merge.

    `cassandra_added` is a list of public, documented beta tokens
    ...
    Cassandra logs. Emitting counts only makes the decision auditable.
    """
    logger.info(
        "event=beta_header_merge provider=%s session_id=%s "
        "client_betas=%d sticky_betas=%d cassandra_added=%s request_id=%s",
        provider,
        session_id or "",
        client_betas_count,
        sticky_betas_count,
        ",".join(cassandra_added) if cassandra_added else "",
        request_id or "",
    )
...
#
# Operator opt-in `CASSANDRA_TOOL_INJECTION_STICKY=disabled` short-
...
_TOOL_INJECTION_STICKY_ENV = "CASSANDRA_TOOL_INJECTION_STICKY"
ToolInjectionStickyMode = Literal["enabled", "disabled"]
_TOOL_INJECTION_STICKY_DEFAULT: ToolInjectionStickyMode = "enabled"

_TOOL_TRACKER_MAX_SESSIONS_ENV = "CASSANDRA_TOOL_TRACKER_MAX_SESSIONS"
...
def serialize_tool_definition_canonical(tool_definition: dict[str, Any]) -> bytes:
    ...
    ``cassandra/proxy/memory_handler.py`` and
    ``cassandra/proxy/memory_tool_adapter.py`` already do.
...
def get_session_tool_tracker() -> SessionToolTracker:
    ...
    (`CASSANDRA_TOOL_TRACKER_MAX_SESSIONS`) is honored at first use.
...
def apply_session_sticky_memory_tools(
    *,
    ...
    Logic (guide §6.3 #2):
      ...
      * If ``CASSANDRA_TOOL_INJECTION_STICKY=disabled``: bypass tracker,
...
# `cassandra_retrieve` tool stays registered in `body["tools"]` for every
...
class SessionCcrTracker:
    ...
    Two pieces of state per session:
      ...
      * ``golden_tool_bytes``: canonical serialization of the
        ``cassandra_retrieve`` tool definition recorded the first time
...
def should_inject_ccr_tool(
    *,
    ...
    """Decide whether the ``cassandra_retrieve`` tool must be injected this turn.
...
def apply_session_sticky_ccr_tool(
    *,
    ...
    """
    from cassandra.ccr.tool_injection import CCR_TOOL_NAME, create_ccr_tool_definition
...
# ``cassandra wrap claude``). For users who run ``claude`` manually we cannot
...
def format_tool_search_disabled_hint(tools: list[Any]) -> str:
    ...
    return (
        ...
        "or launch via `cassandra wrap claude` (which sets it automatically). "
        "See https://github.com/Sarthak-47/cassandra/issues/746"
    )
