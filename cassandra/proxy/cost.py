"""Cost tracking and budget management for the Cassandra proxy.
...
from cassandra.proxy.modes import PROXY_MODE_CACHE
...
def header_safe_transforms(transforms: list[str]) -> list[str]:
    ...
    ``x-cassandra-transforms`` is built as ``",".join(transforms_applied)``, so a
...
def build_prefix_cache_stats(
    ...
    for provider, pc in metrics.cache_by_provider.items():
        ...
        # client (e.g. Claude Code) pays regardless of Cassandra. We track it
    ...
    return {
        ...
        "attribution": (
            ...
            "Cassandra reports cache stats as observed from API responses. "
            "CacheAligner and prefix freeze improve cache hit rates by stabilizing "
            "the message prefix, but baseline caching happens without Cassandra. "
...
def _aggregate_mcp_events() -> dict[str, int]:
    """Aggregate compression / retrieval events written by Cassandra MCP
    server instances to the cross-process shared events file.

    The Cassandra MCP server (``cassandra mcp serve``) records every
    ``cassandra_compress`` and ``cassandra_retrieve`` invocation to a
    file-locked shared log (see :func:`cassandra.ccr.mcp_server._append_shared_event`).
    ...
    Keys: ``compressions`` (count of cassandra_compress calls),
    ``tokens_removed`` (sum of input_tokens-output_tokens across
    compress events), ``retrievals`` (count of cassandra_retrieve
    calls — the load-bearing over-compression signal).
    """
    zero = {"compressions": 0, "tokens_removed": 0, "retrievals": 0}
    try:
        from cassandra.ccr.mcp_server import _read_shared_events
    except ImportError:
        return zero
...
def build_session_summary(
    ...
    "is Cassandra working?" at a glance.
    """
    ...
    cost_with = cost_stats.get("cost_with_cassandra_usd", 0.0)
    compression_savings = cost_stats.get("savings_usd", 0.0)
    ...
    summary: dict[str, Any] = {
        ...
        "cost": {
            "without_cassandra_usd": round(cost_without, 2),
            "with_cassandra_usd": round(cost_with, 2),
    ...
    # MCP-side compression: events written by `cassandra mcp serve`
    ...
    if proxy.config.mode == PROXY_MODE_CACHE and uncompressed_reasons["prefix_frozen"] > 10:
        summary["tip"] = (
            "Most requests are prefix-frozen. Set CASSANDRA_MODE=token "
...
class CostTracker:
    ...
    def estimate_cost(
        ...
        try:
            from cassandra.pricing.litellm_pricing import resolve_litellm_model
    ...
    def record_tokens(
        ...
        Args:
            model: Model name.
            tokens_saved: Tokens removed by compression (Cassandra's count).
            tokens_sent: Compressed message tokens sent (Cassandra's count).
    ...
    def stats(self) -> dict:
        ...
        for model in self._tokens_saved_by_model:
            ...
            if prices:
                ...
                if cr + cw + uncached > 0:
                    ...
                    billed_tokens = sent
                cost_with_cassandra += model_cost
        ...
        return {
            ...
            "total_input_cost_usd": round(cost_with_cassandra, 4),
            ...
            "per_model": per_model,
            "cost_with_cassandra_usd": round(cost_with_cassandra, 4),
            "savings_usd": round(savings_usd, 4),
            # Budget config passthrough — surfaces in /stats["cost"] so
            # `cassandra doctor` can report whether a budget is set.
