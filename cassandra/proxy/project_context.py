...
``cassandra wrap`` launches agents with an ``X-Cassandra-Project`` header
...
from cassandra.proxy.savings_tracker import sanitize_project_name

PROJECT_HEADER = "x-cassandra-project"
PROJECT_PATH_PREFIX = "/p/"

_current_project: ContextVar[str | None] = ContextVar("cassandra_current_project", default=None)
...
def classify_project(headers: Mapping[str, Any] | Any) -> str | None:
    ...
    if get is None:
        return None
    value = get(PROJECT_HEADER) or get("X-Cassandra-Project")
...
def with_project_prefix(base_url: str, project: str | None) -> str:
    ...
    ``cassandra wrap`` for clients that cannot send custom headers.
