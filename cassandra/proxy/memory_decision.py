...
OpenAI chat, Gemini) never gated on ``x-cassandra-bypass``, so
...
Precedence (highest first):
  ...
  4. ``mode_disabled`` — operator CASSANDRA_MEMORY_INJECTION_MODE=disabled
  5. ``mode_tool`` — operator CASSANDRA_MEMORY_INJECTION_MODE=tool
...
class MemoryDecision:
    ...
    #   * "bypass_header"  — user set x-cassandra-bypass/x-cassandra-mode
    #   * "no_handler"     — no memory backend configured on the proxy
    #   * "no_user_id"     — per-request user_id missing
    #   * "mode_disabled"  — operator CASSANDRA_MEMORY_INJECTION_MODE=disabled
    #   * "mode_tool"      — operator CASSANDRA_MEMORY_INJECTION_MODE=tool
    ...
    def decide(
        ...
        *,
        ...
        mode_name
            ...
            ``CASSANDRA_MEMORY_INJECTION_MODE``.
        """
        bypass = _cassandra_bypass_enabled(headers)
