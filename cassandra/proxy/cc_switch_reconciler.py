"""cc-switch reconciler: keep Cassandra in the request path without fighting cc-switch.
...
2. **Captures** the real provider endpoint and sets it as Cassandra's upstream
   (runtime, no restart -- ``CassandraProxy.ANTHROPIC_API_URL`` is a class attr
   read per request).
3. **Rewrites only** ``env.ANTHROPIC_BASE_URL`` back to Cassandra's local URL,
   leaving the token / model / everything else untouched.

Result: ``Claude -> Cassandra (compress) -> selected provider``. cc-switch never
knows; it is the *trigger*, not a coordination partner. The token rides in the
request (Claude -> Cassandra -> upstream, passed through verbatim); Cassandra
never reads or stores it.

Safety
------
- Loop-safe: once ``base_url`` already equals Cassandra's URL, it is left alone.
- Official / empty env (``{"env": {}}``, what cc-switch writes for "Claude
  ...
  base URL is fragile, so v1 does not route it through Cassandra. Set
  ``CASSANDRA_CC_SWITCH_ROUTE_OFFICIAL=1`` to route official through Cassandra too.
- Gated entirely behind ``CASSANDRA_CC_SWITCH_RECONCILE=1`` -- off by default, so
...
def reconciler_enabled() -> bool:
    return os.environ.get("CASSANDRA_CC_SWITCH_RECONCILE", "").strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
...
def _route_official() -> bool:
    return os.environ.get("CASSANDRA_CC_SWITCH_ROUTE_OFFICIAL", "").strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
...
class CCSwitchReconciler:
    """Polls Claude settings.json and keeps Cassandra in the path (see module docstring)."""
    ...
    def tick(self) -> bool:
        ...
        if not url:
            if _route_official():
                ...
                logger.info("cc-switch reconciler: official -> route via Cassandra")
                return True
    ...
    def _atomic_write(self, data: dict) -> None:
        # Per-process temp name: multiple Cassandra processes reconciling the
