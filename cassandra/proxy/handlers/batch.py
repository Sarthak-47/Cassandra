"""Batch handler mixin for CassandraProxy.
...
class BatchHandlerMixin(ProxyHandlerHost):
    ...
    async def handle_google_batch_create(
        ...
        _pre_strip_count_gb = sum(1 for k in headers if k.lower().startswith("x-cassandra-"))
    ...
    async def _google_batch_passthrough(
        ...
        _pre_strip_count_gpt = sum(1 for k in headers if k.lower().startswith("x-cassandra-"))
        headers = _strip_internal_headers(headers)
        log_outbound_headers(
        ...
        # has been synthesized by Cassandra and is canonically serialized.
        ...
        from cassandra.proxy.helpers import get_python_forwarder_mode
    ...
    async def handle_google_batch_passthrough(
        ...
        _pre_strip_count_gp = sum(1 for k in headers if k.lower().startswith("x-cassandra-"))
        headers = _strip_internal_headers(headers)
        log_outbound_headers(
    ...
    async def handle_google_batch_results(
        ...
        _pre_strip_count_gbr = sum(1 for k in headers if k.lower().startswith("x-cassandra-"))
        headers = _strip_internal_headers(headers)
        log_outbound_headers(
    ...
    async def handle_batch_create(self, request: Request) -> Response:
        ...
        _pre_strip_count_oacc = sum(1 for k in headers if k.lower().startswith("x-cassandra-"))
        headers = _strip_internal_headers(headers)
        log_outbound_headers(
        ...
        try:
            ...
            compression_metadata = {
                **metadata,
                "cassandra_compressed": "true",
                "cassandra_original_file_id": input_file_id,
                "cassandra_total_requests": str(stats["total_requests"]),
                "cassandra_tokens_saved": str(stats["total_tokens_saved"]),
                "cassandra_original_tokens": str(stats["total_original_tokens"]),
                "cassandra_compressed_tokens": str(stats["total_compressed_tokens"]),
                "cassandra_savings_percent": f"{stats['savings_percent']:.1f}",
            }
            ...
            # batch_body is synthesized by Cassandra (compressed file_id +
            ...
            response_headers.pop("content-encoding", None)
            response_headers.pop("content-length", None)
            response_headers["x-cassandra-tokens-saved"] = str(stats["total_tokens_saved"])
            response_headers["x-cassandra-savings-percent"] = f"{stats['savings_percent']:.1f}"
    ...
    async def _batch_passthrough(self, request: Request, body: dict) -> Response:
        ...
        headers.pop("content-length", None)
        # PR-A5 (P5-49): strip internal x-cassandra-* before forwarding upstream.
        _pre_strip_count_obp = sum(1 for k in headers if k.lower().startswith("x-cassandra-"))
        headers = _strip_internal_headers(headers)
        log_outbound_headers(
