"""Gemini handler mixin for CassandraProxy.
...
from cassandra.proxy.helpers import COMPRESSION_TIMEOUT_SECONDS, extract_tags
...
class GeminiHandlerMixin(ProxyHandlerHost):
    ...
    async def handle_gemini_generate_content(
        ...
        _pre_strip_count_gem = sum(1 for k in headers if k.lower().startswith("x-cassandra-"))
        ...
        # `x-cassandra-bypass: true`, mutating request bytes under the
        ...
        if len(preserved_indices) == len(contents):
            ...
            if is_streaming:
                ...
                response_headers["x-cassandra-tokens-before"] = str(total_input_tokens)
                response_headers["x-cassandra-tokens-after"] = str(total_input_tokens)
                response_headers["x-cassandra-tokens-saved"] = "0"
    ...
    async def handle_google_cloudcode_stream(
        ...
        # PR-A5 (P5-49): strip internal x-cassandra-* from upstream-bound headers
        # AFTER `_extract_tags` and `is_cloudcode_antigravity` reads.
        from cassandra.proxy.helpers import _strip_internal_headers, log_outbound_headers

        _pre_strip_count_cca = sum(1 for k in headers if k.lower().startswith("x-cassandra-"))
        headers = _strip_internal_headers(headers)
        log_outbound_headers(
    ...
    async def handle_gemini_count_tokens(
        ...
        _pre_strip_count_gem_count = sum(1 for k in headers if k.lower().startswith("x-cassandra-"))
        headers = _strip_internal_headers(headers)
        log_outbound_headers(
