"""Streaming handler mixin for CassandraProxy.
...
from cassandra.copilot_auth import apply_copilot_api_auth
...
class StreamingMixin(ProxyHandlerHost):
    ...
    def _queue_mid_turn_message(self, session_key: str, body: dict) -> dict:
        ...
        return {"status": 202, "event": "cassandra_queued"}
    ...
    def _parse_sse_usage(self, chunk: bytes, provider: str) -> dict[str, int] | None:
        ...
        """
        from cassandra.proxy.helpers import parse_sse_events_from_byte_buffer
    ...
    def _record_ccr_feedback_from_response(
        ...
        """Extract cassandra_retrieve tool calls from a response and record feedback.
        ...
        for block in content:
            ...
            if block.get("type") != "tool_use":
                continue
            if block.get("name") != "cassandra_retrieve":
                continue
    ...
    def _record_ccr_feedback_from_openai_sse(self, full_sse_data: str, request_id: str) -> None:
        """Record cassandra_retrieve feedback from OpenAI Chat Completions SSE.
        ...
        ``cassandra_retrieve`` invocation as a no-op store call for the
        ...
        for slot in tool_calls.values():
            if slot["name"] != "cassandra_retrieve":
                continue
            try:
        ...
        *,
        ...
        async def generate():
            ...
            try:
                ...
                # CCR Feedback: Record cassandra_retrieve tool calls for TOIN learning.
                ...
                if pending_messages:
                    pending_event = json.dumps(
                        {"type": "cassandra_pending_messages", "messages": pending_messages}
                    )
                    yield f"event: cassandra_pending_messages\ndata: {pending_event}\n\n".encode()
        ...
        async def generate():
            try:
                ...
                async for event in self.anthropic_backend.stream_message(body, headers):
                    ...
                    # the token count Cassandra actually sent upstream
    ...
    async def _stream_openai_via_backend(
        ...
        from cassandra.proxy.handlers.openai import _infer_openai_cache_write_tokens
        from cassandra.proxy.outcome import RequestOutcome
        ...
        async def generate():
            ...
            try:
                ...
                # CCR Feedback: record cassandra_retrieve tool calls so
