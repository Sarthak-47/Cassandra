...
Most Cassandra settings are read once at proxy startup into ``Config`` and are
...
process environment, so a *reused* proxy — one ``cassandra wrap`` attaches to
...
process-global override store. ``cassandra wrap`` pushes the values it would
...
(``CASSANDRA_TARGET_RATIO`` etc.) do not belong here — a fresh proxy already
...
RUNTIME_ENV_KNOBS: tuple[Knob, ...] = (
    ...
    Knob(
        "CASSANDRA_OUTPUT_HOLDOUT",
        "float",
        "Fraction of conversations held out for A/B measurement.",
    ),
    Knob(
        "CASSANDRA_INTERCEPT_READ_MIN_CHARS",
        "int",
...
def explicit_env(environ: Mapping[str, str] | None = None) -> dict[str, str]:
    ...
    a default on a shared proxy, set it explicitly (e.g. ``CASSANDRA_OUTPUT_SHAPER=0``).
    """
