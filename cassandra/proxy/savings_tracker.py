...
survive proxy restarts and can be shared by multiple Cassandra frontends.
"""
...
CASSANDRA_SAVINGS_PATH_ENV_VAR = _paths.CASSANDRA_SAVINGS_PATH_ENV
DEFAULT_SAVINGS_DIR = ".cassandra"
...
def get_default_savings_storage_path() -> str:
    ...
    # Preserve legacy behavior: when CASSANDRA_SAVINGS_PATH is set we return
    ...
    env_path = os.environ.get(CASSANDRA_SAVINGS_PATH_ENV_VAR, "").strip()
    if env_path:
        return env_path
...
class SavingsTracker:
    ...
    def __init__(
        ...
        # writes proxy_savings.json (honors CassandraConfig.stateless, which
