...
_MODE_ALIASES = {
    ...
    "token_cassandra": PROXY_MODE_TOKEN,
...
def normalize_proxy_mode(mode: str | None, *, default: str = PROXY_MODE_TOKEN) -> str:
    ...
    if normalized is None:
        logger.warning("Unknown CASSANDRA_MODE '%s', falling back to '%s'", mode, default)
        return default

    if key != normalized:
        logger.info("CASSANDRA_MODE alias '%s' normalized to '%s'", mode, normalized)
    return normalized
