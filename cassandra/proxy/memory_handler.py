...
This module provides memory capabilities for the Cassandra proxy:
...
if TYPE_CHECKING:
    from cassandra.memory.backends.local import LocalBackend
...
# Memory tool names for detection (Cassandra's custom tools)
MEMORY_TOOL_NAMES = {
    "memory_save",
    "memory_search",
    "memory_update",
...
class MemoryConfig:
    ...
    Qdrant connection fields default to values read from ``CASSANDRA_QDRANT_*``
    environment variables (see :mod:`cassandra.memory.qdrant_env`). Passing an
    explicit value to the constructor always wins over the environment.
    """

    enabled: bool = False
    backend: Literal["local", "qdrant-neo4j"] = "local"
    db_path: str = "cassandra_memory.db"
    ...
    # Memory Bridge (bidirectional markdown <-> Cassandra sync)
...
class MemoryHandler:
    ...
    Supports two modes:
    - Custom tools: Cassandra's memory_save, memory_search, etc. (default)
    ...
    def _init_native_memory_dir(self) -> None:
        ...
        if self.config.native_memory_dir:
            ...
            # Default: workspace memories directory (respects CASSANDRA_WORKSPACE_DIR)
    ...
    async def _init_backend_locked(self) -> None:
        ...
        if self.config.backend == "local":
            from cassandra.memory.backends.local import LocalBackend, LocalBackendConfig
            ...
            # Opt-in GPU offload: CASSANDRA_EMBEDDER_RUNTIME=pytorch_mps routes embedding
            ...
            if os.environ.get("CASSANDRA_EMBEDDER_RUNTIME", "").strip().lower() == "pytorch_mps":
                try:
                    ...
                    if torch.backends.mps.is_available():
                        ...
                        logger.info(
                            "Memory: CASSANDRA_EMBEDDER_RUNTIME=pytorch_mps → "
                            "torch embedder on Apple GPU (MPS)"
                        )
                    else:
                        logger.warning(
                            "Memory: CASSANDRA_EMBEDDER_RUNTIME=pytorch_mps requested but "
            ...
            try:
                from cassandra.memory.backends.direct_mem0 import (
                    DirectMem0Adapter,
                    Mem0Config,
                )
                ...
                logger.error(
                    ...
                    "Install with: pip install 'cassandra-ai[memory-stack]'"
                )
                raise
        else:
    ...
    async def _init_and_import_bridge(self) -> None:
        ...
        if self._bridge is not None:
            return
        try:
            from cassandra.memory.bridge import MemoryBridge
            from cassandra.memory.bridge_config import BridgeConfig, MarkdownFormat
    ...
    def _get_memory_tools(self) -> list[dict[str, Any]]:
        ...
        if self._memory_tools is None:
            from cassandra.memory.tools import get_memory_tools_optimized
        ...
        *,
        ...
        Args:
            user_id: User identifier for memory scoping (the base user
                id, derived from ``x-cassandra-user-id`` upstream).
            ...
            ranker: Optional :class:`~cassandra.proxy.memory_ranker.MemoryRanker`
        ...
        """
        from cassandra.proxy.memory_injection import MemoryInjectionBudget
        ...
        # but no x-cassandra-project-id / x-cassandra-cwd / system-prompt
        ...
        try:
            ...
            if ranker is not None:
                from cassandra.proxy.memory_ranker import MemoryCandidate
    ...
    def _append_to_latest_user_tail(
        ...
        *,
        ...
        if provider == "anthropic":
            ...
            # cassandra.proxy.handlers.anthropic which imports MemoryHandler.
        ...
        if provider == "openai":
            from cassandra.proxy.helpers import append_text_to_latest_user_chat_message
