...
External packages hook into the Cassandra proxy at startup by declaring an
entry point in the ``cassandra.proxy_extension`` group in their ``pyproject.toml``:

    [project.entry-points."cassandra.proxy_extension"]
...
Enabling extensions:
  ...
  * CLI:  ``cassandra proxy --proxy-extension shield_enterprise,mypkg``
  * Env:  ``CASSANDRA_PROXY_EXTENSIONS=shield_enterprise,mypkg``
...
ENTRY_POINT_GROUP = "cassandra.proxy_extension"
ENV_VAR = "CASSANDRA_PROXY_EXTENSIONS"
...
def _resolve_enabled(enabled: Iterable[str] | None) -> set[str]:
    ...
    Precedence: explicit ``enabled`` argument > ``CASSANDRA_PROXY_EXTENSIONS``
