...
``cassandra evals probes`` can measure what compression removed from real
proxied sessions. Activated only when ``CASSANDRA_PROBE_RECORD_DIR`` is set;
...
from cassandra.pipeline import PipelineEvent, PipelineStage

logger = logging.getLogger(__name__)

RECORD_DIR_ENV = "CASSANDRA_PROBE_RECORD_DIR"
...
def probe_recorder_from_env() -> CompressionEventRecorder | None:
    """Build a recorder when ``CASSANDRA_PROBE_RECORD_DIR`` is set, else None.
