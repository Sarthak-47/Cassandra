...
Direct port of ``crates/cassandra-core/src/auth_mode.rs``. The two
...
class AuthMode(str, enum.Enum):
    """Three auth-mode classes Cassandra routes compression policy through.
    ...
    #: ``accept-encoding``, never strip; never inject ``X-Cassandra-*``;
...
# short normalized client name. The dashboard / `cassandra perf` use
...
CLIENT_UA_MAP: tuple[tuple[str, str], ...] = (
    ...
    # explicitly (see `cassandra/integrations/strands/README.md`).
...
def classify_client(headers: Mapping[str, Any] | Any, *, default: str | None = None) -> str | None:
    ...
    1. **``X-Client`` header** (explicit override) — clients that
       know they're talking to Cassandra can self-identify with a
