...
----------------------
On Windows, ``cassandra._core`` consumers of the ``ort`` crate (magika
...
package's DLL (which ``cassandra-ai[proxy]`` already depends on).

The fix: before anything can import ``cassandra._core``, resolve the
pip-installed ``onnxruntime\\capi\\onnxruntime.dll`` and export it via
``ORT_DYLIB_PATH``. ``cassandra/__init__.py`` calls this hook, which
...
- Never raises: import-time failure of an optional accelerator must
  not break ``import cassandra``. Without a pin, detection still
  degrades gracefully through CASSANDRA_MAGIKA_INIT_TIMEOUT_SECS and
  the non-ML tiers.
"""
    ...
    try:
        ...
        if spec is None or not spec.origin:
            logger.debug(
                ...
                "tiers via CASSANDRA_MAGIKA_INIT_TIMEOUT_SECS). Install onnxruntime or "
                "set %s explicitly.",
                _ENV_VAR,
                _ENV_VAR,
            )
        ...
        return str(dll)
    except Exception as exc:  # never break `import cassandra` over an accelerator pin
