"""Best-effort "is a newer Cassandra released?" check.
...
Opt out with ``CASSANDRA_UPDATE_CHECK=off``. Also skipped in ``--stateless``
...
def is_update_check_enabled() -> bool:
    ...
    Disabled by ``CASSANDRA_UPDATE_CHECK=off``, offline mode
    (``CASSANDRA_OFFLINE``), stateless mode
    (``CASSANDRA_STATELESS=true``/``1``/``yes``/``on``, matching the proxy's own
    parsing), or any CI environment (``CI`` set).
    """
    from cassandra.offline import is_offline

    if is_offline():
        return False
    if _env_off("CASSANDRA_UPDATE_CHECK"):
        return False
    if _env_on("CASSANDRA_STATELESS"):
        return False
...
def installed_version() -> str | None:
    ...
    ``cassandra._version.get_version()`` — the latter computes a synthetic
...
def _cache_path() -> Path:
    from cassandra.paths import workspace_dir
...
def write_cache(latest_version: str, *, now: float | None = None) -> None:
    ...
    try:
        from cassandra.paths import ensure_workspace_dir
...
def fetch_latest_version(*, allow_pre: bool = False, timeout: float = 4.0) -> str | None:
    ...
    try:
        req = urllib.request.Request(
            _PYPI_JSON_URL,
            headers={"Accept": "application/json", "User-Agent": "cassandra-update-check"},
        )
...
def run_check(*, allow_pre: bool = False, now: float | None = None) -> str | None:
    ...
    Synchronous — used directly by ``cassandra update`` and indirectly by
...
def maybe_check_async() -> threading.Thread | None:
    ...
    try:
        ...
        thread = threading.Thread(target=_worker, name="cassandra-update-check", daemon=True)
        thread.start()
        return thread
    except Exception:
...
def format_update_notice() -> str | None:
    ...
    try:
        ...
        return f"Update available: Cassandra {latest} (you have {current}) - run: cassandra update"
    except Exception:
