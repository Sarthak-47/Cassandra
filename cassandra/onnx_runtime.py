"""ONNX Runtime helpers for long-running Cassandra processes."""
...
# CASSANDRA_HF_PIN=off to bypass pinning (e.g. when intentionally evaluating a
# newer model revision). To upgrade a model, bump its SHA here deliberately.
_PINNED_REVISIONS: dict[str, str] = {
    # Sarthak-47/kompress-v2-base @ 2026-06-10
    "Sarthak-47/kompress-v2-base": "b1563631b35bfdcee37587ad530147497d820d4c",
    "Sarthak-47/technique-router-onnx": "27b0b4bfa510a1cff66d888072c0b807082721a8",
    "Sarthak-47/siglip-image-encoder-onnx": "d0a9fbd66d4bd8c761bff592d44831f7c2ae184e",
...
def _resolve_revision(repo_id: str, revision: str | None) -> str | None:
    ...
    if revision is not None:
        return revision
    if os.environ.get("CASSANDRA_HF_PIN", "").strip().lower() in ("off", "0", "false", "no"):
        return None
...
def create_cpu_session_options(
    ...
    *,
    ...
    Cassandra runs as a long-lived proxy process, so we bias toward predictable
