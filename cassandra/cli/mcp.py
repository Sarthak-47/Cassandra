...
Provides commands to configure and run the Cassandra MCP server, enabling
...
def get_cassandra_command() -> list[str]:
    """Get the command to run cassandra MCP server.

    Returns the CLI invocation used by Claude Code config.
    """
    return ["cassandra", "mcp", "serve"]
...
def mcp() -> None:
    ...
    \b
    The MCP server exposes cassandra_retrieve as a tool that Claude Code
    ...
    \b
    Quick Start:
        cassandra mcp install    # Configure Claude Code
        cassandra proxy          # Start the proxy (in another terminal)
    ...
    How it works:
        ...
        4. When Claude needs full details, it calls cassandra_retrieve
        5. The MCP server fetches original content from the proxy

    \b
    Note on tool naming: MCP clients display tools as
    `mcp__<server>__<tool>`. Our server is named "cassandra" and our
    tools are named cassandra_retrieve / cassandra_compress / etc., so
    Claude Code shows them as `mcp__cassandra__cassandra_retrieve`. The
    "cassandra" doubling is normal MCP namespacing — not a bug. The
    proxy's compression markers (and any docs/prompts) reference the
    bare tool name `cassandra_retrieve`.
    """
    pass
...
@click.option(
    ...
    help=f"Cassandra proxy URL (default: {DEFAULT_PROXY_URL})",
)
@click.option(
    "--agent",
    "agents",
...
@click.option(
    "--force",
    is_flag=True,
    help="Overwrite existing cassandra config in case of mismatch.",
)
def mcp_install(proxy_url: str, agents: tuple[str, ...], force: bool) -> None:
    """Install the Cassandra MCP server into every detected coding agent.
    ...
    \b
    Examples:
        cassandra mcp install                            # every detected agent
        cassandra mcp install --agent claude             # Claude Code only
        cassandra mcp install --proxy-url http://localhost:9000
    """
    try:
        import mcp  # noqa: F401
    except ImportError:
        click.echo("Error: MCP SDK not installed.", err=True)
        click.echo("Install with: pip install 'cassandra-ai[mcp]'", err=True)
        raise SystemExit(1) from None

    from cassandra.mcp_registry import any_succeeded, format_results, install_everywhere
    ...
    click.echo("Installing Cassandra MCP server...")
    ...
    click.echo(
        f"\nNext steps:\n"
        f"  1. Start the Cassandra proxy (if not running): cassandra proxy\n"
...
def mcp_uninstall() -> None:
    """Remove Cassandra MCP server from Claude Code config.

    \b
    Removes cassandra from both the claude CLI registry (Claude Code CLI >=2.x)
    ...
    if claude_cli:
        check = subprocess.run(
            [claude_cli, "mcp", "get", "cassandra"],
            capture_output=True,
        )
        if check.returncode == 0:
            rm = run(
                [claude_cli, "mcp", "remove", "cassandra", "-s", "user"],
                capture_output=True,
                text=True,
            )
            if rm.returncode == 0:
                click.echo("✓ Cassandra MCP server removed (via claude mcp remove)")
                removed = True
            else:
                click.echo(
    ...
    if MCP_CONFIG_PATH.exists():
        ...
        changed = False
        for server_name in ("cassandra", "codebase-memory-mcp"):
    ...
    if not removed:
        if MCP_CONFIG_PATH.exists():
            click.echo("Cassandra MCP is not configured. Nothing to uninstall.")
        else:
...
@mcp.command("status")
def mcp_status() -> None:
    """Check Cassandra MCP configuration status.

    \b
    Shows whether cassandra is configured in Claude Code and if
    the proxy is reachable.
    """
    click.echo("Cassandra MCP Status")
    click.echo("=" * 40)
    ...
    try:
        ...
        click.echo("                pip install 'cassandra-ai[mcp]'")

    # Check config
    if MCP_CONFIG_PATH.exists():
        config = load_mcp_config()
        if "cassandra" in config.get("mcpServers", {}):
            server_config = config["mcpServers"]["cassandra"]
            click.echo("Claude Config:  ✓ Configured")
            click.echo(f"                {MCP_CONFIG_PATH}")

            # Show proxy URL
            env = server_config.get("env", {})
            proxy_url = env.get("CASSANDRA_PROXY_URL", DEFAULT_PROXY_URL)
            click.echo(f"Proxy URL:      {proxy_url}")
        else:
            click.echo("Claude Config:  ✗ Not configured")
            click.echo("                Run: cassandra mcp install")
    else:
    ...
    try:
        ...
        config = load_mcp_config()
        env = config.get("mcpServers", {}).get("cassandra", {}).get("env", {})
        proxy_url = env.get("CASSANDRA_PROXY_URL", DEFAULT_PROXY_URL)
...
def mcp_serve(proxy_url: str | None, direct: bool, debug: bool) -> None:
    ...
    # Check for MCP SDK
    try:
        from cassandra.ccr.mcp_server import create_ccr_mcp_server
