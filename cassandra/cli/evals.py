...
def evals() -> None:
    ...
    \b
    Examples:
        cassandra evals memory       Run LoCoMo memory evaluation
        cassandra evals memory-v2    Run V2 evaluation with LLM-controlled tools
        cassandra evals adversarial  Compression-robustness adversarial grid
        cassandra evals probes       Retention probes over recorded sessions
    """
    pass
...
def memory_eval_v2(
    ...
    \b
    Examples:
        cassandra evals memory-v2 -n 3
        cassandra evals memory-v2 --answer-model gpt-4o --save-model gpt-4o-mini
    """
    _run_memory_eval_v2(
        n_conversations=n_conversations,
        categories=categories,
...
def memory_eval_compat(
    ...
    """Deprecated: Use 'cassandra evals memory' instead."""
    click.echo("Note: 'memory-eval' is deprecated. Use 'cassandra evals memory'", err=True)
    _run_memory_eval(
        n_conversations=n_conversations,
        categories=categories,
        include_adversarial=include_adversarial,
...
def memory_eval_v2_compat(
    ...
    """Deprecated: Use 'cassandra evals memory-v2' instead."""
    click.echo("Note: 'memory-eval-v2' is deprecated. Use 'cassandra evals memory-v2'", err=True)
    _run_memory_eval_v2(
        n_conversations=n_conversations,
        categories=categories,
        include_adversarial=include_adversarial,
...
def _run_memory_eval(
    *,
    ...
    try:
        from cassandra.evals.memory import (
            LoCoMoEvaluator,
            MemoryEvalConfig,
        ...
        from cassandra.memory import MemoryConfig
    except ImportError as e:
        click.echo("Error: Memory eval dependencies not installed.")
        click.echo("Run: pip install cassandra[memory,evals]")
...
def _run_memory_eval_v2(
    *,
    ...
    click.echo(f"""
╔═══════════════════════════════════════════════════════════════════════╗
║                   CASSANDRA MEMORY EVALUATION V2                        ║
...
@click.option(
    ...
    help="Directory of JSONL recordings written via CASSANDRA_PROBE_RECORD_DIR.",
)
@click.option(
    "--json-output",
...
def probes(recordings_dir: Path, json_output: Path | None) -> None:
    ...
    CASSANDRA_PROBE_RECORD_DIR set. Recordings contain full conversation
    content in plaintext and stay on this machine.
    """
    import json as json_module

    from cassandra.evals.session_probes import render_report, run_probes
...
def adversarial(json_output: Path | None) -> None:
    ...
    from cassandra.evals.adversarial_grid import render_report, run_adversarial_grid
