...
def perf(hours: float, raw: bool, output_format: str) -> None:
    ...
    \b
    Reads logs from ~/.cassandra/logs/proxy.log and shows:
    ...
    \b
    Examples:
        cassandra perf                      Analyze last 7 days
        cassandra perf --hours 24           Analyze last 24 hours
        cassandra perf --raw                Show raw parsed records
        cassandra perf --format json        Aggregated report as JSON
        cassandra perf --format csv --hours 24 > last-24h.csv
        cassandra perf --format json --raw  Raw records as a JSON array
    """
    ...
    if raw:
        ...
        if not report.perf_records:
            click.echo("No PERF records found. Run the proxy first: cassandra proxy")
    else:
