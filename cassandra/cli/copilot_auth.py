...
from cassandra.copilot_auth import (
    DEFAULT_GITHUB_HOST,
    cassandra_copilot_auth_path,
    poll_copilot_device_authorization,
    read_cassandra_copilot_oauth_token,
    save_cassandra_copilot_oauth_token,
...
def copilot_auth() -> None:
    """Manage Cassandra's GitHub Copilot OAuth token."""
...
def login(domain: str) -> None:
    ...
    path = save_cassandra_copilot_oauth_token(token, domain=domain)
...
def status() -> None:
    """Show whether Cassandra has a saved Copilot OAuth token."""

    token = read_cassandra_copilot_oauth_token()
