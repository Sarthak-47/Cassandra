...
from cassandra.providers.registry import resolve_api_overrides, resolve_api_targets
from cassandra.proxy.modes import PROXY_MODE_TOKEN, normalize_proxy_mode
...
# Corporate TLS-inspection support (issue #1308). When CASSANDRA_TLS_STRICT=0,
...
try:  # pragma: no cover - exercised via integration, not unit-importable cheaply
    from cassandra.proxy.ssl_context import apply_global_tls_relaxation as _apply_tls_relax
...
@click.option(
    ...
    help="Proxy port (default: 8787, env: CASSANDRA_PORT)",
)
...
def dashboard(port: int, no_open: bool) -> None:
    """Open the Cassandra savings dashboard in your browser.

    Requires a running proxy (start one with `cassandra proxy` or `cassandra wrap ...`).
    """
    import webbrowser
...
@click.option(
    ...
    help="Host to bind to (default: 127.0.0.1, env: CASSANDRA_HOST)",
)
@click.option(
    "--port",
    "-p",
    ...
    type=click.IntRange(1, 65535),
    envvar="CASSANDRA_PORT",
    help="Port to bind to (default: 8787, env: CASSANDRA_PORT)",
)
@click.option(
    "--workers",
    default=1,
    type=click.IntRange(min=1),
    envvar="CASSANDRA_WORKERS",
    help="Number of Uvicorn worker processes (default: 1, env: CASSANDRA_WORKERS)",
)
@click.option(
    "--limit-concurrency",
    default=1000,
    type=click.IntRange(min=1),
    envvar="CASSANDRA_LIMIT_CONCURRENCY",
    help=(
        "Maximum concurrent connections before Uvicorn returns 503 "
        "(default: 1000, env: CASSANDRA_LIMIT_CONCURRENCY)"
    ),
)
@click.option(
    "--max-connections",
    default=500,
    type=click.IntRange(min=1),
    envvar="CASSANDRA_MAX_CONNECTIONS",
    help="Maximum upstream HTTP connections (default: 500, env: CASSANDRA_MAX_CONNECTIONS)",
)
@click.option(
    "--max-keepalive",
    ...
    envvar="CASSANDRA_MAX_KEEPALIVE",
    help="Maximum upstream keep-alive connections (default: 100, env: CASSANDRA_MAX_KEEPALIVE)",
)
@click.option(
    "--http2/--no-http2",
    "http2",
    default=True,
    envvar="CASSANDRA_HTTP2",
    help=(
        "Use HTTP/2 to upstream providers (default: on, env: CASSANDRA_HTTP2). "
...
@click.option(
    ...
    envvar="CASSANDRA_KEEPALIVE_EXPIRY",
    help="Seconds an idle upstream keep-alive connection is kept open (default: 90, env: CASSANDRA_KEEPALIVE_EXPIRY)",
)
@click.option(
    "--mode",
    default=None,
    ...
    help=(
        ...
        "Legacy aliases (token_mode, token_savings, token_cassandra, cache_mode, "
        "cost_savings) are still accepted. Env: CASSANDRA_MODE."
    ),
)
@click.option(
    "--target-ratio",
...
@click.option(
    ...
    default=None,
    type=click.IntRange(min=1),
    envvar="CASSANDRA_RPM",
    help="Max requests per minute. Env: CASSANDRA_RPM. Default: 60.",
)
@click.option(
    "--tpm",
    default=None,
    type=click.IntRange(min=1),
    envvar="CASSANDRA_TPM",
    help="Max tokens per minute. Env: CASSANDRA_TPM. Default: 100000.",
)
@click.option(
    "--no-ccr-inject-tool",
    is_flag=True,
    envvar="CASSANDRA_NO_CCR_INJECT_TOOL",
    help=(
        "Don't inject the CCR cassandra_retrieve tool. Run compression-only — "
        "for streaming / non-MCP clients that can't resolve the retrieve tool "
        "and would otherwise error on it. Env: CASSANDRA_NO_CCR_INJECT_TOOL."
    ),
)
@click.option(
    "--no-ccr-marker",
    is_flag=True,
    envvar="CASSANDRA_NO_CCR_MARKER",
    help=("Don't add CCR retrieval markers to compressed content. Env: CASSANDRA_NO_CCR_MARKER."),
)
@click.option(
    "--no-ccr-proactive-expansion",
    is_flag=True,
    envvar="CASSANDRA_NO_CCR_PROACTIVE_EXPANSION",
    help=(
        "Disable proactive expansion of previously compressed content. "
        "Env: CASSANDRA_NO_CCR_PROACTIVE_EXPANSION."
    ),
)
@click.option(
    "--proxy-extension",
    "proxy_extension",
    multiple=True,
    envvar="CASSANDRA_PROXY_EXTENSIONS",
    help=(
        ...
        "every discovered extension. Env: CASSANDRA_PROXY_EXTENSIONS."
    ),
)
@click.option(
    "--no-subscription-tracking",
    is_flag=True,
    envvar="CASSANDRA_NO_SUBSCRIPTION_TRACKING",
    help=(
        "Disable the Anthropic Claude Code subscription usage poller "
        "(GET /api/oauth/usage). Env: CASSANDRA_NO_SUBSCRIPTION_TRACKING."
    ),
)
@click.option(
    ...
    default=None,
    envvar="CASSANDRA_SUBSCRIPTION_POLL_INTERVAL",
    help=(
        ...
        "Env: CASSANDRA_SUBSCRIPTION_POLL_INTERVAL."
    ),
)
@click.option(
    "--retry-max-attempts",
...
@click.option(
    ...
    type=int,
    default=None,
    envvar="CASSANDRA_REQUEST_TIMEOUT",
    help=(
        "Request timeout in seconds (default: 300). "
        "Useful for slow providers (eg local). "
        "Env: CASSANDRA_REQUEST_TIMEOUT."
    ),
)
@click.option(
    ...
    default=None,
    envvar="CASSANDRA_CONNECT_TIMEOUT_SECONDS",
    help=(
        "Upstream connection timeout in seconds (1–300, default: 10). "
        "Env: CASSANDRA_CONNECT_TIMEOUT_SECONDS."
    ),
)
@click.option(
    ...
    type=click.IntRange(min=1),
    default=None,
    envvar="CASSANDRA_ANTHROPIC_BUFFERED_REQUEST_TIMEOUT_SECONDS",
    help=(
        "Buffered Anthropic read timeout in seconds for non-streaming "
        "message and batch paths (default: 600). "
        "Env: CASSANDRA_ANTHROPIC_BUFFERED_REQUEST_TIMEOUT_SECONDS."
    ),
)
@click.option(
    ...
    type=int,
    default=None,
    envvar="CASSANDRA_ANTHROPIC_PRE_UPSTREAM_CONCURRENCY",
    help=(
        ...
        "Env: CASSANDRA_ANTHROPIC_PRE_UPSTREAM_CONCURRENCY."
    ),
)
@click.option(
    ...
    type=float,
    default=None,
    envvar="CASSANDRA_ANTHROPIC_PRE_UPSTREAM_ACQUIRE_TIMEOUT_SECONDS",
    help=(
        ...
        "Env: CASSANDRA_ANTHROPIC_PRE_UPSTREAM_ACQUIRE_TIMEOUT_SECONDS."
    ),
)
@click.option(
    ...
    type=float,
    default=None,
    envvar="CASSANDRA_ANTHROPIC_PRE_UPSTREAM_MEMORY_CONTEXT_TIMEOUT_SECONDS",
    help=(
        "Fail-open timeout for Anthropic memory-context lookup while the request "
        "still holds a pre-upstream slot. "
        "Default: 2.0 seconds. "
        "Env: CASSANDRA_ANTHROPIC_PRE_UPSTREAM_MEMORY_CONTEXT_TIMEOUT_SECONDS."
    ),
)
@click.option(
    "--log-file",
    default=None,
    envvar="CASSANDRA_LOG_FILE",
    help=(
        ...
        "Disabled in --stateless mode. Env: CASSANDRA_LOG_FILE."
    ),
)
@click.option(
    "--log-messages",
    is_flag=True,
    envvar="CASSANDRA_LOG_MESSAGES",
    help=(
...
@click.option(
    ...
    help=(
        ...
        "~/.cassandra/logs/codex_wire or workspace .cassandra/logs/codex_wire)."
    ),
)
@click.option(
    "--budget",
    ...
    default=None,
    envvar="CASSANDRA_BUDGET",
    help=(
        "Budget limit in USD per --budget-period. Requests are rejected with 429 "
        "once the limit is reached. Env: CASSANDRA_BUDGET."
    ),
)
@click.option(
    "--budget-period",
    ...
    default="daily",
    envvar="CASSANDRA_BUDGET_PERIOD",
    help=(
        ...
        "Env: CASSANDRA_BUDGET_PERIOD."
    ),
)
# Code-aware compression (AST-based, requires `pip install cassandra-ai[code]`).
# Pair of flags so users can override the env-var default in either direction.
# We resolve CASSANDRA_CODE_AWARE_ENABLED in the body (not via Click's envvar=),
...
@click.option(
    ...
    help=(
        ...
        "tree-sitter dependency: pip install cassandra-ai[code]. "
        "Default: disabled. Env: CASSANDRA_CODE_AWARE_ENABLED=1 to enable."
    ),
)
@click.option(
    "--disable-kompress",
    is_flag=True,
    envvar="CASSANDRA_DISABLE_KOMPRESS",
    help=(
        "Disable Kompress ML compression while keeping structural compression enabled. "
        "Env: CASSANDRA_DISABLE_KOMPRESS=1."
    ),
)
@click.option(
    "--disable-kompress-fallback",
    is_flag=True,
    envvar="CASSANDRA_DISABLE_KOMPRESS_FALLBACK",
    help=(
        ...
        "Env: CASSANDRA_DISABLE_KOMPRESS_FALLBACK=1."
    ),
)
@click.option(
    ...
    default=None,
    envvar="CASSANDRA_DISABLE_KOMPRESS_ANTHROPIC",
    help=(
        "Disable (or --enable-) Kompress for the Anthropic pipeline only, overriding "
        "--disable-kompress. Env: CASSANDRA_DISABLE_KOMPRESS_ANTHROPIC=1."
    ),
)
@click.option(
    ...
    default=None,
    envvar="CASSANDRA_DISABLE_KOMPRESS_OPENAI",
    help=(
        "Disable (or --enable-) Kompress for the OpenAI/Codex pipeline only, overriding "
        "--disable-kompress. Env: CASSANDRA_DISABLE_KOMPRESS_OPENAI=1."
    ),
)
...
@click.option(
    "--read-maturation",
    is_flag=True,
    envvar="CASSANDRA_READ_MATURATION",
    help=(
        ...
        "file quiesces (env: CASSANDRA_READ_MATURATION=1)"
    ),
)
@click.option(
    ...
    default=5,
    show_default=True,
    envvar="CASSANDRA_READ_MATURATION_QUIESCE_TURNS",
...
@click.option(
    ...
    default=25,
    show_default=True,
    envvar="CASSANDRA_READ_MATURATION_MAX_HOLD_TURNS",
...
@click.option(
    ...
    default=2048,
    show_default=True,
    envvar="CASSANDRA_READ_MATURATION_MIN_SIZE_BYTES",
...
@click.option(
    ...
    help=(
        ...
        "x-cassandra-user-id and/or x-cassandra-project-id / x-cassandra-cwd request headers."
    ),
)
@click.option(
    "--memory-db-path",
    default="",
    envvar="CASSANDRA_MEMORY_DB_PATH",
    help=(
        "Path to the legacy single-file memory DB (used in --memory-storage=global, "
        "and as the seed for the project-mode storage root). "
        "Default: {cwd}/.cassandra/memory.db. Env: CASSANDRA_MEMORY_DB_PATH."
    ),
)
@click.option(
    "--memory-storage",
    ...
    help=(
        ...
        "no cross-project bleed. user: one DB per x-cassandra-user-id. global: a single "
...
@click.option(
    "--memory-project-root",
    default="",
    envvar="CASSANDRA_MEMORY_PROJECT_ROOT",
    help=(
        ...
        "workspace. Takes effect after the x-cassandra-project-id and x-cassandra-cwd "
        "headers. Env: CASSANDRA_MEMORY_PROJECT_ROOT."
    ),
)
@click.option(
    "--no-memory-tools",
    is_flag=True,
    envvar="CASSANDRA_NO_MEMORY_TOOLS",
    help=(
        "Disable automatic injection of memory_save/memory_search tools into requests. "
        "Env: CASSANDRA_NO_MEMORY_TOOLS."
    ),
)
@click.option(
    "--no-memory-context",
    is_flag=True,
    envvar="CASSANDRA_NO_MEMORY_CONTEXT",
    help=(
        "Disable automatic injection of relevant past memories into the system prompt. "
        "Env: CASSANDRA_NO_MEMORY_CONTEXT."
    ),
)
@click.option(
    "--memory-top-k",
...
@click.option(
    ...
    help=(
        ...
        "Also reads CASSANDRA_QDRANT_URL."
    ),
)
@click.option(
    "--memory-qdrant-host",
    ...
    help=(
        ...
        "(default: localhost, also reads CASSANDRA_QDRANT_HOST)"
    ),
)
@click.option(
    "--memory-qdrant-port",
    ...
    default=None,
    help=(
        "Qdrant port for the qdrant-neo4j backend (default: 6333, also reads CASSANDRA_QDRANT_PORT)"
    ),
)
@click.option(
    "--memory-qdrant-api-key",
    default=None,
    help=("API key for hosted Qdrant (e.g. Qdrant Cloud). Also reads CASSANDRA_QDRANT_API_KEY."),
)
...
@click.option(
    ...
    help=(
        ...
        "cost of slower learning. Default: 5. (env: CASSANDRA_MIN_EVIDENCE)"
    ),
)
...
@click.option(
    "--region",
    default="us-west-2",
    envvar="CASSANDRA_REGION",
    help="Cloud region for Bedrock/Vertex/etc (default: us-west-2). Env: CASSANDRA_REGION.",
)
@click.option(
    "--bedrock-region",
    default=None,
...
@click.option(
    "--telemetry",
    is_flag=True,
    help="Opt in to anonymous usage telemetry — off by default (env: CASSANDRA_TELEMETRY=on)",
)
@click.option(
    "--no-telemetry",
    is_flag=True,
    help="Force anonymous usage telemetry off (already the default; env: CASSANDRA_TELEMETRY=off)",
)
@click.option(
    "--stateless",
    is_flag=True,
...
@click.option(
    ...
    "(env: CASSANDRA_EMBEDDING_SERVER=true)",
)
@click.option(
    "--embedding-server-socket",
    default=None,
    help="Unix socket path for the embedding server sidecar. "
    "Default: /tmp/cassandra-embed-{port}.sock. "
    "(env: CASSANDRA_EMBEDDING_SERVER_SOCKET)",
)
@click.pass_context
def proxy(
    ctx: click.Context,
    ...
    try:
        from cassandra.proxy.server import (
            ProxyConfig,
            _parse_csv_tools,
        ...
        click.secho(
            "Error: Proxy dependencies not installed. Run: pip install cassandra-ai[proxy]",
            fg="red",
            err=True,
        )
    ...
    if intercept_tool_results:
        from cassandra.binaries import ensure_tools
        ...
        if missing:
            ...
            click.secho(
                ...
                f"be installed: {missing}. Run `cassandra tools doctor` to diagnose, "
                "or omit the flag to start the proxy without interceptors.",
                fg="red",
                err=True,
            )
            sys.exit(1)
        os.environ["CASSANDRA_INTERCEPT_ENABLED"] = "1"
    ...
    effective_anyllm_provider = os.environ.get("CASSANDRA_ANYLLM_PROVIDER") or anyllm_provider

    # Resolve mode: CLI flag > env var > default
    effective_mode: str = normalize_proxy_mode(
        mode or os.environ.get("CASSANDRA_MODE") or PROXY_MODE_TOKEN
    )

    # Stateless mode: CLI flag or env var
    is_stateless = stateless or os.environ.get("CASSANDRA_STATELESS", "").lower() in (
        "true",
        "1",
        "yes",
        "on",
    ...
    if telemetry:
        os.environ["CASSANDRA_TELEMETRY"] = "on"
    if no_telemetry:
        os.environ["CASSANDRA_TELEMETRY"] = "off"

    if codex_wire_debug or codex_wire_debug_dir:
        os.environ["CASSANDRA_CODEX_WIRE_DEBUG"] = "1"
        os.environ["CASSANDRA_CODEX_WIRE_DEBUG_DIR"] = codex_wire_debug_dir or str(
            _paths.codex_wire_debug_dir()
        )

    # Stateless mode: suppress TOIN filesystem persistence
    if is_stateless:
        os.environ["CASSANDRA_TOIN_BACKEND"] = "none"

    # License key for managed/enterprise deployments (optional)
    license_key = os.environ.get("CASSANDRA_LICENSE_KEY")
    ...
    # CASSANDRA_QDRANT_* env vars. Explicit CLI values win over env.
    ...
    config = ProxyConfig(
        ...
        compress_user_messages=_get_env_bool("CASSANDRA_COMPRESS_USER_MESSAGES", False),
        min_tokens_to_crush=_get_env_int_optional("CASSANDRA_MIN_TOKENS") or 500,
        max_items_after_crush=_get_env_int_optional("CASSANDRA_MAX_ITEMS") or 50,
        ...
        smart_crusher_with_compaction=_get_env_bool_optional("CASSANDRA_SMART_CRUSHER_COMPACTION"),
        savings_profile=os.environ.get("CASSANDRA_SAVINGS_PROFILE") or None,
        target_ratio=target_ratio,
        compress_system_messages=_get_env_bool_optional("CASSANDRA_COMPRESS_SYSTEM_MESSAGES"),
        protect_recent=_get_env_int_optional("CASSANDRA_PROTECT_RECENT"),
        protect_analysis_context=_get_env_bool_optional("CASSANDRA_PROTECT_ANALYSIS_CONTEXT"),
        ...
        # `--proxy-extension a,b --proxy-extension c` and `CASSANDRA_PROXY_EXTENSIONS=a,b,c`
        ...
        or os.environ.get("CASSANDRA_LOG_MESSAGES", "").lower() in ("true", "1", "yes", "on"),
        ...
        # 2. Otherwise read CASSANDRA_CODE_AWARE_ENABLED (truthy = on).
        ...
        code_aware_enabled=(
            ...
            else os.environ.get("CASSANDRA_CODE_AWARE_ENABLED", "").strip().lower()
            in ("true", "1", "yes", "on")
        ),
        ...
        **qdrant_overrides,
        ...
        traffic_learning_agent_type=os.environ.get("CASSANDRA_AGENT_TYPE", "unknown"),
        ...
        # path. ``None`` -> CassandraProxy computes ``max(2, min(8,
...
"""
        ...
        # LiteLLM backend
        from cassandra.backends.litellm import get_provider_config
...
IMPORTANT for {provider_config.display_name} users:
  ...
  2. Set a dummy Anthropic key: ANTHROPIC_API_KEY="sk-ant-dummy"
     (Cassandra ignores this - it uses your {provider_config.display_name} credentials)
...
Memory (Multi-Provider):
  ...
  - Storage mode: {config.memory_storage_mode} (per-project DB by default — set x-cassandra-project-id / x-cassandra-cwd to override)
...
"""
    ...
    try:
        from cassandra.proxy.extensions import discover as _discover_extensions
    ...
    if not _ext_available:
        ...
        extensions_line = (
            ...
            f"(opt-in: --proxy-extension <name> or CASSANDRA_PROXY_EXTENSIONS=<n>)"
        )
    ...
    _auth_on = bool(config.proxy_token or os.environ.get("CASSANDRA_PROXY_TOKEN"))
    if config.offline:
    ...
    from cassandra.proxy.server import _get_code_aware_banner_status
    ...
    _stable_turn = _get_env_int_optional("CASSANDRA_COMPRESSION_STABLE_AFTER_TURN") or 0
    _stale_turns = _get_env_int_optional("CASSANDRA_STALE_READ_COMPRESS_AFTER_TURNS") or 0
    _embed_socket = os.environ.get("CASSANDRA_EMBEDDING_SERVER_SOCKET") or (
        embedding_server and (embedding_server_socket or f"/tmp/cassandra-embed-{port}.sock")
    )
    _tuning_lines: list[str] = []
    if _stable_turn:
        _tuning_lines.append(
            f"  Prefix stability:        conservative for first {_stable_turn} turns"
            f"  (CASSANDRA_COMPRESSION_STABLE_AFTER_TURN={_stable_turn})"
        )
    if _stale_turns:
        _tuning_lines.append(
            f"  Stale read compression:  reads older than {_stale_turns} turns eligible"
            f"  (CASSANDRA_STALE_READ_COMPRESS_AFTER_TURNS={_stale_turns})"
        )
    if _embed_socket:
    ...
    if _tuning_lines:
        ...
        tuning_section = (
            "\nPerformance Tuning:  (all defaults — set CASSANDRA_COMPRESSION_STABLE_AFTER_TURN"
            " / CASSANDRA_STALE_READ_COMPRESS_AFTER_TURNS to tune)"
        )
...
""")
    ...
    try:
        from cassandra.update_check import format_update_notice
    ...
    if embedding_server:
        _embed_socket = embedding_server_socket or f"/tmp/cassandra-embed-{config.port}.sock"
        # Pass socket path to all worker processes via environment variable
        os.environ["CASSANDRA_EMBEDDING_SERVER_SOCKET"] = _embed_socket
        ...
        async def _start_embed_watchdog() -> Any:
            ...
            from cassandra.memory.adapters.watchdog import EmbeddingServerWatchdog
        ...
        try:
            ...
            os.environ.pop("CASSANDRA_EMBEDDING_SERVER_SOCKET", None)
    ...
    try:
        ...
        # a richer one above. Direct `python -m cassandra.proxy.server` keeps
