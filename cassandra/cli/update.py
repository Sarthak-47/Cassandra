"""`cassandra update` — self-update across the supported install methods.

Detects how Cassandra was installed and runs the matching upgrade command, or
...
from cassandra.update_check import (
    PACKAGE_NAME,
...
class InstallMethod:
    """How Cassandra is installed and how (or whether) to upgrade it."""
...
def _is_source_checkout() -> bool:
    try:
        from cassandra._version import _source_root
...
def _in_docker() -> bool:
    try:
        ...
        return Path("/.dockerenv").exists() or bool(
            os.environ.get("CASSANDRA_IN_DOCKER", "").strip()
        )
    except Exception:
        return False
...
def _package_location() -> str | None:
    """Normalized base directory the cassandra-ai distribution is installed in."""
    try:
        from importlib.metadata import distribution
...
def _managed_env_guidance() -> str:
    if sys.platform == "darwin":
        hint = "`brew upgrade cassandra-ai` (if installed via Homebrew), or reinstall with pipx"
    elif sys.platform.startswith("win"):
        hint = "reinstall with pipx (`pipx install cassandra-ai`) or use a virtualenv"
    else:
        hint = "use your distro package manager, or reinstall with pipx / a virtualenv"
    return (
        "Cassandra is installed in an externally-managed system Python (PEP 668). "
...
def detect_install_method(extras: str | None = None) -> InstallMethod:
    ...
    if _in_docker():
        return InstallMethod(
            ...
            can_self_update=False,
            guidance=(
                "Running inside a container — pull a newer Cassandra image instead of self-updating."
            ),
        )
...
def update(check_only: bool, assume_yes: bool, allow_pre: bool, extras: str | None) -> None:
    """Update Cassandra to the latest release.
    ...
    click.echo("Checking PyPI for the latest Cassandra release...")
    ...
    if current:
        ...
        try:
            if Version(latest) <= Version(current):
                click.echo(f"Cassandra is up to date ({current}).")
                return
        except InvalidVersion:
            pass
        click.echo(f"Update available: {current} → {latest}")
    else:
        click.echo(f"Latest Cassandra release: {latest}")
    ...
    click.echo(f"Cassandra upgraded to {latest}.")
    click.echo("Restart any running `cassandra proxy` to pick up the new version.")
