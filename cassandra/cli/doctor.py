"""`cassandra doctor` — diagnose whether the local Cassandra setup is working.

Cassandra's failure mode is silent: when a client is not routed through the
...
from cassandra.install.health import probe_json
from cassandra.install.paths import claude_settings_path, codex_config_path
from cassandra.install.state import list_manifests
from cassandra.paths import savings_path
...
def check_proxy_liveness(livez: dict[str, Any] | None, base_url: str) -> CheckResult:
    ...
    if livez is None:
        return CheckResult(
            ...
            hint="start it with: cassandra proxy",
        )
...
def check_version_drift(livez: dict[str, Any] | None, installed: str) -> CheckResult:
    ...
    if running != installed:
        return CheckResult(
            ...
            hint="restart the proxy to pick up new code: cassandra proxy",
        )
...
def check_claude_routing(settings_path: Path, port: int) -> CheckResult:
    ...
    if not settings_path.exists():
        return CheckResult(
            ...
            hint="wrap it: cassandra wrap claude",
        )
    try:
...
def check_codex_routing(config_path: Path, port: int) -> CheckResult:
    ...
    Detection keys on the ``[model_providers.cassandra]`` section, which both
    ...
    if not config_path.exists():
        return CheckResult(
            ...
            hint="wrap it: cassandra wrap codex",
        )
    try:
        text = config_path.read_text(encoding="utf-8", errors="replace")
    except OSError as exc:
        return CheckResult(name=name, status=WARN, summary=f"could not read {config_path}: {exc}")
    if "[model_providers.cassandra]" not in text:
        return CheckResult(
            name=name,
            status=WARN,
            summary="not routed (no Cassandra provider in config.toml)",
            hint="wrap it: cassandra wrap codex",
        )
    ...
    if match and int(match.group(1)) != port:
        return CheckResult(
            ...
            hint=f"re-run with: cassandra doctor --port {match.group(1)}",
        )
...
def check_shell_env(environ: Mapping[str, str], port: int) -> CheckResult:
    ...
    return CheckResult(
        ...
        hint=f"export ANTHROPIC_BASE_URL=http://127.0.0.1:{port} (or launch via cassandra wrap)",
    )
...
def _classify_routing_url(name: str, url: str, port: int, *, source: str) -> CheckResult:
    ...
    if match is None:
        return CheckResult(
            name=name,
            status=WARN,
            summary=f"points at {url}, not the local Cassandra proxy ({source})",
        )
    ...
    if found_port != port:
        return CheckResult(
            ...
            hint=f"re-run with: cassandra doctor --port {found_port}",
        )
...
def check_budget(stats: dict[str, Any] | None) -> CheckResult:
    ...
    if limit is None:
        return CheckResult(
            ...
            hint="set one: cassandra proxy --budget 10 (env: CASSANDRA_BUDGET)",
        )
...
def check_deployments(manifests: list[Any], probe: Any = probe_json) -> CheckResult | None:
    ...
    if down:
        return CheckResult(
            ...
            hint="inspect with: cassandra install status --profile <name>",
        )
    return CheckResult(
        name="deployments",
        status=PASS,
...
def _render(checks: list[CheckResult], port: int, installed: str) -> None:
    ...
    console = Console()
    console.print(f"[bold]Cassandra Doctor[/bold] [dim]v{installed} · port {port}[/dim]\n")
...
@click.option(
    ...
    help="Proxy port to check (default: 8787, env: CASSANDRA_PORT)",
)
...
def doctor(port: int, emit_json: bool) -> None:
    """Check that the Cassandra proxy and client routing are working.
