"""CLI commands for Cassandra Learn — offline failure learning."""
...
External plugins register via 'cassandra.learn_plugin' entry point.
...
def learn(
    ...
    be installed via pip (entry point: cassandra.learn_plugin).

    \b
    Examples:
        cassandra learn                        # Auto-detect agent & model
        cassandra learn --apply                # Write recommendations
        cassandra learn --model gpt-4o         # Use GPT-4o for analysis
        cassandra learn --all                  # Analyze all projects
        cassandra learn --agent codex --all    # Analyze all Codex sessions
        cassandra learn --target CLAUDE.md     # Write to the team-shared file
    """
    import os
    ...
    for agent_name, plugin in agent_configs:
        ...
        if analyze_all:
            ...
            if not targets and len(agent_configs) == 1:
                ...
                click.echo("Try: cassandra learn --all  or  cassandra learn --project <path>")
...
def _activate_output_shaper(port: int | None = None) -> tuple[str, int]:
    ...
    ``CASSANDRA_OUTPUT_SHAPER`` is enabled in the proxy that serves traffic.
    ...
    resolved_port = port if port is not None else int(_os.environ.get("CASSANDRA_PORT", "8787"))
    request = urllib.request.Request(
        f"http://127.0.0.1:{resolved_port}/admin/runtime-env",
        data=_json.dumps({"CASSANDRA_OUTPUT_SHAPER": "1"}).encode("utf-8"),
        method="POST",
        ...
        if apply:
            ...
            if status == "live":
                click.echo(
                    ...
                    f"level {profile.level} is live now (while CASSANDRA_VERBOSITY_LEVEL is unset)."
                )
                click.echo(
                    "    To keep it on across restarts: export CASSANDRA_OUTPUT_SHAPER=1 "
                    "before `cassandra wrap ...` (wrap pushes it to the proxy)."
                )
            else:
                click.echo(
                ...
                click.echo(
                    "    Enable it: export CASSANDRA_OUTPUT_SHAPER=1 then `cassandra wrap ...` "
                    "(or start `cassandra proxy` with it set). The learned level is then used "
                    "automatically while CASSANDRA_VERBOSITY_LEVEL is unset."
                )
        else:
