...
Reads the append-only savings ledger (``~/.cassandra/savings_events.jsonl``,
...
def savings(as_json: bool, days: int, reset: bool) -> None:
    ...
    if calls == 0:
        ...
        click.echo(
            "Compress via the Cassandra MCP tool or route traffic through the "
            "proxy, then re-run `cassandra savings`."
        )
