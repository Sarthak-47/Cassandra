"""Wrap CLI commands to run through Cassandra proxy.

Usage:
    cassandra wrap claude                    # Start proxy + context tool + claude
    cassandra wrap copilot -- --model ...    # Start proxy + launch GitHub Copilot CLI
    cassandra wrap codex                     # Start proxy + OpenAI Codex CLI
    cassandra wrap aider                     # Start proxy + aider
    cassandra wrap vibe                      # Start proxy + Mistral Vibe
    cassandra wrap cursor                    # Start proxy + print Cursor config instructions
    cassandra wrap openclaw                  # Install + configure OpenClaw plugin
    cassandra wrap claude --no-context-tool  # Without CLI context-tool setup
    cassandra wrap claude --port 9999        # Custom proxy port
    cassandra wrap claude -- --model opus    # Pass args to claude
"""
...
from cassandra._subprocess import run
...
from cassandra._version import __version__ as _CASSANDRA_VERSION
from cassandra.agent_savings import (
    apply_agent_savings_env_defaults,
)
from cassandra.copilot_auth import (
    has_oauth_auth,
    resolve_client_bearer_token,
    resolve_copilot_api_url,
    resolve_subscription_bearer_token_details,
)
from cassandra.providers.aider import build_launch_env as _build_aider_launch_env
from cassandra.providers.claude import (
    TOOL_SEARCH_DEFAULT,
    TOOL_SEARCH_ENV,
)
from cassandra.providers.claude import (
...
from cassandra.providers.codex import build_launch_env as _build_codex_launch_env
from cassandra.providers.codex.install import codex_uses_chatgpt_auth
from cassandra.providers.codex.threads import retag_to_cassandra, retag_to_native
from cassandra.providers.copilot import (
...
from cassandra.providers.cursor import render_setup_lines as _render_cursor_setup_lines
from cassandra.providers.mistral_vibe import build_launch_env as _build_mistral_vibe_launch_env
from cassandra.providers.openclaw import (
...
from cassandra.providers.opencode import build_launch_env as _build_opencode_launch_env
from cassandra.providers.opencode.config import (
    ...
    strip_opencode_cassandra_blocks,
)
from cassandra.proxy.project_context import with_project_prefix as _with_project_prefix
...
_CONTEXT_TOOL_ENV = "CASSANDRA_CONTEXT_TOOL"
...
_WRAP_PROXY_TIMEOUT_ENV = "CASSANDRA_WRAP_PROXY_TIMEOUT"
...
def _selected_context_tool() -> str:
    ...
    ``CASSANDRA_CONTEXT_TOOL=lean-ctx`` to let lean-ctx configure the supported
...
def _wrap_agent_savings_profile(agent_type: str) -> str | None:
    ...
    if agent_type not in _AGENT_SAVINGS_WRAP_AGENTS:
        return None
    return os.environ.get("CASSANDRA_SAVINGS_PROFILE") or None
...
def _print_telemetry_notice() -> None:
    ...
    Respects the CASSANDRA_TELEMETRY and CASSANDRA_TELEMETRY_WARN feature flags.
    Does nothing when telemetry or warnings are disabled.
    """
    from cassandra.telemetry.beacon import format_telemetry_notice
...
def _check_proxy(port: int) -> bool:
    """Check if Cassandra proxy is running on given port."""
    try:
...
def _format_unbindable_port_error(port: int, error: OSError, agent_type: str) -> str:
    ...
    command = "cassandra proxy"
    if agent_type != "unknown":
        command = f"cassandra wrap {agent_type}"
...
def _get_log_path() -> Path:
    ...
    from cassandra import paths as _paths
...
def _start_proxy(
    ...
    *,
    ...
    """Start Cassandra proxy as a background subprocess.

    Stdout and stderr are written to a dedicated sibling file, usually
    `~/.cassandra/logs/proxy-stdio.log`, to avoid pipe deadlock risk without
    competing with the rotating `proxy.log` runtime log.
    """
    cmd = [sys.executable, "-m", "cassandra.cli", "proxy", "--port", str(port)]

    # Forward CASSANDRA_MODE env var so the proxy respects the user's mode choice
    cassandra_mode = os.environ.get("CASSANDRA_MODE")
    if cassandra_mode:
        cmd.extend(["--mode", cassandra_mode])
    ...
    _backend = backend or os.environ.get("CASSANDRA_BACKEND")
    if _backend:
        cmd.extend(["--backend", _backend])

    _anyllm = anyllm_provider or os.environ.get("CASSANDRA_ANYLLM_PROVIDER")
    if _anyllm:
        cmd.extend(["--anyllm-provider", _anyllm])

    _region = region or os.environ.get("CASSANDRA_REGION")
    if _region:
    ...
    if agent_type != "unknown":
        proxy_env["CASSANDRA_AGENT_TYPE"] = agent_type
        proxy_env.setdefault("CASSANDRA_STACK", f"wrap_{agent_type}")
...
def _setup_rtk(verbose: bool = False) -> Path | None:
    ...
    from cassandra.rtk import get_rtk_path
    from cassandra.rtk.installer import ensure_rtk, register_claude_hooks
...
def _patch_rtk_hook_absolute_path(rtk_path: Path, hook_script_path: Path | None = None) -> bool:
    ...
    to use the absolute path to the RTK binary Cassandra manages.

    ``rtk init --global --auto-patch`` writes ``~/.claude/hooks/rtk-rewrite.sh``
    with a bare ``rtk`` command that depends on PATH lookup. Since
    ``~/.cassandra/bin`` (where Cassandra installs rtk) is not automatically
    ...
    # "/Users/Alice Smith/.cassandra/bin/rtk") must be quoted, or the
...
def _setup_lean_ctx_agent(agent: str, verbose: bool = False) -> Path | None:
    ...
    from cassandra.lean_ctx import get_lean_ctx_path
    from cassandra.lean_ctx.installer import ensure_lean_ctx
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="cassandra-lean-ctx-") as setup_cwd:
...
# Hook-command markers Cassandra manages in Claude settings.json. unwrap drops
# any hook entry whose command contains one of these.
_CASSANDRA_HOOK_MARKERS = ("rtk-rewrite", "cassandra-init-claude")

# Env vars Cassandra's init/wrap inject into Claude settings.json; unwrap removes
# them. ENABLE_TOOL_SEARCH keeps Claude Code's tool deferral on behind the proxy
# (GH #746), paired with init/wrap setting it.
_CASSANDRA_ENV_KEYS = ("ANTHROPIC_BASE_URL", "ENABLE_TOOL_SEARCH")


def _remove_claude_rtk_hooks(settings_path: Path | None = None) -> bool:
    """Remove Cassandra-managed entries from Claude settings.json.

    Reverses what ``cassandra init claude`` and ``rtk init --auto-patch`` add:
      * PreToolUse / SessionStart hooks whose command contains a Cassandra marker
        (``rtk-rewrite`` or ``cassandra-init-claude``), and
                ...
                retained_hooks = [
                    ...
                    if not (
                        ...
                        and any(
                            ...
                            for marker in _CASSANDRA_HOOK_MARKERS
                        )
                    )
                ]
    ...
    if isinstance(env, dict):
        removed_keys = [k for k in _CASSANDRA_ENV_KEYS if env.pop(k, None) is not None]
        if removed_keys:
            changed = True
            if env:
                payload["env"] = env
...
def _write_claude_wrap_base_url(
    ...
    *,
    ...
    routes through the Cassandra proxy without touching the global user settings
...
def _setup_cassandra_mcp(
    registrar: Any, port: int, *, verbose: bool = False, force: bool = False
) -> None:
    """Register the cassandra MCP server with the given agent (idempotent).
    ...
    nothing — the agent has no ``cassandra_retrieve`` tool to call.
    ...
    """
    from cassandra.mcp_registry import build_cassandra_spec, format_result
    ...
    spec = build_cassandra_spec(proxy_url)
    ...
    line = format_result(
        ...
        verbose=verbose,
        overwrite_hint=f"cassandra mcp install --proxy-url {proxy_url} --force",
...
def _setup_serena_mcp(
    ...
    A prior ``cassandra wrap`` may have persisted a Serena entry built from an
    ...
    config is one Cassandra installed, force-update it to the current spec. A
    user-managed Serena (absent from our ledger) is left untouched and the
    mismatch is reported as before.
    """
    from cassandra.mcp_registry import build_serena_spec, format_result
    from cassandra.mcp_registry.base import RegisterStatus
    from cassandra.mcp_registry.ledger import cassandra_installed_matching, record_install
    ...
    # Migrate a stale Cassandra-installed entry. register_server won't overwrite
    # a differing spec without force, so an older Cassandra Serena entry would
    # otherwise persist across re-wraps. Force-update it only when the ledger
    # proves Cassandra installed the entry that's currently on disk — never a
    # user-managed Serena.
    if (
        result.status == RegisterStatus.MISMATCH
        and not force
        and cassandra_installed_matching(registrar.name, registrar.get_server("serena"))
    ):
    ...
    line = format_result(
        ...
        label="Serena MCP",
        verbose=verbose,
        overwrite_hint="update or remove the existing serena MCP entry, then rerun cassandra wrap",
        restart_hint=f"restart {registrar.display_name} if it was already running",
    )
    if line is not None:
        click.echo(line)
...
def _disable_serena_mcp(
    ...
    """Actively disable a Cassandra-installed Serena entry, not merely skip it.

    Serena used to be registered by default, so a prior ``cassandra wrap``
    ...
    run leaves that stale entry in place — so this removes the entry Cassandra
    ...
    status = _remove_cassandra_installed_serena_mcp(registrar)
    if status == "removed":
    ...
    elif status == "not_cassandra_owned":
        click.echo(
            "  Serena MCP is present but user-managed — leaving it in place "
            "(--no-serena only removes entries Cassandra installed)"
        )
    else:  # "failed"
        click.echo(
...
def _ensure_tokensave_binary(verbose: bool = False) -> Path | None:
    ...
    """
    from cassandra.graph.tokensave_installer import ensure_tokensave, get_tokensave_path
...
def _setup_tokensave_mcp(registrar: Any, *, verbose: bool = False, force: bool = False) -> bool:
    ...
    Cassandra-installed ``tokensave`` entry is force-updated to the current
    spec, while a user-managed entry is left untouched.
    """
    from cassandra.mcp_registry import build_tokensave_spec, format_result
    from cassandra.mcp_registry.base import RegisterStatus
    from cassandra.mcp_registry.ledger import cassandra_installed_matching, record_install
    ...
    # Migrate a stale Cassandra-installed entry (e.g. an older binary path or
    # pinned version), mirroring the Serena migration path. Only force-update
    # when the ledger proves Cassandra installed the entry on disk.
    if (
        result.status == RegisterStatus.MISMATCH
        and not force
        and cassandra_installed_matching(registrar.name, registrar.get_server("tokensave"))
    ):
        result = registrar.register_server(spec, force=True)
        if result.status == RegisterStatus.REGISTERED:
    ...
    line = format_result(
        ...
        label="tokensave MCP",
        verbose=verbose,
        overwrite_hint="update or remove the existing tokensave MCP entry, then rerun cassandra wrap",
        restart_hint=f"restart {registrar.display_name} if it was already running",
    )
    if line is not None:
        click.echo(line)
...
def _disable_tokensave_mcp(registrar: Any, *, verbose: bool = False) -> None:
    """Make ``--no-tokensave`` actively remove a Cassandra-installed tokensave entry."""
    if not registrar.detect():
        if verbose:
            click.echo(f"  tokensave MCP: {registrar.display_name} not detected — skipping")
        return
    ...
    status = _remove_cassandra_installed_tokensave_mcp(registrar)
    if status == "removed":
        ...
        click.echo(
            ...
            "(--no-tokensave only removes entries Cassandra installed)"
        )
    else:  # "failed"
        click.echo(
...
def _setup_coding_compressor(registrar: Any, *, serena_context: str, **kwargs: Any) -> None:
    ...
    * tokensave is set up by default; on success it becomes the primary
      compressor and any Cassandra-installed Serena entry is removed.
...
def _setup_code_graph(verbose: bool = False) -> bool:
    ...
    tokensave is Cassandra's primary code-graph compressor and is normally
    ...
    server is no longer installed, and ``cassandra unwrap`` still cleans up any
    legacy ``codebase-memory-mcp`` entry a prior wrap left behind.
    """
    from cassandra.mcp_registry import ClaudeRegistrar
...
_RTK_MARKER = "<!-- cassandra:rtk-instructions -->"

# Memory MCP markers
_MEMORY_MCP_MARKER = "# --- Cassandra memory MCP (auto-injected) ---"
_MEMORY_MCP_END = "# --- end Cassandra memory ---"
_MEMORY_AGENTS_MARKER = "<!-- cassandra:memory-instructions -->"

# Codex config injection markers
_CODEX_TOP_LEVEL_MARKER = "# --- Cassandra proxy (auto-injected by cassandra wrap codex) ---"
_CODEX_END_MARKER = "# --- end Cassandra ---"
_CODEX_MCP_MARKER = "# --- Cassandra MCP server ---"
_CODEX_MCP_END = "# --- end Cassandra MCP server ---"
# File name used for the pre-wrap snapshot of the Codex config file.  The
# snapshot lets `cassandra unwrap codex` restore the exact prior state, even
# if the user had their own `model_provider` / `[model_providers.*]` config
# before running wrap.
_CODEX_CONFIG_BACKUP_SUFFIX = ".cassandra-backup"
...
def _strip_codex_cassandra_blocks(
    content: str,
    *,
    remove_mcp: bool = False,
    remove_named_mcp: bool = True,
) -> str:
    """Remove all Cassandra-managed blocks from a Codex ``config.toml`` string.
    ...
    if remove_mcp:
        # Remove Cassandra-managed MCP blocks written by `wrap codex`.
        content = _remove_marker_span(content, _CODEX_MCP_MARKER, _CODEX_MCP_END)
        if remove_named_mcp:
            content = re.sub(
                r"(?ms)^# --- Cassandra MCP server: [^\n]+ ---\n.*?"
                r"^# --- end Cassandra MCP server: [^\n]+ ---\n?",
                "",
                content,
            )
    ...
    content = re.sub(r'(?m)^[ \t]*model_provider[ \t]*=[ \t]*"cassandra"[ \t]*\r?\n', "", content)
    content = re.sub(
    ...
    # Strip any orphaned `[model_providers.cassandra]` table with the fields we
    # write.  We only remove it if the table is recognisably ours (base_url
    # mentions localhost and a Cassandra proxy port).  This protects users who
    # happen to have a differently configured `cassandra` provider.
    orphan_cassandra_table = re.compile(
        r"(?ms)^\[model_providers\.cassandra\][^\[]*?"
        r'base_url[ \t]*=[ \t]*"http://127\.0\.0\.1:\d+/v1"[^\[]*?'
        r"(?=^\[|\Z)"
    )
    content = orphan_cassandra_table.sub("", content)

    return content.lstrip("\n").rstrip() + "\n" if content.strip() else ""


# Top-level bare keys we redirect to cassandra values when the user already
...
def _redirect_existing_top_level_keys(content: str, port: int) -> str:
    ...
    which would break ``codex`` startup after ``cassandra wrap codex`` runs
    ...
    the value with the cassandra one and append ``# was: <original-value>``
    ...
    pre-wrap file can be restored byte-for-byte on ``cassandra unwrap
    codex``.
...
def _codex_config_has_cassandra_markers(content: str) -> bool:
...
def _snapshot_codex_config_if_unwrapped(config_file: Path, backup_file: Path) -> None:
    ...
    Called as the first step of every Cassandra injection into Codex's
    ``config.toml``.  Guarantees that ``cassandra unwrap codex`` can restore the
    ...
    * If the config already contains any Cassandra-managed Codex marker, a wrap
    ...
    try:
        ...
        return
    if _codex_config_has_cassandra_markers(content):
        return
...
def _ensure_rtk_binary(verbose: bool = False) -> Path | None:
    ...
    from cassandra.rtk import get_rtk_path
    from cassandra.rtk.installer import ensure_rtk
...
# case-insensitively by cassandra.proxy.project_context.PROJECT_HEADER).
_PROJECT_HEADER_NAME = "X-Cassandra-Project"


def _project_name_from_cwd() -> str | None:
    """Project label for X-Cassandra-Project: basename of the launch directory.
...
def _apply_project_header_env(env: dict[str, str]) -> None:
    """Inject X-Cassandra-Project into ``ANTHROPIC_CUSTOM_HEADERS``.
    ...
    Cassandra proxy uses the X-Cassandra-Project header for per-project
    savings attribution.  An existing user-supplied x-cassandra-project
...
def _inject_codex_provider_config(port: int) -> None:
    """Inject a Cassandra model provider into Codex's config.toml.

    Two keys need to be in effect for the proxy to route all traffic:

    * ``model_provider = "cassandra"`` — selects the custom provider for
    ...
    * ``openai_base_url = "http://127.0.0.1:{port}/v1"`` — overrides the
    ...
    in place to the cassandra values (with the previous value kept in a
    ...
    ``config.toml.cassandra-backup`` so ``cassandra unwrap codex``
    ...
    provider_section = (
        ...
        "[model_providers.cassandra]\n"
        'name = "OpenAI via Cassandra proxy"\n'
        f'base_url = "http://127.0.0.1:{port}/v1"\n'
        f"supports_websockets = true\n"
        f"{requires_openai_auth}"
        # Per-project savings: Codex sends the header only when the mapped
        # env var (CASSANDRA_PROJECT, set by `cassandra wrap codex`) exists at
        # Codex runtime.  Inline table keeps the key inside this section so
        # _strip_codex_cassandra_blocks removes it with the rest of the block.
        f'env_http_headers = {{ "{_PROJECT_HEADER_NAME}" = "CASSANDRA_PROJECT" }}\n'
        f"{_CODEX_END_MARKER}\n"
    )

    # The two redirectable keys and their cassandra target values.
    _REDIRECT_TARGETS = {
        "model_provider": "cassandra",
    ...
    try:
        ...
        if config_file.exists():
            content = _read_text(config_file)
            # Remove any prior Cassandra-managed blocks before re-injecting so
            # the operation is idempotent and supports port changes.
            content = _strip_codex_cassandra_blocks(content)
            ...
            # to the cassandra values; for keys the user has not declared,
        ...
        click.echo(f"  Codex config: injected Cassandra provider (WS + HTTP) into {config_file}")
        # Pull existing native threads into the cassandra-provider menu so Codex's
        # history list stays whole once it routes through Cassandra. Best-effort.
        retag_to_cassandra(_codex_home_dir())
    except Exception as e:
...
def _restore_codex_provider_config() -> tuple[str, Path]:
    ...
    * ``"cleaned"``  — no backup existed, but the Cassandra-managed block was
      found and stripped out (preserving surrounding user content).
    * ``"removed"``  — the config file only contained Cassandra-managed
      content (created by wrap) and has been deleted.
    * ``"noop"``     — nothing to undo; no Cassandra marker and no backup.
    """
    config_file, backup_file = _codex_config_paths()
    ...
    if config_file.exists():
        ...
        if _codex_config_has_cassandra_markers(original):
            ...
            cleaned = _strip_codex_cassandra_blocks(
                original,
                remove_mcp=True,
                remove_named_mcp=remove_named_mcp,
            )
            if not cleaned.strip():
                # Nothing left but Cassandra content — remove the file entirely
...
def _emit_wrap_interrupted(agent: str, marker_path: Path | None) -> None:
    ...
    re-running the same ``cassandra wrap <agent>`` command is idempotent and
    safe.
    """
    if marker_path is not None:
        click.echo(
            f"\n  Wrap was interrupted; marker file at {marker_path} is on "
            f"disk. Rerun `cassandra wrap {agent}` to retry — it's idempotent."
        )
    else:
        click.echo(
            f"\n  Wrap was interrupted before any on-disk changes. Rerun "
            f"`cassandra wrap {agent}` to retry — it's idempotent."
        )
...
def _print_wrap_banner(agent: str) -> None:
    """Print a centered ``CASSANDRA WRAP: <AGENT>`` banner.
    ...
    """
    title = f"CASSANDRA WRAP: {agent.upper()}"
...
def _remove_rtk_instructions(file_path: Path) -> bool:
    """Remove Cassandra's marker-fenced rtk guidance from an instruction file."""
    if not file_path.exists():
        return False

    content = _read_text(file_path)
    end_marker = "<!-- /cassandra:rtk-instructions -->"
...
def _inject_memory_mcp_config(user_id: str) -> None:
    """Register cassandra memory as an MCP server in Codex's config.toml.
    ...
    mcp_section = (
        f"\n{_MEMORY_MCP_MARKER}\n"
        f"[mcp_servers.cassandra_memory]\n"
        f'command = "{python_bin}"\n'
        f'args = ["-m", "cassandra.memory.mcp_server", "--user", "{user_id}"]\n'
...
def _inject_memory_agents_md(file_path: Path) -> bool:
    ...
    memory_block = (
        ...
        "## Memory\n\n"
        "Use the `cassandra_memory` MCP server for persistent cross-session knowledge.\n\n"
...
def _inject_continue_rtk_systemmessage(config_file: Path, verbose: bool = False) -> bool:
    ...
    * each ``models[i].systemMessage`` where ``models[i]`` is a dict
    ...
    The RTK marker (``<!-- cassandra:rtk-instructions -->``) is the idempotency
...
def _proxy_health_config(payload: dict[str, Any] | None) -> dict[str, Any] | None:
    """Extract the config block from a Cassandra /health payload."""
    if payload is None:
        return None
...
def _agent_savings_config_mismatches(
    ...
    checks: tuple[tuple[str, str, str, str], ...] = (
        ("CASSANDRA_SAVINGS_PROFILE", "savings_profile", "savings-profile", "str"),
        ("CASSANDRA_TARGET_RATIO", "target_ratio", "target-ratio", "float"),
        (
            "CASSANDRA_COMPRESS_USER_MESSAGES",
            "compress_user_messages",
            "compress-user-messages",
            "bool",
        ),
        (
            "CASSANDRA_COMPRESS_SYSTEM_MESSAGES",
            "compress_system_messages",
            "compress-system-messages",
            "bool",
        ),
        ("CASSANDRA_PROTECT_RECENT", "protect_recent", "protect-recent", "int"),
        (
            "CASSANDRA_PROTECT_ANALYSIS_CONTEXT",
            "protect_analysis_context",
            "protect-analysis-context",
            "bool",
        ),
        ("CASSANDRA_MIN_TOKENS", "min_tokens_to_crush", "min-tokens", "int"),
        ("CASSANDRA_MAX_ITEMS", "max_items_after_crush", "max-items", "int"),
        (
            "CASSANDRA_SMART_CRUSHER_COMPACTION",
            "smart_crusher_with_compaction",
            "smart-crusher-compaction",
            "bool",
        ),
        ("CASSANDRA_ACCURACY_GUARD", "accuracy_guard", "accuracy-guard", "str"),
    )
...
def _proxy_needs_version_restart(payload: dict[str, Any] | None) -> bool:
    """Return True when a running Cassandra proxy uses a different package version."""
    running_version = _proxy_version(payload)
    return (
        running_version is not None
        and running_version != "unknown"
        and _CASSANDRA_VERSION != "unknown"
        and running_version != _CASSANDRA_VERSION
    )
...
def _stop_local_proxy_for_unwrap(port: int) -> str:
    """Stop a local Cassandra proxy for durable unwrap commands.

    Returns a status string:
      * ``"stopped"``: a Cassandra proxy was identified and stopped.
      * ``"not_running"``: nothing is listening on the requested port.
      * ``"unidentified"``: something is listening, but it did not expose
        Cassandra's health/config payload, so we did not kill it.
      * ``"no_pid"``: the service looked like Cassandra but did not expose a PID.
...
def _echo_unwrap_proxy_stop_status(status: str, port: int) -> None:
    ...
    if status == "stopped":
        click.echo(f"  Stopped local Cassandra proxy on port {port}.")
    elif status == "not_running":
        click.echo(f"  No local Cassandra proxy detected on port {port}.")
    elif status == "unidentified":
        click.echo(
            f"  Warning: port {port} is in use, but it did not look like Cassandra; left it running."
        )
    elif status == "no_pid":
        click.echo(
            f"  Warning: Cassandra proxy on port {port} did not expose a PID; left it running."
        )
    else:
        click.echo(f"  Warning: failed to stop Cassandra proxy on port {port}; stop it manually.")


def _find_persistent_manifest(port: int) -> Any:
    """Return a matching persistent deployment manifest for the requested port."""
    from cassandra.install.state import list_manifests
...
def _recover_persistent_proxy(port: int) -> bool:
    ...
    from cassandra.install.health import probe_ready
    from cassandra.install.models import InstallPreset, SupervisorKind
    from cassandra.install.runtime import start_detached_agent, start_persistent_docker, wait_ready
    from cassandra.install.supervisors import start_supervisor
...
def _restart_persistent_proxy(manifest: Any, port: int) -> bool:
    ...
    click.echo(
        ...
        f"with Cassandra {_CASSANDRA_VERSION}..."
    )
    try:
        if manifest.preset == InstallPreset.PERSISTENT_DOCKER.value:
...
def _should_use_copilot_oauth(
    *,
    ...
    effective_backend = backend or os.environ.get("CASSANDRA_BACKEND")
...
def _push_runtime_env(port: int, no_proxy: bool) -> None:
    ...
    if no_proxy:
        return
    from cassandra.proxy import runtime_env as _rt
    ...
    if not no_proxy:
            ...
            if probe_ready(manifest.health_url):
                ...
                if helpers._proxy_needs_version_restart(health_payload):
                    ...
                    if active_sessions > 0 or other_wrappers:
                        ...
                        click.echo(
                            f"  Proxy on port {port} is running Cassandra {running_version}; "
                            f"current CLI is {_CASSANDRA_VERSION}."
                        )
                        click.echo(
                    ...
                    raise click.ClickException(
                        ...
                        f"is running stale Cassandra {running_version} and could not be restarted."
                    )
        ...
        if helpers._check_proxy(port):
            ...
            if helpers._proxy_needs_version_restart(health_payload):
                ...
                click.echo(
                    f"  Proxy on port {port} is running Cassandra {running_version}; "
                    f"restarting with {_CASSANDRA_VERSION}..."
                )
        ...
        click.echo(f"  Starting Cassandra proxy on port {port}...")
        try:
            proc = cast(
...
def _make_cleanup(proxy_proc_holder: list, port: int = 8787) -> Any:
    ...
    Only kills the proxy when no other live cassandra-wrapped clients remain,
...
def _launch_tool(
    ...
    *,
    ...
    try:
        click.echo()
        padded = f"CASSANDRA WRAP: {tool_label}".center(47)
        click.echo("  ╔═══════════════════════════════════════════════╗")
        click.echo(f"  ║{padded}║")
        click.echo("  ╚═══════════════════════════════════════════════╝")
        click.echo()
        ...
        click.echo()
        click.echo(f"  Launching {tool_label} (API routed through Cassandra)...")
...
def _build_openclaw_plugin_entry(
    *,
    ...
    """Merge managed Cassandra plugin settings with any existing entry payload."""
...
def _write_openclaw_plugin_entry(openclaw_bin: str, entry: dict[str, object]) -> None:
    """Persist the Cassandra plugin config entry."""
    _run_checked(
        [
            openclaw_bin,
            "config",
            "set",
            "plugins.entries.cassandra",
            json.dumps(entry, separators=(",", ":")),
            "--strict-json",
        ],
        action="openclaw config set plugins.entries.cassandra",
    )
...
def _copy_openclaw_plugin_into_extensions(
    *,
    ...
    target_dir = extensions_dir / "cassandra"
...
def wrap() -> None:
    ...
    \b
    Starts a Cassandra proxy, configures the environment, and launches
    the target tool so all API calls route through Cassandra automatically.

    \b
    Supported tools (one Click subcommand per tool):
        cassandra wrap claude              # Claude Code (Anthropic)
        cassandra wrap codex               # OpenAI Codex CLI
        cassandra wrap copilot -- --model claude-sonnet-4-20250514
        cassandra wrap aider               # Aider
        cassandra wrap vibe                # Mistral Vibe
        cassandra wrap cursor              # Cursor (prints config instructions)
        cassandra wrap cline               # Cline (VS Code; prints config instructions)
        cassandra wrap continue            # Continue (VS Code/JetBrains; injects systemMessage)
        cassandra wrap goose               # Goose (Block) CLI
        cassandra wrap openhands           # OpenHands CLI
        cassandra wrap openclaw            # OpenClaw plugin bootstrap
        cassandra wrap opencode            # OpenCode CLI

    \b
    `wrap` vs `proxy`:
        - `cassandra wrap <tool>` — convenience: starts the proxy for you,
          sets the right env vars, and launches the wrapped CLI.
        - `cassandra proxy` — just the proxy. Use this with any
...
@click.option(
    "--no-mcp",
    is_flag=True,
    help="Skip cassandra MCP server registration (compression markers will be unactionable)",
)
@click.option(
    "--no-tokensave",
    is_flag=True,
...
@click.option(
    ...
    "(env: CASSANDRA_BACKEND). For Vertex, prefer CLAUDE_CODE_USE_VERTEX=1 (native, "
    "keeps your GCP auth) over a litellm backend.",
)
@click.option(
    "--region",
    default=None,
    help="Cloud region for Vertex/Bedrock backends (env: CASSANDRA_REGION).",
)
@click.option(
    "--1m",
    "context_1m",
...
def claude(
    ...
    """Launch Claude Code through Cassandra proxy.

    \b
    Sets ANTHROPIC_BASE_URL to route all Anthropic API calls through Cassandra.
    All unknown flags are passed through to claude (e.g. --resume, --model).

    \b
    Examples:
        cassandra wrap claude                    # Start everything
        cassandra wrap claude --memory           # With persistent memory
        cassandra wrap claude --resume <id>      # Resume a session
        cassandra wrap claude -- -p              # Claude in print mode
        cassandra wrap claude                    # tokensave code graph (primary)
        cassandra wrap claude --no-tokensave     # Skip tokensave; fall back to Serena
        cassandra wrap claude --serena           # Also register the Serena backup
        cassandra wrap claude --no-context-tool  # Skip CLI context-tool setup
        cassandra wrap claude --no-mcp           # Skip MCP retrieve tool registration
        cassandra wrap claude --no-serena        # Never register the Serena backup
        cassandra wrap claude --1m               # Preserve the 1M context window
    """
    if prepare_only:
        if not no_rtk:
            if _selected_context_tool() == _CONTEXT_TOOL_LEAN_CTX:
    ...
    # Memory sync BEFORE proxy startup — sync cassandra DB ↔ Claude's files
    if memory:
        try:
            mem_dir = Path.cwd() / ".cassandra"
    ...
    try:
        click.echo()
        click.echo("  ╔═══════════════════════════════════════════════╗")
        click.echo("  ║            CASSANDRA WRAP: CLAUDE              ║")
        click.echo("  ╚═══════════════════════════════════════════════╝")
        click.echo()
        ...
        # Cassandra and the proxy compresses the request, then forwards to the
        ...
        # by Cassandra. This is the turnkey Vertex compression path.
        ...
        if not no_mcp:
            ...
            _setup_cassandra_mcp(ClaudeRegistrar(), port, verbose=verbose)
        elif verbose:
        ...
        click.echo()
        click.echo("  Launching Claude Code (API routed through Cassandra)...")
        if use_vertex:
            click.echo(
        ...
        if use_vertex:
            ...
            # we only redirect its Vertex endpoint to Cassandra.
        ...
        # daemon's environment) also route through Cassandra.
        ...
        # directory's name via X-Cassandra-Project (user override wins).
...
@click.option("--no-stop-proxy", is_flag=True, help="Do not stop the local Cassandra proxy")
@click.option("--keep-mcp", is_flag=True, help="Keep Cassandra MCP registrations")
...
def unwrap_claude(
    ...
    """Undo durable setup from ``cassandra wrap claude``."""
    click.echo()
    click.echo("  ╔═══════════════════════════════════════════════╗")
    click.echo("  ║          CASSANDRA UNWRAP: CLAUDE              ║")
    click.echo("  ╚═══════════════════════════════════════════════╝")
    click.echo()
    ...
    if not keep_mcp:
        ...
        if registrar.detect():
            removed_cassandra = registrar.unregister_server("cassandra")
            removed_code_graph = registrar.unregister_server(_CBM_MCP_SERVER_NAME)
            tokensave_status = _remove_cassandra_installed_tokensave_mcp(registrar)
            serena_status = _remove_cassandra_installed_serena_mcp(registrar)
            if removed_cassandra:
                click.echo("  Removed Cassandra MCP retrieve tool from Claude.")
            else:
                click.echo("  Cassandra MCP retrieve tool was not registered in Claude.")
            ...
            if tokensave_status == "removed":
                click.echo("  Removed Cassandra-installed tokensave MCP server from Claude.")
            elif tokensave_status == "failed":
                click.echo(
                    "  tokensave MCP server matched Cassandra ledger but could not be removed."
                )
            if serena_status == "removed":
                click.echo("  Removed Cassandra-installed Serena MCP server from Claude.")
            elif serena_status == "failed":
                click.echo("  Serena MCP server matched Cassandra ledger but could not be removed.")
        else:
    ...
    click.echo()
    click.echo("✓ Claude is no longer durably wrapped by Cassandra.")
...
@click.option(
    "--backend",
    default=None,
    help="API backend for the proxy: 'anthropic', 'anyllm', 'litellm-vertex', etc. (env: CASSANDRA_BACKEND)",
)
@click.option(
    "--anyllm-provider",
    default=None,
    help="Provider for any-llm backend: openai, mistral, groq, etc. (env: CASSANDRA_ANYLLM_PROVIDER)",
)
@click.option(
    "--region", default=None, help="Cloud region for Bedrock/Vertex (env: CASSANDRA_REGION)"
)
@click.option(
    "--provider-type",
...
@click.option(
    ...
    is_flag=True,
    help=(
        "Experimental: route GitHub-authenticated Copilot CLI traffic through Cassandra "
...
def copilot(
    ...
    """Launch GitHub Copilot CLI through Cassandra proxy.

    \b
    Configures Copilot CLI BYOK provider variables so Copilot routes through
    the local Cassandra proxy. In auto mode, the wrapper uses Anthropic-style
    ...
    Examples:
        ...
        cassandra wrap copilot --provider-type openai --wire-api responses -- --model gpt-5.4
        cassandra wrap copilot --subscription -- --model gpt-4.1
        cassandra wrap copilot --no-context-tool -- --prompt "explain this file"
    ...
    if _should_use_copilot_oauth(
        ...
        if not client_bearer:
            raise click.ClickException(
                ...
                "token, but none could be resolved. Run `cassandra copilot-auth login` first, or set "
        ...
        if not env.get("COPILOT_PROVIDER_API_KEY"):
            ...
            click.echo(
                ...
                f"  `cassandra wrap copilot` uses Copilot's BYOK mode, which bypasses GitHub's\n"
                f"  Copilot API and routes requests directly to the model provider through the\n"
                f"  Cassandra proxy. A GitHub Copilot subscription alone is not sufficient.\n\n"
                f"  Set one of:\n"
    ...
    if not subscription and not _copilot_model_configured(copilot_args, env):
        ...
        if _is_auto_model(raw_model):
            click.echo(
                ...
                "      cassandra wrap copilot --subscription -- --model auto"
            )
            raise SystemExit(1)
        else:
            click.echo(
...
def unwrap_copilot(port: int, no_stop_proxy: bool) -> None:
    """Undo durable setup from ``cassandra wrap copilot``."""
    instructions = Path.cwd() / ".github" / "copilot-instructions.md"
    if _remove_rtk_instructions(instructions):
        click.echo("  Removed Cassandra rtk instructions from Copilot.")
    else:
        click.echo("  No Cassandra rtk instructions found for Copilot.")
...
def codex(
    ...
    """Launch OpenAI Codex CLI through Cassandra proxy.

    \b
    Sets OPENAI_BASE_URL to route all OpenAI API calls through Cassandra.
    Sets up the selected CLI context tool so Codex uses token-optimized
    commands (60-90% savings on shell output). Also
    registers the cassandra MCP server in the active Codex config file
    so Codex can call ``cassandra_retrieve`` on compression markers.

    \b
    Examples:
        cassandra wrap codex                         # Start proxy + context tool + mcp + codex
        cassandra wrap codex -- "fix the bug"        # Pass prompt to codex
        cassandra wrap codex --no-context-tool       # Skip CLI context-tool setup
        cassandra wrap codex --no-mcp                # Skip MCP retrieve tool registration
        cassandra wrap codex --no-tokensave          # Skip tokensave; fall back to Serena
        cassandra wrap codex --serena                # Also register the Serena backup
        cassandra wrap codex --no-serena             # Never register the Serena backup
        cassandra wrap codex --port 9999             # Custom proxy port
        cassandra wrap codex --backend anyllm --anyllm-provider groq
    """
    # Snapshot Codex config.toml BEFORE any wrap-time mutation so
    # `cassandra unwrap codex` can restore the user's pre-wrap state
    # byte-for-byte. The snapshot is a no-op if the backup already exists
    # or if the file already has Cassandra markers, so this is safe to
    ...
    # Register cassandra MCP server in Codex config.toml so Codex can
    # call cassandra_retrieve on compression markers from the proxy.
    if not no_mcp:
        from cassandra.mcp_registry import CodexRegistrar
        ...
        _setup_cassandra_mcp(CodexRegistrar(), port, verbose=verbose, force=True)
    elif verbose:
        click.echo("  Skipping MCP retrieve tool (--no-mcp)")
    ...
    if memory:
        ...
        try:
            ...
            from cassandra.memory.sync import _build_sync_backend, sync_import
            from cassandra.memory.sync_adapters.claude_code import (
                ClaudeCodeAdapter,
    ...
    # X-Cassandra-Project header to CASSANDRA_PROJECT via env_http_headers, so
    # Codex sends it only when this var is set.  A user-set value wins.
    _codex_project = _project_name_from_cwd()
    if _codex_project and "CASSANDRA_PROJECT" not in env:
        env["CASSANDRA_PROJECT"] = _codex_project

    # Inject Cassandra provider into Codex config so WebSocket traffic also
...
def aider(
    ...
    """Launch aider through Cassandra proxy.

    \b
    Sets OPENAI_API_BASE to route all API calls through Cassandra.
    Sets up the selected CLI context tool so aider uses token-optimized commands.

    \b
    Examples:
        cassandra wrap aider                              # Start proxy + context tool + aider
        cassandra wrap aider -- --model gpt-4o            # Use GPT-4o
        cassandra wrap aider -- --model claude-sonnet-4   # Use Claude
        cassandra wrap aider --no-context-tool            # Skip CLI context-tool setup
...
def vibe(
    ...
    """Launch Mistral Vibe through Cassandra proxy.

    \b
    Sets VIBE_PROVIDERS to route all Mistral API calls through Cassandra.

    \b
    Examples:
        cassandra wrap vibe                         # Start proxy + vibe
        cassandra wrap vibe -- "fix the bug"        # Pass prompt to vibe
        cassandra wrap vibe --port 9999             # Custom proxy port
        cassandra wrap vibe --no-context-tool       # Skip CLI context-tool setup
    """
    if prepare_only:
        return
...
def cursor(
    ...
    """Start Cassandra proxy for use with Cursor.
    ...
    \b
    Example:
        cassandra wrap cursor                # Start proxy + context-tool instructions
        cassandra wrap cursor --no-context-tool  # Proxy only, no CLI context tool
        cassandra wrap cursor --port 9999    # Custom proxy port
    """
...
def cline(
    ...
    """Start Cassandra proxy for use with Cline (VS Code extension).
    ...
    the API Base URL to point at the local Cassandra proxy.

    \b
    Uninstall: there is no ``cassandra unwrap cline`` subcommand. To remove the
    injected guidance, hand-edit ``.clinerules`` at the project root and
    delete everything between ``<!-- cassandra:rtk-instructions -->`` and
    ``<!-- /cassandra:rtk-instructions -->`` (inclusive). If ``lean-ctx`` mode
    ...
    \b
    Examples:
        cassandra wrap cline                  # Start proxy + .clinerules instructions
        cassandra wrap cline --no-context-tool # Proxy only, no CLI context tool
        cassandra wrap cline --port 9999      # Custom proxy port
    """
...
def continue_dev(
    ...
    """Start Cassandra proxy for use with Continue (VS Code / JetBrains).
    ...
    \b
    Uninstall: there is no ``cassandra unwrap continue`` subcommand. To remove
    the injected guidance, hand-edit ``.continue/config.json`` and delete
    everything between ``<!-- cassandra:rtk-instructions -->`` and
    ``<!-- /cassandra:rtk-instructions -->`` (inclusive) from every
    ...
    \b
    Examples:
        cassandra wrap continue                # Start proxy + inject systemMessage
        cassandra wrap continue --no-context-tool   # Proxy only
        cassandra wrap continue --port 9999    # Custom proxy port
        cassandra wrap continue --config path/to/config.json
    """
...
def goose(
    ...
    """Launch Goose (Block) CLI through Cassandra proxy.

    \b
    Sets OPENAI_BASE_URL and ANTHROPIC_BASE_URL to route Goose's API calls
    through Cassandra. Sets up the selected CLI context tool by injecting RTK
    guidance into .goosehints at the project root (Goose reads this file as
    extra system context).

    \b
    Uninstall: there is no ``cassandra unwrap goose`` subcommand. To remove the
    ...
    \b
    Examples:
        cassandra wrap goose                          # Start proxy + context tool + goose
        cassandra wrap goose -- session               # Start a Goose session
        cassandra wrap goose -- --provider anthropic  # Pass args to goose
        cassandra wrap goose --no-context-tool        # Skip CLI context-tool setup
    """
...
def openhands(
    ...
    """Launch OpenHands CLI through Cassandra proxy.

    \b
    Sets OPENAI_BASE_URL / ANTHROPIC_BASE_URL to route OpenHands' API calls
    through Cassandra. Instructions are injected via the
    ...
    ``<!-- cassandra:rtk-instructions -->`` marker. To uninstall, simply do not
    ...
    \b
    Examples:
        cassandra wrap openhands                # Start proxy + context tool + openhands
        cassandra wrap openhands -- --task ...  # Pass args to openhands
        cassandra wrap openhands --no-context-tool
    """
...
@click.option(
    "--proxy-port", default=8787, type=click.IntRange(1, 65535), help="Cassandra proxy port"
)
...
@click.option(
    ...
    multiple=True,
    help="OpenClaw provider id to route through Cassandra (repeatable; default: openai-codex)",
)
@click.option(
    "--python-path",
    default=None,
...
@click.option(
    "--no-auto-start",
    is_flag=True,
    help="Disable plugin auto-start of local cassandra proxy",
)
@click.option(
    "--no-restart",
    is_flag=True,
...
def openclaw(
    ...
    """Install and configure Cassandra OpenClaw plugin in one command.
    ...
    Example:
      ...
      cassandra wrap openclaw --plugin-path C:\\git\\cassandra\\plugins\\openclaw
    """
    if prepare_only:
    ...
    click.echo()
    click.echo("  ╔═══════════════════════════════════════════════╗")
    click.echo("  ║           CASSANDRA WRAP: OPENCLAW             ║")
    click.echo("  ╚═══════════════════════════════════════════════╝")
    click.echo()
    ...
    existing_entry = _read_openclaw_config_value(openclaw_bin, "plugins.entries.cassandra")
    entry = _build_openclaw_plugin_entry(
        existing_entry=existing_entry,
        proxy_port=proxy_port,
        startup_timeout_ms=startup_timeout_ms,
    ...
    _set_openclaw_context_engine_slot(openclaw_bin, "cassandra")
    _run_checked(
    ...
    inspect_result = _run_checked(
        [openclaw_bin, "plugins", "inspect", "cassandra"],
        action="openclaw plugins inspect cassandra",
    )
    if verbose and inspect_result.stdout.strip():
        click.echo(inspect_result.stdout.strip())

    click.echo()
    click.echo("✓ OpenClaw is configured to use Cassandra context compression.")
    click.echo("  Plugin: cassandra")
    click.echo("  Slot:   plugins.slots.contextEngine = cassandra")
    click.echo()
...
@click.option("--no-mcp", is_flag=True, help="Skip cassandra MCP server registration")
...
def opencode(
    ...
    """Launch OpenCode through Cassandra proxy.

    \b
    Sets OPENCODE_CONFIG_CONTENT to route all OpenCode API calls through
    Cassandra. Configures a cassandra provider via @ai-sdk/openai-compatible.
    Also sets OPENAI_BASE_URL and ANTHROPIC_BASE_URL as fallbacks.

    \b
    Examples:
        cassandra wrap opencode                         # Start proxy + context tool + opencode
        cassandra wrap opencode -- "fix the bug"        # Pass prompt to opencode
        cassandra wrap opencode --no-context-tool       # Skip CLI context-tool setup
        cassandra wrap opencode --no-mcp                # Skip MCP retrieve tool registration
        cassandra wrap opencode --no-serena             # Skip Serena MCP registration
        cassandra wrap opencode --port 9999             # Custom proxy port
        cassandra wrap opencode --backend anyllm --anyllm-provider groq
    """
    # Snapshot OpenCode config.json BEFORE any wrap-time mutation so
    # `cassandra unwrap opencode` can restore the user's pre-wrap state.
    ...
    # Register cassandra MCP server in OpenCode config so OpenCode can
    # call cassandra_retrieve on compression markers from the proxy.
    if not no_mcp:
        from cassandra.mcp_registry import OpencodeRegistrar

        _setup_cassandra_mcp(OpencodeRegistrar(), port, verbose=verbose, force=True)
    elif verbose:
        click.echo("  Skipping MCP retrieve tool (--no-mcp)")
    ...
    # Inject Cassandra provider into OpenCode config so traffic routes through proxy.
    inject_opencode_provider_config(port)
    if memory:
        mem_dir = Path.cwd() / ".cassandra"
...
def unwrap_opencode(port: int, no_stop_proxy: bool) -> None:
    """Undo ``cassandra wrap opencode`` edits to the active OpenCode config file.

    Behaviour:

    * If a pre-wrap backup (``opencode.json.cassandra-backup``) exists, the
      original file is restored byte-for-byte and the backup is removed.
    * Otherwise, if the config file still contains the Cassandra-managed
      block, that block is stripped out and the rest of the file is
      preserved.
    * If the config only ever contained Cassandra-written content, the file
      is removed entirely so OpenCode falls back to its defaults.
    * If neither a backup nor a Cassandra block is present, this is a safe
      no-op.
    """
    click.echo()
    click.echo("  ╔═══════════════════════════════════════════════╗")
    click.echo("  ║         CASSANDRA UNWRAP: OPENCODE             ║")
    click.echo("  ╚═══════════════════════════════════════════════╝")
    click.echo()
    ...
    if backup_file.exists():
        ...
        if _PROVIDER_MARKER_START in content or _MCP_MARKER_START in content:
            ...
            if cleaned.strip():
                ...
                config_file.unlink()
                click.echo(f"  Removed {config_file} (contained only Cassandra-written config).")
                status = "removed"
        else:
            click.echo(f"  Nothing to undo: {config_file} has no Cassandra wrap markers.")
            status = "noop"
    else:
        click.echo(f"  Nothing to undo: {config_file} does not exist.")
        status = "noop"

    # Remove Serena MCP if it was installed by Cassandra.
    # Also remove the cassandra MCP server itself.
    from cassandra.mcp_registry import OpencodeRegistrar

    opencode_registrar = OpencodeRegistrar()
    if opencode_registrar.detect():
        if opencode_registrar.unregister_server("cassandra"):
            click.echo("  Removed Cassandra MCP server from OpenCode.")
        serena_status = _remove_cassandra_installed_serena_mcp(opencode_registrar)
        if serena_status == "removed":
            click.echo("  Removed Cassandra-installed Serena MCP server from OpenCode.")
        elif serena_status == "failed":
            click.echo("  Serena MCP server matched Cassandra ledger but could not be removed.")

    click.echo()
    click.echo("✓ OpenCode is no longer routed through the Cassandra proxy.")
...
def unwrap_openclaw(
    ...
    """Disable the Cassandra OpenClaw plugin and restore the legacy engine slot."""
    if prepare_only:
        click.echo(
            json.dumps(
    ...
    click.echo()
    click.echo("  ╔═══════════════════════════════════════════════╗")
    click.echo("  ║          CASSANDRA UNWRAP: OPENCLAW            ║")
    click.echo("  ╚═══════════════════════════════════════════════╝")
    click.echo()
    click.echo("  Disabling Cassandra plugin and removing engine mapping...")
    ...
    click.echo()
    click.echo("✓ OpenClaw Cassandra wrap removed.")
    click.echo("  Plugin: cassandra (installed, disabled)")
...
def unwrap_codex(port: int, no_stop_proxy: bool) -> None:
    """Undo ``cassandra wrap codex`` edits to the active Codex config file.

    Behaviour:

    * If a pre-wrap backup (``config.toml.cassandra-backup``) exists, the
      original file is restored byte-for-byte and the backup is removed.
    * Otherwise, if the config file still contains the Cassandra-managed
      block, that block is stripped out and the rest of the file is
      preserved.
    ...
    * If neither a backup nor a Cassandra block is present, this is a safe
      ...
      it). When ``CODEX_HOME`` is unset, print a warning hint because Cassandra
      may be looking at the default config while Codex was wrapped with a
      custom home.
    """
    click.echo()
    click.echo("  ╔═══════════════════════════════════════════════╗")
    click.echo("  ║           CASSANDRA UNWRAP: CODEX              ║")
    click.echo("  ╚═══════════════════════════════════════════════╝")
    click.echo()
    ...
    if codex_registrar.detect():
        tokensave_status = _remove_cassandra_installed_tokensave_mcp(codex_registrar)
        if tokensave_status == "removed":
            click.echo("  Removed Cassandra-installed tokensave MCP server from Codex.")
        elif tokensave_status == "failed":
            click.echo("  tokensave MCP server matched Cassandra ledger but could not be removed.")

        serena_status = _remove_cassandra_installed_serena_mcp(codex_registrar)
        if serena_status == "removed":
            click.echo("  Removed Cassandra-installed Serena MCP server from Codex.")
        elif serena_status == "failed":
            click.echo("  Serena MCP server matched Cassandra ledger but could not be removed.")

    if status in {"restored", "cleaned", "removed"}:
        # Hand the threads back to the native-provider menu so the full history
        # stays visible once Codex no longer routes through Cassandra. Best-effort.
        retag_to_native(_codex_home_dir())

    click.echo()
    click.echo("✓ Codex is no longer routed through the Cassandra proxy.")
    if not no_stop_proxy and status != "noop":
        _echo_unwrap_proxy_stop_status(_stop_local_proxy_for_unwrap(port), port)
    click.echo()
