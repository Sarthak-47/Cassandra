...
from cassandra.agent_savings import get_agent_savings_profile
...
@click.option(
    ...
    default=None,
    help="Cassandra eval JSON report proving accuracy preservation.",
)
@click.option(
    "--write-smoke-fixture",
...
def agent_savings(
    ...
    if write_smoke_fixture is not None:
        ...
        click.echo(
            "Verify with: CASSANDRA_WORKSPACE_DIR="
            f"{write_smoke_fixture} cassandra agent-savings --check-perf "
...
def _perf_line(
    ...
    saved = before - after
    return (
        f"{timestamp} - cassandra.proxy - INFO - [{request_id}] PERF "
