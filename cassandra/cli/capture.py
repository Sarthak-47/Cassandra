...
def capture_group() -> None:
    """Capture and compare network traffic for Cassandra investigations."""
...
@click.option(
    ...
    required=True,
    type=click.Path(exists=True, dir_okay=False, path_type=Path),
    help="JSONL capture from the Cassandra-proxied Claude Code lane.",
)
@click.option(
    "--output",
    "markdown_output",
...
def network_diff(
    ...
    """Compare direct and Cassandra MITM capture JSONL files."""
    ...
    diff = compare_captures(direct, cassandra, pair_by=pair_by)
