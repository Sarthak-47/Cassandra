...
def output_savings() -> None:
    ...
    if not path.exists():
        ...
        click.echo("Run `cassandra learn --verbosity --apply` to seed the baseline,")
        click.echo("then enable the shaper (CASSANDRA_OUTPUT_SHAPER=1) and send traffic.")
        return
    ...
    if est.kind == "estimated":
        click.echo(
            ...
            "\n  set CASSANDRA_OUTPUT_HOLDOUT=0.1 to leave 10% of conversations"
