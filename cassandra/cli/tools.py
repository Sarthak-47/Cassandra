...
Exposes:
    ...
    cassandra sg …           ->  ast-grep (from the ast-grep-cli PyPI wheel)
    cassandra diff A B …     ->  difftastic
    cassandra loc [PATH] …   ->  scc
    cassandra tools install  ->  pre-fetch all bundled binaries
    cassandra tools doctor   ->  print a status table
    cassandra tools list     ->  show the registry
...
tool without any Cassandra-specific protocol.
"""
...
def _exec_tool(tool: str, argv: Sequence[str]) -> None:
    try:
        ...
        click.secho(
            f"error: {e}\nHint: run `cassandra tools install` on a networked machine, "
    ...
    # the parent `cassandra` process, not these thin passthroughs).
    cmd = [str(path), *argv]
    if not _is_windows():
    ...
    for name in selected:
        ...
        if binaries._is_pypi_tool(name):  # noqa: SLF001
            ...
            if on_path:
                ...
                click.secho(
                    f"{name}: not on PATH — `pip install cassandra-ai` should provide it",
                    fg="yellow",
                )
                exit_code = 1
            continue
