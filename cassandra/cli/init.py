...
from cassandra.install.models import ConfigScope, InstallPreset, RuntimeKind, SupervisorKind
from cassandra.install.paths import claude_settings_path, codex_config_path, validate_profile_name
from cassandra.install.planner import build_manifest
from cassandra.install.providers import _apply_unix_env_scope, _apply_windows_env_scope
...
from cassandra.install.state import ManifestError, load_manifest, save_manifest
from cassandra.install.supervisors import start_supervisor
from cassandra.providers.claude import TOOL_SEARCH_DEFAULT, TOOL_SEARCH_ENV
from cassandra.providers.codex.install import codex_uses_chatgpt_auth
from cassandra.providers.codex.threads import retag_to_cassandra

from .main import main

logger = logging.getLogger(__name__)

_VERBOSE_HANDLER_ATTR = "_cassandra_init_verbose_handler"

_GLOBAL_PROFILE = "init-user"
_CLAUDE_HOOK_MARKER = "cassandra-init-claude"
_COPILOT_HOOK_MARKER = "cassandra-init-copilot"
_CODEX_HOOK_MARKER = "cassandra-init-codex"
_CODEX_PROVIDER_MARKER_START = "# --- Cassandra init provider ---"
_CODEX_PROVIDER_MARKER_END = "# --- end Cassandra init provider ---"
_CODEX_FEATURE_MARKER_START = "# --- Cassandra init features ---"
_CODEX_FEATURE_MARKER_END = "# --- end Cassandra init features ---"
...
def _hook_command(*parts: str) -> str:
    return _command_string([*resolve_cassandra_command(), "init", "hook", "ensure", *parts])
...
def _enable_verbose_logging() -> None:
    ...
    mutate stdout; all verbose output goes to stderr so ``cassandra init``
    ...
    handler.setFormatter(logging.Formatter("[cassandra init] %(message)s"))
...
def _strip_codex_init_block(content: str) -> str:
    """Remove all Cassandra init-managed blocks and orphan keys from a Codex config.toml string."""
    import re
    ...
    # Strip any orphaned [model_providers.cassandra] table that is recognisably ours.
...
def _ensure_codex_provider(path: Path, port: int) -> None:
    ...
    # native threads vanish once we switch to "cassandra". Retag them to match the
    ...
    # handled by `cassandra unwrap codex`.
    retag_to_cassandra(path.parent)
...
def _marketplace_source() -> str:
    override = os.environ.get("CASSANDRA_MARKETPLACE_SOURCE")
    if override:
        return override
    repo_root = Path(__file__).resolve().parents[2]
    if (repo_root / ".claude-plugin" / "marketplace.json").exists():
        return str(repo_root)
    return "Sarthak-47/cassandra"
...
def _install_claude_marketplace(scope: str) -> None:
    ...
    _run_checked(
        [claude_bin, "plugin", "install", "cassandra@cassandra-marketplace", "--scope", scope],
...
def _install_copilot_marketplace() -> None:
    ...
    _run_checked(
        [copilot_bin, "plugin", "install", "cassandra@cassandra-marketplace"],
...
def _format_empty_detection_error(global_scope: bool) -> str:
    ...
    lines: list[str] = [
        ...
        "",
        "Cassandra probed the following agents via shutil.which():",
    ]
    ...
    lines.extend(
        [
            ...
            "cassandra init just needs to know which agent to target.",
    ...
    for name, _path in probes:
        ...
        lines.append(f"  cassandra init{flag} {name}")

    lines.extend(
        [
            "",
            "Tip: run `cassandra init --help` to see all options.",
        ]
    )
    return "\n".join(lines)
...
def _init_claude(*, global_scope: bool, profile: str, port: int) -> None:
    ...
    click.echo("Restart Claude Code to activate Cassandra hooks and provider routing.")
...
def _init_copilot(*, global_scope: bool, profile: str, port: int, backend: str) -> None:
    ...
    click.echo("Restart Copilot CLI to activate Cassandra hooks and provider routing.")
...
def _init_codex(*, global_scope: bool, profile: str, port: int) -> None:
    ...
    click.echo("Restart Codex to activate Cassandra configuration.")
...
def _init_openclaw(*, global_scope: bool, port: int) -> None:
    ...
    command = [*resolve_cassandra_command(), "wrap", "openclaw", "--proxy-port", str(port)]
...
def _run_init_targets(
    *,
    ...
    # Register the cassandra MCP server with every targeted agent that has
    ...
    _install_cassandra_mcp_for_targets(targets=targets, port=port)


def _install_cassandra_mcp_for_targets(*, targets: list[str], port: int) -> None:
    """Install the cassandra MCP server into each detected target agent."""
    from cassandra.mcp_registry import format_results, install_everywhere
...
def init(
    ...
    """Install durable Cassandra integrations for supported agents."""
    if verbose:
...
def init_openclaw(ctx: click.Context) -> None:
    """Install the durable OpenClaw Cassandra plugin."""
    _run_init_targets(
        targets=["openclaw"],
        global_scope=bool(_ctx_value(ctx, "global_scope")),
        port=int(_ctx_value(ctx, "port") or 8787),
