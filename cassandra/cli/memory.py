...
def _default_db_path() -> str:
    ...
    (``./.cassandra/memory.db``); otherwise fall back to the global workspace
    store (``~/.cassandra/memory.db``). The previous bare ``cassandra_memory.db``
    default pointed at neither, so ``cassandra memory list`` silently read an
    empty/legacy DB.
    """
    project_db = Path.cwd() / ".cassandra" / "memory.db"
...
def db_path_option(fn: Any) -> Any:
    ...
    return click.option(
        ...
        "(./.cassandra/memory.db) if present, else the global store "
        "(~/.cassandra/memory.db).",
...
def memory(ctx: click.Context) -> None:
    ...
    \b
    Examples:
        cassandra memory list                     List all stored memories
        cassandra memory list --limit 10          List the 10 most recent memories
        cassandra memory list --scope USER        List only USER-level memories
        cassandra memory list --since 7d          List memories from the last 7 days
        cassandra memory show <id>                Show full details of a memory
        cassandra memory stats                    Show memory statistics
        cassandra memory edit <id> --content ...  Edit a memory's content
        cassandra memory delete <id>              Delete a memory
        cassandra memory prune --older-than 30d   Delete memories older than 30 days
        cassandra memory purge --confirm          Delete ALL memories
        cassandra memory export --output file.json  Export all memories to JSON
        cassandra memory import file.json         Import memories from JSON
    """
    pass
...
def list_memories(
    ...
    \b
    Examples:
        cassandra memory list                  List most recent memories
        cassandra memory list --limit 10       Show only 10 memories
        cassandra memory list --scope USER     Show only USER-level memories
        cassandra memory list --since 7d       Show memories from the last 7 days
        cassandra memory list --search "api"   Search for memories containing "api"
    """
...
def show_memory(
    ...
    \b
    Examples:
        cassandra memory show abc123            Show memory details
        cassandra memory show abc123 --json     Output as raw JSON
    """
    store = get_store(db_path)
    ...
    try:
        ...
        if old_count > 0:
            console.print(
                ...
                f"Consider running: cassandra memory prune --older-than 30d[/dim]"
            )
...
def edit_memory(
    ...
    \b
    Examples:
        cassandra memory edit abc123 --content "Updated content"
        cassandra memory edit abc123 --importance 0.8
        cassandra memory edit abc123 -c "New content" -i 0.9
    """
...
def delete_memories(
    ...
    \b
    Examples:
        cassandra memory delete abc123           Delete one memory
        cassandra memory delete abc123 def456    Delete multiple memories
        cassandra memory delete abc123 --force   Skip confirmation
    """
    store = get_store(db_path)
...
def prune_memories(
    ...
    Examples:
        cassandra memory prune --older-than 30d              Delete memories older than 30 days
        cassandra memory prune --scope TURN                  Delete all TURN-level memories
        cassandra memory prune --low-importance 0.3          Delete low-importance memories
        cassandra memory prune --older-than 7d --scope TURN  Combine filters
        cassandra memory prune --older-than 30d --dry-run    Preview what would be deleted
    """
...
def export_memories(ctx: click.Context, db_path: str, output: str | None) -> None:
    ...
    \b
    Examples:
        cassandra memory export                       Output to stdout
        cassandra memory export --output backup.json  Save to file
        cassandra memory export -o backup.json        Save to file (short form)
    """
    store = get_store(db_path)
...
def import_memories(ctx: click.Context, db_path: str, file: str, force: bool) -> None:
    ...
    \b
    Examples:
        cassandra memory import backup.json          Import from file
        cassandra memory import backup.json --force  Skip confirmation
    """
    file_path = Path(file)
