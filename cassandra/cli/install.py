...
from cassandra.install.health import probe_json, probe_ready
from cassandra.install.models import (
    ConfigScope,
    DeploymentManifest,
    InstallPreset,
...
from cassandra.install.providers import apply_mutations, revert_mutations
...
from cassandra.install.state import (
    ManifestError,
    delete_manifest,
    load_manifest,
    save_manifest,
...
def _reject_task_lifecycle(manifest: DeploymentManifest, action: str) -> None:
    if manifest.supervisor_kind == SupervisorKind.TASK.value:
        raise click.ClickException(
            ...
            f"`cassandra install {action}` is not supported for task deployments."
        )
...
@click.option(
    ...
    show_default=True,
    help="Runtime used to execute Cassandra for service/task modes.",
)
@click.option(
    "--scope",
...
@click.option(
    "--image",
    default="ghcr.io/Sarthak-47/cassandra:latest",
    show_default=True,
