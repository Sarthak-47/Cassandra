"""One-function compression API for Cassandra.

The simplest way to use Cassandra — no proxy, no config, just compress:
...
class CompressConfig:
    ...
    Pass to ``compress()`` or any integration that uses cassandra.
    ...
    """Kompress model ID. None = default (Sarthak-47/kompress-v2-base).
...
def compress(
    ...
    **kwargs: Any,
    ...
    """Compress messages using Cassandra's full compression pipeline.

    This is the simplest way to use Cassandra. No proxy, no config needed.
...
def compress_spreadsheet(
    ...
    **kwargs: Any,
    ...
    (``pip install cassandra-ai[spreadsheet]``).
    ...
    """
    from cassandra.transforms.spreadsheet_ingest import load_spreadsheet
...
def _get_pipeline() -> Any:
    ...
    with _pipeline_lock:
        ...
        logger.debug("Cassandra compression pipeline initialized")
        return _pipeline
