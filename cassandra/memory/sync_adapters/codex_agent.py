...
Syncs memories to/from a cassandra-managed section in AGENTS.md.
...
from cassandra.memory.sync import AgentMemory, AgentMemoryAdapter

_MARKER_START = "<!-- cassandra:memory:start -->"
_MARKER_END = "<!-- cassandra:memory:end -->"
...
class CodexAdapter(AgentMemoryAdapter):
    ...
    async def read_memories(self) -> list[AgentMemory]:
        """Read memories from the cassandra section of AGENTS.md."""
    ...
    async def write_memories(self, memories: list[dict[str, Any]]) -> int:
        """Write memories into the cassandra section of AGENTS.md."""
        if not memories:
            return 0

        # Build section content
        lines = ["## Cassandra Shared Memory", ""]
        for mem in memories:
