...
Each .md file has:
    ...
    cassandra_id: <uuid>          (added by sync for cross-reference)
...
class ClaudeCodeAdapter(AgentMemoryAdapter):
            ...
            memories.append(
                AgentMemory(
                    ...
                    metadata={
                        ...
                        "cassandra_id": fm.get("cassandra_id", ""),
    ...
    async def write_memories(self, memories: list[dict[str, Any]]) -> int:
        ...
        for mem in memories:
            ...
            cassandra_id = mem.get("cassandra_id", "")
    ...
    def _update_memory_md_index(self, new_entries: list[str]) -> None:
        """Append new entries to MEMORY.md under a Cassandra section."""
        memory_md = self._memory_dir / "MEMORY.md"

        section_marker = "## Cassandra Shared Memory"
        new_section = f"\n{section_marker}\n" + "\n".join(new_entries) + "\n"
