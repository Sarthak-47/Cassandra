"""Configuration dataclasses for Cassandra's hierarchical memory system.
...
class StoreBackend(Enum):
    ...
    SQLITE = "sqlite"
    EXTERNAL = "external"  # Loaded from entry_points(group="cassandra.memory_store")
...
class VectorBackend(Enum):
    ...
    EXTERNAL = "external"  # Loaded from entry_points(group="cassandra.memory_vector")
...
class TextBackend(Enum):
    ...
    FTS5 = "fts5"
    EXTERNAL = "external"  # Loaded from entry_points(group="cassandra.memory_text")
...
class MemoryConfig:
    ...
    db_path: Path = field(default_factory=lambda: Path("cassandra_memory.db"))
