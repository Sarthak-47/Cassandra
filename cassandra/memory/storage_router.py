...
* ``USER``: one DB per ``x-cassandra-user-id`` (no project axis).
...
class ProjectResolver:
    ...
    def resolve(self, ctx: RequestContext) -> tuple[str, str] | None:
        ...
        explicit = self._first_nonempty_header(ctx.headers, "x-cassandra-project-id")
        if explicit:
        ...
        explicit_cwd = self._first_nonempty_header(ctx.headers, "x-cassandra-cwd")
        if explicit_cwd:
        ...
        if ident is None:
            ...
            if fallback == "empty":
                ...
                logger.warning(
                    ...
                    "hint='set x-cassandra-project-id or x-cassandra-cwd header, "
