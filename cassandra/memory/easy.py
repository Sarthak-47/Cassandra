...
This module provides the easiest way to use Cassandra's memory system.
...
if TYPE_CHECKING:
    from cassandra.memory.ports import MemorySearchResult
...
class Memory:
    ...
    Args:
        ...
        db_path: Path for local database (only for "local" backend).
            Defaults to ~/.cassandra/memory.db
        qdrant_url: Full Qdrant URL (only for "qdrant-neo4j" backend). When set,
            ...
            Defaults to the ``CASSANDRA_QDRANT_URL`` env var if unset.
        qdrant_host: Qdrant host (only for "qdrant-neo4j" backend). Defaults
            to the ``CASSANDRA_QDRANT_HOST`` env var or ``localhost``.
        qdrant_port: Qdrant port (only for "qdrant-neo4j" backend). Defaults
            to the ``CASSANDRA_QDRANT_PORT`` env var or ``6333``.
        qdrant_api_key: API key for hosted Qdrant. Defaults to
            ``CASSANDRA_QDRANT_API_KEY`` if unset.
    ...
    Examples:
        ...
        # Hosted Qdrant via URL + API key (or set CASSANDRA_QDRANT_URL /
        # CASSANDRA_QDRANT_API_KEY in the environment)
        memory = Memory(
    ...
    def __init__(
        ...
        if db_path is None:
            # Default: workspace memory.db (respects CASSANDRA_WORKSPACE_DIR)
        ...
        # ``None`` sentinels fall back to CASSANDRA_QDRANT_* env vars so that
