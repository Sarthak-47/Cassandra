"""Memory Bridge: bidirectional bridge between markdown files and Cassandra memory.

Imports markdown memory files (Claude Code MEMORY.md, ChatGPT facts, etc.)
into Cassandra's semantic memory system, and exports Cassandra memories back
to organized markdown files. Supports bidirectional sync with hash-based
change detection.

Usage:
    from cassandra.memory.bridge import MemoryBridge
    from cassandra.memory.bridge_config import BridgeConfig
    ...
    # Import markdown -> Cassandra
    stats = await bridge.import_from_markdown()

    # Export Cassandra -> markdown
...
from cassandra.memory.bridge_parsers import (
    ParsedFile,
    ParsedSection,
...
if TYPE_CHECKING:
    ...
    from cassandra.memory.models import Memory
...
class MemoryBridge:
    """Bidirectional bridge between markdown memory files and Cassandra's
    semantic memory system.

    Supports import (md -> Cassandra), export (Cassandra -> md), and
    ...
    async def import_from_markdown(
        ...
        """Import markdown memory files into Cassandra's vector store.
    ...
    async def _import_section(
        ...
        """Import a single parsed section into Cassandra.
    ...
    async def export_to_markdown(
        ...
        """Export Cassandra memories to a markdown file.
    ...
    async def sync(
        ...
        """Bidirectional sync between markdown files and Cassandra.

        1. Import new/changed sections from md files.
        2. Export new organic Cassandra memories back to md files.
        """
        paths = paths or self._config.md_paths
        user_id = user_id or self._config.user_id
        stats = SyncStats()
