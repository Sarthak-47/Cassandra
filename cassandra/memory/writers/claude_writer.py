...
from cassandra.memory.sync_adapters.claude_code import encode_claude_project_path
from cassandra.memory.writers.base import AgentWriter, MemoryEntry
...
class ClaudeCodeMemoryWriter(AgentWriter):
    ...
    def format_memories(self, memories: list[MemoryEntry]) -> str:
        ...
        lines = [
            "## Cassandra Learned Context",
            "*Auto-maintained by Cassandra — do not edit manually*",
            "",
        ]
    ...
    def export_topics(
        ...
        for category, entries in grouped.items():
            ...
            filename = f"cassandra_{category}.md"
            lines = [
                "---",
                f"name: cassandra-{category}",
                f"description: Cassandra-learned {category.replace('_', ' ')} patterns",
                "type: reference",
                "---",
                "",
