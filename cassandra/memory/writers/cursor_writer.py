...
class CursorMemoryWriter(AgentWriter):
    ...
    def default_path(self) -> Path:
        """Default: .cursor/rules/cassandra-memory.mdc in project root."""
        return self._project_path / ".cursor" / "rules" / "cassandra-memory.mdc"
    ...
    def export(
        ...
        if target.exists():
            ...
            full_content = (
                "---\n"
                "description: Cassandra-learned patterns from proxy traffic\n"
                "alwaysApply: true\n"
                "---\n"
                "\n"
