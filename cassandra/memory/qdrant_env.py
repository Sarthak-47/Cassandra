...
Provides a single source of truth for ``CASSANDRA_QDRANT_*`` env vars so that
...
- ``CASSANDRA_QDRANT_URL``        Full URL (e.g. ``https://xyz.cloud.qdrant.io:6333``).
                                  When set, takes precedence over host/port.
- ``CASSANDRA_QDRANT_HOST``       Hostname. Default: ``localhost``.
- ``CASSANDRA_QDRANT_PORT``       HTTP port. Default: ``6333``.
- ``CASSANDRA_QDRANT_API_KEY``    API key for hosted Qdrant (e.g. Qdrant Cloud).
- ``CASSANDRA_QDRANT_HTTPS``      ``true``/``false``. Forces HTTPS on/off.
- ``CASSANDRA_QDRANT_PREFER_GRPC````true``/``false``. Use gRPC instead of HTTP.
- ``CASSANDRA_QDRANT_GRPC_PORT``  gRPC port. Default: ``6334``.
...
def qdrant_env_url() -> str | None:
    """Return ``CASSANDRA_QDRANT_URL`` or ``None`` if unset."""
    return _strip_env("CASSANDRA_QDRANT_URL")


def qdrant_env_host() -> str:
    """Return ``CASSANDRA_QDRANT_HOST`` or the ``localhost`` default."""
    return _strip_env("CASSANDRA_QDRANT_HOST") or DEFAULT_QDRANT_HOST


def qdrant_env_port() -> int:
    """Return ``CASSANDRA_QDRANT_PORT`` or the ``6333`` default."""
    return (
        _parse_port(_strip_env("CASSANDRA_QDRANT_PORT"), "CASSANDRA_QDRANT_PORT")
        or DEFAULT_QDRANT_PORT
    )


def qdrant_env_api_key() -> str | None:
    """Return ``CASSANDRA_QDRANT_API_KEY`` or ``None`` if unset."""
    return _strip_env("CASSANDRA_QDRANT_API_KEY")


def qdrant_env_https() -> bool | None:
    """Return ``CASSANDRA_QDRANT_HTTPS`` parsed as bool, or ``None`` if unset."""
    return _parse_bool(_strip_env("CASSANDRA_QDRANT_HTTPS"))


def qdrant_env_prefer_grpc() -> bool:
    """Return ``CASSANDRA_QDRANT_PREFER_GRPC`` parsed as bool. Default: ``False``."""
    return _parse_bool(_strip_env("CASSANDRA_QDRANT_PREFER_GRPC")) or False


def qdrant_env_grpc_port() -> int:
    """Return ``CASSANDRA_QDRANT_GRPC_PORT`` or the ``6334`` default."""
    return (
        _parse_port(_strip_env("CASSANDRA_QDRANT_GRPC_PORT"), "CASSANDRA_QDRANT_GRPC_PORT")
