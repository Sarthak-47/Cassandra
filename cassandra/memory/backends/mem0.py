"""Mem0 backend adapter for Cassandra's hierarchical memory system.
...
from cassandra.memory.ports import MemoryFilter, VectorFilter, VectorSearchResult
...
class Mem0Config:
    ...
    # Qdrant settings (defaults resolve from CASSANDRA_QDRANT_* env vars)
    ...
    # Collection settings
    collection_name: str = "cassandra_memories"


class Mem0Backend:
    """
    Mem0 backend implementation for Cassandra memory system.
    ...
    This adapter maps Mem0's API to Cassandra's MemoryBackend interface:
    ...
    async def _ensure_client(self) -> Any:
        ...
        if self._client is None:
            try:
                ...
                raise ImportError(
                    "mem0 package not installed. Install with: pip install 'cassandra-ai[memory-stack]'"
                ) from None
    ...
    def _mem0_result_to_memory(self, result: dict[str, Any]) -> Memory:
        ...
        memory_id = metadata.get("cassandra_id", result.get("id", str(uuid.uuid4())))
