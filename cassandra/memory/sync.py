...
Provides bidirectional sync between cassandra's memory DB and any
...
Usage:
    from cassandra.memory.sync import sync, SyncResult
    from cassandra.memory.sync_adapters.claude_code import ClaudeCodeAdapter
...
logger = logging.getLogger("cassandra.memory.sync")

# State file for fast no-op detection (workspace bucket, respects
# CASSANDRA_WORKSPACE_DIR). Resolved at import time, matching prior behavior.
    ...
    async def write_memories(self, memories: list[dict[str, Any]]) -> int:
        ...
        Args:
            memories: List of dicts with keys: content, category, importance,
                      cassandra_id, source_agent, content_hash.
...
async def sync(
    ...
    """Bidirectional sync between cassandra DB and an agent's memory.
...
# ---------------------------------------------------------------------------
# CLI entry point: python -m cassandra.memory.sync --db ... --user ... --agent ...
# ---------------------------------------------------------------------------
...
def _build_sync_backend(db_path: str) -> Any:
    ...
    Match the proxy MCP server (see ``cassandra/memory/mcp_server.py``): use the
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Cassandra memory sync")
    ...
    async def _run() -> None:
        ...
        if args.agent == "claude":
            ...
            from cassandra.memory.sync_adapters.codex_agent import CodexAdapter
