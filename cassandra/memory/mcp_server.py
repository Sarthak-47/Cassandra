"""Cassandra Memory MCP Server.

A stdio MCP server that exposes cassandra's memory backend as tools
...
Usage:
    ...
    python -m cassandra.memory.mcp_server --db /path/to/.cassandra/memory.db

    # Registered in Codex config.toml (done by `cassandra wrap codex --memory`):
    [mcp_servers.cassandra_memory]
    command = "python"
    args = ["-m", "cassandra.memory.mcp_server", "--user", "alice"]
    # When --db is omitted, the server resolves .cassandra/memory.db from cwd.
"""
...
logger = logging.getLogger("cassandra.memory.mcp")
...
def create_memory_server(db_path: str, user_id: str = "default") -> Server:
    """Create an MCP server backed by cassandra's local memory."""

    server = Server("cassandra-memory")
...
def _memory_mcp_startup_context(
    ...
    active_project_db = (cwd / ".cassandra" / "memory.db").resolve(strict=False)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Cassandra Memory MCP Server")
    parser.add_argument(
        "--db",
        default=str(Path.cwd() / ".cassandra" / "memory.db"),
