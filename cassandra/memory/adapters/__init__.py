"""Memory adapters for Cassandra's hierarchical memory system.
...
from cassandra.memory.adapters.sqlite_graph import SQLiteGraphStore
...
def __getattr__(name: str) -> type | bool:
    ...
    if name == "HNSW_AVAILABLE":
        ...
        if _HNSW_AVAILABLE is None:
            from cassandra.memory.adapters.hnsw import _check_hnswlib_available
    ...
    if name == "SQLITE_VEC_AVAILABLE":
        ...
        if _SQLITE_VEC_AVAILABLE is None:
            from cassandra.memory.adapters.sqlite_vector import is_sqlite_vec_available
