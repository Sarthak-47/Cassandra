...
if TYPE_CHECKING:
    from cassandra.memory.ports import Embedder, MemoryCache, MemoryStore, TextIndex, VectorIndex


# Extension groups for memory backends registered via setuptools entry points.
_MEMORY_STORE_GROUP = "cassandra.memory_store"
_MEMORY_VECTOR_GROUP = "cassandra.memory_vector"
_MEMORY_TEXT_GROUP = "cassandra.memory_text"
...
def _load_external_backend(
    ...
    `cassandra.cache.compression_store._create_default_ccr_backend`.
    """
    if not name:
        raise ValueError(
...
def _create_store(config: MemoryConfig) -> MemoryStore:
    ...
    if config.store_backend == StoreBackend.SQLITE:
        from cassandra.memory.adapters.sqlite import SQLiteMemoryStore
...
def _create_embedder(config: MemoryConfig) -> Embedder:
    ...
    with _EMBEDDER_CACHE_LOCK:
        ...
        if config.embedder_backend == EmbedderBackend.LOCAL:
            ...
            from cassandra.memory.adapters.embedders import OllamaEmbedder
...
def _create_vector_index(config: MemoryConfig) -> VectorIndex:
    ...
    if backend == VectorBackend.AUTO:
        from cassandra.memory.adapters import HNSW_AVAILABLE, SQLITE_VEC_AVAILABLE
        ...
        if SQLITE_VEC_AVAILABLE:
            ...
            raise ValueError(
                ...
                "Or install the full proxy bundle: pip install cassandra-ai[proxy]"
            )

    if backend == VectorBackend.SQLITE_VEC:
        from cassandra.memory.adapters import SQLITE_VEC_AVAILABLE
        ...
        from cassandra.memory.adapters.sqlite_vector import SQLiteVectorIndex
    ...
    if backend == VectorBackend.HNSW:
        from cassandra.memory.adapters import HNSW_AVAILABLE
        ...
        from cassandra.memory.adapters.hnsw import HNSWVectorIndex
...
def _create_text_index(config: MemoryConfig) -> TextIndex:
    ...
    if config.text_backend == TextBackend.FTS5:
        from cassandra.memory.adapters.fts5 import FTS5TextIndex
...
def _create_cache(config: MemoryConfig) -> MemoryCache:
    ...
    """
    from cassandra.memory.adapters.cache import LRUMemoryCache
