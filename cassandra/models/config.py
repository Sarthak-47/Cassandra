"""Central configuration for all ML models used in Cassandra.
...
Usage:
    ...
    # CASSANDRA_SENTENCE_TRANSFORMER=intfloat/e5-small-v2
    # CASSANDRA_SIGLIP=google/siglip-base-patch16-224
"""
...
class MLModelConfig:
    ...
    All model names are defined here. Components throughout Cassandra
    import these defaults, so changing a value here changes it everywhere.

    Environment variables can override any default:
    - CASSANDRA_SENTENCE_TRANSFORMER
    - CASSANDRA_SIGLIP
    - CASSANDRA_SPACY
    - CASSANDRA_TECHNIQUE_ROUTER
    ...
    Attributes:
        ...
        technique_router: Model for image optimization routing.
            Default: Sarthak-47/technique-router (~100MB)
    """

    # Text Embeddings (SentenceTransformer)
    sentence_transformer: str = field(
        default_factory=lambda: os.environ.get("CASSANDRA_SENTENCE_TRANSFORMER", "all-MiniLM-L6-v2")
    )
    sentence_transformer_dim: int = 384

    # Image Embeddings (SIGLIP)
    siglip: str = field(
        default_factory=lambda: os.environ.get("CASSANDRA_SIGLIP", "google/siglip-base-patch16-224")
    )

    # Named Entity Recognition (spaCy)
    spacy: str = field(default_factory=lambda: os.environ.get("CASSANDRA_SPACY", "en_core_web_sm"))

    # Image Technique Router
    technique_router: str = field(
        default_factory=lambda: os.environ.get(
            "CASSANDRA_TECHNIQUE_ROUTER", "Sarthak-47/technique-router"
        )
    )
    ...
    _memory_estimates: dict[str, int] = field(
        default_factory=lambda: {
            ...
            "en_core_web_lg": 560,
            # Technique Router
            "Sarthak-47/technique-router": 100,
        }
    )
