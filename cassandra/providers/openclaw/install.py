...
from cassandra.install.models import DeploymentManifest, ManagedMutation, ToolTarget
from cassandra.install.paths import openclaw_config_path
...
def revert_provider_scope(mutation: ManagedMutation, manifest: DeploymentManifest) -> None:
    ...
    if not shutil_which("openclaw"):
        return
    command = [*resolve_cassandra_command(), "unwrap", "openclaw"]
    _invoke_openclaw(command)
