...
from cassandra.providers.claude import DEFAULT_API_URL as DEFAULT_ANTHROPIC_API_URL
from cassandra.providers.codex import DEFAULT_API_URL as DEFAULT_OPENAI_API_URL
from cassandra.providers.gemini import DEFAULT_API_URL as DEFAULT_GEMINI_API_URL
...
if TYPE_CHECKING:
    from cassandra.backends.base import Backend
...
class ProxyProviderRuntime:
    ...
    def select_passthrough_base_url(self, headers: Mapping[str, str]) -> str:
        ...
        if headers.get("api-key"):
            azure_base = headers.get("x-cassandra-base-url", "")
            if azure_base:
...
def _load_anyllm_backend() -> Any:
    ...
    if AnyLLMBackendType is None:
        from cassandra.backends.anyllm import AnyLLMBackend
...
def _load_litellm_backend() -> Any:
    ...
    if LiteLLMBackendType is None:
        from cassandra.backends.litellm import LiteLLMBackend
