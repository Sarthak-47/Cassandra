...
from cassandra.proxy.handlers.openai import _resolve_codex_routing_headers

logger = logging.getLogger("cassandra.proxy.routes")
...
def _vertex_target_for_location(proxy: Any, location: str) -> str:
    ...
    """
    from cassandra.providers.registry import DEFAULT_VERTEX_API_URL
...
async def _handle_chatgpt_codex_images(
    ...
    from cassandra.proxy.helpers import _strip_internal_headers
...
def register_provider_routes(app: FastAPI, proxy: Any) -> None:
    ...
    # cassandra/proxy/handlers/bedrock.py for the SigV4 caveat.
    ...
    async def passthrough(request: Request, path: str):
        custom_base = request.headers.get("x-cassandra-base-url")
        if custom_base:
