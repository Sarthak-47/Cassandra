...
from .config import CASSANDRA_OPENCODE_PLUGIN
...
def build_opencode_config_content(
    *,
    ...
    Runtime wrap injects the Cassandra provider as a stable explicit fallback,
    plus the Cassandra plugin which transparently routes provider fetch traffic
    ...
    if include_plugin:
        config["plugin"] = [[CASSANDRA_OPENCODE_PLUGIN, {"proxyUrl": base_url}]]
    return config
...
def build_launch_env(
    ...
    *,
    ...
    """Build environment variables for launching OpenCode through Cassandra.

    ``OPENCODE_CONFIG_CONTENT`` carries Cassandra provider/MCP/plugin config.
    ...
    display = ["OPENCODE_CONFIG_CONTENT={provider: cassandra}"]
    if include_plugin:
        display.append(f"plugin={CASSANDRA_OPENCODE_PLUGIN}")

    if project and "CASSANDRA_PROJECT" not in env:
        env["CASSANDRA_PROJECT"] = project
