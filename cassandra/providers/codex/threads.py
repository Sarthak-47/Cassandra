"""Reconcile Codex thread provider tags across the Cassandra proxy boundary.

Codex stamps every thread with the ``model_provider`` it ran under and filters
its history/projects menu by the active provider set.  When Cassandra rewrites
Codex's config to route through the custom ``cassandra`` provider (see
:mod:`cassandra.providers.codex.install`), threads created through Cassandra are
tagged ``cassandra`` while native threads keep ``openai`` -- so the two sets never
appear in the same menu, and connecting Cassandra appears to "lose" history.

To keep the menu whole we retag threads to match whichever provider is active:
``openai -> cassandra`` when Cassandra is enabled, ``cassandra -> openai`` when it is
...
def retag_to_cassandra(codex_home: Path) -> None:
    """Pull existing native threads into the cassandra-provider menu (on enable)."""
    retag_thread_providers(codex_home, frm=NATIVE_PROVIDER, to=CASSANDRA_PROVIDER)


def retag_to_native(codex_home: Path) -> None:
    """Hand threads back to the native-provider menu (on revert)."""
    retag_thread_providers(codex_home, frm=CASSANDRA_PROVIDER, to=NATIVE_PROVIDER)
