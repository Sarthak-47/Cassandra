...
from cassandra.install.models import ConfigScope, DeploymentManifest, ManagedMutation, ToolTarget
from cassandra.install.paths import codex_config_path

from .runtime import proxy_base_url
from .threads import retag_to_cassandra, retag_to_native

_CODEX_MARKER_START = "# --- Cassandra persistent provider ---"
_CODEX_MARKER_END = "# --- end Cassandra persistent provider ---"
...
_ORPHAN_MODEL_PROVIDER = re.compile(r'(?m)^[ \t]*model_provider[ \t]*=[ \t]*"cassandra"[ \t]*\r?\n')
_ORPHAN_OPENAI_BASE_URL = re.compile(
    r'(?m)^[ \t]*openai_base_url[ \t]*=[ \t]*"http://127\.0\.0\.1:\d+/v1"[ \t]*\r?\n'
)
_ORPHAN_CASSANDRA_TABLE = re.compile(
...
def revert_provider_scope(mutation: ManagedMutation, manifest: DeploymentManifest) -> None:
    ...
    # left outside the marker block (mirrors wrap.py _strip_codex_cassandra_blocks).
    content = _ORPHAN_MODEL_PROVIDER.sub("", content)
    content = _ORPHAN_OPENAI_BASE_URL.sub("", content)
    content = _ORPHAN_CASSANDRA_TABLE.sub("", content)
    path.write_text(content.strip() + "\n", encoding="utf-8")
    # Hand the threads back to the native-provider menu so the full history stays
    # visible once Codex no longer routes through Cassandra. Best-effort.
