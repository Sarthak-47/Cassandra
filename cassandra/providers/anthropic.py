"""Anthropic provider implementation for Cassandra SDK.
...
def _load_custom_model_config() -> dict[str, Any]:
    ...
    Checks (in order):
    1. CASSANDRA_MODEL_LIMITS environment variable (JSON string or file path)
    2. ~/.cassandra/models.json config file
    ...
    env_config = os.environ.get("CASSANDRA_MODEL_LIMITS", "")
    if env_config:
        try:
            ...
            logger.debug(f"Loaded custom model config from CASSANDRA_MODEL_LIMITS: {loaded}")
        except (json.JSONDecodeError, OSError) as e:
            logger.warning(f"Failed to load CASSANDRA_MODEL_LIMITS: {e}")
...
class AnthropicTokenCounter(TokenCounter):
    ...
    def __init__(self, model: str, client: Any = None, warn: bool = True):
        ...
        try:
            from cassandra.tokenizers.tiktoken_counter import (
                TiktokenLoadError,
                load_encoding,
            )
...
class AnthropicProvider(Provider):
    ...
    Custom Model Configuration:
        ...
        1. Environment variable (JSON string):
           export CASSANDRA_MODEL_LIMITS='{"context_limits": {"my-model": 200000}}'

        2. Environment variable (file path):
           export CASSANDRA_MODEL_LIMITS=/path/to/models.json

        3. Config file (~/.cassandra/models.json):
           {
             "anthropic": {
    ...
    def get_context_limit(self, model: str) -> int:
        ...
        2. CASSANDRA_MODEL_LIMITS environment variable
        3. ~/.cassandra/models.json config file
    ...
    def _warn_unknown_model(self, model: str, limit: int, reason: str) -> None:
        ...
        if model not in _UNKNOWN_MODEL_WARNINGS:
            ...
            logger.warning(
                ...
                f"To configure explicitly, set CASSANDRA_MODEL_LIMITS env var or "
                f"add to ~/.cassandra/models.json"
            )
