"""Analyze cassandra proxy logs for performance insights.

Parses PERF log lines from ~/.cassandra/logs/proxy.log* and produces
...
# Matches: 2026-03-07 13:38:31,009 - cassandra.proxy - INFO - [hr_...] PERF model=... ...
_PERF_RE = re.compile(
...
def parse_log_files(last_n_hours: float = 168.0) -> PerfReport:
    ...
    log_dir = _paths.log_dir() if os.environ.get("CASSANDRA_WORKSPACE_DIR") else LOG_DIR
    ...
    def _within_window(ts_str: str | None) -> bool:
        ...
        # alternative (silent drop) makes `cassandra perf` lie about coverage.
        if cutoff is None:
            return True
...
def _context_tool_lifetime_savings() -> dict | None:
    ...
    ``proxy.log``, so without this it stays invisible in ``cassandra perf`` even
    ...
    """
    try:
        from cassandra.proxy.helpers import _get_context_tool_stats
...
def format_report(report: PerfReport) -> str:
    ...
    if not report.perf_records and not report.router_records:
        if cli_filtering_lines:
            ...
            lines.append("No proxy performance data in ~/.cassandra/logs/ for this window.")
            lines.append("")
            lines.extend(cli_filtering_lines)
        else:
            lines.append("No performance data found in ~/.cassandra/logs/")
            lines.append("")
            lines.append("Start the proxy to begin collecting data:")
            lines.append("  cassandra proxy")
        return "\n".join(lines)

    # Header
    lines.append("Cassandra Performance Report")
    lines.append("=" * 60)
    ...
    # proxy.log, so surface it here or it stays invisible in `cassandra perf`.
    lines.extend(cli_filtering_lines)
...
def _format_toin_highlights() -> list[str]:
    ...
    try:
        from cassandra.telemetry.toin import get_toin as _get
    ...
    lines.append(
        ...
        f"(eligible for `python -m cassandra.cli.toin_publish`)"
    )
    lines.append("")
