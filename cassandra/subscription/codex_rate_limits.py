...
(``x-codex-primary-used-percent`` etc.) and cassandra captured those headers
...
class CodexRateLimitState(QuotaTracker):
    ...
    be registered with the :class:`~cassandra.subscription.base.QuotaTrackerRegistry`.
    ...
    cassandra, so :meth:`start` and :meth:`stop` are no-ops.
    """
