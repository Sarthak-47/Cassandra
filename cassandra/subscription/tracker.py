...
- Thread-safe state updates via threading.Lock (consistent with cassandra patterns)
...
token breakdowns per window that enable:
  - Cassandra efficiency metrics (tokens saved = raw - what proxy sent)
...
from cassandra.subscription.base import QuotaTracker
from cassandra.subscription.client import SubscriptionClient
...
_PERSIST_FILE_ENV = _paths.CASSANDRA_SUBSCRIPTION_STATE_PATH_ENV
_DEFAULT_PERSIST_DIR = ".cassandra"
...
# without uninstalling the binary or unsetting ``CASSANDRA_CONTEXT_TOOL``.
# Used for diagnostics and for environments where RTK is intentionally
# excluded from cassandra accounting (e.g. shadow tests).
#
...
_RTK_WIRING_ENV = "CASSANDRA_RTK_WIRING"
...
# ``cassandra/proxy/server.py``: a non-blocking ``fcntl.flock`` on
# ``CASSANDRA_RTK_POLL_LOCK`` (default under the workspace dir). Only the
...
_RTK_POLL_LOCK_ENV = "CASSANDRA_RTK_POLL_LOCK"
...
def _validate_rtk_env_at_startup() -> None:
    ...
    Raises ``ValueError`` loudly if ``CASSANDRA_RTK_WIRING`` is set to an
...
class SubscriptionTracker(QuotaTracker):
    ...
    Implements :class:`~cassandra.subscription.base.QuotaTracker` so it can
    be registered with :func:`~cassandra.subscription.base.get_quota_registry`
    ...
    def __init__(
        ...
        # :func:`~cassandra.proxy.helpers._get_context_tool_stats`) so it
        ...
        # entire pre-Cassandra RTK history as one fake delta. Switching to
    ...
    def update_contribution(
        ...
        *,
        ...
        """Update cassandra contribution counters for the current session window.
        ...
        :func:`cassandra.proxy.helpers._get_rtk_stats`) when the caller does
        ...
        reads 0 instead of the entire pre-Cassandra RTK history.
    ...
    def _poll_rtk_delta(self) -> int:
        ...
        :func:`cassandra.proxy.helpers._get_rtk_stats` and reads the
        ...
        - ``CASSANDRA_RTK_WIRING=disabled`` — operator opt-out.
        ...
        PR-G2 remediation (H1): ``CASSANDRA_RTK_WIRING`` is now validated at
        ...
        try:
            ...
            from cassandra.proxy.helpers import _get_rtk_stats
        ...
        # the pre-Cassandra RTK history. Falls back to the top-level
    ...
    def _try_acquire_rtk_poll_lock(self) -> bool:
        ...
        ``cassandra/proxy/server.py`` (fcntl.flock, LOCK_EX | LOCK_NB).
        """
    ...
    def _compute_used_since_reset(self, window: RateLimitWindow | None) -> int | None:
        ...
        if now < window.resets_at:
            return None
        try:
            from cassandra.subscription import session_tracking
    ...
    async def _maybe_poll(self) -> None:
        ...
        try:
            from cassandra.observability.metrics import get_otel_metrics
    ...
    def _maybe_reset_contribution(self, snapshot: SubscriptionSnapshot) -> None:
        ...
        if (
            ...
            logger.info("5h window rolled over; resetting cassandra contribution counters")
            self._state.contribution = CassandraContribution()
...
def configure_subscription_tracker(
    ...
    a typo (``CASSANDRA_RTK_WIRING=enabld``) crashes the proxy at startup
