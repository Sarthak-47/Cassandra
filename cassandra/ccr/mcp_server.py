"""Cassandra MCP Server — Context engineering toolkit for AI coding tools.

Exposes Cassandra's compression, retrieval, and observability as MCP tools
that any MCP-compatible host (Claude Code, Cursor, Codex, etc.) can use.

Tools:
    cassandra_compress   — Compress content on demand (no proxy needed)
    cassandra_retrieve   — Retrieve original uncompressed content by hash
    cassandra_stats      — Session compression statistics
...
CCR_TOOL_NAME = "cassandra_retrieve"
COMPRESS_TOOL_NAME = "cassandra_compress"
STATS_TOOL_NAME = "cassandra_stats"
READ_TOOL_NAME = "cassandra_read"

logger = logging.getLogger("cassandra.ccr.mcp")

# Feature flag: enable cassandra_read tool (file read caching via CCR)
# Set CASSANDRA_MCP_READ=on to enable
_READ_ENABLED = os.environ.get("CASSANDRA_MCP_READ", "off").lower().strip() in (
    "on",
    "true",
    "1",
    "yes",
...
DEFAULT_PROXY_URL = os.environ.get("CASSANDRA_PROXY_URL", "http://127.0.0.1:8787")
...
def _format_session_summary(summary: dict[str, Any], local_stats: dict[str, Any]) -> str:
    ...
    lines: list[str] = []
    lines.append("Cassandra Session Summary")
    lines.append("=" * 40)
    ...
    without = cost.get("without_cassandra_usd", 0)
    with_hr = cost.get("with_cassandra_usd", 0)
    saved = cost.get("total_saved_usd", 0)
    pct = cost.get("savings_pct", 0)
    if without > 0:
        lines.append("Cost Impact:")
        lines.append(f"  Without Cassandra:  ${without:.2f}")
        lines.append(f"  With Cassandra:     ${with_hr:.2f}")
...
# cassandra_stats aggregates across all instances within the session window.
# Respects CASSANDRA_WORKSPACE_DIR.
...
class CassandraMCPServer:
    """MCP Server exposing Cassandra's context engineering toolkit.

    Tools:
        cassandra_compress — Compress content on demand. Stores original for
                           retrieval. Works without a proxy.
        cassandra_retrieve — Retrieve original uncompressed content by hash.
                           Checks local store first, then proxy if configured.
        cassandra_stats    — Session statistics: compressions, savings, cost.
    ...
    def __init__(
        ...
        self.server: Server = Server("cassandra")
    ...
    def _compress_content(self, content: str) -> dict[str, Any]:
        """Compress content using Cassandra's pipeline.

        Returns dict with compressed text, token counts, hash, etc.
        """
        from cassandra.compress import compress
        ...
        # ``CompressResult`` in cassandra/compress.py — "0.0 = no savings, 1.0 =
        ...
        return {
            ...
            "note": f"Original stored with hash={hash_key}. Use mcp__cassandra__{CCR_TOOL_NAME} to get full content later.",
        }
    ...
    async def _retrieve_content(
        ...
        return {
            ...
            "compressed via cassandra_compress is stored for the session; content "
    ...
    def _setup_handlers(self) -> None:
        ...
        async def list_tools() -> list[Tool]:
            tools = [
                Tool(
                    ...
                    description=(
                        ...
                        f"The original is stored and can be retrieved later via mcp__cassandra__{CCR_TOOL_NAME}. "
                ...
                Tool(
                    ...
                    description=(
                        ...
                        "The hash comes from cassandra_compress results or from compression "
            ...
            # Conditionally add cassandra_read (behind feature flag)
            if _READ_ENABLED:
                tools.append(
                    Tool(
                        name=READ_TOOL_NAME,
                        description=(
                            ...
                            f"Use mcp__cassandra__{CCR_TOOL_NAME} with the hash to get full content if needed. "
    ...
    async def _handle_compress(self, arguments: dict[str, Any]) -> list[TextContent]:
        """Handle cassandra_compress tool call."""
        ...
        # Record durably so `cassandra savings` reflects this compression across
    ...
    def _record_savings(self, result: dict[str, Any]) -> None:
        ...
        savings_ledger.record_savings_event(
            ...
            model=os.environ.get("CASSANDRA_MCP_MODEL"),
    ...
    def _current_client(self) -> str:
        ...
        override = os.environ.get("CASSANDRA_MCP_CLIENT")
        if override:
            return override
        try:
    ...
    async def _handle_retrieve(self, arguments: dict[str, Any]) -> list[TextContent]:
        """Handle cassandra_retrieve tool call."""
    ...
    async def _handle_stats(self) -> list[TextContent]:
        """Handle cassandra_stats tool call."""
    ...
    async def _handle_read(self, arguments: dict[str, Any]) -> list[TextContent]:
        """Handle cassandra_read tool call — file read with session caching."""
        import hashlib
        ...
        # in cassandra/ccr/ — use the centralized safe-log decoder so
                            ...
                            text=json.dumps(
                                {
                                    ...
                                    "note": (
                                        ...
                                        f"from the first read. Call mcp__cassandra__{CCR_TOOL_NAME}(hash='{ccr_hash}') "
    ...
    async def run_stdio(self) -> None:
        ...
        async with stdio_server() as (read_stream, write_stream):
            logger.info(f"Cassandra MCP Server starting (proxy: {self.proxy_url})")
...
def create_ccr_mcp_server(
    ...
    """Create a Cassandra MCP server instance.

    Args:
        proxy_url: URL of the Cassandra proxy server (for retrieval fallback).
        direct_mode: Ignored (kept for backward compatibility).

    Returns:
        CassandraMCPServer instance.
    """
    return CassandraMCPServer(proxy_url=proxy_url)


async def main() -> None:
    """Run the Cassandra MCP server."""
    parser = argparse.ArgumentParser(
        description="Cassandra MCP Server — Context engineering toolkit"
    )
    parser.add_argument(
        "--proxy-url",
        default=DEFAULT_PROXY_URL,
        help=f"Cassandra proxy URL for retrieval fallback (default: {DEFAULT_PROXY_URL})",
    )
    parser.add_argument(
        "--direct",
        action="store_true",
    ...
    server = CassandraMCPServer(proxy_url=args.proxy_url)
