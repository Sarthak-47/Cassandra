...
statistics to the Cassandra cloud for billing. Designed to be non-intrusive:
...
logger = logging.getLogger("cassandra.telemetry.reporter")

# Grace period: if the cloud API is unreachable, use cached license for up to 7 days
GRACE_PERIOD_SECONDS = 7 * 24 * 3600  # 7 days

# Default cache location (workspace bucket, respects CASSANDRA_WORKSPACE_DIR).
...
class UsageReporter:
    ...
    usage stats to the Cassandra cloud. If the cloud is unreachable, the proxy
    ...
    def __init__(
        self,
        license_key: str,
        cloud_url: str = "https://app.cassandralabs.ai",
        ...
        self._proxy: CassandraProxy | None = None
    ...
    async def start(self, proxy: CassandraProxy) -> None:
    ...
    async def _get_client(self) -> httpx.AsyncClient:
        if self._http_client is None:
            self._http_client = httpx.AsyncClient(
                ...
                headers={"User-Agent": "cassandra-proxy"},
            )
