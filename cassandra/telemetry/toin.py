...
happened so an offline aggregator (`cassandra.cli.toin_publish`) can emit
...
# Usage
    ...
    #   python -m cassandra.cli.toin_publish --output recommendations.toml
...
TOIN_PATH_ENV_VAR = "CASSANDRA_TOIN_PATH"

# Default TOIN storage directory and file
DEFAULT_TOIN_DIR = ".cassandra"
...
def get_default_toin_storage_path() -> str:
    ...
    Checks for the CASSANDRA_TOIN_PATH environment variable first.
    Falls back to ``${CASSANDRA_WORKSPACE_DIR}/toin.json`` (which defaults
    to ``~/.cassandra/toin.json``) when unset.

    Returns:
        The path string for TOIN storage.
    """
    # Preserve legacy behavior: when CASSANDRA_TOIN_PATH is set we return the
...
class TOINConfig:
    ...
    # Storage
    # Default path is ~/.cassandra/toin.json (or CASSANDRA_TOIN_PATH env var)
...
class ToolIntelligenceNetwork:
    """Aggregates tool patterns across all Cassandra users (observation-only).
    ...
    `cassandra.cli.toin_publish` and consumed by the Rust proxy at startup.
    ...
    def get_recommendation(
        ...
        offline `cassandra.cli.toin_publish` CLI into `recommendations.toml`
        ...
        if not cls._DEPRECATION_WARNED:
            ...
            warnings.warn(
                ...
                "startup. See cassandra/telemetry/toin.py module docstring.",
                DeprecationWarning,
                stacklevel=2,
            )
        return None
    ...
    def iter_patterns(self) -> list[tuple[PatternKey, ToolPattern]]:
        ...
        Used by `cassandra.cli.toin_publish` to walk every aggregated
    ...
    def import_patterns(self, data: dict[str, Any]) -> None:
        ...
        Cassandra instances without sharing actual data.
...
TOIN_BACKEND_ENV_VAR = "CASSANDRA_TOIN_BACKEND"


def _create_default_toin_backend() -> Any:
    """Create a TOIN backend from env (e.g. CASSANDRA_TOIN_BACKEND=redis).

    Loads adapters via setuptools entry point 'cassandra.toin_backend'.
    ...
    try:
        ...
        all_eps = entry_points(group="cassandra.toin_backend")
        ep = next((e for e in all_eps if e.name == backend_type), None)
        if ep is None:
            logger.warning(
                "CASSANDRA_TOIN_BACKEND=%s but no entry point cassandra.toin_backend[%s]",
                backend_type,
                backend_type,
            )
            return None
        ...
        kwargs = {
            "url": os.environ.get("CASSANDRA_TOIN_URL", ""),
            "tenant_prefix": os.environ.get("CASSANDRA_TOIN_TENANT_PREFIX", ""),
        }
        return fn(**kwargs)
    except Exception as e:
...
def get_toin(config: TOINConfig | None = None) -> ToolIntelligenceNetwork:
    ...
    On first call, checks CASSANDRA_TOIN_BACKEND env var. If set, loads the
    backend via setuptools entry point 'cassandra.toin_backend'. Otherwise
