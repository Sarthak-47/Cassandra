...
* ``cassandra_stack`` — how Cassandra is being invoked
...
# from unbounded label explosion when clients send arbitrary X-Cassandra-Stack
# header values.
...
def detect_install_mode(port: int) -> str:
    ...
    1. ``CASSANDRA_AGENT_TYPE`` env var set → ``wrapped`` (spawned by ``cassandra wrap``).
    ...
    try:
        if os.environ.get("CASSANDRA_AGENT_TYPE"):
            return "wrapped"
...
def detect_stack(stats: dict[str, Any] | None = None) -> str:
    """Classify how Cassandra is being invoked.

    Resolution order:

    1. ``CASSANDRA_STACK`` env var set → use that slug verbatim.
    2. ``CASSANDRA_AGENT_TYPE`` env var set → ``wrap_<agent>``.
    ...
    try:
        explicit = normalize_stack(os.environ.get("CASSANDRA_STACK"))
        if explicit:
            return explicit

        agent_type = os.environ.get("CASSANDRA_AGENT_TYPE")
        if agent_type:
