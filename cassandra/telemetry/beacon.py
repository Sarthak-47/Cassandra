"""Telemetry opt-in state for Cassandra.

Cassandra collects only **local**, aggregate telemetry (tokens saved, compression
ratios, timings) for the in-process collector and the ``/stats`` /
``/v1/telemetry`` endpoints. **Nothing is sent to Cassandra Labs:** the anonymous
...
via ``CASSANDRA_OTEL_METRICS_*`` (a destination you control).

This module holds the ``CASSANDRA_TELEMETRY`` opt-in predicate (off by default)
...
def is_telemetry_enabled() -> bool:
    ...
    Fail-closed: only enabled when CASSANDRA_TELEMETRY is set to an explicit
    ...
    transmitted to Cassandra Labs.
    """
    from cassandra.offline import is_offline

    if is_offline():
        return False
    val = os.environ.get("CASSANDRA_TELEMETRY", "").lower().strip()
...
def is_telemetry_warn_enabled() -> bool:
    ...
    Set CASSANDRA_TELEMETRY_WARN=off to suppress startup/wrap notices.
    ...
    """
    val = os.environ.get("CASSANDRA_TELEMETRY_WARN", "on").lower().strip()
...
def format_telemetry_notice(*, prefix: str = "") -> str:
    ...
    return (
        ...
        "Disable: CASSANDRA_TELEMETRY=off or --no-telemetry"
    )
