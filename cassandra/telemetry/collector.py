...
def get_telemetry_collector(
    ...
    if _telemetry_collector is None:
        with _collector_lock:
            if _telemetry_collector is None:
                # Honour CASSANDRA_TELEMETRY (the documented opt-out var; see
                # the predicate in telemetry/beacon.py). Collection is local
                # only — nothing is sent externally.
                # Pre-#390 this only checked CASSANDRA_TELEMETRY_DISABLED,
                # so users who set CASSANDRA_TELEMETRY=off (the value in
                # the docs) still saw /v1/telemetry report enabled=true.
                # CASSANDRA_TELEMETRY_DISABLED stays accepted for back-compat.
                from cassandra.telemetry.beacon import is_telemetry_enabled

                disabled_legacy = os.environ.get("CASSANDRA_TELEMETRY_DISABLED", "").lower() in (
                    "1",
                    "true",
                )
