...
# Load the UTF-8-forcing subprocess wrapper WITHOUT importing the `cassandra`
# package. The release workflow runs this file as a bare script
# (`python cassandra/release_version.py`), where `sys.path[0]` is `cassandra/`
# rather than the repo root, so `from cassandra._subprocess import run` fails
# with `ModuleNotFoundError: No module named 'cassandra'` — and even when it
# resolves, it drags in `cassandra/__init__.py` (the Rust `_core` import), which
...
try:  # normal package context (tests import `cassandra.release_version`)
    from cassandra._subprocess import run
except ModuleNotFoundError:  # bare-script context (the release workflow)
    import importlib.util as _ilu

    _spec = _ilu.spec_from_file_location(
        "_cassandra_subprocess", Path(__file__).resolve().parent / "_subprocess.py"
    )
