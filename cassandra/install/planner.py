...
from cassandra.providers.install_registry import build_install_target_envs
...
def build_manifest(
    *,
    ...
    base_env = {
        "CASSANDRA_PORT": str(port),
        "CASSANDRA_HOST": "127.0.0.1",
        "CASSANDRA_MODE": proxy_mode,
        "CASSANDRA_BACKEND": backend,
    }
    if anyllm_provider:
        base_env["CASSANDRA_ANYLLM_PROVIDER"] = anyllm_provider
    if region:
        base_env["CASSANDRA_REGION"] = region
    ...
    base_env["CASSANDRA_TELEMETRY"] = "on" if telemetry_enabled else "off"
    if memory_enabled:
        base_env["CASSANDRA_MEMORY_ENABLED"] = "1"
    ...
    container_name = f"cassandra-{normalized_profile}"
    return DeploymentManifest(
        ...
        image=image,
        service_name=f"cassandra-{normalized_profile}",
