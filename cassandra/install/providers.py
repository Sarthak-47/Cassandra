...
from cassandra.providers.install_registry import (
...
_ENV_MARKER_START = "# >>> cassandra persistent env >>>"
_ENV_MARKER_END = "# <<< cassandra persistent env <<<"
    ...
    for name, value in merged.items():
        previous = run(
            [
                ...
                "if ($null -eq $value) { '__CASSANDRA_UNSET__' } else { $value }",
            ],
            capture_output=True,
            text=True,
            check=True,
        ...
        mutations.append(
            ManagedMutation(
                ...
                data={
                    "name": name,
                    "scope": scope_name,
                    "previous": None if previous == "__CASSANDRA_UNSET__" else previous,
                },
            )
        )
    return mutations
