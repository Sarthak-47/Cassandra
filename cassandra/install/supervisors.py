...
from .runtime import resolve_cassandra_command
...
def _command_for_script(*parts: str) -> list[str]:
    return [*resolve_cassandra_command(), *parts]
...
def _linux_service_unit(manifest: DeploymentManifest, run_script: Path) -> tuple[Path, str]:
    ...
    content = f"""[Unit]
Description=Cassandra ({manifest.profile})
...
def _macos_launchd_plist(
    ...
    plist_path = base_dir / f"com.cassandra.{manifest.profile}.plist"
    ...
    keys = [
        ...
        "<dict>",
        "  <key>Label</key>",
        f"  <string>com.cassandra.{manifest.profile}</string>",
...
def _linux_task_spec(manifest: DeploymentManifest, ensure_script: Path) -> tuple[Path | None, str]:
    ...
    marker_start = f"# >>> cassandra {manifest.profile} >>>"
    marker_end = f"# <<< cassandra {manifest.profile} <<<"
    content = (
...
def install_supervisor(manifest: DeploymentManifest) -> list[ArtifactRecord]:
    ...
    if (
        ...
        unit_path, content = _linux_service_unit(manifest, artifact_paths["run-cassandra.sh"])
    ...
    if sys.platform.startswith("linux") and manifest.supervisor_kind == SupervisorKind.TASK.value:
        cron_path, content = _linux_task_spec(manifest, artifact_paths["ensure-cassandra.sh"])
    ...
    if sys.platform == "darwin":
        if manifest.supervisor_kind == SupervisorKind.SERVICE.value:
            plist_path, content = _macos_launchd_plist(manifest, artifact_paths["run-cassandra.sh"])
        else:
            plist_path, content = _macos_launchd_plist(
                manifest, artifact_paths["ensure-cassandra.sh"], interval=300
            )
...
def start_supervisor(manifest: DeploymentManifest) -> None:
    ...
    if sys.platform.startswith("linux"):
        ...
        return
    if sys.platform == "darwin":
        label = f"com.cassandra.{manifest.profile}"
        domain = (
            "system"
            if manifest.scope == "system"
            and manifest.supervisor_kind == SupervisorKind.SERVICE.value
