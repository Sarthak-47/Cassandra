"""Canonical filesystem contract for Cassandra.

This module defines the single source of truth for where Cassandra reads and
writes files. It introduces two canonical roots:

* ``CASSANDRA_CONFIG_DIR`` -- read-mostly configuration (defaults to
  ``~/.cassandra/config``). Holds model catalogs, plugin settings, and other
  configuration that users or admins edit.
* ``CASSANDRA_WORKSPACE_DIR`` -- read-write state (defaults to ``~/.cassandra``).
...
per-resource override (``CASSANDRA_SAVINGS_PATH``, ``CASSANDRA_TOIN_PATH``,
...
CASSANDRA_CONFIG_DIR_ENV = "CASSANDRA_CONFIG_DIR"
CASSANDRA_WORKSPACE_DIR_ENV = "CASSANDRA_WORKSPACE_DIR"

# ---------------------------------------------------------------------------
# Legacy per-resource env vars (kept for backward compatibility)
# ---------------------------------------------------------------------------

CASSANDRA_SAVINGS_PATH_ENV = "CASSANDRA_SAVINGS_PATH"
CASSANDRA_SAVINGS_EVENTS_PATH_ENV = "CASSANDRA_SAVINGS_EVENTS_PATH"
CASSANDRA_TOIN_PATH_ENV = "CASSANDRA_TOIN_PATH"
CASSANDRA_SUBSCRIPTION_STATE_PATH_ENV = "CASSANDRA_SUBSCRIPTION_STATE_PATH"

# ---------------------------------------------------------------------------
# Default sub-path fragments
# ---------------------------------------------------------------------------

_WORKSPACE_DIR_DEFAULT = ".cassandra"
...
def process_is_stateless() -> bool:
    ...
    True if ``set_process_stateless(True)`` was called OR the ``CASSANDRA_STATELESS``
    environment variable is set, so non-proxy entrypoints honor it too.
    """

    if _PROCESS_STATELESS:
        return True
    return _env("CASSANDRA_STATELESS").lower() in ("1", "true", "yes", "on")
...
def workspace_dir() -> Path:
    ...
    1. ``$CASSANDRA_WORKSPACE_DIR`` (trimmed, tilde-expanded) if set.
    2. ``~/.cassandra`` otherwise.
    """

    env_value = _env(CASSANDRA_WORKSPACE_DIR_ENV)
    if env_value:
        return Path(env_value).expanduser()
...
def config_dir() -> Path:
    ...
    1. ``$CASSANDRA_CONFIG_DIR`` (trimmed, tilde-expanded) if set.
    2. ``$CASSANDRA_WORKSPACE_DIR/config`` when the workspace env var is set
       so that a single override relocates both roots coherently.
    3. ``~/.cassandra/config`` otherwise.
    """

    env_value = _env(CASSANDRA_CONFIG_DIR_ENV)
    if env_value:
        return Path(env_value).expanduser()
    workspace_env = _env(CASSANDRA_WORKSPACE_DIR_ENV)
    if workspace_env:
...
def toin_path(explicit: str | os.PathLike[str] | None = None) -> Path:
    ...
    ``~/.cassandra/toin.json`` to preserve byte-for-byte backward compat.
    """
...
def savings_events_path(explicit: str | os.PathLike[str] | None = None) -> Path:
    ...
    MCP processes, and is the source of truth for ``cassandra savings``.
    """

    return _resolve(
        explicit,
        CASSANDRA_SAVINGS_EVENTS_PATH_ENV,
...
def log_dir() -> Path:
    """Return the directory for Cassandra log files."""
...
def bin_dir() -> Path:
    """Return the directory where Cassandra ships vendored binaries."""
...
def models_config_path() -> Path:
    ...
    Note: the ``CASSANDRA_MODEL_LIMITS`` env var is a *content* override
    ...
    location and deliberately ignores ``CASSANDRA_MODEL_LIMITS``.
    """
