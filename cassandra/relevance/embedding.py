"""Embedding-based relevance scorer for Cassandra SDK.
...
Install with: pip install cassandra[relevance]
...
# Set CASSANDRA_HF_PIN=off to bypass (mirrors cassandra.onnx_runtime pinning).
...
def _pinned_revision(model_name: str) -> str | None:
    ...
    Returns ``None`` for custom models or when ``CASSANDRA_HF_PIN=off``.
    """
...
class EmbeddingScorer(RelevanceScorer):
    ...
    Note:
        Requires fastembed: pip install cassandra[relevance]
    """
    ...
    def _get_model(self) -> TextEmbedding:
        ...
        if not self.is_available():
            raise RuntimeError(
                "EmbeddingScorer requires fastembed. Install with: pip install cassandra[relevance]"
            )
