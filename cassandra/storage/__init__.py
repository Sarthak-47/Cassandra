"""Storage modules for Cassandra SDK."""
...
def create_storage(store_url: str) -> Storage:
    ...
    the setuptools entry point cassandra.storage_backend with name=<scheme>.
    ...
    if store_url.startswith("sqlite://"):
        ...
        # Unknown scheme: try entry point cassandra.storage_backend[name=scheme]
        scheme = store_url.split("://", 1)[0].lower() if "://" in store_url else ""
        if scheme:
            try:
                from importlib.metadata import entry_points

                all_eps = entry_points(group="cassandra.storage_backend")
