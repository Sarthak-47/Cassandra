...
`pip install cassandra-ai` pulls `ast-grep-cli` as a proper PyPI binary wheel
...
Env vars:
    CASSANDRA_BINARIES_MIRROR   base URL that replaces https://github.com
    CASSANDRA_BINARIES_CACHE    override cache dir
    CASSANDRA_BINARIES_OFFLINE  if set, never reach the network
"""
...
class OfflineError(BinaryError):
    """Raised when a network fetch is required but CASSANDRA_BINARIES_OFFLINE is set."""
...
def cache_dir() -> Path:
    override = os.environ.get("CASSANDRA_BINARIES_CACHE")
    if override:
        return Path(override).expanduser().resolve()
    if sys.platform.startswith("win"):
        base = os.environ.get("LOCALAPPDATA") or str(Path.home() / "AppData" / "Local")
        return Path(base) / "cassandra" / "bin"
    if sys.platform == "darwin":
        return Path.home() / "Library" / "Caches" / "cassandra" / "bin"
    xdg = os.environ.get("XDG_CACHE_HOME")
    base = Path(xdg) if xdg else Path.home() / ".cache"
    return base / "cassandra" / "bin"
...
def _asset_for_platform(tool: str, plat: PlatformKey) -> dict[str, Any]:
    ...
    if _is_pypi_tool(tool):
        raise PlatformNotSupported(
            f"{tool}: distributed via PyPI only; `pip install cassandra-ai` "
...
def _mirror_url(url: str) -> str:
    mirror = os.environ.get("CASSANDRA_BINARIES_MIRROR")
    if not mirror:
        return url
...
def _download(url: str, dest: Path, *, progress: bool = True) -> None:
    if os.environ.get("CASSANDRA_BINARIES_OFFLINE"):
        raise OfflineError(f"offline mode (CASSANDRA_BINARIES_OFFLINE=1) but fetch required: {url}")
    ...
    req = urllib.request.Request(final_url, headers={"User-Agent": "cassandra-binaries/1"})
    try:
...
def resolve(tool: str) -> Path:
    ...
    OfflineError if a fetch is required but CASSANDRA_BINARIES_OFFLINE is set,
    ...
    with tempfile.TemporaryDirectory(prefix="cassandra-fetch-") as tmp:
        tmp_dir = Path(tmp)
        ...
        # `cassandra proxy` starts don't race on the same .partial path.
...
def ensure_tools(quiet: bool = False) -> dict[str, Path | None]:
    ...
    Called at proxy startup and on first `cassandra` CLI invocation so that no
    ...
    for name in _registry().get("tools", {}):
        try:
            ...
            out[name] = None
            if not quiet:
                print(f"cassandra: skipping {name}: {e}", file=sys.stderr)
    return out
...
def status() -> list[dict[str, Any]]:
    ...
    Used by `cassandra tools doctor`. Never fetches — only inspects.
    """
