"""Configuration models for Cassandra SDK."""
...
from cassandra.models.config import ML_MODEL_DEFAULTS


class CassandraMode(str, Enum):
    """Operating modes for Cassandra."""
...
class RelevanceScorerConfig:
    ...
    For full hybrid support, install: pip install cassandra[relevance]
...
# CASSANDRA_PROTECT_TOOL_RESULTS=Bash or --protect-tool-results Bash.
...
class ReadMaturationConfig:
    ...
    Motivation (measured by `cassandra audit-reads`): the median Read stays
...
class CacheOptimizerConfig:
    ...
    auto_detect_provider: bool = True  # Auto-detect from CassandraClient provider
...
class CCRConfig:
    ...
    4. RETRIEVE: If LLM needs more, it calls cassandra_retrieve(hash) to get the full original back
    ...
    # with Rust DEFAULT_TTL (crates/cassandra-core/src/ccr/mod.rs) and
    ...
    inject_tool: bool = True  # Inject cassandra_retrieve tool into tools array
...
class PrefixFreezeConfig:
    ...
    Cassandra from invalidating the provider's prefix cache (which would replace
...
class CassandraConfig:
    """Main configuration for CassandraClient."""

    store_url: str = "sqlite:///cassandra.db"
    default_mode: CassandraMode = CassandraMode.AUDIT
    ...
    # Env var CASSANDRA_INTERCEPT_ENABLED=1 also enables (for CLI `--intercept-tool-results`).
...
class DiffArtifact:
    ...
    Opt-in via CassandraConfig.generate_diff_artifact = True.
