"""OTEL tracing helpers for Cassandra and Langfuse."""
...
from .metrics import _cassandra_version, _parse_bool, _parse_key_value_pairs
...
_tracing_lock = Lock()
_global_tracer: CassandraTracer | None = None
...
class LangfuseTracingConfig:
    """Configuration for Cassandra-managed Langfuse OTLP trace export."""
    ...
    @classmethod
    def from_env(cls, *, default_service_name: str = "cassandra") -> LangfuseTracingConfig:
        public_key = os.environ.get("LANGFUSE_PUBLIC_KEY", "").strip()
        secret_key = os.environ.get("LANGFUSE_SECRET_KEY", "").strip()

        return cls(
            enabled=_parse_bool(
                os.environ.get("CASSANDRA_LANGFUSE_ENABLED"),
                default=False,
            ),
            ...
            service_name=os.environ.get(
                "CASSANDRA_LANGFUSE_SERVICE_NAME", default_service_name
            ).strip()
            or default_service_name,
            resource_attributes=_parse_key_value_pairs(
                os.environ.get("CASSANDRA_LANGFUSE_RESOURCE_ATTRIBUTES")
            ),
        )
...
class CassandraTracer:
    """Tracer facade used by shared Cassandra compression paths."""

    def __init__(self, tracer_provider: Any | None = None):
        if tracer_provider is None:
            self._tracer = trace.get_tracer(_SCOPE_NAME, _cassandra_version())
        else:
            self._tracer = tracer_provider.get_tracer(_SCOPE_NAME, _cassandra_version())
...
def get_cassandra_tracer() -> CassandraTracer:
    global _global_tracer

    if _global_tracer is None:
        with _tracing_lock:
            if _global_tracer is None:
                _global_tracer = CassandraTracer()

    return _global_tracer


def set_cassandra_tracer(cassandra_tracer: CassandraTracer) -> CassandraTracer:
    global _global_tracer
    with _tracing_lock:
        _global_tracer = cassandra_tracer
    return cassandra_tracer
...
def configure_langfuse_tracing(
    ...
    try:
        ...
        logger.warning(
            ...
            "Install cassandra-ai[otel] to enable Langfuse OTLP tracing."
        )
        return get_cassandra_tracer()
    ...
    cassandra_tracer = CassandraTracer(tracer_provider=tracer_provider)
...
def get_langfuse_tracing_status() -> dict[str, Any]:
    ...
    return LangfuseTracingConfig.from_env(default_service_name="cassandra-proxy").status()


def shutdown_cassandra_tracing() -> None:
    global _global_tracer
    global _owned_tracer_provider
    global _owned_langfuse_config
...
def reset_cassandra_tracing() -> None:
