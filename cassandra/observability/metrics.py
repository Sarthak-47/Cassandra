"""OpenTelemetry-backed operational metrics for Cassandra."""
...
class OTelMetricsConfig:
    """Configuration for Cassandra-managed OTEL metric export."""
    ...
    @classmethod
    def from_env(cls, *, default_service_name: str = "cassandra") -> OTelMetricsConfig:
        exporter_raw = (
            os.environ.get("CASSANDRA_OTEL_METRICS_EXPORTER", "otlp_http")
            .strip()
            .lower()
            .replace("-", "_")
        )
        if exporter_raw not in {"console", "otlp_http"}:
            logger.warning(
                "Unknown CASSANDRA_OTEL_METRICS_EXPORTER=%s; falling back to otlp_http",
                exporter_raw,
            )
            exporter_raw = "otlp_http"

        return cls(
            enabled=_parse_bool(os.environ.get("CASSANDRA_OTEL_METRICS_ENABLED"), default=False),
            service_name=os.environ.get("CASSANDRA_OTEL_SERVICE_NAME", default_service_name).strip()
            or default_service_name,
            exporter=exporter_raw,  # type: ignore[arg-type]
            endpoint=os.environ.get("CASSANDRA_OTEL_METRICS_ENDPOINT") or None,
            headers=_parse_key_value_pairs(os.environ.get("CASSANDRA_OTEL_METRICS_HEADERS")),
            export_interval_millis=_parse_int(
                os.environ.get("CASSANDRA_OTEL_METRICS_EXPORT_INTERVAL_MS"),
                _DEFAULT_EXPORT_INTERVAL_MS,
            ),
            resource_attributes=_parse_key_value_pairs(
                os.environ.get("CASSANDRA_OTEL_RESOURCE_ATTRIBUTES")
            ),
        )
...
class CassandraOtelMetrics:
    """Shared OTEL metrics facade for Cassandra operations."""

    def __init__(self, meter_provider: Any | None = None):
        if meter_provider is None:
            self._meter = metrics.get_meter(_SCOPE_NAME, _cassandra_version())
        else:
            self._meter = meter_provider.get_meter(_SCOPE_NAME, _cassandra_version())

        self._proxy_requests = self._meter.create_counter(
            "cassandra.proxy.requests",
            description="Proxy requests handled by Cassandra.",
            unit="1",
        )
        self._proxy_cached_requests = self._meter.create_counter(
            "cassandra.proxy.requests.cached",
        ...
        self._proxy_cache_write_ttl_tokens = self._meter.create_counter(
            "cassandra.proxy.cache.write_ttl_tokens",
        ...
        self._proxy_latency = self._meter.create_histogram(
            "cassandra.proxy.request.duration",
            description="End-to-end proxy request duration.",
            unit="s",
        )
        self._proxy_overhead = self._meter.create_histogram(
            "cassandra.proxy.overhead.duration",
            description="Time spent inside Cassandra optimization logic.",
            unit="s",
        )
        self._proxy_ttfb = self._meter.create_histogram(
            "cassandra.proxy.ttfb.duration",
            description="Upstream time to first byte observed by Cassandra.",
            unit="s",
        )
        self._compression_runs = self._meter.create_counter(
            "cassandra.compression.runs",
            description="Compression pipeline runs executed by Cassandra.",
            unit="1",
        )
        self._compression_failures = self._meter.create_counter(
            "cassandra.compression.failures",
...
def get_otel_metrics() -> CassandraOtelMetrics:
    global _global_metrics

    if _global_metrics is None:
        with _metrics_lock:
            if _global_metrics is None:
                _global_metrics = CassandraOtelMetrics()

    return _global_metrics


def set_otel_metrics(otel_metrics: CassandraOtelMetrics) -> CassandraOtelMetrics:
    global _global_metrics
    with _metrics_lock:
...
def configure_otel_metrics(config: OTelMetricsConfig | None = None) -> CassandraOtelMetrics:
    global _global_metrics
    ...
    try:
        ...
        logger.warning(
            ...
            "Install cassandra-ai[otel] to enable managed OTEL metric export."
        )
        return get_otel_metrics()
    ...
    resource = Resource.create(
        {
            ...
            SERVICE_VERSION: _cassandra_version(),
            **resolved.resource_attributes,
        }
    )
    meter_provider = MeterProvider(resource=resource, metric_readers=[reader])
    otel_metrics = CassandraOtelMetrics(meter_provider=meter_provider)
...
def get_otel_metrics_status() -> dict[str, Any]:
    ...
    return OTelMetricsConfig.from_env(default_service_name="cassandra-proxy").status()
