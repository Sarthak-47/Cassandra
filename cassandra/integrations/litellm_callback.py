"""LiteLLM callback — add Cassandra compression to LiteLLM with one line.
    ...
    litellm.callbacks = [CassandraCallback(api_key="hdr_xxx")]
...
class CassandraCallback:
    ...
    def __init__(
        ...
        # Falls back to CASSANDRA_API_KEY env var
        import os
    ...
    async def async_pre_call_hook(
        ...
        try:
            ...
            if result and result.get("tokens_saved", 0) > 0 and "messages" in result:
                ...
                logger.info(
                    "Cassandra%s: %d→%d tokens (saved %d, %.0f%%) [total saved: %d]",
            ...
            logger.warning("Cassandra compression failed, using original messages: %s", e)
