"""LangChain integration for Cassandra SDK.
...
Components:
1. CassandraChatModel - Wraps any BaseChatModel to apply Cassandra transforms
2. CassandraCallbackHandler - Tracks metrics and token usage (observability only)
3. CassandraRunnable - LCEL-compatible Runnable for chain composition
...
Example:
    ...
    optimized_llm = CassandraChatModel(llm)

    # Use normally - Cassandra automatically optimizes context
...
from cassandra.transforms import TransformPipeline

from .providers import get_cassandra_provider, get_model_name_from_langchain
...
def _check_langchain_available() -> None:
    ...
    if not LANGCHAIN_AVAILABLE:
        raise ImportError(
            ...
            "Install with: pip install cassandra[langchain] "
...
class CassandraChatModel(BaseChatModel):
    """LangChain chat model wrapper that applies Cassandra optimizations.

    Wraps any LangChain BaseChatModel and automatically optimizes the context
    before each API call. This is the recommended way to use Cassandra with
    LangChain because:
    ...
    Example:
        ...
        # Basic usage
        llm = ChatOpenAI(model="gpt-4o")
        optimized = CassandraChatModel(llm)
        response = optimized.invoke([HumanMessage("Hello!")])

        # With custom config
        from cassandra import CassandraConfig, CassandraMode
        config = CassandraConfig(default_mode=CassandraMode.OPTIMIZE)
        optimized = CassandraChatModel(llm, config=config)
    ...
    Attributes:
        ...
        cassandra_client: CassandraClient instance for optimization
    ...
    cassandra_config: Any = Field(default=None, description="Cassandra configuration")
    mode: CassandraMode = Field(default=CassandraMode.OPTIMIZE, description="Cassandra mode")
    ...
    def __init__(
        ...
        mode: CassandraMode = CassandraMode.OPTIMIZE,
        auto_detect_provider: bool = True,
        **kwargs: Any,
    ) -> None:
        """Initialize CassandraChatModel.

        Args:
            wrapped_model: Any LangChain BaseChatModel to wrap
            config: CassandraConfig for optimization settings
            mode: CassandraMode (AUDIT, OPTIMIZE, or SIMULATE)
            auto_detect_provider: Auto-detect provider from wrapped model.
                ...
                Cassandra provider for accurate token counting.
        ...
        super().__init__(  # type: ignore[call-arg]
            ...
            cassandra_config=config or CassandraConfig(),
            mode=mode,
    ...
    def _llm_type(self) -> str:
        ...
        return f"cassandra-{self.wrapped_model._llm_type}"
    ...
    def _identifying_params(self) -> dict[str, Any]:
        ...
        return {
            ...
            "cassandra_mode": self.mode.value,
        }
    ...
    def pipeline(self) -> TransformPipeline:
        ...
        if self._pipeline is None:
            if self.auto_detect_provider:
                self._provider = get_cassandra_provider(self.wrapped_model)
    ...
    def _convert_messages_to_openai(self, messages: list[BaseMessage]) -> list[dict[str, Any]]:
        """Convert LangChain messages to OpenAI format for Cassandra."""
        result = []
        for msg in messages:
    ...
    def _optimize_messages(
        ...
        """Apply Cassandra optimization to messages."""
        ...
        # Apply Cassandra transforms via pipeline
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        """Generate response with Cassandra optimization.
        ...
        logger.info(
            f"Cassandra optimized: {metrics.tokens_before} -> {metrics.tokens_after} tokens "
    ...
    def _stream(
        ...
        **kwargs: Any,
        ...
        """Stream response with Cassandra optimization."""
        # Optimize messages
        optimized_messages, metrics = self._optimize_messages(messages)

        logger.info(
            f"Cassandra optimized (streaming): {metrics.tokens_before} -> "
    ...
    async def _agenerate(
        ...
        **kwargs: Any,
        ...
        """Async generate response with Cassandra optimization.
        ...
        logger.info(
            f"Cassandra optimized (async): {metrics.tokens_before} -> {metrics.tokens_after} tokens "
            f"({metrics.savings_percent:.1f}% saved)"
        )
        ...
        # same CassandraChatModel instance. (GitHub #1285, review feedback)
    ...
    async def _astream(
        ...
        **kwargs: Any,
        ...
        """Async stream response with Cassandra optimization.
        ...
        logger.info(
            f"Cassandra optimized (async streaming): {metrics.tokens_before} -> "
            f"{metrics.tokens_after} tokens"
        )
    ...
    def bind_tools(self, tools: Sequence[Any], **kwargs: Any) -> CassandraChatModel:
        """Bind tools to the wrapped model."""
        new_wrapped = self.wrapped_model.bind_tools(tools, **kwargs)
        return CassandraChatModel(
...
class CassandraCallbackHandler(BaseCallbackHandler):
    """LangChain callback handler for Cassandra metrics and observability.

    NOTE: Callbacks CANNOT modify messages in LangChain (by design).
    Use CassandraChatModel for actual optimization. This handler is for:
...
class CassandraRunnable:
    """LCEL-compatible Runnable for Cassandra optimization.

    Use this to add Cassandra optimization to any LangChain chain using LCEL.

    Example:
        from langchain_openai import ChatOpenAI
        from langchain_core.prompts import ChatPromptTemplate
        from cassandra.integrations import CassandraRunnable
        ...
        # Add Cassandra optimization to chain
        chain = prompt | CassandraRunnable() | llm
...
def optimize_messages(
    ...
    config = config or CassandraConfig()
