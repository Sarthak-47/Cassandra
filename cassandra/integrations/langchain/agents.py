...
This module provides CassandraToolWrapper and wrap_tools_with_cassandra
...
Example:
    ...
    # Wrap with Cassandra compression
    wrapped_tools = wrap_tools_with_cassandra(tools)
...
class CassandraToolWrapper:
    ...
    Example:
        from langchain.tools import Tool
        from cassandra.integrations import CassandraToolWrapper
        ...
        wrapped = CassandraToolWrapper(search_tool)
    ...
    def __init__(
        ...
        """Initialize CassandraToolWrapper.
    ...
    def _record_metrics(self, original: str, compressed: str, was_compressed: bool) -> None:
        ...
        if was_compressed and chars_saved > 0:
            logger.info(
                f"CassandraToolWrapper[{self.name}]: {chars_before} -> {chars_after} chars "
...
def wrap_tools_with_cassandra(
    tools: list[BaseTool],
    min_chars_to_compress: int = 1000,
    metrics_collector: ToolMetricsCollector | None = None,
) -> list[StructuredTool]:
    """Wrap multiple LangChain tools with Cassandra compression.
