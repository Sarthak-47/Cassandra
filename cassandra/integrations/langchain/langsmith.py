"""LangSmith integration for Cassandra compression metrics.

This module provides CassandraLangSmithCallbackHandler, a LangChain callback
handler that adds Cassandra compression metrics to LangSmith traces.

When used with CassandraChatModel, it automatically captures:
...
Example:
    ...
    # Create handler
    handler = CassandraLangSmithCallbackHandler()

    # Use with CassandraChatModel
    llm = CassandraChatModel(
        ChatOpenAI(model="gpt-4o"),
        callbacks=[handler],
    )

    # Traces will include cassandra.* metadata
...
class CassandraLangSmithCallbackHandler(BaseCallbackHandler):
    """Callback handler that adds Cassandra metrics to LangSmith traces.

    Integrates with LangSmith to provide visibility into context
    optimization within traces. Metrics appear as metadata with
    the `cassandra.` prefix.

    Works automatically when:
    1. LANGCHAIN_TRACING_V2=true is set
    2. Used as a callback with CassandraChatModel
    ...
    Example:
        ...
        # - cassandra.savings_percent
        # - cassandra.transforms_applied
    ...
    def __init__(
        ...
        """Initialize CassandraLangSmithCallbackHandler.
        ...
        Args:
            ...
            auto_update_runs: If True, automatically updates LangSmith
                runs with Cassandra metadata. Default True.
        """
    ...
    def set_cassandra_metrics(
        ...
        """Set Cassandra metrics for a run.

        Call this from CassandraChatModel after optimization to attach
        ...
        logger.debug(
            f"Cassandra metrics set for run {run_id_str}: "
    ...
    def on_llm_end(
        ...
        **kwargs: Any,
        ...
        Attaches pending Cassandra metrics to the LangSmith run.
        """
        run_id_str = str(run_id)
    ...
    def _attach_metrics_to_run(self, run_id: str, metrics: PendingMetrics) -> None:
        """Attach Cassandra metrics to a LangSmith run.
        ...
        """
        metadata = {
            "cassandra.tokens_before": metrics.tokens_before,
            "cassandra.tokens_after": metrics.tokens_after,
            "cassandra.tokens_saved": metrics.tokens_saved,
            "cassandra.savings_percent": round(metrics.savings_percent, 2),
            "cassandra.transforms_applied": metrics.transforms_applied,
            "cassandra.optimization_timestamp": metrics.timestamp.isoformat(),
        }
        ...
        if self._client and self._auto_update:
            try:
                ...
                logger.debug(f"Updated LangSmith run {run_id} with Cassandra metrics")
            except Exception as e:
                logger.debug(f"Could not update LangSmith run: {e}")

    def get_run_metrics(self, run_id: str | UUID) -> dict[str, Any]:
        """Get Cassandra metrics for a specific run.

        Args:
            run_id: The run ID.

        Returns:
            Dictionary of cassandra.* metrics for the run.
        """
    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        total_saved = sum(m.get("cassandra.tokens_saved", 0) for m in self._run_metrics.values())
        savings_percents = [
            m.get("cassandra.savings_percent", 0) for m in self._run_metrics.values()
        ]
