...
class paths to identify the appropriate Cassandra provider.

Example:
    from langchain_anthropic import ChatAnthropic
    from cassandra.integrations.langchain import get_cassandra_provider

    model = ChatAnthropic(model="claude-3-5-sonnet-20241022")
    provider = get_cassandra_provider(model)  # Returns AnthropicProvider
"""
...
def get_cassandra_provider(model: Any) -> Provider:
    """Get appropriate Cassandra Provider instance for a LangChain model.

    This function automatically detects the provider from the model type
    and returns a configured Cassandra provider for accurate token counting
    ...
    Returns:
        Configured Cassandra Provider instance
