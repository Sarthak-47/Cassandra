"""ASGI Middleware — add Cassandra compression to any Python proxy.
...
Cloud mode (managed CCR, TOIN, analytics via Cassandra Cloud):
...
_DEFAULT_CLOUD_URL = "https://api.cassandralabs.ai"
...
class CompressionMiddleware:
    ...
    Two modes:
    - Local (default): Compresses in-process using cassandra.compress().
    - Cloud (api_key set): Calls Cassandra Cloud API for managed compression
      with org-scoped CCR, TOIN learning, and analytics dashboards.

    Response headers include compression metrics:
    - x-cassandra-tokens-before: original token count
    - x-cassandra-tokens-after: compressed token count
    - x-cassandra-tokens-saved: tokens removed
    - x-cassandra-compressed: "true" if compression occurred
    """
    ...
    def __init__(
        ...
        # Cloud mode: if api_key is set, compress via Cassandra Cloud API
        self._api_key = api_key or os.environ.get("CASSANDRA_API_KEY", "").strip() or None
        self._api_url = (
            api_url or os.environ.get("CASSANDRA_API_URL", "").strip() or _DEFAULT_CLOUD_URL
        ).rstrip("/")
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        try:
            ...
            if messages:
                ...
                if result and result.get("tokens_saved", 0) > 0 and "messages" in result:
                    ...
                    logger.info(
                        "Cassandra%s: %d→%d tokens (saved %d, %.0f%%)",
            ...
            logger.debug("Cassandra middleware: skipping non-JSON request: %s", e)
        ...
        async def metrics_send(message: MutableMapping[str, Any]) -> None:
            if message["type"] == "http.response.start" and tokens_saved > 0:
                ...
                headers.append((b"x-cassandra-compressed", b"true"))
                headers.append((b"x-cassandra-tokens-before", str(tokens_before).encode()))
                headers.append((b"x-cassandra-tokens-after", str(tokens_after).encode()))
                headers.append((b"x-cassandra-tokens-saved", str(tokens_saved).encode()))
    ...
    def _local_compress(self, messages: list[dict], model: str) -> dict[str, Any] | None:
        """Compress locally using cassandra.compress()."""
    ...
    async def _cloud_compress(self, messages: list[dict], model: str) -> dict[str, Any] | None:
        """Compress via Cassandra Cloud API (managed CCR, TOIN, analytics)."""
        if self._client is None:
            try:
                import httpx
            except ImportError as e:
                raise ImportError(
                    "httpx is required for Cassandra Cloud mode: pip install httpx"
                ) from e
        ...
        if resp.status_code != 200:
            logger.warning("Cassandra Cloud API error: %d %s", resp.status_code, resp.text[:200])
            return None
