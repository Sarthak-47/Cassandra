"""Agno hooks for Cassandra integration.

This module provides pre_hooks and post_hooks that can be used with
Agno agents to apply Cassandra optimization at the agent level.
"""
...
@dataclass
class HookMetrics:
    """Metrics collected by Cassandra pre-hooks.

    Note: These metrics track request counts and timing, not token savings.
    For actual token optimization metrics, use CassandraAgnoModel which
    ...
    # Use CassandraAgnoModel for actual token optimization
    tokens_before: int = 0
    tokens_after: int = 0
    tokens_saved: int = 0
...
class CassandraPreHook:
    ...
    optimization, use CassandraAgnoModel to wrap your model.
    ...
    using CassandraAgnoModel.

    Example:
        from agno.agent import Agent
        from agno.models.openai import OpenAIChat
        from cassandra.integrations.agno import CassandraPreHook, CassandraAgnoModel
    ...
    def __init__(
        ...
        """Initialize CassandraPreHook.

        Args:
            config: CassandraConfig for optimization settings (stored for future use)
            mode: CassandraMode (stored for future use)
    ...
    def __call__(self, run_input: Any, **kwargs: Any) -> Any:
        ...
        logger.debug(f"CassandraPreHook tracking request {request_id}")
        ...
        # Return input unchanged (use CassandraAgnoModel for actual optimization)
        return run_input
...
def create_cassandra_hooks(
    config: CassandraConfig | None = None,
    mode: CassandraMode = CassandraMode.OPTIMIZE,
    model: str = "gpt-4o",
    log_level: str = "INFO",
    token_alert_threshold: int | None = None,
) -> tuple[CassandraPreHook, CassandraPostHook]:
    """Create a pair of Cassandra hooks for Agno agents.
    ...
    """
    pre_hook = CassandraPreHook(config=config, mode=mode, model=model)
    post_hook = CassandraPostHook(
        log_level=log_level,
