"""Agno model wrapper for Cassandra optimization.

This module provides CassandraAgnoModel, which wraps any Agno model
to apply Cassandra context optimization before API calls.
"""
...
from cassandra.parser import _coerce_tool_call_to_dict
...
@dataclass
class CassandraAgnoModel(Model):  # type: ignore[misc]
    """Agno model wrapper that applies Cassandra optimizations.
    ...
    Example:
        ...
        optimized = CassandraAgnoModel(wrapped_model=model)
        ...
        optimized = CassandraAgnoModel(wrapped_model=model, cassandra_config=config)

        # Agno reasoning with CassandraAgnoModel
        model = OpenAIChat(id="gpt-4o")
        wrapped = CassandraAgnoModel(wrapped_model=model)
        agent = Agent(
            model=wrapped,
            reasoning=True,
    ...
    id: str = field(default="cassandra-wrapper")
    name: str | None = field(default=None)
    provider: str | None = field(default=None)

    # CassandraAgnoModel specific fields
    wrapped_model: Any = field(default=None)
    cassandra_config: CassandraConfig | None = field(default=None)
    cassandra_mode: CassandraMode | None = field(default=None)
    ...
    _cassandra_provider: Any = field(default=None, repr=False, compare=False)
    _lock: threading.Lock = field(default_factory=threading.Lock, repr=False, compare=False)
    _initialized: bool = field(default=False, repr=False, compare=False)

    def __post_init__(self) -> None:
        """Initialize CassandraAgnoModel after dataclass construction."""
        ...
        if hasattr(self.wrapped_model, "id"):
            self.id = f"cassandra:{self.wrapped_model.id}"
        ...
        # Initialize config
        if self.cassandra_config is None:
            self.cassandra_config = CassandraConfig()

        # Handle deprecated mode parameter
        if self.cassandra_mode is not None:
            warnings.warn(
                "The 'cassandra_mode' parameter is deprecated. Use CassandraConfig(default_mode=...) instead.",
                DeprecationWarning,
                stacklevel=2,
            )
    ...
    def underlying_model(self) -> Any:
        ...
        Example:
            wrapped = CassandraAgnoModel(wrapped_model=Claude(...))
            ...
            with self._lock:
                ...
                if self._pipeline is None:
                    if self.auto_detect_provider:
                        self._cassandra_provider = get_cassandra_provider(self.wrapped_model)
                        logger.debug(
                            f"Auto-detected provider: {self._cassandra_provider.__class__.__name__}"
                        )
                    else:
                        self._cassandra_provider = OpenAIProvider()
                    self._pipeline = TransformPipeline(
                        config=self.cassandra_config,
                        provider=self._cassandra_provider,
                    )
    ...
    def _convert_messages_to_openai(self, messages: list[Any]) -> list[dict[str, Any]]:
        """Convert Agno messages to OpenAI format for Cassandra.
        ...
        for msg in messages:
            ...
            if hasattr(msg, "role") and hasattr(msg, "content"):
                ...
                # Cassandra pipeline + Agno's own re-serialization both call
    ...
    def _optimize_messages(self, messages: list[Any]) -> tuple[list[Any], OptimizationMetrics]:
        ...
        try:
            ...
            logger.warning(
                f"Cassandra optimization failed, using original messages: {type(e).__name__}: {e}"
            )
    ...
    def response(self, messages: list[Any], **kwargs: Any) -> Any:  # type: ignore[override]
        ...
        applies Cassandra optimization before delegating to wrapped_model.invoke().
    ...
    # These delegate to the wrapped model after applying Cassandra optimization
    # =========================================================================
    ...
    def invoke(self, messages: list[Any], **kwargs: Any) -> Any:
        ...
        logger.info(
            f"Cassandra optimized (invoke): {metrics.tokens_before} -> {metrics.tokens_after} tokens "
    ...
    async def ainvoke(self, messages: list[Any], **kwargs: Any) -> Any:
        ...
        logger.info(
            f"Cassandra optimized (ainvoke): {metrics.tokens_before} -> {metrics.tokens_after} tokens "
            f"({metrics.savings_percent:.1f}% saved)"
        )
    ...
    def invoke_stream(self, messages: list[Any], **kwargs: Any) -> Iterator[Any]:
        ...
        logger.info(
            f"Cassandra optimized (invoke_stream): {metrics.tokens_before} -> {metrics.tokens_after} tokens "
            f"({metrics.savings_percent:.1f}% saved)"
        )
    ...
    async def ainvoke_stream(self, messages: list[Any], **kwargs: Any) -> AsyncIterator[Any]:
        ...
        logger.info(
            f"Cassandra optimized (ainvoke_stream): {metrics.tokens_before} -> {metrics.tokens_after} tokens "
            f"({metrics.savings_percent:.1f}% saved)"
        )
