"""Strands SDK model wrapper for Cassandra optimization.

This module provides CassandraStrandsModel, which wraps any Strands model
...
from cassandra import CassandraConfig  # noqa: E402
from cassandra.providers import OpenAIProvider  # noqa: E402
from cassandra.transforms import TransformPipeline  # noqa: E402

from .providers import get_cassandra_provider, get_model_name_from_strands  # noqa: E402
...
class CassandraStrandsModel(Model):  # type: ignore[misc]
    """Strands model wrapper that applies Cassandra optimizations.
    ...
    Example:
        ...
        optimized = CassandraStrandsModel(wrapped_model=model)
        ...
        optimized = CassandraStrandsModel(wrapped_model=model, config=config)
    ...
    def __init__(
        ...
        """Initialize CassandraStrandsModel.

        Args:
            wrapped_model: The Strands model to wrap (e.g., BedrockModel, OpenAIModel)
            config: Optional CassandraConfig for optimization settings
            auto_detect_provider: Whether to auto-detect the Cassandra provider
        ...
        self.cassandra_config = config or CassandraConfig()
        ...
        self._cassandra_provider: Any = None
    ...
    def _convert_messages_to_openai(self, messages: list[Any]) -> list[dict[str, Any]]:
        """Convert Strands messages to OpenAI format for Cassandra.
    ...
    async def stream(
        ...
        **kwargs: Any,
        ...
        logger.info(
            f"Cassandra optimized (stream): {metrics.tokens_before} -> "
    ...
    async def structured_output(
        ...
        **kwargs: Any,
        ...
        """Generate structured output with Cassandra optimization.
        ...
        logger.info(
            f"Cassandra optimized (structured_output): {metrics.tokens_before} -> "
            f"{metrics.tokens_after} tokens ({metrics.savings_percent:.1f}% saved)"
        )
...
def optimize_messages(
    ...
    Example:
        from cassandra.integrations.strands import optimize_messages
