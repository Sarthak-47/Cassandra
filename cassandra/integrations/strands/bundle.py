"""CassandraBundle — single-helper MCP wiring for a Strands Agent.

The cleanest production setup for Strands is the same kit
``cassandra wrap claude`` installs for Claude Code, restated as
Strands-native primitives:

* **Cassandra MCP** (``cassandra mcp serve``) — exposes
  ``cassandra_retrieve`` / ``cassandra_compress`` / ``cassandra_stats``
  via stdio. The proxy emits ``Retrieve original: hash=...`` markers
  in compressed content; the LLM calls ``cassandra_retrieve`` when it
...
* **CassandraHookProvider** — the RTK-equivalent for Strands.
...
.. code-block:: python
    ...
    from cassandra.integrations.strands import CassandraBundle
    ...
    bundle = CassandraBundle(proxy_url="http://127.0.0.1:8787")
    agent = Agent(
        model=model,
...
``cassandra proxy``. This keeps the bundle stateless and lets the
...
from cassandra.mcp_registry.install import (
    DEFAULT_PROXY_URL,
    build_cassandra_spec,
    build_serena_spec,
    build_tokensave_spec,
)

from .hooks import CassandraHookProvider
...
#: code-aware agent loop (the same context ``cassandra wrap claude`` uses).
...
def _make_cassandra_client(proxy_url: str) -> MCPClient:
    return _client_for(build_cassandra_spec(proxy_url))
...
class CassandraBundle:
    """Single helper that hands a Strands Agent every Cassandra integration.

    Attributes:
        proxy_url: HTTP URL the Cassandra MCP server should contact for
            retrieval. Default :data:`DEFAULT_PROXY_URL`
            (``http://127.0.0.1:8787``).
        serena_context: Serena context label. Default ``"ide-assistant"``.
        enable_cassandra_mcp: Include the Cassandra MCP server. Default True.
        ...
        enable_hooks: Include :class:`CassandraHookProvider` for in-place
            tool-output compression (the RTK-equivalent for Strands).
            Default True.
        config: Optional :class:`CassandraConfig` passed to
            :class:`CassandraHookProvider`. Default uses framework
            defaults.
    ...
    enable_cassandra_mcp: bool = True
    ...
    # (CassandraHookProvider) is an optimisation for memory/network when
    ...
    _cassandra_mcp: MCPClient | None = field(default=None, init=False, repr=False, compare=False)
    ...
    _hook: CassandraHookProvider | None = field(default=None, init=False, repr=False, compare=False)

    def __post_init__(self) -> None:
        if self.enable_cassandra_mcp:
            self._cassandra_mcp = _make_cassandra_client(self.proxy_url)
            logger.info(
                "CassandraBundle: Cassandra MCP client constructed (proxy_url=%s)",
                self.proxy_url,
            )
        if self.enable_tokensave_mcp:
            self._tokensave_mcp = _make_tokensave_client()
            logger.info("CassandraBundle: tokensave MCP client constructed (primary)")
        if self.enable_serena_mcp:
            self._serena_mcp = _make_serena_client(self.serena_context)
            logger.info(
                "CassandraBundle: Serena MCP client constructed (backup, context=%s)",
                self.serena_context,
            )
        if self.enable_hooks:
            self._hook = CassandraHookProvider(config=self.config)
            logger.info("CassandraBundle: CassandraHookProvider attached")
    ...
    def tools(self) -> list[Any]:
        ...
        """
        out: list[Any] = []
        if self._cassandra_mcp is not None:
            out.append(self._cassandra_mcp)
    ...
    @property
    def cassandra_mcp(self) -> MCPClient | None:
        """Direct handle to the Cassandra MCPClient (for advanced callers)."""
        return self._cassandra_mcp
