"""Strands SDK hook provider for Cassandra tool output compression.

This module provides CassandraHookProvider, which implements Strands' HookProvider
interface to intercept tool outputs and compress them using Cassandra's SmartCrusher.
...
@dataclass
class CassandraHookProvider(HookProvider):  # type: ignore[misc]
    """Strands HookProvider that compresses tool outputs using Cassandra.

    This hook provider intercepts tool call results via AfterToolCallEvent
    and applies Cassandra's SmartCrusher to compress large outputs, reducing
    ...
    Example:
        ...
        hook = CassandraHookProvider(min_tokens_to_compress=50)
    ...
    config: CassandraConfig | None = field(default=None)
    ...
    def __post_init__(self) -> None:
        ...
        logger.debug(
            "CassandraHookProvider initialized: compress=%s, min_tokens=%d, preserve_errors=%s",
    ...
    def crusher(self) -> SmartCrusher:
        ...
        if self._crusher is None:
            with self._lock:
                ...
                if self._crusher is None:
                    # Use config from CassandraConfig if available
    ...
    def register_hooks(self, registry: HookRegistry, **kwargs: Any) -> None:
        ...
        logger.info(
            "CassandraHookProvider registered: compressing tool outputs >= %d tokens",
            self.min_tokens_to_compress,
        )
    ...
    def reset(self) -> None:
        ...
        logger.debug("CassandraHookProvider metrics reset")
