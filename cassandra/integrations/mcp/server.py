"""Cassandra MCP integration helpers for compressing tool outputs.

This module currently provides these MCP integration surfaces:

1. CassandraMCPCompressor - core compression logic for MCP tool results
2. compress_tool_result() - standalone helper for host applications
3. CassandraMCPClientWrapper - client wrapper that compresses tool outputs
4. create_cassandra_mcp_proxy() - configuration helper for custom proxy setups

The key insight: MCP tool outputs are the PERFECT use case for Cassandra.
...
Example - Custom proxy configuration:
    ...
    proxy_config = create_cassandra_mcp_proxy(
        upstream_servers=[
            ("slack", slack_server),
            ("database", db_server),
...
Example - Middleware (for MCP client libraries):
    ...
    middleware = CassandraMCPMiddleware(config)
...
from cassandra.config import CassandraConfig, SmartCrusherConfig
from cassandra.providers.openai import OpenAIProvider
from cassandra.transforms.smart_crusher import SmartCrusher
...
class CassandraMCPCompressor:
    ...
    def compress(
        ...
        # Remove any Cassandra markers for clean output
        compressed_content = re.sub(r"\n<cassandra:[^>]+>", "", compressed_content)
...
def compress_tool_result(
    ...
    This is the simplest way to use Cassandra with MCP in your host application.
    ...
    Args:
        ...
        config: Optional Cassandra configuration.
    ...
    Example:
        ...
        ```
    """
    compressor = CassandraMCPCompressor(config=config)
...
class CassandraMCPClientWrapper:
    ...
    Example:
        ...
        from cassandra.integrations.mcp import CassandraMCPClientWrapper

        # Wrap your MCP client
        base_client = Client(transport)
        client = CassandraMCPClientWrapper(base_client)
    ...
    def __init__(
        ...
        """
        self._client = client
        self._compressor = CassandraMCPCompressor(config=config)
...
def create_cassandra_mcp_proxy(
    upstream_servers: list[tuple[str, MCPServer]],
    config: CassandraConfig | None = None,
) -> dict[str, Any]:
    """Create configuration for a custom Cassandra MCP proxy/server wrapper.

    This returns the compressor and upstream-server mapping needed by an
    application-defined MCP proxy/server wrapper. Cassandra does not yet ship
    a ready-to-run ``CassandraMCPProxy`` server implementation.
