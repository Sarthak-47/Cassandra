...
class AgentSavingsProfile:
    ...
    def proxy_env(self) -> dict[str, str]:
        """Return env vars for Cassandra proxy/wrapper entry points."""

        return {
            "CASSANDRA_MODE": self.proxy_mode,
            "CASSANDRA_SAVINGS_PROFILE": self.name,
            "CASSANDRA_SAVINGS_TARGET": f"{self.target_savings:.2f}",
            "CASSANDRA_TARGET_RATIO": f"{self.target_ratio:.2f}",
            "CASSANDRA_COMPRESS_USER_MESSAGES": ("1" if self.compress_user_messages else "0"),
            "CASSANDRA_COMPRESS_SYSTEM_MESSAGES": ("1" if self.compress_system_messages else "0"),
            "CASSANDRA_PROTECT_RECENT": str(self.protect_recent),
            "CASSANDRA_PROTECT_ANALYSIS_CONTEXT": ("1" if self.protect_analysis_context else "0"),
            "CASSANDRA_MIN_TOKENS": str(self.min_tokens_to_compress),
            "CASSANDRA_MAX_ITEMS": str(self.max_items_after_crush),
            "CASSANDRA_SMART_CRUSHER_COMPACTION": (
                "1" if self.smart_crusher_with_compaction else "0"
            ),
            "CASSANDRA_FORCE_KOMPRESS": "1" if self.force_kompress else "0",
            "CASSANDRA_ACCURACY_GUARD": self.accuracy_guard,
        }
