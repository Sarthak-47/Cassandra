...
``crates/cassandra-core/src/signals/`` (see the trait architecture in
``signals/README.md``). This module is now a compatibility surface that:

1. Pulls the keyword tables out of Rust via
   ``cassandra._core.keyword_registry_snapshot()`` so the Python side
...
def content_has_error_indicators(text: str) -> bool:
    ...
    scoring in :mod:`cassandra._core.score_line` because the triage
