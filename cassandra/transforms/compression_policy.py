...
Hand-mirrored port of `cassandra_core::compression_policy::CompressionPolicy`
...
See `crates/cassandra-core/src/compression_policy.rs` for the canonical
...
# ``crates/cassandra-core/src/compression_policy.rs``. Centralised here so
...
def policy_default_payg() -> CompressionPolicy:
    ...
    ``CASSANDRA_PROXY_AUTH_MODE_POLICY_ENFORCEMENT`` flag is disabled
...
_ENFORCEMENT_ENV = "CASSANDRA_PROXY_AUTH_MODE_POLICY_ENFORCEMENT"
...
def is_enforcement_enabled() -> bool:
    ...
    # (`cassandra/telemetry/beacon.py::_OFF_VALUES`) so operators don't
