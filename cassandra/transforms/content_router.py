...
Usage:
    >>> from cassandra.transforms import ContentRouter
...
def _resolve_detect_backend() -> str:
    ...
    backend = os.environ.get("CASSANDRA_DETECT_BACKEND", "").strip().lower()
...
def _detect_content(content: str) -> DetectionResult:
    ...
    Stage-3d (PR5) wired this through `cassandra._core.detect_content_type`,
    ...
    Set `CASSANDRA_DETECT_BACKEND=rust` or `python` to force a backend.
    ...
    if backend == "python":
        if not _detect_backend_warned:
            ...
            logger.warning(
                ...
                "override with CASSANDRA_DETECT_BACKEND=rust)."
            )
        return _regex_detect_content_type(content)

    from cassandra._core import detect_content_type as _rust_detect
...
def _net_cost_cache_ttl_seconds() -> float:
    ...
    ``CASSANDRA_NET_COST_CACHE_TTL_SECONDS`` for other providers/tiers. A
    ...
    the other ``CASSANDRA_NET_COST_*`` env guards).
    """
    raw = os.environ.get("CASSANDRA_NET_COST_CACHE_TTL_SECONDS", "")
    if not raw:
    ...
    try:
        ...
        logger.warning(
            "CASSANDRA_NET_COST_CACHE_TTL_SECONDS malformed; using default %s",
            _NET_COST_CACHE_TTL_SECONDS,
        )
        return _NET_COST_CACHE_TTL_SECONDS
    if not math.isfinite(ttl) or ttl <= 0.0:
        logger.warning(
            "CASSANDRA_NET_COST_CACHE_TTL_SECONDS invalid; using default %s",
            _NET_COST_CACHE_TTL_SECONDS,
        )
        return _NET_COST_CACHE_TTL_SECONDS
    return ttl
...
class ContentRouterConfig:
    ...
    # from the proxy's `CASSANDRA_LOSSLESS_ONLY` env var so a real session
...
class ContentRouter(Transform):
    ...
    ContentRouter is the recommended entry point for Cassandra's compression.
    ...
    def __init__(
        ...
        Args:
            ...
            observer: Optional `CompressionObserver` (see
                `cassandra.transforms.observability`) called once per
        ...
        try:
            self._kompress_max_tokens: int = int(
                os.environ.get("CASSANDRA_KOMPRESS_MAX_TOKENS", "50000")
            )
        except ValueError:
    ...
    def _get_text_crusher(self) -> Any:
        ...
        when the native ``cassandra._core`` extension is not built (mirrors the
        ...
        if self._text_crusher is None:
            try:
                ...
                logger.debug("TextCrusher (cassandra._core) unavailable; disabling gate route")
    ...
    def _get_diff_compressor(self) -> Any:
        ...
        retired in Stage 3b. The wheel (`cassandra._core`) is a hard import.
        """
    ...
    def eager_load_compressors(self) -> dict[str, str]:
        ...
        # On Windows `cassandra._ort` pins ORT_DYLIB_PATH at import time; an
        ...
        # (Win11 24H2+, see cassandra/_ort.py).
    ...
    def _get_kompress(self) -> Any:
        ...
        - None: use default (Sarthak-47/kompress-v2-base) — cached on self
    ...
    def _get_image_optimizer(self) -> Any:
        ...
        - Trained MiniLM classifier from HuggingFace (Sarthak-47/technique-router)
    ...
    def optimize_images_in_messages(
        ...
        (Sarthak-47/technique-router) + SigLIP for image analysis.
    ...
    def _net_cost_allows(
        ...
        *,
        ...
        (``CASSANDRA_NET_COST_EXPECTED_READS``, default 10;
        ``CASSANDRA_NET_COST_P_ALIVE``, default 1.0 — the conservative
        ...
        by the caller, replaces the static ``CASSANDRA_NET_COST_P_ALIVE``
        ...
        try:
            _reads = float(os.environ.get("CASSANDRA_NET_COST_EXPECTED_READS", "") or 10.0)
            if not math.isfinite(_reads):
                raise ValueError("non-finite")
            reads = _reads
        except ValueError:
            logger.warning("CASSANDRA_NET_COST_EXPECTED_READS malformed; using 10")
        ...
        if p_alive_override is not None:
            ...
            try:
                _p_alive = float(os.environ.get("CASSANDRA_NET_COST_P_ALIVE", "") or 1.0)
                if not math.isfinite(_p_alive):
                    raise ValueError("non-finite")
                p_alive = _p_alive
            except ValueError:
                logger.warning("CASSANDRA_NET_COST_P_ALIVE malformed; using 1.0")
    ...
    def apply(
        ...
        **kwargs: Any,
        ...
        netcost_enabled = os.environ.get("CASSANDRA_NET_COST_POLICY") == "1"
        ...
        # times over. With CASSANDRA_NET_COST_POLICY=1 a *string-content*
        ...
        if pending_tasks:
            max_workers = min(
                len(pending_tasks), int(os.environ.get("CASSANDRA_COMPRESS_WORKERS", "4"))
            )
