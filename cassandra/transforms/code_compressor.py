...
def _get_parser(language: str) -> Any:
    ...
    from any thread other than its creator.  Because Cassandra runs
    ...
    if not _tree_sitter_importable():
        raise ImportError(
            "tree-sitter is not installed. Install with: pip install cassandra-ai[code]\n"
...
class CodeAwareCompressor(Transform):
    ...
    def _verify_syntax(self, code: str, language: CodeLanguage) -> bool:
        ...
        try:
            if language == CodeLanguage.PYTHON:
                ...
                ast.parse(code)
                compile(code, "<cassandra-compressed>", "exec")
    ...
    def apply(
        ...
        **kwargs: Any,
        ...
        if not _check_tree_sitter_available():
            warnings.append(
                "tree-sitter not installed. Install with: pip install cassandra-ai[code]"
            )
