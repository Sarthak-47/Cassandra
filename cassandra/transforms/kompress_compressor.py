...
Auto-downloads the model from HuggingFace (Sarthak-47/kompress-v2-base)
on first use.

Requires the [ml] extra: pip install cassandra-ai[ml]

Usage:
    >>> from cassandra.transforms.kompress_compressor import KompressCompressor
...
HF_MODEL_ID = "Sarthak-47/kompress-v2-base"
...
# Disable with CASSANDRA_KOMPRESS_MUST_KEEP=0.
...
_KOMPRESS_MUST_KEEP_ENV = "CASSANDRA_KOMPRESS_MUST_KEEP"
KOMPRESS_BACKEND_ENV = "CASSANDRA_KOMPRESS_BACKEND"
KOMPRESS_ONNX_FILENAME_ENV = "CASSANDRA_KOMPRESS_ONNX_FILENAME"
...
#
# An operator can pin an exact file via CASSANDRA_KOMPRESS_ONNX_FILENAME.
_DEFAULT_ONNX_FILENAMES = (
    "onnx/kompress-int8-wo.onnx",
    "onnx/kompress-fp32.onnx",
    "onnx/kompress-int8.onnx",
)
KOMPRESS_ONNX_INTRA_THREADS_ENV = "CASSANDRA_KOMPRESS_ONNX_INTRA_THREADS"
KOMPRESS_ONNX_INTER_THREADS_ENV = "CASSANDRA_KOMPRESS_ONNX_INTER_THREADS"
KOMPRESS_COREML_CACHE_DIR_ENV = "CASSANDRA_KOMPRESS_COREML_CACHE_DIR"
KOMPRESS_MAX_CONCURRENT_ENV = "CASSANDRA_KOMPRESS_MAX_CONCURRENT"
KOMPRESS_BATCH_SIZE_ENV = "CASSANDRA_KOMPRESS_BATCH_SIZE"
...
def _get_model_class() -> type:
    """Return the CassandraCompressorModel class, importing torch on demand."""
    import torch
    import torch.nn as nn
    from transformers import AutoModel

    class CassandraCompressorModel(nn.Module):
    ...
    return CassandraCompressorModel
...
def _load_kompress_pytorch(
    ...
    with _kompress_lock:
        ...
        CassandraCompressorModel = _get_model_class()
        model = CassandraCompressorModel()
...
def _load_kompress(
    ...
    # allowing opt-in MPS/CoreML experiments via CASSANDRA_KOMPRESS_BACKEND.
    if _is_onnx_available():
        try:
            return _load_kompress_onnx(model_id, use_coreml=False, allow_download=allow_download)
        except KompressModelNotCached:
    ...
    raise ImportError(
        "Kompress requires onnxruntime or torch. Install with: pip install cassandra-ai[proxy]"
    )
...
# compression timeout (CASSANDRA_COMPRESSION_TIMEOUT_SECONDS, default 30s). The
...
class KompressConfig:
    ...
    Example — financial documents::
        ...
        KompressConfig(
            model_id="Sarthak-47/kompress-finance",
            chunk_words=50,
            score_threshold=0.5,
        )
    """
...
class KompressCompressor(Transform):
    ...
    def compress(
        ...
        *,
        ...
        # disables. Env CASSANDRA_COMPRESSION_DEADLINE_MS overrides (default 20s).
        ...
        if deadline_s is None:
            try:
                deadline_s = max(
                    0.0,
                    float(os.environ.get("CASSANDRA_COMPRESSION_DEADLINE_MS", "20000")) / 1000.0,
                )
            except ValueError:
                deadline_s = 20.0
        ...
        try:
            ...
            for chunk_start in range(0, n_words, max_chunk_words):
                ...
                # reconstruct from context. Disable via CASSANDRA_KOMPRESS_MUST_KEEP=0.
