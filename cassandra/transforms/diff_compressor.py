...
diff compression now goes through `cassandra._core.DiffCompressor` (built
from `crates/cassandra-py`). The byte-equality of the two implementations
was verified against 27 recorded fixtures before the Python source was
removed; the Rust crate has its own test coverage in `crates/cassandra-core/`.
...
class DiffCompressor:
    """Rust-backed `DiffCompressor` (via PyO3 / `cassandra._core`).
