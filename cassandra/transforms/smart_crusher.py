...
All array compression now goes through `cassandra._core.SmartCrusher`
(built from `crates/cassandra-py`). Byte-equality of the two
...
removed; the Rust crate has its own coverage in `crates/cassandra-core/`
...
The `cassandra._core` extension is a hard import: there is no Python
...
# `crates/cassandra-core/.../compaction/mod.rs`.
...
class SmartCrusherConfig:
    ...
    # crates/cassandra-core/src/transforms/smart_crusher/compaction/compactor.rs).
...
class SmartCrusher(Transform):
    """Rust-backed `SmartCrusher` (via PyO3 / `cassandra._core`).
    ...
    def __init__(
        ...
        # `cassandra.config.SmartCrusherConfig`, which also carries it.
        self._lossless_only = (
            bool(getattr(cfg, "lossless_only", False))
            if lossless_only is None
            else bool(lossless_only)
        )
        # `observer`: see `cassandra.transforms.observability`. The
        ...
        # back (`cassandra.proxy.server` does). Both `enabled=False` and
        ...
        self._rust_cfg_kwargs = {
            ...
            # `cassandra.config.SmartCrusherConfig` (MCP server, SDK) or a
        ...
        # CASSANDRA_COMPACTION_FORMAT env var when the kwarg is None.
        # Ignored when with_compaction=False.
        resolved_format = compaction_format or os.environ.get(
            "CASSANDRA_COMPACTION_FORMAT", "csv-schema"
        )
    ...
    def _mirror_single_hash_to_python_store(
        ...
        # the Rust store's `DEFAULT_TTL` (see crates/cassandra-core/src/
    ...
    def _notify_observer(self, original_tokens: int, compressed_tokens: int) -> None:
        ...
        `CompressionObserver` (see `cassandra.transforms.observability`).
        ...
        **kwargs: Any,
        ...
        for msg_idx, msg in enumerate(result_messages):
            ...
            if msg.get("role") == "tool":
                # #1077: never re-compress cassandra_retrieve results — they ARE
            ...
            if isinstance(content, list):
                for i, block in enumerate(content):
                    if not isinstance(block, dict) or block.get("type") != "tool_result":
                        continue
                    # #1077: skip cassandra_retrieve results — compressing them
