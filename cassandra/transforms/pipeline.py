"""Transform pipeline orchestration for Cassandra SDK."""
...
from ..observability import get_cassandra_tracer, get_otel_metrics
...
class TransformPipeline:
    ...
    def _build_default_transforms(self) -> list[Transform]:
        ...
        # OPT-IN: enable via CassandraConfig.intercept_tool_results, or for
        # non-config callers (CLI / SDK / tests) the env var
        # CASSANDRA_INTERCEPT_ENABLED=1. Off by default while this ships — lets
        ...
        **kwargs: Any,
        ...
        if model_limit is None:
            raise ValueError(
                ...
                "configure model_context_limits in CassandraClient."
            )
        ...
        tracer = get_cassandra_tracer()
        span_attributes = {
            "cassandra.model": model,
            "cassandra.provider": provider_name or "unknown",
            "cassandra.message_count": len(messages),
            "cassandra.tokens.before": tokens_before,
        }
        pipeline_span_context = (
            tracer.start_as_current_span(
                "cassandra.compression.pipeline",
        ...
        with pipeline_span_context as pipeline_span:
            ...
            for transform in self.transforms:
                ...
                transform_span_context = (
                    tracer.start_as_current_span(
                        "cassandra.compression.transform",
                        attributes={
                            "cassandra.model": model,
                            "cassandra.provider": provider_name or "unknown",
                            "cassandra.transform": transform.name,
                        },
                    )
                    if record_metrics
                    else nullcontext()
                ...
                with transform_span_context as transform_span:
                    ...
                    if transform_span is not None and transform_span.is_recording():
                        transform_span.set_attribute(
                            "cassandra.tokens.before", tokens_before_transform
                        )
                        transform_span.set_attribute(
                            "cassandra.tokens.after", tokens_after_transform
                        )
                        transform_span.set_attribute(
                            "cassandra.tokens.saved",
                            tokens_before_transform - tokens_after_transform,
                        )
                        transform_span.set_attribute("cassandra.duration_ms", duration_ms)
                        transform_span.set_attribute(
            ...
            if pipeline_span is not None and pipeline_span.is_recording():
                pipeline_span.set_attribute("cassandra.tokens.after", tokens_after)
                pipeline_span.set_attribute("cassandra.tokens.saved", total_saved)
                pipeline_span.set_attribute("cassandra.duration_ms", pipeline_ms)
                pipeline_span.set_attribute("cassandra.transforms_applied", len(all_transforms))
                pipeline_span.set_attribute("cassandra.warnings", len(all_warnings))
