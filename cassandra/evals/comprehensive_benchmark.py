...
This module runs industry-standard benchmarks to prove Cassandra preserves accuracy.

Approach:
1. Run benchmarks directly against LLM provider (baseline)
2. Run same benchmarks through Cassandra proxy
...
Usage:
    ...
    python -m cassandra.evals.comprehensive_benchmark --quick

    # Full benchmark
    python -m cassandra.evals.comprehensive_benchmark --tasks mmlu,hellaswag

    # Compare with baseline
    python -m cassandra.evals.comprehensive_benchmark --compare
"""
...
class ComparisonResult:
    """Comparison between baseline and Cassandra results."""

    task: str
    metric: str
    baseline_score: float
    cassandra_score: float

    @property
    def delta(self) -> float:
        return self.cassandra_score - self.baseline_score

    @property
    def accuracy_preserved(self) -> bool:
        """True if Cassandra score is within 2% of baseline."""
        return self.cassandra_score >= self.baseline_score - 0.02

    @property
    def accuracy_improved(self) -> bool:
        """True if Cassandra score exceeds baseline."""
        return self.cassandra_score > self.baseline_score
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "cassandra": round(self.cassandra_score, 4),
...
class BenchmarkSuiteResult:
    ...
    cassandra_results: list[BenchmarkResult] = field(default_factory=list)
    ...
    def avg_delta(self) -> float:
        """Average score delta (positive = Cassandra better)."""
        if not self.comparisons:
            return 0.0
    ...
    def summary(self) -> dict[str, Any]:
        return {
            ...
            "cassandra": [r.to_dict() for r in self.cassandra_results],
    ...
    def print_summary(self) -> None:
        ...
        print("\n" + "=" * 70)
        print("CASSANDRA COMPREHENSIVE BENCHMARK RESULTS")
        print("=" * 70)
        ...
        if self.comparisons:
            print(f"{'Task':<20} {'Baseline':>10} {'Cassandra':>10} {'Delta':>10} {'Status':>10}")
            print("-" * 70)
            for c in self.comparisons:
                status = "PASS" if c.accuracy_preserved else "FAIL"
                delta_str = f"{c.delta:+.4f}"
                print(
                    f"{c.task:<20} {c.baseline_score:>10.4f} {c.cassandra_score:>10.4f} {delta_str:>10} {status:>10}"
                )
            print("-" * 70)
            ...
            print("Cassandra results only (no baseline comparison):")
            print(f"{'Task':<20} {'Score':>10} {'Metric':<15}")
            print("-" * 50)
            for r in self.cassandra_results:
...
def run_lm_eval(
    ...
    Args:
        ...
        base_url: Base URL for API (for Cassandra proxy)
...
def run_cassandra_benchmark(
    model: str = "gpt-4o-mini",
    tasks: list[str] | None = None,
    limit: int | None = None,
    cassandra_port: int = 8787,
) -> list[BenchmarkResult]:
    """Run benchmark through Cassandra proxy."""
    logger.info(f"Running Cassandra benchmark with {model} through proxy...")

    # lm_eval expects base_url to be the full path to chat/completions
    base_url = f"http://localhost:{cassandra_port}/v1/chat/completions"
...
def compare_results(
    ...
    cassandra: list[BenchmarkResult],
) -> list[ComparisonResult]:
    """Compare baseline and Cassandra results."""
    comparisons = []

    # Match results by task
    baseline_by_task = {r.task: r for r in baseline}
    cassandra_by_task = {r.task: r for r in cassandra}

    for task in baseline_by_task:
        if task in cassandra_by_task:
            b = baseline_by_task[task]
            h = cassandra_by_task[task]
            comparisons.append(
                ComparisonResult(
                    task=task,
                    metric=b.metric,
                    baseline_score=b.score,
                    cassandra_score=h.score,
                )
            )
...
def run_comprehensive_benchmark(
    ...
    Args:
        ...
        cassandra_port: Port where Cassandra proxy is running
    ...
    # This ensures a fair comparison - any speedup from Cassandra is real, not from warm cache
    if compare_baseline:
    ...
    # Run Cassandra benchmark (after baseline warms things up)
    suite.cassandra_results = run_cassandra_benchmark(
        model=model,
        tasks=tasks,
        limit=limit,
    ...
    # Compare results
    if compare_baseline:
        suite.comparisons = compare_results(suite.baseline_results, suite.cassandra_results)

    suite.total_duration_seconds = time.time() - start_time
    return suite


def check_cassandra_proxy(port: int = 8787) -> bool:
    """Check if Cassandra proxy is running."""
    import socket
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Run comprehensive LLM benchmarks to verify Cassandra accuracy"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--compare", "-c", action="store_true", help="Compare with baseline (run without Cassandra)"
    )
    parser.add_argument(
        "--port", "-p", type=int, default=8787, help="Cassandra proxy port (default: 8787)"
    )
    parser.add_argument("--output", "-o", default=None, help="Output file for results JSON")
    parser.add_argument(
        "--cassandra-only", action="store_true", help="Only run through Cassandra (skip baseline)"
    )
    ...
    if not check_cassandra_proxy(args.port):
        print(f"ERROR: Cassandra proxy not running on port {args.port}")
        print(f"Start it with: cassandra proxy --port {args.port}")
        sys.exit(1)

    # Run benchmarks
    compare = args.compare and not args.cassandra_only
