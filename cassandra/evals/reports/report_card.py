...
baseline vs Cassandra accuracy + token savings.
"""
...
class BenchmarkRunResult:
    ...
    # For lm-eval (standard benchmarks): baseline vs cassandra scores
    baseline_score: float | None = None
    cassandra_score: float | None = None
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if self.cassandra_score is not None:
            d["cassandra_score"] = round(self.cassandra_score, 4)
...
class SuiteResult:
    ...
    def standard_benchmarks(self) -> list[BenchmarkRunResult]:
        """Benchmarks with baseline vs cassandra comparison."""
...
def generate_markdown(result: SuiteResult) -> str:
    ...
    lines = [
        "## Cassandra Accuracy Report Card",
        "",
    ...
    if standard:
        lines.extend(
            [
                ...
                "",
                "| Benchmark | Category | N | Baseline | Cassandra | Delta | Tokens Saved | Status |",
                "|-----------|----------|---|----------|----------|-------|--------------|--------|",
            ]
        )
        for b in standard:
            ...
            cassandra_str = f"{b.cassandra_score:.3f}" if b.cassandra_score is not None else "N/A"
            ...
            lines.append(
                ...
                f"{cassandra_str} | {delta_str} | {comp_str} | {status} |"
            )
        lines.append("")
...
def generate_html(result: SuiteResult) -> str:
    ...
    for b in standard:
        ...
        standard_rows += f"""        <tr>
            ...
            <td>{baseline_str}</td><td>{cassandra_str}</td><td>{delta_str}</td>
            <td>{comp_str}</td>
...
<html>
<head>
    <title>Cassandra Accuracy Report Card</title>
    <style>
        body {{
...
<body>
    <h1>Cassandra Accuracy Report Card</h1>
    <div class="meta">
    ...
    {"<table><tr><th>Benchmark</th><th>Category</th><th>N</th><th>Baseline</th><th>Cassandra</th><th>Delta</th><th>Saved</th><th>Status</th></tr>" if standard else ""}
    {standard_rows}
    ...
    <footer>Generated by <strong>Cassandra Evaluation Framework</strong> | <code>pip install cassandra-ai[evals]</code></footer>
</body>
</html>"""
