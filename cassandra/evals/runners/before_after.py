...
from cassandra.evals.core import (
    EvalCase,
    EvalMode,
    EvalResult,
    EvalSuite,
...
from cassandra.evals.metrics import compute_semantic_similarity
from cassandra.transforms.content_router import ContentRouter, ContentRouterConfig
from cassandra.transforms.smart_crusher import SmartCrusherConfig
...
class LLMConfig:
    ...
    max_tokens: int = 1024
    cassandra_proxy_url: str | None = None  # e.g. "http://localhost:8787" for full-stack eval
...
class BeforeAfterRunner:
    ...
    def __init__(
        ...
        # This gives us the FULL Cassandra stack (compression + CCR + cache alignment)
        # rather than just local ContentRouter compression.
        self._proxy_client: Any = None
        if self.llm_config.cassandra_proxy_url:
    ...
    def _init_proxy_client(self) -> Any:
        """Initialize an OpenAI client pointing at the Cassandra proxy."""
        import openai

        return openai.OpenAI(
            base_url=f"{self.llm_config.cassandra_proxy_url}/v1",
            api_key=os.environ.get("OPENAI_API_KEY", ""),
        )

    def _call_llm_via_proxy(self, context: str, query: str) -> str:
        """Call LLM through Cassandra proxy (full stack: compression + CCR)."""
...
Answer:"""
    ...
    def evaluate_case(
        ...
        """
        from cassandra.evals.metrics import compute_exact_match, compute_f1
        ...
        # Run LLM with ORIGINAL context (direct to API, no Cassandra)
        start = time.time()
        try:
        ...
        # Run LLM with Cassandra context
    ...
    def _evaluate_ground_truth(
        ...
        """
        from cassandra.evals.metrics import compute_f1
        ...
        if case.ground_truth:
            try:
                if self._judge_fn is None:
                    from cassandra.evals.memory.judge import create_openai_judge
...
def run_quick_eval(
    ...
    Returns:
        Evaluation results
    """
    from cassandra.evals.datasets import load_tool_output_samples
