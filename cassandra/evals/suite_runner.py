...
if TYPE_CHECKING:
    from cassandra.evals.reports.report_card import SuiteResult
...
class BenchmarkSpec:
    ...
    pass_threshold: float = 0.98  # Cassandra score >= baseline - (1-threshold)
...
BENCHMARK_SUITE: list[BenchmarkSpec] = [
    ...
    # -----------------------------------------------------------------------
    # Standard benchmarks via lm-eval harness (through Cassandra proxy)
    BenchmarkSpec(
        name="GSM8K",
        category="reasoning",
        tier=1,
...
def _start_proxy(port: int) -> subprocess.Popen | None:
    """Start Cassandra proxy as a subprocess. Returns process handle."""
    logger.info(f"Starting Cassandra proxy on port {port}...")
    try:
        proc = subprocess.Popen(
            [sys.executable, "-m", "cassandra.proxy.server", "--port", str(port)],
        ...
        for _ in range(30):
            ...
            if _check_proxy(port):
                logger.info(f"Cassandra proxy ready on port {port}")
                return proc
...
class SuiteRunner:
    ...
    def _ensure_proxy(self) -> bool:
        """Ensure the Cassandra proxy is running (needed for lm-eval benchmarks)."""
        if _check_proxy(self.cassandra_port):
            return True
        if self.auto_start_proxy:
            self._proxy_proc = _start_proxy(self.cassandra_port)
    ...
    def _cleanup_proxy(self) -> None:
        ...
        if self._proxy_proc:
            logger.info("Stopping Cassandra proxy...")
    ...
    def _run_lm_eval_benchmark(
        ...
        from cassandra.evals.comprehensive_benchmark import (
            compare_results,
            run_baseline_benchmark,
            run_cassandra_benchmark,
        )
        ...
        # Run through Cassandra proxy
        print("    Running through Cassandra proxy...")
        cassandra_results = run_cassandra_benchmark(
            model=spec.model or self.model,
            tasks=tasks,
            limit=limit,
        ...
        # Compare
        comparisons = compare_results(baseline_results, cassandra_results)

        # Extract primary result
        baseline_score = None
        cassandra_score = None
        delta = None
        passed = True
        ...
        if comparisons:
            ...
            cassandra_score = c.cassandra_score
            delta = c.delta
            passed = c.accuracy_preserved

        return {
            "baseline_score": baseline_score,
            "cassandra_score": cassandra_score,
            "delta": delta,
            "passed": passed,
            "n_samples": limit,
        }
    ...
    def _run_before_after_benchmark(
        ...
        from cassandra.evals.core import EvalMode
        from cassandra.evals.datasets import load_dataset_by_name, load_tool_output_samples
        ...
        proxy_url = (
            f"http://localhost:{self.cassandra_port}" if _check_proxy(self.cassandra_port) else None
        )
    ...
    def run(self) -> SuiteResult:
        ...
        from cassandra.evals.cost_tracker import CostTracker
        from cassandra.evals.reports.report_card import BenchmarkRunResult, SuiteResult
        ...
        if has_lm_eval:
            ...
            if not proxy_available:
                print("WARNING: Cassandra proxy not available. Skipping lm-eval benchmarks.")
                print("  Start with: cassandra proxy --port 8787")
                    ...
                    if spec.runner_type == "lm_eval":
                        ...
                        results.append(
                            BenchmarkRunResult(
                                ...
                                cassandra_score=raw.get("cassandra_score"),
                                delta=raw.get("delta"),
    ...
    def _print_summary(self, result: SuiteResult, tracker: Any) -> None:
        ...
        from cassandra.evals.reports.report_card import generate_markdown
