...
batch API requests through the Cassandra proxy.
...
Usage:
    >>> from cassandra.evals.batch_compression_eval import (
...
CLI:
    python -m cassandra.evals.batch_compression_eval --provider anthropic --samples 10
"""
...
from cassandra.evals.metrics import (
    compute_exact_match,
    compute_f1,
    compute_rouge_l,
...
Context:
The Cassandra SDK is a context optimization layer for LLM applications. It was created
...
Question: Which repository has the most stars? Answer with just the repository name.""",
            ...
            context_facts=["cassandra", "1250", "llm-cache", "890", "prompt-optimizer", "2100"],
        ),
        BatchTestCase(
            id="json_003",
            category=TestCategory.JSON_EXTRACTION,
...
def evaluate_token_counting_accuracy(
    ...
    This verifies that Cassandra's token counting matches the provider's
