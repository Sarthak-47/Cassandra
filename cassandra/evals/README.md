# Cassandra Evaluation Framework
...
| Benchmark | Category | N | Baseline | Cassandra | Delta |
|-----------|----------|---|----------|----------|-------|
...
```bash
pip install "cassandra-ai[all]"    # Everything including evals (recommended)
pip install "cassandra-ai[evals]"  # Evaluation framework only
```
...
python -m cassandra.evals quick -n 8 --provider openai --model gpt-4o-mini

# Full Tier 1 suite (~$3, ~15 min) — requires proxy running
python -m cassandra.evals suite --tier 1 -o eval_results/

# Extended suite (Tiers 1+2, ~$8, ~1 hr)
python -m cassandra.evals suite --tier 2 -o eval_results/

# CI mode — exit 1 on any regression
python -m cassandra.evals suite --tier 1 --ci

# List all available datasets
python -m cassandra.evals list
```

### Running with the Proxy (Recommended)

For the most accurate evaluation, run through the Cassandra proxy which provides
...
CASSANDRA_PROBE_RECORD_DIR=~/.cassandra/probe-recordings cassandra proxy start

# 2. Use your agent through the proxy as normal, then score retention:
cassandra evals probes --recordings ~/.cassandra/probe-recordings
```
...
When the Cassandra proxy is running, the "compressed" path goes through the full
...
on:
  pull_request:
    paths: ['cassandra/transforms/**', 'cassandra/evals/**']
  schedule:
    - cron: '0 6 * * 1'  # Weekly
    ...
    steps:
      ...
      - name: CCR Round-trip (zero cost)
        run: |
          ...
          "
      - name: Quick eval
        run: python -m cassandra.evals quick -n 8 --provider openai --model gpt-4o-mini
        env:
