...
This module provides tools to verify that Cassandra preserves the semantic
meaning of prompts by comparing prompts before and after processing.

This is CRITICAL for proving that Cassandra doesn't change the meaning of prompts
when they go through the proxy.

Usage:
    from cassandra.evals.prompt_comparison import compare_prompts, PromptComparer

    # Simple comparison
    result = compare_prompts(
        original_prompt="What is the capital of France?",
        cassandra_modified_prompt="What is the capital of France?",
    )
...
class PromptComparisonResult:
    ...
    Attributes:
        original: The original prompt before Cassandra processing.
        cassandra_modified: The prompt after Cassandra processing.
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "cassandra_modified": self.cassandra_modified,
    ...
    def from_dict(cls, data: dict[str, Any]) -> PromptComparisonResult:
        ...
        return cls(
            ...
            cassandra_modified=data["cassandra_modified"],
...
def compare_prompts(
    ...
    Args:
        original_prompt: The original prompt before Cassandra processing.
        cassandra_modified_prompt: The prompt after Cassandra processing.
    ...
    Example:
        result = compare_prompts(
            ...
            cassandra_modified_prompt="Explain quantum computing in simple terms.",
        )
    ...
    judge_prompt = SEMANTIC_EQUIVALENCE_JUDGE_PROMPT.format(
        ...
        modified_prompt=cassandra_modified_prompt,
    )
    ...
    return PromptComparisonResult(
        ...
        cassandra_modified=cassandra_modified_prompt,
...
class PromptComparer:
    ...
    def compare(
        ...
        result = compare_prompts(
            original_prompt=original_prompt,
            cassandra_modified_prompt=cassandra_modified_prompt,
            judge_model=self.judge_model,
    ...
    def compare_and_log(
        ...
        Returns:
            PromptComparisonResult with the comparison outcome.
        """
        result = self.compare(original_prompt, cassandra_modified_prompt, metadata)
...
def compare_messages(
    ...
    Args:
        original_messages: Original message list before Cassandra.
        modified_messages: Modified message list after Cassandra.
        judge_model: The OpenAI model to use as judge.
    ...
    result = compare_prompts(
        ...
        cassandra_modified_prompt=modified_str,
        judge_model=judge_model,
        api_key=api_key,
        metadata={
...
def batch_compare_prompts(
    ...
    def compare_pair(
        ...
        result = compare_prompts(
            ...
            cassandra_modified_prompt=modified,
            judge_model=judge_model,
            api_key=api_key,
...
def verify_cassandra_preservation(
    original_messages: list[dict[str, Any]],
    cassandra_messages: list[dict[str, Any]],
    judge_model: str = "gpt-4o",
    api_key: str | None = None,
    fail_on_difference: bool = False,
) -> PromptComparisonResult:
    """Verify that Cassandra preserves the semantic meaning of a request.

    This is the main entry point for verifying Cassandra's accuracy preservation.
    It compares the original messages with those modified by Cassandra and
    raises an error if differences are found (when fail_on_difference=True).

    Args:
        original_messages: Messages before Cassandra processing.
        cassandra_messages: Messages after Cassandra processing.
        judge_model: The model to use for judging.
        api_key: OpenAI API key.
    ...
    Example:
        # Capture messages before and after Cassandra
        original = [{"role": "user", "content": "Hello, how are you?"}]
        after_cassandra = [{"role": "user", "content": "Hello, how are you?"}]

        result = verify_cassandra_preservation(
            original, after_cassandra, fail_on_difference=True
        )
    """
    result = compare_messages(
        original_messages=original_messages,
        modified_messages=cassandra_messages,
        judge_model=judge_model,
        api_key=api_key,
    )
    ...
    if fail_on_difference and not result.are_equivalent:
        raise ValueError(
            f"Cassandra modified prompt semantics! "
