"""CLI entry point for Cassandra evaluation framework.

Usage:
    python -m cassandra.evals quick              # Quick sanity check (5 samples)
    python -m cassandra.evals benchmark          # Full benchmark suite
    python -m cassandra.evals list               # List available datasets
    python -m cassandra.evals --help             # Show help

Install dependencies:
    pip install cassandra-ai[evals]
"""
...
if TYPE_CHECKING:
    from cassandra.evals.core import EvalResult


def cmd_quick(args: argparse.Namespace) -> None:
    """Run quick sanity check."""
    from cassandra.evals.runners.before_after import run_quick_eval
...
def cmd_list(args: argparse.Namespace) -> None:
    ...
    from cassandra.evals.datasets import DATASET_REGISTRY, list_available_datasets
    ...
    print("\n" + "=" * 60)
    print("Usage: python -m cassandra.evals benchmark --dataset <name> -n <samples>")
    print("=" * 60 + "\n")


def cmd_benchmark(args: argparse.Namespace) -> None:
    """Run full benchmark suite."""
    from cassandra.evals.datasets import (
        DATASET_REGISTRY,
        load_dataset_by_name,
        load_tool_output_samples,
    )
    from cassandra.evals.runners.before_after import BeforeAfterRunner, LLMConfig

    print("=" * 60)
    print("CASSANDRA COMPRESSION ACCURACY BENCHMARK")
    print("=" * 60)
    ...
    if args.dataset == "all":
        ...
        from cassandra.evals.datasets import list_available_datasets
    ...
    for name in dataset_names:
        ...
        try:
            ...
            print("  (Install with: pip install cassandra-ai[evals])")
...
def cmd_suite(args: argparse.Namespace) -> None:
    ...
    from cassandra.evals.reports.report_card import save_reports
    from cassandra.evals.suite_runner import SuiteRunner

    tiers = list(range(1, args.tier + 1))

    print("=" * 60)
    print("CASSANDRA EVALUATION SUITE")
    print("=" * 60)
...
<html>
<head>
    <title>Cassandra Evaluation Report</title>
    <style>
        body {{
...
<body>
    <h1>Cassandra Compression Accuracy Report</h1>
...
"""
    ...
    html += """
    <footer>
        <p>Generated by <strong>Cassandra Evaluation Framework</strong></p>
        <p>Install: <code>pip install cassandra-ai[evals]</code></p>
    </footer>
</body>
</html>
"""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Cassandra Evaluation Framework - Prove compression preserves accuracy",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python -m cassandra.evals quick                        # Quick 5-sample check
  python -m cassandra.evals quick -n 10                  # Quick 10-sample check
  python -m cassandra.evals list                         # List available datasets
  python -m cassandra.evals benchmark                    # Run tool_outputs benchmark
  python -m cassandra.evals benchmark --dataset hotpotqa -n 50
  python -m cassandra.evals benchmark --dataset rag      # Run all RAG datasets
  python -m cassandra.evals benchmark --dataset all      # Run ALL datasets
  python -m cassandra.evals suite --tier 1               # Run Tier 1 suite (~$3)
  python -m cassandra.evals suite --tier 2               # Run Tiers 1+2 (~$8)
  python -m cassandra.evals suite --tier 1 --ci          # CI mode (exit 1 on fail)
  python -m cassandra.evals report -i results.json       # Generate HTML report
...
""",
    ...
    suite_parser.add_argument("--port", type=int, default=8787, help="Cassandra proxy port")
