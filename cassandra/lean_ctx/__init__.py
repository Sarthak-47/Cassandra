"""lean-ctx integration for Cassandra.

lean-ctx configures supported coding agents to route tool output through its
context-filtering layer. Cassandra downloads and manages the lean-ctx binary.
"""
...
def _managed_lean_ctx_candidates() -> list[Path]:
    """Return known Cassandra-managed lean-ctx binary paths."""
...
def get_lean_ctx_path() -> Path | None:
    """Get path to lean-ctx binary — check PATH first, then ~/.cassandra/bin/."""
