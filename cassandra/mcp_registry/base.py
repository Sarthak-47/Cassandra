...
The MCP protocol itself is universal, and the cassandra MCP server
(``cassandra mcp serve``) is a single stdio binary that any compliant client
...
The orchestrator in :mod:`cassandra.mcp_registry.install` calls a fleet of
registrars to install cassandra across every detected agent.
"""
