"""Cassandra-owned MCP install ledger.

The ledger tracks MCP servers that Cassandra registered on the user's behalf
when the target agent config cannot carry Cassandra-specific ownership markers.
It lets unwrap remove only entries still matching the spec Cassandra installed,
...
def ledger_path() -> Path:
    """Return the Cassandra MCP install ledger path."""
...
def record_install(agent: str, spec: ServerSpec, *, path: Path | None = None) -> None:
    """Record that Cassandra installed ``spec`` for ``agent``."""
...
def cassandra_installed_matching(
    agent: str,
    current_spec: ServerSpec | None,
    *,
    path: Path | None = None,
) -> bool:
    """Return True when the ledger says Cassandra installed ``current_spec``."""
