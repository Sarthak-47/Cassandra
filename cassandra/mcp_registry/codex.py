...
_MARKER_START = "# --- Cassandra MCP server ---"
_MARKER_END = "# --- end Cassandra MCP server ---"


def _marker_start(server_name: str) -> str:
    if server_name == "cassandra":
        return _MARKER_START
    return f"# --- Cassandra MCP server: {server_name} ---"


def _marker_end(server_name: str) -> str:
    if server_name == "cassandra":
        return _MARKER_END
    return f"# --- end Cassandra MCP server: {server_name} ---"
    ...
    def register_server(self, spec: ServerSpec, *, force: bool = False) -> RegisterResult:
        ...
        if existing is not None and not force:
            ...
            if _marker_start(spec.name) not in content:
                ...
                return RegisterResult(
                    ...
                    f"{spec.name}] entry outside Cassandra markers; "
        ...
        if existing is not None and force:
            ...
            if _marker_start(spec.name) not in content:
                ...
                # Cassandra owns. Otherwise appending our table would create a
            ...
            # Drop any prior Cassandra block before re-writing.
...
def _render_block(spec: ServerSpec) -> str:
    """Render a Cassandra-marked TOML block for ``spec``."""
    lines: list[str] = [
