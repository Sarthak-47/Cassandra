"""Top-level orchestration: register Cassandra MCP across detected agents."""

from __future__ import annotations

from collections.abc import Iterable

from cassandra.install.runtime import resolve_cassandra_command
...
def build_cassandra_spec(proxy_url: str = DEFAULT_PROXY_URL) -> ServerSpec:
    """Construct the canonical :class:`ServerSpec` for the cassandra server.
    ...
    if proxy_url and proxy_url != DEFAULT_PROXY_URL:
        env["CASSANDRA_PROXY_URL"] = proxy_url
    command = resolve_cassandra_command()
    return ServerSpec(
        name="cassandra",
        command=command[0],
...
def build_serena_spec(context: str) -> ServerSpec:
    ...
    startup. Cassandra installs Serena by default, so without this flag every
...
def install_everywhere(
    ...
    *,
    ...
    """Install the cassandra MCP server into every detected agent.
