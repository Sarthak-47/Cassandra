...
This benchmark demonstrates WHY Cassandra matters by showing:
...
2. THE SOLUTION: Cassandra's impact on real workloads
...
# Cassandra imports
from cassandra.transforms.smart_crusher import SmartCrusherConfig, smart_crush_tool_output
...
def benchmark_coding_agent_explosion() -> BenchmarkResult:
    ...
    # Apply Cassandra transforms using convenience function
...
def benchmark_cache_alignment() -> BenchmarkResult:
    ...
    """
    from cassandra.cache import DetectorConfig, DynamicContentDetector
...
def benchmark_rag_scaling() -> BenchmarkResult:
    """
    Show how RAG context grows and how Cassandra manages it.
...
def benchmark_conversation_scaling() -> list[BenchmarkResult]:
    ...
    and shows the scaling curve with and without Cassandra.
    """
    results = []
...
def _generate_terminal_report(results: list[BenchmarkResult]) -> str:
    ...
    lines.append("")
    lines.append("=" * 80)
    lines.append("  CASSANDRA AGENT COST BENCHMARK")
    ...
    lines.append(f"    Without Cassandra:    ${total_baseline * 1_000_000:.2f}")
    lines.append(f"    With Cassandra:       ${(total_baseline - total_savings) * 1_000_000:.2f}")
...
def _generate_markdown_report(results: list[BenchmarkResult]) -> str:
    ...
    lines.append("")
    lines.append("This benchmark demonstrates Cassandra's impact on real-world agent workloads:")
    lines.append("")
    ...
    lines.append("")
    lines.append("| Scale | Without Cassandra | With Cassandra | Monthly Savings |")
    lines.append("|-------|------------------|---------------|-----------------|")
...
def main():
    parser = argparse.ArgumentParser(description="Cassandra Agent Cost Benchmark")
