#!/usr/bin/env python3
"""Latency benchmark for Cassandra compression pipeline.
...
class ProfilingPipeline:
    ...
    def apply(
        ...
        from cassandra.utils import deep_copy_messages
...
def format_terminal_report(results: list[LatencyResult]) -> str:
    ...
    lines.append("")
    lines.append("=" * 100)
    lines.append("  CASSANDRA LATENCY BENCHMARK")
    lines.append("=" * 100)
    lines.append("")
...
def format_markdown_report(results: list[LatencyResult]) -> str:
    ...
    lines.append("# Cassandra Latency Benchmarks")
    lines.append("")
    lines.append(
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Cassandra latency benchmark",
    ...
    print("Cassandra Latency Benchmark")
    print("=" * 40)
    print()
