"""
Cassandra Worst-Case Benchmark: Where Compression Hurts

This benchmark tests scenarios where Cassandra's statistical compression
may NOT be beneficial - to understand the limits of the approach.

Worst cases for Cassandra:
...
def generate_unique_support_tickets(num_tickets: int = 50) -> dict:
    ...
    This is hard for Cassandra because:
...
class WorstCaseScenario:
    """A scenario where Cassandra might struggle."""
...
def run_worst_case_benchmark(api_key: str = None) -> dict:
    ...
    print("=" * 70)
    print("CASSANDRA WORST-CASE BENCHMARK")
    ...
    if CASSANDRA_AVAILABLE:
        db_path = os.path.join(tempfile.gettempdir(), "cassandra_worst_case.db")
        ...
        print("WARNING: Cassandra not available, running baseline only")
    ...
    for scenario in scenarios:
        ...
        if cassandra_client:
            ...
            cassandra_result = run_scenario(cassandra_client, scenario, "cassandra")
            print(f"   Input tokens: {cassandra_result.input_tokens:,}")
            print(f"   Output tokens: {cassandra_result.output_tokens:,}")
            print(f"   Cost: ${cassandra_result.cost_usd:.4f}")

            avg_cassandra_score = (
                sum(v["score"] for v in cassandra_result.validation_scores.values())
                / len(cassandra_result.validation_scores)
                if cassandra_result.validation_scores
                else 0
            )
            print(f"   Validation score: {avg_cassandra_score:.1%}")
            ...
            if baseline_result.input_tokens > 0:
                token_change = (
                    cassandra_result.input_tokens - baseline_result.input_tokens
                ) / baseline_result.input_tokens
                quality_change = avg_cassandra_score - avg_baseline_score
    ...
    print(f"\n{'Scenario':<30} {'Baseline Tokens':>15} {'Cassandra Tokens':>15} {'Quality Δ':>12}")
    print("-" * 72)

    for br in baseline_results:
        hr = next((r for r in cassandra_results if r.scenario_name == br.scenario_name), None)
        if hr:
            b_score = (
