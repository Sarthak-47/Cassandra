...
Strategies:
  1. Baseline — no Cassandra, no markers
  2. Cassandra compression — full pipeline, CompressionCache keeps bytes stable
  3. Cassandra + prefix freeze — pipeline skips frozen (already-cached) messages
  4. Cassandra + explicit markers — pipeline + 4 cache_control breakpoints
...
CACHE_MODE_EXPLICIT = "explicit_4"  # Cassandra's 4 strategic breakpoints
...
def main():
    ...
    # Strategy 3: Cassandra explicit markers (4 strategic breakpoints)
    if "markers" in strategies_to_run:
        step += 1
        print(f"[{step}/{num_strategies}] Cassandra explicit (4 strategic breakpoints)...")
        r = run_strategy(
            "Cassandra 4x",
            api_key,
            model,
