"""
Truncation vs Summarization vs Cassandra: A Fair Benchmark
...
3. Cassandra - Statistical compression with retrieval
...
try:
    ...
    from cassandra.tokenizers import TiktokenCounter
...
try:
    from cassandra.transforms.kompress_compressor import KompressCompressor, is_kompress_available
...
def generate_metrics_data(n_points: int = 500) -> tuple[list[dict], list[Question]]:
    ...
    This is where Cassandra should excel - detecting statistical outliers.
    """
...
def kompress_compress(data: list[dict]) -> tuple[str, dict]:
    ...
    if not KOMPRESS_AVAILABLE:
        raise RuntimeError("Kompress not available. Install with: pip install cassandra-ai[ml]")
...
def cassandra_compress(data: list[dict], query_context: str = "") -> tuple[list[dict], dict]:
    """
    Use Cassandra's SmartCrusher for statistical compression.
...
def run_scenario_benchmark(
    ...
    if config.run_kompress:
        ...
        if KOMPRESS_AVAILABLE:
            ...
            print("   Kompress not available. Install with: pip install cassandra-ai[ml]")

    # --- CASSANDRA ---
    print("\n[4/4] Running Cassandra...")
    if CASSANDRA_AVAILABLE:
        try:
            # Use first question as query context (realistic usage)
            query_context = scenario.questions[0].text if scenario.questions else ""
            compressed, metadata = cassandra_compress(scenario.data, query_context)
...
def run_full_benchmark(client: "OpenAI", config: BenchmarkConfig = None) -> dict:
    ...
    print("\n" + "=" * 70)
    print("TRUNCATION vs SUMMARIZATION vs LLMLINGUA-2 vs CASSANDRA BENCHMARK")
    print("=" * 70)

    # Generate scenarios
    scenarios = []

    # Scenario 1: Logs (Cassandra should win - needs anomaly detection)
    ...
    # Scenario 3: Metrics (Cassandra should win - statistical outliers)
...
def generate_summary(results: list[ApproachResult], scenarios: list[Scenario]) -> str:
    ...
    for approach in ["truncation", "summarization", "llmlingua-2", "cassandra"]:
