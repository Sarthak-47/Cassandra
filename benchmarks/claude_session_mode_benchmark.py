...
from cassandra.cache.prefix_tracker import PrefixCacheTracker
from cassandra.pricing.litellm_pricing import get_model_pricing
...
try:
    from cassandra.proxy.modes import PROXY_MODE_CACHE, PROXY_MODE_TOKEN
except ImportError:
...
def _make_proxy(mode: str) -> CassandraProxy:
    cfg = ProxyConfig(
        mode=mode,
        optimize=True,
        image_optimize=True,
    ...
    return CassandraProxy(cfg)


def _apply_mode_to_messages(
    proxy: CassandraProxy | None,
    mode: str,
...
def _disable_cassandra_benchmark_logging() -> None:
    logging.raiseExceptions = False
    for logger_name in (
        "cassandra",
        "cassandra.cache",
        "cassandra.cache.compression_cache",
        "cassandra.proxy",
        "cassandra.transforms",
    ):
...
def _simulate_single_replay_mode(
    ...
    _disable_cassandra_benchmark_logging()
    ...
    for mode in ("baseline", PROXY_MODE_TOKEN, PROXY_MODE_CACHE):
        ...
        if worker_count > 1 and total > 1:
            with concurrent.futures.ProcessPoolExecutor(
                ...
                initializer=_disable_cassandra_benchmark_logging,
            ) as executor:
...
<body>
  <div class="shell">
    <section class="hero">
      ...
      <p>Observed usage is read directly from <code>~/.claude/projects</code>. Baseline, token, and cache are replayed locally through Cassandra without making API calls.</p>
...
def main() -> int:
    args = parse_args()
    logging.getLogger("cassandra.transforms").setLevel(logging.WARNING)
    logging.getLogger("cassandra.proxy").setLevel(logging.WARNING)
