"""RTK-loop eval — does Cassandra Learn catch a loop and write a guardrail that
...
Usage:
    ...
    CASSANDRA_LEARN_CLI=claude python benchmarks/rtk_loop_learn_eval.py --real  # via CLI
"""
...
from cassandra.learn.analyzer import SessionAnalyzer  # noqa: E402
from cassandra.learn.fixtures import rtk_refetch_loop_session  # noqa: E402
from cassandra.learn.loops import detect_loops  # noqa: E402
from cassandra.learn.models import (  # noqa: E402
    ProjectInfo,
    SessionData,
    ToolCall,
)
...
def run_eval(*, use_real_llm: bool) -> Scorecard:
    ...
    phase1_ctx = (
        nullcontext()
        if use_real_llm
        else patch("cassandra.learn.analyzer._call_llm", _stub_llm_phase1)
    )
    with phase1_ctx:
    ...
    if guardrail:
        ...
        phase2_ctx = (
            nullcontext()
            if use_real_llm
            else patch("cassandra.learn.analyzer._call_llm", _stub_llm_phase2)
        )
        with phase2_ctx:
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "(claude / gemini / codex; force one with CASSANDRA_LEARN_CLI=claude).",
    )
