...
from cassandra.cache.compression_feedback import (
...
from cassandra.transforms.smart_crusher import (
    SmartCrusherConfig,
...
def test_cassandra_marker_collision() -> AdversarialResult:
    """
    ATTACK: Input data already contains __cassandra_ fields.
    """
    result = AdversarialResult(
        name="Marker Field Collision",
        category="injection",
        expected_behavior="Should not confuse existing __cassandra_ fields with our markers",
        severity="high",
    )

    # Data that already has __cassandra_ fields
    items = [
        {
            "id": i,
            "__cassandra_compressed": True,  # Fake marker!
            "__cassandra_hash": "fakehash12345678",
            "__cassandra_stats": {"fake": True},
        }
        for i in range(100)
    ]
...
def main():
    ...
    tests = [
        ...
        test_cassandra_marker_collision,
