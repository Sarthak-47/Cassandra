"""
Cassandra ADVERSARIAL Benchmark: True Worst Cases
...
Focus on methodology differences and key findings.""",
        ...
        expected_behavior="Cassandra should have minimal compression - prose has no structural redundancy",
    )
...
Don't skip any file or change.""",
        ...
        expected_behavior="Cassandra should struggle - code changes are unique and can't be summarized",
    )
...
Describe what you observe about the data format and structure.""",
        ...
        expected_behavior="Cassandra CANNOT compress this - random data has no patterns",
    )
...
def create_small_data_scenario() -> AdversarialScenario:
    return AdversarialScenario(
        ...
        expected_behavior="Cassandra has no opportunity - data is already minimal",
    )
...
Pay attention to who said what and how opinions evolved.""",
        ...
        expected_behavior="Cassandra should preserve conversation flow - context matters",
    )
...
def run_adversarial_benchmark(api_key: str = None) -> dict:
    ...
    print("=" * 70)
    print("CASSANDRA ADVERSARIAL BENCHMARK")
    ...
    if CASSANDRA_AVAILABLE:
        db_path = os.path.join(tempfile.gettempdir(), "cassandra_adversarial.db")
    ...
    for scenario in scenarios:
        ...
        if cassandra_client:
            print("\n[2/2] CASSANDRA...")
            cassandra = run_scenario(cassandra_client, scenario, "cassandra")
            print(f"   Input tokens: {cassandra.input_tokens:,}")
            print(f"   Cost: ${cassandra.cost_usd:.4f}")
            results.append(cassandra)

            if baseline.input_tokens > 0:
                change = (cassandra.input_tokens - baseline.input_tokens) / baseline.input_tokens
                print(f"\n   📊 Token change: {change:+.1%}")
                if change > 0:
                    print("   ⚠️  CASSANDRA INCREASED TOKENS (overhead > savings)")
                elif change > -0.1:
    ...
    print(f"\n{'Scenario':<30} {'Baseline':>12} {'Cassandra':>12} {'Change':>12}")
    print("-" * 66)
