"""
Real-World Agent Benchmark: MCP Tools + Cassandra
...
We measure:
- Token usage with vs without Cassandra
- Cost savings
...
def run_agent_scenario(
    ...
    # Determine if using CassandraClient
    is_cassandra = isinstance(client, CassandraClient) if CASSANDRA_AVAILABLE else False
    mode = "cassandra" if is_cassandra else "baseline"
...
def run_full_benchmark(api_key: str = None) -> dict:
    """Run complete benchmark comparing baseline vs Cassandra."""
    ...
    # Cassandra-wrapped client
    db_path = os.path.join(tempfile.gettempdir(), "cassandra_benchmark.db")
    ...
    print("\n" + "=" * 70)
    print("REAL-WORLD AGENT BENCHMARK: MCP Tools + Cassandra")
    print("=" * 70)
    ...
    for scenario in scenarios:
        ...
        # Run with Cassandra
        print("\n[2/2] Running CASSANDRA (optimized)...")
        cassandra_result = run_agent_scenario(cassandra_client, scenario)
        print(f"   Input tokens: {cassandra_result.total_input_tokens:,}")
        print(f"   Output tokens: {cassandra_result.total_output_tokens:,}")
        print(f"   Cost: ${cassandra_result.cost_usd:.4f}")
        print(f"   Answer quality: {cassandra_result.answer_quality:.1%}")
        results.append(cassandra_result)

        # Calculate savings
        if baseline_result.total_input_tokens > 0:
            token_savings = 1 - (
                cassandra_result.total_input_tokens / baseline_result.total_input_tokens
            )
            cost_savings = (
                1 - (cassandra_result.cost_usd / baseline_result.cost_usd)
            ...
            print(
                f"   Quality preserved: {'✓' if cassandra_result.answer_quality >= baseline_result.answer_quality * 0.9 else '✗'}"
            )
    ...
    cassandra_results = [r for r in results if r.mode == "cassandra"]

    total_baseline_tokens = sum(r.total_input_tokens for r in baseline_results)
    total_cassandra_tokens = sum(r.total_input_tokens for r in cassandra_results)
    total_baseline_cost = sum(r.cost_usd for r in baseline_results)
    total_cassandra_cost = sum(r.cost_usd for r in cassandra_results)

    print(f"\n{'Metric':<25} {'Baseline':>15} {'Cassandra':>15} {'Savings':>15}")
    print("-" * 70)
    ...
    print(
        f"{'Total Cost':<25} ${total_baseline_cost:>14.4f} ${total_cassandra_cost:>14.4f} {cost_savings:>14.1%}"
    )
    ...
    avg_cassandra_quality = (
        sum(r.answer_quality for r in cassandra_results) / len(cassandra_results)
        if cassandra_results
        else 0
    )
    print(
        f"{'Avg Answer Quality':<25} {avg_baseline_quality:>14.1%} {avg_cassandra_quality:>14.1%} {'preserved' if avg_cassandra_quality >= avg_baseline_quality * 0.9 else 'degraded':>15}"
    )

    return {
        "baseline": [r.__dict__ for r in baseline_results],
        "cassandra": [r.__dict__ for r in cassandra_results],
        "summary": {
            "total_baseline_tokens": total_baseline_tokens,
            "total_cassandra_tokens": total_cassandra_tokens,
            "token_savings": token_savings,
            "total_baseline_cost": total_baseline_cost,
            "total_cassandra_cost": total_cassandra_cost,
