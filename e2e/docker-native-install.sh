...
IMAGE="${CASSANDRA_DOCKER_IMAGE:?set CASSANDRA_DOCKER_IMAGE to a built test image}"
PROFILE="ci-smoke"
TMP_HOME="$(mktemp -d)"
PORT="$(python3 - <<'PY'
import socket
...
cleanup() {
  docker rm -f "cassandra-${PROFILE}" >/dev/null 2>&1 || true
  rm -rf "${TMP_HOME}"
}
trap cleanup EXIT
...
export CASSANDRA_DOCKER_IMAGE="${IMAGE}"

bash "${ROOT_DIR}/scripts/install.sh"

WRAPPER="${HOME}/.local/bin/cassandra"
[[ -x "${WRAPPER}" ]]
...
manifest = json.loads((home / ".cassandra" / "deploy" / profile / "manifest.json").read_text())
...
# the container down.
CONTAINER_NAME="cassandra-${PROFILE}"
expected_workspace_dir="/tmp/cassandra-home/.cassandra"
expected_config_dir="/tmp/cassandra-home/.cassandra/config"

container_env="$(docker inspect --format '{{range .Config.Env}}{{println .}}{{end}}' "${CONTAINER_NAME}")"
printf '%s\n' "${container_env}"

if ! grep -Fxq "CASSANDRA_WORKSPACE_DIR=${expected_workspace_dir}" <<<"${container_env}"; then
  echo "CASSANDRA_WORKSPACE_DIR missing from ${CONTAINER_NAME} env (expected=${expected_workspace_dir})" >&2
  exit 1
fi
if ! grep -Fxq "CASSANDRA_CONFIG_DIR=${expected_config_dir}" <<<"${container_env}"; then
  echo "CASSANDRA_CONFIG_DIR missing from ${CONTAINER_NAME} env (expected=${expected_config_dir})" >&2
  exit 1
fi
...
if ! grep -Fxq "CASSANDRA_WORKSPACE_DIR=${expected_workspace_dir}" <<<"${exec_env}"; then
  echo "CASSANDRA_WORKSPACE_DIR not visible to docker exec in ${CONTAINER_NAME}" >&2
  exit 1
fi
if ! grep -Fxq "CASSANDRA_CONFIG_DIR=${expected_config_dir}" <<<"${exec_env}"; then
  echo "CASSANDRA_CONFIG_DIR not visible to docker exec in ${CONTAINER_NAME}" >&2
  exit 1
fi
...
[[ ! -e "${HOME}/.cassandra/deploy/${PROFILE}" ]]
