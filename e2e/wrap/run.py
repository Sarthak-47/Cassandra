...
RTK_MARKER = "<!-- cassandra:rtk-instructions -->"
PROXY_PORT = 28887
CODEX_PORT = 28888
AIDER_PORT = 28889
CURSOR_PORT = 28890
...
def create_shims(shim_dir: Path) -> None:
    ...
    codex_shim = textwrap.dedent(
        ...
        if openai_base:
            ...
            chat_status, chat_body = request_json(
                ...
                payload={
                    ...
                    "messages": [
                        {
                            "role": "user",
                            "content": "Confirm Cassandra received this wrapped Codex message.",
                        }
                    ],
                },
                headers=auth_headers,
            ...
            record["cassandra_request_total"] = (
                requests.get("total") if isinstance(requests, dict) else None
            )
            record["cassandra_model_count"] = (
...
def start_proxy(port: int, env: dict[str, str]) -> subprocess.Popen[str]:
    log(f"Starting cassandra proxy on port {port}")
    proc = subprocess.Popen(
        ["cassandra", "proxy", "--host", "127.0.0.1", "--port", str(port)],
        env=env,
...
def verify_installs() -> None:
    ...
    for tool in ("cassandra", "codex", "aider", "openclaw"):
        assert_true(shutil.which(tool) is not None, f"Expected '{tool}' on PATH")
    run(["cassandra", "--help"], timeout=30)
...
def prepare_local_openclaw_plugin(base_env: dict[str, str], tmp_dir: Path) -> Path:
    ...
    package_json["dependencies"]["cassandra-ai"] = f"file:{tarball_path.as_posix()}"
...
def verify_codex_wrap(
    ...
    port = CODEX_PORT
    run(
        ["cassandra", "wrap", "codex", "--port", str(port), "--", "--help"],
        env=base_env,
        cwd=project_dir,
        timeout=120,
    )
    ...
    assert_true(
        ...
        "Codex wrap should inject the cassandra provider base_url",
    )
    assert_true(
        'env_key = "OPENAI_API_KEY"' not in config,
        "Codex wrap should preserve OAuth and never inject env_key",
    )
    # Bug 3 (#406): requires_openai_auth must be absent from cassandra provider blocks.
    assert_true(
        "requires_openai_auth" not in config,
        "Codex wrap must NOT inject requires_openai_auth into the cassandra provider block",
    )
    assert_true(
    ...
    assert_true(
        ...
        "Codex shim should prove OPENAI_BASE_URL points at a live proxy and that Cassandra logged the wrapped message",
    )
    assert_true(
        entries[-1].get("chat_completion") == "mock completion from upstream",
        "Codex wrap should receive the mock upstream completion through Cassandra",
    )
    assert_true(
        entries[-1].get("cassandra_model_count", 0) >= 1,
        "Codex wrap should appear in Cassandra request stats",
    )
    assert_true(
        any(
            item["path"] == "/v1/chat/completions"
            and isinstance(item.get("body"), dict)
            and item["body"].get("model") == "cassandra-wrap-e2e"
            for item in mock_server.requests
        ),
        "Codex wrap should forward the wrapped message upstream through Cassandra",
    )
...
def verify_aider_wrap(base_env: dict[str, str], project_dir: Path, log_dir: Path) -> None:
    port = AIDER_PORT
    run(
        ["cassandra", "wrap", "aider", "--port", str(port), "--", "--help"],
        env=base_env,
        cwd=project_dir,
        timeout=120,
    )
...
def verify_cursor_wrap(base_env: dict[str, str], project_dir: Path) -> None:
    port = CURSOR_PORT
    proc = subprocess.Popen(
        ["cassandra", "wrap", "cursor", "--port", str(port)],
        env=base_env,
        cwd=str(project_dir),
        stdout=subprocess.PIPE,
        stderr=subprocess.STDOUT,
...
def verify_cline_wrap(base_env: dict[str, str], project_dir: Path) -> None:
    ...
    run(
        ["cassandra", "wrap", "cline", "--prepare-only", "--port", str(CLINE_PORT)],
        env=base_env,
        cwd=project_dir,
        timeout=60,
    )
...
def verify_continue_wrap(base_env: dict[str, str], project_dir: Path) -> None:
    ...
    run(
        ["cassandra", "wrap", "continue", "--prepare-only", "--port", str(CONTINUE_PORT)],
        env=base_env,
        cwd=project_dir,
        timeout=60,
    )
...
def verify_goose_wrap(base_env: dict[str, str], project_dir: Path) -> None:
    ...
    run(
        ["cassandra", "wrap", "goose", "--prepare-only", "--port", str(GOOSE_PORT)],
        env=base_env,
        cwd=project_dir,
        timeout=60,
    )
...
def verify_openclaw_wrap(
    ...
    try:
        ...
        entry = state["plugins"]["entries"]["cassandra"]
        ...
        assert_true(
            state["plugins"]["slots"]["contextEngine"] == "cassandra",
    ...
    run(["cassandra", "unwrap", "openclaw"], env=base_env, cwd=project_dir, timeout=120)
    state = json.loads(config_path.read_text(encoding="utf-8"))
    assert_true(
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(
        prefix="cassandra-wrap-e2e-", ignore_cleanup_errors=True
    ) as tmp_dir_str:
        ...
        base_env.update(
            {
                ...
                "CASSANDRA_E2E_LOG_DIR": str(log_dir),
...
def verify_opencode_wrap(base_env: dict[str, str], project_dir: Path, log_dir: Path) -> None:
    port = OPENCODE_PORT
    run(
        ["cassandra", "wrap", "opencode", "--port", str(port), "--", "--help"],
        env=base_env,
        cwd=project_dir,
        timeout=120,
    )
    ...
    assert_true(
        config["provider"]["cassandra"]["options"]["baseURL"] == f"http://127.0.0.1:{port}/v1",
        "Opencode wrap should inject cassandra provider baseURL",
    )

    run(
        ["cassandra", "unwrap", "opencode", "--port", str(port)],
        env=base_env,
        cwd=project_dir,
        timeout=120,
    )
    ...
    if config_path.exists():
        ...
        assert_true(
            "cassandra" not in content,
            "Opencode unwrap should remove cassandra provider from config",
        )
