"""Docker e2e cases for ``cassandra init``.
...
1. **existing sequence**: preserves the original scenario that exercised
   ``cassandra init claude`` (local) -> ``init -g copilot`` (global) ->
...
def _expected_cassandra_mcp_calls(proxy_url: str) -> list[list[str]]:
    # The harness restores PATH before assertions, but `cassandra init` ran with a
    ...
    prefix = [
        ...
        "user",
        "-e",
        f"CASSANDRA_PROXY_URL={proxy_url}",
        "--",
    ]
    variants = [
        [*prefix, *resolve_cassandra_command(), "mcp", "serve"],
        [*prefix, sys.executable, "-m", "cassandra.cli", "mcp", "serve"],
    ]
...
def _read_manifest(home: Path, profile: str) -> dict[str, object]:
    path = home / ".cassandra" / "deploy" / profile / "manifest.json"
    if not path.exists():
...
def _verify_claude_local(ctx: CaseContext) -> None:
    ...
    # `init` auto-registers the cassandra MCP server after the marketplace
    # install (see d9d8972 — keeps `[Retrieve more: hash=…]` markers from
    # being dead pointers for users who never ran `cassandra mcp install`).
    # The `-e CASSANDRA_PROXY_URL=…` arg is only emitted when the proxy
    # port differs from the 8787 default; this case uses --port 9011.
    expected_prefix = [
        ["plugin", "marketplace", "add", str(REPO_ROOT_IN_CONTAINER)],
        ["plugin", "install", "cassandra@cassandra-marketplace", "--scope", "local"],
    ]
    expected_mcp_calls = _expected_cassandra_mcp_calls("http://127.0.0.1:9011")
    if (
        len(claude_calls) != 3
...
def _verify_copilot_global(ctx: CaseContext) -> None:
    ...
    expected = [
        ["plugin", "marketplace", "add", str(REPO_ROOT_IN_CONTAINER)],
        ["plugin", "install", "cassandra@cassandra-marketplace"],
    ]
...
def _verify_codex_local(ctx: CaseContext) -> None:
    ...
    if "requires_openai_auth" in config:
        raise AssertionError(
            "Codex local init must NOT inject requires_openai_auth into the cassandra provider block"
        )
...
def _verify_codex_global(ctx: CaseContext) -> None:
    ...
    if "requires_openai_auth" in config:
        raise AssertionError(
            "Codex global init must NOT inject requires_openai_auth into the cassandra provider block"
        )
    if "supports_websockets = true" not in config:
...
def bare_init_g_cases() -> list[Case]:
    """Bare ``cassandra init -g`` — the direct coverage of issue #245."""
...
def per_subcommand_cases() -> list[Case]:
    """One case per ``cassandra init -g <agent>`` with only that agent's shim."""
    ...
    return [
        ...
        # openclaw delegates to `cassandra wrap openclaw` which has its own
