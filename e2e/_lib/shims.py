...
tests drive `cassandra init` without requiring a real Claude/Codex install.
...
* ``record-args``  — appends a JSON record of (tool, argv, cwd) to the file at
                     ``$CASSANDRA_E2E_SHIM_LOG``, then exits 0. Useful for
...
tool="${0##*/}"
log="${CASSANDRA_E2E_SHIM_LOG:-/dev/null}"
...
_RECORD_CMD = (
    "@echo off\r\n"
    "setlocal\r\n"
    'if "%CASSANDRA_E2E_SHIM_LOG%"=="" set CASSANDRA_E2E_SHIM_LOG=NUL\r\n'
    "python -c \"import json,os,sys; name=r'%~n0'; log=os.environ['CASSANDRA_E2E_SHIM_LOG']; "
