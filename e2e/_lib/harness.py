...
* invoking the ``cassandra`` subprocess with the case's argv
...
class Case:
    ...
    Attributes:
        ...
        argv: Arguments passed to ``cassandra`` (e.g. ``["init", "-g", "claude"]``).
...
def _resolve_cassandra_bin(name: str) -> str:
    """Return the absolute path to the cassandra binary before PATH is scrubbed.

    ``with_clean_path`` intentionally narrows PATH so agent shims dominate;
    that would also hide the real ``cassandra`` binary (typically at
    ``/opt/*venv/bin/cassandra`` or similar). Resolving up-front lets the
...
def _run_single(case: Case, cassandra_bin: str = "cassandra") -> bool:
    """Execute one case. Return True on pass, False on fail."""

    with tempfile.TemporaryDirectory(prefix=f"cassandra-e2e-{case.name}-") as temp_raw:
        ...
        # Resolve cassandra to its absolute path BEFORE mutating PATH so the
        # shim dir can dominate PATH without losing the cassandra binary.
        resolved_bin = _resolve_cassandra_bin(cassandra_bin)

        with with_clean_path([shim_dir]) as env:
            env["HOME"] = str(home)
            env["USERPROFILE"] = str(home)
            env["CASSANDRA_E2E_SHIM_LOG"] = str(shim_log)
...
def run_cases(
    ...
    *,
    ...
    failed = 0
    for case in cases:
        ok = _run_single(case, cassandra_bin=cassandra_bin)
        if ok:
            passed += 1
        else:
            failed += 1
...
def run_case_sequence(
    ...
    *,
    ...
    ``cassandra init`` accumulating targets in a shared manifest across
    successive calls).
    """

    passed = 0
    failed = 0
    with tempfile.TemporaryDirectory(prefix=f"cassandra-e2e-{label}-") as temp_raw:
        temp_root = Path(temp_raw)
        home = temp_root / "home"
        project = temp_root / "project"
        shim_dir = temp_root / "bin"
