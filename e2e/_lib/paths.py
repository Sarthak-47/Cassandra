...
These paths mirror the logic in ``cassandra.cli.init`` so e2e tests can
...
def agent_settings_path(agent: Agent, *, scope: Scope, home: Path, project: Path) -> Path:
    """Return the file that ``cassandra init`` should have written for ``agent``.

    ``home`` is the test's simulated HOME directory and ``project`` is the cwd
    used when invoking ``cassandra init``. For global (``-g``) invocations only
    ...
    if agent == "openclaw":
        # OpenClaw init is delegated to `cassandra wrap openclaw`; it writes
