...
**Owner:** Cassandra realignment.

## TL;DR

**RTK is a wrap-CLI hook, not a proxy-side compressor.** The Cassandra
...
wrap-CLI tail — `cassandra wrap claude`, `cassandra wrap codex`, etc.
...
The Rust proxy already has a `crates/cassandra-core/src/transforms/log_compressor.rs`
...
Every `cassandra wrap <agent>` subcommand:

1. Ensures the RTK binary is installed via `_ensure_rtk_binary()`.
2. Injects the `<!-- cassandra:rtk-instructions -->` block into the
...
See `cassandra/cli/wrap/` for the per-agent shims.
...
- `cassandra/cli/wrap/` — wrap-CLI implementation.
- `crates/cassandra-core/src/transforms/log_compressor.rs` — the
