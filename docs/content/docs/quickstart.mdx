---
title: Quickstart
description: Get Cassandra running in 5 minutes. Install, compress, and send to your LLM with fewer tokens.
---
...
The TypeScript SDK sends messages to a local Cassandra proxy for compression. Start the proxy before using the TS SDK:
...
If you do not want to change any code, run Cassandra as a proxy and point your existing client at it:
...
All requests flow through Cassandra automatically. Check savings at any time:
...
The biggest savings come from tool outputs -- search results, database rows, log files, API responses. Cassandra auto-detects the content type and routes it to the best compressor. No configuration needed.
