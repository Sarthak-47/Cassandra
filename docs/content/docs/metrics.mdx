...
description: Monitor compression performance, cost savings, and system health with Cassandra's built-in metrics, Prometheus endpoint, and SDK APIs.
---

Cassandra provides comprehensive metrics for monitoring compression performance, cost savings, and system health through both the proxy server and the SDK.
...
Persistent savings are stored at `~/.cassandra/proxy_savings.json` and survive proxy restarts. Override the path with `CASSANDRA_SAVINGS_PATH`.
...
```
# HELP cassandra_requests_total Total requests processed
cassandra_requests_total{mode="optimize"} 1234

# HELP cassandra_tokens_saved_total Total tokens saved
cassandra_tokens_saved_total 5678900

# HELP cassandra_compression_ratio Compression ratio histogram
cassandra_compression_ratio_bucket{le="0.5"} 890
cassandra_compression_ratio_bucket{le="0.7"} 1100
cassandra_compression_ratio_bucket{le="0.9"} 1200

# HELP cassandra_latency_seconds Request latency histogram
cassandra_latency_seconds_bucket{le="0.01"} 800
cassandra_latency_seconds_bucket{le="0.1"} 1150

# HELP cassandra_cache_hits_total Cache hit counter
...
# Increase verbosity
cassandra proxy --log-level debug
```
</Tab>
</Tabs>
...
| Panel | PromQL |
|-------|--------|
| Tokens Saved | `cassandra_tokens_saved_total` |
| Compression Ratio (median) | `histogram_quantile(0.5, cassandra_compression_ratio_bucket)` |
| Request Latency (p99) | `histogram_quantile(0.99, cassandra_latency_seconds_bucket)` |
| Cache Hit Rate | `cassandra_cache_hits_total / (cassandra_cache_hits_total + cassandra_cache_misses_total)` |
