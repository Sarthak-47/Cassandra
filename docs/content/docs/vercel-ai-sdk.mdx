---
title: Vercel AI SDK
description: Compress LLM context with the Vercel AI SDK using middleware, withCassandra(), or standalone compression.
---

Cassandra integrates with the [Vercel AI SDK](https://sdk.vercel.ai) through three patterns: a one-liner wrapper, composable middleware, and standalone message compression.

## Installation

```bash
npm install cassandra-ai ai @ai-sdk/openai
```

<Callout type="info" title="Proxy required">
The TypeScript SDK sends messages to a local Cassandra proxy for compression. Start the proxy before using the SDK:
...
`withCassandra()` calls `wrapLanguageModel` + `cassandraMiddleware()` under the hood. It works with any provider (`@ai-sdk/openai`, `@ai-sdk/anthropic`, `@ai-sdk/google`, etc.).

## cassandraMiddleware() for composition
...
const middleware = cassandraMiddleware({
  model: 'gpt-4o',
...
2. Cassandra compresses them via the proxy's `/v1/compress` endpoint
