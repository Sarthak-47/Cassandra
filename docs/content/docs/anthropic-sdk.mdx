---
title: Anthropic SDK
description: Auto-compress messages in the Anthropic TypeScript SDK with a single withCassandra() wrapper.
---

Cassandra wraps the Anthropic TypeScript SDK to automatically compress messages before every `messages.create()` call. All other methods pass through unchanged.

## Installation

```bash
npm install cassandra-ai @anthropic-ai/sdk
```
...
`withCassandra()` returns a proxy around your Anthropic client that intercepts `messages.create()`:
