...
description: Solutions for common Cassandra issues including proxy startup, connection errors, no token savings, high latency, and installation problems.
---
...
**Fix**: Cassandra pins `ORT_DYLIB_PATH` automatically at import time to the
DLL inside the `onnxruntime` pip package (included in `cassandra-ai[proxy]`).
...
`CASSANDRA_MAGIKA_INIT_TIMEOUT_SECS` (default `5`) bounds the init as a safety
...
**Symptom**: After pointing Claude Code at Cassandra (`ANTHROPIC_BASE_URL`), `/context all`
...
```bash
# Easiest: `cassandra wrap claude` sets ENABLE_TOOL_SEARCH=true automatically.
...
cassandra wrap claude --tool-search auto
...
See [issue #746](https://github.com/Sarthak-47/cassandra/issues/746) for the full analysis.
...
### pipx installs an older Cassandra version

**Symptom**: PyPI shows a newer `cassandra-ai` release, but `pipx install` or
...
environment uses a Python version that Cassandra does not publish wheels for yet,
...
If you already have Cassandra installed under `pipx`, uninstall it first or
...
4. File an issue at [github.com/Sarthak-47/cassandra](https://github.com/Sarthak-47/cassandra/issues) with your Cassandra version, Python version, provider, debug log output, and minimal reproduction code
