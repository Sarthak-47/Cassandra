---
title: OpenAI SDK
description: Auto-compress messages in the OpenAI Node.js SDK with a single withCassandra() wrapper.
---

Cassandra wraps the OpenAI Node.js SDK to automatically compress messages before every `chat.completions.create()` call. All other methods (embeddings, images, audio) pass through unchanged.

## Installation

```bash
npm install cassandra-ai openai
```
...
`withCassandra()` returns a proxy around your OpenAI client that intercepts `chat.completions.create()`:

1. Extracts `messages` from the request params
2. Sends them to the Cassandra proxy's `/v1/compress` endpoint
