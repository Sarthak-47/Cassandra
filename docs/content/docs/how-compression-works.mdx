...
description: Understand Cassandra's three-stage compression pipeline, automatic content routing, and how different content types are compressed.
---

Cassandra automatically detects what kind of content you're sending and routes it to the right compressor. You don't need to configure anything -- just call `compress()` and the pipeline handles the rest.
...
<Tab value="Python">
```python
# Advanced / Internal API -- prefer `from cassandra import compress` for typical use
...
Cassandra doesn't blindly truncate. It identifies what matters in each content type and preserves it:
