...
description: Run Claude Code against Claude models on Azure AI Foundry, with Cassandra compressing your prompts — fewer input tokens, same answers, your own Azure credentials.
---

If your Claude models live on **Azure AI Foundry**, you can still get Cassandra's
prompt compression. Cassandra sits between Claude Code and Azure: it shrinks the
big stuff in each request (file reads, logs, tool output) and forwards the rest to
Azure using **your own Azure credentials**. You keep your Azure setup; Cassandra
...
- **No new secrets** — Cassandra never holds your Azure credentials. Claude Code
  keeps authenticating to Azure AI Foundry with its own `api-key` or Entra Bearer
  token; Cassandra passes it through.

## Before you start

You should already have Claude Code working against Azure AI Foundry **without**
Cassandra. That means these are set in your shell (or your `~/.claude/settings.json`
`env` block):
...
That's it. Because `CLAUDE_CODE_USE_FOUNDRY=1` is set, `cassandra wrap claude`
automatically:

1. derives your Azure AI Foundry endpoint from `ANTHROPIC_FOUNDRY_RESOURCE`,
2. starts the Cassandra proxy with that endpoint as the upstream,
...
```
Claude Code ──(Foundry request)──▶ Cassandra ──(compressed)──▶ Azure AI Foundry (Claude)
   in Foundry mode                  compresses                 your resource
   (your api-key / Entra) ──────── passed through ───────────▶ authenticates you
```

Claude Code sends its Foundry request (Anthropic API format) to Cassandra. Cassandra
...
endpoint URL, Cassandra uses it directly and `ANTHROPIC_FOUNDRY_RESOURCE` is not
...
2. Or check response headers: `x-cassandra-tokens-before`, `x-cassandra-tokens-after`,
   `x-cassandra-tokens-saved`.

## Troubleshooting

**Cassandra says "ANTHROPIC_BASE_URL" instead of "Foundry mode"**

`CLAUDE_CODE_USE_FOUNDRY` is not set in the shell where you ran `cassandra wrap
...
works against Azure AI Foundry directly (without Cassandra) before wrapping. If it
works direct but not through Cassandra, open an issue with the proxy log.
...
extension may not be installed (`pip install "cassandra-ai[proxy]"` includes it).
