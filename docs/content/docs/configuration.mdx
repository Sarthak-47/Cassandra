---
title: Configuration
description: All configuration options for the Cassandra Python and TypeScript SDKs, proxy server, and per-request overrides.
---

Cassandra can be configured via the SDK constructor, proxy command line, environment variables, or per-request overrides.

## CLI Context Tool

`cassandra wrap ...` uses RTK for local shell-output filtering by default.
Set `CASSANDRA_CONTEXT_TOOL=lean-ctx` to have wrap commands install or reuse
`lean-ctx` and run `lean-ctx init --agent <tool>` instead of RTK setup.

```bash
export CASSANDRA_CONTEXT_TOOL=lean-ctx
cassandra wrap claude
cassandra wrap codex --prepare-only
```
...
`CASSANDRA_RTK_GAIN_SCOPE=project` to query `rtk gain --project` from the
proxy process working directory.

## SDK Modes (`default_mode` / `cassandra_mode`)

These modes apply to SDK usage via `CassandraClient(default_mode=...)` or per-request `cassandra_mode=...`. They are **not** the same as the proxy `--mode` flag.
...
> **Proxy `--mode` is a separate axis**: `cassandra proxy --mode token` (maximize compression) or `--mode cache` (freeze prior turns for prefix-cache stability). The proxy does not accept `audit`, `optimize`, or `simulate`.
...
const explicit = new CassandraClient({
...
response = client.chat.completions.create(
    ...
    cassandra_output_buffer_tokens=4000,
    # Keep last N turns uncompressed
    cassandra_keep_turns=3,
)
```

The `RollingWindowConfig`, `IntelligentContextConfig`, and `ScoringWeights` classes are no longer part of Cassandra. Context management now happens automatically inside the pipeline (live-zone-only compression).
...
cassandra proxy --learn --min-evidence 3
```
...
|----------|-------------|---------|
| `CASSANDRA_HOST` | Proxy bind host | `127.0.0.1` |
| `CASSANDRA_PORT` | Proxy bind port | `8787` |
| `CASSANDRA_MODE` | Proxy optimization mode: `token` or `cache` | `token` |
| `CASSANDRA_WORKERS` | Uvicorn worker count | `1` |
| `CASSANDRA_LIMIT_CONCURRENCY` | Maximum concurrent connections before 503 | `1000` |
| `CASSANDRA_MAX_CONNECTIONS` | Maximum upstream HTTP connections | `500` |
| `CASSANDRA_MAX_KEEPALIVE` | Maximum upstream keep-alive connections | `100` |
| `CASSANDRA_KEEPALIVE_EXPIRY` | Seconds an idle upstream keep-alive connection is kept open | `90` |
| `CASSANDRA_BUDGET` | Daily budget limit in USD | -- |
| `CASSANDRA_TELEMETRY` | Set to `on` to opt in to anonymous telemetry | `off` (opt-in) |
| `CASSANDRA_STATELESS` | Set to `true` to disable filesystem writes | `false` |
| `CASSANDRA_MODEL_LIMITS` | Custom model config (JSON string or file path) | -- |
| `CASSANDRA_BASE_URL` | Base URL of the Cassandra proxy (TypeScript SDK) | `http://localhost:8787` |
| `CASSANDRA_API_KEY` | Optional API key for authenticated Cassandra endpoints (TypeScript SDK) | -- |
...
| `CASSANDRA_WORKSPACE_DIR` | Canonical workspace (read-write state) root. Derives savings, memory DB, logs, TOIN, subscription state, and more when set. | `~/.cassandra` |
| `CASSANDRA_SAVINGS_PATH` | Override persistent savings file location. Always wins when set. | derived from `${CASSANDRA_WORKSPACE_DIR}` |
| `CASSANDRA_TOIN_PATH` | Override TOIN telemetry file location. Always wins when set. | derived from `${CASSANDRA_WORKSPACE_DIR}` |
| `CASSANDRA_SUBSCRIPTION_STATE_PATH` | Override subscription tracker state file. Always wins when set. | derived from `${CASSANDRA_WORKSPACE_DIR}` |
| `CASSANDRA_TELEMETRY` | Set to `on` to opt in to anonymous telemetry | `off` |
| `CASSANDRA_PERIODIC_TOIN_STATS` | Controls periodic TOIN stats logging in long-lived proxy workers. Set to `0`, `false`, `off`, or `no` to disable the 5-minute stats loop without disabling TOIN learning or request-time feedback. | `true` |
| `CASSANDRA_MEMORY_INJECTION_MODE` | Memory-context routing mode: `live_zone_tail` (default) or `disabled`. The legacy `system_prompt` mode was retired by PR-A2; supplying it raises. | `live_zone_tail` |
| `CASSANDRA_PROXY_PYTHON_FORWARDER_MODE` | Python forwarder serialization mode. `byte_faithful` (default) forwards original request bytes verbatim when no transform mutated the body and re-serializes canonically only when needed — keeps Anthropic prompt-cache hit-rate intact. `legacy_json_kwarg` is an explicit operator opt-in for emergency rollback to the historical `httpx ... json=body` behavior. NOT a fallback — only flip on explicit operator decision. | `byte_faithful` |
| `CASSANDRA_STRIP_INTERNAL_HEADERS` | Python proxy: whether to strip internal `x-cassandra-*` request headers (e.g. `x-cassandra-bypass`, `x-cassandra-mode`, `x-cassandra-user-id`, `x-cassandra-stack`, `x-cassandra-base-url`) before every upstream forwarder call (PR-A5, fixes P5-49). `enabled` (default) stops fingerprinting / leakage. `disabled` is an explicit operator opt-in for diagnostic shadow tracing — NOT a fallback. Inbound reads of these headers (bypass gating, memory user-id resolution) are unaffected because they read `request.headers` directly. | `enabled` |
| `CASSANDRA_PROXY_STRIP_INTERNAL_HEADERS` | Rust proxy: same policy as `CASSANDRA_STRIP_INTERNAL_HEADERS` but for the Rust transparent proxy. Stripping happens inside `build_forward_request_headers` so both HTTP and WebSocket upstream calls are gated by one flag. `enabled` default; `disabled` operator opt-in for diagnostic shadow tracing. Response-side `X-Cassandra-*` injection (e.g. `x-cassandra-tokens-saved`) is unrelated and stays. | `enabled` |
...
(`CASSANDRA_CONFIG_DIR` + `CASSANDRA_WORKSPACE_DIR`) with additive
...
See the **[Filesystem Contract](https://github.com/Sarthak-47/cassandra/blob/main/wiki/filesystem-contract.md)**
page for the full bucket table, plugin-author guidance, and the Docker
naming overlap note (`CASSANDRA_WORKSPACE` is *not* the same as
`CASSANDRA_WORKSPACE_DIR`).
...
Save as `${CASSANDRA_CONFIG_DIR}/models.json` (defaults to
`~/.cassandra/config/models.json`), or set `CASSANDRA_MODEL_LIMITS` to a
JSON string or file path. Installs that still have
`~/.cassandra/models.json` (the legacy location) continue to work.

Settings are resolved in this order (later overrides earlier):

1. Built-in defaults
2. `${CASSANDRA_CONFIG_DIR}/models.json` (new canonical location); falls
   back to `~/.cassandra/models.json` (legacy) when the canonical file
   is absent
...
<Tab value="Google">
```python
from cassandra.providers import GoogleProvider
