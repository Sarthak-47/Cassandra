---
title: Architecture
description: How Cassandra's three-stage compression pipeline works, from message parsing through transform execution to provider cache optimization.
---

Cassandra sits between your application and the LLM provider. It intercepts messages, compresses them intelligently, and forwards the optimized request. The response comes back unchanged.
...
Cassandra can be used in three ways, all feeding into the same pipeline:

| Entry Point | How It Works | Code Changes |
|-------------|-------------|--------------|
| **SDK Mode** | Wrap your LLM client with `CassandraClient` | Minimal -- swap client constructor |
| **Proxy Mode** | Run `cassandra proxy` and point your client at it | Zero -- just change the base URL |
...
After the pipeline, Cassandra applies provider-specific cache hints:
