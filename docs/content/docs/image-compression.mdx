...
Vision models charge by the token, and images are expensive. A single 1024x1024 image costs ~765 tokens on OpenAI. Cassandra's image compression uses a trained ML router to analyze your query and automatically select the optimal compression technique, saving 40-90% of image tokens.
...
The router is a fine-tuned MiniLM classifier (`Sarthak-47/technique-router` on HuggingFace) with 93.7% accuracy across 1,157 training examples.
...
<Callout type="info" title="Automatic with the proxy">
  When using the Cassandra proxy, image compression happens automatically on every request that contains images. No code changes needed.
</Callout>
