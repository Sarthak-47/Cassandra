...
SmartCrusher is Cassandra's compressor for JSON tool outputs. It analyzes arrays statistically, keeps the important items (errors, anomalies, relevant matches), and drops the rest. This is the compressor that fires automatically when ContentRouter detects JSON arrays.
...
// Configure via the Cassandra proxy or CassandraClient
const result = await compress(messages, {
  model: "gpt-4o",
...
<Tab value="Python">
```python
from cassandra import SmartCrusher, SmartCrusherConfig
