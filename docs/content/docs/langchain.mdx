...
Cassandra integrates with LangChain to compress context across all LangChain patterns: chat models, memory, retrievers, agents, and streaming.
...
llm = CassandraChatModel(ChatAnthropic(model="claude-sonnet-4-20250514"))
```

## Memory integration

`CassandraChatMessageHistory` wraps any chat history with automatic compression. Long conversations stay under your token budget:
...
`CassandraDocumentCompressor` filters retrieved documents by relevance. Retrieve many for recall, keep the best for precision:
...
`wrap_tools_with_cassandra` compresses tool outputs before they re-enter the agent's context:
