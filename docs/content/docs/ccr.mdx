...
Cassandra's CCR (Compress-Cache-Retrieve) architecture makes compression **reversible**. When content is compressed, the original data is cached locally. If the LLM needs the full data, it retrieves it instantly.
...
The client never sees CCR tool calls -- they are handled transparently by Cassandra.
...
import type { CCRConfig } from "cassandra-ai";
...
const ccrConfig: CCRConfig = {
  enabled: true,
  injectTool: true,             // Inject cassandra_retrieve tool
...
# The LLM calls cassandra_retrieve when it needs more data.
...
agent runs, set `CASSANDRA_CCR_TTL_SECONDS` before starting the proxy:

```bash
CASSANDRA_CCR_TTL_SECONDS=7200 cassandra proxy
```
