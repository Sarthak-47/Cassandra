...
Cassandra uses `release-please` to maintain a release PR from conventional commits on `main`. Merging that release PR creates the release tag and GitHub Release, which triggers `.github/workflows/release.yml` to publish all packages, build version-matched Docker images, and attach release assets.
...
|---------|------|----------|----------------------|
| `cassandra-ai` | Python | PyPI | `PYPI_PACKAGE` |
| `cassandra-ai` | TypeScript SDK | npmjs.org | `NPM_SDK_PACKAGE` |
| `cassandra-openclaw` | TypeScript plugin | npmjs.org | `NPM_OPENCLAW_PACKAGE` |
| `@{owner}/cassandra-ai` | TypeScript SDK | GitHub Package Registry | — |
| `@{owner}/cassandra-openclaw` | TypeScript plugin | GitHub Package Registry | — |
| `cassandra-ai-{version}.tar.gz` / `cassandra_ai-{version}-py3-none-any.whl` | Python package distributions | GitHub Release (`{owner}/cassandra`) | — |
| `cassandra-ai-{version}.tgz` / `cassandra-openclaw-{version}.tgz` | Node release assets | GitHub Release (`{owner}/cassandra`) | — |
| `ghcr.io/{owner}/cassandra` | Docker image | GitHub Container Registry | — |
...
- `cassandra/_version.py` - `__version__`, synced at build time
...
### smoke-import-wheels
Installs the built wheels into representative customer environments and imports `cassandra._core`. This blocks publishing if a wheel builds successfully but cannot import on its promised platform floor.
...
- `sdk/typescript/` as `cassandra-ai`
- `plugins/openclaw/` as `cassandra-openclaw`
...
- `sdk/typescript/` as `@{owner}/cassandra-ai`
- `plugins/openclaw/` as `@{owner}/cassandra-openclaw`
...
```yaml
env:
  PYPI_PACKAGE: cassandra-ai
  PYPI_ENVIRONMENT: pypi
  NPM_REGISTRY_URL: https://registry.npmjs.org
  NPM_SDK_PACKAGE: cassandra-ai
  NPM_OPENCLAW_PACKAGE: cassandra-openclaw
