...
Cassandra integrates with [Agno](https://github.com/agno-agi/agno) (formerly Phidata) to compress context for AI agents. Wrap any Agno model for automatic optimization, and use hooks for observability.

## Installation

```bash
pip install "cassandra-ai[agno]" agno
```
...
claude_model = CassandraAgnoModel(Claude(id="claude-sonnet-4-20250514"))
