...
Context management is now handled automatically inside the pipeline (live-zone-only compression). Cassandra **never drops messages** from the conversation history and does not do position-based or score-based context management.

## How It Works

Cassandra compresses only the **newest content blocks** — the latest user message and the latest tool result / tool output. Compression is type-aware and reversible via [CCR](/docs/ccr), so the LLM can retrieve the original content on demand.
...
Cassandra enforces several protections to ensure model output quality:
...
response = client.chat.completions.create(
    model="gpt-4o",
    messages=messages,
    cassandra_output_buffer_tokens=8000,  # More room for long responses
    cassandra_keep_turns=5,               # Protect last 5 turns
)
```
</Tab>
</Tabs>

> **Note:** The `IntelligentContextConfig`, `ScoringWeights`, and `RollingWindowConfig` classes are no longer part of Cassandra. Context management is now handled automatically inside the pipeline (live-zone-only compression).
