...
Cassandra provides specialized compressors for text-based content that isn't JSON or source code. Each one understands the structure of its content type and preserves what the LLM needs while dropping the noise.
...
```python
from cassandra.transforms import DiffCompressor
