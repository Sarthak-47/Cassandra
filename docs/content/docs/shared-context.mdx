...
When agents hand off to each other, context gets replayed in full. SharedContext compresses what moves between agents using Cassandra's compression pipeline, typically saving **~80% of tokens** on agent handoffs.
...
Under the hood, `put()` calls `cassandra.compress()` -- the same pipeline used by the Cassandra proxy -- and stores the original in memory. `get()` returns the compressed version. `get(full=True)` returns the original.
