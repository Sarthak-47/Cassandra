...
Cassandra's MCP server exposes compression, retrieval, and observability as tools that any MCP-compatible AI coding tool can call -- Claude Code, Cursor, Codex, and more. No proxy required.
...
The original is stored locally for 1 hour. If the LLM needs the full content later, it calls `cassandra_retrieve`.
...
Sub-agent stats are aggregated via a shared stats file at `~/.cassandra/session_stats.jsonl`.
...
For MCP hosts that let you configure a local stdio server, point them at `cassandra mcp serve`. If you also run the proxy, pass the proxy URL explicitly so retrieval and stats come from the intended proxy instance.
...
### `command: "cassandra"` fails to start

The configurations above use `"command": "cassandra"`, which only works if the `cassandra`
executable is on the PATH your MCP host (Codex, etc.) sees at startup. If you installed Cassandra
into a project virtualenv — for example with `uv add cassandra-ai` — the CLI lives only inside
that venv, and the host fails at launch with:

```text
MCP client for `cassandra` failed to start: MCP startup failed: No such file or directory (os error 2)
```

Install Cassandra so it's globally on PATH — `uv tool install "cassandra-ai[mcp]"` (or
`pipx install "cassandra-ai[mcp]"`) — or replace `"cassandra"` with the absolute path to the binary
(`command -v cassandra`, or `where cassandra` on Windows).
...
The LLM calls `cassandra_compress` on demand. Compression happens locally in the MCP process. Originals are stored in a local `CompressionStore` with 1-hour TTL.
...
`cassandra_retrieve` checks the local store first, then falls back to the proxy's store.

## Troubleshooting

**"MCP SDK not installed"** -- Run `pip install "cassandra-ai[mcp]"`.

**"Proxy not running"** -- Start the proxy with `cassandra proxy` in another terminal. Only needed for proxy-backed retrieval.

**"Entry not found or expired"** -- Local content expires after 1 hour, proxy content after 5 minutes.

**Claude doesn't see cassandra tools** -- Run `cassandra mcp status`, restart Claude Code, and verify with `/mcp` inside Claude Code.

### Claude Code `/usage` attributes a large share to `cassandra` MCP

Claude Code counts MCP tool calls and MCP tool results as session context. If a
long-running workflow or a subagent-heavy command calls `cassandra_compress`,
`cassandra_retrieve`, or `cassandra_stats` many times, `/usage` can show a visible
share under the `cassandra` MCP server even when Cassandra is saving tokens inside
individual tool results.

That number is not a direct "Cassandra overhead" bill. It means Claude Code kept
Cassandra MCP interactions in the conversation context. Deep research workflows
...
- Run `cassandra_stats` and compare `tokens_saved` with the number of MCP calls.
...
- Prefer the proxy path for automatic compression of normal Claude Code traffic:
  `cassandra proxy` plus `ANTHROPIC_BASE_URL=http://127.0.0.1:8787 claude`.
- Disable the MCP server for sessions where you only want proxy-level
  compression and do not need on-demand `cassandra_compress` or
  `cassandra_retrieve`.
