...
This page is the quick visual map for Cassandra automation. Use it when opening a PR, reviewing a PR, cutting a release, or deciding which workflow owns a failure.
...
flowchart TD
    ...
    C --> D[Smoke-test image imports pydantic_core and cassandra._core]
...
| `.github/workflows/init-e2e.yml` | Dockerized `cassandra init` behavior |
| `.github/workflows/wrap-e2e.yml` | Dockerized `cassandra wrap` behavior |
| `.github/workflows/init-native-e2e.yml` | Host-specific `cassandra init -g` smoke tests |
