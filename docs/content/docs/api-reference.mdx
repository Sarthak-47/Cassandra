---
title: API Reference
description: Complete API reference for the Cassandra Python and TypeScript SDKs. Core client, configuration types, result types, errors, and utilities.
---

Complete API reference for the Cassandra Python and TypeScript SDKs.

## Core

### CassandraClient

The main entry point for the Cassandra SDK.

<Tabs groupId="lang" items={['TypeScript', 'Python']}>
<Tab value="TypeScript">

<TypeTable type={{
  baseUrl: { type: 'string', description: 'Base URL for the Cassandra proxy' },
...
Accepts all standard OpenAI/Anthropic parameters plus Cassandra-specific overrides:

<TypeTable type={{
  cassandra_mode: { type: '"audit" | "optimize" | "simulate"', description: 'Override mode for this request', default: 'Client default' },
  cassandra_query: { type: 'str', description: 'Query for relevance scoring', default: 'None' },
  cassandra_output_buffer_tokens: { type: 'int', description: 'Reserve tokens for output', default: '4000' },
  cassandra_keep_turns: { type: 'int', description: 'Keep last N turns uncompressed', default: '2' },
  cassandra_tool_profiles: { type: 'dict', description: 'Per-tool compression overrides', default: '{}' },
}} />
...
Top-level function to compress messages via the Cassandra proxy.
...
<TypeTable type={{
  ...
  fallback: { type: 'boolean', description: 'Return original messages on failure instead of throwing' },
  retries: { type: 'number', description: 'Number of retry attempts on failure' },
  client: { type: 'CassandraClientInterface', description: 'Pre-configured client instance to use' },
...
Context management is now handled automatically inside the pipeline (live-zone-only compression). Cassandra never drops messages from the conversation history; it compresses only the newest content blocks (latest user message, latest tool result) and keeps the cache hot zone — system prompt, tools, and older turns — untouched. Use the `cassandra_keep_turns` / `cassandra_output_buffer_tokens` per-request overrides to tune behavior. The `RollingWindowConfig`, `IntelligentContextConfig`, and `ScoringWeights` classes are no longer part of Cassandra.
...
<TypeTable type={{
  ...
  defaultMode: { type: 'CassandraMode', description: 'Default compression mode' },
...
# Note: rolling_window has been removed — use cassandra_keep_turns per-request instead
```
...
Semantic similarity scoring (requires `cassandra-ai[relevance]`):
...
| Exception | Meaning |
|-----------|---------|
| `CassandraError` | Base class for all errors |
| `CassandraConnectionError` | Cannot reach proxy |
| `CassandraAuthError` | 401 from proxy |
| `CassandraCompressError` | Compression failed (includes `statusCode`, `errorType`) |
...
| Exception | Meaning |
|-----------|---------|
| `CassandraError` | Base class for all Cassandra errors |
