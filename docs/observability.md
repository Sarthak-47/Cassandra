...
The Cassandra Rust proxy exposes Prometheus-format metrics on the
...
`crates/cassandra-proxy/src/observability/metric_names.rs`, so any
...
The alarm metric is wired in `crates/cassandra-proxy/src/proxy.rs`:
...
| Name | Type | Labels | Purpose |
|------|------|--------|---------|
| `proxy_image_generation_call_log_redacted_total` | Counter | _none_ | Base64-encoded image payloads redacted from request logs. Driven from `cassandra.proxy.request_logger.redactions_total()`. |
...
# Upstream rate-limit cassandra (smaller = closer to throttle).
...
`crates/cassandra-proxy/Cargo.toml` ever bumps the `prometheus`
...
- `content_type`: `&'static str` from `cassandra_core::transforms::ContentType`.
...
- `tool` (Python-side `wrap_rtk_invocations_total`): bounded by the
  ...
  `cassandra.cli.wrap_rtk_metrics`.
...
- `crates/cassandra-proxy/src/observability/` — implementation.
