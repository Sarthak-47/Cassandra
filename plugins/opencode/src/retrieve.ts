import type { CompressResult } from "cassandra-ai";
...
export function getDefaultProxyUrl(): string {
  return _proxyUrlCache ?? process.env.CASSANDRA_BASE_URL ?? "http://localhost:8787";
}
...
export function createCassandraRetrieveTool(config: RetrieveToolConfig) {
  const origin = config.proxyBaseUrl.replace(/\/+$/, "");

  return {
    name: "cassandra_retrieve",
    description:
      "Retrieve original uncompressed content from Cassandra's compression store. " +
...
export async function compressWithCassandra(
  messages: unknown[],
  options: {
    model?: string;
    tokenBudget?: number;
