...
import { installCassandraTransport, uninstallCassandraTransport } from "./transport.js";

afterEach(() => {
  uninstallCassandraTransport();
  vi.restoreAllMocks();
});
...
describe("Cassandra OpenCode transport", () => {
  it("routes external fetch calls through the proxy without pre-registering providers", async () => {
    ...
    installCassandraTransport({ proxyUrl: "http://127.0.0.1:8787/v1" });
    ...
    expect(new Headers(fetchMock.mock.calls[0][1]?.headers).get("x-cassandra-base-url")).toBe(
      "https://api.deepseek.com",
    );
    expect(fetchMock.mock.calls[1][0]).toEqual(new URL("http://127.0.0.1:8787/base/v1/messages"));
    expect(new Headers(fetchMock.mock.calls[1][1]?.headers).get("x-cassandra-base-url")).toBe(
  ...
  it("bypasses local, OpenCode, and Cassandra proxy fetch URLs", async () => {
    const originalFetch = globalThis.fetch;
    const fetchMock = vi.fn(async (..._args: FetchCall) => new Response("ok"));
    globalThis.fetch = fetchMock as unknown as typeof fetch;
  ...
  it("routes external https.request calls through the proxy", async () => {
    ...
    installCassandraTransport({ proxyUrl: proxy.url });
    ...
    expect(proxy.seen[0].headers["x-cassandra-base-url"]).toBe("https://api.anthropic.com");
  ...
  it("preloads the Cassandra shim into child Node processes", () => {
    const originalNodeOptions = process.env.NODE_OPTIONS;
    const originalProxyUrl = process.env.CASSANDRA_OPENCODE_TRANSPORT_PROXY_URL;

    try {
      process.env.NODE_OPTIONS = "--trace-warnings";
      delete process.env.CASSANDRA_OPENCODE_TRANSPORT_PROXY_URL;

      installCassandraTransport({ proxyUrl: "http://127.0.0.1:8787/v1" });

      expect(process.env.CASSANDRA_OPENCODE_TRANSPORT_PROXY_URL).toBe("http://127.0.0.1:8787/v1");
      ...
      if (originalProxyUrl === undefined) {
        ...
        process.env.CASSANDRA_OPENCODE_TRANSPORT_PROXY_URL = originalProxyUrl;
      }
      uninstallCassandraTransport();
    }
  });

  it("injects the Cassandra shim into child processes with custom env", () => {
    ...
    try {
      ...
      expect(options.env.CASSANDRA_OPENCODE_TRANSPORT_PROXY_URL).toBe("http://127.0.0.1:8787/v1");
  ...
  it("restores patched transports only after the final disposer", () => {
    ...
    const firstDispose = installCassandraTransport({ proxyUrl: "http://127.0.0.1:8787/v1" });
    const secondDispose = installCassandraTransport({ proxyUrl: "http://127.0.0.1:8788/v1" });
