...
const BASE_URL_HEADER = "x-cassandra-base-url";
const PROXY_ENV = "CASSANDRA_OPENCODE_TRANSPORT_PROXY_URL";
const STATE_KEY = Symbol.for("cassandra.opencode.transport");
...
interface GlobalWithCassandraTransport {
...
function getState(): TransportState | undefined {
  return (globalThis as GlobalWithCassandraTransport)[STATE_KEY];
}

function setState(state: TransportState | undefined): void {
  (globalThis as GlobalWithCassandraTransport)[STATE_KEY] = state;
}
...
function wrapSpawn(originalSpawn: ChildSpawn): ChildSpawn {
  return function cassandraSpawn(this: unknown, ...args: unknown[]) {
...
function wrapExec(originalExec: ChildExec): ChildExec {
  return function cassandraExec(this: unknown, ...args: unknown[]) {
    const state = getState();
    if (!state) {
...
function wrapExecFile(originalExecFile: ChildExecFile): ChildExecFile {
  return function cassandraExecFile(this: unknown, ...args: unknown[]) {
    const state = getState();
    if (!state) {
...
function wrapFork(originalFork: ChildFork): ChildFork {
  return function cassandraFork(this: unknown, ...args: unknown[]) {
    const state = getState();
    if (!state) {
...
function wrapRequest(
  ...
  return function cassandraRequest(this: unknown, ...args: unknown[]) {
    const state = getState();
    if (!state) {
...
function wrapGet(request: HttpRequest | HttpsRequest): HttpGet | HttpsGet {
  return function cassandraGet(this: unknown, ...args: unknown[]) {
...
function wrapHttp2Connect(originalConnect: Http2Connect): Http2Connect {
  return function cassandraHttp2Connect(this: unknown, authority: string | URL, ...args: unknown[]) {
    const state = getState();
    if (state) {
      const proxy = normalizeProxyUrl(state.proxyUrl);
      ...
      if (shouldRoute(upstream, proxy)) {
        throw new Error(
          `Cassandra OpenCode wrap blocked direct HTTP/2 connection to ${upstream.origin}. ` +
            "Use fetch, http, or https so traffic can be routed through Cassandra.",
        );
      }
    }
...
export function installCassandraTransport(options: InstallOptions): () => void {
  ...
  if (existing) {
    ...
    return () => uninstallCassandraTransport();
  }
...
export function uninstallCassandraTransport(): void {
  const state = getState();
  if (!state) {
    return;
  }
