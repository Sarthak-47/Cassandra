"""Cassandra CCR retrieve plugin.

The cassandra proxy (127.0.0.1:8787) compresses large tool outputs in LLM
...
CASSANDRA_RETRIEVE_SCHEMA = {
    "name": "cassandra_retrieve",
    "description": (
        "Retrieve the original uncompressed content behind a cassandra "
...
def _handle_cassandra_retrieve(args: dict, **kw) -> str:
    ...
    try:
        ...
        return tool_error(
            f"cassandra proxy unreachable at {_PROXY_URL} ({type(exc).__name__}). "
    ...
    if resp.status_code != 200:
        return tool_error(f"cassandra proxy returned HTTP {resp.status_code}: {resp.text[:200]}")
...
def register(ctx) -> None:
    """Register the cassandra_retrieve tool. Called by the plugin loader."""
    ctx.register_tool(
        name="cassandra_retrieve",
        toolset="cassandra",
        schema=CASSANDRA_RETRIEVE_SCHEMA,
        handler=_handle_cassandra_retrieve,
        emoji="🗜️",
    )
