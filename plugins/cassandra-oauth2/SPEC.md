# Spec — `cassandra-oauth2` (generic OAuth2 client-credentials upstream auth)

## Summary

A vendor-neutral proxy extension (registers on Cassandra's `cassandra.proxy_extension` seam) that
mints an OAuth2 **client-credentials** (RFC 6749 §4.4) bearer from a configured token endpoint and
injects it as the upstream `Authorization` on every proxied request. Lets Cassandra front any
...
Opt-in only, via Cassandra's existing flags — **no new CLI flags**:

    cassandra proxy --backend litellm-openai --proxy-extension oauth2
    # or CASSANDRA_PROXY_EXTENSIONS=oauth2

All configuration is env (12-factor; nothing baked in):

| Env var | Required | Meaning |
|---|---|---|
| `CASSANDRA_OAUTH2_TOKEN_URL` | yes (else no-op) | OAuth2 token endpoint; must be `https` (loopback `http` allowed for tests) |
| `CASSANDRA_OAUTH2_CLIENT_ID` / `_CLIENT_SECRET` | yes | client credentials |
| `CASSANDRA_OAUTH2_SCOPES` | no | space/comma-separated scopes |
| `CASSANDRA_OAUTH2_AUDIENCE` | no | `audience` form param |
| `CASSANDRA_OAUTH2_RESOURCE` | no | RFC 8707 target `resource` form param |
| `CASSANDRA_OAUTH2_GRANT_TYPE` | no | default `client_credentials` |
| `CASSANDRA_OAUTH2_AUTH_STYLE` | no | `post` (form creds) or `basic` (HTTP Basic) |
| `CASSANDRA_OAUTH2_HEADERS` | no | static upstream headers, `K=V,K2=V2` (control chars rejected) |
| `CASSANDRA_OAUTH2_TIMEOUT` / `_SKEW` | no | token request timeout / pre-expiry refresh skew (s) |
| `CASSANDRA_OAUTH2_ALLOW_INSECURE` | no | `1` to allow a non-loopback `http` token_url (discouraged) |
...
- **None unless explicitly enabled.** The entry point is dormant until `--proxy-extension oauth2`
  is passed, and even then a **no-op** unless `CASSANDRA_OAUTH2_TOKEN_URL` is set.
- When active, it **overwrites the request `Authorization` header** with the minted bearer before
  ...
  request then carries a bearer, Cassandra classifies it as OAuth-mode auth — same as supplying a
...
| Malformed `CASSANDRA_OAUTH2_HEADERS` (CR/LF) | offending pair dropped with a warning (no header injection) |
...
- **Wrappers (`cassandra wrap`, agent hooks):** the extension lives at the proxy layer, so anything
...
- No metrics/telemetry emitted; piggybacks on Cassandra's existing request logging. (A future
...
- **Instant rollback:** drop `--proxy-extension oauth2` (or unset `CASSANDRA_PROXY_EXTENSIONS`), or
...
- **Runtime:** standard library only (no new core dependency). `litellm` is touched **only** if
  `CASSANDRA_OAUTH2_HEADERS` is set, and it is already a Cassandra backend dependency — declared here
