...
def test_provider_from_env_wires_knobs(idp):
    env = {
        "CASSANDRA_OAUTH2_TOKEN_URL": idp,
        "CASSANDRA_OAUTH2_CLIENT_ID": "c",
        "CASSANDRA_OAUTH2_CLIENT_SECRET": "s",
        "CASSANDRA_OAUTH2_RESOURCE": "https://api.example",
        "CASSANDRA_OAUTH2_TIMEOUT": "5",
        "CASSANDRA_OAUTH2_SKEW": "10",
    }
...
def test_install_noop_when_unset(monkeypatch):
    monkeypatch.delenv("CASSANDRA_OAUTH2_TOKEN_URL", raising=False)
...
def test_install_fail_closed_on_bad_config(monkeypatch):
    monkeypatch.setenv("CASSANDRA_OAUTH2_TOKEN_URL", "https://idp.example.com/token")
    monkeypatch.setenv("CASSANDRA_OAUTH2_CLIENT_ID", "")  # missing -> ValueError -> RuntimeError
...
def test_install_warns_for_envauth_backend(monkeypatch, caplog):
    monkeypatch.setenv("CASSANDRA_OAUTH2_TOKEN_URL", "https://idp.example.com/token")
    monkeypatch.setenv("CASSANDRA_OAUTH2_CLIENT_ID", "c")
    monkeypatch.setenv("CASSANDRA_OAUTH2_CLIENT_SECRET", "s")
    monkeypatch.delenv("CASSANDRA_OAUTH2_HEADERS", raising=False)
    installed = []
...
def test_install_fail_closed_on_bad_timeout(monkeypatch):
    ...
    monkeypatch.setenv("CASSANDRA_OAUTH2_CLIENT_ID", "c")
    monkeypatch.setenv("CASSANDRA_OAUTH2_CLIENT_SECRET", "s")
    monkeypatch.setenv("CASSANDRA_OAUTH2_TIMEOUT", "not-a-number")  # invalid -> fail closed
    with pytest.raises(RuntimeError):
        install(object(), _cfg("litellm-openai"))
...
def test_allow_insecure_env_permits_nonloopback_http():
    p = provider_from_env(
        {
            "CASSANDRA_OAUTH2_TOKEN_URL": "http://example.com/token",
            "CASSANDRA_OAUTH2_CLIENT_ID": "c",
            "CASSANDRA_OAUTH2_CLIENT_SECRET": "s",
            "CASSANDRA_OAUTH2_ALLOW_INSECURE": "1",
        }
    )
...
def test_auth_style_basic_via_env(idp):
    p = provider_from_env(
        {
            "CASSANDRA_OAUTH2_TOKEN_URL": idp,
            "CASSANDRA_OAUTH2_CLIENT_ID": "cid",
            "CASSANDRA_OAUTH2_CLIENT_SECRET": "csec",
            "CASSANDRA_OAUTH2_AUTH_STYLE": "basic",
        }
    )
...
def test_install_sets_static_headers(monkeypatch):
    ...
    monkeypatch.setenv("CASSANDRA_OAUTH2_CLIENT_ID", "c")
    monkeypatch.setenv("CASSANDRA_OAUTH2_CLIENT_SECRET", "s")
    monkeypatch.setenv("CASSANDRA_OAUTH2_HEADERS", "X-App=demo,Bad Key=x")
