"""Generic OAuth2 client-credentials upstream-auth extension for the Cassandra proxy.

Enable: `--proxy-extension oauth2` (or CASSANDRA_PROXY_EXTENSIONS=oauth2).
No-op unless CASSANDRA_OAUTH2_TOKEN_URL is set. See README for env config.
"""
...
__version__ = "0.1.0"
log = logging.getLogger("cassandra_oauth2")
...
def parse_headers(s: str | None) -> dict[str, str]:
    ...
    for pair in (s or "").split(","):
        ...
        if _ctrl(k) or _ctrl(v) or " " in k or ":" in k:
            log.warning("cassandra-oauth2: dropping malformed static header: %r", k)
            continue
        out[k] = v
    return out
...
def provider_from_env(env: dict | None = None) -> OAuth2ClientCredentials | None:
    """Build a provider from ``CASSANDRA_OAUTH2_*`` env vars, or None if TOKEN_URL is unset.

    Raises ValueError on malformed config so callers can fail closed.
    """
    env = os.environ if env is None else env
    token_url = env.get("CASSANDRA_OAUTH2_TOKEN_URL")
    if not token_url:
        return None
    allow_insecure = env.get("CASSANDRA_OAUTH2_ALLOW_INSECURE", "").strip().lower() in (
        "1",
        "true",
        "yes",
    )
    if allow_insecure:
        log.warning("cassandra-oauth2: ALLOW_INSECURE set -- token endpoint TLS check disabled")
    resource = env.get("CASSANDRA_OAUTH2_RESOURCE")  # RFC 8707 target service
    timeout = _int(env, "CASSANDRA_OAUTH2_TIMEOUT")
    skew = _int(env, "CASSANDRA_OAUTH2_SKEW")
    kwargs = {}
    ...
    return OAuth2ClientCredentials(
        token_url=token_url,
        client_id=env.get("CASSANDRA_OAUTH2_CLIENT_ID", ""),
        client_secret=env.get("CASSANDRA_OAUTH2_CLIENT_SECRET", ""),
        scopes=_split(env.get("CASSANDRA_OAUTH2_SCOPES")),
        audience=env.get("CASSANDRA_OAUTH2_AUDIENCE") or None,
        grant_type=env.get("CASSANDRA_OAUTH2_GRANT_TYPE", "client_credentials"),
        auth_style=env.get("CASSANDRA_OAUTH2_AUTH_STYLE", "post"),
...
def install(app: Any, config: Any) -> None:
    """Cassandra proxy-extension entry point: install(app, config) -> None."""
    try:
        provider = provider_from_env()
    except ValueError as e:
        raise RuntimeError(f"cassandra-oauth2 misconfigured: {e}") from None  # fail-closed
    if provider is None:
        log.info("cassandra-oauth2 loaded but CASSANDRA_OAUTH2_TOKEN_URL unset; no-op")
        return
    static = parse_headers(os.environ.get("CASSANDRA_OAUTH2_HEADERS"))
    if static:
        try:
            ...
            log.info("cassandra-oauth2: static upstream headers: %s", list(static))
        except Exception as e:  # pragma: no cover
            log.warning("cassandra-oauth2: could not set litellm.headers: %s", e)
    ...
    if any(p in backend for p in ("bedrock", "vertex", "sagemaker")):
        log.warning(
            "cassandra-oauth2: backend %r authenticates from env (bedrock/vertex/sagemaker) and "
    ...
    log.info(
        "cassandra-oauth2: client-credentials auth installed (token_url=%s, style=%s)",
        provider.token_url,
        provider.auth_style,
    )
