...
function stubProbeNonCassandra() {
  // Every endpoint reachable but non-OK => reachable, non-Cassandra (occupied port).
...
describe("normalizeAndValidateProxyUrl", () => {
  ...
  it("accepts remote URLs", () => {
    ...
    expect(normalizeAndValidateProxyUrl("https://cassandra.example.com")).toBe("https://cassandra.example.com");
    expect(normalizeAndValidateProxyUrl("https://cassandra.example.com:9090")).toBe("https://cassandra.example.com:9090");
  });
...
describe("isLocalProxyUrl", () => {
  ...
  it("returns false for remote addresses", () => {
    ...
    expect(isLocalProxyUrl("https://cassandra.example.com")).toBe(false);
  });
...
describe("probeCassandraProxy", () => {
  ...
  it("does not treat /readyz success alone as Cassandra identity", async () => {
    stubByPath({
    ...
    const result = await probeCassandraProxy("http://127.0.0.1:8787");
    expect(result.reachable).toBe(true);
    expect(result.isCassandra).toBe(false);
  });

  it("treats Cassandra-shaped /v1/retrieve/stats 200 as Cassandra even when /readyz is OK and /health is 503", async () => {
    stubByPath({
    ...
    const result = await probeCassandraProxy("http://127.0.0.1:8787");
    expect(result).toEqual({ reachable: true, isCassandra: true });
  });

  it("treats Cassandra-shaped /v1/retrieve/stats 200 as Cassandra even when /health is 503", async () => {
    stubByPath({
      "/health": { ok: false, status: 503 },
  ...
  it("does not treat generic /v1/retrieve/stats 200 as Cassandra identity", async () => {
    stubByPath({
      "/readyz": { ok: true, status: 200 },
  ...
  it("falls through from auth-gated /v1/retrieve/stats to Cassandra-shaped /stats", async () => {
    stubByPath({
      "/readyz": { ok: true, status: 200 },
  ...
  it("falls back to /stats only when the response has a Cassandra stats shape", async () => {
    stubByPath({
  ...
  it("does not treat generic /stats 200 as Cassandra identity", async () => {
    stubByPath({
      "/readyz": { ok: false, status: 404 },
      "/v1/retrieve/stats": { ok: false, status: 404 },
  ...
  it("returns reachable but non-cassandra when identity endpoints are non-OK", async () => {
    stubProbeNonCassandra();
    const result = await probeCassandraProxy("http://127.0.0.1:8787");
    expect(result.reachable).toBe(true);
    expect(result.isCassandra).toBe(false);
...
describe("ProxyManager.start", () => {
  it("auto-detects running proxy on default candidates", async () => {
    ...
    const startSpy = vi.spyOn(manager as any, "startCassandraProxy");
  ...
  it("fails when explicit URL is reachable but not a cassandra proxy", async () => {
    const manager = new ProxyManager({ proxyUrl: "http://127.0.0.1:8787" });
    stubProbeNonCassandra();
    await expect(manager.start()).rejects.toThrow(/does not appear to be a Cassandra proxy/);
  });
  ...
  it("applies default proxyPort when explicit proxyUrl omits port", async () => {
    ...
    const startSpy = vi.spyOn(manager as any, "startCassandraProxy").mockResolvedValue(undefined);
  ...
  it("connects to remote proxy without auto-start", async () => {
    const manager = new ProxyManager({ proxyUrl: "http://cassandra.remote.example:8787", autoStart: true });
    const startSpy = vi.spyOn(manager as any, "startCassandraProxy").mockResolvedValue(undefined);
    stubProbeSuccess();

    const url = await manager.start();
    expect(url).toBe("http://cassandra.remote.example:8787");
    expect(startSpy).not.toHaveBeenCalled();
  });

  it("does not apply proxyPort default to remote URLs", async () => {
    const manager = new ProxyManager({ proxyUrl: "https://cassandra.remote.example", proxyPort: 9999 });
    stubProbeSuccess();

    const url = await manager.start();
    expect(url).toBe("https://cassandra.remote.example");
  });

  it("fails fast for unreachable remote proxy without attempting auto-start", async () => {
    const manager = new ProxyManager({ proxyUrl: "https://cassandra.remote.example:8787", autoStart: true });
    const startSpy = vi.spyOn(manager as any, "startCassandraProxy").mockResolvedValue(undefined);
    stubProbeUnreachable();

    await expect(manager.start()).rejects.toThrow(/Remote Cassandra proxy not reachable/);
    expect(startSpy).not.toHaveBeenCalled();
  });
...
describe("ProxyManager launch internals", () => {
  ...
  it("prefers configured pythonPath ahead of PATH launchers", () => {
    const manager = new ProxyManager({ pythonPath: "C:\\Python311\\python.exe" });
    vi.spyOn(manager as any, "getPyenvResolvedCassandra").mockReturnValue(null);
    ...
    expect(specs[0]?.args).toEqual(["-m", "cassandra.cli", "proxy", "--host", "127.0.0.1", "--port", "8787"]);
  });

  it("uses lightweight PATH checks instead of booting the cassandra CLI", () => {
    const manager = new ProxyManager({});
    const specs = (manager as any).buildLaunchSpecs("127.0.0.1", "8787") as Array<Record<string, unknown>>;
    const pathSpec = specs.find((spec) => spec.command === "cassandra");

    expect(pathSpec).toBeDefined();
    expect(pathSpec.command).toBe("cassandra");
    expect(pathSpec.args).toEqual(["proxy", "--host", "127.0.0.1", "--port", "8787"]);
    if (process.platform === "win32") {
      expect(pathSpec.checkCommand).toBe("where.exe");
      expect(pathSpec.checkArgs).toEqual(["cassandra"]);
      expect(pathSpec.checkUseShell).toBe(false);
    } else {
      expect(pathSpec.checkCommand).toBe("sh");
      expect(pathSpec.checkArgs).toEqual(["-lc", "command -v cassandra >/dev/null 2>&1"]);
    }
  });
  ...
  it("prefers a resolved pyenv executable on Windows before PATH shims", () => {
    ...
    const manager = new ProxyManager({});
    vi.spyOn(manager as any, "getPyenvResolvedCassandra").mockReturnValue("C:\\Python312\\Scripts\\cassandra.exe");

    const specs = (manager as any).buildLaunchSpecs("127.0.0.1", "8787") as Array<Record<string, unknown>>;

    expect(specs[0]?.label).toContain("pyenv:");
    expect(specs[0]?.command).toBe("C:\\Python312\\Scripts\\cassandra.exe");
    expect(specs[0]?.useShell).toBe(false);
    expect(specs[1]?.command).toBe("cassandra");
  });
  ...
  it("uses first available launcher from provided specs", async () => {
    ...
    await (manager as any).startCassandraProxy("http://127.0.0.1:8787");
  ...
  it("supports shell-backed launch specs for PATH shims and script wrappers", async () => {
    ...
    await (manager as any).startCassandraProxy("http://127.0.0.1:8787", 8787);
  ...
  it("throws when no launcher is executable", async () => {
    ...
    await expect((manager as any).startCassandraProxy("http://127.0.0.1:8787")).rejects.toThrow(
      /No usable Cassandra launcher found/,
    );
  });
});
