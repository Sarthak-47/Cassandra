...
openclaw plugins install --dangerously-force-unsafe-install cassandra-ai/openclaw
```

This plugin can auto-start a local `cassandra proxy` when needed. OpenClaw treats process-launching plugins as unsafe by default, so `--dangerously-force-unsafe-install` is required even if you plan to use a remote proxy (the capability is declared at install time).
...
- "local npm bin" means `plugins/openclaw/node_modules/.bin/cassandra` relative to the source checkout.
...
Install automatically selects the `contextEngine` slot for `cassandra` on current OpenClaw releases. If you need to switch back manually, set `plugins.slots.contextEngine` to `"legacy"` or another engine id.
...
By default, the plugin also rewrites the built-in `openai-codex` provider base URL to a verified active Cassandra proxy at runtime. That means Codex provider traffic flows through Cassandra, so `/stats` can observe real upstream request and cache activity instead of only local context compression.

This does not replace Cassandra's existing Codex routing rules. The proxy already decides between `api.openai.com` and `chatgpt.com/backend-api/codex/responses` based on ChatGPT auth. The plugin change only points OpenClaw's provider config at the active proxy in memory and preserves the rest of the provider config.
...
- disabling the plugin, clearing `gatewayProviderIds`, or restarting without Cassandra restores OpenClaw's normal provider resolution
- if you want durable provider rewrites, use `cassandra wrap openclaw` instead of relying on plugin install side effects
...
When `proxyUrl` points to localhost (or is omitted), the plugin will auto-start `cassandra proxy` if no running proxy is detected. Launch order:
1. `cassandra` from `PATH`
2. local npm bin (`node_modules/.bin/cassandra`)
3. global npm bin
4. Python module (`python -m cassandra.cli proxy ...`)

If `pythonPath` is set, it is tried first in the Python fallback step.

Docker-native Cassandra installs intentionally leave `pythonPath` unset so this launcher order prefers the installed host `cassandra` wrapper on `PATH`, which then runs Cassandra in Docker.

### Remote proxy (connect-only)

Point `proxyUrl` to any reachable Cassandra instance:

```json
{
  "config": {
    "proxyUrl": "https://cassandra.example.com:8787"
  }
}
```
...
```bash
pip install "cassandra-ai[proxy]"
cassandra proxy --host 127.0.0.1 --port 8787
```

NPM install:

```bash
npm install -g cassandra-ai
cassandra proxy --host 127.0.0.1 --port 8787
```
...
Compression is lossless via CCR (Compress-Cache-Retrieve): originals are stored and the agent gets a `cassandra_retrieve` tool to access full details when needed.

## Configuration Options

| Option | Default | Description |
|--------|---------|-------------|
| `proxyUrl` | auto-detected | Optional URL of a Cassandra proxy. Configured URLs are probe-gated before provider routing. Remote URLs (`https://cassandra.example.com`) are connect-only. |
...
| `autoStart` | `false` | Opt-in auto-start for a local `cassandra proxy` if not already running (local URLs only; ignored for remote proxies). Keep `false` when systemd owns the proxy. |
| `startupTimeoutMs` | `20000` | Time to wait for auto-started proxy to become healthy |
| `routeCodexViaProxy` | `true` | Rewrite OpenClaw's built-in `openai-codex` provider to use the active Cassandra proxy in memory so upstream Codex requests pass through Cassandra. |
| `gatewayProviderIds` | `[]` | Optional explicit list of OpenClaw provider ids to route through the active Cassandra proxy in memory. Friendly aliases `codex`, `claude`, `copilot`, and `gemini` are also accepted. When set, this overrides the default `openai-codex` routing list. |

## Comparison with lossless-claw

| | lossless-claw | cassandra |
|---|---|---|
...
| Retrieval | `lcm_grep`, `lcm_expand` | `cassandra_retrieve` (instant) |
