...
export interface OpenAIMessage {
  ...
  tool_call_id?: string;
  name?: string;
  _cassandraMeta?: Record<string, unknown>;
}
  ...
  for (const msg of messages) {
    ...
    if (role === "assistant") {
      ...
      if (typeof content === "string") {
        result.push({ role: "assistant", content, _cassandraMeta: buildMeta() });
        continue;
      }
...
export function openAIToAgent(messages: OpenAIMessage[]): any[] {
  ...
  for (const msg of messages) {
    const meta = (msg._cassandraMeta ?? {}) as Record<string, unknown>;
    const timestamp =
    ...
    if (msg.role === "assistant") {
      ...
      result.push({
        ...
        role: "assistant",
        content: blocks,
        api: typeof meta.api === "string" ? meta.api : "cassandra",
        provider: typeof meta.provider === "string" ? meta.provider : "cassandra",
        model: typeof meta.model === "string" ? meta.model : "cassandra",
        usage:
          isRecord(meta.usage)
            ? meta.usage
            : {
    ...
    if (msg.role === "tool") {
      ...
      result.push({
        ...
        toolName:
          typeof meta.toolName === "string" ? meta.toolName : "cassandra",
...
function normalizeAssistantMessage(message: Record<string, any>): Record<string, any> {
  ...
  return {
    ...
    api: typeof message.api === "string" ? message.api : "cassandra",
    provider: typeof message.provider === "string" ? message.provider : "cassandra",
    model: typeof message.model === "string" ? message.model : "cassandra",
...
function normalizeToolResultMessage(message: Record<string, any>): Record<string, any> {
  ...
  return {
    ...
    toolName: typeof message.toolName === "string" ? message.toolName : "cassandra",
