/**
 * CassandraContextEngine — ContextEngine implementation for OpenClaw.
 *
 * Compresses tool outputs and conversation context using the Cassandra proxy.
...
export interface CassandraEngineConfig extends ProxyManagerConfig {
  enabled?: boolean;
}

export class CassandraContextEngine {
  readonly info = {
    id: "cassandra",
    name: "Cassandra Context Compression",
    version: "0.1.0",
    ownsCompaction: true,
  };
  ...
  private config: CassandraEngineConfig;
  ...
  constructor(config: CassandraEngineConfig = {}, logger?: ProxyManagerLogger) {
    this.config = config;
      ...
      return {
        ...
        systemPromptAddition:
          result.tokensSaved > 100
            ? `[Context compressed by Cassandra: ${result.tokensSaved} tokens saved. Use cassandra_retrieve with the hash to get full details.]`
            : undefined,
      };
    } catch (error) {
   ...
   * - CCR: stores originals for retrieval via cassandra_retrieve tool
   *
  ...
  async compact(params: {
    ...
    return {
      ok: true,
      compacted: true,
      reason: "Cassandra applies SmartCrusher + Kompress + RollingWindow on next assemble()",
    };
  }
  ...
  ensureProxyStarted(): void {
    ...
    this.proxyStartupPromise = this.proxyManager
      ...
      .then(async (proxyUrl) => {
        ...
        this.logger.info(`Cassandra proxy ready at ${proxyUrl}`);
        return proxyUrl;
      })
      .catch((error) => {
        this.proxyStartupError = error;
        this.logger.warn(`Cassandra proxy unavailable: ${error}`);
        throw error;
      })
      .finally(() => {
  ...
  async ensureProxyUrl(): Promise<string> {
    ...
    if (!this.proxyStartupPromise) {
      throw new Error("Cassandra proxy startup is disabled");
    }
  ...
  private async notifyProxyReady(proxyUrl: string): Promise<void> {
    for (const listener of this.proxyReadyListeners) {
      try {
        ...
        this.logger.warn(`Cassandra proxy ready listener failed: ${error}`);
      }
    }
  }
}
