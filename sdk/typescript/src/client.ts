/**
 * CassandraClient — HTTP client for the Cassandra compression proxy.
 *
 * Supports:
...
import { mapProxyError, CassandraConnectionError, CassandraAuthError, CassandraCompressError } from "./errors.js";
import { deepCamelCase, deepSnakeCase } from "./utils/case.js";
import { parseSSE } from "./utils/stream.js";
import type { CassandraConfig, CassandraMode } from "./types/config.js";
import type {
  SimulationResult,
  RequestMetrics,
  MetricsSummary,
...
// --- Cassandra request params ---

export interface CassandraParams {
  cassandraMode?: CassandraMode;
  cassandraCachePrefixTokens?: number;
  cassandraOutputBufferTokens?: number;
  cassandraKeepTurns?: number;
  cassandraToolProfiles?: Record<string, Record<string, any>>;
}
...
class ChatCompletions {
   ...
   */
  async create(
    params: { model: string; messages: OpenAIMessage[]; stream?: boolean; [key: string]: any } & CassandraParams,
  ): Promise<any> {
    const { cassandraMode, cassandraCachePrefixTokens, cassandraOutputBufferTokens, cassandraKeepTurns, cassandraToolProfiles, ...apiParams } = params;

    const headers: Record<string, string> = {};
    if (cassandraMode) headers["x-cassandra-mode"] = cassandraMode;
   ...
   */
  async simulate(
    params: { model: string; messages: OpenAIMessage[] } & CassandraParams,
...
class Messages {
   ...
   */
  async create(
    params: { model: string; messages: any[]; max_tokens?: number; system?: string | any[]; stream?: boolean; [key: string]: any } & CassandraParams,
  ): Promise<any> {
    const { cassandraMode, cassandraCachePrefixTokens, cassandraOutputBufferTokens, cassandraKeepTurns, cassandraToolProfiles, ...apiParams } = params;
   ...
   */
  stream(
    params: { model: string; messages: any[]; max_tokens?: number; system?: string | any[]; [key: string]: any } & CassandraParams,
   ...
   */
  async simulate(
    params: { model: string; messages: any[] } & CassandraParams,
  ): Promise<SimulationResult> {
    const body = {
      messages: params.messages,
      model: params.model,
...
export interface ExtendedClientOptions extends CassandraClientOptions {
  providerApiKey?: string;
  defaultMode?: CassandraMode;
  config?: CassandraConfig;
}

export class CassandraClient implements CassandraClientInterface {
  private baseUrl: string;
  private apiKey: string | undefined;
  private timeout: number;
  private fallback: boolean;
  private retries: number;
  private config: CassandraConfig | undefined;
  ...
  constructor(options: ExtendedClientOptions = {}) {
    this.baseUrl = (
      options.baseUrl ??
      getEnv("CASSANDRA_BASE_URL") ??
      DEFAULT_BASE_URL
    ).replace(/\/+$/, "");
    this.apiKey = options.apiKey ?? getEnv("CASSANDRA_API_KEY");
  ...
  async compress(
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      try {
        ...
        lastError = error;
        if (error instanceof CassandraAuthError) throw error;
        if (
          error instanceof CassandraCompressError &&
    ...
    if (lastError instanceof CassandraConnectionError) throw lastError;
    if (lastError instanceof CassandraCompressError) throw lastError;
  ...
  /** Handle an LLM tool call for cassandra_retrieve. */
  ...
  async rawFetch(
    ...
    if (this.stack && !headers["X-Cassandra-Stack"]) {
      headers["X-Cassandra-Stack"] = this.stack;
    }
    ...
    try {
      ...
      throw new CassandraConnectionError(
        `Failed to connect to Cassandra at ${this.baseUrl}: ${error}`,
      );
    }
