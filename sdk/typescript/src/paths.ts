/**
 * Canonical filesystem contract for Cassandra — parity shell for the npm SDK.
 *
 * The TypeScript SDK is an HTTP client today and does not touch the
 * filesystem directly. This module mirrors `cassandra/paths.py` so that
 * future local features (e.g. cache/log co-location with the Python
 * proxy) land on the same contract.
 *
 * Two canonical roots:
 *   - CASSANDRA_CONFIG_DIR     — read-mostly configuration
 *                               (default: ~/.cassandra/config)
 *   - CASSANDRA_WORKSPACE_DIR  — read-write state
 *                               (default: ~/.cassandra)
 *
 ...
 * shape of the API matches Python's `cassandra.paths` module.
 */
...
export const CASSANDRA_CONFIG_DIR_ENV = "CASSANDRA_CONFIG_DIR";
export const CASSANDRA_WORKSPACE_DIR_ENV = "CASSANDRA_WORKSPACE_DIR";

export const CASSANDRA_SAVINGS_PATH_ENV = "CASSANDRA_SAVINGS_PATH";
export const CASSANDRA_TOIN_PATH_ENV = "CASSANDRA_TOIN_PATH";
export const CASSANDRA_SUBSCRIPTION_STATE_PATH_ENV =
...
export function workspaceDir(): string {
  if (!isNode()) return "";
  const envValue = getEnv(CASSANDRA_WORKSPACE_DIR_ENV);
  if (envValue) return expandTilde(envValue);
  const home = homeDir();
  if (!home) return "";
  return joinPath(home, ".cassandra");
}

export function configDir(): string {
  if (!isNode()) return "";
  const envValue = getEnv(CASSANDRA_CONFIG_DIR_ENV);
  if (envValue) return expandTilde(envValue);
  const workspaceEnv = getEnv(CASSANDRA_WORKSPACE_DIR_ENV);
  if (workspaceEnv) {
    return joinPath(expandTilde(workspaceEnv), "config");
  }
  const home = homeDir();
  if (!home) return "";
  return joinPath(home, ".cassandra", "config");
}
