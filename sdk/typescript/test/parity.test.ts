 ...
 * to the Python SDK when both talk to the same Cassandra proxy.
 *
 * Requires:
 *   - CASSANDRA_INTEGRATION=1 (env flag)
 *   - cassandra proxy running on localhost:8787
 *   - OPENAI_API_KEY set in environment
 *
 * Run: CASSANDRA_INTEGRATION=1 OPENAI_API_KEY=sk-... npx vitest run test/parity.test.ts
 */
import { describe, it, expect, beforeAll } from "vitest";
import { compress, CassandraClient, SharedContext, CompressionHooks, simulate } from "../src/index.js";
import type { CompressResult, CompressEvent, CompressContext } from "../src/index.js";
import { execSync } from "child_process";

const INTEGRATION = process.env.CASSANDRA_INTEGRATION === "1";
...
`;
  ...
  describe("CassandraClient proxy API parity", () => {
