/**
 * Integration tests for the Cassandra TypeScript SDK.
 *
 * These tests run against a real Cassandra proxy server.
 * They require:
 *   - The proxy running on http://localhost:8787
 *   - OPENAI_API_KEY and ANTHROPIC_API_KEY in .env
 *
 * Run with: CASSANDRA_INTEGRATION=1 npx vitest run test/integration.test.ts
 */
...
const RUN_INTEGRATION = process.env.CASSANDRA_INTEGRATION === "1";
...
describe.skipIf(!RUN_INTEGRATION)("Integration: compress() with real proxy", () => {
  beforeAll(async () => {
    ...
    try {
      ...
      throw new Error(
        `Proxy not running at ${PROXY_URL}. Start with: cassandra proxy --port 8787\n${e}`,
      );
    }
  });
  ...
  it("CassandraClient can be reused across calls", async () => {
    const { CassandraClient } = await import("../src/client.js");

    const client = new CassandraClient({ baseUrl: PROXY_URL });
