/**
 * Tests for Gemini adapter — withCassandra for Google Generative AI.
 */
import { describe, it, expect, vi, beforeEach } from "vitest";
import { withCassandra } from "../../src/adapters/gemini.js";
...
describe("Gemini withCassandra", () => {
  ...
  it("wraps generateContent with compression", async () => {
    ...
    const wrapped = withCassandra(mockModel, { baseUrl: "http://test:8787" });
  ...
  it("uses model option for compression", async () => {
    ...
    const wrapped = withCassandra(mockModel, {
