...
describe("cassandraMiddleware", () => {
  beforeEach(() => {
    mockFetch.mockReset();
  });

  it("returns object with transformParams function", () => {
    const mw = cassandraMiddleware({ baseUrl: "http://localhost:8787" });
...
describe("withCassandra", () => {
  ...
  it("returns a LanguageModelV3 with correct provider and modelId", () => {
    ...
    const wrapped = withCassandra(fakeModel as any, {
      baseUrl: "http://localhost:8787",
    });
  ...
  it("passes options through to cassandraMiddleware", async () => {
    mockFetch.mockResolvedValueOnce(
      mockCompressResponse([{ role: "user", content: "x" }]),
    );
