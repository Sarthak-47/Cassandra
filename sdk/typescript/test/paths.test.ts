...
describe("canonical roots", () => {
  ...
  it("workspaceDir defaults to ~/.cassandra", () => {
    expect(workspaceDir()).toBe(path.join(os.homedir(), ".cassandra"));
  });

  it("workspaceDir honors CASSANDRA_WORKSPACE_DIR env override", () => {
    process.env[CASSANDRA_WORKSPACE_DIR_ENV] = "/tmp/alt_ws";
    expect(workspaceDir()).toBe("/tmp/alt_ws");
  });

  it("workspaceDir ignores blank env value", () => {
    process.env[CASSANDRA_WORKSPACE_DIR_ENV] = "   ";
    expect(workspaceDir()).toBe(path.join(os.homedir(), ".cassandra"));
  });

  it("workspaceDir expands tilde", () => {
    process.env[CASSANDRA_WORKSPACE_DIR_ENV] = "~/custom-ws";
    expect(workspaceDir()).toBe(path.join(os.homedir(), "custom-ws"));
  });

  it("configDir defaults to ~/.cassandra/config", () => {
    expect(configDir()).toBe(path.join(os.homedir(), ".cassandra", "config"));
  });

  it("configDir follows CASSANDRA_WORKSPACE_DIR when only workspace set", () => {
    process.env[CASSANDRA_WORKSPACE_DIR_ENV] = "/tmp/alt_ws";
    expect(configDir()).toBe(path.join("/tmp/alt_ws", "config"));
  });

  it("explicit CASSANDRA_CONFIG_DIR env beats workspace env", () => {
    process.env[CASSANDRA_WORKSPACE_DIR_ENV] = "/tmp/alt_ws";
    process.env[CASSANDRA_CONFIG_DIR_ENV] = "/tmp/alt_cfg";
...
const RESOURCES: ResourceCase[] = [
  {
    name: "savingsPath",
    fn: savingsPath,
    envVar: CASSANDRA_SAVINGS_PATH_ENV,
    filename: "proxy_savings.json",
  },
  {
    name: "toinPath",
    fn: toinPath,
    envVar: CASSANDRA_TOIN_PATH_ENV,
    filename: "toin.json",
  },
  {
    name: "subscriptionStatePath",
    fn: subscriptionStatePath,
    envVar: CASSANDRA_SUBSCRIPTION_STATE_PATH_ENV,
...
describe.each(RESOURCES)(
  ...
  ({ fn, envVar, filename }) => {
    ...
    it("default under ~/.cassandra", () => {
      expect(fn()).toBe(path.join(os.homedir(), ".cassandra", filename));
    });

    it("derived from CASSANDRA_WORKSPACE_DIR when set", () => {
      process.env[CASSANDRA_WORKSPACE_DIR_ENV] = "/tmp/state";
...
describe("derived-only resources", () => {
  ...
  it("memoryDbPath", () => {
    expect(memoryDbPath()).toBe(
      path.join(os.homedir(), ".cassandra", "memory.db"),
    );
  });

  it("nativeMemoryDir", () => {
    expect(nativeMemoryDir()).toBe(
      path.join(os.homedir(), ".cassandra", "memories"),
    );
  });

  it("licenseCachePath", () => {
    expect(licenseCachePath()).toBe(
      path.join(os.homedir(), ".cassandra", "license_cache.json"),
    );
  });

  it("sessionStatsPath", () => {
    expect(sessionStatsPath()).toBe(
      path.join(os.homedir(), ".cassandra", "session_stats.jsonl"),
    );
  });

  it("syncStatePath", () => {
    expect(syncStatePath()).toBe(
      path.join(os.homedir(), ".cassandra", "sync_state.json"),
    );
  });

  it("bridgeStatePath", () => {
    expect(bridgeStatePath()).toBe(
      path.join(os.homedir(), ".cassandra", "bridge_state.json"),
    );
  });

  it("logDir", () => {
    expect(logDir()).toBe(path.join(os.homedir(), ".cassandra", "logs"));
  });

  it("proxyLogPath", () => {
    expect(proxyLogPath()).toBe(
      path.join(os.homedir(), ".cassandra", "logs", "proxy.log"),
    );
  });

  it("debug400Dir", () => {
    expect(debug400Dir()).toBe(
      path.join(os.homedir(), ".cassandra", "logs", "debug_400"),
    );
  });

  it("binDir", () => {
    expect(binDir()).toBe(path.join(os.homedir(), ".cassandra", "bin"));
  });
...
// ---------------------------------------------------------------------------
// Derived-only helpers must follow CASSANDRA_WORKSPACE_DIR end-to-end
// ---------------------------------------------------------------------------
...
describe("plugin dirs", () => {
  ...
  it("pluginConfigDir namespaced under configDir/plugins", () => {
    ...
    expect(a).not.toBe(b);
    expect(a).toBe(
      path.join(os.homedir(), ".cassandra", "config", "plugins", "alpha"),
    );
  });
  ...
  it("pluginWorkspaceDir namespaced under workspaceDir/plugins", () => {
    ...
    expect(a).not.toBe(b);
    expect(a).toBe(
      path.join(os.homedir(), ".cassandra", "plugins", "alpha"),
    );
  });
