...
describe("CassandraClient", () => {
  ...
  it("sends POST to /v1/compress with messages and model", async () => {
    ...
    const client = new CassandraClient({ baseUrl: "http://localhost:8787" });
  ...
  it("throws CassandraAuthError on 401 (always, even with fallback)", async () => {
    mockFetch.mockResolvedValueOnce(
      errorResponse(401, {
    ...
    ).rejects.toThrow(CassandraAuthError);
  });

  it("throws CassandraCompressError on 400 (always, even with fallback)", async () => {
    mockFetch.mockResolvedValueOnce(
      errorResponse(400, {
    ...
    await expect(
      client.compress(sampleMessages, { model: "gpt-4o" }),
    ).rejects.toThrow(CassandraCompressError);
  });
  ...
  it("throws CassandraConnectionError on network error when fallback=false", async () => {
    mockFetch.mockRejectedValueOnce(new TypeError("fetch failed"));
    ...
    await expect(
      client.compress(sampleMessages, { model: "gpt-4o" }),
    ).rejects.toThrow(CassandraConnectionError);
  });
