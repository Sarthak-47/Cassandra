/**
 * End-to-end test: Vercel AI SDK + Cassandra compress()
 *
 * Tests REAL calls through the Vercel AI SDK with Cassandra compression.
 ...
 *
 * Run: CASSANDRA_INTEGRATION=1 npx vitest run test/vercel-ai-e2e.test.ts
 */
...
describe.skipIf(!RUN)("E2E: Vercel AI SDK + Cassandra", () => {
  beforeAll(async () => {
  ...
  it("cassandraMiddleware() transparently compresses before LLM call (OpenAI)", { timeout: 30000 }, async () => {
    ...
    // Wrap model with Cassandra middleware
