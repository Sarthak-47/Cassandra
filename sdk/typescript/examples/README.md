# cassandra-ai examples

Runnable examples showing how to use the Cassandra TypeScript SDK for context compression.

## Prerequisites

- Node.js 18+
- A running Cassandra proxy (`pip install "cassandra-ai[proxy]" && cassandra proxy`)
...
| Example | Description |
|---------|-------------|
| [with-cassandra-vercel.ts](with-cassandra-vercel.ts) | One-liner `withCassandra(openai('gpt-4o'))` — simplest integration |
| [streaming-chat.ts](streaming-chat.ts) | `withCassandra` + `streamText` for real-time streaming |
...
| [middleware-composition.ts](middleware-composition.ts) | Stack `cassandraMiddleware` with other middlewares (`extractReasoningMiddleware`) |
...
| Example | Description |
|---------|-------------|
| [openai-anthropic-adapters.ts](openai-anthropic-adapters.ts) | `withCassandra` for native OpenAI and Anthropic SDKs (no Vercel AI SDK) |
