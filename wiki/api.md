...
The main entry point for Cassandra SDK.
...
response = client.chat.completions.create(
    model="gpt-4o",
    messages=[...],
    cassandra_mode="optimize",  # Override default mode
)
```
...
|-----------|------|-------------|
| `cassandra_mode` | `str` | Override mode for this request |
| `cassandra_query` | `str` | Query for relevance scoring |
...
```python
from cassandra import RelevanceScorerConfig
...
```python
from cassandra import BM25Scorer
...
```python
from cassandra import EmbeddingScorer, embedding_available
...
```python
from cassandra import HybridScorer
...
```python
from cassandra import create_scorer
...
> removed and are no longer part of Cassandra.

### TransformPipeline

```python
from cassandra import TransformPipeline
...
```python
from cassandra import Tokenizer, count_tokens_text, count_tokens_messages
...
```python
from cassandra import generate_report

report = generate_report(
    store_url="sqlite:///cassandra.db",
    format="html",
    period="day",
)
```
...
The TypeScript SDK provides `compress()`, `CassandraClient`, and framework adapters for Vercel AI SDK, OpenAI, and Anthropic.
