...
cassandra learn --apply --target CLAUDE.md

# Analyze a specific project
cassandra learn --project ~/my-project --apply

# Analyze all projects
cassandra learn --all --apply
```
...
The core innovation. Instead of cataloging failures ("Read failed 5 times"), Cassandra finds what the model did to fix each failure:
...
Cassandra manages a clearly-delimited section in each file:

```markdown
<!-- cassandra:learn:start -->
## Cassandra Learned Patterns
*Auto-generated by `cassandra learn` — do not edit manually*
...
<!-- cassandra:learn:end -->
```
...
Cassandra Learn uses a plugin architecture where each agent is a self-contained plugin:
...
**Plugins** bundle scanning, detection, and writing for one agent. Built-in plugins are auto-discovered from `cassandra.learn.plugins.*`. External plugins register via the `cassandra.learn_plugin` entry point.

**The Analyzer** is shared — it uses an LLM (Sonnet, GPT-4o, or Gemini Flash) to find patterns. Same analysis for any agent.

### Adding Support for a New Agent

1. Create `cassandra/learn/plugins/myagent.py`
2. Implement `LearnPlugin` + `ConversationScanner` (scanner + writer + detection)
3. Add `plugin = MyAgentPlugin()` at module scope
4. Done — `cassandra learn --agent myagent` works automatically

Or install an external plugin: `pip install cassandra-learn-cursor` (registers via entry point).

## CLI Reference

```
cassandra learn [OPTIONS]
...
`cassandra learn --verbosity` analyzes past sessions to infer the ideal output verbosity level for your project and writes a `verbosity.json` profile.
...
- Print instructions to set `CASSANDRA_OUTPUT_SHAPER=1` before `cassandra wrap ...`

To keep the shaper on across proxy restarts, add `export CASSANDRA_OUTPUT_SHAPER=1` to your shell profile before starting the proxy.
...
`cassandra learn` needs an LLM to analyze your sessions. It picks one automatically using this priority:

| Priority | Source | Example |
|----------|--------|---------|
| 1 | `--model` flag | `cassandra learn --model gpt-4o` |
| 2 | API key env var | `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `GEMINI_API_KEY` |
| 3 | `CASSANDRA_LEARN_CLI` env var | `export CASSANDRA_LEARN_CLI=gemini` |
| 4 | Auto-detect installed CLIs | Checks PATH for `claude`, `gemini`, `codex` |

### Using without an API key

If you use Claude Code, Gemini CLI, or Codex via subscription (no raw API key), `cassandra learn` can call them directly:
...
cassandra learn --model gemini-cli

# Pin a CLI via environment variable
export CASSANDRA_LEARN_CLI=codex
cassandra learn
```

Valid values for `CASSANDRA_LEARN_CLI`: `claude`, `gemini`, `codex`.
