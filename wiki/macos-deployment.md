...
This guide covers deploying the cassandra proxy server as a background service on macOS using LaunchAgent. The service will start automatically on login and restart on crash.
...
- cassandra-ai installed with proxy support
- Anthropic API key configured

### Installing Cassandra with Proxy Support
...
```bash
# Clone or navigate to cassandra repository
...
1. Detect your cassandra installation
...
```bash
mkdir -p ~/Library/Logs/cassandra
```
...
```bash
cd examples/deployment/macos-launchagent
cp com.cassandra.proxy.plist.template ~/Library/LaunchAgents/com.cassandra.proxy.plist
```

Edit `~/Library/LaunchAgents/com.cassandra.proxy.plist`:

1. Replace `__CASSANDRA_PATH__` with your cassandra path:

   ```bash
   command -v cassandra
   # Example output: /usr/local/bin/cassandra
   ```
...
```bash
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.cassandra.proxy.plist
```
...
launchctl print gui/$(id -u)/com.cassandra.proxy
...
3. Update shell integration: `export CASSANDRA_PORT=9000`

### Log Location

Logs are written to standard macOS locations:

- **Standard output**: `~/Library/Logs/cassandra/proxy.log`
- **Error output**: `~/Library/Logs/cassandra/proxy-error.log`
...
(`CASSANDRA_COMPRESSION_PROVIDER=llmlingua`, `CASSANDRA_LLMLINGUA_DEVICE`,
the `cassandra-ai[llmlingua]` extra) were retired with the
...
source /path/to/cassandra/examples/deployment/macos-launchagent/shell-integration.sh
```
...
tail -f ~/Library/Logs/cassandra/proxy.log

# Tail error output
tail -f ~/Library/Logs/cassandra/proxy-error.log

# View last 50 lines
tail -n 50 ~/Library/Logs/cassandra/proxy-error.log
```
...
launchctl kickstart -k gui/$(id -u)/com.cassandra.proxy

# Manual stop/start
launchctl bootout gui/$(id -u)/com.cassandra.proxy
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.cassandra.proxy.plist
```
...
launchctl disable gui/$(id -u)/com.cassandra.proxy

# Re-enable
launchctl enable gui/$(id -u)/com.cassandra.proxy
```
...
Should show cassandra listening on port 8787.
...
```bash
tail -n 20 ~/Library/Logs/cassandra/proxy-error.log
```
...
| `ModuleNotFoundError: No module named 'cassandra'` | Install: `pip install cassandra-ai[proxy]` |
| `command not found: cassandra` | Update plist with correct path: `command -v cassandra` |
...
- Missing dependencies: `pip install cassandra-ai[proxy]`
...
```bash
# Should be set to 1
echo $CASSANDRA_SHELL_INTEGRATION_LOADED
```
...
```bash
grep -A1 RunAtLoad ~/Library/LaunchAgents/com.cassandra.proxy.plist
```
...
```bash
chmod 644 ~/Library/LaunchAgents/com.cassandra.proxy.plist
```

**Verify ownership:**

```bash
ls -l ~/Library/LaunchAgents/com.cassandra.proxy.plist
```
...
# Remove plist
rm ~/Library/LaunchAgents/com.cassandra.proxy.plist

# Remove logs (optional)
rm -rf ~/Library/Logs/cassandra
```
...
- [Architecture](ARCHITECTURE.md) - How Cassandra works internally
...
cp com.cassandra.proxy.plist.template ~/Library/LaunchAgents/com.cassandra.proxy-2.plist
# Edit: Change Label to com.cassandra.proxy-2, port to 8788
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.cassandra.proxy-2.plist
```
...
```bash
pip install 'cassandra-ai[pytorch-mps]'   # also works as [pytorch_mps]
export CASSANDRA_EMBEDDER_RUNTIME=pytorch_mps
```
...
```xml
<key>CASSANDRA_EMBEDDER_RUNTIME</key>
...
**Q: Why LaunchAgent instead of running `cassandra proxy` manually?**
...
A: Yes, fully compatible. ML compression (Kompress, opt-in via `cassandra-ai[ml]`) auto-detects MPS on Apple Silicon.
