...
Cassandra provides explicit exceptions for debugging, with a safety guarantee that compression failures never break your LLM calls.

## Exception Hierarchy

```python
from cassandra import (
    CassandraError,        # Base class - catch all Cassandra errors
...
This is a core design principle. Your LLM calls never fail due to Cassandra:
...
All Cassandra exceptions include a `details` dict with context:
...
result = client.validate_setup()
if not result["valid"]:
    raise SystemExit(f"Cassandra setup invalid: {result['issues']}")
...
# DEBUG:cassandra.transforms.smart_crusher:Analyzing 1000 items...
# DEBUG:cassandra.transforms.smart_crusher:Kept 15 items (errors: 2, anomalies: 3)
```
