...
Cassandra supports Google Cloud Vertex AI publisher endpoints through the proxy
passthrough surface. Configure the proxy with a regional Vertex base URL, then
send normal Vertex REST requests through Cassandra.
...
```bash
cassandra proxy --vertex-api-url https://us-central1-aiplatform.googleapis.com
```
...
Cassandra also forwards Anthropic publisher calls on Vertex:
...
## Claude Code with Cassandra compression (validated)

To run **Claude Code** against Claude-on-Vertex **with Cassandra compressing the
context**, use the dedicated, tested runbook:

➡️ **[Claude Code + Vertex + Cassandra](https://cassandra-docs.vercel.app/docs/claude-code-vertex)**
...
--code-aware`; Cassandra holds the GCP ADC creds and calls Vertex.
