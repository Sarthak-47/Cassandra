...
Cassandra provides comprehensive metrics for monitoring compression performance, cost savings, and system health.
...
`${CASSANDRA_WORKSPACE_DIR}/proxy_savings.json` (i.e.
`~/.cassandra/proxy_savings.json` when `CASSANDRA_WORKSPACE_DIR` is unset).
Use `CASSANDRA_SAVINGS_PATH` to override the file location directly, or
set `CASSANDRA_WORKSPACE_DIR` to relocate the entire state root. See the
...
> on 3.13 (`pipx reinstall cassandra-ai --python python3.13`) rather than showing
> a misleading `$0.00`.
...
```prometheus
# HELP cassandra_requests_total Total number of requests
cassandra_requests_total 1234

# HELP cassandra_latency_ms_count Count of observed request latencies
cassandra_latency_ms_count 1234

# HELP cassandra_tokens_saved_total Tokens saved by optimization
cassandra_tokens_saved_total 5678900

# HELP cassandra_requests_by_provider Requests by provider
cassandra_requests_by_provider{provider="anthropic"} 800
cassandra_requests_by_provider{provider="openai"} 434

# HELP cassandra_requests_by_stack Requests by Cassandra integration stack
cassandra_requests_by_stack{stack="wrap_claude"} 612
cassandra_requests_by_stack{stack="adapter_ts_openai"} 48

# HELP cassandra_transform_timing_ms_sum Sum of transform timing in milliseconds
cassandra_transform_timing_ms_sum{transform="router"} 5123.7

# HELP cassandra_cache_write_ttl_tokens_total Provider cache write tokens by observed TTL bucket
cassandra_cache_write_ttl_tokens_total{provider="anthropic",ttl="5m"} 20000
cassandra_cache_write_ttl_tokens_total{provider="anthropic",ttl="1h"} 50000
```
...
Cassandra now emits the same operational events through a shared OTEL metrics facade.

There are two integration modes:

1. **Ambient OTEL app setup** - if your application already configures a global OTEL meter provider, Cassandra records into that provider automatically.
2. **Cassandra-managed export** - if you want the proxy to configure its own OTEL metrics exporter, install:

```bash
pip install "cassandra-ai[proxy,otel]"
```

Then set:

```bash
CASSANDRA_OTEL_METRICS_ENABLED=1
CASSANDRA_OTEL_METRICS_EXPORTER=otlp_http
CASSANDRA_OTEL_METRICS_ENDPOINT=http://127.0.0.1:4318/v1/metrics
CASSANDRA_OTEL_SERVICE_NAME=cassandra-proxy
CASSANDRA_OTEL_RESOURCE_ATTRIBUTES=deployment.environment=dev,service.namespace=cassandra
```
...
```bash
CASSANDRA_OTEL_METRICS_ENABLED=1
CASSANDRA_OTEL_METRICS_EXPORTER=console
cassandra proxy
```

The proxy's `/stats` response now includes an `otel` block that reports whether Cassandra is managing an OTEL exporter for the current process.

Cassandra's managed OTEL exporters are intentionally scoped to Cassandra's own instrumentation. If you already manage global OTEL providers in your app, keep using those and let Cassandra record into the ambient providers instead of enabling `CASSANDRA_OTEL_*`.

### OTEL Environment Variables

| Variable | Default | Description |
|----------|---------|-------------|
| `CASSANDRA_OTEL_METRICS_ENABLED` | `0` | Enables Cassandra-managed OTEL metric export |
| `CASSANDRA_OTEL_METRICS_EXPORTER` | `otlp_http` | Exporter type: `otlp_http` or `console` |
| `CASSANDRA_OTEL_METRICS_ENDPOINT` | unset | OTLP HTTP metrics endpoint |
| `CASSANDRA_OTEL_METRICS_HEADERS` | unset | Comma-separated `key=value` headers for OTLP export |
| `CASSANDRA_OTEL_METRICS_EXPORT_INTERVAL_MS` | `10000` | Periodic export interval in milliseconds |
| `CASSANDRA_OTEL_SERVICE_NAME` | `cassandra-proxy` in proxy mode | OTEL `service.name` |
| `CASSANDRA_OTEL_RESOURCE_ATTRIBUTES` | unset | Comma-separated resource attributes |

### Anonymous Telemetry vs OTEL

Cassandra has two separate systems:

- `CASSANDRA_TELEMETRY` / `--telemetry` / `--no-telemetry` controls the privacy-preserving anonymous data-flywheel beacon and TOIN-related aggregate reporting. It is **off by default** (opt-in): set `CASSANDRA_TELEMETRY=on` or pass `--telemetry` to enable it.
- `CASSANDRA_OTEL_*` controls operational OTEL metric export.
...
- `cassandra_stack` — how Cassandra is invoked in this process. Values:
  ...
  `CASSANDRA_STACK`; `cassandra wrap <tool>` sets it automatically.
- `install_mode` — how the proxy is deployed. Values: `wrapped` (spawned by
  `cassandra wrap`), `persistent` (long-lived service on a fixed port),
...
- `requests_by_stack` — for proxies serving multiple integrations (e.g. a
  ...
  request count dict mirroring the `cassandra_requests_by_stack` counter.

Clients tag requests with an `X-Cassandra-Stack` header; the proxy's FastAPI
...
- Cassandra metrics continue to go to `/metrics` and/or your OTEL metrics exporter.
- Langfuse receives OTLP traces for Cassandra's compression pipeline.
- Cassandra's `/stats` response includes a `langfuse` block when Cassandra is managing Langfuse trace export for the process.

Enable it with:

```bash
CASSANDRA_LANGFUSE_ENABLED=1
...
```
INFO:cassandra.transforms.pipeline:Pipeline complete: 45000 -> 4500 tokens (saved 40500, 90.0% reduction)
INFO:cassandra.transforms.smart_crusher:SmartCrusher applied top_n strategy: kept 15 of 1000 items
INFO:cassandra.cache.compression_store:CCR cache hit: hash=abc123, retrieved 1000 items
DEBUG:cassandra.transforms.smart_crusher:Kept items: [0,1,2,42,77,97,98,99] (errors at 42, warnings at 77)
```
...
```bash
# Log to file
cassandra proxy --log-file cassandra.jsonl

# Enable request logging
cassandra proxy --log-messages
```
  ...
  "panels": [
    {
      "title": "Tokens Saved",
      "type": "stat",
      "targets": [{"expr": "cassandra_tokens_saved_total"}]
    },
    {
      "title": "Average Request Latency (ms)",
      "type": "gauge",
      "targets": [{"expr": "cassandra_latency_ms_sum / clamp_min(cassandra_latency_ms_count, 1)"}]
    },
    {
      "title": "Max Request Latency (ms)",
      "type": "graph",
      "targets": [{"expr": "cassandra_latency_ms_max"}]
    },
    {
      "title": "Provider Cache Hit Rate",
      "type": "gauge",
      "targets": [{"expr": "cassandra_provider_cache_hit_requests_total / clamp_min(cassandra_provider_cache_requests_total, 1)"}]
    }
  ]
}
```
