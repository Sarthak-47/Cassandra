...
Get Cassandra running in 5 minutes with these copy-paste examples.
...
See [Docker-native install](docker-install.md) if you want Docker to provide the Cassandra runtime while your agent CLIs stay on the host.
...
See [Persistent Installs](persistent-installs.md) if you want Cassandra to stay up in the background and be reused by `wrap`.
...
messages = [
    ...
    },
    {
        "role": "tool",
        "tool_call_id": "call_1",
        # This is where Cassandra shines - compressing large outputs
        "content": json.dumps({
...
# Cassandra compresses the 500 results to ~20, keeping the most relevant
response = client.chat.completions.create(
    model="gpt-4o",
    messages=messages,
)
...
# INFO:cassandra.transforms.smart_crusher:SmartCrusher: keeping 15 of 500 items
```
...
```python
from cassandra import CassandraClient, OpenAIProvider, CassandraConfig
...
response = client.chat.completions.create(
    model="gpt-4o",
    messages=messages,
    cassandra_mode="optimize",  # Enable for this request only
)
```
...
| Content Type | What Cassandra Does | Typical Savings |
|--------------|-------------------|-----------------|
