...
The Cassandra SDK wraps your existing LLM client to add compression and optimization transparently.

## Installation

```bash
pip install cassandra-ai openai
```
...
Real savings happen with tool outputs. Here's where Cassandra shines:
...
# Cassandra compresses 500 results to ~15, keeping highest-scoring items
response = client.chat.completions.create(
    model="gpt-4o-mini",
    messages=messages
)
...
```python
from cassandra import CassandraClient, GoogleProvider
...
# INFO:cassandra.transforms.smart_crusher:SmartCrusher: kept 15 of 1000 items
```
