...
Cassandra provides several transforms that work together to optimize LLM context.
...
```python
from cassandra import SmartCrusherConfig
...
```python
from cassandra import SmartCrusher
...
```python
from cassandra import CacheAligner
...
```python
from cassandra import CacheAlignerConfig
...
(live-zone-only compression). Cassandra **never** drops messages from the
...
the `cassandra-ai[llmlingua]` extra, and the `--llmlingua` proxy flag)
was retired in 0.9.x and replaced by **Kompress** (ModernBERT).
`pip install 'cassandra-ai[llmlingua]'` no longer resolves; use the
...
```bash
pip install "cassandra-ai[code]"  # Adds tree-sitter-language-pack
```

### Configuration

```python
from cassandra.transforms import CodeAwareCompressor, CodeCompressorConfig, DocstringMode
...
```python
from cassandra.transforms import CodeAwareCompressor
...
```python
from cassandra.transforms import is_tree_sitter_available, unload_tree_sitter
...
```python
from cassandra.transforms import ContentRouter, ContentRouterConfig, CompressionStrategy
...
```python
from cassandra import TransformPipeline, SmartCrusher, CacheAligner
