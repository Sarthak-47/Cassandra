...
Cassandra automatically compresses images in your LLM requests, reducing token usage by **40-90%** while maintaining answer accuracy.
...
Cassandra's image compression uses a **trained ML router** to analyze your query and automatically select the optimal compression technique:
...
### With Cassandra Proxy (Zero Code Changes)
...
client = CassandraClient(provider="openai")
...
cassandra proxy --image-optimize

# Disable image compression
cassandra proxy --no-image-optimize
```
...
compressor = ImageCompressor(
    model_id="Sarthak-47/technique-router",  # HuggingFace model
...
- **Model**: `Sarthak-47/technique-router` on HuggingFace
- **Size**: ~128MB
...
class ImageCompressor:
    def __init__(
        self,
        model_id: str = "Sarthak-47/technique-router",
