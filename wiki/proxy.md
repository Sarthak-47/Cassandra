...
The Cassandra proxy server is a production-ready HTTP server that applies context optimization to all requests passing through it.
...
# Custom port
cassandra proxy --port 8080

# With all options
cassandra proxy \
  --host 0.0.0.0 \
  --port 8787 \
  --log-file /var/log/cassandra.jsonl \
  --budget 100.0
```
...
`cassandra wrap copilot` uses Copilot CLI's BYOK provider settings under the hood. In `provider-type=auto`, it chooses Cassandra's Anthropic route for the default proxy backend and the OpenAI-compatible `/v1` route for translated backends such as `anyllm` and LiteLLM.

Anonymous aggregate telemetry is **off by default** (opt-in). Opt in with `CASSANDRA_TELEMETRY=on` or `cassandra proxy --telemetry`. Downstream apps can set `CASSANDRA_SDK=cassandra-app` to override the anonymous telemetry `sdk` label; the default remains `proxy`.

Operational OTEL metrics are configured separately and are **off by default**. Install `cassandra-ai[proxy,otel]` and set:
...
Use `CASSANDRA_OTEL_METRICS_EXPORTER=console` for local smoke testing. `CASSANDRA_TELEMETRY` controls the anonymous data-flywheel beacon only; it does not disable or enable OTEL export.

Langfuse can be enabled alongside this OTEL path for **trace ingestion**. Langfuse does **not** ingest OTEL metrics, so Cassandra keeps metrics and Langfuse traces as complementary signals:
...
When configured, Cassandra emits OTLP traces for the shared compression pipeline to Langfuse while continuing to expose metrics through `/metrics` and OTEL metric exporters.
...
| `--code-aware` | true | Enable AST-based code compression (env: CASSANDRA_CODE_AWARE_ENABLED) |
...
Cassandra proxy has two explicit run modes:
...
```bash
cassandra proxy --mode token
CASSANDRA_MODE=cache cassandra proxy
```
...
Legacy values (`token_cassandra`, `cost_savings`) are still accepted as aliases.
...
| Option | Description |
|--------|-------------|
| `--no-ccr-inject-tool` | Do not inject the `cassandra_retrieve` tool into the LLM's available tools |
...
the `cassandra-ai[llmlingua]` extra were retired and replaced by Kompress
(ModernBERT). For the current opt-in path, install `cassandra-ai[ml]`
...
`/readyz` returns HTTP 503 when Cassandra has not completed startup or a required enabled subsystem is unavailable. This is the endpoint used by the container health checks.
...
other Cassandra frontends. It returns:
...
Set `CASSANDRA_SAVINGS_PATH` to override the location directly, or set
`CASSANDRA_WORKSPACE_DIR` to relocate the full state root. See the
...
**Headers:**
- `x-cassandra-bypass: true` — skip compression, return messages as-is
...
1. Start the proxy: `cassandra proxy`
...
> `cassandra-ai[llmlingua]`, `LLMLinguaCompressor`) was retired and
> replaced by **Kompress** (ModernBERT). Install with `pip install
> 'cassandra-ai[ml]'`. See [transforms.md](transforms.md) and
...
```
cassandra_requests_total
cassandra_tokens_saved_total
cassandra_cost_usd_total
cassandra_latency_ms_sum
```

## Configuration via Environment

```bash
export CASSANDRA_HOST=0.0.0.0
export CASSANDRA_PORT=8787
export CASSANDRA_BUDGET=100.0
...
# Run with gunicorn
gunicorn cassandra.proxy.server:app \
  --workers 4 \
  --bind 0.0.0.0:8787 \
...
EXPOSE 8787
CMD ["cassandra", "proxy", "--host", "0.0.0.0"]
```

> **Note:** `build-essential` is required at install time because `cassandra-ai` includes `hnswlib`, a C++ extension that must be compiled from source. It is removed after installation to keep the image slim.
