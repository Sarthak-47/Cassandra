...
## What Makes Cassandra Memory Different?

| Feature | Cassandra | Letta (MemGPT) | Mem0 |
|---------|----------|----------------|------|
...
cassandra proxy --memory

# Or use wrap (auto-starts proxy)
cassandra wrap claude --memory    # Claude Code with persistent memory
cassandra wrap codex --memory     # Codex with the SAME memory store
cassandra wrap aider --memory     # Aider shares it too
```
...
Memory is stored per-project at `{cwd}/.cassandra/memory.db`. Each
...
> canonical `CASSANDRA_WORKSPACE_DIR` env var. This preserves the
...
User ID is auto-detected from `$USER` (your OS username). Override per-request with the `x-cassandra-user-id` header. All memories are scoped to the user — multiple developers on the same project have separate memory stores.
...
When the LLM calls `memory_save`, cassandra:
...
The LLM then decides whether to merge — using the user's own LLM, not a separate model. No extra cost to cassandra.
...
from cassandra import with_memory
...
Memories evolve over time. When facts change, Cassandra creates a **supersession chain** preserving history:

```python
from cassandra.memory import HierarchicalMemory, MemoryConfig
...
```python
import asyncio
from cassandra.memory import (
    HierarchicalMemory,
    MemoryConfig,
    MemoryCategory,
    EmbedderBackend,
)
from cassandra.memory.ports import MemoryFilter, VectorFilter
...
```python
from cassandra.memory import MemoryConfig, EmbedderBackend
...
Cassandra Memory uses **Protocol interfaces** (ports) for all components, enabling easy swapping:
...
**Letta** pioneered inline memory extraction. Cassandra builds on this with:

| Aspect | Cassandra | Letta |
|--------|----------|-------|
...
**When to use Cassandra**: You want memory as a layer on your existing stack.

### vs Mem0

**Mem0** provides a managed memory service. Cassandra differs:

| Aspect | Cassandra | Mem0 |
|--------|----------|------|
...
**When to use Cassandra**: You want embedded memory with no external services.

### Feature Matrix

| Feature | Cassandra | Letta | Mem0 |
|---------|:--------:|:-----:|:----:|
