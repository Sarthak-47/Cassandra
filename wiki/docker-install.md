...
Run Cassandra without installing Python or Node.js on the host. The install scripts add a native `cassandra` wrapper that keeps **Cassandra itself** in Docker while orchestrating the rest of your workflow on the host OS.
...
```bash
curl -fsSL https://raw.githubusercontent.com/Sarthak-47/cassandra/main/scripts/install.sh | "$(brew --prefix bash)/bin/bash"
```
...
2. Pulls `ghcr.io/Sarthak-47/cassandra:latest` by default, or reuses / pulls `CASSANDRA_DOCKER_IMAGE` when you set a custom image override.
3. Installs a `cassandra` wrapper into `~/.local/bin` or `~/bin`.
4. Updates shell startup files so the wrapper directory is on `PATH`.

The wrapper keeps Cassandra inside Docker and mounts host state back into the container so native behavior stays consistent:
...
Published releases also push versioned GHCR tags such as `ghcr.io/Sarthak-47/cassandra:0.5.26`, and those images are built with the same synced package version used for the matching PyPI and npm release.

## How the wrapper behaves

### Native Cassandra commands
...
docker run --rm -it \
  ...
  cassandra proxy --host 0.0.0.0 --port 8787
```
...
- the wrapper starts the Cassandra proxy in Docker
- container-side prep writes Cassandra config, memory, and `rtk` guidance into mounted host files
...
- `cassandra wrap openclaw` installs/configures the Cassandra plugin through the host `openclaw` CLI
- plugin auto-start still launches the installed host `cassandra` wrapper from `PATH`, which then runs Cassandra in Docker
...
The Docker-native `cassandra` wrapper now exposes the persistent Docker lifecycle directly:

```bash
cassandra install apply --profile default --preset persistent-docker
...
Those broader lifecycle and config-mutation flows still belong to the Python-native `cassandra install ...` command.
...
Use `docker/docker-compose.native.yml` when you want an explicit compose-managed proxy or CLI shell, or when you prefer compose over the native wrapper's `cassandra install ...` surface.

### Persistent Docker runtime

The `proxy` service now uses `restart: unless-stopped`, so compose can act as the always-on Docker runtime for Cassandra:
...
```powershell
$env:CASSANDRA_HOST_HOME = $HOME
$env:CASSANDRA_WORKSPACE = (Get-Location).Path
docker compose -f docker/docker-compose.native.yml up -d proxy
```
...
#### `CASSANDRA_WORKSPACE` vs `CASSANDRA_WORKSPACE_DIR`

These are two different variables — both are set by the compose file,
and both are retained for backward compatibility:

- **`CASSANDRA_WORKSPACE`** (host-side) is the directory the compose file
  bind-mounts into the container as `/workspace`. It behaves like CWD
  in a native (non-Docker) run.
- **`CASSANDRA_WORKSPACE_DIR`** (inside-the-container) is the canonical
  Cassandra state root — part of the [filesystem contract][fs]
  introduced in issue #175. The compose file sets it to
  `/tmp/cassandra-home/.cassandra` so the proxy resolves savings, logs,
  TOIN, and memory under the bind-mounted `${HOME}/.cassandra`.

You do not need to set `CASSANDRA_WORKSPACE_DIR` manually when using the
...
The wrapper forwards Cassandra and provider environment variables into the container, including common prefixes such as:
...
- Docker is the only required Cassandra runtime dependency on the host.
- Wrapped tools like Claude Code, Codex CLI, Aider, and Cursor still run on the host when you use `cassandra wrap ...`.
...
- For persistent service and task installs, use the Python-native `cassandra install ...` workflow described in [Persistent Installs](persistent-installs.md).
- For Docker-native `cassandra install ...`, the wrapper persists its profile manifest under `~/.cassandra/deploy/<profile>/`.
