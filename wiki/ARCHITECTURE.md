# Cassandra SDK: A Complete Explanation
...
flowchart TB
    subgraph Entry["Entry Points"]
        ...
        SDK["SDK Mode<br/><i>CassandraClient</i>"]
...
**Cassandra creates "cassandra"** - it intelligently compresses your input tokens so you have more room (and budget) for what matters.

---

## How Cassandra Works: The Big Picture
...
### 1. CassandraClient (`client.py`) - The Wrapper
...
# After (with Cassandra)
base = OpenAI(api_key="...")
client = CassandraClient(original_client=base, provider=OpenAIProvider())
response = client.chat.completions.create(model="gpt-4o", messages=[...])
```
...
**Why this matters:** Token counting is model-specific. GPT-4 uses different tokenization than Claude. Cassandra needs accurate counts to know how much to compress.

---

### 3. Parser (`parser.py`) - Understanding Your Messages

Before optimizing, Cassandra needs to understand what's in your messages:
...
This is where the real work happens. Cassandra has 4 transforms that run in sequence:
...
# Smart compression:
{
    "__cassandra_constants": {"host": "prod-1"},  # Factor out
    "__cassandra_summary": "items 0-44: cpu stable at ~45",  # Summarize boring part
    "data": [
...
'cassandra-ai[ml]'`; see `wiki/transforms.md` for current configuration.

**Note:** The earlier LLMLingua-2 integration (`--llmlingua` flag, the
`cassandra-ai[llmlingua]` extra, and the `LLMLinguaCompressor` class) was
retired and replaced by Kompress. `pip install 'cassandra-ai[llmlingua]'`
...
**Approach:** Cassandra never drops messages from conversation history, and it does not do position-based or score-based context management. Context management is handled automatically inside the pipeline by compressing only the **live zone** — the newest content blocks (the latest user message and the latest tool result / tool output).
...
**Why this matters:** Leaving the hot zone untouched preserves provider prompt caching (e.g. Claude's cached-prefix read discount). Because only the newest blocks change, earlier turns stay byte-stable across requests and keep hitting the KV cache. Compression is type-aware (handled by ContentRouter and its compressors) and fully reversible — the LLM can call `cassandra_retrieve` to restore any compressed block.
...
CREATE TABLE requests (
    ...
    tokens_input_before INTEGER,  -- Before Cassandra
    tokens_input_after INTEGER,   -- After Cassandra
...
response = client.chat.completions.create(
    ...
    cassandra_mode="optimize",
)
```

### Step 2: CassandraClient intercepts
```python
# In client.py:
...
def apply(self, messages, ...):
    ...
    # Transform 3: Kompress ML compressor (opt-in via cassandra-ai[ml])
...
**Location:** `cassandra/cache/compression_store.py`
...
When compression happens, Cassandra injects retrieval instructions into the LLM context.
...
When running as MCP server, Cassandra exposes retrieval as a tool:
...
```json
{
    "__cassandra_compressed": true,
    "__cassandra_hash": "abc123def456",
    "__cassandra_stats": {
        "original_items": 1000,
        "kept_items": 20,
        "errors_preserved": 5
...
**Location:** `cassandra/cache/compression_feedback.py`
...
**Location:** `cassandra/ccr/response_handler.py`

**The Problem:** When the proxy injects the `cassandra_retrieve` tool, the LLM might call it. But who handles that tool call? Without response handling, the tool call would go back to the client unhandled.
...
│     └─ Contains: tool_use(cassandra_retrieve, hash=abc123)       │
│                                                                   │
...
**Location:** `cassandra/ccr/context_tracker.py`
...
Vision models charge by the token, and images are expensive (765-2900 tokens for a typical image). Cassandra's image compression uses a **trained ML router** to automatically select the optimal compression technique.
...
- **Model**: `Sarthak-47/technique-router`
...
├── client.py            # CassandraClient - the main wrapper
...
**Cassandra is a Context Budget Controller that:**
