...
[![PyPI](https://img.shields.io/pypi/v/cassandra-ai.svg)](https://pypi.org/project/cassandra-ai/)
[![Python](https://img.shields.io/pypi/pyversions/cassandra-ai.svg)](https://pypi.org/project/cassandra-ai/)
[![License](https://img.shields.io/badge/license-Apache%202.0-blue.svg)](https://github.com/Sarthak-47/cassandra/blob/main/LICENSE)
...
Every tool call, DB query, file read, and RAG retrieval your agent makes is 70-95% boilerplate. Cassandra compresses it away before it hits the model. The LLM sees less noise, responds faster, and costs less.
...
Cassandra works as a **transparent proxy** (zero code changes), a **Python function** (`compress()`), or a **framework integration** (LangChain, Agno, Strands, LiteLLM, MCP).
...
```python
from cassandra.integrations.strands import CassandraStrandsModel

model = CassandraStrandsModel(wrapped_model=bedrock_model)
agent = Agent(model=model)
```
...
Three tools for Claude Code, Cursor, or any MCP client: `cassandra_compress`, `cassandra_retrieve`, `cassandra_stats`.

```bash
cassandra mcp install && claude
```
...
[OpenClaw Plugin &rarr;](https://github.com/Sarthak-47/cassandra/tree/main/plugins/openclaw)
...
Context management is handled automatically inside the pipeline (live-zone-only compression): Cassandra compresses only the newest content blocks (the latest user message and tool results) and never drops messages from history. The system prompt, tool definitions, and older turns — the provider cache hot zone — are left untouched so prompt caching keeps working.

**Nothing is lost.** Compressed content goes into the CCR store (Compress-Cache-Retrieve). The LLM gets a `cassandra_retrieve` tool and can fetch full originals when it needs more detail.
...
```bash
cassandra proxy                                          # Direct Anthropic/OpenAI
cassandra proxy --backend bedrock --region us-east-1     # AWS Bedrock
cassandra proxy --backend vertex_ai --region us-central1 # Google Vertex AI
cassandra proxy --backend azure                          # Azure OpenAI
cassandra proxy --backend openrouter                     # OpenRouter (400+ models)
```
...
```bash
pip install cassandra-ai                # Core library (Python)
pip install "cassandra-ai[all]"         # Everything (recommended)
npm install cassandra-ai                # TypeScript / Node.js
pip install "cassandra-ai[proxy]"       # Proxy server + MCP tools
pip install "cassandra-ai[ml]"          # ML compression (Kompress, requires torch)
pip install "cassandra-ai[langchain]"   # LangChain integration
pip install "cassandra-ai[agno]"        # Agno integration
pip install "cassandra-ai[evals]"       # Evaluation framework
```
...
- **[Integration Guide](integration-guide.md)** — Every way to add Cassandra to your stack
...
Apache 2.0 — Free for commercial use. [GitHub](https://github.com/Sarthak-47/cassandra) | [PyPI](https://pypi.org/project/cassandra-ai/) | [Discord](https://discord.gg/yRmaUNpsPJ)
