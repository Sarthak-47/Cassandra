...
When agents hand off to each other, context gets replayed in full. SharedContext compresses what moves between agents using Cassandra's compression pipeline.

## Quick Start

```python
from cassandra import SharedContext
...
Store content under a key. Compresses automatically using Cassandra's full pipeline (SmartCrusher for JSON, CodeCompressor for code, Kompress for text).
...
Under the hood, `put()` calls `cassandra.compress()` (the same pipeline used by the proxy) and stores the original in memory. `get()` returns the compressed version. `get(full=True)` returns the original.
