...
You don't need to run the Cassandra proxy. Cassandra is a compression library that works with **any** LLM client, proxy, or framework.
...
| Claude Code / Cursor / Copilot CLI | [Cassandra proxy](#proxy) | 1 command or env var |
| Agno agents | [Agno integration](#agno) | Wrap model |
| LangChain | [LangChain integration](#langchain) | Wrap model |
| Non-Python app | [Cassandra proxy](#proxy) | HTTP |
| TypeScript SDK | [`compress()`](#typescript-sdk) | `npm install cassandra-ai` |
| Vercel AI SDK | [`cassandraMiddleware()`](#typescript-sdk) | Middleware adapter |
| OpenAI Node SDK | [`withCassandra()`](#typescript-sdk) | Client wrapper |
| Anthropic TS SDK | [`withCassandra()`](#typescript-sdk) | Client wrapper |
...
If you're already using LiteLLM as your LLM gateway, add Cassandra as a callback:
...
- `x-cassandra-compressed: true` — compression was applied
- `x-cassandra-tokens-saved: 1234` — tokens removed

---

## Proxy

The Cassandra proxy is a standalone HTTP server. Best for non-Python apps or tools that only support base URL configuration (Claude Code, Cursor, GitHub Copilot CLI).
...
For translated backends, the Copilot wrapper can switch to Cassandra's OpenAI-compatible route:

```bash
cassandra wrap copilot --backend anyllm --anyllm-provider groq -- --model gpt-4o
```

By default, `cassandra wrap copilot` installs `rtk` and appends token-optimized shell guidance to `.github/copilot-instructions.md` so Copilot sessions reuse the same command-saving conventions as other wrapped agent CLIs. Use `--no-rtk` to skip that step.

For Copilot's **hosted** API (`--subscription` and the implicit OAuth path), Cassandra routes to the generic host `https://api.githubcopilot.com`, which serves the full model set. **Enterprise / data-residency** tenants on a dedicated Copilot host pin it with `GITHUB_COPILOT_API_URL` (e.g. `export GITHUB_COPILOT_API_URL=https://api.<your-host>.githubcopilot.com`); the override flows through to the upstream request. See [`TESTING-copilot-subscription.md`](https://github.com/Sarthak-47/cassandra/blob/main/TESTING-copilot-subscription.md).

### With Cloud Providers

```bash
# AWS Bedrock
cassandra proxy --backend bedrock --region us-east-1

# Google Vertex AI
cassandra proxy --backend vertex_ai --region us-central1

# Azure OpenAI
cassandra proxy --backend azure

# OpenRouter (400+ models)
OPENROUTER_API_KEY=sk-or-... cassandra proxy --backend openrouter
```
...
model = CassandraAgnoModel(Claude(id="claude-sonnet-4-20250514"))
...
```json
{ "plugins": { "slots": { "contextEngine": "cassandra" } } }
```
...
The plugin auto-detects a running Cassandra proxy or starts one. Compression happens in `assemble()` — zero changes to the agent's behavior.

See the [OpenClaw plugin documentation](https://github.com/Sarthak-47/cassandra/tree/main/plugins/openclaw) for full setup.

---

## Compression Hooks (Advanced)

Customize compression behavior without modifying Cassandra's code:

```python
from cassandra import compress, CompressionHooks, CompressContext
...
**Q: Does Cassandra change the response format?**
No. Your LLM returns the same response format. Cassandra only modifies the input messages.

**Q: What if compression removes something the LLM needs?**
Cassandra stores originals in CCR (Compress-Cache-Retrieve). The LLM can call `cassandra_retrieve` to get full uncompressed content. Compression summaries tell the LLM what's available.
