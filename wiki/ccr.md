...
Cassandra's CCR architecture makes compression **reversible**. When content is compressed, the original data is cached. If the LLM needs more data, it can retrieve it instantly.
...
│  Option B: LLM calls cassandra_retrieve(hash=abc123)             │
...
Cassandra injects a `cassandra_retrieve` tool into the LLM's available tools:

```json
{
  "name": "cassandra_retrieve",
  "description": "Retrieve original uncompressed data from Cassandra cache",
  "parameters": {
...
When the LLM calls `cassandra_retrieve`:
...
Cassandra never drops whole messages from conversation history. CCR is purely about compressed **content blocks** — the newest tool outputs, tool results, and user content that the live-zone pipeline compresses. The original block is stored in the cache and is retrievable on demand:
...
│            → Calls cassandra_retrieve(hash=def456)               │
...
| Feature | Description |
|---------|-------------|
| **Automatic Response Handling** | When LLM calls `cassandra_retrieve`, the proxy handles it automatically |
| **Multi-Turn Context Tracking** | Tracks compressed content across turns, proactively expands when relevant |
| **Hash-Keyed Retrieval** | `cassandra_retrieve(hash)` always returns the full original content |
...
cassandra proxy --no-ccr-responses

# Disable proactive expansion
cassandra proxy --no-ccr-expansion
```
