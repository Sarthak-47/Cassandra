...
Harden the shared Cassandra proxy so it can survive real multi-agent Codex traffic — especially the **large Anthropic `/v1/messages?beta=true` reconnect/retry storm** that hits the proxy immediately after a restart — without appearing dead (`/livez` timing out, new `/v1/responses` websocket handshakes hanging) and without bypassing compression.
...
- **Not** modifying `cassandra-ai[ml]` dependencies, HuggingFace model IDs, or the embedder's own backend.
...
- `cassandra/proxy/handlers/openai.py:1206` — `handle_openai_responses_ws` (Codex WS entry point, 600+ lines)
- `cassandra/proxy/handlers/openai.py:1559-1767` — upstream WS connect retry loop, `open_timeout` handling
- `cassandra/proxy/handlers/openai.py:1815` — `_ws_http_fallback` (preserved as-is)
- `cassandra/proxy/handlers/openai.py:1439-1452` — memory context timeout fail-open (preserved as-is)
- `cassandra/proxy/handlers/anthropic.py:293` — `handle_anthropic_messages` (HTTP entry point, also covers the `?beta=true` replay traffic)
- `cassandra/proxy/server.py:586-733` — `CassandraProxy.startup` (where eager preload runs)
- `cassandra/proxy/server.py:634-637` — current eager preload iterates **only** `anthropic_pipeline.transforms` and breaks on first match
- `cassandra/proxy/server.py:301-308` — both pipelines currently share the same `transforms` list (so the module-level `_kompress_cache` is de facto shared, but this is fragile)
- `cassandra/proxy/server.py:1190-1212` — `/livez`, `/readyz`, `/health` handlers (trivial JSON, no I/O)
- `cassandra/proxy/memory_handler.py:134-207` — `MemoryHandler._ensure_initialized` (lazy, no `asyncio.Lock`)
- `cassandra/transforms/content_router.py:1221-1297` — `eager_load_compressors` (Kompress + Magika + Code-Aware + SmartCrusher)
- `cassandra/transforms/kompress_compressor.py:163-221` — `_load_kompress_onnx` / `_load_kompress` with module-level `_kompress_cache` + `threading.Lock`
- `cassandra/proxy/request_logger.py` — existing structured request log sink (extend, don't replace)
- `cassandra/proxy/prometheus_metrics.py` — existing `PrometheusMetrics` class; add counters/gauges here
- `cassandra/proxy/helpers.py:138-207` — `_read_request_json` (pre-upstream work on Anthropic path)
...
- *Q: Target upstream or the fork?* Resolved: target the fork (`fix/responses-retries-keep-compression`), structured so each unit is cherry-pickable into a PR against `Sarthak-47/cassandra#172`.
...
- **Exact default for the Anthropic pre-upstream semaphore.** Start at `max(2, min(8, os.cpu_count()))` and expose via `--anthropic-pre-upstream-concurrency` / `CASSANDRA_ANTHROPIC_PRE_UPSTREAM_CONCURRENCY`. Tune after Unit 2's stage timings land.
...
- **Final placement of `WebSocketSessionRegistry`**: `cassandra/proxy/ws_session_registry.py` as a new module, or folded into `cassandra/proxy/server.py`. Likely new module; confirm during implementation once imports are real.
...
**Files:**
- Modify: `cassandra/proxy/server.py` (startup orchestration around lines 627-675)
- Modify: `cassandra/proxy/memory_handler.py` (add `asyncio.Lock` in `_ensure_initialized`)
- Modify: `cassandra/transforms/content_router.py` (no behavior change — ensure `eager_load_compressors` is idempotent when called via both pipelines)
- Create: `cassandra/proxy/warmup.py` (new `WarmupRegistry` holding preloaded handles)
- Test: `tests/test_proxy_warmup.py` (new)
- Test: `tests/test_memory_handler_concurrent_init.py` (new)

**Approach:**
- Introduce `WarmupRegistry` with typed slots for kompress, magika, code-aware, smart-crusher, memory-embedder, memory-backend. Populate during `CassandraProxy.startup`, expose via `proxy.warmup` for `/debug/warmup` (Unit 5) and `/readyz`.
...
**Files:**
- Modify: `cassandra/proxy/handlers/openai.py` (WS path at lines 1206+; HTTP path at 798+)
- Modify: `cassandra/proxy/handlers/anthropic.py` (`handle_anthropic_messages` around line 293 and downstream)
- Modify: `cassandra/proxy/request_logger.py` (extend schema)
- Modify: `cassandra/proxy/prometheus_metrics.py` (add histograms per stage)
- Modify: `cassandra/proxy/helpers.py` (thread `stage_timer` through `_read_request_json` / compression helpers)
- Create: `cassandra/proxy/stage_timer.py` (small context-manager util)
...
**Patterns to follow:**
- `cassandra/proxy/request_logger.py` structured-field schema.
...
**Files:**
- Create: `cassandra/proxy/ws_session_registry.py` (new)
- Modify: `cassandra/proxy/handlers/openai.py` (`handle_openai_responses_ws` at 1206; relay task construction around 1559-1767; `_upstream_to_client` at 1565)
- Modify: `cassandra/proxy/prometheus_metrics.py` (add `active_ws_sessions` gauge, `active_relay_tasks` gauge, `ws_session_duration` histogram)
...
**Files:**
- Modify: `cassandra/proxy/handlers/anthropic.py` (wrap the pre-upstream phases in `handle_anthropic_messages` at ~293 through first upstream call)
- Modify: `cassandra/proxy/models.py` or config module (add `anthropic_pre_upstream_concurrency: int` config field)
- Modify: `cassandra/proxy/server.py` (construct the `asyncio.Semaphore` on `CassandraProxy.__init__` so it's per-process, not per-request)
...
**Approach:**
- Add `self.anthropic_pre_upstream_sem = asyncio.Semaphore(config.anthropic_pre_upstream_concurrency or max(2, min(8, os.cpu_count())))` in `CassandraProxy.__init__`.
...
- Default config: `CASSANDRA_ANTHROPIC_PRE_UPSTREAM_CONCURRENCY` env var honored; CLI flag overrides; unset defaults to the computed floor.

**Patterns to follow:**
- Existing config field addition pattern in `cassandra/proxy/models.py`.
- Existing CLI flag plumbing in `cassandra/cli/`.
...
**Files:**
- Modify: `cassandra/proxy/server.py` (add `/debug/tasks`, `/debug/ws-sessions`, `/debug/warmup` routes near the existing `/livez` at 1190)
- Create: `cassandra/proxy/debug_introspection.py` (pure functions that serialize state)
- Create: `cassandra/proxy/loopback_guard.py` (middleware / dep that 404s non-loopback requests)
...
- Integration: after Unit 4 lands, harness with default Anthropic concurrency limits shows livez p99 < 100ms; harness with `CASSANDRA_ANTHROPIC_PRE_UPSTREAM_CONCURRENCY=10000` (unbounded) reproduces the original starvation (p99 > 5s).
...
- **Interaction graph:** the new `WarmupRegistry` (Unit 1) and `WebSocketSessionRegistry` (Unit 3) are accessed from `CassandraProxy` during request handling and by `/debug/*` routes. Both are single-writer-per-event-loop; no locks needed beyond the `asyncio.Lock` in `MemoryHandler._ensure_initialized` (Unit 1).
...
- Update `CHANGELOG.md` under an "Unreleased" heading with: (a) compression preserved — no behavioral change for existing Codex users; (b) new stage timings visible in logs; (c) new `CASSANDRA_ANTHROPIC_PRE_UPSTREAM_CONCURRENCY` env var; (d) new `/debug/*` loopback endpoints.
...
- Related code:
  - `cassandra/proxy/handlers/openai.py` — `handle_openai_responses_ws`, `_client_to_upstream`, `_upstream_to_client`, `_ws_http_fallback`
  - `cassandra/proxy/handlers/anthropic.py` — `handle_anthropic_messages`
  - `cassandra/proxy/server.py` — `CassandraProxy.__init__`, `.startup`, health routes
  - `cassandra/proxy/memory_handler.py` — `MemoryHandler._ensure_initialized`
  - `cassandra/transforms/content_router.py` — `eager_load_compressors`
  - `cassandra/transforms/kompress_compressor.py` — `_load_kompress_onnx`, `_kompress_cache`
- Related PRs/issues:
  - Upstream: `https://github.com/Sarthak-47/cassandra/issues/172`
