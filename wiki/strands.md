...
Cassandra integrates with [Strands Agents](https://github.com/strands-agents/sdk-python) to provide automatic context optimization. Two integration patterns: wrap the model, or hook into tool calls.
...
```bash
pip install cassandra-ai strands-agents
```
...
```python
from cassandra.integrations.strands import CassandraStrandsModel, CassandraHookProvider

optimized = CassandraStrandsModel(wrapped_model=model)
hooks = CassandraHookProvider(compress_tool_outputs=True)
...
CassandraStrandsModel supports Strands' structured output feature:
...
Tool executes, returns result
    │
    ▼
CassandraHookProvider (optional)
...
The model wrapper uses Cassandra's full pipeline (CacheAligner → ContentRouter). The hook provider uses SmartCrusher directly for fast JSON compression of individual tool results.

---

## Supported Providers

CassandraStrandsModel auto-detects the provider from the wrapped model:
