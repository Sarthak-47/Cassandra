...
Cassandra's core promise: **compress context without losing accuracy**. This page shows accuracy benchmarks, compression performance, and real-world production telemetry from 250+ active proxy instances.
...
Tested on Apple M-series (CPU), cassandra v0.5.18. Each test runs `compress()` on realistic tool outputs.
...
Real-world data from **50,000+ proxy sessions** across 250+ unique instances (March 30 – April 2, 2026). Collected via anonymous telemetry beacon (opt-in: `CASSANDRA_TELEMETRY=on`; telemetry is off by default).
...
```bash
# Run it yourself
pip install "cassandra-ai[html]" datasets
...
### What Cassandra Does NOT Compress
...
### When Cassandra Adds the Most Value
...
### When Cassandra Adds Little Value
...
- Opt-in: `CASSANDRA_TELEMETRY=on` (telemetry is off by default)
...
```bash
# Clone the repo
git clone https://github.com/Sarthak-47/cassandra.git
cd cassandra
...
python -c "from cassandra import compress; print(compress([{'role':'user','content':'test'}]))"
