...
Cassandra provides seamless integration with LangChain, enabling automatic context optimization across all LangChain patterns: chat models, memory, retrievers, agents, and observability.

## Installation

```bash
pip install "cassandra-ai[langchain]"
```

This installs Cassandra with LangChain dependencies (`langchain-core`).
...
The `CassandraChatModel` wraps any LangChain `BaseChatModel`:
...
llm = CassandraChatModel(ChatAnthropic(model="claude-3-5-sonnet-20241022"))
...
`CassandraChatMessageHistory` wraps any chat history with automatic compression:

```python
from langchain.memory import ConversationBufferMemory
from langchain_community.chat_message_histories import ChatMessageHistory
from cassandra.integrations import CassandraChatMessageHistory

# Wrap any history
base_history = ChatMessageHistory()
compressed_history = CassandraChatMessageHistory(
    base_history,
...
**Why this matters**: Long conversations can blow up to 50K+ tokens. CassandraChatMessageHistory automatically compresses older turns while preserving recent context.
...
`CassandraDocumentCompressor` filters retrieved documents by relevance:

```python
from langchain.retrievers import ContextualCompressionRetriever
from langchain_community.vectorstores import FAISS
from cassandra.integrations import CassandraDocumentCompressor

# Create vector store retriever (retrieve many for recall)
vectorstore = FAISS.from_documents(documents, embeddings)
base_retriever = vectorstore.as_retriever(search_kwargs={"k": 50})

# Wrap with Cassandra compression (keep best for precision)
compressor = CassandraDocumentCompressor(
...
**Why this matters**: Vector search often returns many marginally-relevant documents. CassandraDocumentCompressor uses BM25-style scoring to keep only the most relevant ones, reducing context size while improving answer quality.

---

### 4. Agent Tool Wrapping

`wrap_tools_with_cassandra` compresses tool outputs for agents:

```python
from langchain.agents import create_openai_tools_agent, AgentExecutor
from langchain_core.tools import tool
from cassandra.integrations import wrap_tools_with_cassandra
...
wrapped_tools = wrap_tools_with_cassandra(
    tools,
...
```python
from cassandra.integrations import get_tool_metrics
...
```python
from cassandra.integrations import StreamingMetricsTracker
...
```python
from cassandra.integrations import StreamingMetricsCallback
...
Add Cassandra metrics to LangSmith traces:

```python
from cassandra.integrations import CassandraLangSmithCallbackHandler

# Create callback handler
langsmith_handler = CassandraLangSmithCallbackHandler()
...
# - cassandra.tokens_before
# - cassandra.tokens_after
# - cassandra.tokens_saved
...
from cassandra.integrations import CassandraChatModel, wrap_tools_with_cassandra
...
# Wrap model with Cassandra
llm = CassandraChatModel(ChatOpenAI(model="gpt-4o"))

# Wrap tools with compression
tools = wrap_tools_with_cassandra([search_web, query_database])
...
**Without Cassandra**: Each tool call adds 10-50K tokens to context.
**With Cassandra**: Tool outputs compressed to 1-2K tokens, agent runs faster and cheaper.
...
from cassandra.integrations.langchain import create_compress_tool_messages_node
...
```python
from cassandra.integrations.langchain import compress_tool_messages
...
from langchain.retrievers import ContextualCompressionRetriever
from cassandra.integrations import CassandraChatModel, CassandraDocumentCompressor
...
# Cassandra compressor for precision
compressor = CassandraDocumentCompressor(
...
- With Cassandra: 5 docs × ~500 tokens = 2.5K context tokens (90% reduction)
...
from cassandra.integrations import CassandraChatModel, CassandraChatMessageHistory
...
**Impact**: Without compression, 100-turn conversation = 100K+ tokens. With CassandraChatMessageHistory, it stays under 8K tokens while preserving recent context.
...
# Wrap everything
llm = CassandraChatModel(ChatOpenAI(model="gpt-4o"))
tools = wrap_tools_with_cassandra([search_arxiv, search_github, fetch_documentation])
...
CassandraChatModel(
    ...
    cassandra_config=CassandraConfig(),  # Cassandra configuration
...
from cassandra.integrations.langchain.memory import CassandraChatMessageHistory
```
...
```python
from cassandra.integrations import langchain_available

if not langchain_available():
    print("Install with: pip install cassandra-ai[langchain]")
```
