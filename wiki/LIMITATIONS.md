# Cassandra Limitations & Known Behavior

Honest documentation of when Cassandra helps, when it doesn't, and what to watch out for.

## When Cassandra Helps (and When It Doesn't)
...
Cassandra includes an AST-aware CodeCompressor (tree-sitter, 8 languages) but it's gated behind safety protections that prevent it from firing in most real-world scenarios. This is intentional.
...
**Where code savings come from**: Cassandra does not strip function bodies from active code or drop old code messages. Code savings come from compressing the newest content blocks (live-zone-only compression) when they are not protected, leaving the conversation history intact.

**Override**: Set `protect_analysis_context=False` in `ContentRouterConfig` for aggressive code compression. Requires `cassandra-ai[code]` for tree-sitter.
...
- **Requires**: `cassandra-ai[ml]` — downloads model weights and needs GPU/CPU RAM for inference
...
> The earlier LLMLingua-2 integration (`cassandra-ai[llmlingua]`) was retired and is no longer installable.
