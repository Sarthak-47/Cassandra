#!/usr/bin/env python3
"""Synchronize version across all cassandra packages."""
...
def update_plugin_versions(root: Path, version: str) -> None:
    ...
    update_plugin_manifest(
        root / "plugins" / "cassandra-agent-hooks" / ".claude-plugin" / "plugin.json", version
    )
    update_plugin_manifest(
        root / "plugins" / "cassandra-agent-hooks" / ".github" / "plugin" / "plugin.json",
        version,
    )


def update_openclaw_package_json(file_path: Path, version: str, sdk_version: str) -> None:
    """Update openclaw package.json version and cassandra-ai dependency range."""
    with open(file_path, encoding="utf-8") as f:
        data = json.load(f)
    data["version"] = version
    if "dependencies" in data and "cassandra-ai" in data["dependencies"]:
        data["dependencies"]["cassandra-ai"] = f"^{sdk_version}"
    with open(file_path, "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2)
        f.write("\n")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Synchronize version across cassandra packages")
    parser.add_argument(
        "--root",
        type=Path,
