...
def test_main_is_noop_on_feature_branch(monkeypatch, capsys) -> None:
    ...
    False (feature branch, no CASSANDRA_SYNC_VERSIONS opt-in), main()
...
def test_should_sync_main_branch_runs(monkeypatch) -> None:
    ...
    module = _load_module()
    monkeypatch.delenv("CASSANDRA_SYNC_VERSIONS", raising=False)
