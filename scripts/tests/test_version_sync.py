...
def temp_project(tmp_path: Path) -> dict[str, Path]:
    ...
    agent_hooks_claude = plugins / "cassandra-agent-hooks" / ".claude-plugin"
    agent_hooks_claude.mkdir(parents=True)
    agent_hooks_github = plugins / "cassandra-agent-hooks" / ".github" / "plugin"
    ...
    # cassandra/_version.py is runtime-derived and must not be rewritten by version-sync.
    version_py = cassandra / "_version.py"
        ...
        json.dumps(
            {
                ...
                "plugins": [{"name": "cassandra-agent-hooks", "version": "0.1.0"}],
            }
        )
    )
    ...
    claude_plugin.write_text(json.dumps({"name": "cassandra-agent-hooks", "version": "0.1.0"}))

    github_plugin = agent_hooks_github / "plugin.json"
    github_plugin.write_text(json.dumps({"name": "cassandra-agent-hooks", "version": "0.1.0"}))
...
def test_version_sync_explicit_version(temp_project: dict[str, Path]) -> None:
    ...
    # Verify cassandra/_version.py is not a synced manifest.
