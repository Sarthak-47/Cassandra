...
Sync now only runs when EITHER:
  ...
  * ``CASSANDRA_SYNC_VERSIONS=1`` is set explicitly (release workflow)
...
try:
    from cassandra.release_version import (  # noqa: E402
    ...
    print("skip: cassandra deps not installed (run from a dev venv to enable)")
    sys.exit(0)
...
def _should_sync(root: Path) -> bool:
    ...
    Release workflow opts in via ``CASSANDRA_SYNC_VERSIONS=1``; otherwise
    ...
    """
    if os.environ.get("CASSANDRA_SYNC_VERSIONS") == "1":
        return True
...
def main() -> None:
    ...
    if not _should_sync(root):
        ...
        print(
            f"sync-plugin-versions: skipping on branch '{branch}' (set CASSANDRA_SYNC_VERSIONS=1 to force)"
        )
        return
