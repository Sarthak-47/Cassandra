...
Utility scripts bundled with the Cassandra repo. Most are one-off operator
...
against a local Cassandra proxy (default `http://127.0.0.1:8787`). Use it to:
...
- Empirically tune the Unit 4 pre-upstream semaphore default
  (`CASSANDRA_ANTHROPIC_PRE_UPSTREAM_CONCURRENCY`).
...
- `/livez p99` under threshold means the event loop is not starved during the
  ...
  (`CASSANDRA_ANTHROPIC_PRE_UPSTREAM_CONCURRENCY=10000`) and drops back under
