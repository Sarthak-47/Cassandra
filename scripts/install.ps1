...
$ImageDefault = 'ghcr.io/Sarthak-47/cassandra:latest'
$InstallImage = if ($env:CASSANDRA_DOCKER_IMAGE) { $env:CASSANDRA_DOCKER_IMAGE } else { $ImageDefault }
...
function Ensure-ProfileBlock {
    ...
    $markerStart = '# >>> cassandra docker-native >>>'
    $markerEnd = '# <<< cassandra docker-native <<<'
...
function Write-Wrapper {
    ...
    $wrapperPath = Join-Path $TargetDir 'cassandra.ps1'
    $cmdPath = Join-Path $TargetDir 'cassandra.cmd'
    $resolvedInstallImage = $InstallImage.Replace("'", "''")

    $wrapper = @'
$ErrorActionPreference = 'Stop'

$CassandraImage = if ($env:CASSANDRA_DOCKER_IMAGE) { $env:CASSANDRA_DOCKER_IMAGE } else { '__CASSANDRA_INSTALL_IMAGE__' }
$ContainerHome = if ($env:CASSANDRA_CONTAINER_HOME) { $env:CASSANDRA_CONTAINER_HOME } else { '/tmp/cassandra-home' }
$HostHome = $HOME
...
function Get-PassthroughEnvArgs {
    ...
    $prefixes = @(
        'CASSANDRA_','ANTHROPIC_','OPENAI_','GEMINI_','AWS_','AZURE_','VERTEX_',
...
function Get-SharedDockerArgs {
    ...
    $args.Add('--env')
    $args.Add("CASSANDRA_WORKSPACE_DIR=$ContainerHome/.cassandra")
    $args.Add('--env')
    $args.Add("CASSANDRA_CONFIG_DIR=$ContainerHome/.cassandra/config")
    $args.Add('--volume')
    $args.Add("${PWD}:/workspace")
    $args.Add('--volume')
    $args.Add((Join-Path $HostHome '.cassandra') + ":$ContainerHome/.cassandra")
    $args.Add('--volume')
...
function Invoke-CassandraDocker {
    ...
    $dockerArgs.Add('cassandra')
    $dockerArgs.Add($CassandraImage)
...
function Wait-Proxy {
    ...
    throw "Cassandra proxy failed to start on port $Port"
}
...
function Start-ProxyContainer {
    ...
    $containerName = "cassandra-proxy-$Port-$PID"
    $dockerArgs = New-Object System.Collections.Generic.List[string]
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to start Cassandra proxy container"
    }
...
function Get-PersistentProfileRoot {
    ...
    return Join-Path (Join-Path $HostHome '.cassandra\deploy') $Profile
}
...
function Get-ManifestProxyArgs {
    ...
    if ($Memory) {
        $args.AddRange([string[]]@('--memory','--memory-db-path',"$ContainerHome/.cassandra/memory.db"))
    }
    if ($AnyllmProvider) {
...
function Write-PersistentManifest {
    ...
    $manifest = [ordered]@{
        ...
        proxy_mode = $Mode
        memory_enabled = $Memory
        memory_db_path = "$ContainerHome/.cassandra/memory.db"
        telemetry_enabled = $TelemetryEnabled
        image = $Image
        service_name = "cassandra-$Profile"
        container_name = Get-PersistentContainerName -Profile $Profile
        health_url = "http://127.0.0.1:$Port/readyz"
        base_env = $baseEnv
        tool_envs = @{}
...
function Parse-InstallApplyArgs {
    ...
    while ($i -lt $Arguments.Count) {
        ...
        switch -Regex ($arg) {
            ...
            default {
                Fail "Unsupported option for 'cassandra install apply': $arg"
            }
        }
    }
...
function Parse-InstallProfileArgs {
    ...
    while ($i -lt $Arguments.Count) {
        ...
        switch -Regex ($arg) {
            ...
            default {
                Fail "Unsupported option for 'cassandra install': $arg"
            }
        }
    }
...
function Invoke-ClaudeRtkInit {
    $rtkPath = Join-Path $HostHome '.cassandra\bin\rtk.exe'
...
function Get-ContextTool {
    $value = $env:CASSANDRA_CONTEXT_TOOL
    ...
    if ($value -ne 'rtk' -and $value -ne 'lean-ctx') {
        Fail 'CASSANDRA_CONTEXT_TOOL must be one of: lean-ctx, rtk'
    }
    return $value
}
...
function Parse-OpenClawWrapArgs {
    ...
    $pluginPath = $null
    $pluginSpec = 'cassandra-ai/openclaw'
    $skipBuild = $false
    $copy = $false
    $proxyPort = 8787
    ...
    while ($i -lt $Arguments.Count) {
        ...
        switch -Regex ($arg) {
            ...
            default {
                Fail "Unsupported option for 'cassandra wrap openclaw': $arg"
            }
        }
    }
...
function Parse-OpenClawUnwrapArgs {
    ...
    while ($i -lt $Arguments.Count) {
        ...
        switch -Regex ($arg) {
            ...
            default {
                Fail "Unsupported option for 'cassandra unwrap openclaw': $arg"
            }
        }
    }
...
function Get-OpenClawExistingEntryJson {
    $output = (& openclaw config get plugins.entries.cassandra 2>$null | Out-String).Trim()
    if ($LASTEXITCODE -ne 0) {
        return $null
    }
    return $output
...
function Copy-OpenClawPluginIntoExtensions {
    ...
    $targetDir = Join-Path $extensionsDir 'cassandra'
...
function Invoke-OpenClawWrap {
    ...
    Write-Host ""
    Write-Host "  ╔═══════════════════════════════════════════════╗"
    Write-Host "  ║           CASSANDRA WRAP: OPENCLAW             ║"
    Write-Host "  ╚═══════════════════════════════════════════════╝"
    Write-Host ""
    if ($parsed.PluginPath) {
    ...
    [void](Invoke-CapturedCommand -Action 'openclaw config set plugins.entries.cassandra' -Command 'openclaw' -Arguments @('config','set','plugins.entries.cassandra',$entryJson,'--strict-json'))
    Write-Host '  Installing OpenClaw plugin with required unsafe-install flag...'
    Install-OpenClawPlugin -Parsed $parsed
    [void](Invoke-CapturedCommand -Action 'openclaw config set plugins.slots.contextEngine' -Command 'openclaw' -Arguments @('config','set','plugins.slots.contextEngine','"cassandra"','--strict-json'))
    ...
    $inspectOutput = Invoke-CapturedCommand -Action 'openclaw plugins inspect cassandra' -Command 'openclaw' -Arguments @('plugins','inspect','cassandra')
    if ($parsed.Verbose -and $inspectOutput) {
        Write-Host $inspectOutput
    }

    Write-Host ""
    Write-Host "✓ OpenClaw is configured to use Cassandra context compression."
    Write-Host "  Plugin: cassandra"
    Write-Host "  Slot:   plugins.slots.contextEngine = cassandra"
    Write-Host ""
}
...
function Invoke-OpenClawUnwrap {
    ...
    Write-Host ""
    Write-Host "  ╔═══════════════════════════════════════════════╗"
    Write-Host "  ║          CASSANDRA UNWRAP: OPENCLAW            ║"
    Write-Host "  ╚═══════════════════════════════════════════════╝"
    Write-Host ""
    Write-Host '  Disabling Cassandra plugin and removing engine mapping...'
    ...
    Write-Host ""
    Write-Host "✓ OpenClaw Cassandra wrap removed."
    Write-Host "  Plugin: cassandra (installed, disabled)"
...
function Invoke-PrepareOnly {
    ...
    $dockerArgs.AddRange((Get-SharedDockerArgs))
    $dockerArgs.Add('--env')
    $dockerArgs.Add("CASSANDRA_RTK_TARGET=$(Get-RtkTarget)")
    $dockerArgs.Add('--entrypoint')
    $dockerArgs.Add('cassandra')
    $dockerArgs.Add($CassandraImage)
...
if ($args.Count -eq 0) {
    Invoke-CassandraDocker -Arguments @('--help')
    exit 0
}
...
switch ($args[0]) {
    ...
    'wrap' {
        ...
        if ($args.Count -lt 2) {
            Fail 'Usage: cassandra wrap <claude|codex|aider|cursor|openclaw|opencode> [...]'
        }
        ...
        if ($tool -eq 'openclaw') {
            if (Test-HelpFlag -Arguments $wrapArgs) {
                ...
                Invoke-CassandraDocker -Arguments $helpArgs
                exit 0
            }
        ...
        try {
            ...
            switch ($tool) {
                ...
                'cursor' {
                    Write-Host "Cassandra proxy is running for Cursor."
                    Write-Host ""
    ...
    'unwrap' {
        if ($args.Count -eq 1 -or $args[1] -eq '--help' -or $args[1] -eq '-?') {
            Invoke-CassandraDocker -Arguments @('unwrap','--help')
            exit 0
        }
        ...
        Invoke-CassandraDocker -Arguments $args
    }
    'proxy' {
        $port = 8787
...
'@
    ...
    $wrapper = $wrapper.Replace('__CASSANDRA_INSTALL_IMAGE__', $resolvedInstallImage)

    $cmdWrapper = ([string][char]64) + "echo off`r`npowershell -NoLogo -NoProfile -ExecutionPolicy Bypass -File ""%~dp0cassandra.ps1"" %*`r`n"
...
if ($env:CASSANDRA_DOCKER_IMAGE) {
    $null = docker image inspect $InstallImage 2>$null
    if ($LASTEXITCODE -eq 0) {
        Write-Info "Using existing CASSANDRA_DOCKER_IMAGE=$InstallImage"
    } else {
...
Write-Host ""
Write-Host "Cassandra Docker-native install complete."
Write-Host ""
Write-Host "Installed wrappers:"
Write-Host "  $InstallDir\cassandra.ps1"
Write-Host "  $InstallDir\cassandra.cmd"
Write-Host ""
Write-Host "Next steps:"
Write-Host "  1. Restart PowerShell"
Write-Host "  2. Try: cassandra proxy"
Write-Host "  3. Docs: https://github.com/Sarthak-47/cassandra/blob/main/docs/docker-install.md"
