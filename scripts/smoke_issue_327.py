...
(NO proxy in the loop) and against the local Cassandra proxy, and asserts:
  ...
  2. When proxied through Cassandra in token mode, at least one post-warmup
...
USAGE
    ...
    RUN_LIVE_API=1 CASSANDRA_PROXY_URL=http://localhost:8787 \
...
PROXY_URL = os.environ.get("CASSANDRA_PROXY_URL")  # optional
TOOLS = [
    {
        "name": "get_lines",
