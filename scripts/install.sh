...
IMAGE_DEFAULT="ghcr.io/Sarthak-47/cassandra:latest"
INSTALL_IMAGE="${CASSANDRA_DOCKER_IMAGE:-${IMAGE_DEFAULT}}"
...
if ((BASH_VERSINFO[0] < 4 || (BASH_VERSINFO[0] == 4 && BASH_VERSINFO[1] < 3))); then
  printf 'ERROR: Cassandra Docker-native install requires bash >= 4.3\n' >&2
  exit 1
fi
...
append_path_block() {
  local target_file="$1"
  local marker_start="# >>> cassandra docker-native >>>"
  local marker_end="# <<< cassandra docker-native <<<"
...
write_wrapper() {
  local wrapper_path="${INSTALL_DIR}/cassandra"

  {
    printf '#!%s\n\n' "${BASH_PATH}"
    printf 'CASSANDRA_IMAGE_DEFAULT=%q\n' "${INSTALL_IMAGE}"
    cat <<'WRAPPER'

set -euo pipefail

CASSANDRA_IMAGE="${CASSANDRA_DOCKER_IMAGE:-${CASSANDRA_IMAGE_DEFAULT}}"
CASSANDRA_CONTAINER_HOME="${CASSANDRA_CONTAINER_HOME:-/tmp/cassandra-home}"
CASSANDRA_HOST_HOME="${HOME:?}"

if ((BASH_VERSINFO[0] < 4 || (BASH_VERSINFO[0] == 4 && BASH_VERSINFO[1] < 3))); then
  printf 'ERROR: Cassandra Docker-native wrapper requires bash >= 4.3\n' >&2
  exit 1
fi
...
ensure_host_dirs() {
  mkdir -p \
    "${CASSANDRA_HOST_HOME}/.cassandra" \
    "${CASSANDRA_HOST_HOME}/.claude" \
    "${CASSANDRA_HOST_HOME}/.codex" \
    "${CASSANDRA_HOST_HOME}/.gemini"
}
...
append_passthrough_envs() {
  ...
  for name in $(compgen -e); do
    case "${name}" in
      CASSANDRA_*|ANTHROPIC_*|OPENAI_*|GEMINI_*|AWS_*|AZURE_*|VERTEX_*|GOOGLE_*|GOOGLE_CLOUD_*|MISTRAL_*|GROQ_*|OPENROUTER_*|XAI_*|TOGETHER_*|COHERE_*|OLLAMA_*|LITELLM_*|OTEL_*|SUPABASE_*|QDRANT_*|NEO4J_*|LANGSMITH_*)
        ref+=(--env "${name}")
        ;;
    esac
  done
...
append_common_container_args() {
  ...
  ensure_host_dirs
  ref+=(-w /workspace)
  ref+=(--env "HOME=${CASSANDRA_CONTAINER_HOME}")
  ref+=(--env "PYTHONUNBUFFERED=1")
  # Canonical Cassandra filesystem contract (issue #175) — forward into the
  # container so the proxy resolves state/config to the bind-mounted path.
  ref+=(--env "CASSANDRA_WORKSPACE_DIR=${CASSANDRA_CONTAINER_HOME}/.cassandra")
  ref+=(--env "CASSANDRA_CONFIG_DIR=${CASSANDRA_CONTAINER_HOME}/.cassandra/config")
  ref+=(-v "${PWD}:/workspace")
  ref+=(-v "${CASSANDRA_HOST_HOME}/.cassandra:${CASSANDRA_CONTAINER_HOME}/.cassandra")
  ref+=(-v "${CASSANDRA_HOST_HOME}/.claude:${CASSANDRA_CONTAINER_HOME}/.claude")
  ref+=(-v "${CASSANDRA_HOST_HOME}/.codex:${CASSANDRA_CONTAINER_HOME}/.codex")
  ref+=(-v "${CASSANDRA_HOST_HOME}/.gemini:${CASSANDRA_CONTAINER_HOME}/.gemini")
...
run_cassandra() {
  ...
  args+=(--entrypoint cassandra "${CASSANDRA_IMAGE}" "$@")
  "${args[@]}"
}
...
start_proxy_container() {
  ...
  local container_name="cassandra-proxy-${port}-$$"
  local args=()
  args=(docker run -d --rm --name "${container_name}" -p "${port}:${port}")
  append_common_container_args args
  args+=("${CASSANDRA_IMAGE}" --host 0.0.0.0 --port "${port}" "$@")
  "${args[@]}" >/dev/null

  if ! wait_for_proxy "${container_name}" "${port}"; then
    docker stop "${container_name}" >/dev/null 2>&1 || true
    die "Cassandra proxy failed to start on port ${port}"
  fi
...
persistent_profile_root() {
  ...
  printf '%s/.cassandra/deploy/%s\n' "${CASSANDRA_HOST_HOME}" "${profile}"
}
...
append_persistent_container_args() {
  ...
  ensure_host_dirs
  ref+=(--workdir "${CASSANDRA_CONTAINER_HOME}")
  ref+=(--env "HOME=${CASSANDRA_CONTAINER_HOME}")
  ref+=(--env "PYTHONUNBUFFERED=1")
  # Canonical Cassandra filesystem contract (issue #175).
  ref+=(--env "CASSANDRA_WORKSPACE_DIR=${CASSANDRA_CONTAINER_HOME}/.cassandra")
  ref+=(--env "CASSANDRA_CONFIG_DIR=${CASSANDRA_CONTAINER_HOME}/.cassandra/config")
  ref+=(-v "${CASSANDRA_HOST_HOME}/.cassandra:${CASSANDRA_CONTAINER_HOME}/.cassandra")
  ref+=(-v "${CASSANDRA_HOST_HOME}/.claude:${CASSANDRA_CONTAINER_HOME}/.claude")
...
build_manifest_proxy_args() {
  ...
  if [[ "${memory_enabled}" -eq 1 ]]; then
    out_args+=(--memory --memory-db-path "${CASSANDRA_CONTAINER_HOME}/.cassandra/memory.db")
  fi
  if [[ -n "${anyllm}" ]]; then
  ...
  cat >"${manifest_path}" <<EOF
{
  ...
  "memory_db_path": "$(json_escape "${CASSANDRA_CONTAINER_HOME}/.cassandra/memory.db")",
  "telemetry_enabled": ${telemetry_json},
  "image": "$(json_escape "${image}")",
  "service_name": "cassandra-$(json_escape "${profile}")",
  "container_name": "$(json_escape "$(persistent_container_name "${profile}")")",
  "health_url": "http://127.0.0.1:${port}/readyz",
  "base_env": {
    "CASSANDRA_PORT": "${port}",
    "CASSANDRA_HOST": "127.0.0.1",
    "CASSANDRA_MODE": "$(json_escape "${proxy_mode}")",
    "CASSANDRA_BACKEND": "$(json_escape "${backend}")"
  },
  "tool_envs": {},
...
start_persistent_docker_install() {
  ...
  args+=(
    --env "CASSANDRA_DEPLOYMENT_PROFILE=${profile}"
    --env "CASSANDRA_DEPLOYMENT_PRESET=persistent-docker"
    --env "CASSANDRA_DEPLOYMENT_RUNTIME=docker"
    --env "CASSANDRA_DEPLOYMENT_SUPERVISOR=none"
    --env "CASSANDRA_DEPLOYMENT_SCOPE=user"
  )
  ...
  if ! wait_for_proxy "${container_name}" "${port}"; then
    docker rm -f "${container_name}" >/dev/null 2>&1 || true
    die "Cassandra persistent Docker deployment failed to start on port ${port}"
  fi
...
Usage: cassandra install [OPTIONS] COMMAND [ARGS]...
  ...
  Manage persistent Docker-native Cassandra deployments.

  The Docker-native wrapper currently supports the persistent-docker preset only.
  Use the Python-native `cassandra install` command for persistent-service and
...
Options:
  ...
  --image TEXT                  Docker image to use.  [default: CASSANDRA_DOCKER_IMAGE or ghcr.io/Sarthak-47/cassandra:latest]
  -?, --help                    Show this message and exit.
EOF
}
...
print_wrap_help() {
  cat <<'EOF'
Usage: cassandra wrap <COMMAND> [OPTIONS] [-- ARGS...]

  Launch supported host tools through a Docker-native Cassandra proxy.
  ...
  while (($#)); do
    case "$1" in
      ...
      --help|-?)
        ...
        ;;
      *)
        die "Unsupported option for 'cassandra install apply': $1"
        ;;
    esac
  done
  ...
  while (($#)); do
    case "$1" in
      ...
      --help|-?)
        ...
        ;;
      *)
        die "Unsupported option for 'cassandra install': $1"
        ;;
    esac
  done
}
...
run_claude_rtk_init() {
  local rtk_bin="${CASSANDRA_HOST_HOME}/.cassandra/bin/rtk"
  if [[ ! -x "${rtk_bin}" ]]; then
...
selected_context_tool() {
  local value="${CASSANDRA_CONTEXT_TOOL:-rtk}"
  value="${value,,}"
  value="${value//_/-}"
  if [[ -z "${value}" ]]; then
    value="rtk"
  ...
  case "${value}" in
    rtk|lean-ctx)
      ...
      ;;
    *)
      die "CASSANDRA_CONTEXT_TOOL must be one of: lean-ctx, rtk"
      ;;
  esac
}
...
run_prepare_only() {
  ...
  append_tty_args args
  append_common_container_args args
  args+=(--env "CASSANDRA_RTK_TARGET=$(detect_rtk_target)")
  args+=(--entrypoint cassandra "${CASSANDRA_IMAGE}" wrap "${tool}" --prepare-only "$@")
  "${args[@]}"
}
...
parse_openclaw_wrap_args() {
  ...
  out_plugin_path=""
  out_plugin_spec="cassandra-ai/openclaw"
  out_skip_build=0
  out_copy=0
  out_proxy_port=8787
  ...
  while (($#)); do
    case "$1" in
      ...
      --verbose|-v)
        ...
        ;;
      *)
        die "Unsupported option for 'cassandra wrap openclaw': $1"
        ;;
    esac
  done
}
  ...
  while (($#)); do
    case "$1" in
      ...
      --verbose|-v)
        ...
        ;;
      *)
        die "Unsupported option for 'cassandra unwrap openclaw': $1"
        ;;
    esac
  done
}
...
get_openclaw_existing_entry_json() {
  local output=""
  if output="$(openclaw config get plugins.entries.cassandra 2>/dev/null)"; then
    printf '%s' "${output}"
  fi
}
...
prepare_openclaw_entry_json() {
  ...
  args=(docker run --rm)
  append_common_container_args args
  args+=(--entrypoint cassandra "${CASSANDRA_IMAGE}" wrap openclaw --prepare-only)
...
prepare_openclaw_unwrap_entry_json() {
  ...
  args=(docker run --rm)
  append_common_container_args args
  args+=(--entrypoint cassandra "${CASSANDRA_IMAGE}" unwrap openclaw --prepare-only)
  if [[ -n "${existing_entry_json}" ]]; then
    args+=(--existing-entry-json "${existing_entry_json}")
  fi
  "${args[@]}"
...
copy_openclaw_plugin_into_extensions() {
  ...
  local target_dir="${extensions_dir}/cassandra"
  mkdir -p "${target_dir}"
...
wrap_openclaw_host() {
  ...
  printf '\n  ╔═══════════════════════════════════════════════╗\n'
  printf '  ║           CASSANDRA WRAP: OPENCLAW             ║\n'
  printf '  ╚═══════════════════════════════════════════════╝\n\n'
  if [[ -n "${plugin_path}" ]]; then
  ...
  run_openclaw_checked \
    "openclaw config set plugins.entries.cassandra" \
    openclaw config set plugins.entries.cassandra "${entry_json}" --strict-json >/dev/null
  ...
  run_openclaw_checked \
    ...
    openclaw config set plugins.slots.contextEngine '"cassandra"' --strict-json >/dev/null
  ...
  inspect_output="$(run_openclaw_checked "openclaw plugins inspect cassandra" openclaw plugins inspect cassandra)"
  if [[ "${verbose}" -eq 1 && -n "${inspect_output}" ]]; then
    printf '%s\n' "${inspect_output}"
  fi

  printf '\n✓ OpenClaw is configured to use Cassandra context compression.\n'
  printf '  Plugin: cassandra\n'
  printf '  Slot:   plugins.slots.contextEngine = cassandra\n\n'
}
...
unwrap_openclaw_host() {
  ...
  printf '\n  ╔═══════════════════════════════════════════════╗\n'
  printf '  ║          CASSANDRA UNWRAP: OPENCLAW            ║\n'
  printf '  ╚═══════════════════════════════════════════════╝\n\n'
  printf '  Disabling Cassandra plugin and removing engine mapping...\n'
  ...
  printf '\n✓ OpenClaw Cassandra wrap removed.\n'
  printf '  Plugin: cassandra (installed, disabled)\n'
...
main() {
  ...
  case "$1" in
    ...
    wrap)
      ...
      (($# >= 2)) || die "Usage: cassandra wrap <claude|codex|aider|cursor|openclaw|opencode> [...]"
      local tool="$2"
      shift 2
      ...
      case "${tool}" in
        ...
        cursor)
          cat <<EOF
Cassandra proxy is running for Cursor.
...
EOF
    ...
    proxy)
      ...
      run_args+=(--entrypoint cassandra "${CASSANDRA_IMAGE}" "${args[@]}")
      "${run_args[@]}"
      ;;
    *)
      run_cassandra "$@"
...
main() {
  ...
  if [[ -n "${CASSANDRA_DOCKER_IMAGE:-}" ]]; then
    if docker image inspect "${INSTALL_IMAGE}" >/dev/null 2>&1; then
      info "Using existing CASSANDRA_DOCKER_IMAGE=${INSTALL_IMAGE}"
    else
...
Cassandra Docker-native install complete.

Installed wrapper:
  ${INSTALL_DIR}/cassandra

Next steps:
  1. Restart your shell or run: export PATH="${INSTALL_DIR}:\$PATH"
  2. Try: cassandra proxy
  3. Docs: https://github.com/Sarthak-47/cassandra/blob/main/docs/docker-install.md
EOF
}
