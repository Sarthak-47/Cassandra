...
+ ``httpx`` only, all already available in the Cassandra dev environment.
"""
...
def format_summary(result: dict[str, Any]) -> str:
    if result.get("reason") == "proxy_unreachable":
        return (
            ...
            "  {detail}\n"
            "  Hint: start the proxy with `cassandra proxy` and retry."
        ).format(**result)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        "storm against a local Cassandra proxy.",
