#!/usr/bin/env bash
# Build + install the Rust extension (cassandra._core) into the active venv.
#
...
# in-tree `cassandra/` directory because the dual-package layout left the
# .so in `crates/cassandra-py/python/cassandra/`. That dance is no longer
...
# wheel contains both the Python source and the compiled `cassandra/_core.so`,
...
log "verifying \`from cassandra._core import DiffCompressor, SmartCrusher\`"
python -c '
import sys
try:
    from cassandra._core import DiffCompressor, SmartCrusher
...
log "cassandra._core build + install + verify: OK"
