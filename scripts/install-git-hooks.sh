#!/usr/bin/env bash
# Install git hooks for the Cassandra repo:
...
if ! command -v npx &>/dev/null; then
    echo "error: npx not found — install Node 18+ before installing Cassandra's git hooks." >&2
    exit 1
fi
...
#!/usr/bin/env bash
# Cassandra pre-push hook — runs `make ci-precheck` so CI never finds a
