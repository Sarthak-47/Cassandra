...
`ImportError: undefined symbol` at `import cassandra._core`.
...
-----
    ...
    scripts/audit_wheel_glibc_symbols.py path/to/cassandra_ai-*.whl
...
def parse_manylinux_floor(wheel_filename: str) -> tuple[int, int] | None:
    ...
    `cassandra_ai-0.20.26-cp312-cp312-manylinux_2_28_x86_64.whl` → (2, 28).
...
def main() -> int:
    ...
    print(
        ...
        "and `crates/cassandra-py/glibc_compat.c` for the shim pattern."
    )
    return 1
