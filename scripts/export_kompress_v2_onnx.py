#!/usr/bin/env python
"""Export a Kompress PyTorch checkpoint to ONNX INT8 for Cassandra's light path.

Why this exists
---------------
Cassandra's ``[proxy]`` extra ships ``onnxruntime`` but **not** torch — the
proxy runs Kompress text compression on ONNX Runtime alone. The loader
(``cassandra/transforms/kompress_compressor.py``) downloads
...
``Sarthak-47/kompress-v2-base`` ships only PyTorch weights
(``model.safetensors`` / ``merged.pt``) — no ONNX. So pointing Cassandra at v2
...
Requires
--------
    pip install cassandra-ai[ml] onnxruntime    # torch + transformers + onnxruntime

Usage
-----
    # Convert + verify locally (writes onnx/kompress-int8.onnx):
    python scripts/export_kompress_v2_onnx.py --model-id Sarthak-47/kompress-v2-base

    # Convert, verify, and upload back to the HF repo (needs `huggingface-cli login`):
    python scripts/export_kompress_v2_onnx.py --model-id Sarthak-47/kompress-v2-base --upload
"""
...
DEFAULT_MODEL_ID = "Sarthak-47/kompress-v2-base"


def _build_core(model_id: str):
    """Instantiate CassandraCompressorModel and load the merged v2 weights.

    The v2 repo's ``model.safetensors`` is the *unmerged* PEFT structure
    (``encoder.base_model.model...`` with separate ``base_layer`` + LoRA
    adapters), which does not map onto ``CassandraCompressorModel``. The
    ...
    from cassandra.transforms.kompress_compressor import _get_model_class
...
def upload(model_id: str, out_path: Path) -> None:
    ...
    upload_file(
        ...
        repo_id=model_id,
        commit_message="Add ONNX export for Cassandra lightweight (no-torch) path",
    )
    logger.info("Uploaded. Cassandra's ONNX loader will now find it on next cold start.")
