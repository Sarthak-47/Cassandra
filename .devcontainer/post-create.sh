...
profile="${1:-default}"
project_env="${UV_PROJECT_ENVIRONMENT:-/home/vscode/.venvs/cassandra}"
project_env_root="$(dirname "$project_env")"
cache_root="${HOME}/.cache"
script_dir="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
workspace_root="$(cd "$script_dir/.." && pwd)"
git_profile_script="/etc/profile.d/cassandra-worktree-git.sh"
...
echo "Cassandra devcontainer is ready."
if [[ "$profile" == "memory-stack" ]]; then
...
echo "  uv run mypy cassandra --ignore-missing-imports"
