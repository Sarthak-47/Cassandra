...
from cassandra.cli import wrap as wrap_module
from cassandra.cli.main import main
from cassandra.compress import CompressConfig, compress
...
compress_module = import_module("cassandra.compress")
...
def test_agent_90_profile_exports_cross_agent_proxy_env() -> None:
    ...
    assert env["CASSANDRA_MODE"] == "token"
    assert env["CASSANDRA_SAVINGS_PROFILE"] == "agent-90"
    assert env["CASSANDRA_SAVINGS_TARGET"] == "0.90"
    assert env["CASSANDRA_TARGET_RATIO"] == "0.10"
    assert env["CASSANDRA_COMPRESS_USER_MESSAGES"] == "1"
    assert env["CASSANDRA_COMPRESS_SYSTEM_MESSAGES"] == "1"
    assert env["CASSANDRA_MAX_ITEMS"] == "8"
    assert env["CASSANDRA_SMART_CRUSHER_COMPACTION"] == "0"
    assert env["CASSANDRA_FORCE_KOMPRESS"] == "1"
    assert env["CASSANDRA_ACCURACY_GUARD"] == "strict"


def test_agent_savings_env_defaults_preserve_user_overrides() -> None:
    env = {
        "CASSANDRA_TARGET_RATIO": "0.25",
        "CASSANDRA_MAX_ITEMS": "12",
    }

    apply_agent_savings_env_defaults(env, AGENT_90_PROFILE)

    assert env["CASSANDRA_SAVINGS_PROFILE"] == "agent-90"
    assert env["CASSANDRA_TARGET_RATIO"] == "0.25"
...
def test_agent_savings_cli_renders_shell_exports() -> None:
    ...
    assert 'export CASSANDRA_SAVINGS_PROFILE="agent-90"' in result.output
    assert 'export CASSANDRA_SAVINGS_TARGET="0.90"' in result.output
    assert 'export CASSANDRA_ACCURACY_GUARD="strict"' in result.output
...
def test_agent_savings_cli_renders_json() -> None:
    ...
    assert result.exit_code == 0
    assert '"CASSANDRA_TARGET_RATIO": "0.10"' in result.output
...
def test_wrap_agent_savings_profile_is_opt_in(monkeypatch) -> None:
    monkeypatch.delenv("CASSANDRA_SAVINGS_PROFILE", raising=False)

    assert wrap_module._wrap_agent_savings_profile("codex") is None

    monkeypatch.setenv("CASSANDRA_SAVINGS_PROFILE", AGENT_90_PROFILE)
...
def test_start_proxy_does_not_inject_agent_savings_by_default(monkeypatch, tmp_path) -> None:
    ...
    assert "CASSANDRA_SAVINGS_PROFILE" not in captured_env
    assert "CASSANDRA_TARGET_RATIO" not in captured_env
...
def test_start_proxy_injects_explicit_agent_savings_profile(monkeypatch, tmp_path) -> None:
    ...
    assert captured_env["CASSANDRA_SAVINGS_PROFILE"] == AGENT_90_PROFILE
    assert captured_env["CASSANDRA_TARGET_RATIO"] == "0.10"
...
def test_router_lossless_only_flag_reaches_crusher() -> None:
    # CASSANDRA_LOSSLESS_ONLY=1 sets this field on the proxy router; it
...
def test_proxy_cli_reads_agent_90_profile_env() -> None:
    ...
    with pytest.MonkeyPatch.context() as mp:
        mp.setattr("cassandra.proxy.server.run_server", mock_run_server)
        result = runner.invoke(
            main,
            ["proxy"],
            env={"CASSANDRA_SAVINGS_PROFILE": "agent-90"},
...
def test_agent_savings_requires_each_agent_to_meet_target(monkeypatch) -> None:
    from cassandra.perf import analyzer
    from cassandra.perf.analyzer import PerfRecord, PerfReport
...
def test_agent_savings_smoke_fixture_passes_real_gate(tmp_path) -> None:
    ...
    gate_result = runner.invoke(
        ...
        env={"CASSANDRA_WORKSPACE_DIR": str(workspace)},
    )
