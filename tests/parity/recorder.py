...
This module is new (no modifications to existing Python under `cassandra/`).
...
LOG = logging.getLogger("cassandra.parity.recorder")
...
def record_all(root: Path | None = None) -> dict[str, str]:
    ...
    # --- log_compressor ----------------------------------------------------
    try:
        from cassandra.transforms.log_compressor import LogCompressor
    ...
    try:
        from cassandra.transforms.cache_aligner import CacheAligner
    ...
    try:
        from cassandra.ccr.tool_injection import CCRToolInjector
    ...
    try:
        from cassandra.transforms import content_detector as _cd_mod
