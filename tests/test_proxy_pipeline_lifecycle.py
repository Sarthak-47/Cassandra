...
def _assert_compressed_event_carries_originals(events: list) -> None:
    ...
    The probe recorder (cassandra.proxy.probe_recorder) depends on this
...
def test_proxy_shutdown_unloads_image_models() -> None:
    ...
    with (
        patch("cassandra.proxy.server.get_quota_registry", return_value=quota_registry),
        patch("cassandra.models.ml_models.MLModelRegistry.unload_prefix") as unload_prefix,
    ):
...
def test_openai_chat_pipeline_events_cover_proxy_lifecycle(monkeypatch) -> None:
    ...
    with TestClient(app) as client:
        ...
        response = client.post(
            "/v1/chat/completions",
            headers={"Authorization": "Bearer sk-test", "x-cassandra-user-id": "user-1"},
            json={
                "model": "gpt-5.4",
