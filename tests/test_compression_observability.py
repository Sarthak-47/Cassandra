...
from cassandra.transforms.content_detector import ContentType
...
from cassandra.transforms.observability import CompressionObserver
...
def isolated_toin(tmp_path, monkeypatch):
    ...
    `~/.cassandra/toin.json`, which persists across pytest invocations.
    ...
    from cassandra.telemetry.toin import TOIN_PATH_ENV_VAR, reset_toin
...
def test_prometheus_export_does_not_leak_per_strategy_metrics():
    ...
    assert "cassandra_compressions_total" not in output
    assert "cassandra_tokens_saved_by_strategy_total" not in output
