...
import cassandra.transforms.content_router as content_router_module
from cassandra.transforms.content_detector import ContentType, DetectionResult
...
def test_content_signature_and_detection_helpers(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    (`cassandra._core.detect_content_type` → magika → unidiff →
    ...
    import cassandra._core as _core
    ...
    monkeypatch.setenv("CASSANDRA_DETECT_BACKEND", "rust")
...
def test_detect_backend_env_python_forces_python_path(
    ...
    """CASSANDRA_DETECT_BACKEND=python forces the pure-Python regex path."""
    import cassandra._core as _core

    monkeypatch.setenv("CASSANDRA_DETECT_BACKEND", "python")
...
def test_detect_backend_env_override(monkeypatch: pytest.MonkeyPatch) -> None:
    """CASSANDRA_DETECT_BACKEND pins the detector on any platform."""
    resolve = content_router_module._resolve_detect_backend

    monkeypatch.setenv("CASSANDRA_DETECT_BACKEND", "python")
    assert resolve() == "python"

    monkeypatch.setenv("CASSANDRA_DETECT_BACKEND", "RUST")  # case-insensitive
    assert resolve() == "rust"

    # Unrecognized values fall back to the platform default.
    monkeypatch.setenv("CASSANDRA_DETECT_BACKEND", "bogus")
...
def test_detect_backend_defaults_to_python_on_windows(
    ...
    monkeypatch.delenv("CASSANDRA_DETECT_BACKEND", raising=False)
