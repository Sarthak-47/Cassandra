...
from cassandra.ccr.context_tracker import (
    CompressedContext,
    ContextTracker,
...
class TestExpansionFormatting:
    ...
    def test_format_full_expansion(self):
        ...
        assert formatted.startswith("<cassandra_proactive_expansion>\n")
    ...
    def test_format_expansion_xml_wrapper(self):
        ...
        assert result.startswith("<cassandra_proactive_expansion>\n")
    ...
    def test_proactive_expansion_identifiable_after_injection(self):
        ...
        from cassandra.ccr.context_tracker import ContextTracker
        ...
        # Cassandra-injected content is identifiable by XML tag, distinct from peer content
        assert "<cassandra_proactive_expansion>" in injected
        assert "</cassandra_proactive_expansion>" in injected
    ...
    def test_format_expansion_xml_close_tag_in_payload_escaped(self):
        ...
        expansions = [
            {
                "hash": "h1",
                "type": "full",
                "content": "return '</cassandra_proactive_expansion>'",
                "item_count": 1,
                "reason": "high relevance",
            }
        ]
        ...
        assert result.startswith("<cassandra_proactive_expansion>\n")
        assert result.endswith("\n</cassandra_proactive_expansion>")
        assert result.count("<cassandra_proactive_expansion>") == 1
        assert result.count("</cassandra_proactive_expansion>") == 1
