...
LIVE_CONFIG: dict[str, Any] = {
    ...
    # OpenAI — small + cheap.
    "openai_model": os.environ.get("CASSANDRA_LIVE_OPENAI_MODEL", "gpt-4o-mini"),
    # Gemini — flash variants.
    "gemini_model": os.environ.get("CASSANDRA_LIVE_GEMINI_MODEL", "gemini-2.0-flash"),
...
def _build_cache_prefix() -> str:
    ...
    """
    seed = (
        "Cassandra is a context-engineering layer for LLM applications. "
...
def test_anthropic_cache_stable_when_live_zone_compresses(
    ...
    # (Proxy emits x-cassandra-tokens-{before,after,saved} on every optimized
    # /v1/messages response; we assert direction, not magnitude.)
    tokens_before = resp2.headers.get("x-cassandra-tokens-before")
    tokens_after = resp2.headers.get("x-cassandra-tokens-after")
    assert tokens_before is not None, "proxy did not emit x-cassandra-tokens-before"
    assert tokens_after is not None, "proxy did not emit x-cassandra-tokens-after"
...
def test_anthropic_cache_control_passthrough_byte_faithful(
    ...
    proxy's own internal x-cassandra-* header strip and the Authorization
    rewrite).
...
def test_ccr_marker_round_trip_live() -> None:
    ...
    Verifies two halves of the B7 contract:
      ...
      (a) **Tool injection** — when ``ccr_inject_tool=True``, the proxy
          injects the ``cassandra_retrieve`` tool into the upstream-bound
    ...
    The combination is the "model calls cassandra_retrieve and gets original
    bytes back" story end-to-end.
    """
    from cassandra.cache.compression_store import (
    ...
    # that cassandra_retrieve was injected.
    ...
    with TestClient(ccr_app) as ccr_proxy_client:
        ...
        try:
            ...
            #     cassandra_retrieve with existing tools rather than only
            ...
            #     forces ``cassandra_retrieve`` into the tools array on
        ...
        assert "cassandra_retrieve" in tool_names, (
            f"cassandra_retrieve tool not injected; forwarded tools={tool_names}"
        )
        ...
        with TestClient(app) as memory_client:
            ...
            try:
                ...
                resp = _anthropic_call(
                    ...
                    extra_headers={
                        "x-cassandra-user-id": LIVE_CONFIG["memory_user_id"],
                    },
                )
            finally:
                proxy._retry_request = original_retry  # type: ignore[assignment]
...
def _classify_auth_mode_from_headers(headers: dict[str, str]) -> str:
    """Cassandra's load-bearing auth-mode classifier (Phase F-prep).
    ...
    ``cassandra/proxy/auth_mode.py``, this is the canonical ruleset:
