...
~/.cassandra/ccr_store.db by default) and shared across processes, so stale
            ...
            for n in range(0, 7):
                r = client.post(
                    ...
                    headers={
                        ...
                        "x-cassandra-session-id": "nobust-1",
...
def test_read_maturation_knobs_from_env(monkeypatch):
    ...
    from cassandra.proxy.server import _MULTI_WORKER_CONFIG_ENV, _proxy_config_from_env

    # _proxy_config_from_env short-circuits on a prebuilt multi-worker JSON
    # config and ignores the CASSANDRA_* vars entirely. Clear it so this test
    # actually exercises the env-var parsing path it claims to (and isn't
    # poisoned by a leaked CASSANDRA_PROXY_CONFIG_JSON from another test).
    monkeypatch.delenv(_MULTI_WORKER_CONFIG_ENV, raising=False)

    monkeypatch.setenv("CASSANDRA_READ_MATURATION", "1")
    monkeypatch.setenv("CASSANDRA_READ_MATURATION_QUIESCE_TURNS", "3")
    monkeypatch.setenv("CASSANDRA_READ_MATURATION_MAX_HOLD_TURNS", "10")
    monkeypatch.setenv("CASSANDRA_READ_MATURATION_MIN_SIZE_BYTES", "4096")
