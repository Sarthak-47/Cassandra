...
from cassandra.rtk import get_rtk_path, installer
...
def test_get_rtk_path_finds_windows_managed_binary(tmp_path: Path) -> None:
    ...
    with patch("cassandra.rtk.RTK_BIN_DIR", managed_dir):
        with patch("cassandra.rtk.RTK_BIN_PATH", managed_dir / "rtk"):
            with patch("cassandra.rtk.shutil.which", return_value=None):
                assert get_rtk_path() == managed_path


def test_get_target_triple_uses_override(monkeypatch) -> None:
    monkeypatch.setenv("CASSANDRA_RTK_TARGET", "x86_64-pc-windows-msvc")
...
def test_download_rtk_skips_verify_for_non_native_target(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv("CASSANDRA_RTK_TARGET", "x86_64-apple-darwin")
