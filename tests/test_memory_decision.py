"""Tests for :class:`cassandra.proxy.memory_decision.MemoryDecision`.
...
* Sites 1/2/3 (Anthropic ``/v1/messages``, Gemini
  ...
  mutated requests under ``x-cassandra-bypass: true``.
...
* ``MemoryMode`` values (``auto_tail`` / ``tool``) and the env-driven
  ``CASSANDRA_MEMORY_INJECTION_MODE`` (``disabled`` / ``auto_tail`` /
...
def test_bypass_header_wins_over_every_other_gate() -> None:
    """``x-cassandra-bypass: true`` is the user's "do not touch my
...
def test_passthrough_mode_header_also_triggers_bypass_skip() -> None:
    """``x-cassandra-mode: passthrough`` is the alternate spelling of
    the bypass signal — mirrors _cassandra_bypass_enabled semantics."""
    d = MemoryDecision.decide(
...
def test_mode_disabled_is_skip() -> None:
    """Operator override via CASSANDRA_MEMORY_INJECTION_MODE=disabled."""
    d = MemoryDecision.decide(
