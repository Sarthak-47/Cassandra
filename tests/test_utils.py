...
def test_timestamp_marker_and_json_helpers() -> None:
    ...
    assert marker == '<cassandra:tool_digest sha256="abc" count="2">'
    assert utils.create_tool_digest_marker("abc") == '<cassandra:tool_digest sha256="abc">'
    assert utils.create_dropped_context_marker("budget") == (
        '<cassandra:dropped_context reason="budget">'
    )
    assert utils.create_dropped_context_marker("budget", count=4) == (
        '<cassandra:dropped_context reason="budget" count="4">'
    )
    assert utils.create_truncated_marker(100, 25) == (
        '<cassandra:truncated original="100" truncated_to="25">'
    )

    extracted = utils.extract_markers(
        'x <cassandra:tool_digest sha256="abc"> y <cassandra:dropped_context reason="budget" count="2">'
    )
    assert extracted == [
