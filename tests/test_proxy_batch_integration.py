...
class TestOpenAIBatchCreate:
    ...
    def test_batch_create_with_compression(self, openai_batch_client, openai_api_key):
        ...
        try:
            ...
            if "x-cassandra-tokens-saved" in response.headers:
                tokens_saved = int(response.headers["x-cassandra-tokens-saved"])
                assert tokens_saved >= 0

            if "x-cassandra-savings-percent" in response.headers:
                savings_percent = float(response.headers["x-cassandra-savings-percent"])
                assert 0 <= savings_percent <= 100

            # Verify compression metadata was added
            metadata = batch_data.get("metadata", {})
            if metadata.get("cassandra_compressed") == "true":
                # Compression was applied
                assert "cassandra_tokens_saved" in metadata
                assert "cassandra_original_tokens" in metadata
                assert "cassandra_compressed_tokens" in metadata
                tokens_saved = int(metadata["cassandra_tokens_saved"])
                assert tokens_saved >= 0
