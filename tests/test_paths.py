"""Tests for ``cassandra.paths`` -- canonical filesystem contract."""
...
def clean_env(monkeypatch: pytest.MonkeyPatch) -> pytest.MonkeyPatch:
    """Ensure every CASSANDRA_* env var this module touches is unset."""

    for name in (
        paths.CASSANDRA_CONFIG_DIR_ENV,
        paths.CASSANDRA_WORKSPACE_DIR_ENV,
        paths.CASSANDRA_SAVINGS_PATH_ENV,
        paths.CASSANDRA_TOIN_PATH_ENV,
        paths.CASSANDRA_SUBSCRIPTION_STATE_PATH_ENV,
    ):
...
def test_workspace_dir_default(fake_home: Path) -> None:
    assert paths.workspace_dir() == fake_home / ".cassandra"
...
def test_workspace_dir_env_override(
    ...
    clean_env.setenv(paths.CASSANDRA_WORKSPACE_DIR_ENV, str(override))
...
def test_workspace_dir_tilde_expansion(fake_home: Path, clean_env: pytest.MonkeyPatch) -> None:
    clean_env.setenv(paths.CASSANDRA_WORKSPACE_DIR_ENV, "~/custom")
...
def test_workspace_dir_blank_env_is_ignored(fake_home: Path, clean_env: pytest.MonkeyPatch) -> None:
    clean_env.setenv(paths.CASSANDRA_WORKSPACE_DIR_ENV, "   ")
    assert paths.workspace_dir() == fake_home / ".cassandra"


def test_config_dir_default(fake_home: Path) -> None:
    assert paths.config_dir() == fake_home / ".cassandra" / "config"
...
def test_config_dir_explicit_env_overrides_workspace(
    ...
    clean_env.setenv(paths.CASSANDRA_WORKSPACE_DIR_ENV, str(tmp_path / "ws"))
    config_override = tmp_path / "cfg"
    clean_env.setenv(paths.CASSANDRA_CONFIG_DIR_ENV, str(config_override))
...
def test_config_dir_tilde_expansion(fake_home: Path, clean_env: pytest.MonkeyPatch) -> None:
    clean_env.setenv(paths.CASSANDRA_CONFIG_DIR_ENV, "~/cfg")
...
def test_per_resource_getters_no_mkdir(fake_home: Path) -> None:
    ...
    assert not (fake_home / ".cassandra").exists()
...
def test_resource_default_under_home(
    ...
    assert fn() == fake_home / ".cassandra" / filename
...
def test_resource_derived_from_workspace_env(
    ...
    ws = tmp_path / "state"
    clean_env.setenv(paths.CASSANDRA_WORKSPACE_DIR_ENV, str(ws))
    fn = getattr(paths, fn_name)
...
def test_resource_explicit_none_falls_through(
    ...
    fn = getattr(paths, fn_name)
    assert fn(None) == fake_home / ".cassandra" / filename
...
def test_memory_db_path_default(fake_home: Path) -> None:
    assert paths.memory_db_path() == fake_home / ".cassandra" / "memory.db"
...
def test_native_memory_dir_default(fake_home: Path) -> None:
    assert paths.native_memory_dir() == fake_home / ".cassandra" / "memories"


def test_license_cache_path_default(fake_home: Path) -> None:
    assert paths.license_cache_path() == fake_home / ".cassandra" / "license_cache.json"


def test_session_stats_path_default(fake_home: Path) -> None:
    assert paths.session_stats_path() == fake_home / ".cassandra" / "session_stats.jsonl"


def test_sync_state_path_default(fake_home: Path) -> None:
    assert paths.sync_state_path() == fake_home / ".cassandra" / "sync_state.json"


def test_bridge_state_path_default(fake_home: Path) -> None:
    assert paths.bridge_state_path() == fake_home / ".cassandra" / "bridge_state.json"


def test_log_dir_default(fake_home: Path) -> None:
    assert paths.log_dir() == fake_home / ".cassandra" / "logs"


def test_proxy_log_path_default(fake_home: Path) -> None:
    assert paths.proxy_log_path() == fake_home / ".cassandra" / "logs" / "proxy.log"


def test_debug_400_dir_default(fake_home: Path) -> None:
    assert paths.debug_400_dir() == fake_home / ".cassandra" / "logs" / "debug_400"


def test_bin_dir_default(fake_home: Path) -> None:
    assert paths.bin_dir() == fake_home / ".cassandra" / "bin"
...
def test_deploy_root_default(fake_home: Path) -> None:
    assert paths.deploy_root() == fake_home / ".cassandra" / "deploy"


def test_beacon_lock_path_includes_port(fake_home: Path) -> None:
    assert paths.beacon_lock_path(8787) == fake_home / ".cassandra" / ".beacon_lock_8787"


# Every derived-only helper must also honor CASSANDRA_WORKSPACE_DIR overrides so
...
def test_ensure_config_dir_follows_env(
    ...
    cfg = tmp_path / "cfg"
    clean_env.setenv(paths.CASSANDRA_CONFIG_DIR_ENV, str(cfg))
    result = paths.ensure_config_dir()
    assert result == cfg
    assert result.is_dir()
...
def test_models_config_path_default(fake_home: Path) -> None:
    assert paths.models_config_path() == fake_home / ".cassandra" / "config" / "models.json"


def test_models_config_path_follows_config_env(
    fake_home: Path, clean_env: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    clean_env.setenv(paths.CASSANDRA_CONFIG_DIR_ENV, str(tmp_path / "cfg"))
...
def test_plugin_config_dir_namespaced(fake_home: Path) -> None:
    ...
    assert a != b
    assert a == fake_home / ".cassandra" / "config" / "plugins" / "alpha"
    assert b == fake_home / ".cassandra" / "config" / "plugins" / "beta"
...
def test_plugin_workspace_dir_namespaced(fake_home: Path) -> None:
    ...
    assert a != b
    assert a == fake_home / ".cassandra" / "plugins" / "alpha"
    assert b == fake_home / ".cassandra" / "plugins" / "beta"
