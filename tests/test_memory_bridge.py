"""Tests for the Memory Bridge (markdown <-> Cassandra bidirectional sync).
...
## Project Overview
- **Cassandra**: Context optimization layer for LLM applications
- **Repos**: OSS at ~/claude-projects/cassandra
...
class TestRelationshipExtraction:
    def test_bold_colon_pattern(self):
        section = ParsedSection(
            heading="Test",
            heading_level=2,
            content="- **Cassandra**: Context optimization layer",
            facts=["**Cassandra**: Context optimization layer"],
        )
        rels = extract_relationships_from_section(section)
        assert len(rels) >= 1
        assert rels[0]["source"] == "Cassandra"
    ...
    def test_verb_patterns(self):
        section = ParsedSection(
            heading="Test",
            heading_level=2,
            content="Cassandra uses SQLite for storage",
            facts=["Cassandra uses SQLite for storage"],
        )
        rels = extract_relationships_from_section(section)
...
class TestMemoryBridgeExport:
    ...
    async def test_export_claude_code_style(self, bridge, tmp_dir, backend):
        ...
        await backend.save_memory(
            content="Cassandra is a context optimization layer",
            user_id="test_user",
            importance=0.8,
