...
@pytest.fixture(autouse=True)
def _enable_cassandra_log_propagation() -> Iterator[None]:
    """Re-enable propagation on the cassandra logger during tests.

    configure_proxy_logging() calls cassandra_logger.propagate = False to prevent
    ...
    for the duration of each test so caplog captures cassandra log records correctly.
    """
    cassandra_logger = logging.getLogger("cassandra")
    original = cassandra_logger.propagate
    cassandra_logger.propagate = True
    yield
    cassandra_logger.propagate = original
...
class TestCorruptMemoryGoldenBytes:
    ...
    def test_corrupt_bytes_logs_error(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        with caplog.at_level(logging.ERROR, logger="cassandra.proxy"):
            apply_session_sticky_memory_tools(
                provider="anthropic",
                session_id=session_id,
                request_id="req-2",
