...
def _ensure_binding():
    ...
    try:
        from cassandra._core import compress_openai_responses_live_zone

        return compress_openai_responses_live_zone
    except ImportError:
        pytest.skip("cassandra._core not built — run scripts/build_rust_extension.sh")
...
class TestModelDefault:
    """Empty `model` defaults to `cassandra_core`'s `DEFAULT_MODEL`."""
...
class TestTelemetryFields:
    ...
    without recounting tokens. See `crates/cassandra-core/src/transforms/
