...
The fix is a shared wrapper at ``cassandra._subprocess`` that automatically sets
...
_PACKAGE = Path(__file__).resolve().parents[2] / "cassandra"
...
def test_text_mode_subprocess_calls_use_wrapper() -> None:
    ...
    assert not offenders, (
        "raw subprocess calls with text=True found (use cassandra._subprocess wrapper):\n"
        + "\n".join(offenders)
    )
