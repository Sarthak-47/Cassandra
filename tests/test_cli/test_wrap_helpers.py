...
from cassandra import paths as paths_mod
...
def test_print_wrap_banner_box_is_inner_width_chars_wide(agent: str) -> None:
    ...
    assert f"CASSANDRA WRAP: {agent.upper()}" in title_line
...
def test_setup_context_tool_lean_ctx_calls_lean_ctx_setup(monkeypatch: pytest.MonkeyPatch) -> None:
    """When CASSANDRA_CONTEXT_TOOL=lean-ctx, helper calls _setup_lean_ctx_agent."""
...
def test_run_proxy_only_watcher_calls_setup_lines_callback(
    ...
    assert "CASSANDRA WRAP: CLINE" in inv.output
...
# header injection for `cassandra wrap claude` (issue: per-project savings).
# ---------------------------------------------------------------------------


class TestApplyProjectHeaderEnv:
    """X-Cassandra-Project injection into ANTHROPIC_CUSTOM_HEADERS."""
    ...
    def test_sets_header_from_cwd_basename(
        ...
        assert env["ANTHROPIC_CUSTOM_HEADERS"] == "X-Cassandra-Project: my-project"
    ...
    def test_appends_to_existing_custom_headers(
        ...
        assert env["ANTHROPIC_CUSTOM_HEADERS"] == (
            "X-Custom-Trace: abc123\nX-Cassandra-Project: proj"
        )

    @pytest.mark.parametrize(
        "user_value",
        [
            "X-Cassandra-Project: their-name",
            "x-cassandra-project: their-name",
            "X-CASSANDRA-PROJECT: their-name",
            "X-Other: 1\nx-Cassandra-Project: their-name",
        ],
    ...
    @pytest.mark.parametrize(
        "user_value",
        [
            "X-Cassandra-Project-Id: other",
            "X-Trace: mentions x-cassandra-project in the value",
        ],
    )
    ...
    def test_project_name_from_cwd_returns_basename(
        ...
        project_dir = tmp_path / "vibe-cassandra"
        project_dir.mkdir()
        monkeypatch.chdir(project_dir)

        assert wrap_mod._project_name_from_cwd() == "vibe-cassandra"
    ...
    def test_non_ascii_cwd_header_is_ascii_safe(
        ...
        """X-Cassandra-Project header value must be ASCII when cwd has non-ASCII chars."""
        ...
        assert header_value.startswith("X-Cassandra-Project: ")
