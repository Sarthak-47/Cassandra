"""Tests for `cassandra wrap openclaw` command."""
...
from cassandra.cli import wrap as wrap_cli
...
def plugin_dir(tmp_path: Path) -> Path:
    ...
    (plugin / "package.json").write_text('{"name":"cassandra-openclaw"}\n')
    (plugin / "openclaw.plugin.json").write_text('{"id":"cassandra"}\n')
...
def test_wrap_openclaw_default_installs_from_npm_and_restarts(runner: CliRunner) -> None:
    ...
    with patch("cassandra.cli.wrap.shutil.which", side_effect=which):
        with patch("cassandra.cli.wrap.subprocess.run", side_effect=_make_successful_run(calls)):
    ...
    assert ["openclaw", "plugins", "inspect", "cassandra"] in cmds

    config_set_index = next(
        i
        for i, cmd in enumerate(cmds)
        if cmd[:4] == ["openclaw", "config", "set", "plugins.entries.cassandra"]
    )
    install_index = next(
        i
        for i, cmd in enumerate(cmds)
    ...
    set_entry = next(
        c
        for c in calls
        if c["cmd"][:4] == ["openclaw", "config", "set", "plugins.entries.cassandra"]
    )
...
def test_wrap_openclaw_fails_when_plugin_path_invalid(runner: CliRunner, tmp_path: Path) -> None:
    ...
    with patch("cassandra.cli.wrap.shutil.which", return_value="openclaw"):
...
def test_wrap_openclaw_uses_extension_fallback_on_linked_install_bug(
    ...
    with patch("cassandra.cli.wrap.shutil.which", side_effect=which):
        with patch("cassandra.cli.wrap.subprocess.run", side_effect=run):
            with patch(
                "cassandra.cli.wrap._copy_openclaw_plugin_into_extensions",
                return_value=Path("C:/Users/test/.openclaw/extensions/cassandra"),
            ) as copy_fallback:
                result = runner.invoke(main, ["wrap", "openclaw", "--plugin-path", str(plugin_dir)])
...
def test_wrap_openclaw_continues_when_plugin_already_exists(
    ...
    def run(cmd, **kwargs):  # noqa: ANN001
        ...
        if cmd[:3] == ["openclaw", "plugins", "install"]:
            return MagicMock(
                returncode=1,
                stdout="plugin already exists: C:\\Users\\test\\.openclaw\\extensions\\cassandra",
                stderr="",
            )
        return MagicMock(returncode=0, stdout="", stderr="")
...
def test_read_openclaw_config_value_handles_missing_and_raw_strings() -> None:
    ...
    with patch("cassandra.cli.wrap.subprocess.run", side_effect=[missing, raw_string]):
        assert wrap_cli._read_openclaw_config_value("openclaw", "plugins.entries.cassandra") is None
        assert (
            wrap_cli._read_openclaw_config_value(
                "openclaw", "plugins.entries.cassandra.config.pythonPath"
            )
            == "plain-text-value"
        )
...
def test_build_openclaw_unwrap_entry_preserves_top_level_metadata() -> None:
    ...
    assert entry["source"] == "cassandra-ai/openclaw"
...
def test_run_checked_raises_click_exception_on_command_errors() -> None:
    with patch("cassandra.cli.wrap.subprocess.run", side_effect=FileNotFoundError()):
    ...
    with patch("cassandra.cli.wrap.subprocess.run", side_effect=cpe_stderr):
    ...
    with patch("cassandra.cli.wrap.subprocess.run", side_effect=cpe_stdout):
...
def test_resolve_openclaw_extensions_dir_empty_output_raises() -> None:
    with patch(
        "cassandra.cli.wrap._run_checked",
...
def test_copy_openclaw_plugin_into_extensions_handles_missing_and_existing_dist(
    ...
    with patch("cassandra.cli.wrap._resolve_openclaw_extensions_dir", return_value=tmp_path):
    ...
    target_cassandra = ext_root / "cassandra"
    target_dist = target_cassandra / "dist"
    target_hook_shim = target_cassandra / "hook-shim"
    ...
    with patch("cassandra.cli.wrap._resolve_openclaw_extensions_dir", return_value=ext_root):
        out = wrap_cli._copy_openclaw_plugin_into_extensions(
            plugin_dir=plugin, openclaw_bin="openclaw"
        )

    assert out == target_cassandra
...
def test_unwrap_openclaw_disables_plugin_and_restores_legacy_slot(runner: CliRunner) -> None:
    ...
    def run(cmd, **kwargs):  # noqa: ANN001
        calls.append({"cmd": list(cmd), **kwargs})
        if cmd[:4] == ["openclaw", "config", "get", "plugins.entries.cassandra"]:
            return MagicMock(
                returncode=0,
                stdout=json.dumps(
                    {
