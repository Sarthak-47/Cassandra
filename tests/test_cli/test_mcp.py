...
def mock_claude_config_path(temp_claude_dir):
    ...
    with patch("cassandra.cli.mcp.MCP_CONFIG_PATH", config_path):
        with patch("cassandra.cli.mcp.CLAUDE_CONFIG_DIR", temp_claude_dir):
            with patch("cassandra.cli.mcp.shutil.which", side_effect=which_no_claude):
                yield config_path
...
class TestMCPConfigFunctions:
    ...
    def test_get_cassandra_command_returns_list(self):
        """Command should be a list suitable for subprocess."""
        cmd = get_cassandra_command()
    ...
    def test_save_config_creates_directory(self, tmp_path):
        ...
        with patch("cassandra.cli.mcp.MCP_CONFIG_PATH", config_path):
            with patch("cassandra.cli.mcp.CLAUDE_CONFIG_DIR", claude_dir):
...
class TestMCPUninstallCommand:
    """Test 'cassandra mcp uninstall' command."""

    def test_uninstall_removes_cassandra(self, mock_claude_config_path, mock_mcp_available):
        """Uninstall removes cassandra from the legacy config file."""
        ...
        mock_claude_config_path.write_text(
            json.dumps(
                {
                    "mcpServers": {
                        "cassandra": {"command": "cassandra", "args": ["mcp", "serve"]},
                    }
                }
            )
        )
    ...
    def test_uninstall_not_configured(self, mock_claude_config_path):
        """Uninstall when cassandra not in config exits cleanly."""
        # Create config without cassandra
...
class TestMCPStatusCommand:
    """Test 'cassandra mcp status' command."""
    ...
    def test_status_configured(self, mock_claude_config_path, mock_mcp_available):
        """Status reports configured when the legacy config has cassandra."""
...
class TestMCPServeCommand:
    """Test 'cassandra mcp serve' command."""
...
class TestMCPServerInitialization:
    ...
    def test_mcp_server_has_correct_tool_name(self):
        """MCP server is configured for cassandra_retrieve tool."""
        from cassandra.ccr.mcp_server import create_ccr_mcp_server
        ...
        assert server.server.name == "cassandra"
        # The tool name should be cassandra_retrieve
        assert CCR_TOOL_NAME == "cassandra_retrieve"
...
class TestMCPUninstallWithClaudeCLI:
    ...
    def test_uninstall_calls_claude_mcp_remove(self):
        """Uninstall calls claude mcp remove when cassandra is registered."""
        calls = []
        ...
        runner = CliRunner()
        with patch("cassandra.cli.mcp.shutil.which", return_value="/usr/bin/claude"):
            with patch("cassandra.cli.mcp.subprocess.run", side_effect=capturing_run):
                result = runner.invoke(main, ["mcp", "uninstall"])
    ...
    def test_uninstall_skips_remove_when_not_registered(self):
        """Uninstall does not call remove when cassandra is not registered via claude CLI."""
        calls = []
