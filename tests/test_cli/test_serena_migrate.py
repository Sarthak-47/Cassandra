"""Re-wrap must migrate a stale Cassandra-installed Serena entry.
...
``_setup_serena_mcp`` closes that gap: when the ledger proves Cassandra
...
from cassandra.mcp_registry.base import RegisterResult, RegisterStatus, ServerSpec
from cassandra.mcp_registry.ledger import cassandra_installed_matching, record_install

# The Serena spec Cassandra wrote before the dashboard flag existed.
...
def test_rewrap_migrates_stale_cassandra_serena(
    capsys: pytest.CaptureFixture[str],
) -> None:
    # Ledger proves Cassandra installed the stale entry that's on disk.
    ...
    assert cassandra_installed_matching("claude", fresh)


def test_rewrap_leaves_user_managed_serena(
    capsys: pytest.CaptureFixture[str],
) -> None:
    # Differs from the current spec but is NOT in Cassandra's ledger.
...
def test_rewrap_fresh_install_records_dashboard_off_spec(
    ...
    assert cassandra_installed_matching("claude", entry)
