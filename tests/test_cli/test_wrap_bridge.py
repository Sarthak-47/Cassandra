...
from cassandra.cli.wrap import _setup_lean_ctx_agent
...
def test_wrap_claude_prepare_only_skips_host_binary_lookup() -> None:
    ...
    with patch("cassandra.cli.wrap._prepare_wrap_rtk") as prepare_rtk:
        with patch("cassandra.cli.wrap.shutil.which") as which_mock:
...
def test_wrap_claude_prepare_only_uses_lean_ctx_when_configured(monkeypatch) -> None:
    ...
    with patch("cassandra.cli.wrap._prepare_wrap_rtk") as prepare_rtk:
        with patch(
            "cassandra.cli.wrap._setup_lean_ctx_agent",
...
def test_setup_lean_ctx_agent_runs_outside_project_root(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("cassandra.lean_ctx.get_lean_ctx_path", lambda: lean_ctx)
    monkeypatch.setattr("cassandra.cli.wrap.subprocess.run", fake_run)
...
def test_wrap_codex_prepare_only_uses_lean_ctx_when_configured(monkeypatch, tmp_path: Path) -> None:
    ...
    with runner.isolated_filesystem(temp_dir=str(tmp_path)):
        with patch("cassandra.cli.wrap._ensure_rtk_binary") as ensure_rtk:
            with patch(
                "cassandra.cli.wrap._setup_lean_ctx_agent",
                return_value=Path("lean-ctx"),
            ) as setup:
...
def test_wrap_codex_prepare_only_accepts_no_context_tool_alias(monkeypatch, tmp_path: Path) -> None:
    ...
    with runner.isolated_filesystem(temp_dir=str(tmp_path)):
        with patch("cassandra.cli.wrap._ensure_rtk_binary") as ensure_rtk:
            with patch("cassandra.cli.wrap._setup_lean_ctx_agent") as setup:
                result = runner.invoke(
                    main,
                    [
                        "wrap",
...
def test_wrap_aider_prepare_only_injects_conventions(monkeypatch, tmp_path: Path) -> None:
    ...
    with runner.isolated_filesystem(temp_dir=str(tmp_path)):
        with patch("cassandra.cli.wrap._ensure_rtk_binary", return_value=Path("rtk")):
            result = runner.invoke(main, ["wrap", "aider", "--prepare-only"])

        assert result.exit_code == 0, result.output
        conventions = Path("CONVENTIONS.md")
        assert conventions.exists()
        assert "cassandra:rtk-instructions" in conventions.read_text(encoding="utf-8")
...
def test_wrap_cursor_prepare_only_injects_cursorrules(monkeypatch, tmp_path: Path) -> None:
    ...
    with runner.isolated_filesystem(temp_dir=str(tmp_path)):
        ...
        assert "cassandra:rtk-instructions" in cursorrules.read_text(encoding="utf-8")
