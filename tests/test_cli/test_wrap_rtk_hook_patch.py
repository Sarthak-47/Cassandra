...
``~/.cassandra/bin`` is not automatically added to PATH, that lookup fails
...
from cassandra.cli.wrap import _patch_rtk_hook_absolute_path
...
def test_patches_bare_rtk_to_absolute_path(tmp_path: Path) -> None:
    ...
    rtk_path = Path("/home/user/.cassandra/bin/rtk")
...
def test_quotes_path_containing_spaces(tmp_path: Path) -> None:
    ...
    rtk_path = Path("/Users/Alice Smith/.cassandra/bin/rtk")
    changed = _patch_rtk_hook_absolute_path(rtk_path, hook_script)
    ...
    assert "exec /Users/Alice Smith/.cassandra/bin/rtk rewrite" not in content
