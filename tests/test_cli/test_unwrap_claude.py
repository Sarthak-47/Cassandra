...
def test_unwrap_claude_removes_mcp_rtk_and_stops_proxy(
    ...
    with (
        patch("cassandra.mcp_registry.ClaudeRegistrar", return_value=Registrar()),
        patch(
    ...
    assert unregistered == ["cassandra", "codebase-memory-mcp"]
    assert stopped == [9999]
...
def test_unwrap_claude_preserves_user_managed_serena(
    ...
    monkeypatch.setenv("CASSANDRA_WORKSPACE_DIR", str(tmp_path / ".cassandra"))
    unregistered: list[str] = []
    ...
    with (
        patch("cassandra.mcp_registry.ClaudeRegistrar", return_value=Registrar()),
        patch("cassandra.cli.wrap._remove_claude_rtk_hooks", return_value=False),
        patch("cassandra.cli.wrap._stop_local_proxy_for_unwrap"),
    ):
...
def test_unwrap_claude_removes_cassandra_installed_serena(
    runner: CliRunner,
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    from cassandra.mcp_registry import build_serena_spec
    from cassandra.mcp_registry.ledger import record_install
    ...
    assert result.exit_code == 0, result.output
    assert unregistered == ["cassandra", "codebase-memory-mcp", "serena"]
    assert "Removed Cassandra-installed Serena MCP server" in result.output
...
def test_unwrap_claude_keep_flags_skip_cleanup(
    ...
    with (
        patch("cassandra.mcp_registry.ClaudeRegistrar") as registrar,
        patch("cassandra.cli.wrap._remove_claude_rtk_hooks") as remove_rtk,
        patch("cassandra.cli.wrap._stop_local_proxy_for_unwrap") as stop_proxy,
    ):
                            ...
                            "hooks": [
                                {
                                    "type": "command",
                                    "command": (
                                        "/home/u/.local/bin/cassandra init hook ensure "
                                        "--profile init-user --marker cassandra-init-claude"
                                    ),
                                    "timeout": 15,
                                }
                            ],
