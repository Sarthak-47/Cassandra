"""Tests for `cassandra wrap codex` and `cassandra unwrap codex`.

These exercise the Codex-specific ``config.toml`` injection and restoration
helpers that route Codex through the Cassandra proxy.  They are deliberately
...
from cassandra.cli import wrap as wrap_mod
from cassandra.cli.main import main
from cassandra.mcp_registry.install import build_cassandra_spec
...
class TestStripCodexCassandraBlocks:
    """Tests for the regex-based cleanup helper."""

    def test_empty_content_returns_empty(self) -> None:
        assert wrap_mod._strip_codex_cassandra_blocks("") == ""

    def test_returns_content_unchanged_when_no_markers(self) -> None:
        original = '[profiles.default]\nmodel = "gpt-4o"\n'
        cleaned = wrap_mod._strip_codex_cassandra_blocks(original)
    ...
    def test_removes_complete_cassandra_block(self) -> None:
        wrapped = (
        ...
        assert wrap_mod._strip_codex_cassandra_blocks(wrapped) == ""
    ...
    def test_preserves_user_content_around_block(self) -> None:
        ...
        cleaned = wrap_mod._strip_codex_cassandra_blocks(wrapped)
    ...
    def test_removes_stray_top_level_model_provider_line(self) -> None:
        # Old wrap versions left `model_provider = "cassandra"` outside markers.
        content = 'foo = 1\nmodel_provider = "cassandra"\nbar = 2\n'
        cleaned = wrap_mod._strip_codex_cassandra_blocks(content, remove_mcp=True)
        assert 'model_provider = "cassandra"' not in cleaned
    ...
    def test_removes_codex_mcp_blocks(self) -> None:
        content = (
            ...
            "[mcp_servers.cassandra]\n"
            'command = "cassandra"\n'
            f"{wrap_mod._CODEX_MCP_END}\n\n"
            "# --- Cassandra MCP server: serena ---\n"
            "[mcp_servers.serena]\n"
            'command = "uvx"\n'
            "# --- end Cassandra MCP server: serena ---\n\n"
            f"{wrap_mod._MEMORY_MCP_MARKER}\n"
            "[mcp_servers.cassandra_memory]\n"
            'command = "python"\n'
        ...
        assert "[mcp_servers.cassandra]" not in cleaned
        assert "[mcp_servers.serena]" not in cleaned
        assert "[mcp_servers.cassandra_memory]" not in cleaned
        assert 'model = "gpt-4o"' in cleaned
    ...
    def test_preserves_named_mcp_blocks_when_remove_named_mcp_false(self) -> None:
        ...
        cleaned = wrap_mod._strip_codex_cassandra_blocks(
...
class TestSnapshotCodexConfig:
    ...
    def test_creates_backup_on_first_call(self, tmp_path: Path) -> None:
        ...
        backup_file = tmp_path / "config.toml.cassandra-backup"
    ...
    def test_no_backup_when_config_already_contains_memory_mcp_block(self, tmp_path: Path) -> None:
        ...
        config_file.write_text(
            ...
            "[mcp_servers.cassandra_memory]\n"
            'command = "python"\n'
            'args = ["-m", "cassandra.memory.mcp_server", "--user", "codex-user"]\n'
            f"{wrap_mod._MEMORY_MCP_END}\n"
        )
    ...
    def test_backup_when_config_contains_named_mcp_marker(self, tmp_path: Path) -> None:
        ...
        backup_file = tmp_path / "config.toml.cassandra-backup"
        original = (
            "# --- Cassandra MCP server: cassandra ---\n"
            "[mcp_servers.cassandra]\n"
            'command = "cassandra"\n'
            "# --- end Cassandra MCP server: cassandra ---\n"
        )
...
class TestCodexMemoryMcpConfig:
    ...
    def test_inject_omits_db_and_replaces_existing_memory_block(
        ...
        config_file.write_text(
            ...
            "[mcp_servers.cassandra_memory]\n"
            'command = "python"\n'
            'args = ["-m", "cassandra.memory.mcp_server", "--db", "/tmp/project-a/.cassandra/memory.db", "--user", "old-user"]\n'
            f"{wrap_mod._MEMORY_MCP_END}\n"
        )
        ...
        assert "[mcp_servers.cassandra_memory]" in content
...
class TestInjectAndRestoreRoundTrip:
    ...
    def test_wrap_unwrap_restores_empty_state(
        ...
        assert 'model_provider = "cassandra"' in config_file.read_text(encoding="utf-8")

        status, _ = wrap_mod._restore_codex_provider_config()
        # No prior config existed → the injected file is fully removed.
        assert status == "removed"
        assert not config_file.exists()
        assert not (tmp_path / ".codex" / "config.toml.cassandra-backup").exists()
    ...
    def test_wrap_unwrap_restores_prior_model_provider(
        ...
        assert 'model_provider = "cassandra"' in wrapped
        assert "[model_providers.cassandra]" in wrapped

        status, _ = wrap_mod._restore_codex_provider_config()
        assert status == "restored"
        assert config_file.read_text(encoding="utf-8") == original
        assert not (config_dir / "config.toml.cassandra-backup").exists()
    ...
    def test_wrap_is_idempotent(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        ...
        # Exactly two Cassandra blocks — a top-level-key block and the
    ...
    def test_unwrap_without_backup_removes_provider_and_mcp_blocks(
        ...
        cleaned = config_file.read_text(encoding="utf-8")
        assert 'model = "gpt-4o"' in cleaned
        assert "cassandra" not in cleaned
    ...
    def test_memory_only_wrap_without_backup_preserves_named_mcp_block(
        ...
        config_dir.mkdir()
        config_file = config_dir / "config.toml"
        backup_file = config_dir / "config.toml.cassandra-backup"
        original = (
            "# --- Cassandra MCP server: cassandra ---\n"
            "[mcp_servers.cassandra]\n"
            'command = "cassandra"\n'
...
# ---------------------------------------------------------------------------
# Thread retag: wrap pulls native threads into the cassandra menu, unwrap hands
# them back, so the Codex history list stays whole across the proxy boundary.
# ---------------------------------------------------------------------------


class TestWrapRetagsThreadProviders:
    """``wrap codex`` retags ``openai`` threads to ``cassandra`` and back."""
    ...
    def test_wrap_unwrap_round_trips_thread_providers(
        ...
        self._seed_threads(gui_db, [("a", "openai"), ("b", "cassandra"), ("c", "anthropic")])
        ...
        # Native threads are now visible under the cassandra provider menu;
        # third-party providers are left untouched.
        assert self._count(gui_db, "cassandra") == 2
        assert self._count(gui_db, "openai") == 0
        assert self._count(gui_db, "anthropic") == 1
        assert self._count(cli_db, "cassandra") == 1

        with patch("cassandra.cli.wrap._stop_local_proxy_for_unwrap", return_value="stopped"):
        ...
        assert self._count(gui_db, "cassandra") == 0
        assert self._count(gui_db, "anthropic") == 1
...
class TestSubscriptionRouting:
    ...
    def test_strip_cleans_orphaned_openai_base_url(self) -> None:
        ...
        cleaned = wrap_mod._strip_codex_cassandra_blocks(content)
...
class TestInjectAvoidsDuplicateTopLevelKeys:
    ...
    def test_inject_does_not_create_duplicate_model_provider(
        ...
        # And the rewritten values are the cassandra ones.
        assert 'model_provider = "cassandra"' in content
        assert 'openai_base_url = "http://127.0.0.1:8787/v1"' in content
    ...
    def test_inject_empty_file_still_uses_marker_block(
        ...
        assert 'model_provider = "cassandra"' in content
        assert 'openai_base_url = "http://127.0.0.1:8787/v1"' in content
        assert "[model_providers.cassandra]" in content
...
# ---------------------------------------------------------------------------
# Integration tests: full `cassandra wrap codex` / `cassandra unwrap codex`
# ---------------------------------------------------------------------------
...
def test_wrap_codex_prepare_only_creates_backup_and_config(
    ...
    assert 'model_provider = "cassandra"' in config_file.read_text(encoding="utf-8")
    backup = tmp_path / ".codex" / "config.toml.cassandra-backup"
...
def test_wrap_codex_prepare_only_respects_codex_home(
    ...
    content = config_file.read_text(encoding="utf-8")
    assert 'model_provider = "cassandra"' in content
    assert "[mcp_servers.cassandra]" in content
    assert not (tmp_path / ".codex" / "config.toml").exists()
...
def test_wrap_codex_injects_rtk_globally_without_changing_project_agents(
    ...
    with patch(
        "cassandra.cli.wrap._ensure_rtk_binary",
...
def test_unwrap_codex_without_codex_home_warns_on_ambiguous_noop(
    ...
    assert unwrap_result.exit_code == 0, unwrap_result.output
    assert "Warning: found no Cassandra wrap markers in the default Codex config" in (
        unwrap_result.output
    )
    assert "If you wrapped Codex with CODEX_HOME" in unwrap_result.output
    assert "CODEX_HOME=/path/to/codex-home cassandra unwrap codex" in unwrap_result.output
...
def test_start_proxy_does_not_apply_agent_90_defaults(
    ...
    assert "CASSANDRA_SAVINGS_PROFILE" not in env
    assert "CASSANDRA_TARGET_RATIO" not in env
    assert "CASSANDRA_MAX_ITEMS" not in env
    assert "CASSANDRA_SMART_CRUSHER_COMPACTION" not in env
...
def test_start_proxy_preserves_explicit_savings_overrides(
    ...
    monkeypatch.setenv("CASSANDRA_TARGET_RATIO", "0.20")
    monkeypatch.setenv("CASSANDRA_MAX_ITEMS", "12")
    monkeypatch.setattr(wrap_mod, "_get_log_path", lambda: tmp_path / "proxy.log")
    monkeypatch.setattr(wrap_mod, "_check_proxy", lambda port: True)
    monkeypatch.setattr(wrap_mod.subprocess, "Popen", fake_popen)
    ...
    env = popen_kwargs["env"]
    assert isinstance(env, dict)
    assert env["CASSANDRA_TARGET_RATIO"] == "0.20"
    assert env["CASSANDRA_MAX_ITEMS"] == "12"
...
def test_wrap_codex_prepare_only_updates_stale_mcp_proxy_url(
    ...
    config_file.write_text(
        ...
        'args = ["mcp", "serve"]\n'
        "\n"
        "[mcp_servers.cassandra.env]\n"
        'CASSANDRA_PROXY_URL = "http://127.0.0.1:9000"\n'
        "# --- end Cassandra MCP server ---\n",
        encoding="utf-8",
    )
    ...
    expected = build_cassandra_spec()
    cassandra_mcp = parsed["mcp_servers"]["cassandra"]
    assert "[mcp_servers.cassandra]" in content
    assert cassandra_mcp["command"] == expected.command
    assert cassandra_mcp["args"] == list(expected.args)
    assert "env" not in cassandra_mcp or "CASSANDRA_PROXY_URL" not in cassandra_mcp["env"]
...
def test_wrap_codex_memory_prepare_only_uses_local_db_without_persisting_it(
        ...
        with patch(
            ...
            with patch("cassandra.memory.sync.sync_import", side_effect=fake_sync_import):
                with patch(
                    ...
                    with patch(
                        "cassandra.memory.sync_adapters.claude_code.get_claude_memory_dir",
    ...
    assert result.exit_code == 0, result.output
    assert backend_paths == [str(project_dir / ".cassandra" / "memory.db")]
...
def test_wrap_codex_prepare_only_registers_serena_when_uvx_exists(
    ...
    with patch("cassandra.cli.wrap._ensure_rtk_binary", return_value=None):
        with patch("cassandra.cli.wrap.shutil.which", side_effect=fake_which):
...
def test_unwrap_codex_restores_prior_config_end_to_end(
    ...
    with patch(
        "cassandra.cli.wrap._stop_local_proxy_for_unwrap",
    ...
    # proxy is stopped.
    assert config_file.read_text(encoding="utf-8") == original
    assert 'model_provider = "cassandra"' not in config_file.read_text(encoding="utf-8")
    assert not (tmp_path / ".codex" / "config.toml.cassandra-backup").exists()
    assert stopped == [9999]
    assert "Stopped local Cassandra proxy on port 9999" in unwrap_result.output
...
def test_unwrap_codex_no_stop_proxy_leaves_proxy_alone(
    ...
    with patch("cassandra.cli.wrap._stop_local_proxy_for_unwrap") as stop_proxy:
...
def test_wrap_codex_memory_prepare_only_unwrap_removes_memory_mcp_without_prior_config(
    ...
    with patch("cassandra.cli.wrap._ensure_rtk_binary", return_value=None):
        with patch("cassandra.memory.sync._build_sync_backend", return_value=FakeBackend()):
            with patch("cassandra.memory.sync.sync_import", side_effect=fake_sync_import):
                with patch(
                    "cassandra.memory.sync_adapters.claude_code.ClaudeCodeAdapter",
                    autospec=True,
...
def test_stop_local_proxy_for_unwrap_kills_identified_cassandra_proxy(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_stop_local_proxy_for_unwrap_refuses_unidentified_listener(
    ...
    with patch("cassandra.cli.wrap._kill_proxy_by_pid") as kill_proxy:
...
def test_unwrap_codex_removes_cassandra_only_config_file(
    runner: CliRunner, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    _set_test_home(monkeypatch, tmp_path)
...
class TestCodexProjectHeaderConfig:
    """The injected provider maps X-Cassandra-Project to CASSANDRA_PROJECT.

    Codex's ``env_http_headers`` sends a header only when the mapped env var
    is set at Codex runtime, so `cassandra wrap codex` exports
    ``CASSANDRA_PROJECT`` and the proxy attributes savings per project.
    """
    ...
    def test_inject_writes_env_http_headers_mapping(
        ...
        content = (tmp_path / ".codex" / "config.toml").read_text(encoding="utf-8")
        assert 'env_http_headers = { "X-Cassandra-Project" = "CASSANDRA_PROJECT" }' in content

    def test_env_http_headers_inside_provider_section(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
        """The mapping must live inside [model_providers.cassandra], before
        the closing marker, so it applies to the Cassandra provider."""
        _set_test_home(monkeypatch, tmp_path)

        wrap_mod._inject_codex_provider_config(8787)

        content = (tmp_path / ".codex" / "config.toml").read_text(encoding="utf-8")
        section_start = content.index("[model_providers.cassandra]")
    ...
    def test_strip_removes_block_with_env_http_headers(
        ...
        """_strip_codex_cassandra_blocks removes the whole injected block,
        ...
        assert "X-Cassandra-Project" not in cleaned
        assert "[model_providers.cassandra]" not in cleaned
        assert 'model = "gpt-4o"' in cleaned
