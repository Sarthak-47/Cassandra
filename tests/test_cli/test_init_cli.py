...
def _load_init_module(monkeypatch):
    monkeypatch.delitem(sys.modules, "cassandra.cli.init", raising=False)
    monkeypatch.delitem(sys.modules, "cassandra.cli.main", raising=False)
    ...
    monkeypatch.setitem(sys.modules, "cassandra.cli.main", fake_main_module)
    importlib.invalidate_caches()
    init_cli = importlib.import_module("cassandra.cli.init")
    monkeypatch.delitem(sys.modules, "cassandra.cli.init", raising=False)
...
def test_init_fails_when_auto_detection_empty(monkeypatch) -> None:
    """Bare ``cassandra init`` with no agents on PATH prints a guided error.
    ...
    assert "cassandra init -g claude" in result.output
...
def test_format_empty_detection_error_local_scope(monkeypatch) -> None:
    ...
    assert "cassandra init copilot" not in message
    assert "cassandra init openclaw" not in message
    assert "cassandra init claude" in message
    assert "cassandra init codex" in message
...
def test_init_verbose_enables_debug_logging_on_stderr(monkeypatch) -> None:
    """``cassandra init -v`` should emit diagnostic lines to stderr.
    ...
    assert "[cassandra init]" in stderr
...
def test_init_codex_creates_hooks_feature_flag_on_first_init(
    ...
    assert parsed["model_provider"] == "cassandra"
...
def test_init_openclaw_delegates_to_wrap(monkeypatch) -> None:
    ...
    monkeypatch.setattr(init_cli, "resolve_cassandra_command", lambda: ["cassandra"])
    monkeypatch.setattr(
        init_cli.subprocess,
        "run",
    ...
    assert calls == [["cassandra", "wrap", "openclaw", "--proxy-port", "9999"]]
...
def test_marketplace_source_prefers_env_override(monkeypatch) -> None:
    init_cli, _ = _load_init_module(monkeypatch)
    monkeypatch.setenv("CASSANDRA_MARKETPLACE_SOURCE", "custom/source")
...
def test_command_string_and_matcher_on_windows(monkeypatch) -> None:
    ...
    assert init_cli._command_string(["cassandra", "init"]) == "joined-command"
...
def test_command_string_normalizes_backslashes_on_windows(monkeypatch) -> None:
    ...
    result = init_cli._command_string(
        ["C:\\Users\\user\\.local\\bin\\cassandra.exe", "init", "hook", "ensure"]
    )
    assert "\\" not in result
    assert "C:/Users/user/.local/bin/cassandra.exe" in result
...
def test_command_string_quotes_spaces_after_normalization(monkeypatch) -> None:
    ...
    result = init_cli._command_string(
        ["C:\\Program Files\\cassandra\\cassandra.exe", "init", "hook", "ensure"]
    )
    assert "\\" not in result
    assert '"C:/Program Files/cassandra/cassandra.exe"' in result
...
def test_ensure_claude_hooks_rewrites_existing_entries(monkeypatch, tmp_path: Path) -> None:
                    ...
                    "SessionStart": [
                        ...
                        },
                        {
                            ...
                            "hooks": [
                                {
                                    "type": "command",
                                    "command": "cassandra init hook ensure --marker cassandra-init-claude",
                                }
                            ],
                        },
                    ]
    ...
    monkeypatch.setattr(init_cli, "_hook_command", lambda *parts: "cassandra init hook ensure")
    ...
    assert session_entries[-1]["hooks"][0]["command"].endswith("--marker cassandra-init-claude")
...
def test_ensure_copilot_hooks_replaces_existing_marker(monkeypatch, tmp_path: Path) -> None:
        ...
        json.dumps(
            {
                "hooks": {
                    "SessionStart": [
                        ...
                        {
                            "type": "command",
                            "command": "cassandra init hook ensure --marker cassandra-init-copilot",
                        },
                    ]
                }
            }
    ...
    assert commands == ["echo keep", "cassandra init hook ensure --marker cassandra-init-copilot"]
...
def test_ensure_codex_provider_keeps_root_keys_above_existing_table(
    ...
    'invalid type: string "cassandra", expected a boolean in features'.
    """
    init_cli, _ = _load_init_module(monkeypatch)
    path = tmp_path / "config.toml"
    ...
    assert parsed["features"]["hooks"] is True
    assert parsed["model_providers"]["cassandra"]["base_url"] == "http://127.0.0.1:8787/v1"
...
def test_marketplace_source_prefers_repo_checkout(monkeypatch) -> None:
    init_cli, _ = _load_init_module(monkeypatch)
    monkeypatch.delenv("CASSANDRA_MARKETPLACE_SOURCE", raising=False)
...
def test_install_claude_marketplace_runs_expected_commands(monkeypatch) -> None:
    ...
    assert calls == [
        ...
        (
            ["claude", "plugin", "install", "cassandra@cassandra-marketplace", "--scope", "user"],
...
def test_install_copilot_marketplace_runs_expected_commands(monkeypatch) -> None:
    ...
    assert calls == [
        ...
        (
            ["copilot", "plugin", "install", "cassandra@cassandra-marketplace"],
...
def test_init_codex_writes_openai_base_url(monkeypatch, tmp_path: Path) -> None:
    ...
    subscription (ChatGPT plan) users are routed through cassandra even when the
...
def test_init_codex_provider_retags_existing_threads(monkeypatch, tmp_path: Path) -> None:
    """`cassandra init` injects `model_provider = "cassandra"` for Codex, which
    ...
    must retag existing threads openai->cassandra so the history stays visible —
    ...
    try:
        ...
        conn.close()
    # Native threads now live under the active cassandra provider (stay visible);
    # third-party providers are left untouched.
    assert counts.get("cassandra") == 2, f"existing openai threads not retagged: {counts}"
