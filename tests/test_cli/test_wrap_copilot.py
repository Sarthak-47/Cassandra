"""Tests for `cassandra wrap copilot` command."""
...
from cassandra.copilot_auth import DEFAULT_API_URL, CopilotSubscriptionTokenResolution
...
def _subscription_resolution(
    ...
    *,
    ...
    source: str = "cassandra-copilot-auth:/tmp/copilot_auth.json:token-exchange",
...
def wrap_modules(monkeypatch: pytest.MonkeyPatch) -> tuple[types.ModuleType, click.Group]:
    cassandra_pkg = sys.modules.get("cassandra")
    saved_cassandra_cli_attr = (
        cassandra_pkg.cli if cassandra_pkg is not None and hasattr(cassandra_pkg, "cli") else None
    )
    saved_modules = {
        name: sys.modules.get(name)
        for name in ("cassandra.cli", "cassandra.cli.main", "cassandra.cli.wrap")
    }

    fake_main_module = types.ModuleType("cassandra.cli.main")
    fake_main_module.main = click.Group()
    sys.modules["cassandra.cli.main"] = fake_main_module
    sys.modules.pop("cassandra.cli", None)
    sys.modules.pop("cassandra.cli.wrap", None)

    wrap_cli = importlib.import_module("cassandra.cli.wrap")
    monkeypatch.setattr(wrap_cli, "_check_proxy", lambda _port: False)

    try:
        yield wrap_cli, fake_main_module.main
    finally:
        for name in ("cassandra.cli.wrap", "cassandra.cli.main", "cassandra.cli"):
            sys.modules.pop(name, None)
        for name, module in saved_modules.items():
            if module is not None:
                sys.modules[name] = module
        if saved_modules["cassandra.cli"] is not None:
            cli_pkg = saved_modules["cassandra.cli"]
            if saved_modules["cassandra.cli.main"] is not None:
                cli_pkg.main = saved_modules["cassandra.cli.main"]
            if saved_modules["cassandra.cli.wrap"] is not None:
                cli_pkg.wrap = saved_modules["cassandra.cli.wrap"]
        if cassandra_pkg is not None:
            if saved_cassandra_cli_attr is None:
                if hasattr(cassandra_pkg, "cli"):
                    delattr(cassandra_pkg, "cli")
            else:
                cassandra_pkg.cli = saved_cassandra_cli_attr
...
def test_wrap_copilot_auto_anthropic_injects_instructions(
    ...
    with (
        patch("cassandra.cli.wrap.shutil.which", return_value="copilot"),
        patch("cassandra.cli.wrap.has_oauth_auth", return_value=False),
        patch("cassandra.cli.wrap._ensure_rtk_binary", return_value=Path("/tmp/rtk")),
        patch("cassandra.cli.wrap._launch_tool", side_effect=fake_launch_tool),
    ):
...
def test_wrap_copilot_byok_rejects_auto_model_before_launch(
    ...
    with (
        patch("cassandra.cli.wrap.shutil.which", return_value="copilot"),
        patch("cassandra.cli.wrap.has_oauth_auth", return_value=False),
        patch("cassandra.cli.wrap._launch_tool", side_effect=fail_launch_tool),
    ):
        result = runner.invoke(
            main,
            [
...
def test_wrap_copilot_auto_detects_running_proxy_backend(
    ...
    with (
        patch("cassandra.cli.wrap.shutil.which", return_value="copilot"),
        patch("cassandra.cli.wrap.has_oauth_auth", return_value=False),
        patch("cassandra.cli.wrap._check_proxy", return_value=True),
        patch("cassandra.cli.wrap._detect_running_proxy_backend", return_value="anyllm"),
        patch("cassandra.cli.wrap._launch_tool", side_effect=fake_launch_tool),
    ):
        result = runner.invoke(
            main,
...
def test_wrap_copilot_prefers_existing_oauth_session(
    ...
    with patch("cassandra.cli.wrap.shutil.which", return_value="copilot"):
        with patch("cassandra.cli.wrap.resolve_client_bearer_token", return_value="gho-existing"):
            with patch("cassandra.cli.wrap.has_oauth_auth", return_value=True):
                with patch("cassandra.cli.wrap._launch_tool", side_effect=fake_launch_tool):
                    result = runner.invoke(
                        main,
...
def test_wrap_copilot_subscription_uses_github_auth_without_provider_key(
    ...
    with (
        patch("cassandra.cli.wrap.shutil.which", return_value="copilot"),
        patch(
            "cassandra.cli.wrap.resolve_subscription_bearer_token_details",
...
def test_wrap_copilot_subscription_requires_reusable_auth(
    ...
    with (
        patch("cassandra.cli.wrap.shutil.which", return_value="copilot"),
        patch("cassandra.cli.wrap.resolve_subscription_bearer_token_details", return_value=None),
    ):
        result = runner.invoke(main, ["wrap", "copilot", "--subscription", "--no-rtk"])

    assert result.exit_code != 0
    assert "subscription mode requires a reusable GitHub/Copilot bearer token" in result.output
    assert "cassandra copilot-auth login" in result.output
...
def test_wrap_copilot_fails_when_binary_missing(
    ...
    _wrap_cli, main = wrap_modules
    with patch("cassandra.cli.wrap.shutil.which", return_value=None):
...
def test_unwrap_copilot_removes_rtk_instructions_and_stops_proxy(
    ...
    assert "Removed Cassandra rtk instructions from Copilot." in result.output
    assert "Stopped local Cassandra proxy on port 9999" in result.output
...
def test_unwrap_copilot_leaves_malformed_marker_content_unchanged(
    ...
    instructions = tmp_path / ".github" / "copilot-instructions.md"
    instructions.parent.mkdir()
    content = f"<!-- /cassandra:rtk-instructions -->\nKeep user guidance.\n{wrap_cli._RTK_MARKER}\n"
    ...
    assert "No Cassandra rtk instructions found for Copilot." in result.output
...
def test_wrap_copilot_oauth_keeps_generic_endpoint_when_account_advertised(
    ...
    with (
        patch("cassandra.cli.wrap.shutil.which", return_value="copilot"),
        patch("cassandra.cli.wrap.resolve_client_bearer_token", return_value="gho-oauth"),
        patch("cassandra.cli.wrap.has_oauth_auth", return_value=True),
        patch("cassandra.copilot_auth._fetch_copilot_user_info", return_value=_ACCOUNT_USER_INFO),
        patch("cassandra.cli.wrap._launch_tool", side_effect=fake_launch_tool),
    ):
...
def test_wrap_copilot_byok_never_resolves_copilot_endpoint(
    ...
    with (
        patch("cassandra.cli.wrap.shutil.which", return_value="copilot"),
        patch("cassandra.cli.wrap.has_oauth_auth", return_value=False),
        patch("cassandra.cli.wrap.resolve_copilot_api_url", side_effect=tripwire),
        patch("cassandra.cli.wrap._launch_tool", side_effect=fake_launch_tool),
    ):
        result = runner.invoke(
            main,
...
def test_resolve_copilot_api_url_ignores_user_info_and_never_calls_network(
    ...
    from cassandra import copilot_auth
