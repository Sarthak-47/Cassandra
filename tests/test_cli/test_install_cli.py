...
def test_install_apply_starts_service_supervisor(monkeypatch) -> None:
    ...
    monkeypatch.setattr("cassandra.cli.install.build_manifest", lambda **_: manifest)
    monkeypatch.setattr("cassandra.cli.install.load_manifest", lambda profile: None)
    monkeypatch.setattr("cassandra.cli.install.apply_mutations", lambda deployment: [])
    monkeypatch.setattr("cassandra.cli.install.install_supervisor", lambda deployment: [])
    monkeypatch.setattr(
        "cassandra.cli.install.save_manifest", lambda deployment: calls.append("save")
    )
    monkeypatch.setattr(
        "cassandra.cli.install.start_supervisor", lambda deployment: calls.append("start_service")
    )
    monkeypatch.setattr(
        "cassandra.cli.install.start_detached_agent", lambda profile: calls.append("start_agent")
    )
    monkeypatch.setattr(
        "cassandra.cli.install.start_persistent_docker",
        lambda deployment: calls.append("start_docker"),
    )
    monkeypatch.setattr(
        "cassandra.cli.install.wait_ready", lambda deployment, timeout_seconds=45: True
    )
...
def test_install_status_includes_backend_from_health_probe(monkeypatch) -> None:
    ...
    monkeypatch.setattr("cassandra.cli.install.load_manifest", lambda profile: Manifest())
    monkeypatch.setattr("cassandra.cli.install.runtime_status", lambda manifest: "running")
    monkeypatch.setattr("cassandra.cli.install.probe_ready", lambda url: True)
    monkeypatch.setattr(
        "cassandra.cli.install.probe_json",
...
def test_install_restart_uses_internal_helpers(monkeypatch) -> None:
    ...
    monkeypatch.setattr("cassandra.cli.install.load_manifest", lambda profile: Manifest())
    monkeypatch.setattr(
        "cassandra.cli.install.stop_supervisor", lambda manifest: calls.append("stop_supervisor")
    )
    monkeypatch.setattr(
        "cassandra.cli.install.stop_runtime", lambda manifest: calls.append("stop_runtime")
    )
    monkeypatch.setattr(
        "cassandra.cli.install.start_supervisor", lambda manifest: calls.append("start_supervisor")
    )
    monkeypatch.setattr(
        "cassandra.cli.install.wait_ready", lambda manifest, timeout_seconds=45: True
    )
...
def test_install_apply_accepts_opencode_target(monkeypatch) -> None:
    ...
    monkeypatch.setattr("cassandra.cli.install.build_manifest", fake_build_manifest)
    monkeypatch.setattr("cassandra.cli.install.load_manifest", lambda profile: None)
    monkeypatch.setattr("cassandra.cli.install.apply_mutations", lambda deployment: [])
    monkeypatch.setattr("cassandra.cli.install.install_supervisor", lambda deployment: [])
    monkeypatch.setattr("cassandra.cli.install.save_manifest", lambda deployment: None)
    monkeypatch.setattr("cassandra.cli.install.start_supervisor", lambda deployment: None)
    monkeypatch.setattr("cassandra.cli.install.start_detached_agent", lambda profile: None)
    monkeypatch.setattr(
        "cassandra.cli.install.wait_ready", lambda deployment, timeout_seconds=45: True
    )
...
def test_install_apply_restores_previous_deployment_after_failed_update(monkeypatch) -> None:
    ...
    monkeypatch.setattr("cassandra.cli.install.build_manifest", lambda **_: new_manifest)
    monkeypatch.setattr("cassandra.cli.install.load_manifest", lambda profile: existing_manifest)
    monkeypatch.setattr(
        "cassandra.cli.install.apply_mutations",
        lambda deployment: calls.append(f"apply:{','.join(deployment.targets)}") or [],
    )
    monkeypatch.setattr(
        "cassandra.cli.install.install_supervisor",
        lambda deployment: calls.append(f"supervisor:{','.join(deployment.targets)}") or [],
    )
    monkeypatch.setattr(
        "cassandra.cli.install.save_manifest",
        lambda deployment: calls.append(f"save:{','.join(deployment.targets)}"),
    )
    monkeypatch.setattr(
        "cassandra.cli.install.stop_supervisor",
        lambda deployment: calls.append(f"stop-supervisor:{','.join(deployment.targets)}"),
    )
    monkeypatch.setattr(
        "cassandra.cli.install.stop_runtime",
    ...
    monkeypatch.setattr(
        "cassandra.cli.install.revert_mutations",
        lambda deployment: calls.append(f"revert:{','.join(deployment.targets)}"),
    )
    monkeypatch.setattr(
        "cassandra.cli.install.delete_manifest",
    ...
    monkeypatch.setattr("cassandra.cli.install._start_deployment", _start)
...
def test_install_start_rejects_task_lifecycle(monkeypatch) -> None:
    ...
    assert result.exit_code != 0
    assert "cassandra install start" in result.output
...
def test_install_apply_uses_docker_runtime_for_persistent_docker(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "cassandra.cli.install.shutil.which",
...
def test_install_remove_continues_when_runtime_teardown_errors(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "cassandra.cli.install.delete_manifest", lambda profile: calls.append("delete")
    )
...
def test_install_agent_run_exits_with_foreground_status(monkeypatch) -> None:
    ...
    monkeypatch.setattr("cassandra.cli.install.load_manifest", lambda profile: Manifest())
    monkeypatch.setattr("cassandra.cli.install.run_foreground", lambda manifest: 7)
...
def test_install_agent_ensure_no_spawn_when_lock_not_acquired(monkeypatch) -> None:
    ...
    monkeypatch.setattr("cassandra.cli.install.load_manifest", lambda profile: Manifest())
    monkeypatch.setattr("cassandra.cli.install.probe_ready", lambda url: False)
    ...
    monkeypatch.setattr("cassandra.cli.install.acquire_runtime_start_lock", fake_lock)
    monkeypatch.setattr(
        "cassandra.cli.install.start_detached_agent",
...
def test_install_agent_ensure_stops_wedged_runtime_before_restart(monkeypatch) -> None:
    ...
    monkeypatch.setattr("cassandra.cli.install.probe_ready", lambda url: False)
    monkeypatch.setattr("cassandra.cli.install.runtime_status", lambda manifest: "running")
    monkeypatch.setattr("cassandra.cli.install.wait_ready", lambda manifest, timeout_seconds: False)
    monkeypatch.setattr("cassandra.cli.install.stop_runtime", lambda manifest: calls.append("stop"))
    monkeypatch.setattr(
        "cassandra.cli.install.start_detached_agent",
        lambda profile: calls.append("start_agent"),
    )
    ...
    monkeypatch.setattr("cassandra.cli.install.acquire_runtime_start_lock", fake_lock)
    monkeypatch.setattr(
        "cassandra.cli.install._start_deployment", lambda manifest: calls.append("start_deployment")
    )
...
def test_install_agent_ensure_starts_when_stopped_and_lock_acquired(monkeypatch) -> None:
    ...
    monkeypatch.setattr("cassandra.cli.install.load_manifest", lambda profile: Manifest())
    monkeypatch.setattr("cassandra.cli.install.probe_ready", lambda url: False)
    monkeypatch.setattr("cassandra.cli.install.runtime_status", lambda manifest: "stopped")
    monkeypatch.setattr(
        "cassandra.cli.install.start_detached_agent",
        lambda profile: calls.append("start_agent"),
    )
    ...
    monkeypatch.setattr("cassandra.cli.install.acquire_runtime_start_lock", fake_lock)
    monkeypatch.setattr("cassandra.cli.install.wait_ready", lambda manifest, timeout_seconds: True)
...
def test_install_agent_ensure_no_duplicate_spawn_after_lock_recheck(monkeypatch) -> None:
    ...
    monkeypatch.setattr("cassandra.cli.install.load_manifest", lambda profile: Manifest())
    monkeypatch.setattr("cassandra.cli.install.probe_ready", lambda url: next(probe_results))
...
def test_install_agent_ensure_propagates_start_deployment_failure(monkeypatch) -> None:
    ...
    monkeypatch.setattr("cassandra.cli.install._start_deployment", boom)
