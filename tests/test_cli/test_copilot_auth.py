...
def test_copilot_auth_login_saves_token(
    ...
    monkeypatch.setenv("CASSANDRA_COPILOT_AUTH_FILE", str(auth_file))
    monkeypatch.setattr(
        "cassandra.cli.copilot_auth.start_copilot_device_authorization",
        lambda domain: {
    ...
    monkeypatch.setattr(
        "cassandra.cli.copilot_auth.poll_copilot_device_authorization",
        lambda device_code, *, domain, interval, expires_in: "gho-cassandra",
    )
    ...
    assert "gho-cassandra" not in result.output
...
def test_copilot_auth_status_reports_missing_login(
    ...
    monkeypatch.setenv("CASSANDRA_COPILOT_AUTH_FILE", str(tmp_path / "missing.json"))
