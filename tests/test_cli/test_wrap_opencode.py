"""Tests for `cassandra wrap opencode` and `cassandra unwrap opencode`."""
...
def test_wrap_opencode_sets_config_content_env(
    ...
    """OPENCODE_CONFIG_CONTENT env var is set with the cassandra provider."""
    monkeypatch.chdir(tmp_path)
    monkeypatch.delenv("CASSANDRA_CONTEXT_TOOL", raising=False)
    ...
    assert config["provider"]["cassandra"]["options"]["baseURL"] == "http://127.0.0.1:9000/v1"
...
def test_wrap_opencode_no_mcp_skips_mcp_injection(
    ...
    assert "cassandra" not in persisted_config.get("mcp", {})
...
def test_unwrap_opencode_strips_blocks_when_no_backup(
    ...
    """Unwrap strips Cassandra blocks when no backup exists."""
    monkeypatch.chdir(tmp_path)
    _set_test_home(monkeypatch, tmp_path)
    ...
    assert result.exit_code == 0, result.output
    assert "Removed Cassandra block" in result.output
...
def test_wrap_opencode_preserves_existing_user_providers(
    ...
    """Wrap merges cassandra provider without disturbing user's existing providers."""
    monkeypatch.chdir(tmp_path)
    monkeypatch.delenv("CASSANDRA_CONTEXT_TOOL", raising=False)
    _set_test_home(monkeypatch, tmp_path)
    ...
    assert result.exit_code == 0, result.output
    config = json.loads(config_file.read_text(encoding="utf-8"))
    assert "cassandra" in config["provider"], "cassandra provider not injected"
...
def test_wrap_opencode_port_change_updates_existing_config(
    ...
    config_file = tmp_path / ".config" / "opencode" / "opencode.json"
    config = json.loads(config_file.read_text(encoding="utf-8"))
    assert config["provider"]["cassandra"]["options"]["baseURL"] == "http://127.0.0.1:9001/v1"
...
def test_wrap_opencode_handles_malformed_config_file(
    ...
    # The config file is now valid JSON with cassandra provider.
    config = json.loads(config_file.read_text(encoding="utf-8"))
...
def test_wrap_opencode_cassandra_project_from_cwd(
    runner: CliRunner,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """CASSANDRA_PROJECT is set based on the current working directory name."""
    ...
    assert result.exit_code == 0, result.output
    env = captured["env"]
    assert env.get("CASSANDRA_PROJECT") == "my-project"


def test_wrap_opencode_respects_existing_cassandra_project(
    runner: CliRunner,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """User-set CASSANDRA_PROJECT env var is preserved, not overridden."""
    monkeypatch.chdir(tmp_path)
    monkeypatch.delenv("CASSANDRA_CONTEXT_TOOL", raising=False)
    _set_test_home(monkeypatch, tmp_path)
    monkeypatch.setenv("CASSANDRA_PROJECT", "user-set-value")
    ...
    assert result.exit_code == 0, result.output
    env = captured["env"]
    assert env["CASSANDRA_PROJECT"] == "user-set-value"
...
def test_unwrap_opencode_removes_config_when_only_cassandra_content(
    runner: CliRunner,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Unwrap removes the config file entirely when it contained only Cassandra content."""
    monkeypatch.chdir(tmp_path)
    _set_test_home(monkeypatch, tmp_path)
...
def test_unwrap_opencode_noop_when_no_cassandra_markers(
    runner: CliRunner,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Unwrap is a safe no-op when the config has no Cassandra markers."""
    monkeypatch.chdir(tmp_path)
    _set_test_home(monkeypatch, tmp_path)
    ...
    assert result.exit_code == 0, result.output
    assert "no Cassandra wrap markers" in result.output
...
def test_wrap_unwrap_rewrap_is_idempotent(
    ...
    assert "cassandra" not in after_unwrap.get("provider", {})
    ...
    # After re-wrap, cassandra should be back, model unchanged
    after_rewrap = json.loads(config_file.read_text(encoding="utf-8"))
    assert after_rewrap["model"] == "openai/gpt-4o"
    assert "cassandra" in after_rewrap.get("provider", {})
...
def test_wrap_opencode_no_arguments_is_valid(
    ...
    """`cassandra wrap opencode` with no additional arguments is a valid command."""
    monkeypatch.chdir(tmp_path)
    monkeypatch.delenv("CASSANDRA_CONTEXT_TOOL", raising=False)
    _set_test_home(monkeypatch, tmp_path)
...
def test_unwrap_opencode_preserves_utf8_user_content(
    ...
    """Unwrap strips Cassandra blocks but preserves non-ASCII UTF-8 user content (#1126)."""
    monkeypatch.chdir(tmp_path)
    _set_test_home(monkeypatch, tmp_path)
    ...
    with patch.object(wrap_mod, "_stop_local_proxy_for_unwrap", return_value="stopped"):
        with patch("cassandra.mcp_registry.OpencodeRegistrar", return_value=fake_registrar):
            result = runner.invoke(main, ["unwrap", "opencode"])
