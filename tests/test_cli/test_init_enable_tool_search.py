"""`cassandra init claude` must set ENABLE_TOOL_SEARCH (GH #746).
...
`cassandra wrap claude` already sets it; `init` (and the persistent install) must
...
from cassandra.providers.claude import install as claude_install
