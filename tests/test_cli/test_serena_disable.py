...
Serena is installed by default, so a prior `cassandra wrap` persists a
`serena` MCP entry and the agent keeps launching it (dashboard popup and
all). These tests pin that a later `--no-serena` removes the entry Cassandra
...
def test_disable_preserves_user_managed_serena(
    ...
    monkeypatch.setenv("CASSANDRA_WORKSPACE_DIR", str(tmp_path / ".cassandra"))
    # Present in the agent config but NOT in Cassandra's ledger → user-owned.
...
def test_unwrap_codex_removes_cassandra_installed_serena(
    ...
    with (
        patch("cassandra.mcp_registry.CodexRegistrar", return_value=registrar),
        patch(
            "cassandra.cli.wrap._restore_codex_provider_config",
    ...
    assert registrar.unregistered == ["serena"]
    assert "Removed Cassandra-installed Serena MCP server from Codex" in result.output
