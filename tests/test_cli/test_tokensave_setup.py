...
from cassandra.mcp_registry import build_tokensave_spec
...
def test_setup_registers_and_records_when_binary_available(
    ...
    # Ledger now proves Cassandra owns the entry.
    assert cassandra_installed_matching("claude", registrar.get_server("tokensave"))
...
def test_setup_migrates_stale_cassandra_entry(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(wrap_cli, "_ensure_tokensave_binary", lambda verbose=False: _FAKE_BIN)
    # A stale Cassandra-installed entry (different binary path) is on disk.
...
def test_disable_removes_cassandra_installed(capsys: pytest.CaptureFixture[str]) -> None:
