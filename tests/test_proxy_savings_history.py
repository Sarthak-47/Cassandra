...
import cassandra.proxy.savings_tracker as savings_tracker_module
from cassandra.proxy.savings_tracker import CASSANDRA_SAVINGS_PATH_ENV_VAR, SavingsTracker
...
def test_savings_tracker_helpers_normalize_inputs_and_paths(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv(CASSANDRA_SAVINGS_PATH_ENV_VAR, str(override_path))
    assert savings_tracker_module.get_default_savings_storage_path() == str(override_path)

    monkeypatch.delenv(CASSANDRA_SAVINGS_PATH_ENV_VAR, raising=False)
    default_path = savings_tracker_module.get_default_savings_storage_path()
    assert Path(default_path).as_posix().endswith(".cassandra/proxy_savings.json")
...
def test_savings_tracker_rollups_preserve_spend_and_input_history(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "cassandra.proxy.savings_tracker._estimate_compression_savings_usd",
        lambda model, tokens_saved: tokens_saved / 1000.0,
    )
...
def test_savings_tracker_rollup_attributes_savings_per_model(tmp_path, monkeypatch):
    ...
    # The expected no-cassandra cost is derivable per bucket: actual input cost
...
def test_stats_history_persists_across_restarts_and_stats_stays_compatible(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CASSANDRA_SAVINGS_PATH", str(savings_path))
    monkeypatch.setattr(
        "cassandra.proxy.server.CostTracker._get_cache_prices",
...
def test_stats_history_csv_export_is_frontend_friendly(tmp_path, monkeypatch):
    ...
    with TestClient(create_app(config)) as client:
        ...
        assert (
            'attachment; filename="cassandra-stats-history-daily.csv"'
...
def test_dashboard_includes_history_toggle_and_endpoint(tmp_path, monkeypatch):
    ...
    with TestClient(create_app(config)) as client:
        ...
        assert "Expected cost (without Cassandra)" in html
