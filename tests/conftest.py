"""Shared pytest fixtures for Cassandra tests."""
...
def _reset_cassandra_logger_propagation():
    """Keep `cassandra.*` log records flowing to pytest's caplog handler.

    Two sources disable propagation on the cassandra logger tree and never
    ...
    - ``cassandra.proxy.helpers._setup_file_logging`` sets
      ``getLogger("cassandra").propagate = False`` on proxy startup.
    - ``benchmarks.claude_session_mode_benchmark._disable_cassandra_benchmark_logging``
      (exercised by ``test_claude_session_mode_benchmark``) sets
      ``propagate = False`` + ``CRITICAL`` on ``cassandra``, ``cassandra.proxy``,
      ``cassandra.transforms``, ``cassandra.cache`` (and children).

    Resetting only ``"cassandra"`` is not enough — a child like
    ``"cassandra.proxy"`` left non-propagating blocks the record before it
    reaches root. Reset the whole subtree before every test so capture is
    deterministic regardless of run order.
    """
    import logging as _logging

    for _name in ("cassandra", *list(_logging.root.manager.loggerDict)):
        if _name == "cassandra" or _name.startswith("cassandra."):
...
def openai_tokenizer():
    ...
    from cassandra.providers.openai import OpenAITokenCounter
...
def sample_request_metrics():
    ...
    from cassandra.config import RequestMetrics
