...
`x-cassandra-*` request headers are internal control flags consumed by the
...
for bypass gating, `_extract_tags` reading `x-cassandra-*`) keep working
because they never depended on the local outbound-bound dict.

Operator opt-in `CASSANDRA_STRIP_INTERNAL_HEADERS=disabled` keeps the
...
from cassandra.proxy.server import ProxyConfig, create_app
...
def test_strip_returns_new_dict_does_not_mutate_caller() -> None:
    ...
    original = {
        ...
        "x-cassandra-bypass": "true",
    }
    out = _strip_internal_headers(original)
    assert out is not original
    assert "x-cassandra-bypass" in original
    assert "x-cassandra-bypass" not in out
    assert out["authorization"] == "Bearer x"


def test_strip_x_cassandra_bypass_removed() -> None:
    out = _strip_internal_headers({"x-cassandra-bypass": "true", "k": "v"})
    assert "x-cassandra-bypass" not in out
    assert out["k"] == "v"


def test_strip_x_cassandra_mode_removed() -> None:
    out = _strip_internal_headers({"x-cassandra-mode": "passthrough", "k": "v"})
    assert "x-cassandra-mode" not in out
    assert out["k"] == "v"


def test_strip_x_cassandra_user_id_removed() -> None:
    out = _strip_internal_headers({"x-cassandra-user-id": "u1", "k": "v"})
    assert "x-cassandra-user-id" not in out
    assert out["k"] == "v"


def test_strip_x_cassandra_stack_removed() -> None:
    out = _strip_internal_headers({"x-cassandra-stack": "engineer", "k": "v"})
    assert "x-cassandra-stack" not in out
    assert out["k"] == "v"


def test_strip_x_cassandra_base_url_removed() -> None:
    out = _strip_internal_headers({"x-cassandra-base-url": "http://x", "k": "v"})
    assert "x-cassandra-base-url" not in out
    assert out["k"] == "v"


def test_strip_case_insensitive_prefix_match() -> None:
    """Mixed-case `X-Cassandra-Foo`, `x-Cassandra-Bar`, `X-CASSANDRA-BAZ` all stripped."""
    ...
    assert "X-Cassandra-Foo" not in out
    assert "x-Cassandra-Bar" not in out
    assert "X-CASSANDRA-BAZ" not in out
    assert out["Authorization"] == "Bearer x"
...
def test_strip_disabled_mode_passes_internal_headers_through(
    ...
    """`CASSANDRA_STRIP_INTERNAL_HEADERS=disabled` is operator opt-in for diag."""
    monkeypatch.setenv("CASSANDRA_STRIP_INTERNAL_HEADERS", "disabled")
    out = _strip_internal_headers({"x-cassandra-bypass": "true", "authorization": "Bearer x"})
    assert out["x-cassandra-bypass"] == "true"
    assert out["authorization"] == "Bearer x"
...
# ---------------------------------------------------------------------------
# End-to-end: x-cassandra-* never reaches the upstream
# ---------------------------------------------------------------------------
...
def test_x_cassandra_bypass_not_forwarded() -> None:
    ...
    assert "x-cassandra-bypass" not in upstream
...
def test_x_cassandra_mode_not_forwarded() -> None:
    client, transport = _make_anthropic_app()
    resp = client.post(
        "/v1/messages",
        headers={
    ...
    assert transport.captured_headers is not None
    upstream = {k.lower(): v for k, v in transport.captured_headers.items()}
    assert "x-cassandra-mode" not in upstream


def test_x_cassandra_user_id_not_forwarded() -> None:
    client, transport = _make_anthropic_app()
    resp = client.post(
        "/v1/messages",
        headers={
    ...
    assert transport.captured_headers is not None
    upstream = {k.lower(): v for k, v in transport.captured_headers.items()}
    assert "x-cassandra-user-id" not in upstream


def test_x_cassandra_stack_not_forwarded() -> None:
    client, transport = _make_anthropic_app()
    resp = client.post(
        "/v1/messages",
        headers={
    ...
    assert transport.captured_headers is not None
    upstream = {k.lower(): v for k, v in transport.captured_headers.items()}
    assert "x-cassandra-stack" not in upstream


def test_x_cassandra_base_url_not_forwarded() -> None:
    client, transport = _make_anthropic_app()
    resp = client.post(
        "/v1/messages",
        headers={
            ...
            "anthropic-version": "2023-06-01",
            "content-type": "application/json",
            "x-cassandra-base-url": "https://override.example",
        },
        json={
            "model": "claude-sonnet-4-6",
            "max_tokens": 16,
    ...
    assert transport.captured_headers is not None
    upstream = {k.lower(): v for k, v in transport.captured_headers.items()}
    assert "x-cassandra-base-url" not in upstream
...
def test_case_insensitive_prefix_match_e2e() -> None:
    ...
    assert "x-cassandra-foo" not in upstream_lower
    assert "x-cassandra-bar" not in upstream_lower
    assert "x-cassandra-baz" not in upstream_lower
...
def test_inbound_read_path_still_reads_x_cassandra_bypass() -> None:
    """Bypass header still gates compression even though it's stripped from upstream.

    The handler reads `request.headers.get('x-cassandra-bypass')` directly.
    ...
    #   2. The upstream did NOT receive the `x-cassandra-bypass` header.
    resp = client.post(
        "/v1/messages",
        headers={
            "x-api-key": "test-key",
...
def test_disabled_mode_passes_through_e2e(
    ...
    """`CASSANDRA_STRIP_INTERNAL_HEADERS=disabled` lets internal headers through."""
    monkeypatch.setenv("CASSANDRA_STRIP_INTERNAL_HEADERS", "disabled")
    client, transport = _make_anthropic_app()
    resp = client.post(
        "/v1/messages",
    ...
    assert upstream.get("x-cassandra-mode") == "passthrough"
...
def test_openai_chat_x_cassandra_bypass_not_forwarded() -> None:
    """OpenAI handler also strips x-cassandra-* before upstream call."""
    config = ProxyConfig(
        optimize=False,
        cache_enabled=False,
        rate_limit_enabled=False,
    ...
    assert "x-cassandra-bypass" not in sent_headers
    assert "x-cassandra-user-id" not in sent_headers
