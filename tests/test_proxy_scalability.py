...
class TestWorkerConfiguration:
    ...
    def test_run_server_uses_import_string_for_multiple_workers(self, monkeypatch):
        ...
        from cassandra.proxy.server import _MULTI_WORKER_CONFIG_ENV, run_server
        ...
        try:
            with patch("cassandra.proxy.server.uvicorn.run", fake_run):
                run_server(config, workers=4, limit_concurrency=250)

            assert captured["app"] == "cassandra.proxy.server:create_app_from_env"
            ...
            # later tests (e.g. _proxy_config_from_env then ignores CASSANDRA_*).
            os.environ.pop(_MULTI_WORKER_CONFIG_ENV, None)

    def test_run_server_uses_selector_loop_on_windows(self, monkeypatch):
        from cassandra.proxy import server as server_mod
        from cassandra.proxy.models import ProxyConfig
