...
from cassandra.providers.codex import threads
...
def test_retag_one_moves_only_matching_provider(tmp_path: Path) -> None:
    ...
    _seed(db, [("a", "openai"), ("b", "openai"), ("c", "cassandra"), ("d", "anthropic")])

    moved = threads._retag_one(db, frm="openai", to="cassandra")
    assert moved == 2
    assert _count(db, "openai") == 0
    assert _count(db, "cassandra") == 3
    # Third-party providers are left alone.
    assert _count(db, "anthropic") == 1

    back = threads._retag_one(db, frm="cassandra", to="openai")
    assert back == 3
    assert _count(db, "cassandra") == 0
...
def test_retag_one_noop_without_threads_table(tmp_path: Path) -> None:
    ...
    assert threads._retag_one(db, frm="openai", to="cassandra") == 0
...
def test_retag_thread_providers_silent_when_no_store(tmp_path: Path) -> None:
    ...
    threads.retag_thread_providers(tmp_path, frm="openai", to="cassandra")
...
def test_enable_disable_wrappers_retag_expected_direction(tmp_path: Path) -> None:
    db = tmp_path / "state_5.sqlite"
    _seed(db, [("a", "openai"), ("b", "cassandra")])

    threads.retag_to_cassandra(tmp_path)
    assert _count(db, "cassandra") == 2
    assert _count(db, "openai") == 0
