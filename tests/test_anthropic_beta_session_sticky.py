...
def test_merge_helper_only_cassandra() -> None:
    assert merge_anthropic_beta(None, ["a", "b"]) == "a,b"


def test_merge_helper_preserves_client_order_and_appends_cassandra() -> None:
    # Client tokens FIRST in their original order, cassandra AFTER in passed order.
    assert (
        merge_anthropic_beta("client-1,client-2", ["cassandra-a", "cassandra-b"])
        == "client-1,client-2,cassandra-a,cassandra-b"
    )


def test_dedupe_case_insensitive_preserves_first_casing() -> None:
    # Client casing wins over cassandra casing.
    assert (
...
def test_merge_helper_no_double_inject_when_already_present() -> None:
    # Cassandra token already in client value → not re-appended.
    assert (
        merge_anthropic_beta("context-management-2025-06-27", ["context-management-2025-06-27"])
        == "context-management-2025-06-27"
    )
...
def _isolate_tracker(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv("CASSANDRA_BETA_HEADER_STICKY", raising=False)
    monkeypatch.delenv("CASSANDRA_BETA_TRACKER_MAX_SESSIONS", raising=False)
    _reset_session_beta_tracker_for_test()
    yield
    _reset_session_beta_tracker_for_test()
...
def test_client_value_preserved_when_no_injection() -> None:
    """First-turn client value flows through unchanged when no cassandra adds."""
    tracker = SessionBetaTracker(max_sessions=10)
...
def test_disabled_mode_passes_through(monkeypatch: pytest.MonkeyPatch) -> None:
    """`CASSANDRA_BETA_HEADER_STICKY=disabled` returns client value verbatim."""
    monkeypatch.setenv("CASSANDRA_BETA_HEADER_STICKY", "disabled")
    tracker = SessionBetaTracker(max_sessions=10)
...
def test_disabled_mode_invalid_value_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("CASSANDRA_BETA_HEADER_STICKY", "yolo")
    with pytest.raises(ValueError, match="CASSANDRA_BETA_HEADER_STICKY"):
        get_beta_header_sticky_mode()
...
def test_max_sessions_env_var_custom(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("CASSANDRA_BETA_TRACKER_MAX_SESSIONS", "42")
    assert get_beta_tracker_max_sessions() == 42


def test_max_sessions_env_var_invalid_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("CASSANDRA_BETA_TRACKER_MAX_SESSIONS", "0")
    with pytest.raises(ValueError):
        get_beta_tracker_max_sessions()
    monkeypatch.setenv("CASSANDRA_BETA_TRACKER_MAX_SESSIONS", "-3")
    with pytest.raises(ValueError):
        get_beta_tracker_max_sessions()
    monkeypatch.setenv("CASSANDRA_BETA_TRACKER_MAX_SESSIONS", "not-int")
    with pytest.raises(ValueError):
        get_beta_tracker_max_sessions()
...
def test_memory_injection_appends_deterministic_order() -> None:
    ...
    new merge helper. Order is client first, cassandra token after.
    """
