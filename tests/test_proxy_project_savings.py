"""Tests for per-project savings attribution (X-Cassandra-Project)."""
...
from cassandra.proxy.outcome import RequestOutcome, emit_request_outcome  # noqa: E402
from cassandra.proxy.project_context import (  # noqa: E402
    classify_project,
    get_current_project,
    set_current_project,
    split_project_path,
...
from cassandra.proxy.savings_tracker import (  # noqa: E402
...
def test_classify_project_reads_header_case_insensitively():
    assert classify_project({"x-cassandra-project": "frontend"}) == "frontend"
    assert classify_project({"X-Cassandra-Project": " frontend "}) == "frontend"
...
def test_funnel_attributes_savings_from_context_and_stats_exposes_them(tmp_path, monkeypatch):
    monkeypatch.setenv("CASSANDRA_SAVINGS_PATH", str(tmp_path / "savings.json"))
...
def test_middleware_binds_project_header_to_context(tmp_path, monkeypatch):
    ...
    import cassandra.proxy.server as server_module
    ...
    with TestClient(create_app(config)) as client:
        ...
        assert (
            client.get(
                "/p/prefix-project/health", headers={"X-Cassandra-Project": "header-project"}
            ).status_code
            == 200
        )
