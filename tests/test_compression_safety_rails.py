...
3. Library inflation guard — ``cassandra.compress()`` reverts to the
...
from cassandra.config import CassandraConfig, TransformResult
from cassandra.tokenizer import Tokenizer as TokenizerType
...
# ``cassandra.compress`` the submodule is shadowed by the function of the
# same name re-exported in ``cassandra/__init__.py``.
compress_module = importlib.import_module("cassandra.compress")
...
class TestPipelineCircuitBreaker:
    def test_opens_after_threshold_and_passes_through(self) -> None:
        pipeline = TransformPipeline(CassandraConfig(), transforms=[_FailingTransform()])
        for _ in range(3):
    ...
    def test_success_resets_consecutive_failures(self) -> None:
        ...
        pipeline = TransformPipeline(CassandraConfig(), transforms=[flaky])
        for _ in range(2):
            with pytest.raises(RuntimeError):
                pipeline.apply(_MESSAGES, model="gpt-4o", model_limit=1024)
    ...
    def test_cooldown_expiry_closes_breaker(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("CASSANDRA_PIPELINE_BREAKER_COOLDOWN_S", "0.05")
    ...
    def test_invalid_env_values_fall_back_to_defaults(
        ...
        monkeypatch.setenv("CASSANDRA_PIPELINE_BREAKER_THRESHOLD", "three")
        monkeypatch.setenv("CASSANDRA_PIPELINE_BREAKER_COOLDOWN_S", "1m")
        pipeline = TransformPipeline(CassandraConfig(), transforms=[_FailingTransform()])
    ...
    def test_disabled_via_env(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("CASSANDRA_PIPELINE_BREAKER_THRESHOLD", "0")
        pipeline = TransformPipeline(CassandraConfig(), transforms=[_FailingTransform()])
        for _ in range(5):
            with pytest.raises(RuntimeError):
                pipeline.apply(_MESSAGES, model="gpt-4o", model_limit=1024)
