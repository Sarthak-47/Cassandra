"""Tests for :class:`cassandra.proxy.image_compression_decision.ImageCompressionDecision`.
...
def _config(*, image_optimize: bool = True) -> Any:
    """Minimal stand-in for ``CassandraConfig`` — only the field the
    decision reads."""
...
def test_passthrough_mode_header_also_triggers_bypass_skip() -> None:
    """``x-cassandra-mode: passthrough`` alt spelling — mirrors
    ``_cassandra_bypass_enabled`` semantics across all proxy gates."""
    d = ImageCompressionDecision.decide(
...
def test_apply_to_tags_stamps_reason_when_passthrough() -> None:
    d = ImageCompressionDecision.decide(
        headers={"x-cassandra-bypass": "true"}, config=_config(), messages=_msgs()
    )
    tags: dict[str, str] = {}
