...
def test_batch_context_defaults_and_expiry(monkeypatch) -> None:
    monkeypatch.setattr("cassandra.ccr.batch_store.time.time", lambda: 100.0)
    ...
    monkeypatch.setattr("cassandra.ccr.batch_store.time.time", lambda: context.expires_at + 1)
    assert context.is_expired is True


async def test_batch_context_store_core_operations(monkeypatch) -> None:
    now = {"value": 100.0}
    monkeypatch.setattr("cassandra.ccr.batch_store.time.time", lambda: now["value"])
