"""Eval: Does the LLM invoke cassandra_retrieve when summaries are present?

The REAL test — it's not enough for the LLM to know something is missing.
It must actually call the tool to fetch it.

Compares:
- WITH summary: LLM sees "2 failed, 1 error" → should call cassandra_retrieve
...
# The cassandra_retrieve tool definition (same as what CCR injects)
CASSANDRA_RETRIEVE_TOOL = {
    "name": "cassandra_retrieve",
    "description": (
        "Retrieve original uncompressed content from Cassandra's compression cache. "
...
class TestToolInvocationWithSummary:
    """The real eval: does the LLM call cassandra_retrieve?"""

    def test_with_summary_triggers_tool_call(self):
        """WITH compression summary → LLM should call cassandra_retrieve."""
        test_results = _make_test_results(100)
        kept = test_results[:10]  # All passing

        from cassandra.transforms.compression_summary import summarize_dropped_items
        ...
        compressed += (
            ...
            f' Retrieve specific items: cassandra_retrieve(hash="ccr_test_abc123", query="your search")]'
        )
        ...
        resp = _call_claude_with_tools(messages, [CASSANDRA_RETRIEVE_TOOL])
        ...
        if stop_reason == "tool_use":
            ...
            call = tool_calls[0]
            assert call["name"] == "cassandra_retrieve"
            ...
            print("  RESULT: LLM invoked cassandra_retrieve with relevant query ✓")
        else:
            ...
            mentions_retrieval = any(
                term in text.lower()
                for term in ["retrieve", "cassandra_retrieve", "fetch", "see more", "compressed"]
            )
    ...
    def get_balance(self) -> float:
        # [2 lines omitted]
        pass
# [180 tokens compressed. removed: def charge (12 lines), def refund (6 lines). Retrieve full code: cassandra_retrieve(hash="ccr_code_xyz", query="function name")]'''
