...
from cassandra.backends.litellm import (  # noqa: E402  (must follow importorskip)
    _VERTEX_MODEL_MAP,
    LiteLLMBackend,
...
class TestLiteLLMToolsForwarding:
    ...
    async def test_tools_forwarded_in_send_message(self):
        ...
        with (
            patch("cassandra.backends.litellm.acompletion", new_callable=AsyncMock) as mock_acomp,
            patch("cassandra.backends.litellm._fetch_bedrock_inference_profiles", return_value={}),
        ):
...
class TestConvertMessagesToolBlocks:
    ...
    def _make_backend(self):
        with patch("cassandra.backends.litellm._fetch_bedrock_inference_profiles", return_value={}):
...
class TestOpenAIURLNormalization:
    ...
    def test_v1_suffix_stripped(self):
        ...
        original = CassandraProxy.OPENAI_API_URL
        try:
            config = ProxyConfig(
            ...
            CassandraProxy.OPENAI_API_URL = original
