...
Requires the rebuilt `cassandra._core` extension — these tests fail loudly
...
from cassandra.transforms.search_compressor import SearchCompressor, SearchCompressorConfig
...
class TestSmartCrusherConfigExposure:
    ...
    def test_rust_default_lockstep(self):
        ...
        from cassandra._core import SmartCrusherConfig as RustConfig
    ...
    def test_foreign_config_object_tolerated(self):
        """cassandra.config.SmartCrusherConfig (the SDK-surface class) is
        structurally similar and flows through getattr fallbacks."""
        from cassandra.config import SmartCrusherConfig as SdkConfig
