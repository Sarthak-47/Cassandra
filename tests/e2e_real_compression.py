...
Cassandra only compresses content the model has already seen — assistant
...
def main() -> int:
    ...
    proc = subprocess.Popen(
        [
            str(REPO_ROOT / ".venv/bin/cassandra"),
            "proxy",
            "--port",
            str(port),
            "--no-telemetry",
        ],
        env={**os.environ, "CASSANDRA_REQUIRE_RUST_CORE": "true"},
        stdout=log_fp,
    ...
    try:
        ...
        time.sleep(1.5)
        canonical_log = Path.home() / ".cassandra" / "logs" / "proxy.log"
