...
from cassandra.proxy.interceptors import (
    INTERCEPTORS,
...
from cassandra.proxy.interceptors.astgrep import AstGrepReadOutline
from cassandra.proxy.interceptors.base import reset_interceptor_failure_counts
...
def test_proxy_pipeline_includes_interceptor_when_env_enabled(monkeypatch):
    """When CASSANDRA_INTERCEPT_ENABLED=1, ToolResultInterceptorTransform is at index 0 in both pipelines."""
    monkeypatch.setenv("CASSANDRA_INTERCEPT_ENABLED", "1")
    ...
    proxy = CassandraProxy(ProxyConfig())
...
def test_proxy_pipeline_excludes_interceptor_when_env_not_set(monkeypatch):
    """When CASSANDRA_INTERCEPT_ENABLED is unset, no interceptor in either pipeline."""
    monkeypatch.delenv("CASSANDRA_INTERCEPT_ENABLED", raising=False)
    from cassandra.proxy.interceptors import ToolResultInterceptorTransform
    from cassandra.proxy.models import ProxyConfig
    from cassandra.proxy.server import CassandraProxy
