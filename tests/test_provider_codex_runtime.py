...
from cassandra.cli import init as init_cli
from cassandra.install.models import ConfigScope, DeploymentManifest
from cassandra.providers.codex import build_launch_env, proxy_base_url
from cassandra.providers.codex.install import apply_provider_scope, build_install_env
...
def codex_proxy_stack(tmp_path_factory: pytest.TempPathFactory) -> Iterator[_CodexProxyStack]:
    ...
    previous_env = {name: os.environ.get(name) for name in ("CASSANDRA_REQUIRE_RUST_CORE", "HOME")}
    os.environ["CASSANDRA_REQUIRE_RUST_CORE"] = "false"
...
def _assert_delivery(
    ...
    *,
    ...
    while time.time() < deadline:
        ...
        raise AssertionError(f"Cassandra never recorded model {model!r} in /stats")
...
def _codex_base_url_from_config(path: Path) -> str:
    ...
    return str(parsed["model_providers"]["cassandra"]["base_url"])
...
def test_codex_launch_env_routes_messages_through_cassandra(
    ...
    _assert_delivery(
        ...
        content="Verify temporary launch env traffic reaches Cassandra.",
    )


def test_codex_install_env_routes_messages_through_cassandra(
    codex_proxy_stack: _CodexProxyStack,
) -> None:
    ...
    _assert_delivery(
        ...
        content="Verify persistent install env traffic reaches Cassandra.",
    )


def test_init_codex_config_routes_messages_through_cassandra(
    ...
    _assert_delivery(
        ...
        content="Verify init-generated Codex config sends traffic to Cassandra.",
    )


def test_provider_scope_codex_config_routes_messages_through_cassandra(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    codex_proxy_stack: _CodexProxyStack,
) -> None:
    ...
    _assert_delivery(
        ...
        content="Verify persistent provider config sends traffic to Cassandra.",
    )
