"""Regression tests for UTF-8 decoding/encoding of cassandra-owned assets.

These guard against ``UnicodeDecodeError`` on systems whose default text
encoding is not UTF-8 (e.g. Windows ``cp949``/``cp1252`` locales). Cassandra
...
from cassandra.dashboard import TEMPLATES_DIR, get_dashboard_html
from cassandra.memory.sync import _load_sync_state, _save_sync_state
