...
from cassandra.memory.backends.local import LocalBackendConfig
...
def test_resolver_tier1_explicit_project_id_wins() -> None:
    ...
    out = r.resolve(
        _ctx(
            headers={"x-cassandra-project-id": "billing-svc"},
...
def test_resolver_tier2_explicit_cwd_header() -> None:
    r = ProjectResolver()
    out = r.resolve(_ctx(headers={"x-cassandra-cwd": "/Users/foo/code/project-b"}))
    assert out is not None
    key, display = out
...
def test_resolver_tier4_env_block_primary_working_directory() -> None:
    ...
    prompt = (
        ...
        " - Primary working directory: /Users/foo/code/cassandra\n"
        " - Is a git repo: yes\n"
    )
    out = r.resolve(_ctx(system_prompt=prompt))
    assert out is not None
    _, display = out
    assert display == "cassandra"
...
def test_resolver_same_cwd_yields_stable_key_across_calls() -> None:
    r = ProjectResolver()
    k1, _ = r.resolve(_ctx(headers={"x-cassandra-cwd": "/Users/foo/code/x"}))  # type: ignore[misc]
    k2, _ = r.resolve(_ctx(headers={"x-cassandra-cwd": "/Users/foo/code/x"}))  # type: ignore[misc]
    assert k1 == k2


def test_resolver_distinct_cwds_yield_distinct_keys() -> None:
    r = ProjectResolver()
    k1, _ = r.resolve(_ctx(headers={"x-cassandra-cwd": "/Users/foo/code/a"}))  # type: ignore[misc]
    k2, _ = r.resolve(_ctx(headers={"x-cassandra-cwd": "/Users/foo/code/b"}))  # type: ignore[misc]
    assert k1 != k2


def test_resolver_sanitises_unsafe_basename_chars() -> None:
    r = ProjectResolver()
    out = r.resolve(_ctx(headers={"x-cassandra-project-id": "../etc/passwd; rm -rf /"}))
    assert out is not None
    key, _ = out
...
def _make_router(
    ...
    monkeypatch.setattr(
        "cassandra.memory.storage_router.LocalBackend",
        _FakeBackend,
    )
...
def test_router_project_mode_two_cwds_two_paths(
    ...
    ctx_a = _ctx(headers={"x-cassandra-cwd": "/code/a"})
    ctx_b = _ctx(headers={"x-cassandra-cwd": "/code/b"})
...
def test_router_global_mode_reuses_legacy_path(
    ...
    _, scope = router.backend_for(_ctx(headers={"x-cassandra-cwd": "/code/anything"}))
    assert scope.mode is MemoryStorageMode.GLOBAL
...
def test_router_backend_cache_returns_same_instance(
    ...
    ctx = _ctx(headers={"x-cassandra-cwd": "/code/sticky"})
...
def test_router_lru_eviction_drops_oldest(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    router = _make_router(tmp_path, MemoryStorageMode.PROJECT, monkeypatch)
    for i in range(5):
        router.backend_for(_ctx(headers={"x-cassandra-cwd": f"/code/p{i}"}))
