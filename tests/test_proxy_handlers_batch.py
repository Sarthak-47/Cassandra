...
from cassandra.proxy.handlers import batch as batch_module
...
class DummyBatchHandler(batch_module.BatchHandlerMixin):
    ...
    async def _record_request_outcome(self, outcome) -> None:  # noqa: ANN001
        # Mirror of CassandraProxy._record_request_outcome for the batch
    ...
    def _extract_tags(self, headers: dict) -> dict[str, str]:
        # Mirror of CassandraProxy._extract_tags. Handlers now call this
        # at entry to capture x-cassandra-* slicing tags into the outcome.
        return {
...
def install_batch_support_modules(
    ...
    *,
    ...
    monkeypatch.setitem(sys.modules, "cassandra.ccr", SimpleNamespace(CCRToolInjector=FakeInjector))
    monkeypatch.setitem(
        sys.modules,
        "cassandra.tokenizers",
...
async def test_handle_batch_create_validates_json_and_required_fields(
    ...
    monkeypatch.setattr("cassandra.proxy.helpers._read_request_json", raise_bad_json)
    ...
    monkeypatch.setattr("cassandra.proxy.helpers._read_request_json", missing_file_payload)
    ...
    monkeypatch.setattr("cassandra.proxy.helpers._read_request_json", missing_endpoint_payload)
...
async def test_handle_batch_create_passthrough_and_download_failure(
    ...
    monkeypatch.setattr("cassandra.proxy.helpers._read_request_json", passthrough_payload)
    ...
    monkeypatch.setattr("cassandra.proxy.helpers._read_request_json", download_missing_payload)
...
async def test_handle_batch_create_handles_empty_upload_failure_and_success(
    ...
    monkeypatch.setattr("cassandra.proxy.helpers._read_request_json", request_payload)
    ...
    assert success_headers["x-cassandra-tokens-saved"] == "10"
    assert success_headers["x-cassandra-savings-percent"] == "50.0"
    ...
    assert sent_body["metadata"]["cassandra_compressed"] == "true"
    assert sent_body["metadata"]["cassandra_original_file_id"] == "file-1"
...
async def test_handle_google_batch_create_validates_and_passthroughs(
    ...
    monkeypatch.setattr("cassandra.proxy.helpers._read_request_json", bad_json)
    ...
    monkeypatch.setattr("cassandra.proxy.helpers._read_request_json", no_inline)
...
async def test_handle_google_batch_create_success_and_failure_paths(
    ...
    monkeypatch.setattr("cassandra.proxy.helpers._read_request_json", good_payload)
...
async def test_handle_google_batch_create_covers_passthrough_revert_and_store_failures(
    ...
    monkeypatch.setattr("cassandra.proxy.helpers._read_request_json", payload)
