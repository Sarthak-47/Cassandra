"""Tests for CassandraClient cache optimizer integration."""
...
from cassandra.cache.base import CacheMetrics, CacheResult
...
class TestCassandraClientCacheIntegration:
    """Test CassandraClient cache optimizer integration."""
...
class TestCacheOptimizerInvocation:
    ...
    @patch("cassandra.storage.sqlite.SQLiteStorage.save")
    def test_optimizer_optimize_is_called_during_chat(self, mock_save, temp_db):
        ...
        """
        from cassandra import CassandraMode
        ...
        client.chat.completions.create(
            ...
            messages=messages,
            max_tokens=100,
            cassandra_mode=CassandraMode.OPTIMIZE,
        )
    ...
    def test_optimizer_transforms_applied_in_response(self, mock_save, temp_db):
        ...
        assert hasattr(result, "cassandra"), "Response should have cassandra metadata"
        cassandra_meta = result.cassandra

        # Check that cache optimizer was reported
        assert cassandra_meta.cache_optimizer_used is not None or any(
            "cache_optimizer" in t for t in (cassandra_meta.transforms_applied or [])
    ...
    def test_optimizer_not_called_in_audit_mode(self, mock_save, temp_db):
        ...
        client.chat.completions.create(
            ...
            messages=messages,
            max_tokens=100,
            cassandra_mode=CassandraMode.AUDIT,
        )
...
class TestSemanticCacheIntegration:
    """Test semantic cache integration with CassandraClient.
    ...
    def test_semantic_cache_hit_returns_cached_response_without_api_call(self, mock_save, temp_db):
        ...
        if client._semantic_cache_layer is not None:
            from cassandra.cache import OptimizationContext
...
class TestSessionStatsTracking:
    """Test session statistics tracking in CassandraClient.
