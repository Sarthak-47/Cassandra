"""Unit tests for cassandra.binaries — the lazy fetcher for bundled CLI tools.
...
from cassandra import binaries
...
def _clear_caches(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("CASSANDRA_BINARIES_CACHE", str(tmp_path / "cache"))
    monkeypatch.delenv("CASSANDRA_BINARIES_MIRROR", raising=False)
    monkeypatch.delenv("CASSANDRA_BINARIES_OFFLINE", raising=False)
    yield
    binaries.detect_platform.cache_clear()
    binaries._registry.cache_clear()
...
def test_cache_dir_respects_env_override(monkeypatch, tmp_path):
    monkeypatch.setenv("CASSANDRA_BINARIES_CACHE", str(tmp_path / "custom"))
...
def test_pypi_only_tool_raises_with_helpful_message(monkeypatch):
    ...
    assert "pip install cassandra-ai" in str(exc.value)
...
def test_offline_error_when_fetch_required(monkeypatch):
    _set_platform(monkeypatch, sys_plat="darwin", machine="arm64")
    monkeypatch.setattr(binaries.shutil, "which", lambda _name: None)
    monkeypatch.setenv("CASSANDRA_BINARIES_OFFLINE", "1")
    with pytest.raises(binaries.OfflineError):
        binaries.resolve("difft")


def test_mirror_substitution(monkeypatch):
    monkeypatch.setenv("CASSANDRA_BINARIES_MIRROR", "https://mirror.example.com/gh")
...
def test_ensure_tools_survives_readonly_cache_dir(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("CASSANDRA_BINARIES_CACHE", str(readonly_parent / "cache"))
