...
def test_memory_disabled_is_no_op(monkeypatch: pytest.MonkeyPatch) -> None:
    """``CASSANDRA_MEMORY_INJECTION_MODE=disabled`` skips injection."""
...
def test_invalid_injection_mode_raises() -> None:
    ...
    prev = os.environ.get("CASSANDRA_MEMORY_INJECTION_MODE")
    os.environ["CASSANDRA_MEMORY_INJECTION_MODE"] = "system_prompt"
    try:
        with pytest.raises(ValueError, match="Invalid CASSANDRA_MEMORY_INJECTION_MODE"):
            get_memory_injection_mode()
    finally:
        if prev is None:
            os.environ.pop("CASSANDRA_MEMORY_INJECTION_MODE", None)
        else:
            os.environ["CASSANDRA_MEMORY_INJECTION_MODE"] = prev
