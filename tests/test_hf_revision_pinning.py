...
from cassandra.onnx_runtime import _PINNED_REVISIONS, _resolve_revision
...
def test_pin_can_be_disabled_via_env(monkeypatch, value):
    monkeypatch.setenv("CASSANDRA_HF_PIN", value)
    assert _resolve_revision("Sarthak-47/kompress-v2-base", None) is None


def test_pin_disabled_still_respects_explicit_revision(monkeypatch):
    monkeypatch.setenv("CASSANDRA_HF_PIN", "off")
    assert _resolve_revision("Sarthak-47/kompress-v2-base", "abc123") == "abc123"
