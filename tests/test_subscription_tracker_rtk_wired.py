...
Phase G of the Cassandra realignment retires the dead ``tokens_saved_rtk``
field by sourcing it from RTK's own stats endpoint (``rtk gain --format
json`` via :func:`cassandra.proxy.helpers._get_rtk_stats`) and writing the
per-call delta into ``CassandraContribution.tokens_saved_rtk``.
...
- Configurable: ``CASSANDRA_RTK_WIRING=disabled`` opts the polling out and
...
def _session_payload(tokens_saved: int, *, lifetime: int | None = None) -> dict[str, Any]:
    ...
    if lifetime is None:
        lifetime = tokens_saved + 50_000  # arbitrary pre-Cassandra history
    return {
...
def _stub_rtk_stats(
    ...
    monkeypatch.setattr(
        "cassandra.proxy.helpers._get_rtk_stats",
        fake_get_rtk_stats,
    )
    return call_count
...
def test_tokens_saved_rtk_populated_from_session_field(
    ...
    ``lifetime_tokens_saved`` and emitted the entire pre-Cassandra RTK
...
def test_first_poll_zero_when_session_baseline_fresh(
    ...
    _stub_rtk_stats(
        monkeypatch,
        [
            # Pre-Cassandra lifetime = 50 000 tokens. Helper rebaselines at
    ...
    assert tracker._state.contribution.tokens_saved_rtk == 0, (
        "first poll must NOT emit pre-Cassandra RTK history as a phantom delta"
    )
...
def test_rtk_stats_exception_zero_delta_no_throw_with_log(
    ...
    monkeypatch.setattr("cassandra.proxy.helpers._get_rtk_stats", boom)

    caplog.set_level(logging.WARNING, logger="cassandra.subscription.tracker")
...
def test_disabled_env_returns_zero(monkeypatch: pytest.MonkeyPatch) -> None:
    """``CASSANDRA_RTK_WIRING=disabled`` skips the poll entirely."""
...
# ---------------------------------------------------------------------------
# Test 8 (H1) — invalid CASSANDRA_RTK_WIRING fails loudly at startup
# ---------------------------------------------------------------------------
...
def test_garbage_wiring_env_raises_at_startup(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(ValueError, match="CASSANDRA_RTK_WIRING"):
...
def test_garbage_wiring_env_logs_loudly_at_runtime(
    ...
    caplog.set_level(logging.ERROR, logger="cassandra.subscription.tracker")
    ...
    ), "expected ERROR-level structured log on invalid CASSANDRA_RTK_WIRING"
...
def test_legacy_state_migrates_rtk_from_cli_filtering(
    ...
    caplog.set_level(logging.INFO, logger="cassandra.subscription.tracker")
...
def test_rtk_subprocess_failure_logs_structured_warning(
    ...
    (both scoped to ``logger="cassandra.proxy"`` and root-level capture).
    ...
    import cassandra.rtk as _rtk
    from cassandra.proxy import helpers as _helpers
    ...
    monkeypatch.setattr(_rtk, "get_rtk_path", lambda: "/nonexistent/cassandra-test-rtk")
