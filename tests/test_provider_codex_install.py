...
from cassandra.providers.codex.install import build_provider_section, codex_uses_chatgpt_auth
...
def test_codex_provider_section_omits_requires_openai_auth_by_default() -> None:
    ...
    Setting requires_openai_auth on a custom [model_providers.cassandra] block
    forces codex to demand OpenAI OAuth login for every cassandra-routed request,
    which breaks API-key users; so callers opt in explicitly for ChatGPT users.
    """
    section = build_provider_section(port=8787, name="OpenAI via Cassandra proxy")

    assert 'name = "OpenAI via Cassandra proxy"' in section
...
def test_codex_provider_section_emits_requires_openai_auth_when_flagged() -> None:
    section = build_provider_section(
        port=8787, name="OpenAI via Cassandra proxy", requires_openai_auth=True
    )
