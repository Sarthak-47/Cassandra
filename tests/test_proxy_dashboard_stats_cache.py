...
from cassandra.proxy import helpers as proxy_helpers
...
def _reset_rtk_stats_cache(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv("CASSANDRA_RTK_GAIN_SCOPE", raising=False)
...
def test_get_rtk_stats_can_read_project_scoped_gain(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("CASSANDRA_RTK_GAIN_SCOPE", "project")
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/bin/rtk")
    monkeypatch.setattr(subprocess, "run", _fake_run)
...
def test_get_rtk_stats_invalid_scope_defaults_to_global(
    ...
    monkeypatch.setenv("CASSANDRA_RTK_GAIN_SCOPE", "workspace")
...
def test_get_context_tool_stats_reads_lean_ctx_gain(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("CASSANDRA_CONTEXT_TOOL", "lean-ctx")
    now = {"value": 100.0}
    calls = {"run": 0}
    totals = [
        {
    ...
    monkeypatch.setattr(proxy_helpers.time, "monotonic", lambda: now["value"])
    monkeypatch.setattr(
        "cassandra.lean_ctx.get_lean_ctx_path",
...
def test_stats_cached_query_reuses_short_ttl_snapshot(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    import cassandra.proxy.server as server
...
def test_cost_merge_uses_generic_cli_filtering_name() -> None:
    from cassandra.proxy.cost import merge_cost_stats
...
def test_session_summary_uses_generic_cli_filtering_keys() -> None:
    from cassandra.proxy.cost import build_session_summary
...
def test_stats_reset_clears_runtime_proxy_counters(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    import cassandra.proxy.server as server
    from cassandra.proxy.loopback_guard import require_loopback
    from cassandra.proxy.server import ProxyConfig, create_app
...
def test_proxy_throughput_in_stats_endpoint(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    `from cassandra.perf.analyzer import ...` on every call, so we patch the
    names directly on the `cassandra.perf.analyzer` module so the local import
    inside the closure picks up our fakes.

    Skipped locally when cassandra._core (Rust extension) is not compiled.
    """
    pytest.importorskip("fastapi")
    from fastapi.testclient import TestClient

    import cassandra.perf.analyzer as _analyzer_mod
    ...
    try:
        ...
        pytest.skip(f"cassandra._core not available (Rust extension not compiled): {exc}")

    from cassandra.config import ProxyConfig
