...
from cassandra.providers.opencode.config import (
    CASSANDRA_OPENCODE_PLUGIN,
    _inject_key_into_json,
    _parse_json_loose,
    append_cassandra_plugin,
...
def test_opencode_config_paths_default(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert backup_file == tmp_path / ".config" / "opencode" / "opencode.json.cassandra-backup"
...
def test_opencode_config_paths_from_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert backup_file == tmp_path / "custom" / "opencode.json.cassandra-backup"
...
def test_snapshot_creates_backup(tmp_path: Path) -> None:
    ...
    backup_file = tmp_path / "opencode.json.cassandra-backup"
...
def test_snapshot_skips_if_markers_present(tmp_path: Path) -> None:
    """snapshot skips if the config already contains Cassandra markers."""
    config_file = tmp_path / "opencode.json"
    backup_file = tmp_path / "opencode.json.cassandra-backup"
    config_file.write_text("// --- Cassandra proxy provider ---\n{}")
    snapshot_opencode_config_if_unwrapped(config_file, backup_file)
...
def test_strip_blocks_removes_provider_and_mcp() -> None:
    ...
    content = (
        "// --- Cassandra proxy provider ---\n"
        '{"provider": {}}\n'
        "// --- end Cassandra proxy provider ---\n"
...
def test_inject_key_merges_dicts() -> None:
    ...
    data = _inject_key_into_json(data, "provider", {"cassandra": {}})
    assert "openai" in data["provider"]
    assert "cassandra" in data["provider"]


def test_inject_key_overwrites_non_dict() -> None:
    """_inject_key_into_json overwrites when existing value is not a dict."""
    data = {"model": "gpt-4o"}
    data = _inject_key_into_json(data, "model", "cassandra/claude-sonnet-4-6")
    assert data["model"] == "cassandra/claude-sonnet-4-6"
...
def test_inject_provider_config_creates_file(
    ...
    assert config["provider"]["cassandra"]["npm"] == "@ai-sdk/openai-compatible"
    assert "mcp" not in config
    assert "model" not in config  # cassandra provider is a transparent pass-through
...
def test_inject_provider_config_idempotent(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    config_file = tmp_path / ".config" / "opencode" / "opencode.json"
    config = _parse_json_loose(config_file.read_text())
    assert config["provider"]["cassandra"]["options"]["baseURL"] == "http://127.0.0.1:9999/v1"
...
def test_parse_json_loose_handles_comments_and_urls() -> None:
    ...
    data = _parse_json_loose(text)
    assert data["baseURL"] == "http://127.0.0.1:8787/v1"
    assert data["name"] == "Cassandra // Proxy"
...
def test_strip_blocks_handles_empty_string() -> None:
    """strip_opencode_cassandra_blocks returns empty string for empty input."""
    assert strip_opencode_cassandra_blocks("") == ""


def test_strip_blocks_handles_whitespace_only() -> None:
    """strip_opencode_cassandra_blocks returns empty string for whitespace input."""
    assert strip_opencode_cassandra_blocks("  \n  ") == ""


def test_strip_blocks_preserves_non_cassandra_jsonc() -> None:
    """strip_opencode_cassandra_blocks preserves JSONC comments not from Cassandra."""
...
def test_strip_blocks_removes_only_one_of_two_identical_blocks() -> None:
    """strip_opencode_cassandra_blocks removes all provider blocks, not just the first."""
    from cassandra.providers.opencode.config import _PROVIDER_MARKER_END, _PROVIDER_MARKER_START
...
def test_strip_blocks_handles_only_mcp_markers() -> None:
    """strip_opencode_cassandra_blocks also strips MCP markers."""
    from cassandra.providers.opencode.config import _MCP_MARKER_END, _MCP_MARKER_START
...
def test_inject_provider_config_preserves_existing_mcp(
    ...
    """inject_opencode_provider_config preserves MCP without adding cassandra."""
    _set_test_home(monkeypatch, tmp_path)
    config_file = tmp_path / ".config" / "opencode" / "opencode.json"
    ...
    assert "cassandra" not in config["mcp"]
...
def test_inject_provider_config_idempotent_with_complex_config(
    ...
    assert "cassandra" in config["provider"]
...
def test_inject_provider_config_preserves_unrelated_top_level_keys(
    ...
    assert "cassandra" in config.get("provider", {})


def test_append_cassandra_plugin_adds_plugin_once() -> None:
    config: dict[str, object] = {"plugin": ["some-plugin"]}

    assert append_cassandra_plugin(config) is True
    assert append_cassandra_plugin(config) is False

    assert config["plugin"] == ["some-plugin", CASSANDRA_OPENCODE_PLUGIN]


def test_append_cassandra_plugin_preserves_configured_tuple_entry() -> None:
    config: dict[str, object] = {
        "plugin": [[CASSANDRA_OPENCODE_PLUGIN, {"proxyUrl": "http://127.0.0.1:8787"}]]
    }

    assert append_cassandra_plugin(config) is False
    assert config["plugin"] == [[CASSANDRA_OPENCODE_PLUGIN, {"proxyUrl": "http://127.0.0.1:8787"}]]
...
def test_build_opencode_config_content_without_mcp() -> None:
    from cassandra.providers.opencode.runtime import build_opencode_config_content

    config = build_opencode_config_content(port=8787, include_mcp=False)
    assert "provider" in config
    assert "mcp" not in config
    assert "model" not in config
    assert config["plugin"] == [["cassandra-opencode", {"proxyUrl": "http://127.0.0.1:8787/v1"}]]
...
def test_build_opencode_config_content_with_mcp_uses_local_stdio() -> None:
    ...
    assert config["mcp"]["cassandra"] == {
        "type": "local",
        "command": ["cassandra", "mcp", "serve"],
        "enabled": True,
        "environment": {"CASSANDRA_PROXY_URL": "http://127.0.0.1:9000"},
    }


def test_build_launch_env_with_project(monkeypatch: pytest.MonkeyPatch) -> None:
    from cassandra.providers.opencode.runtime import build_launch_env

    monkeypatch.delenv("CASSANDRA_PROJECT", raising=False)
    ...
    assert env["CASSANDRA_PROJECT"] == "test-proj"
    assert "cassandra-opencode" in env["OPENCODE_CONFIG_CONTENT"]
...
def test_proxy_base_url() -> None:
    from cassandra.providers.opencode.runtime import proxy_base_url
...
def test_inject_provider_config_strips_existing_markers(
    ...
    assert "cassandra" in first

    inject_opencode_provider_config(port=9001)
    second = config_file.read_text()
    assert "cassandra" in second
    assert second.count("cassandra") == first.count("cassandra")
