...
Verifies that ``CassandraProxy`` gates the pre-upstream phase of
...
- env var ``CASSANDRA_ANTHROPIC_PRE_UPSTREAM_CONCURRENCY`` with flag override
"""
...
class _DummyAnthropicHandler(AnthropicHandlerMixin):
    ...
    def __init__(
        ...
        *,
        ...
        # ``CassandraProxy._run_compression_in_executor`` for bounded thread
    ...
    async def _run_compression_in_executor(self, fn, *, timeout):  # noqa: ANN001
        # Mirror of ``CassandraProxy._run_compression_in_executor`` for the
    ...
    async def _record_request_outcome(self, outcome) -> None:  # noqa: ANN001
        # Mirror of ``CassandraProxy._record_request_outcome`` for the
...
def stage_log_capture():
    target = logging.getLogger("cassandra.proxy")
...
def _tokenizer_patch():
    import cassandra.tokenizers as _tk
...
def _run_cli_capture(args: list[str], env: dict | None = None) -> ProxyConfig:
    ...
    via ``from cassandra.proxy.server import ... run_server``) short-
...
def test_env_var_sets_pre_upstream_concurrency():
    ...
    env = {"CASSANDRA_ANTHROPIC_PRE_UPSTREAM_CONCURRENCY": "4"}
...
def test_cli_env_sets_pre_upstream_timeouts():
    env = {
        "CASSANDRA_ANTHROPIC_PRE_UPSTREAM_ACQUIRE_TIMEOUT_SECONDS": "9.5",
        "CASSANDRA_ANTHROPIC_PRE_UPSTREAM_MEMORY_CONTEXT_TIMEOUT_SECONDS": "3.25",
    }
    config = _run_cli_capture([], env=env)
...
# --------------------------------------------------------------------------- #
# Sanity: CassandraProxy auto-computes default when config value is None.       #
# --------------------------------------------------------------------------- #
