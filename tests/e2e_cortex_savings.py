#!/usr/bin/env python3
"""
Real end-to-end token-savings test for Cortex Code + Cassandra.
...
Three test patterns:
  ...
  1. System-message context (Snowflake Cortex compatible)
     ...
     message → cassandra's SmartCrusher compresses them.
...
# ── Bootstrap: make cassandra importable from the project venv ─────────────────
REPO_ROOT = Path(__file__).resolve().parent.parent
_VENV_SITE = REPO_ROOT / ".venv" / "lib"
try:
    from cassandra import compress as _hc_check  # noqa: F401
except ImportError:
...
def main() -> int:
    print()
    print("╔══════════════════════════════════════════════════════════╗")
    print("║   Cortex Code × Cassandra  —  Real REST API savings      ║")
