...
def mock_run_server():
    ...
    with patch("cassandra.proxy.server.run_server", _mock):
        yield captured
...
class TestMissingProxyDepsError:
    ...
    def test_import_error_message_is_actionable(self, runner: CliRunner) -> None:
        ...
        def patched_import(name, *args, **kwargs):
            if name == "cassandra.proxy.server":
                raise ImportError("No module named 'cassandra.proxy.server'")
...
class TestKeyboardInterruptExitCode:
    ...
    def test_keyboard_interrupt_exits_130(self, runner: CliRunner) -> None:
        ...
        with patch("cassandra.proxy.server.run_server", _run_server_raises):
            result = runner.invoke(main, ["proxy"])
...
class TestNewEnvVarWiring:
    ...
    def test_cassandra_memory_db_path_from_env(
        self, runner: CliRunner, mock_run_server: dict
    ) -> None:
        result = runner.invoke(
            main,
            ["proxy", "--memory"],
            env={"CASSANDRA_MEMORY_DB_PATH": "/tmp/test-memory.db"},
            catch_exceptions=False,
        )
        assert result.exit_code == 0, result.output
        assert mock_run_server["config"].memory_db_path == "/tmp/test-memory.db"

    def test_cassandra_retry_max_attempts_from_env(
        self, runner: CliRunner, mock_run_server: dict
    ) -> None:
        result = runner.invoke(
            main,
            ["proxy"],
            env={"CASSANDRA_RETRY_MAX_ATTEMPTS": "5"},
            catch_exceptions=False,
        )
        assert result.exit_code == 0, result.output
        assert mock_run_server["config"].retry_max_attempts == 5

    def test_cassandra_connect_timeout_from_env(
        self, runner: CliRunner, mock_run_server: dict
    ) -> None:
        result = runner.invoke(
            main,
            ["proxy"],
            env={"CASSANDRA_CONNECT_TIMEOUT_SECONDS": "30"},
            catch_exceptions=False,
        )
        assert result.exit_code == 0, result.output
        assert mock_run_server["config"].connect_timeout_seconds == 30

    def test_cassandra_anthropic_buffered_timeout_from_env(
        self, runner: CliRunner, mock_run_server: dict
    ) -> None:
        result = runner.invoke(
            main,
            ["proxy"],
            env={"CASSANDRA_ANTHROPIC_BUFFERED_REQUEST_TIMEOUT_SECONDS": "900"},
            catch_exceptions=False,
        )
        assert result.exit_code == 0, result.output
    ...
    def test_direct_server_env_timeout_zero_falls_back_to_default(
        ...
        import cassandra.proxy.server as server_mod

        monkeypatch.delenv(server_mod._MULTI_WORKER_CONFIG_ENV, raising=False)
        monkeypatch.setenv("CASSANDRA_ANTHROPIC_BUFFERED_REQUEST_TIMEOUT_SECONDS", "0")
    ...
    def test_cassandra_backend_from_env(self, runner: CliRunner, mock_run_server: dict) -> None:
        result = runner.invoke(
            main,
            ["proxy"],
            env={"CASSANDRA_BACKEND": "bedrock"},
            catch_exceptions=False,
        )
        assert result.exit_code == 0, result.output
        assert mock_run_server["config"].backend == "bedrock"

    def test_cassandra_region_from_env(self, runner: CliRunner, mock_run_server: dict) -> None:
        result = runner.invoke(
            main,
            ["proxy"],
            env={"CASSANDRA_REGION": "eu-west-1"},
            catch_exceptions=False,
        )
        assert result.exit_code == 0, result.output
