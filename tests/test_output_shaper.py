"""Tests for cassandra.proxy.output_shaper.
...
class TestVerbositySteering:
    ...
    def test_level_change_replaces_block_in_place(self):
        ...
        steering_blocks = [
            b for b in body["system"] if b["text"].startswith("<cassandra_output_shaping>")
        ]
...
class TestShapeRequest:
    ...
    def test_from_env_defaults_off(self, monkeypatch):
        monkeypatch.delenv("CASSANDRA_OUTPUT_SHAPER", raising=False)
    ...
    def test_from_env_enabled_with_overrides(self, monkeypatch):
        ...
        monkeypatch.setenv("CASSANDRA_MECHANICAL_EFFORT", "medium")
    ...
    def test_from_env_clamps_bad_values(self, monkeypatch):
        monkeypatch.setenv("CASSANDRA_OUTPUT_SHAPER", "true")
        monkeypatch.setenv("CASSANDRA_VERBOSITY_LEVEL", "99")
        monkeypatch.setenv("CASSANDRA_MECHANICAL_EFFORT", "bogus")
        settings = OutputShaperSettings.from_env()
