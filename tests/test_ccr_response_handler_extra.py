...
from cassandra.ccr.response_handler import (
    CCRResponseHandler,
    CCRToolCall,
    CCRToolResult,
    StreamingCCRBuffer,
...
def test_execute_retrieval_error_paths(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "cassandra.ccr.response_handler.get_compression_store",
...
def test_create_tool_result_message_google_and_generic_formats() -> None:
    handler = CCRResponseHandler()
    results = [
        CCRToolResult(tool_call_id="cassandra_retrieve", content='{"count": 1}', success=True)
    ]
    google_message = handler._create_tool_result_message(results, "google")
    assert google_message == {
        "role": "user",
        "parts": [{"functionResponse": {"name": "cassandra_retrieve", "response": {"count": 1}}}],
    }
    ...
    invalid_google = handler._create_tool_result_message(
        [CCRToolResult(tool_call_id="cassandra_retrieve", content="not-json", success=True)],
        "google",
    )
...
def test_streaming_buffer_and_parse_sse_helpers() -> None:
    ...
    anthropic_data = b"\n\n".join(
        [
            ...
            b'data: {"type":"content_block_start","content_block":{"type":"tool_use","id":"tool_1","name":"cassandra_retrieve"}}',
    ...
    assert parsed["content"][1]["name"] == "cassandra_retrieve"
...
async def test_streaming_handler_process_stream_pass_through_and_ccr(
    ...
    ccr_chunks = [
        b'{"type":"tool_use","name":"cassandra_retrieve"',
...
async def test_streaming_handler_falls_back_to_buffer_on_processing_error(
    ...
    chunks = [b'{"type":"tool_use","name":"cassandra_retrieve"', b',"stop_reason":"tool_use"}']
    streamed = [
        chunk
