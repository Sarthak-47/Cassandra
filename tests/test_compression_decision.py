"""Tests for :class:`cassandra.proxy.compression_decision.CompressionDecision`,
...
* ``handlers/gemini.py`` had THREE compression sites that NEVER checked
  the ``x-cassandra-bypass`` header — explicit user requests to skip
...
def _config(*, optimize: bool = True) -> Any:
    """Minimal stand-in for ``CassandraConfig`` — only the fields the
    decision reads."""
...
def test_bypass_header_wins_over_every_other_gate() -> None:
    """``x-cassandra-bypass`` is the user's explicit "do not touch my
    ...
    d = CompressionDecision.decide(
        headers={"x-cassandra-bypass": "true"},
        config=_config(optimize=True),
        usage_reporter=_usage_reporter(should_compress=True),
        messages=_msgs(),
    )
...
def test_passthrough_mode_header_also_triggers_bypass() -> None:
    """``x-cassandra-mode: passthrough`` is the alternate spelling of the
    bypass signal — both go through the same path. This mirrors the
    pre-existing ``_cassandra_bypass_enabled`` semantics in helpers.py;
    re-asserted here so a refactor of that helper can't silently
    diverge."""
    d = CompressionDecision.decide(
        headers={"x-cassandra-mode": "passthrough"},
        config=_config(optimize=True),
        usage_reporter=_usage_reporter(should_compress=True),
        messages=_msgs(),
    )
...
def test_bypass_header_is_case_insensitive() -> None:
    """Real client UAs send ``X-Cassandra-Bypass`` (title-case) or
    ``x-cassandra-bypass`` (lower-case). Both must work — the existing
    helper normalised both, so the consolidated decision must too."""
    for header_key in ("X-Cassandra-Bypass", "x-cassandra-bypass", "X-CASSANDRA-BYPASS"):
        ...
        if header_key == "x-cassandra-bypass":
...
def test_bypass_header_value_must_be_true() -> None:
    ...
    for value in ("false", "0", "", "yes", "no"):
        d = CompressionDecision.decide(
            headers={"x-cassandra-bypass": value},
            config=_config(),
            usage_reporter=_usage_reporter(),
            messages=_msgs(),
        )
...
def test_decide_accepts_fastapi_style_starlette_headers() -> None:
    ...
    h = _FakeStarletteHeaders({"X-Cassandra-Bypass": "true"})
    d = CompressionDecision.decide(
