...
from cassandra.install.models import DeploymentManifest, ManagedMutation
from cassandra.install.providers import _apply_windows_env_scope, _remove_windows_env_scope
from cassandra.providers.claude.install import apply_provider_scope as apply_claude_provider_scope
from cassandra.providers.claude.install import build_install_env as build_claude_install_env
from cassandra.providers.claude.install import revert_provider_scope as revert_claude_provider_scope
from cassandra.providers.codex.install import apply_provider_scope as apply_codex_provider_scope
from cassandra.providers.codex.install import build_install_env as build_codex_install_env
from cassandra.providers.codex.install import revert_provider_scope as revert_codex_provider_scope
from cassandra.providers.copilot.install import build_install_env as build_copilot_install_env
from cassandra.providers.opencode.install import (
    apply_provider_scope as apply_opencode_provider_scope,
)
from cassandra.providers.opencode.install import build_install_env as build_opencode_install_env
from cassandra.providers.opencode.install import (
...
def test_apply_and_revert_claude_provider_scope(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "cassandra.providers.claude.install.claude_settings_path", lambda: settings_path
    )
...
def test_apply_and_revert_codex_provider_scope(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("cassandra.providers.codex.install.codex_config_path", lambda: config_path)
    manifest = _manifest(tmp_path)
    ...
    assert 'model_provider = "cassandra"' not in reverted
...
def test_apply_codex_provider_scope_replaces_existing_managed_block(
    ...
    config_path.write_text(
        'model = "gpt-4o"\n\n'
        "# --- Cassandra persistent provider ---\n"
        'model_provider = "cassandra"\n\n'
        "[model_providers.cassandra]\n"
        'name = "Cassandra persistent proxy"\n'
        'base_url = "http://127.0.0.1:1111/v1"\n'
        "requires_openai_auth = true\n"
        "supports_websockets = true\n"
        "# --- end Cassandra persistent provider ---\n"
    )
    monkeypatch.setattr("cassandra.providers.codex.install.codex_config_path", lambda: config_path)
    manifest = _manifest(tmp_path)
    manifest.port = 9999
    ...
    content = config_path.read_text()
    assert content.count("# --- Cassandra persistent provider ---") == 1
...
def test_apply_openclaw_provider_scope_uses_manifest_port(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "cassandra.providers.openclaw.install._invoke_openclaw",
        lambda command: recorded.append(command),
    )
    monkeypatch.setattr(
        "cassandra.providers.openclaw.install.openclaw_config_path",
        lambda: tmp_path / "openclaw.json",
    )
    manifest = _manifest(tmp_path)
    manifest.port = 9999

    from cassandra.providers.openclaw.install import (
...
def test_openclaw_apply_provider_scope_requires_installed_binary(
    ...
    monkeypatch.setattr("cassandra.providers.openclaw.install.shutil_which", lambda name: None)
...
def test_openclaw_helper_wrappers_delegate_to_stdlib(monkeypatch) -> None:
    ...
    from cassandra.providers.openclaw.install import _invoke_openclaw, shutil_which

    assert shutil_which("openclaw") == "/fake/openclaw"
    _invoke_openclaw(["cassandra", "wrap", "openclaw"])

    assert recorded == [(["cassandra", "wrap", "openclaw"], True)]
...
def test_openclaw_revert_provider_scope_skips_without_binary(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("cassandra.providers.openclaw.install._invoke_openclaw", fail_if_called)
...
def test_openclaw_revert_provider_scope_invokes_unwrap(monkeypatch, tmp_path: Path) -> None:
    ...
    assert recorded == [["cassandra", "unwrap", "openclaw"]]
...
def test_windows_env_scope_restores_previous_values(monkeypatch, tmp_path: Path) -> None:
    ...
    manifest.base_env = {"CASSANDRA_PORT": "8787"}
    ...
    def fake_run(command: list[str], **kwargs):
        ...
        if "GetEnvironmentVariable" in script:
            ...
            value = previous_values.get(name, "__CASSANDRA_UNSET__")
            return Result(stdout=value)
        return Result()

    monkeypatch.setattr("cassandra.install.providers.subprocess.run", fake_run)
    ...
    assert previous_by_name["CASSANDRA_PORT"] == "7777"
    assert previous_by_name["ANTHROPIC_BASE_URL"] == "https://old"
    assert any(
        "[Environment]::SetEnvironmentVariable('CASSANDRA_PORT','7777','User')" in command[-1]
...
def test_apply_mutations_runs_openclaw_for_user_scope(monkeypatch, tmp_path: Path) -> None:
    ...
    if os.name == "nt":
        monkeypatch.setattr(
            "cassandra.install.providers._apply_windows_env_scope", lambda deployment: []
        )
    else:
        monkeypatch.setattr(
            "cassandra.install.providers._apply_unix_env_scope", lambda deployment: []
        )
    monkeypatch.setattr(
        "cassandra.install.providers.apply_provider_scope_mutations",
        lambda deployment: [ManagedMutation(target="openclaw", kind="openclaw-wrap")],
    )

    from cassandra.install.providers import apply_mutations
...
def test_apply_and_revert_opencode_provider_scope(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "cassandra.providers.opencode.install.opencode_config_path", lambda: config_path
    )
    manifest = _manifest(tmp_path)
    ...
    assert data["provider"]["cassandra"]["options"]["baseURL"] == "http://127.0.0.1:8787/v1"
    ...
    assert "cassandra" not in reverted.get("provider", {})
...
# must never appear in the cassandra provider block.
# ---------------------------------------------------------------------------


def test_cassandra_provider_block_never_sets_requires_openai_auth(
    monkeypatch, tmp_path: Path
) -> None:
    """apply_provider_scope must NOT emit requires_openai_auth in the cassandra block.

    Bug 3 (#406): requires_openai_auth = true on a custom [model_providers.cassandra]
    block forces codex to demand OpenAI OAuth login for cassandra-routed traffic.
    Cassandra is a local proxy — it must not require OpenAI auth.
    """
    for port in (8787, 9999):
        config_path = tmp_path / f"config_{port}.toml"
        monkeypatch.setattr(
            "cassandra.providers.codex.install.codex_config_path",
        ...
        content = config_path.read_text()
        # The rendered TOML for the cassandra provider block must not contain this
        # field.  If it does, codex will prompt for OpenAI OAuth on every startup.
        assert "requires_openai_auth" not in content, (
            f"requires_openai_auth must be absent from the cassandra provider block "
            f"(port={port}); got:\n{content}"
        )
...
def test_inject_codex_provider_config_writes_openai_base_url(
    ...
    1. Strip ``requires_openai_auth`` from the cassandra provider block (done in 3ca48d3).
    2. Inject ``openai_base_url`` at the top level so subscription (ChatGPT plan)
       users are also routed through cassandra.
    ...
    assert 'openai_base_url = "http://127.0.0.1:8787/v1"' in content, (
        ...
        "so that Codex subscription (ChatGPT plan) users are routed through cassandra; "
        f"got:\n{content}"
    )
...
def test_unwrap_removes_top_level_openai_base_url(
    ...
    # Also verify via _strip_codex_cassandra_blocks directly — the orphan-cleanup
    ...
    stripped = wrap_mod._strip_codex_cassandra_blocks(orphan_content)
    assert "openai_base_url" not in stripped, (
        f"_strip_codex_cassandra_blocks must remove orphaned openai_base_url lines; got:\n{stripped}"
    )
...
def test_apply_provider_scope_writes_openai_base_url(monkeypatch, tmp_path: Path) -> None:
    ...
    through cassandra regardless of which entry point they used."""
    config_path = tmp_path / "config.toml"
    config_path.write_text('model = "gpt-4o"\n')
    monkeypatch.setattr("cassandra.providers.codex.install.codex_config_path", lambda: config_path)
    manifest = _manifest(tmp_path)
...
def test_planner_resolves_opencode_as_install_target() -> None:
    from cassandra.install.planner import resolve_targets
...
def test_planner_opencode_in_supported_targets_enum() -> None:
    from cassandra.install.models import ToolTarget
    from cassandra.install.planner import PROVIDER_SCOPE_TARGETS, SUPPORTED_TARGETS
...
def test_planner_build_tool_envs_includes_opencode() -> None:
    from cassandra.install.planner import build_tool_envs
...
def test_revert_opencode_provider_scope_fallback_on_oserror(monkeypatch, tmp_path: Path) -> None:
    ...
    config_path = tmp_path / "opencode.json"
    backup_path = config_path.with_suffix(".json.cassandra-backup")
    config_path.parent.mkdir(parents=True, exist_ok=True)

    from cassandra.install.models import ManagedMutation
    ...
    provider_json = '{"cassandra":{"npm":"@ai-sdk/openai-compatible","name":"Cassandra Proxy","options":{"baseURL":"http://127.0.0.1:8787/v1"}}}'
    config_path.write_text(
        f'{_PROVIDER_MARKER_START}\n"provider": {provider_json},\n{_PROVIDER_MARKER_END}\n'
    )

    monkeypatch.setattr(
        "cassandra.providers.opencode.install.opencode_config_path",
        lambda: config_path,
    )

    from cassandra.providers.opencode.install import revert_provider_scope
    ...
    assert not config_path.exists() or "cassandra" not in config_path.read_text()
