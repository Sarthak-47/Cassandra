...
def test_probe_ready_accepts_ready_and_healthy(monkeypatch) -> None:
    monkeypatch.setattr(
        "cassandra.install.health.probe_json", lambda url, timeout=2.0: {"ready": True}
    )
    assert probe_ready("http://example.test")

    monkeypatch.setattr(
        "cassandra.install.health.probe_json", lambda url, timeout=2.0: {"status": "healthy"}
    )
    assert probe_ready("http://example.test")

    monkeypatch.setattr("cassandra.install.health.probe_json", lambda url, timeout=2.0: None)
