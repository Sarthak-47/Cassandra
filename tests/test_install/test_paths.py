...
from cassandra.install import paths as install_paths
...
def test_profile_and_artifact_paths(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr("cassandra.install.paths._paths.deploy_root", lambda: tmp_path / "deploy")
...
def test_env_target_and_config_paths(monkeypatch, tmp_path: Path) -> None:
    ...
    assert install_paths.unix_system_env_targets() == [Path("/etc/profile.d/cassandra.sh")]

    monkeypatch.setattr("cassandra.install.paths.sys.platform", "darwin")
