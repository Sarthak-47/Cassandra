...
from cassandra.install.models import DeploymentManifest, SupervisorKind
from cassandra.install.supervisors import (
    _command_for_script,
    _linux_service_unit,
    _linux_task_spec,
...
def _manifest(
    *, profile: str = "default", scope: str = "user", supervisor: str = "service"
    ...
    return DeploymentManifest(
        ...
        host="127.0.0.1",
        backend="anthropic",
        service_name=f"cassandra-{profile}",
    )
...
def test_linux_service_unit_uses_user_systemd_path(monkeypatch, tmp_path: Path) -> None:
    ...
    unit_path, content = _linux_service_unit(manifest, tmp_path / "run-cassandra.sh")

    assert unit_path == tmp_path / ".config" / "systemd" / "user" / "cassandra-default.service"
    assert "ExecStart=" + str(tmp_path / "run-cassandra.sh") in content
    assert "Restart=on-failure" in content


def test_command_for_script_and_unix_runner(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr(
        "cassandra.install.supervisors.resolve_cassandra_command",
        lambda: ["python", "-m", "cassandra"],
    )
    ...
    record = _render_unix_runner(
        tmp_path / "scripts" / "run-cassandra.sh", ["cassandra", "run", "--flag"]
    )
    assert record.kind == "script"
    content = Path(record.path).read_text(encoding="utf-8")
    assert content.startswith("#!/usr/bin/env bash")
    assert "exec cassandra run --flag" in content
...
def test_linux_task_spec_for_user_scope_includes_crontab_markers(tmp_path: Path) -> None:
    ...
    cron_path, content = _linux_task_spec(manifest, tmp_path / "ensure-cassandra.sh")

    assert cron_path is None
    assert "# >>> cassandra smoke >>>" in content
    assert "# <<< cassandra smoke <<<" in content
...
def test_macos_launchd_plist_switches_between_keepalive_and_interval(
    ...
    service_path, service_content = _macos_launchd_plist(
        service_manifest, tmp_path / "run-cassandra.sh"
    )
    assert service_path == tmp_path / "Library" / "LaunchAgents" / "com.cassandra.default.plist"
    ...
    task_path, task_content = _macos_launchd_plist(
        task_manifest, tmp_path / "ensure-cassandra.sh", interval=300
    )
    assert task_path == tmp_path / "Library" / "LaunchAgents" / "com.cassandra.tasky.plist"
...
def test_render_windows_runner_writes_ps1_and_cmd_wrappers(tmp_path: Path) -> None:
    ps1_path = tmp_path / "run-cassandra.ps1"
    cmd_path = tmp_path / "run-cassandra.cmd"

    records = _render_windows_runner(
        ps1_path,
        cmd_path,
        ["C:\\Program Files\\Python\\python.exe", "cassandra", "install", "agent", "run"],
    )
    ...
    assert '& "C:\\Program Files\\Python\\python.exe" cassandra install agent run' in ps1_content
    assert (
        'powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0run-cassandra.ps1" %*'
        in cmd_content
    )


def test_render_runner_scripts_writes_unix_scripts(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr("cassandra.install.supervisors.sys.platform", "linux")
    monkeypatch.setattr(
        "cassandra.install.supervisors.resolve_cassandra_command", lambda: ["cassandra"]
    )
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    manifest = _manifest()
...
def test_render_runner_scripts_writes_windows_scripts(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr("cassandra.install.supervisors.sys.platform", "win32")
    monkeypatch.setattr(
        "cassandra.install.supervisors.resolve_cassandra_command", lambda: ["cassandra.exe"]
    )
    monkeypatch.setattr(
        "cassandra.install.supervisors.windows_run_script_path",
        lambda profile: tmp_path / "run-cassandra.ps1",
    )
    monkeypatch.setattr(
        "cassandra.install.supervisors.windows_run_cmd_path",
        lambda profile: tmp_path / "run-cassandra.cmd",
    )
    monkeypatch.setattr(
        "cassandra.install.supervisors.windows_ensure_script_path",
        lambda profile: tmp_path / "ensure-cassandra.ps1",
    )
    monkeypatch.setattr(
        "cassandra.install.supervisors.windows_ensure_cmd_path",
        lambda profile: tmp_path / "ensure-cassandra.cmd",
    )
    ...
    assert [Path(record.path).name for record in records] == [
        "run-cassandra.ps1",
        "run-cassandra.cmd",
        "ensure-cassandra.ps1",
        "ensure-cassandra.cmd",
    ]
...
def test_start_and_stop_supervisor_use_linux_systemctl(monkeypatch) -> None:
    ...
    monkeypatch.setattr("cassandra.install.supervisors.sys.platform", "linux")
    monkeypatch.setattr(
        "cassandra.install.supervisors.subprocess.run",
    ...
    assert calls == [
        ["systemctl", "--user", "restart", "cassandra-default"],
        ["systemctl", "--user", "stop", "cassandra-default"],
    ]
...
def test_install_supervisor_linux_service_and_tasks(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr("cassandra.install.supervisors.sys.platform", "linux")
    monkeypatch.setattr("cassandra.install.supervisors.sys.platform", "linux")
    run_script = tmp_path / "run-cassandra.sh"
    ensure_script = tmp_path / "ensure-cassandra.sh"
    monkeypatch.setattr(
        "cassandra.install.supervisors.render_runner_scripts",
        lambda manifest: [
            type("Record", (), {"kind": "script", "path": run_script.as_posix()})(),
            type("Record", (), {"kind": "script", "path": ensure_script.as_posix()})(),
        ],
    )
    unit_path = tmp_path / "cassandra-default.service"
    monkeypatch.setattr(
        "cassandra.install.supervisors._linux_service_unit",
    ...
    monkeypatch.setattr("cassandra.install.supervisors.subprocess.run", fake_run)
    ...
    assert ["systemctl", "--user", "enable", "cassandra-default"] in [call[0] for call in calls]
    assert service_records[-1].kind == "service-unit"

    cron_path = tmp_path / "cassandra-system"
    monkeypatch.setattr(
        "cassandra.install.supervisors._linux_task_spec",
...
def test_install_supervisor_darwin_windows_and_unsupported(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("cassandra.install.supervisors.os.getuid", lambda: 123, raising=False)

    plist_path = tmp_path / "com.cassandra.default.plist"
    monkeypatch.setattr(
        "cassandra.install.supervisors._macos_launchd_plist",
        lambda manifest, script, interval=None: (plist_path, f"plist-{interval}"),
    )
    monkeypatch.setattr("cassandra.install.supervisors.sys.platform", "darwin")
    monkeypatch.setattr("cassandra.install.supervisors.sys.platform", "darwin")
    service_records = install_supervisor(_manifest(supervisor=SupervisorKind.SERVICE.value))
    ...
    monkeypatch.setattr(
        "cassandra.install.supervisors.windows_run_cmd_path",
        lambda profile: Path(f"C:\\tmp\\{profile}\\run-cassandra.cmd"),
    )
    monkeypatch.setattr(
        "cassandra.install.supervisors.windows_ensure_cmd_path",
        lambda profile: Path(f"C:\\tmp\\{profile}\\ensure-cassandra.cmd"),
    )
    ...
    assert [
        ...
        "create",
        "cassandra-default",
        'binPath= cmd.exe /c "C:\\tmp\\default\\run-cassandra.cmd"',
        "start= auto",
    ] in calls
    assert [
        "schtasks",
        "/Create",
        "/TN",
        "cassandra-default-health",
        "/TR",
        "C:\\tmp\\default\\ensure-cassandra.cmd",
        "/SC",
        "MINUTE",
        "/MO",
        "5",
    ...
    monkeypatch.setattr("cassandra.install.supervisors.sys.platform", "plan9")
    monkeypatch.setattr("cassandra.install.supervisors.sys.platform", "plan9")
...
def test_start_and_stop_supervisor_darwin_windows_and_none(monkeypatch) -> None:
    ...
    monkeypatch.setattr("cassandra.install.supervisors.os.getuid", lambda: 77, raising=False)
    ...
    assert calls == [
        ["launchctl", "kickstart", "-k", "gui/77/com.cassandra.default"],
        ["launchctl", "bootout", "gui/77/com.cassandra.default"],
    ]
    ...
    stop_supervisor(_manifest(supervisor=SupervisorKind.SERVICE.value))
    assert calls == [
        ["sc.exe", "start", "cassandra-default"],
        ["sc.exe", "stop", "cassandra-default"],
    ]
...
def test_macos_start_bootstraps_when_job_not_registered(monkeypatch, tmp_path: Path) -> None:
    ...
    plist_path = tmp_path / "Library" / "LaunchAgents" / "com.cassandra.default.plist"
    assert calls == [
        ["launchctl", "kickstart", "-k", "gui/77/com.cassandra.default"],
...
def test_macos_start_retries_bootstrap_until_launchd_settles(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("cassandra.install.supervisors.os.getuid", lambda: 77, raising=False)
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setattr("cassandra.install.supervisors.time.sleep", lambda _s: None)
...
def test_macos_start_raises_after_bootstrap_keeps_failing(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setattr("cassandra.install.supervisors.time.sleep", lambda _s: None)
    monkeypatch.setattr("cassandra.install.supervisors._MACOS_BOOTSTRAP_RETRIES", 3)
...
def test_macos_stop_tolerates_missing_job(monkeypatch) -> None:
    ...
    stop_supervisor(_manifest(supervisor=SupervisorKind.SERVICE.value))
    assert calls == [["launchctl", "bootout", "gui/77/com.cassandra.default"]]
...
def test_remove_supervisor_linux_service_cron_path_and_missing_crontab(
    ...
    assert ["systemctl", "--user", "disable", "--now", "cassandra-default"] in calls
    assert ["systemctl", "--user", "daemon-reload"] in calls

    cron_path = tmp_path / "cassandra-task"
...
def test_remove_supervisor_darwin_and_windows(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("cassandra.install.supervisors.os.getuid", lambda: 55, raising=False)

    plist_path = tmp_path / "com.cassandra.default.plist"
    plist_path.write_text("plist", encoding="utf-8")
    monkeypatch.setattr(
        "cassandra.install.supervisors.unix_run_script_path",
        lambda profile: tmp_path / "run-cassandra.sh",
    )
    monkeypatch.setattr(
        "cassandra.install.supervisors.unix_ensure_script_path",
        lambda profile: tmp_path / "ensure-cassandra.sh",
    )
    monkeypatch.setattr(
        "cassandra.install.supervisors._macos_launchd_plist",
    ...
    assert calls[0] == ["launchctl", "bootout", "gui/55/com.cassandra.default"]
    ...
    assert calls == [
        ["sc.exe", "stop", "cassandra-default"],
        ["sc.exe", "delete", "cassandra-default"],
        ["schtasks", "/Delete", "/TN", "cassandra-default-startup", "/F"],
        ["schtasks", "/Delete", "/TN", "cassandra-default-health", "/F"],
    ]
