...
from cassandra.install.models import ArtifactRecord, DeploymentManifest, ManagedMutation
from cassandra.install.state import delete_manifest, list_manifests, load_manifest, save_manifest
...
def _manifest() -> DeploymentManifest:
    return DeploymentManifest(
        ...
        artifacts=[ArtifactRecord(kind="script", path="run-cassandra.sh")],
    )
...
def test_list_manifests_ignores_invalid_payloads(monkeypatch, tmp_path: Path) -> None:
    ...
    broken_dir = tmp_path / ".cassandra" / "deploy" / "broken"
...
def test_delete_manifest_removes_profile_root(monkeypatch, tmp_path: Path) -> None:
    ...
    manifest = _manifest()
    save_manifest(manifest)
    extra_file = tmp_path / ".cassandra" / "deploy" / "default" / "runner.log"
