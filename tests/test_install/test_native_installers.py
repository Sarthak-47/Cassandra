...
def test_bash_native_installer_supports_persistent_docker_lifecycle(tmp_path: Path) -> None:
    ...
    env["CASSANDRA_DOCKER_IMAGE"] = "cassandra:test-image"
    ...
    try:
        ...
        assert "CASSANDRA_IMAGE_DEFAULT=cassandra:test-image" in wrapper.read_text(encoding="utf-8")
        ...
        manifest_path = home / ".cassandra" / "deploy" / "smoke" / "manifest.json"
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        assert manifest["preset"] == "persistent-docker"
        assert manifest["port"] == port
        assert manifest["memory_enabled"] is True
        assert manifest["memory_db_path"] == "/tmp/cassandra-home/.cassandra/memory.db"
        assert manifest["telemetry_enabled"] is False

        state_path = home / ".cassandra" / "deploy" / "smoke" / "docker-native.env"
        ...
        assert "/tmp/cassandra-home/.cassandra/memory.db" in install_call
        ...
        assert "CASSANDRA_WORKSPACE_DIR=/tmp/cassandra-home/.cassandra" in install_call
        assert "CASSANDRA_CONFIG_DIR=/tmp/cassandra-home/.cassandra/config" in install_call
...
def test_powershell_native_installer_supports_persistent_docker_lifecycle(tmp_path: Path) -> None:
    ...
    try:
        ...
        wrapper = home / ".local" / "bin" / "cassandra.ps1"
        assert wrapper.exists()
        assert "__CASSANDRA_INSTALL_IMAGE__" not in wrapper.read_text(encoding="utf-8")
        assert "cassandra:test-image" in wrapper.read_text(encoding="utf-8")
        cmd_wrapper = home / ".local" / "bin" / "cassandra.cmd"
        ...
        manifest_path = home / ".cassandra" / "deploy" / "smoke" / "manifest.json"
        state_path = home / ".cassandra" / "deploy" / "smoke" / "docker-native.json"
        ...
        assert manifest["memory_db_path"] == "/tmp/cassandra-home/.cassandra/memory.db"
        assert manifest["telemetry_enabled"] is False
        assert state["container_name"] == "cassandra-smoke"
