...
from cassandra.install.models import ConfigScope, InstallPreset, ProviderSelectionMode, ToolTarget
from cassandra.install.planner import build_manifest, resolve_targets


def test_resolve_targets_auto_falls_back_when_detection_empty(monkeypatch) -> None:
    monkeypatch.setattr("cassandra.install.planner.detect_targets", lambda: [])
...
def test_build_manifest_for_persistent_docker_sets_expected_defaults() -> None:
    ...
    assert manifest.base_env["CASSANDRA_PORT"] == "8787"
    assert manifest.base_env["CASSANDRA_TELEMETRY"] == "off"
...
def test_build_manifest_uses_provider_slice_env_builders_for_all_supported_targets() -> None:
    ...
    assert manifest.base_env["CASSANDRA_TELEMETRY"] == "on"
