...
Verifies that:
1. Provider target URL env vars are read by `cassandra proxy`
2. litellm-* backends are accepted by both CLI and argparse paths
3. CASSANDRA_WRAP_PROXY_TIMEOUT controls `cassandra wrap` proxy readiness waits
"""
...
from cassandra.cli import wrap as wrap_mod  # noqa: E402
from cassandra.cli.main import main  # noqa: E402
...
class TestCLIProxyEnvVars:
    ...
    def test_cassandra_host_from_env(self, runner):
        """CASSANDRA_HOST env var should be passed to ProxyConfig."""
        captured_config = {}

        def mock_run_server(config, **kwargs):
            captured_config["config"] = config

        with patch("cassandra.proxy.server.run_server", mock_run_server):
    ...
    def test_cassandra_port_from_env(self, runner):
        """CASSANDRA_PORT env var should be passed to ProxyConfig."""
        captured_config = {}
    ...
    def test_cassandra_min_tokens_from_env(self, runner):
        """CASSANDRA_MIN_TOKENS env var should be passed to ProxyConfig."""
        captured_config = {}
    ...
    def test_cassandra_budget_from_env(self, runner):
        """CASSANDRA_BUDGET env var should be passed to ProxyConfig."""
        captured_config = {}
    ...
    def test_budget_period_flag_and_env(self, runner):
        """--budget-period and CASSANDRA_BUDGET_PERIOD should reach ProxyConfig."""
        captured_config = {}
        ...
        with patch("cassandra.proxy.server.run_server", mock_run_server):
            result = runner.invoke(
                main,
                ["proxy"],
                env={"CASSANDRA_BUDGET_PERIOD": "hourly"},
                catch_exceptions=False,
            )
    ...
    def test_code_aware_enabled_from_env(self, runner):
        """CASSANDRA_CODE_AWARE_ENABLED env var should be passed to ProxyConfig."""
        captured_config = {}
        ...
        with patch("cassandra.proxy.server.run_server", mock_run_server):
            result = runner.invoke(
                main,
                ["proxy"],
                env={"CASSANDRA_CODE_AWARE_ENABLED": "true"},
                catch_exceptions=False,
            )
    ...
    def test_code_aware_enabled_defaults_false(self, runner):
        """Without CASSANDRA_CODE_AWARE_ENABLED, code-aware stays disabled in the wrapper."""
        captured_config = {}

        def mock_run_server(config, **kwargs):
            captured_config["config"] = config

        env = {k: v for k, v in os.environ.items() if k != "CASSANDRA_CODE_AWARE_ENABLED"}

        with (
            patch("cassandra.proxy.server.run_server", mock_run_server),
    ...
    def test_disable_kompress_from_env(self, runner):
        """CASSANDRA_DISABLE_KOMPRESS should be passed to ProxyConfig."""
        captured_config = {}
    ...
    def test_code_aware_flag_overrides_env_var(self, runner):
        """--code-aware should win over CASSANDRA_CODE_AWARE_ENABLED=false."""
        captured_config = {}
        ...
        with patch("cassandra.proxy.server.run_server", mock_run_server):
            result = runner.invoke(
                main,
                ["proxy", "--code-aware"],
                env={"CASSANDRA_CODE_AWARE_ENABLED": "false"},
                catch_exceptions=False,
            )
    ...
    def test_request_timeout_from_env(self, runner, timeout):
        """CASSANDRA_REQUEST_TIMEOUT env var should be passed to ProxyConfig."""
        captured_config = {}
        ...
        with patch("cassandra.proxy.server.run_server", mock_run_server):
            result = runner.invoke(
                main,
                ["proxy"],
                env={"CASSANDRA_REQUEST_TIMEOUT": f"{timeout}"},
                catch_exceptions=False,
            )
    ...
    def test_production_scaling_env_vars(self, runner):
        ...
        with patch("cassandra.proxy.server.run_server", mock_run_server):
            result = runner.invoke(
                ...
                env={
                    "CASSANDRA_WORKERS": "4",
                    "CASSANDRA_LIMIT_CONCURRENCY": "250",
                    "CASSANDRA_MAX_CONNECTIONS": "200",
                    "CASSANDRA_MAX_KEEPALIVE": "50",
                },
                catch_exceptions=False,
            )
    ...
    def test_keepalive_expiry_env_var(self, runner):
        ...
        with patch("cassandra.proxy.server.run_server", mock_run_server):
            result = runner.invoke(
                main,
                ["proxy"],
                env={"CASSANDRA_KEEPALIVE_EXPIRY": "45"},
                catch_exceptions=False,
            )
...
class TestCLIAnyllmProviderEnv:
    """Test that CASSANDRA_ANYLLM_PROVIDER env var is read by the CLI."""

    def test_anyllm_provider_from_env(self, runner):
        """CASSANDRA_ANYLLM_PROVIDER env var should override the default."""
        captured_config = {}
        ...
        with patch("cassandra.proxy.server.run_server", mock_run_server):
            result = runner.invoke(
                ...
                env={"CASSANDRA_ANYLLM_PROVIDER": "llamacpp"},
                catch_exceptions=False,
            )
...
class TestCLICompressionOnlyFlags:
    ...
    that can't resolve the injected cassandra_retrieve tool (issue #645).
    """
    ...
    def test_no_ccr_marker_from_env(self, runner):
        """CASSANDRA_NO_CCR_MARKER env var disables marker injection."""
        captured_config = {}
...
class TestNoCcrMarkerCompressors:
    ...
    def test_search_compressor_suppresses_markers_with_enable_ccr_false(self):
        ...
        from cassandra.transforms.search_compressor import (
            SearchCompressor,
    ...
    def test_log_compressor_suppresses_markers_with_enable_ccr_false(self):
        ...
        from cassandra.transforms.log_compressor import (
            LogCompressor,
            LogCompressorConfig,
        )
    ...
    def test_diff_compressor_suppresses_markers_with_enable_ccr_false(self):
        ...
        from cassandra.transforms.diff_compressor import (
            DiffCompressor,
    ...
    def test_code_compressor_suppresses_markers_with_enable_ccr_false(self):
        ...
        from cassandra.transforms.code_compressor import (
            CodeAwareCompressor,
...
class TestArgparseBackendValidation:
    """Test that the argparse path (python -m cassandra.proxy.server) accepts litellm-* backends."""
    ...
    def test_proxy_config_from_env_reads_disable_kompress(self):
        """The direct server env path should honor CASSANDRA_DISABLE_KOMPRESS."""
        from cassandra.proxy.server import _proxy_config_from_env

        with patch.dict(os.environ, {"CASSANDRA_DISABLE_KOMPRESS": "1"}):
    ...
    def test_proxy_config_from_env_reads_disable_kompress_fallback(self):
        """The direct server env path should honor CASSANDRA_DISABLE_KOMPRESS_FALLBACK."""
        from cassandra.proxy.server import _proxy_config_from_env

        with patch.dict(os.environ, {"CASSANDRA_DISABLE_KOMPRESS_FALLBACK": "1"}):
            config = _proxy_config_from_env()

        assert config.disable_kompress_fallback is True

    def test_argparse_registers_keepalive_expiry_flag(self):
        """The argparse path (python -m cassandra.proxy.server) must register
        --keepalive-expiry as a float flag, so it can override the
        CASSANDRA_KEEPALIVE_EXPIRY fallback. A bad value makes argparse exit
        ...
        result = subprocess.run(
            [sys.executable, "-m", "cassandra.proxy.server", "--keepalive-expiry", "notafloat"],
            capture_output=True,
            text=True,
...
class TestCLIProxyExcludeToolsEnvVar:
    """CASSANDRA_EXCLUDE_TOOLS and CASSANDRA_TOOL_PROFILES must reach ProxyConfig via the Click path.

    Regression coverage for issue #825: the Click entrypoint (cassandra/cli/proxy.py)
    ...
    shared/deployed services that launch via `cassandra proxy`.
    """

    def test_exclude_tools_single_name_from_env(self, runner):
        """CASSANDRA_EXCLUDE_TOOLS=WebSearch propagates to ProxyConfig.exclude_tools."""
        captured_config = {}
        ...
        with patch("cassandra.proxy.server.run_server", mock_run_server):
            result = runner.invoke(
                main,
                ["proxy"],
                env={"CASSANDRA_EXCLUDE_TOOLS": "WebSearch"},
                catch_exceptions=False,
            )
    ...
    def test_exclude_tools_multi_name_from_env(self, runner):
        """CASSANDRA_EXCLUDE_TOOLS=WebSearch,WebFetch yields both names (and lowercased) in result."""
        captured_config = {}
        ...
        with patch("cassandra.proxy.server.run_server", mock_run_server):
            result = runner.invoke(
                main,
                ["proxy"],
                env={"CASSANDRA_EXCLUDE_TOOLS": "WebSearch,WebFetch"},
                catch_exceptions=False,
            )
    ...
    def test_exclude_tools_unset_leaves_none(self, runner):
        """Without CASSANDRA_EXCLUDE_TOOLS, exclude_tools stays None (DEFAULT_EXCLUDE_TOOLS used)."""
        captured_config = {}

        def mock_run_server(config, **kwargs):
            captured_config["config"] = config

        env = {k: v for k, v in os.environ.items() if k != "CASSANDRA_EXCLUDE_TOOLS"}
    ...
    def test_tool_profiles_from_env(self, runner):
        """CASSANDRA_TOOL_PROFILES=Grep:conservative propagates to ProxyConfig.tool_profiles."""
        captured_config = {}
        ...
        with patch("cassandra.proxy.server.run_server", mock_run_server):
            result = runner.invoke(
                main,
                ["proxy"],
                env={"CASSANDRA_TOOL_PROFILES": "Grep:conservative"},
                catch_exceptions=False,
            )
    ...
    def test_tool_profiles_unset_leaves_none(self, runner):
        """Without CASSANDRA_TOOL_PROFILES, tool_profiles stays None."""
        captured_config = {}

        def mock_run_server(config, **kwargs):
            captured_config["config"] = config

        env = {k: v for k, v in os.environ.items() if k != "CASSANDRA_TOOL_PROFILES"}
...
class TestCLIProxyRpmTpm:
    """--rpm/--tpm flags and CASSANDRA_RPM/CASSANDRA_TPM env vars must reach ProxyConfig."""
    ...
    def test_rpm_env_var(self, runner):
        """CASSANDRA_RPM=20 should set rate_limit_requests_per_minute to 20."""
        captured_config = {}
    ...
    def test_tpm_env_var(self, runner):
        """CASSANDRA_TPM=80000 should set rate_limit_tokens_per_minute to 80000."""
        captured_config = {}
