...
class TestModelResolutionCaching:
    ...
    def setup_method(self):
        ...
        import cassandra.pricing.litellm_pricing as lp
    ...
    def test_cache_returns_same_result_on_second_call(self):
        ...
        with patch(
            "cassandra.pricing.litellm_pricing._resolve_litellm_model_uncached",
    ...
    def test_uncached_adds_provider_prefix_for_claude(self):
        ...
        with (
            patch("cassandra.pricing.litellm_pricing.LITELLM_AVAILABLE", True),
            patch("cassandra.pricing.litellm_pricing.litellm") as mock_litellm,
        ):
    ...
    def test_uncached_returns_original_when_litellm_unavailable(self):
        ...
        with patch("cassandra.pricing.litellm_pricing.LITELLM_AVAILABLE", False):
            result = lp._resolve_litellm_model_uncached("claude-opus-4-6")
...
class TestStreamingErrorHandling:
    ...
    def _create_mock_proxy(self):
        """Create a CassandraProxy-like object with mocked internals for testing generate()."""
...
class TestConcurrentSessionSafety:
    ...
    async def test_estimate_cost_concurrent_with_caching(self):
        ...
        import cassandra.pricing.litellm_pricing as lp
        from cassandra.proxy.server import CostTracker
        ...
        with (
            patch("cassandra.proxy.cost.LITELLM_AVAILABLE", True),
            patch("cassandra.pricing.litellm_pricing.litellm") as mock_litellm,
            patch("cassandra.proxy.cost.litellm") as mock_cost_litellm,
        ):
            mock_litellm.cost_per_token.return_value = (0.001, 0.002)
...
class TestCostTrackingAccuracy:
    ...
    def test_estimate_cost_separates_input_and_cache(self):
        ...
        with (
            patch("cassandra.proxy.cost.LITELLM_AVAILABLE", True),
            patch("cassandra.proxy.cost.litellm") as mock_litellm,
        ):
    ...
    def test_estimate_cost_returns_none_without_litellm(self):
        ...
        with patch("cassandra.proxy.cost.LITELLM_AVAILABLE", False):
            cost = tracker.estimate_cost("gpt-4o", input_tokens=1000, output_tokens=100)
            assert cost is None
