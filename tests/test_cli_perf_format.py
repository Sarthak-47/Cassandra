"""Tests for `cassandra perf --format {text,json,csv}` (issue #595)."""
...
from cassandra.perf import analyzer
from cassandra.perf.analyzer import (
    PerfRecord,
    PerfReport,
    TransformRecord,
    build_perf_summary,
...
def test_parse_perf_line_preserves_client_field(monkeypatch, tmp_path):
    ...
    (log_dir / "proxy.log").write_text(
        "2026-06-10 10:00:00,000 - cassandra.proxy - INFO - "
...
def test_perf_text_default_unchanged(runner, monkeypatch):
    ...
    assert result.exit_code == 0, result.output
    assert "Cassandra Performance Report" in result.output
...
def test_parse_perf_line_preserves_blank_client_field(
    ...
    (logs_dir / "proxy.log").write_text(
        "2026-06-10 10:00:00,000 - cassandra.proxy - INFO - [req-blank] PERF "
...
def test_throughput_parsing_and_calculations(monkeypatch, tmp_path):
    ...
    log_content = (
        '2026-06-10 10:00:00,000 - cassandra.proxy - INFO - [req1] STAGE_TIMINGS {"event": "stage_timings", "stages": {"compression_first_stage": 100.0, "upstream_connect": 50.0}}\n'
        "2026-06-10 10:00:01,000 - cassandra.proxy - INFO - [req1] PERF model=gpt-5 msgs=1 tok_before=1000 tok_after=400 tok_saved=600 opt_ms=10 total_ms=500 tok_out=500 ttfb_ms=100 transforms=test client=codex\n"
        '2026-06-10 10:00:02,000 - cassandra.proxy - INFO - [req2] STAGE_TIMINGS {"event": "stage_timings", "stages": {"compression": 200.0, "upstream_connect": 50.0}}\n'
        "2026-06-10 10:00:03,000 - cassandra.proxy - INFO - [req2] PERF model=gpt-5 msgs=1 tok_before=2000 tok_after=1000 tok_saved=1000 opt_ms=20 total_ms=1000 tok_out=1000 ttfb_ms=200 transforms=test client=codex\n"
        "2026-06-10 10:00:05,000 - cassandra.proxy - INFO - [req3] PERF model=gpt-5 msgs=1 tok_before=1500 tok_after=500 tok_saved=1000 opt_ms=15 total_ms=600 tok_out=600 ttfb_ms=150 transforms=test client=codex\n"
        '2026-06-10 10:00:06,000 - cassandra.proxy - INFO - [req4] STAGE_TIMINGS {"event": "stage_timings", "stages": {"compression_first_stage": 150.0, "upstream_connect": 50.0}}\n'
        "2026-06-10 10:00:07,000 - cassandra.proxy - INFO - [req4] PERF model=gpt-5 msgs=1 tok_before=1200 tok_after=300 tok_saved=900 opt_ms=12 total_ms=400 tok_out=400 ttfb_ms=80 transforms=test client=codex\n"
        '2026-06-10 10:00:08,000 - cassandra.proxy - INFO - [req5] STAGE_TIMINGS {"event": "stage_timings", "stages": {"compression_first_stage": 50.0, "upstream_connect": 50.0}}\n'
        "2026-06-10 10:00:09,000 - cassandra.proxy - INFO - [req5] PERF model=gpt-5 msgs=1 tok_before=800 tok_after=200 tok_saved=600 opt_ms=5 total_ms=300 tok_out=300 ttfb_ms=50 transforms=test client=codex\n"
    )
