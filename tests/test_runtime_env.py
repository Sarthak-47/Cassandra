...
from cassandra.proxy import runtime_env as rt
...
from cassandra.proxy.server import ProxyConfig, create_app  # noqa: E402
...
def test_getenv_falls_back_to_environment(monkeypatch):
    monkeypatch.setenv("CASSANDRA_OUTPUT_SHAPER", "1")
    assert rt.getenv("CASSANDRA_OUTPUT_SHAPER") == "1"
    assert rt.getenv("CASSANDRA_VERBOSITY_LEVEL", "2") == "2"  # unset -> default
    assert rt.getenv("CASSANDRA_VERBOSITY_LEVEL") is None


def test_getenv_override_wins_over_environment(monkeypatch):
    monkeypatch.setenv("CASSANDRA_OUTPUT_SHAPER", "0")
    rt.set_overrides({"CASSANDRA_OUTPUT_SHAPER": "1"})
    assert rt.getenv("CASSANDRA_OUTPUT_SHAPER") == "1"
...
def test_set_overrides_ignores_unknown_keys_and_non_strings():
    applied = rt.set_overrides(
        {
            "CASSANDRA_OUTPUT_SHAPER": "1",
            "NOT_A_KNOB": "x",
            "CASSANDRA_VERBOSITY_LEVEL": 3,  # non-string ignored
        }
    )
    assert applied == {"CASSANDRA_OUTPUT_SHAPER": "1"}
...
def test_explicit_env_returns_only_explicitly_set_knobs():
    environ = {
        "CASSANDRA_OUTPUT_SHAPER": "1",
        "CASSANDRA_MECHANICAL_EFFORT": "low",
        "CASSANDRA_VERBOSITY_LEVEL": "   ",  # blank -> not "explicitly set"
...
def test_effective_runtime_env_reports_override_or_none(monkeypatch):
    monkeypatch.setenv("CASSANDRA_EFFORT_ROUTER", "0")
    rt.set_overrides({"CASSANDRA_OUTPUT_SHAPER": "1"})
    eff = rt.effective_runtime_env()
    assert eff["CASSANDRA_OUTPUT_SHAPER"] == "1"  # from override
    assert eff["CASSANDRA_EFFORT_ROUTER"] == "0"  # from env
    assert eff["CASSANDRA_VERBOSITY_LEVEL"] is None  # unset
...
def test_clear_overrides_resets(monkeypatch):
    rt.set_overrides({"CASSANDRA_OUTPUT_SHAPER": "1"})
    rt.clear_overrides()
    assert rt.getenv("CASSANDRA_OUTPUT_SHAPER") is None
...
def test_override_enables_output_shaper_without_env():
    from cassandra.proxy.output_shaper import OutputShaperSettings

    assert OutputShaperSettings.from_env().enabled is False
    rt.set_overrides({"CASSANDRA_OUTPUT_SHAPER": "1", "CASSANDRA_VERBOSITY_LEVEL": "3"})
...
def test_override_changes_astgrep_threshold_without_env():
    from cassandra.proxy.interceptors import astgrep

    assert astgrep._min_chars_to_rewrite() == 500
    rt.set_overrides({"CASSANDRA_INTERCEPT_READ_MIN_CHARS": "999"})
    assert astgrep._min_chars_to_rewrite() == 999
    # Bad value falls back to the documented default rather than raising.
    rt.set_overrides({"CASSANDRA_INTERCEPT_READ_MIN_CHARS": "not-an-int"})
    assert astgrep._min_chars_to_rewrite() == 500
...
def loopback_client(monkeypatch):
    monkeypatch.setenv("CASSANDRA_SKIP_UPSTREAM_CHECK", "1")
    config = ProxyConfig(
        optimize=False,
        cache_enabled=False,
...
def test_health_exposes_runtime_env(loopback_client):
    ...
    assert config["runtime_env"]["CASSANDRA_OUTPUT_SHAPER"] is None


def test_admin_runtime_env_applies_and_reflects_in_health(loopback_client):
    resp = loopback_client.post(
        "/admin/runtime-env",
        json={"CASSANDRA_OUTPUT_SHAPER": "1", "CASSANDRA_VERBOSITY_LEVEL": "3", "BOGUS": "x"},
    )
    assert resp.status_code == 200
    body = resp.json()
    assert body["applied"] == {"CASSANDRA_OUTPUT_SHAPER": "1", "CASSANDRA_VERBOSITY_LEVEL": "3"}
    assert body["runtime_env"]["CASSANDRA_OUTPUT_SHAPER"] == "1"
    # And it is observable on the live /health surface.
    health = loopback_client.get("/health").json()["config"]["runtime_env"]
    assert health["CASSANDRA_OUTPUT_SHAPER"] == "1"
    assert health["CASSANDRA_VERBOSITY_LEVEL"] == "3"
...
def test_admin_runtime_env_is_loopback_only():
    ...
    with TestClient(app, base_url="http://127.0.0.1", client=("10.0.0.1", 54321)) as external:
        resp = external.post("/admin/runtime-env", json={"CASSANDRA_OUTPUT_SHAPER": "1"})
    assert resp.status_code == 404  # invisible to non-loopback callers
    assert rt.getenv("CASSANDRA_OUTPUT_SHAPER") is None  # nothing applied
...
def test_push_runtime_env_posts_explicit_env(monkeypatch):
    ...
    from cassandra.cli import wrap

    monkeypatch.setenv("CASSANDRA_OUTPUT_SHAPER", "1")
    monkeypatch.setenv("CASSANDRA_VERBOSITY_LEVEL", "3")
