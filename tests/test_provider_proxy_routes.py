...
from cassandra.proxy.server import CassandraProxy, ProxyConfig, create_app
...
def test_provider_passthrough_routes_forward_expected_targets(monkeypatch) -> None:
    ...
    monkeypatch.setattr(CassandraProxy, "handle_passthrough", fake_passthrough)
    monkeypatch.setattr(CassandraProxy, "handle_gemini_generate_content", fake_gemini_generate)
    monkeypatch.setattr(CassandraProxy, "handle_gemini_count_tokens", fake_gemini_count)
    monkeypatch.setattr(CassandraProxy, "handle_anthropic_messages", fake_anthropic_messages)
    ...
    with TestClient(_app()) as client:
        ...
        assert (
            client.get(
                ...
                headers={
                    "api-key": "azure-key",
                    "x-cassandra-base-url": "https://azure.example/openai/",
                },
            ).json()["base_url"]
        ...
        assert (
            client.get(
                "/unhandled/path",
                headers={"x-cassandra-base-url": "https://custom.example/base/"},
            ).json()["base_url"]
...
def test_proxy_route_helpers_prefer_legacy_targets_and_gemini_passthrough() -> None:
    proxy_routes = importlib.import_module("cassandra.providers.proxy_routes")
    proxy = type(
        "Proxy",
        (),
        {
    ...
    assert (
        proxy_routes._select_passthrough_base_url(
            proxy, {"api-key": "azure", "x-cassandra-base-url": "https://azure.example/base/"}
        )
...
def test_provider_specific_routes_delegate_to_expected_proxy_handlers(monkeypatch) -> None:
    ...
    def install(name: str) -> None:
        ...
        monkeypatch.setattr(CassandraProxy, name, fake)
...
def test_openai_response_websocket_aliases_delegate_to_openai_ws_handler(monkeypatch) -> None:
    ...
    monkeypatch.setattr(CassandraProxy, "handle_openai_responses_ws", fake_ws)
...
def test_openai_response_subpath_passthrough_returns_502_on_http_failure() -> None:
    ...
    with TestClient(_app()) as client:
        ...
        with patch("cassandra.providers.proxy_routes.logger") as logger:
...
def test_openai_response_subpath_aliases_and_chatgpt_auth_use_expected_targets(monkeypatch) -> None:
    monkeypatch.setattr(
        "cassandra.providers.proxy_routes._resolve_codex_routing_headers",
...
def test_openai_image_routes_use_codex_backend_under_chatgpt_auth(monkeypatch) -> None:
    ...
    assert "x-cassandra-bypass" not in generate_headers
...
def test_v1_models_still_forwards_under_non_chatgpt_auth() -> None:
    ...
    with patch.object(CassandraProxy, "handle_passthrough", fake_passthrough):
        with TestClient(_app()) as client:
            response = client.get(
                "/v1/models",
