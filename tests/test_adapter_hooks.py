...
from cassandra.cache.backends import CompressionStoreBackend, InMemoryBackend
...
from cassandra.storage import Storage, create_storage
from cassandra.telemetry.backends import FileSystemTOINBackend, TOINBackend
from cassandra.telemetry.models import ToolSignature
...
class TestTOINEntryPointLoading:
    ...
    def test_no_env_returns_none(self, monkeypatch):
        """No CASSANDRA_TOIN_BACKEND env → returns None (use default)."""
        monkeypatch.delenv("CASSANDRA_TOIN_BACKEND", raising=False)
        assert _create_default_toin_backend() is None

    def test_empty_env_returns_none(self, monkeypatch):
        """Empty CASSANDRA_TOIN_BACKEND → returns None."""
        monkeypatch.setenv("CASSANDRA_TOIN_BACKEND", "")
        assert _create_default_toin_backend() is None

    def test_filesystem_env_returns_none(self, monkeypatch):
        """CASSANDRA_TOIN_BACKEND=filesystem → returns None (use default)."""
        monkeypatch.setenv("CASSANDRA_TOIN_BACKEND", "filesystem")
        assert _create_default_toin_backend() is None

    def test_unknown_backend_returns_none(self, monkeypatch):
        """Unknown backend name with no entry point → returns None with warning."""
        monkeypatch.setenv("CASSANDRA_TOIN_BACKEND", "nonexistent_backend_xyz")
    ...
    def test_get_toin_respects_env_backend(self, monkeypatch, tmp_toin_path):
        ...
        monkeypatch.delenv("CASSANDRA_TOIN_BACKEND", raising=False)
        monkeypatch.setenv("CASSANDRA_TOIN_PATH", tmp_toin_path)

        toin = get_toin()
        assert toin is not None
        # Should use FileSystemTOINBackend since no CASSANDRA_TOIN_BACKEND set
...
class TestCCREntryPointLoading:
    ...
    def test_no_env_returns_sqlite(self, monkeypatch, tmp_path):
        """No CASSANDRA_CCR_BACKEND → SQLiteBackend (the persistent default;
        restart survival + cross-worker sharing for the 30-min TTL)."""
        monkeypatch.delenv("CASSANDRA_CCR_BACKEND", raising=False)
        monkeypatch.setenv("CASSANDRA_CCR_SQLITE_PATH", str(tmp_path / "ccr.db"))
        from cassandra.cache.compression_store import _create_default_ccr_backend
    ...
    def test_memory_env_returns_none(self, monkeypatch):
        """CASSANDRA_CCR_BACKEND=memory → returns None (use default)."""
        monkeypatch.setenv("CASSANDRA_CCR_BACKEND", "memory")
        from cassandra.cache.compression_store import _create_default_ccr_backend
    ...
    def test_unknown_backend_returns_none(self, monkeypatch):
        ...
        monkeypatch.setenv("CASSANDRA_CCR_BACKEND", "nonexistent_backend_xyz")
        from cassandra.cache.compression_store import _create_default_ccr_backend
...
class TestStorageEntryPointLoading:
    ...
    def test_sqlite_scheme(self, tmp_path):
        ...
        from cassandra.storage.sqlite import SQLiteStorage
    ...
    def test_jsonl_scheme(self, tmp_path):
        ...
        from cassandra.storage.jsonl import JSONLStorage
