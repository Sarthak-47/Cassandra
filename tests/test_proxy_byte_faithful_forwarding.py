...
``CASSANDRA_PROXY_PYTHON_FORWARDER_MODE=legacy_json_kwarg`` for emergency
...
from cassandra.pipeline import PipelineStage
...
def test_python_forwarder_mode_default_is_byte_faithful(
    ...
    monkeypatch.delenv("CASSANDRA_PROXY_PYTHON_FORWARDER_MODE", raising=False)
...
def test_python_forwarder_mode_invalid_raises(
    ...
    monkeypatch.setenv("CASSANDRA_PROXY_PYTHON_FORWARDER_MODE", "garbage")
    with pytest.raises(ValueError, match="CASSANDRA_PROXY_PYTHON_FORWARDER_MODE"):
        get_python_forwarder_mode()
...
def test_python_forwarder_mode_legacy_value_accepted(
    ...
    monkeypatch.setenv("CASSANDRA_PROXY_PYTHON_FORWARDER_MODE", "legacy_json_kwarg")
...
def test_log_outbound_request_emits_structured_fields() -> None:
    ...
    proxy_logger = logging.getLogger("cassandra.proxy")
...
def test_openai_chat_memory_disabled_mode_no_op(
    ...
    monkeypatch.setenv("CASSANDRA_MEMORY_INJECTION_MODE", "disabled")
    config = ProxyConfig(
        optimize=False,
        cache_enabled=False,
        rate_limit_enabled=False,
