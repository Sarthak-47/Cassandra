...
from cassandra.providers.copilot.wrap import (
    build_launch_env,
...
def test_detect_running_proxy_backend_requires_string_backend(monkeypatch) -> None:
    monkeypatch.setattr(
        "cassandra.providers.copilot.wrap.query_proxy_config",
...
def test_resolve_provider_type_prefers_explicit_and_env() -> None:
    assert (
        resolve_provider_type(
            "anthropic",
            "openai",
            {"COPILOT_PROVIDER_TYPE": "anthropic", "CASSANDRA_BACKEND": "anthropic"},
        )
        == "openai"
    )
    assert (
        resolve_provider_type(
            "anthropic",
            "auto",
            {"COPILOT_PROVIDER_TYPE": "openai", "CASSANDRA_BACKEND": "anthropic"},
        )
        == "openai"
    )
    assert (
        resolve_provider_type(
            None,
            "auto",
            {"COPILOT_PROVIDER_TYPE": "not-a-provider", "CASSANDRA_BACKEND": "anthropic"},
        )
        == "anthropic"
    )
    assert resolve_provider_type(None, "auto", {"CASSANDRA_BACKEND": "anthropic"}) == "anthropic"
    assert resolve_provider_type(None, "auto", {"CASSANDRA_BACKEND": "anyllm"}) == "openai"
