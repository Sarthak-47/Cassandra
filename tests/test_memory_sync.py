...
from cassandra.memory.sync import (
    _build_sync_backend,
    sync,
    sync_export,
    sync_import,
...
class TestSyncExport:
    ...
    async def test_export_new_memory_to_claude_files(self, backend, claude_dir):
        ...
        md_files = list(claude_dir.glob("cassandra_*.md"))
        assert len(md_files) == 1

        content = md_files[0].read_text()
        assert "Python 3.12" in content
        assert "cassandra_id: mem_0001" in content
    ...
    async def test_export_updates_memory_md_index(self, backend, claude_dir):
        ...
        assert "Cassandra Shared Memory" in memory_md
...
class TestBidirectionalSync:
    ...
    async def test_bidirectional_sync(self, backend, claude_dir, state_path):
        ...
        all_files = list(claude_dir.glob("cassandra_*.md"))
...
class TestLineageAndGovernance:
    ...
    @pytest.mark.asyncio
    async def test_exported_files_have_cassandra_id(self, backend, claude_dir):
        ...
        assert len(md_files) == 1
        content = md_files[0].read_text()
        assert "cassandra_id:" in content
...
class TestClaudeCodeAdapter:
    ...
    async def test_write_creates_valid_md(self, memory_dir):
        ...
        assert written == 1
        files = list(memory_dir.glob("cassandra_*.md"))
        assert len(files) == 1

        content = files[0].read_text()
        fm, body = _parse_frontmatter(content)
        assert fm["type"] == "architecture"
        assert fm["cassandra_id"] == "mem_001"
...
class TestCodexAdapter:
    ...
    async def test_read_from_agents_md(self, agents_md):
        agents_md.write_text(
            "# Instructions\n\n"
            "<!-- cassandra:memory:start -->\n"
            "## Cassandra Shared Memory\n\n"
            "- Secret name is TC\n"
            "- Uses Python 3.12\n"
            "<!-- cassandra:memory:end -->\n"
        )
    ...
    async def test_write_to_agents_md(self, agents_md):
        ...
        assert "cassandra:memory:start" in content
...
class TestCrossAgentInterop:
    ...
    async def test_codex_saves_claude_finds(self, backend, claude_dir, state_path):
        ...
        files = list(claude_dir.glob("cassandra_*.md"))
        assert len(files) == 1
    ...
    async def test_full_round_trip(self, backend, claude_dir, agents_md, state_path):
        ...
        all_claude = " ".join(f.read_text() for f in claude_dir.glob("cassandra_*.md"))
