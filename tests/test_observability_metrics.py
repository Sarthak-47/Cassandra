...
from cassandra.observability import CassandraOtelMetrics, reset_otel_metrics, set_otel_metrics
...
def test_cassandra_otel_metrics_records_proxy_and_pipeline_metrics() -> None:
    reader = InMemoryMetricReader()
    provider = MeterProvider(metric_readers=[reader])
    otel_metrics = CassandraOtelMetrics(meter_provider=provider)
    ...
    requests = metrics["cassandra.proxy.requests"]
    ...
    latency = metrics["cassandra.proxy.request.duration"]
    ...
    ttl_tokens = metrics["cassandra.proxy.cache.write_ttl_tokens"]
    ...
    compression_runs = metrics["cassandra.compression.runs"]
    ...
    stage_duration = metrics["cassandra.compression.stage.duration"]
    ...
    waste_tokens = metrics["cassandra.compression.waste.tokens"]
...
def test_proxy_failure_and_rate_limit_metrics_include_provider_labels() -> None:
    ...
    failed_point = _find_point(metrics["cassandra.proxy.requests.failed"], provider="openai")
    assert failed_point.value == 1

    rate_limited_point = _find_point(
        metrics["cassandra.proxy.requests.rate_limited"],
        provider="anthropic",
        model="claude-sonnet",
    )
