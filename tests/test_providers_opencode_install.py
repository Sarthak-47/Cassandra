...
def test_apply_provider_scope_creates_config(
    ...
    """apply_provider_scope creates the opencode config with cassandra provider."""
    home = str(tmp_path)
    ...
    assert config["provider"]["cassandra"]["options"]["baseURL"] == "http://127.0.0.1:8787/v1"
...
def test_revert_provider_scope_restores_file(
    ...
    """revert_provider_scope strips the Cassandra block from the config."""
    home = str(tmp_path)
    monkeypatch.setenv("HOME", home)
    monkeypatch.setenv("USERPROFILE", home)
    monkeypatch.delenv("OPENCODE_HOME", raising=False)
