...
def _make_proxy():  # noqa: ANN202 — returns the internal CassandraProxy
    app = create_app(
        ProxyConfig(
            optimize=True,
            cache_enabled=False,
...
def test_gemini_handlers_are_async_and_import_the_timeout() -> None:
    ...
    from cassandra.proxy.handlers import gemini
...
async def test_compression_offload_runs_on_worker_thread() -> None:
    """apply() runs on a 'cassandra-compress' executor thread, not the event-loop thread."""
    proxy = _make_proxy()
    ...
    assert seen["thread"].startswith("cassandra-compress")
