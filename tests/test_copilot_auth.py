...
def _isolated_copilot_auth(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv("CASSANDRA_COPILOT_AUTH_FILE", str(tmp_path / "copilot_auth.json"))
...
def test_read_cached_oauth_token_prefers_cassandra_login(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("GITHUB_COPILOT_TOKEN", "gho-env")
    copilot_auth.save_cassandra_copilot_oauth_token("gho-cassandra")

    assert copilot_auth.read_cached_oauth_token() == "gho-cassandra"
...
def test_resolve_subscription_bearer_token_details_exchanges_oauth_candidate(
    ...
    monkeypatch.setattr(
        ...
        lambda: [
            copilot_auth.CopilotTokenCandidate(
                token="gho-oauth",
                source="cassandra-copilot-auth:/tmp/copilot_auth.json",
    ...
    assert resolution.source == "cassandra-copilot-auth:/tmp/copilot_auth.json:token-exchange"
