...
- 2.1 optional inbound auth token (CASSANDRA_PROXY_TOKEN) on the data plane
- 3.1 response security headers
- 2.4 admin/state-mutating audit log
- 2.2 air-gap master switch (CASSANDRA_OFFLINE)
"""
...
from cassandra.cache.compression_store import reset_compression_store
from cassandra.offline import apply_offline_env, is_offline
from cassandra.proxy.audit import is_auditable_path
...
class TestInboundAuthToken:
    ...
    def test_token_set_accepts_custom_header(self):
        app = _make_app(proxy_token="s3cr3t-token")
        with TestClient(app, base_url="http://testserver", client=NONLOOPBACK) as c:
            resp = c.get("/stats", headers={"X-Cassandra-Proxy-Token": "s3cr3t-token"})
            assert resp.status_code != 401
...
class TestAdminAuditLog:
    ...
    def test_cache_clear_emits_audit_event(self):
        ...
        handler = _Capture()
        audit_logger = logging.getLogger("cassandra.audit")
        ...
        assert any("cassandra_admin_audit" in m for m in messages)
...
class TestOfflineSwitch:
    def test_is_offline_reads_env(self, monkeypatch):
        monkeypatch.delenv("CASSANDRA_OFFLINE", raising=False)
        assert is_offline() is False
        monkeypatch.setenv("CASSANDRA_OFFLINE", "1")
        assert is_offline() is True
        monkeypatch.setenv("CASSANDRA_OFFLINE", "off")
        assert is_offline() is False
    ...
    def test_offline_disables_update_check(self, monkeypatch):
        from cassandra.update_check import is_update_check_enabled

        monkeypatch.delenv("CI", raising=False)
        monkeypatch.delenv("CASSANDRA_STATELESS", raising=False)
        monkeypatch.setenv("CASSANDRA_OFFLINE", "1")
