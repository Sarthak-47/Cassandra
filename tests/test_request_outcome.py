"""Tests for :class:`cassandra.proxy.outcome.RequestOutcome` and the
:meth:`CassandraProxy._record_request_outcome` funnel.
...
def test_cache_hit_pct_rounds_to_int() -> None:
    """PERF log line consumed by ``cassandra perf`` parses an integer here;
...
class _FunnelHarness:
    """Pulls just enough of CassandraProxy onto an object to exercise
    ...
    def __init__(self, *, with_cost_tracker: bool = True, with_logger: bool = True) -> None:
        ...
        self._record_request_outcome = CassandraProxy._record_request_outcome.__get__(
            self, type(self)
        )
...
async def test_funnel_emits_perf_log_with_canonical_shape(
    ...
    """``cassandra perf`` parses this exact ``key=value`` format. Changing
    ...
    # records (the proxy disables ``cassandra.*`` propagation once started).
...
async def test_funnel_appends_client_to_perf_log_when_set() -> None:
    """``cassandra perf --client X`` filtering relies on the ``client=X``
