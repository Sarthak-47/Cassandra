...
``cassandra_core::compression_policy::CompressionPolicy``. These tests
...
``crates/cassandra-core/src/compression_policy.rs``.
"""
...
from cassandra.proxy.auth_mode import AuthMode
from cassandra.transforms.compression_policy import (
    CompressionPolicy,
    policy_default_payg,
    policy_for_mode,
)
...
class TestRustParityFieldMap:
    ...
    ``crates/cassandra-core/src/compression_policy.rs``. When you add a
    ...
    def test_field_set_matches_rust(self):
        ...
        assert actual_fields == expected_fields, (
            ...
            f"Update both `cassandra/transforms/compression_policy.py` "
            f"and `crates/cassandra-core/src/compression_policy.rs` in "
            f"the same commit."
        )
...
class TestNetCostFormula:
    ...
    ``crates/cassandra-core/src/compression_policy.rs`` assert the
