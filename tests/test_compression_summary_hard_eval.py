...
class TestHardCases:
    ...
    def test_config_lookup_with_summary(self):
        ...
        compressed_output += (
            ...
            f" Omitted: {summary}."
            f' Retrieve specific items: cassandra_retrieve(hash="config_hash", query="search")]'
        )
        ...
        resp = _call_claude(messages, [CASSANDRA_RETRIEVE_TOOL])
        ...
        if resp.get("stop_reason") == "tool_use":
            assert tool_calls[0]["name"] == "cassandra_retrieve"
    ...
    def test_specific_user_in_large_list_with_summary(self):
        ...
        compressed_output += (
            f"\n[{len(all_items) - len(visible)} items compressed to {len(visible)}."
            f" Omitted: {summary}."
            f' Retrieve: cassandra_retrieve(hash="users_hash", query="search")]'
        )
