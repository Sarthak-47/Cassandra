...
from cassandra.proxy.memory_handler import (
...
async def test_concurrent_ensure_initialized_runs_init_once(tmp_path, monkeypatch):
    ...
    import cassandra.memory.backends.local as local_mod
...
async def test_ensure_initialized_timeout_leaves_handler_unready(tmp_path, monkeypatch):
    ...
    mem_logger = _logging.getLogger("cassandra.proxy.memory_handler")
    ...
    try:
        ...
        with patch("cassandra.proxy.memory_handler.STARTUP_INIT_TIMEOUT_SECONDS", 0.1):
            await handler._ensure_initialized()
    finally:
...
async def test_ensure_initialized_timeout_nulls_partially_initialized_backend(
    ...
    with patch("cassandra.proxy.memory_handler.STARTUP_INIT_TIMEOUT_SECONDS", 0.01):
        await handler._ensure_initialized()
