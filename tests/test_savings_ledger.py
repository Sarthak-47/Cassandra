"""Tests for the durable savings event ledger and the `cassandra savings` CLI."""
...
from cassandra import savings_ledger as L
...
def _events_env(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("CASSANDRA_SAVINGS_EVENTS_PATH", str(path))
    return path
...
def test_cli_reset_deletes_ledger(monkeypatch, tmp_path):
    ...
    from cassandra.cli.savings import savings
...
def test_mcp_compress_records_durable_event(monkeypatch, tmp_path):
    ...
    from cassandra.ccr import mcp_server

    _events_env(monkeypatch, tmp_path)
    monkeypatch.setenv("CASSANDRA_MCP_CLIENT", "claude-code")

    server = mcp_server.CassandraMCPServer(check_proxy=False)
...
def test_proxy_record_request_appends_ledger_event(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CASSANDRA_SAVINGS_PATH", str(tmp_path / "proxy_savings.json"))
    monkeypatch.setenv("CASSANDRA_SAVINGS_EVENTS_PATH", str(tmp_path / "savings_events.jsonl"))
    monkeypatch.setattr(
