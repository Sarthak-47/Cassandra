"""Tests for the Codex read-pattern audit (cassandra.audit.codex)."""
...
from cassandra.audit.codex import audit_codex, classify_command, render_codex_text, strip_wrappers
...
class TestClassifier:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("nl cassandra/config.py", "read", False),
            ("rg -n 'def apply' cassandra/", "search", False),
    ...
    def test_sed_range_not_mistaken_for_path(self):
        _, path, _ = classify_command("sed -n '5,30p' cassandra/config.py")
        assert path == "cassandra/config.py"
