...
When frozen_message_count > 0, the old code deferred cassandra_retrieve tool
...
from cassandra.ccr.tool_injection import CCR_TOOL_NAME, CCRToolInjector
...
class TestCCRInjectionEndToEnd:
    ...
    def test_marker_in_frozen_prefix_yields_injected_tool(self):
        ...
        with patch("cassandra.proxy.helpers.get_session_ccr_tracker") as mock_tracker_fn:
        ...
        assert CCR_TOOL_NAME in tool_names, (
            f"cassandra_retrieve not injected when markers emitted and prefix frozen (#1006). "
            f"tools={tool_names}"
        )
    ...
    def test_no_marker_in_frozen_prefix_skips_tool(self):
        ...
        assert CCR_TOOL_NAME not in tool_names, (
            "cassandra_retrieve should NOT be injected when no markers and frozen prefix"
        )
