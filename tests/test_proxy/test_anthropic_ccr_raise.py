...
the raw ``cassandra_retrieve`` tool-call body to the client.
"""
...
def _tool_call_response() -> dict:
    """Anthropic-shaped response containing a cassandra_retrieve tool use block."""
    return {
        "id": "msg_ccr_test",
        "type": "message",
        "role": "assistant",
...
def test_anthropic_ccr_exception_reraises_not_swallowed():
    ...
    with patch("cassandra.proxy.server.AnyLLMBackend"):
    ...
    # the raw cassandra_retrieve tool-call body to the client as a 200.
    ...
    # Confirm the raw cassandra_retrieve tool_use block was NOT returned.
    for block in body.get("content", []):
        assert block.get("name") != "cassandra_retrieve", (
