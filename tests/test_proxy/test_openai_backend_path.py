...
2. Backend response with cassandra_retrieve tool call → ccr_response_handler.handle_response
...
from cassandra.backends.base import BackendResponse  # noqa: E402
...
def test_backend_response_updates_prefix_tracker_with_bedrock_cache_fields():
    ...
    with patch("cassandra.proxy.server.AnyLLMBackend", return_value=mock_backend):
...
def test_backend_response_with_ccr_tool_call_is_intercepted_and_resolved():
    """OpenAI-shape response carrying cassandra_retrieve → CCR handler resolves it."""
    config = _make_config()
    # First response: tool_call for cassandra_retrieve
    tool_call_response = {
        "id": "chatcmpl-ccr-1",
        "object": "chat.completion",
        "model": "anthropic.claude-3-5-sonnet-20241022-v2:0",
