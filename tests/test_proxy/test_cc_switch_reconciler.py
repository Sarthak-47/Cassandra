...
The reconciler keeps Cassandra in the request path while cc-switch overwrites
``~/.claude/settings.json`` on every provider switch. See
``cassandra/proxy/cc_switch_reconciler.py``.
"""
...
from cassandra.proxy.cc_switch_reconciler import CCSwitchReconciler
...
def test_third_party_captured_and_base_url_rewritten(tmp_path):
    ...
    # base_url repointed to Cassandra; token + model preserved verbatim.
...
def test_no_rewrite_loop(tmp_path):
    ...
    assert r.tick() is True
    # Already pointing at Cassandra now -> must be a no-op (no infinite loop).
...
def test_official_left_direct_by_default(tmp_path, monkeypatch):
    monkeypatch.delenv("CASSANDRA_CC_SWITCH_ROUTE_OFFICIAL", raising=False)
    r, sf, _ = _make(tmp_path)
    _write(sf, {"env": {}})
...
def test_official_routed_when_opted_in(tmp_path, monkeypatch):
    monkeypatch.setenv("CASSANDRA_CC_SWITCH_ROUTE_OFFICIAL", "1")
    r, sf, captured = _make(tmp_path)
    _write(sf, {"env": {}})
    assert r.tick() is True
...
def test_enabled_flag(monkeypatch, val, expected):
    from cassandra.proxy.cc_switch_reconciler import reconciler_enabled

    monkeypatch.setenv("CASSANDRA_CC_SWITCH_RECONCILE", val)
