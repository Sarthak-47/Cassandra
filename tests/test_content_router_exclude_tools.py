"""Tests for --protect-tool-results / CASSANDRA_PROTECT_TOOL_RESULTS.

Three behavioral tests:
1. protect_tool_results merges into the exclude set so named tools are never
   lossy-compressed.
2. _parse_csv_tools parses CSV strings without merging CASSANDRA_EXCLUDE_TOOLS.
...
from cassandra.config import DEFAULT_EXCLUDE_TOOLS
...
def _build(**overrides: object) -> CassandraProxy:
    config = ProxyConfig(
        optimize=False,
        cache_enabled=False,
    ...
    return CassandraProxy(config)


def _router(proxy: CassandraProxy):
...
def test_protect_tool_results_env_var_csv() -> None:
    ...
    and lowercase entries without merging CASSANDRA_EXCLUDE_TOOLS."""
