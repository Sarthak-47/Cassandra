...
HANDLER_FILES = [
    Path("cassandra/proxy/handlers/anthropic.py"),
    Path("cassandra/proxy/handlers/openai.py"),
    Path("cassandra/proxy/handlers/gemini.py"),
    Path("cassandra/proxy/handlers/batch.py"),
]
...
def test_every_search_and_format_context_call_passes_query_kwarg() -> None:
    ...
    if offenders:
        ...
        pytest.fail(
            ...
            "untruncated query value type. See cassandra/proxy/memory_query.py."
        )
