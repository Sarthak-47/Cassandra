...
from cassandra.cli.learn import _AgentChoice
...
class FakeWriter:
    ...
    def write(self, recommendations, project, dry_run: bool):  # noqa: ANN001, ANN201
        ...
        return SimpleNamespace(
            dry_run=dry_run,
            content_by_file={
                Path(project.project_path) / "AGENTS.md": "<!-- cassandra -->\nRule 1\nRule 2"
            },
        )
...
def test_agent_choice_convert_and_shell_complete(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "cassandra.learn.registry.get_registry",
        lambda: {"codex": object(), "claude": object()},
    )
    monkeypatch.setattr(
        "cassandra.learn.registry.available_agent_names",
        lambda: ["claude", "codex"],
    )
...
def test_learn_exits_cleanly_when_model_detection_fails(
    ...
    monkeypatch.setattr(
        "cassandra.learn.analyzer._detect_default_model",
...
def test_learn_auto_agent_reports_no_detected_plugins(
    ...
    monkeypatch.setattr("cassandra.learn.analyzer._detect_default_model", lambda: "gpt-4o")
    monkeypatch.setattr("cassandra.learn.registry.auto_detect_plugins", lambda: [])
    monkeypatch.setattr("cassandra.learn.analyzer.SessionAnalyzer", FakeAnalyzer)
...
def test_learn_single_agent_shows_available_projects_when_cwd_missing(
    ...
    monkeypatch.setattr("cassandra.learn.analyzer._detect_default_model", lambda: "gpt-4o")
    monkeypatch.setattr("cassandra.learn.registry.get_plugin", lambda name: plugin)
    monkeypatch.setattr("cassandra.learn.analyzer.SessionAnalyzer", FakeAnalyzer)
...
def test_learn_project_lookup_and_apply_flow(
    ...
    monkeypatch.setattr("cassandra.learn.analyzer._detect_default_model", lambda: "gpt-4o")
    monkeypatch.setattr("cassandra.learn.registry.get_plugin", lambda name: plugin)
    monkeypatch.setattr("cassandra.learn.analyzer.SessionAnalyzer", lambda model=None: analyzer)
...
def test_learn_analyze_all_uses_default_workers_and_prints_summary(
    ...
    monkeypatch.setattr("cassandra.learn.analyzer._detect_default_model", lambda: "gpt-4o")
    monkeypatch.setattr(
        "cassandra.learn.registry.auto_detect_plugins",
...
class TargetAwareWriter(FakeWriter):
    ...
    def write(self, recommendations, project, dry_run: bool):  # noqa: ANN001, ANN201
        ...
        return SimpleNamespace(
            dry_run=dry_run,
            content_by_file={
                Path(project.project_path) / "CLAUDE.local.md": "<!-- cassandra -->\nRule 1"
            },
            warnings=["Moved Cassandra learnings out of CLAUDE.md into CLAUDE.local.md."],
        )
...
def test_learn_target_threads_to_writer_and_prints_warnings(
    ...
    assert "Moved Cassandra learnings" in result.output
