...
Tests cover:
1. CassandraChatModel - Wrapper for any BaseChatModel
2. CassandraCallbackHandler - Metrics and observability
3. CassandraRunnable - LCEL chain composition
...
class TestLangchainAvailable:
    ...
    def test_returns_bool(self):
        ...
        from cassandra.integrations.langchain import langchain_available
...
class TestCassandraChatModel:
    ...
    def test_init_with_custom_config(self, mock_chat_model):
        ...
        model = CassandraChatModel(
            mock_chat_model,
            config=config,
            mode=CassandraMode.SIMULATE,
        )

        assert model.cassandra_config is config
        assert model.mode == CassandraMode.SIMULATE
    ...
    def test_llm_type(self, mock_chat_model):
        ...
        model = CassandraChatModel(mock_chat_model)
        assert "cassandra" in model._llm_type
    ...
    def test_identifying_params(self, mock_chat_model):
        ...
        assert "cassandra_mode" in params
    ...
    def test_generate_applies_optimization(self, mock_chat_model, sample_messages):
        """_generate applies Cassandra optimization."""
        from cassandra.integrations import CassandraChatModel
    ...
    def test_get_savings_summary_with_data(self, mock_chat_model):
        ...
        from cassandra.integrations import CassandraChatModel
        from cassandra.integrations.langchain import OptimizationMetrics
...
class TestCassandraCallbackHandler:
    """Tests for CassandraCallbackHandler."""

    def test_init_defaults(self):
        """Initialize with default settings."""
        from cassandra.integrations import CassandraCallbackHandler

        handler = CassandraCallbackHandler()
    ...
    def test_on_chat_model_start_triggers_alert(self):
        ...
        handler = CassandraCallbackHandler(token_alert_threshold=5)
...
class TestCassandraRunnable:
    """Tests for CassandraRunnable LCEL component."""

    def test_init_defaults(self):
        """Initialize with defaults."""
        from cassandra.integrations.langchain import CassandraRunnable

        runnable = CassandraRunnable()

        assert runnable.mode == CassandraMode.OPTIMIZE
    ...
    def test_init_custom_config(self):
        ...
        config = CassandraConfig(default_mode=CassandraMode.AUDIT)
        runnable = CassandraRunnable(config=config, mode=CassandraMode.SIMULATE)

        assert runnable.config is config
        assert runnable.mode == CassandraMode.SIMULATE
...
class TestOptimizeMessages:
    ...
    def test_basic_optimization(self, sample_messages):
        ...
        from cassandra.integrations import optimize_messages

        with patch("cassandra.integrations.langchain.chat_model.TransformPipeline") as MockPipeline:
...
class TestAinvokeStreamingTrue:
    ...
    def _patched_pipeline(self, streaming_mock_model):
        """Create a CassandraChatModel with a mocked optimization pipeline."""
        from cassandra.integrations import CassandraChatModel
        from cassandra.providers import OpenAIProvider

        model = CassandraChatModel(streaming_mock_model)
        model._provider = OpenAIProvider()
        _ = model.pipeline  # Force lazy init
...
class TestOllamaIntegration:
    ...
    def test_cassandra_chat_model_with_ollama(self, ollama_model_name):
        """Test CassandraChatModel wrapping real ChatOllama model."""
        ...
        cassandra_model = CassandraChatModel(base_model)

        assert cassandra_model.wrapped_model is base_model
        assert isinstance(cassandra_model, CassandraChatModel)
    ...
    def test_invoke_with_ollama(self, ollama_model_name):
        ...
        result = cassandra_model.invoke(messages)
    ...
    def test_generate_with_ollama(self, ollama_model_name):
        ...
        result = cassandra_model._generate(messages)
    ...
    def test_optimization_tracked_with_ollama(self, ollama_model_name):
        ...
        cassandra_model.invoke(messages)

        # Metrics should be tracked
        assert len(cassandra_model._metrics_history) >= 1
    ...
    def test_cassandra_optimization_reduces_tokens(self, ollama_model_name):
        """Test that Cassandra optimization actually reduces token count."""
        from langchain_ollama import ChatOllama

        from cassandra.integrations import CassandraChatModel

        base_model = ChatOllama(model=ollama_model_name)
        cassandra_model = CassandraChatModel(base_model, mode=CassandraMode.OPTIMIZE)
        ...
        if cassandra_model._metrics_history:
            metrics = cassandra_model._metrics_history[-1]
            # With a large conversation, we expect some savings
            assert metrics.tokens_before >= metrics.tokens_after

    def test_callback_handler_with_ollama(self, ollama_model_name):
        """Test CassandraCallbackHandler with real Ollama calls."""
        from langchain_ollama import ChatOllama

        from cassandra.integrations import CassandraCallbackHandler, CassandraChatModel
        ...
        cassandra_model.invoke(messages, config={"callbacks": [handler]})
...
class TestRealLangChainIntegration:
    ...
    def test_lcel_chain_with_cassandra(self, ollama_model_name):
        """Test LCEL chain composition with CassandraChatModel."""
        ...
        # Create chain: prompt -> cassandra model -> output parser
        base_model = ChatOllama(model=ollama_model_name)
        cassandra_model = CassandraChatModel(base_model)
        ...
        chain = prompt | cassandra_model | StrOutputParser()
