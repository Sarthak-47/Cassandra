...
# --- CassandraChatModel: invoke (sync) ---


class TestCassandraChatModelLiveOpenAI:
    """Live tests: CassandraChatModel wrapping ChatOpenAI."""

    def test_wrap_openai_and_invoke(self, openai_llm):
        from cassandra.integrations import CassandraChatModel

        model = CassandraChatModel(openai_llm)
...
class TestCassandraChatModelLiveAnthropic:
    """Live tests: CassandraChatModel wrapping ChatAnthropic.
    ...
    def test_wrap_anthropic_and_invoke(self, anthropic_llm):
        ...
        model = CassandraChatModel(anthropic_llm)
        messages = [HumanMessage(content="Reply with exactly: OK")]
        try:
            response = model.invoke(messages)
...
class TestCassandraChatModelStreamingLive:
...
class TestCassandraChatModelToolCallsLive:
    ...
    def test_messages_with_tool_result_compressed(self, openai_llm):
        """Conversation with tool call + large tool result; Cassandra should compress the tool result."""
        import json
...
class TestCassandraLCELive:
    """Live LCEL chain tests."""

    def test_prompt_pipe_cassandra_pipe_llm(self, openai_llm):
...
class TestOptimizeMessagesLive:
    """Live optimize_messages with real Cassandra pipeline (no API key needed for this)."""
