...
Tests cover:
1. CassandraAgnoModel - Wrapper for any Agno model
...
class TestCassandraAgnoModel:
    """Tests for CassandraAgnoModel wrapper."""

    def test_init_with_defaults(self, mock_agno_model):
        """Initialize with default config."""
        from cassandra.integrations.agno import CassandraAgnoModel

        model = CassandraAgnoModel(wrapped_model=mock_agno_model)

        assert model.wrapped_model is mock_agno_model
        assert model.cassandra_config is not None
    ...
    def test_init_with_custom_config(self, mock_agno_model):
        ...
        config = CassandraConfig(default_mode=CassandraMode.AUDIT)
        model = CassandraAgnoModel(
            wrapped_model=mock_agno_model,
            cassandra_config=config,
            cassandra_mode=CassandraMode.SIMULATE,
        )

        assert model.cassandra_config is config
        assert model.cassandra_mode == CassandraMode.SIMULATE

    def test_init_auto_detect_provider(self, mock_agno_model):
        """Auto-detect provider from wrapped model."""
        from cassandra.integrations.agno import CassandraAgnoModel

        model = CassandraAgnoModel(wrapped_model=mock_agno_model, auto_detect_provider=True)
    ...
    def test_convert_messages_normalizes_streaming_tool_call_objects(self, mock_agno_model):
        ...
        must flatten them to OpenAI-format dicts so neither the Cassandra
    ...
    def test_response_applies_optimization(self, mock_agno_model, sample_messages):
        """response() applies Cassandra optimization."""
        from cassandra.integrations.agno import CassandraAgnoModel
        from cassandra.providers import OpenAIProvider

        model = CassandraAgnoModel(wrapped_model=mock_agno_model)

        # Initialize provider and pipeline for mocking
        model._cassandra_provider = OpenAIProvider()
    ...
    def test_response_stream_applies_optimization(self, mock_agno_model, sample_messages):
        """response_stream() applies Cassandra optimization."""
        from cassandra.integrations.agno import CassandraAgnoModel
        from cassandra.providers import OpenAIProvider
    ...
    def test_get_savings_summary_with_data(self, mock_agno_model):
        ...
        from cassandra.integrations.agno import CassandraAgnoModel
        from cassandra.integrations.agno.model import OptimizationMetrics
...
class TestProviderDetection:
    ...
    def test_detect_openai_provider(self, mock_agno_model):
        ...
        from cassandra.integrations.agno.providers import get_cassandra_provider
        from cassandra.providers import OpenAIProvider

        provider = get_cassandra_provider(mock_agno_model)
    ...
    def test_detect_anthropic_provider(self, mock_claude_model):
        ...
        from cassandra.integrations.agno.providers import get_cassandra_provider
        from cassandra.providers import AnthropicProvider

        provider = get_cassandra_provider(mock_claude_model)
    ...
    def test_detect_from_model_id(self):
        ...
        from cassandra.integrations.agno.providers import get_cassandra_provider
        from cassandra.providers import AnthropicProvider, GoogleProvider, OpenAIProvider
        ...
        assert isinstance(get_cassandra_provider(mock_gpt), OpenAIProvider)
        ...
        assert isinstance(get_cassandra_provider(mock_claude), AnthropicProvider)
        ...
        assert isinstance(get_cassandra_provider(mock_gemini), GoogleProvider)
    ...
    def test_fallback_to_openai(self):
        ...
        provider = get_cassandra_provider(mock)

        assert isinstance(provider, OpenAIProvider)

    def test_get_model_name(self, mock_agno_model):
        """Extract model name from Agno model."""
        from cassandra.integrations.agno.providers import get_model_name_from_agno
...
class TestOptimizeMessages:
    ...
    def test_basic_optimization(self, sample_messages):
        ...
        with patch("cassandra.integrations.agno.model.TransformPipeline") as MockPipeline:
...
class TestIntegrationWithRealCassandra:
    """Integration tests using real Cassandra components (no mocking)."""

    def test_real_optimization_pipeline(self, sample_messages):
        """Test with real Cassandra client (no API calls)."""
        from cassandra.integrations.agno import optimize_messages

        # This uses real Cassandra transforms but no LLM API calls
        optimized, metrics = optimize_messages(
            sample_messages,
            mode=CassandraMode.OPTIMIZE,
        )
    ...
    def test_model_wrapper_real_optimization(self, mock_agno_model, sample_messages):
        """Test CassandraAgnoModel with real Cassandra optimization."""
        from cassandra.integrations.agno import CassandraAgnoModel
...
class TestReasoningCapabilityForwarding:
    """Tests for reasoning capability forwarding in CassandraAgnoModel.

    These tests verify that CassandraAgnoModel properly forwards
    ...
    def test_underlying_model_property_returns_wrapped_model(self, mock_agno_model):
        ...
        wrapped = CassandraAgnoModel(wrapped_model=mock_agno_model)
...
class TestRealAgnoIntegration:
    ...
    These tests verify that CassandraAgnoModel:
    ...
    def test_is_subclass_of_agno_model(self):
        """CassandraAgnoModel must be a subclass of agno.models.base.Model."""
        from agno.models.base import Model

        from cassandra.integrations.agno import CassandraAgnoModel

        assert issubclass(CassandraAgnoModel, Model)

    def test_passes_agno_get_model_validation(self):
        """CassandraAgnoModel must pass Agno's get_model() validation."""
        from agno.models.openai import OpenAIChat
        ...
        base_model = OpenAIChat(id="gpt-4o")
        cassandra_model = CassandraAgnoModel(wrapped_model=base_model)

        # This should NOT raise "Model must be a Model instance, string, or None"
        result = get_model(cassandra_model)

        assert result is cassandra_model
        assert isinstance(result, CassandraAgnoModel)

    def test_agent_accepts_cassandra_model(self):
        """Agno Agent must accept CassandraAgnoModel as model parameter."""
        ...
        agent = Agent(model=cassandra_model, markdown=False)

        assert agent.model is cassandra_model
        assert agent.model.wrapped_model is base_model

    def test_model_id_reflects_wrapped_model(self):
        """CassandraAgnoModel id should reflect the wrapped model."""
        from agno.models.openai import OpenAIChat
        ...
        assert "gpt-4o-mini" in cassandra_model.id
        assert cassandra_model.id.startswith("cassandra:")

    def test_cassandra_model_has_required_abstract_methods(self):
        """CassandraAgnoModel must implement all required abstract methods."""
        from agno.models.openai import OpenAIChat
        ...
        assert hasattr(cassandra_model, "invoke")
        assert callable(cassandra_model.invoke)

        assert hasattr(cassandra_model, "ainvoke")
        assert callable(cassandra_model.ainvoke)

        assert hasattr(cassandra_model, "invoke_stream")
        assert callable(cassandra_model.invoke_stream)

        assert hasattr(cassandra_model, "ainvoke_stream")
        assert callable(cassandra_model.ainvoke_stream)

        assert hasattr(cassandra_model, "_parse_provider_response")
        assert callable(cassandra_model._parse_provider_response)

        assert hasattr(cassandra_model, "_parse_provider_response_delta")
        assert callable(cassandra_model._parse_provider_response_delta)
    ...
    def test_isinstance_check_passes(self):
        ...
        assert isinstance(cassandra_model, Model)

    def test_model_with_custom_cassandra_config(self):
        """Test with custom Cassandra configuration."""
        from agno.agent import Agent
        from agno.models.openai import OpenAIChat
        ...
        config = CassandraConfig(default_mode=CassandraMode.AUDIT)
        base_model = OpenAIChat(id="gpt-4o")
        cassandra_model = CassandraAgnoModel(
        ...
        assert agent.model.cassandra_config is config
        assert agent.model.cassandra_config.default_mode == CassandraMode.AUDIT
    ...
    def test_response_method_delegates_to_wrapped(self):
        ...
        sig = inspect.signature(cassandra_model.response)
    ...
    def test_optimization_tracked_across_calls(self):
        ...
        assert cassandra_model.total_tokens_saved == 0
        assert len(cassandra_model.metrics_history) == 0
        ...
        optimized, metrics = cassandra_model._optimize_messages(messages)

        # Should have tracked metrics
        assert len(cassandra_model.metrics_history) == 1
        assert cassandra_model.total_tokens_saved >= 0
...
class TestOllamaIntegration:
    ...
    def test_agent_with_ollama_model(self, ollama_model_name):
        """Test Agent with CassandraAgnoModel wrapping real Ollama model."""
        from agno.agent import Agent
        ...
        # Create agent - this validates CassandraAgnoModel works with Agent
        agent = Agent(model=cassandra_model, markdown=False)

        assert agent.model is cassandra_model
        assert isinstance(agent.model, CassandraAgnoModel)
    ...
    def test_agent_run_with_ollama(self, ollama_model_name):
        ...
        # Verify Cassandra optimization was applied
        assert len(cassandra_model.metrics_history) >= 1
    ...
    def test_agent_with_system_prompt_and_ollama(self, ollama_model_name):
        ...
        # Cassandra should have processed the system prompt
        assert cassandra_model.total_tokens_saved >= 0
    ...
    def test_multiple_turns_with_ollama(self, ollama_model_name):
        ...
        assert len(cassandra_model.metrics_history) >= 2

    def test_cassandra_optimization_reduces_tokens(self, ollama_model_name, large_conversation):
        """Test that Cassandra actually reduces tokens on large conversations."""
        from agno.models.ollama import Ollama
        ...
        optimized, metrics = cassandra_model._optimize_messages(large_conversation)
