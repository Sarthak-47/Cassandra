...
Tests cover:
1. CassandraPreHook - Pre-hook for tracking before LLM calls
2. CassandraPostHook - Post-hook for tracking after LLM calls
3. create_cassandra_hooks - Convenience function
"""
...
class TestCassandraPreHook:
    """Tests for CassandraPreHook."""

    def test_init_defaults(self):
        """Initialize with default settings."""
        from cassandra.integrations.agno import CassandraPreHook

        hook = CassandraPreHook()

        assert hook.mode == CassandraMode.OPTIMIZE
    ...
    def test_init_with_custom_config(self):
        ...
        assert hook.mode == CassandraMode.SIMULATE
...
class TestCassandraPostHook:
    """Tests for CassandraPostHook."""

    def test_init_defaults(self):
        """Initialize with default settings."""
        from cassandra.integrations.agno import CassandraPostHook

        hook = CassandraPostHook()
    ...
    def test_call_triggers_alert(self):
        ...
        hook = CassandraPostHook(token_alert_threshold=50)
    ...
    def test_call_no_alert_below_threshold(self):
        ...
        hook = CassandraPostHook(token_alert_threshold=100)
...
class TestCreateCassandraHooks:
    """Tests for create_cassandra_hooks convenience function."""
    ...
    def test_returns_tuple(self):
        ...
        assert isinstance(pre_hook, CassandraPreHook)
        assert isinstance(post_hook, CassandraPostHook)
    ...
    def test_passes_config_to_pre_hook(self):
        ...
        config = CassandraConfig(default_mode=CassandraMode.AUDIT)
        pre_hook, _ = create_cassandra_hooks(config=config)
    ...
    def test_passes_mode_to_pre_hook(self):
        ...
        pre_hook, _ = create_cassandra_hooks(mode=CassandraMode.SIMULATE)

        assert pre_hook.mode == CassandraMode.SIMULATE

    def test_passes_model_to_pre_hook(self):
        """Passes model to pre_hook."""
        from cassandra.integrations.agno import create_cassandra_hooks

        pre_hook, _ = create_cassandra_hooks(model="claude-3-5-sonnet-20241022")
    ...
    def test_passes_log_level_to_post_hook(self):
        ...
        _, post_hook = create_cassandra_hooks(log_level="DEBUG")
    ...
    def test_passes_threshold_to_post_hook(self):
        ...
        _, post_hook = create_cassandra_hooks(token_alert_threshold=5000)
    ...
    def test_all_parameters(self):
        ...
        assert pre_hook.config is config
        assert pre_hook.mode == CassandraMode.AUDIT
