...
from cassandra.config import CacheAlignerConfig
from cassandra.transforms.cache_aligner import (
    CacheAligner,
...
def _capture_warnings() -> Iterator[list[str]]:
    ...
    target_logger = logging.getLogger("cassandra.transforms.cache_aligner")
...
def test_should_apply_false_when_policy_disables_aligner(tokenizer: Tokenizer) -> None:
    ...
    """
    from cassandra.transforms.compression_policy import CompressionPolicy
...
def test_no_regex_imported() -> None:
    ...
    import cassandra.transforms.cache_aligner as mod

    # ``re`` should not be in the module's globals.
    assert "re" not in mod.__dict__, (
        "cassandra.transforms.cache_aligner must not depend on the regex module"
    )
