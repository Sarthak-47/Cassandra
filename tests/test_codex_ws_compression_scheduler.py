...
* Adds a ``PERF`` log emission from ``handle_openai_responses_ws`` so
  Codex traffic is no longer invisible to ``cassandra perf``.
...
OPENAI_HANDLER = REPO_ROOT / "cassandra" / "proxy" / "handlers" / "openai.py"
...
def test_module_global_unit_semaphore_is_removed() -> None:
    ...
    assert "CASSANDRA_CODEX_WS_UNIT_WORKERS" not in source, (
        "The CASSANDRA_CODEX_WS_UNIT_WORKERS env knob was removed. It only "
...
#
# Codex WS traffic was invisible to ``cassandra perf`` pre-fix because
...
class _DirectLogCapture(logging.Handler):
    """Direct handler attached to ``cassandra.proxy`` so the proxy's
...
async def test_codex_ws_emits_perf_log_with_cache_keys() -> None:
    """``handle_openai_responses_ws`` must emit a PERF line so ``cassandra
    perf`` counts Codex traffic instead of reporting it as zero requests.

    Asserts on the structured-PERF kv fragment used by ``cassandra/perf/
