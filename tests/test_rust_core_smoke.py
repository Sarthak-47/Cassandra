...
Background — Finding #2 of CASSANDRA_PROXY_LOG_FINDINGS_2026_05_03.md.
A customer's production proxy was silently running without the
`cassandra._core` PyO3 extension because the Docker image never built it
...
1. The PyO3 module exposes a `hello()` marker function returning
   ``"cassandra-core"`` so the deployment smoke test has something stable
...
2. The proxy lifespan refuses to start when ``cassandra._core`` is
...
3. An explicit opt-out — ``CASSANDRA_REQUIRE_RUST_CORE=false`` — keeps
...
def test_rust_core_imports() -> None:
    """`cassandra._core.hello()` returns the documented sentinel.

    The deployment smoke test (`cassandra.proxy.server._check_rust_core`)
    ...
    """
    from cassandra._core import hello

    assert hello() == "cassandra-core"
...
def test_proxy_refuses_to_start_when_rust_core_missing(
    ...
    """When `cassandra._core` raises ImportError on import and the opt-out
    ...
    """
    from cassandra.proxy import server

    # Ensure the env var is absent so the default fail-loud path runs.
    monkeypatch.delenv("CASSANDRA_REQUIRE_RUST_CORE", raising=False)

    # Force the import to raise. Patching the symbol on the module is
    # not enough because `_check_rust_core` runs `from cassandra._core
    ...
    sys.modules.pop("cassandra._core", None)
    ...
    # sys.modules["cassandra._core"] to None forces a real ImportError.
    sys.modules["cassandra._core"] = None  # type: ignore[assignment]
...
# ──────────────────────────────────────────────────────────────────────────
# 3. Opt-out path: CASSANDRA_REQUIRE_RUST_CORE=false → degraded mode + /health.
# ──────────────────────────────────────────────────────────────────────────
def test_proxy_starts_in_degraded_mode_when_opt_out_set(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """With the opt-out env var set, the lifespan must start successfully
    even when `cassandra._core` is missing, and `/health` must surface
    ...
    monkeypatch.setenv("CASSANDRA_REQUIRE_RUST_CORE", "false")

    # Force the import to fail at lifespan time. Same trick as above:
    # sys.modules["cassandra._core"] = None makes Python treat the module
...
def test_check_rust_core_returns_loaded_when_extension_present(
    ...
    """When `cassandra._core` is loadable and `hello()` returns the
