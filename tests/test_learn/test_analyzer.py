...
from cassandra.learn.analyzer import (
    SessionAnalyzer,
    _build_digest,
    _call_cli_llm,
    _call_llm,
...
from cassandra.learn.models import (
    AnalysisResult,
    ErrorCategory,
    ProjectInfo,
...
_MARKER_BLOCK = (
    "<!-- cassandra:learn:start -->\n"
    "## Cassandra Learned Patterns\n"
    "*Auto-generated by `cassandra learn` on 2026-04-01 — do not edit manually*\n"
    "\n"
    "### Large Files\n"
...
class TestPriorPatternsInjection:
    ...
    def test_digest_omits_section_when_no_files_exist(self, tmp_path):
        ...
        assert "<!-- cassandra:learn" not in digest

    def test_digest_omits_section_when_file_has_no_marker_block(self, tmp_path):
        """CLAUDE.md exists but has no cassandra block → no prior section emitted."""
        project = _project_with_files(
            tmp_path,
            claude_md_text="# Project\n\nJust a regular readme, no cassandra block.\n",
            memory_md_text=None,
        )
        digest = _build_digest(project, [])
        assert "Prior Learned Patterns" not in digest
    ...
    @patch("cassandra.learn.analyzer._call_llm")
...
class TestDetectDefaultModel:
    ...
    def test_no_keys_no_cli_raises(self, monkeypatch):
        ...
        monkeypatch.delenv("OPENAI_API_KEY", raising=False)
        monkeypatch.delenv("GEMINI_API_KEY", raising=False)
        monkeypatch.setattr("cassandra.learn.analyzer.shutil.which", lambda _name: None)
    ...
    def test_cli_fallback_claude(self, monkeypatch):
        ...
        monkeypatch.delenv("GEMINI_API_KEY", raising=False)
        monkeypatch.setattr(
            "cassandra.learn.analyzer.shutil.which",
    ...
    def test_env_var_selects_gemini(self, monkeypatch):
        ...
        monkeypatch.delenv("OPENAI_API_KEY", raising=False)
        monkeypatch.delenv("GEMINI_API_KEY", raising=False)
        monkeypatch.setenv("CASSANDRA_LEARN_CLI", "gemini")
        assert _detect_default_model() == "gemini-cli"
    ...
    def test_env_var_selects_codex(self, monkeypatch):
        ...
        monkeypatch.delenv("OPENAI_API_KEY", raising=False)
        monkeypatch.delenv("GEMINI_API_KEY", raising=False)
        monkeypatch.setenv("CASSANDRA_LEARN_CLI", "codex")
        assert _detect_default_model() == "codex-cli"
    ...
    def test_env_var_invalid_raises(self, monkeypatch):
        ...
        monkeypatch.delenv("OPENAI_API_KEY", raising=False)
        monkeypatch.delenv("GEMINI_API_KEY", raising=False)
        monkeypatch.setenv("CASSANDRA_LEARN_CLI", "unknown-tool")
...
class TestCallCliLlm:
    def test_claude_cli_streams_and_parses_result_event(self):
        ...
        with patch(
            "cassandra.learn.analyzer.subprocess.Popen", _fake_claude_popen(stdout_lines=stdout)
        ) as popen:
    ...
    def test_claude_cli_idle_timeout_kills_hang(self, monkeypatch):
        ...
        monkeypatch.setenv("CASSANDRA_LEARN_CLI_IDLE_TIMEOUT_SECS", "1")
        ...
        with patch("cassandra.learn.analyzer.subprocess.Popen", popen):
            with pytest.raises(RuntimeError, match="produced no output"):
                _call_cli_llm("test digest", "claude-cli")

    def test_claude_cli_hard_cap_kills_continuous_chatter(self, monkeypatch):
        monkeypatch.setenv("CASSANDRA_LEARN_CLI_TIMEOUT_SECS", "1")
        monkeypatch.setenv("CASSANDRA_LEARN_CLI_IDLE_TIMEOUT_SECS", "10")
    ...
    @patch("cassandra.learn.analyzer.subprocess.run")
    ...
    def test_codex_timeout_honors_env_override(self, mock_run: MagicMock, monkeypatch):
        monkeypatch.setenv("CASSANDRA_LEARN_CLI_TIMEOUT_SECS", "42")
...
class TestParseStreamEvent:
    def test_returns_none_for_empty_line(self):
        from cassandra.learn.analyzer import _parse_stream_event
...
class TestClaudeCliEdgeCases:
    ...
    def test_stderr_on_success_is_logged_not_raised(self, caplog):
        ...
        with caplog.at_level(logging.DEBUG, logger="cassandra.learn.analyzer"):
            with patch("cassandra.learn.analyzer.subprocess.Popen", popen):
                result = _call_cli_llm("test digest", "claude-cli")
        assert result == {"context_file_rules": [], "memory_file_rules": []}
    ...
    def test_resolve_timeout_logs_warning_for_invalid(self, caplog, monkeypatch):
        ...
        monkeypatch.setenv("CASSANDRA_LEARN_CLI_TIMEOUT_SECS", "abc")
        with caplog.at_level(logging.WARNING, logger="cassandra.learn.analyzer"):
            assert _resolve_timeout_secs("CASSANDRA_LEARN_CLI_TIMEOUT_SECS", 300) == 300
        assert any(
            "Invalid CASSANDRA_LEARN_CLI_TIMEOUT_SECS" in rec.message for rec in caplog.records
        )

    def test_resolve_timeout_logs_warning_for_non_positive(self, caplog, monkeypatch):
        import logging

        monkeypatch.setenv("CASSANDRA_LEARN_CLI_TIMEOUT_SECS", "-5")
        with caplog.at_level(logging.WARNING, logger="cassandra.learn.analyzer"):
            assert _resolve_timeout_secs("CASSANDRA_LEARN_CLI_TIMEOUT_SECS", 300) == 300
...
class TestResolveTimeoutSecs:
    def test_uses_default_when_unset(self, monkeypatch):
        monkeypatch.delenv("CASSANDRA_LEARN_CLI_TIMEOUT_SECS", raising=False)
        assert _resolve_timeout_secs("CASSANDRA_LEARN_CLI_TIMEOUT_SECS", 300) == 300

    def test_uses_default_when_empty(self, monkeypatch):
        monkeypatch.setenv("CASSANDRA_LEARN_CLI_TIMEOUT_SECS", "")
        assert _resolve_timeout_secs("CASSANDRA_LEARN_CLI_TIMEOUT_SECS", 300) == 300

    def test_uses_default_for_non_integer(self, monkeypatch):
        monkeypatch.setenv("CASSANDRA_LEARN_CLI_TIMEOUT_SECS", "not-a-number")
        assert _resolve_timeout_secs("CASSANDRA_LEARN_CLI_TIMEOUT_SECS", 300) == 300

    def test_uses_default_for_non_positive(self, monkeypatch):
        monkeypatch.setenv("CASSANDRA_LEARN_CLI_TIMEOUT_SECS", "0")
        assert _resolve_timeout_secs("CASSANDRA_LEARN_CLI_TIMEOUT_SECS", 300) == 300

    def test_returns_overridden_value(self, monkeypatch):
        monkeypatch.setenv("CASSANDRA_LEARN_CLI_TIMEOUT_SECS", "777")
        assert _resolve_timeout_secs("CASSANDRA_LEARN_CLI_TIMEOUT_SECS", 300) == 777


class TestCallLlmRouting:
    @patch("cassandra.learn.analyzer._call_cli_llm")
...
class TestFailureAnalyzerCompat:
    ...
    def test_legacy_alias_works(self, mock_call_llm: MagicMock):
        from cassandra.learn.analyzer import FailureAnalyzer
