...
from cassandra.learn.scanner import ClaudeCodeScanner, _decode_project_path, _greedy_path_decode
...
class TestGreedyPathDecode:
    ...
    def test_simple_directory(self, tmp_path: Path) -> None:
        _make_dirs(tmp_path, "cassandra")
        result = _greedy_path_decode(tmp_path, ["cassandra"])
        assert result == tmp_path / "cassandra"
    ...
    def test_multi_hyphen_dot_dir_containing_subproject(self, tmp_path: Path) -> None:
        """Path like my-cool-project.nosync/cassandra — hardest combination."""
        _make_dirs(tmp_path, "my-cool-project.nosync/cassandra")
        result = _greedy_path_decode(tmp_path, ["my", "cool", "project.nosync", "cassandra"])
        assert result == tmp_path / "my-cool-project.nosync" / "cassandra"
...
class TestDecodeProjectPath:
    ...
    def test_simple_replace_finds_dot_path(self, users_tmp: Path) -> None:
        ...
        """
        project = users_tmp / "GitHub.nosync" / "cassandra"
    ...
    def users_tmp(self, tmp_path: Path) -> Generator[Path, None, None]:
        ...
        if str(home).startswith("/Users/"):
            base = home / ".pytest_cassandra_tmp"
            try:
    ...
    def test_home_dir_username_stays_single_component(self) -> None:
        ...
        ``/Users/first/last`` — causing ``cassandra learn --apply`` to fail with
        ...
        base = home / f"pytest_cassandra_{uuid4().hex}"
        try:
            base.mkdir()
