...
from cassandra.learn.models import ErrorCategory
from cassandra.learn.plugins.opencode import OpenCodePlugin
from cassandra.learn.registry import get_registry, reset_registry
from cassandra.learn.writer import CodexWriter
...
def _create_opencode_db(db_path: Path, project_path: Path) -> None:
    ...
    try:
        ...
        conn.execute(
            ...
            ("project-1", "Cassandra", str(project_path)),
        )
        conn.execute(
...
def test_opencode_plugin_discovers_projects_and_scans_tool_failures(tmp_path: Path) -> None:
    ...
    assert projects[0].name == "Cassandra"
