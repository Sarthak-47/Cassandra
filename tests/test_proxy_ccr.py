...
class TestCCRRetrieveEndpoint:
    ...
    def test_retrieve_expired_hash_reports_expiration_detail(self, client):
        ...
        with patch("cassandra.cache.compression_store.time.time", return_value=1000.0):
            hash_key = store.store(original="payload", compressed="payload")

        with patch("cassandra.cache.compression_store.time.time", return_value=1002.0):
...
class TestCCRStatsEndpoint:
    ...
    def test_stats_exposes_env_configured_ttl(self, client, monkeypatch):
        ...
        reset_compression_store()
        monkeypatch.setenv("CASSANDRA_CCR_TTL_SECONDS", "7200")
...
class TestEndToEndTOINIntegration:
    ...
    def test_pipeline_compresses_tool_output_and_records_toin(
        ...
        """
        from cassandra.config import CCRConfig, SmartCrusherConfig
        from cassandra.providers import AnthropicProvider
        from cassandra.telemetry import ToolSignature
        from cassandra.transforms import SmartCrusher, TransformPipeline
    ...
    def test_retrieval_through_proxy_updates_toin_field_semantics(
        ...
        from cassandra.telemetry.toin import _make_pattern_key
