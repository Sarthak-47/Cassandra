...
from cassandra.proxy.output_savings import SavingsRecorder
from cassandra.relevance.embedding import (
...
def _reset_stateless_globals():
    ...
    try:
        from cassandra.telemetry.toin import reset_toin
...
def test_process_stateless_env(monkeypatch, value, expected):
    paths.set_process_stateless(False)
    monkeypatch.setenv("CASSANDRA_STATELESS", value)
...
def test_memory_disabled_under_stateless(tmp_path, monkeypatch):
    ...
    assert not (tmp_path / ".cassandra" / "memory.db").exists()
...
def test_fastembed_default_model_is_pinned_to_sha(monkeypatch):
    monkeypatch.delenv("CASSANDRA_HF_PIN", raising=False)
...
def test_fastembed_pin_can_be_disabled(monkeypatch):
    monkeypatch.setenv("CASSANDRA_HF_PIN", "off")
