...
from cassandra.memory.traffic_learner import (
    ExtractedPattern,
    PatternCategory,
    TrafficLearner,
...
class TestCommandsRelatedAsRetry:
    ...
    def test_same_binary_unrelated_args_rejected(self):
        ...
        failed = (
            'grep -nE "smoke|CASSANDRA_SMOKE_TEST_TIMEOUT|smoke_test" '
...
class TestCommandsRelatedAsRetrySubstantiveToken:
    def test_substantive_token_beats_distance(self):
        ...
        # share the substantive token "cassandra-config", so the token-overlap
        # path accepts the pair.
        failed = "python -m foo --cassandra-config=/etc/h.toml"
        success = "python -m bar --cassandra-config=/etc/h.toml --extra"
...
class TestProjectForPattern:
    def _project(self, path: str):
        ...
        from cassandra.learn.models import ProjectInfo
...
class TestPatternsToRecommendations:
    def test_routes_preference_to_memory_file(self):
        from cassandra.learn.models import RecommendationTarget
...
class TestFlushDebounce:
    ...
    async def test_flush_worker_rate_limits(self, monkeypatch):
        ...
        from cassandra.memory import traffic_learner as tl_mod
...
def _install_plugin_registry(monkeypatch, plugin):
    """Stub out cassandra.learn.registry so flush_to_file uses our fake."""
    import sys
    import types as _types

    fake = _types.ModuleType("cassandra.learn.registry")
    ...
    monkeypatch.setitem(sys.modules, "cassandra.learn.registry", fake)
    import cassandra.learn as learn_pkg
...
class TestBackendResolution:
    def test_resolve_none_backend(self):
        from cassandra.memory.traffic_learner import _resolve_backend_db_path
