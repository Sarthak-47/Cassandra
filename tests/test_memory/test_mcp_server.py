...
mcp_server_mod = import_module_with_mcp_stub("cassandra.memory.mcp_server")
...
def test_memory_mcp_startup_context_reports_dynamic_project_db(tmp_path) -> None:
    ...
    project_dir.mkdir()
    configured_db = str(project_dir / ".cassandra" / "memory.db")
...
def test_main_logs_memory_mcp_startup_context(monkeypatch, tmp_path, caplog) -> None:
    ...
    caplog.set_level("INFO", logger="cassandra.memory.mcp")

    mcp_server_mod.main()

    assert captured_run_payloads == [
        ("run", str(project_dir / ".cassandra" / "memory.db"), "codex-user")
    ]
    assert any(
