...
from cassandra.memory.writers.base import (
    MARKER_END,
    MARKER_START,
    MemoryEntry,
    _estimate_tokens,
...
from cassandra.memory.writers.claude_writer import ClaudeCodeMemoryWriter
from cassandra.memory.writers.codex_writer import CodexMemoryWriter
from cassandra.memory.writers.cursor_writer import CursorMemoryWriter
from cassandra.memory.writers.generic_writer import GenericMemoryWriter
...
class TestClaudeCodeWriter:
    def test_format_memories(self):
        ...
        assert "## Cassandra Learned Context" in formatted
        assert "Auto-maintained by Cassandra" in formatted
    ...
    def test_export_topics(self, tmp_path: Path):
        ...
        for filename, content in topics.items():
            assert filename.startswith("cassandra_")
...
class TestCursorWriter:
    ...
    def test_creates_mdc_with_frontmatter(self, tmp_path: Path):
        ...
        mdc_path = tmp_path / ".cursor" / "rules" / "cassandra-memory.mdc"
    ...
    def test_updates_existing_mdc(self, tmp_path: Path):
        ...
        mdc_file = mdc_dir / "cassandra-memory.mdc"
        mdc_file.write_text(
            "---\ndescription: test\nalwaysApply: true\n---\n\n"
            f"# Header\n\n{MARKER_START}\nold stuff\n{MARKER_END}\n"
        )
...
class TestGenericWriter:
    ...
    def test_export(self, tmp_path: Path):
        ...
        assert (tmp_path / "CASSANDRA_MEMORY.md").exists()
        assert result.memories_exported == 3
