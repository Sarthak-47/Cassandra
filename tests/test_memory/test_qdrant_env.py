...
from cassandra.memory import qdrant_env
from cassandra.memory.qdrant_env import (
...
# All CASSANDRA_QDRANT_* vars are cleared before every test so that the host
# environment (e.g. a developer's local ``direnv``) cannot leak into unit tests.
_QDRANT_ENV_VARS = (
    "CASSANDRA_QDRANT_URL",
    "CASSANDRA_QDRANT_HOST",
    "CASSANDRA_QDRANT_PORT",
    "CASSANDRA_QDRANT_API_KEY",
    "CASSANDRA_QDRANT_HTTPS",
    "CASSANDRA_QDRANT_PREFER_GRPC",
    "CASSANDRA_QDRANT_GRPC_PORT",
)
...
class TestEnvReaders:
    ...
    def test_host_reads_env(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("CASSANDRA_QDRANT_HOST", "qdrant.internal")
        assert qdrant_env_host() == "qdrant.internal"

    def test_host_trims_whitespace(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("CASSANDRA_QDRANT_HOST", "  qdrant.example  ")
    ...
    def test_host_empty_string_falls_back_to_default(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("CASSANDRA_QDRANT_HOST", "")
        assert qdrant_env_host() == DEFAULT_QDRANT_HOST
    ...
    def test_url_reads_env(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("CASSANDRA_QDRANT_URL", "https://qdrant.cloud:6333")
    ...
    def test_api_key_reads_env(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("CASSANDRA_QDRANT_API_KEY", "secret-token")
    ...
    def test_https_truthy_values(self, monkeypatch: pytest.MonkeyPatch, truthy: str) -> None:
        monkeypatch.setenv("CASSANDRA_QDRANT_HTTPS", truthy)
    ...
    def test_prefer_grpc_reads_env(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("CASSANDRA_QDRANT_PREFER_GRPC", "true")
    ...
    def test_grpc_port_reads_env(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("CASSANDRA_QDRANT_GRPC_PORT", "9999")
...
def test_direct_mem0_config_defaults_read_env(
    ...
    from cassandra.memory.backends.direct_mem0 import Mem0Config

    monkeypatch.setenv("CASSANDRA_QDRANT_HOST", "custom.example")
    monkeypatch.setenv("CASSANDRA_QDRANT_PORT", "7777")
    monkeypatch.setenv("CASSANDRA_QDRANT_URL", "https://cloud.example:6333")
    monkeypatch.setenv("CASSANDRA_QDRANT_API_KEY", "my-api-key")
...
def test_direct_mem0_config_explicit_wins_over_env(
    ...
    monkeypatch.setenv("CASSANDRA_QDRANT_HOST", "env-host")
    monkeypatch.setenv("CASSANDRA_QDRANT_PORT", "7777")
...
def test_mem0_config_defaults_read_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from cassandra.memory.backends.mem0 import Mem0Config

    monkeypatch.setenv("CASSANDRA_QDRANT_URL", "https://tenant.qdrant.io")
    monkeypatch.setenv("CASSANDRA_QDRANT_API_KEY", "k")
...
def test_proxy_memory_config_defaults_read_env(
    ...
    from cassandra.proxy.memory_handler import MemoryConfig

    monkeypatch.setenv("CASSANDRA_QDRANT_HOST", "proxy.qdrant")
    monkeypatch.setenv("CASSANDRA_QDRANT_PORT", "8888")
...
def test_proxy_config_defaults_read_env(
    ...
    from cassandra.proxy.models import ProxyConfig

    monkeypatch.setenv("CASSANDRA_QDRANT_URL", "https://shared.qdrant")
    cfg = ProxyConfig()
...
def test_memory_easy_picks_up_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from cassandra.memory.easy import Memory

    monkeypatch.setenv("CASSANDRA_QDRANT_URL", "https://hosted.qdrant.io:6333")
    monkeypatch.setenv("CASSANDRA_QDRANT_API_KEY", "xoxp-1234")
...
def test_memory_easy_explicit_args_override_env(
    ...
    monkeypatch.setenv("CASSANDRA_QDRANT_HOST", "env-host")
    monkeypatch.setenv("CASSANDRA_QDRANT_PORT", "1234")
