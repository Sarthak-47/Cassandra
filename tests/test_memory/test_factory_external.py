...
setuptools entry points:
  - cassandra.memory_store
  - cassandra.memory_vector
  - cassandra.memory_text
...
from cassandra.memory.factory import (
    _create_store,
    _create_text_index,
...
def _patch_entry_points(monkeypatch, expected_group: str, name: str, target):
    """Patch cassandra.memory.factory.entry_points to return our fake EP."""
    from cassandra.memory import factory as factory_mod
...
class TestExternalStoreBackend:
    """EXTERNAL store backend loads via entry_points(group='cassandra.memory_store')."""
    ...
    def test_loads_external_store(self, monkeypatch):
        ...
        _patch_entry_points(monkeypatch, "cassandra.memory_store", "myvec", make_store)
...
class TestExternalVectorBackend:
    """EXTERNAL vector backend loads via entry_points(group='cassandra.memory_vector')."""

    def test_loads_external_vector(self, monkeypatch):
        sentinel = object()
        _patch_entry_points(monkeypatch, "cassandra.memory_vector", "myvec", lambda cfg: sentinel)
...
class TestExternalTextBackend:
    """EXTERNAL text backend loads via entry_points(group='cassandra.memory_text')."""

    def test_loads_external_text(self, monkeypatch):
        sentinel = object()
        _patch_entry_points(monkeypatch, "cassandra.memory_text", "mytext", lambda cfg: sentinel)
