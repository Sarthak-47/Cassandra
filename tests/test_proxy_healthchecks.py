...
def test_livez_reports_process_health(client):
    ...
    data = response.json()
    assert data["service"] == "cassandra-proxy"
...
def test_health_includes_deployment_metadata_when_present(monkeypatch):
    monkeypatch.setenv("CASSANDRA_SKIP_UPSTREAM_CHECK", "1")
    monkeypatch.setenv("CASSANDRA_DEPLOYMENT_PROFILE", "default")
    monkeypatch.setenv("CASSANDRA_DEPLOYMENT_PRESET", "persistent-service")
    monkeypatch.setenv("CASSANDRA_DEPLOYMENT_RUNTIME", "python")
    monkeypatch.setenv("CASSANDRA_DEPLOYMENT_SUPERVISOR", "service")
    monkeypatch.setenv("CASSANDRA_DEPLOYMENT_SCOPE", "user")
...
def test_readyz_reports_memory_backend_when_enabled(tmp_path, monkeypatch):
    ...
    config = ProxyConfig(
        ...
        memory_db_path=str(tmp_path / "cassandra_memory.db"),
...
def test_readyz_initializes_qdrant_memory_backend(monkeypatch):
    monkeypatch.setenv("CASSANDRA_SKIP_UPSTREAM_CHECK", "1")
    from cassandra.memory.backends import direct_mem0
...
def test_readyz_upstream_check_disabled_by_env_var(monkeypatch):
    """CASSANDRA_SKIP_UPSTREAM_CHECK=1 suppresses the probe and reports ready."""
    monkeypatch.setenv("CASSANDRA_SKIP_UPSTREAM_CHECK", "1")
    config = ProxyConfig(
        optimize=False,
        cache_enabled=False,
...
def test_readyz_upstream_check_failure_returns_503(monkeypatch):
    ...
    monkeypatch.delenv("CASSANDRA_SKIP_UPSTREAM_CHECK", raising=False)
