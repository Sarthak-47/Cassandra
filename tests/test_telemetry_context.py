"""Tests for cassandra.telemetry.context (install_mode + cassandra_stack detection)."""
...
from cassandra.telemetry.context import (
    MAX_DISTINCT_STACKS,
    detect_install_mode,
    detect_stack,
    normalize_stack,
...
def _clean_env(monkeypatch):
    ...
    monkeypatch.delenv("CASSANDRA_STACK", raising=False)
    monkeypatch.delenv("CASSANDRA_AGENT_TYPE", raising=False)
    yield


class TestDetectInstallMode:
    def test_wrapped_when_agent_type_set(self, monkeypatch):
        monkeypatch.setenv("CASSANDRA_AGENT_TYPE", "claude")
        assert detect_install_mode(8787) == "wrapped"

    def test_on_demand_when_no_env_and_no_manifest(self, monkeypatch):
        monkeypatch.setattr("cassandra.install.state.list_manifests", lambda: [])
    ...
    def test_persistent_when_manifest_matches_port(self, monkeypatch):
        ...
        monkeypatch.setattr("cassandra.install.state.list_manifests", lambda: [manifest])
    ...
    def test_wrapped_takes_precedence_over_manifest(self, monkeypatch):
        monkeypatch.setenv("CASSANDRA_AGENT_TYPE", "codex")
        manifest = SimpleNamespace(port=8787, profile="default")
        monkeypatch.setattr("cassandra.install.state.list_manifests", lambda: [manifest])
        assert detect_install_mode(8787) == "wrapped"
    ...
    def test_manifest_crash_falls_back_to_on_demand(self, monkeypatch):
        ...
        monkeypatch.setattr("cassandra.install.state.list_manifests", _boom)
...
class TestDetectStack:
    ...
    def test_unknown_agent_type_rejected(self, monkeypatch):
        monkeypatch.setenv("CASSANDRA_AGENT_TYPE", "somebespoke")
    ...
    def test_env_beats_stats(self, monkeypatch):
        monkeypatch.setenv("CASSANDRA_STACK", "wrap_claude")
    ...
    def test_invalid_env_falls_through_to_proxy(self, monkeypatch):
        ...
        monkeypatch.setenv("CASSANDRA_STACK", "bad slug with spaces!")
        assert detect_stack() == "proxy"

    def test_invalid_env_allows_agent_type_fallback(self, monkeypatch):
        monkeypatch.setenv("CASSANDRA_STACK", "Has-Dashes-And-Caps")
        monkeypatch.setenv("CASSANDRA_AGENT_TYPE", "claude")
        assert detect_stack() == "wrap_claude"
