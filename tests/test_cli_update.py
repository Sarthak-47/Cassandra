"""Tests for the `cassandra update` command + install-method detection."""
...
def test_detect_pipx_by_path(monkeypatch):
    monkeypatch.setattr(up.sys, "prefix", "/home/u/.local/pipx/venvs/cassandra-ai")
    m = up.detect_install_method()
    assert m.kind == "pipx" and m.argv == ["pipx", "upgrade", "cassandra-ai"]


def test_detect_pipx_windows_path(monkeypatch):
    monkeypatch.setattr(up.sys, "prefix", r"C:\\Users\\u\\pipx\\venvs\\cassandra-ai")
    m = up.detect_install_method()
    assert m.kind == "pipx"


def test_detect_uv_tool(monkeypatch):
    monkeypatch.setattr(up.sys, "prefix", "/home/u/.local/share/uv/tools/cassandra-ai")
    m = up.detect_install_method()
    assert m.kind == "uv-tool" and m.argv == ["uv", "tool", "upgrade", "cassandra-ai"]
