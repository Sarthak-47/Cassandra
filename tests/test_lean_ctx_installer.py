...
from cassandra.lean_ctx import get_lean_ctx_path, installer


def test_get_lean_ctx_path_finds_windows_managed_binary(tmp_path: Path) -> None:
    managed_dir = tmp_path / ".cassandra" / "bin"
    managed_dir.mkdir(parents=True)
    managed_path = managed_dir / "lean-ctx.exe"
    managed_path.write_bytes(b"binary")

    with patch("cassandra.lean_ctx.LEAN_CTX_BIN_DIR", managed_dir):
        with patch("cassandra.lean_ctx.LEAN_CTX_BIN_PATH", managed_dir / "lean-ctx"):
            with patch("cassandra.lean_ctx.shutil.which", return_value=None):
                assert get_lean_ctx_path() == managed_path


def test_get_target_triple_uses_override(monkeypatch) -> None:
    monkeypatch.setenv("CASSANDRA_LEAN_CTX_TARGET", "x86_64-pc-windows-msvc")
...
def test_get_download_url_uses_windows_zip(monkeypatch) -> None:
    monkeypatch.delenv("CASSANDRA_LEAN_CTX_TARGET", raising=False)
...
def test_download_lean_ctx_skips_verify_for_non_native_target(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv("CASSANDRA_LEAN_CTX_TARGET", "x86_64-apple-darwin")
...
def test_ensure_lean_ctx_returns_none_when_download_fails() -> None:
    with patch("cassandra.lean_ctx.get_lean_ctx_path", return_value=None):
        with patch.object(
