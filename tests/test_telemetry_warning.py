...
from cassandra.telemetry.beacon import (  # noqa: E402
...
class TestIsTelemetryWarnEnabled:
    """Tests for the CASSANDRA_TELEMETRY_WARN feature flag."""

    def test_enabled_by_default(self, monkeypatch):
        monkeypatch.delenv("CASSANDRA_TELEMETRY_WARN", raising=False)
    ...
    def test_disabled_by_env_var(self, monkeypatch, value):
        monkeypatch.setenv("CASSANDRA_TELEMETRY_WARN", value)
...
class TestProxyCLITelemetryBanner:
    ...
    def test_banner_shows_telemetry_enabled(self, runner, monkeypatch):
        ...
        with patch("cassandra.proxy.server.run_server", side_effect=SystemExit(0)):
    ...
    def test_banner_disabled_by_default(self, runner, monkeypatch):
        ...
        monkeypatch.delenv("CASSANDRA_TELEMETRY", raising=False)
        ...
        assert "CASSANDRA_TELEMETRY=on" in result.output or "--telemetry" in result.output
    ...
    def test_banner_shows_opt_out_instructions_when_enabled(self, runner, monkeypatch):
        ...
        assert "CASSANDRA_TELEMETRY=off" in result.output or "--no-telemetry" in result.output
...
class TestWrapCLITelemetryNotice:
    ...
    def test_print_notice_outputs_when_telemetry_on(self, monkeypatch, capsys):
        ...
        from cassandra.cli.wrap import _print_telemetry_notice

        _print_telemetry_notice()
        captured = capsys.readouterr()
        assert "Telemetry" in captured.out
        assert "CASSANDRA_TELEMETRY=off" in captured.out
...
class TestTelemetryModuleExports:
    """New helpers must be exported from cassandra.telemetry."""

    def test_is_telemetry_warn_enabled_exported(self):
        from cassandra.telemetry import is_telemetry_warn_enabled as fn

        assert callable(fn)

    def test_is_telemetry_enabled_exported(self):
        from cassandra.telemetry import is_telemetry_enabled as fn

        assert callable(fn)

    def test_format_telemetry_notice_exported(self):
        from cassandra.telemetry import format_telemetry_notice as fn
