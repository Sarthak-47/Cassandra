...
def test_no_openssl_sys_in_wheel_build_tree() -> None:
    ...
    those rustls features enabled in cassandra-core, our entire build
    ...
    for crate in ("cassandra-py", "cassandra-proxy", "cassandra-core"):
        try:
...
def test_fastembed_uses_rustls_features() -> None:
    ...
    fastembed's explicit rustls feature selection in cassandra-core.
    ...
    """
    cargo = (ROOT / "crates" / "cassandra-core" / "Cargo.toml").read_text(encoding="utf-8")
...
def test_glibc_compat_shim_present_in_cassandra_py() -> None:
    """STRUCTURAL INVARIANT: the cassandra-py crate ships a glibc-2.38
    compatibility shim that defines weak `__isoc23_*` aliases.

    Issue #355 (https://github.com/Sarthak-47/cassandra/issues/355) —
    ...
    The fix is `crates/cassandra-py/glibc_compat.c` which provides
    ...
    cc dep).
    """
    cassandra_py_dir = ROOT / "crates" / "cassandra-py"

    shim = cassandra_py_dir / "glibc_compat.c"
    assert shim.exists(), (
        "crates/cassandra-py/glibc_compat.c is missing — without it, "
    ...
    build_rs = cassandra_py_dir / "build.rs"
    assert build_rs.exists(), "crates/cassandra-py/build.rs is missing"
    ...
    cargo_toml = (cassandra_py_dir / "Cargo.toml").read_text(encoding="utf-8")
    assert 'build = "build.rs"' in cargo_toml, (
        'cassandra-py/Cargo.toml must declare `build = "build.rs"` — '
    ...
    assert "[build-dependencies]" in cargo_toml and 'cc = "1"' in cargo_toml, (
        'cassandra-py/Cargo.toml must declare `cc = "1"` in '
...
def test_release_workflow_audits_wheel_glibc_symbols() -> None:
    ...
    Companion to `test_glibc_compat_shim_present_in_cassandra_py` —
...
def test_release_workflow_has_smoke_import_wheel_gate() -> None:
    ...
    through `import cassandra._core` on a matrix of representative
    ...
    # The actual import command must hit `from cassandra._core import hello`
    # — this is the same call the proxy's `_check_rust_core` makes on
    # startup (per `cassandra/proxy/server.py` and the issue #355 backtrace).
    # Anything else (e.g. just `import cassandra`) fails to exercise the
    # Rust _core.so binary.
    assert "from cassandra._core import hello" in content, (
        "smoke-import command must call `from cassandra._core import hello` "
        "— that's what the proxy does at startup. A weaker check (e.g. "
        "`import cassandra`) wouldn't exercise the .so and wouldn't catch "
...
def test_release_workflow_runs_dry_run_on_pull_request() -> None:
    ...
    2. Path filter is narrow enough to skip source-only PRs to
       `crates/cassandra-core` / `crates/cassandra-proxy` (where wheel
       layout doesn't change), but wide enough to cover release.yml,
       docker.yml, cassandra-py crate, pyproject.toml, root Cargo.
...
def test_release_please_config_and_manifest_are_present_and_consistent() -> None:
    ...
    # Matches the same fallback pattern in cassandra/release_version.py.
    try:
        import tomllib
    ...
    assert root_pkg["package-name"] == "cassandra-ai"

    # Tag format: existing tags in this repo are `vX.Y.Z`, NOT
    # `cassandra-ai-vX.Y.Z`. release-please's default for manifest
    # configs prepends the component name; that would produce
    # `cassandra-ai-v0.22.4` and the bot would never find the existing
    ...
    assert config.get("include-component-in-tag") is False, (
        ...
        "`vX.Y.Z`, not `cassandra-ai-vX.Y.Z`. Reverting this setting "
