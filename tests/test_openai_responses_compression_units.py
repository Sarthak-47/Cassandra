...
from cassandra.proxy.handlers import openai as openai_handler
from cassandra.proxy.handlers.openai import OpenAIHandlerMixin
from cassandra.transforms.compression_units import UnitCompressionResult
...
def test_openai_responses_unit_parallelism_env_defaults_and_clamps(monkeypatch):
    monkeypatch.delenv("CASSANDRA_TOOL_OUTPUT_COMPRESSION_PARALLELISM", raising=False)
    assert openai_handler._openai_responses_unit_parallelism() == 4

    monkeypatch.setenv("CASSANDRA_TOOL_OUTPUT_COMPRESSION_PARALLELISM", "bad")
    assert openai_handler._openai_responses_unit_parallelism() == 4

    monkeypatch.setenv("CASSANDRA_TOOL_OUTPUT_COMPRESSION_PARALLELISM", "0")
    assert openai_handler._openai_responses_unit_parallelism() == 1

    monkeypatch.setenv("CASSANDRA_TOOL_OUTPUT_COMPRESSION_PARALLELISM", "999")
...
def test_openai_responses_adapter_parallelizes_cache_misses_preserving_order(monkeypatch):
    monkeypatch.setenv("CASSANDRA_TOOL_OUTPUT_COMPRESSION_PARALLELISM", "4")
    router = ContentRouter()
    lock = threading.Lock()
...
def test_openai_responses_adapter_preserves_cassandra_retrieve_outputs():
    router = ContentRouter()
        ...
        "input": [
            {
                ...
                "name": "mcp__cassandra__cassandra_retrieve",
                "arguments": "{}",
            },
            {
...
def test_openai_responses_adapter_preserves_excluded_tool_outputs():
    """Regression for #940: outputs for CASSANDRA_EXCLUDE_TOOLS tools stay raw.
...
def test_openai_responses_payload_routes_through_content_router_without_rust(
    ...
    import cassandra._core as core
