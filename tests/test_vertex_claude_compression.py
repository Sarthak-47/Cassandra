...
Covers the fixes that let `cassandra wrap claude` + Vertex actually deliver
compression:
...
from cassandra.providers import proxy_routes, registry
...
def test_vertex_rawpredict_anthropic_runs_compression_handler(monkeypatch) -> None:
    ...
    monkeypatch.setattr(CassandraProxy, "handle_anthropic_messages", fake)
