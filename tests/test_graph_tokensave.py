...
from cassandra.graph import tokensave_installer as ts
...
def test_download_tokensave_tarball(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv("CASSANDRA_TOKENSAVE_ALLOW_UNVERIFIED", "1")
...
def test_verify_asset_digest_refuses_unpinned_without_optout(monkeypatch) -> None:
    ...
    monkeypatch.delenv("CASSANDRA_TOKENSAVE_ALLOW_UNVERIFIED", raising=False)
...
def test_pinned_version_env_override(monkeypatch) -> None:
    monkeypatch.setenv("CASSANDRA_TOKENSAVE_VERSION", "v9.9.9")
    assert ts._pinned_version() == "v9.9.9"
    monkeypatch.delenv("CASSANDRA_TOKENSAVE_VERSION", raising=False)
