...
the keyword tables exposed by the Rust `cassandra._core.signals` module.
These tests pin:
...
def test_legacy_re_exports_present():
    from cassandra.transforms import error_detection as ed
...
def test_bug_fix_error_regex_now_matches_canonical_keyword_set():
    ...
    from cassandra.transforms.error_detection import ERROR_KEYWORDS, ERROR_PATTERN
...
def test_bug_fix_security_keywords_dropped_token():
    ...
    from cassandra.transforms.error_detection import SECURITY_KEYWORDS, SECURITY_PATTERN
...
def test_content_has_error_indicators_lax_substring_semantics():
    from cassandra.transforms.error_detection import content_has_error_indicators
...
def test_rust_signals_bridge_score_line_diff_context():
    ...
    it."""
    from cassandra.transforms.error_detection import score_line
...
def test_raw_rust_score_line_returns_none_on_unknown_context():
    """The raw `cassandra._core.score_line` returns `None` for unknown
    ...
    from cassandra._core import score_line as _raw
...
def test_keyword_registry_snapshot_has_dropped_token_and_added_indicators():
    from cassandra._core import keyword_registry_snapshot
...
def test_python_regex_recompiled_from_rust_keyword_tables():
    ...
    from cassandra._core import keyword_registry_snapshot
    from cassandra.transforms.error_detection import (
        ERROR_KEYWORDS,
        IMPORTANCE_KEYWORDS,
        SECURITY_KEYWORDS,
    )
