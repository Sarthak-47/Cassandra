...
from cassandra import update_check as uc
from cassandra.cli import update as up
...
def _env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CASSANDRA_UPDATE_CHECK", "on")
    monkeypatch.delenv("CASSANDRA_STATELESS", raising=False)
    monkeypatch.delenv("CI", raising=False)
    monkeypatch.delenv("CASSANDRA_IN_DOCKER", raising=False)
...
def test_in_docker_env_flag(monkeypatch):
    monkeypatch.setenv("CASSANDRA_IN_DOCKER", "1")
    assert up._in_docker() is True


def test_in_docker_default_false():
    # No CASSANDRA_IN_DOCKER and (almost certainly) no /.dockerenv on the runner.
...
def test_user_site_and_membership(monkeypatch):
    ...
    assert up._is_user_site_install("/home/u/.local/site/cassandra_ai") is True
...
def test_format_cmd_quotes_spaces(monkeypatch):
    ...
    out = up._format_cmd(["/path with space/python", "-m", "pip", "install", "-U", "cassandra-ai"])
...
def test_spec_with_and_without_extras():
    assert up._spec(None) == "cassandra-ai"
    assert up._spec("all") == "cassandra-ai[all]"
    assert up._spec("[proxy]") == "cassandra-ai[proxy]"
...
def test_update_missing_tool_surfaces_command(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda extras=None: up.InstallMethod(
            kind="pipx", can_self_update=True, argv=["pipx", "upgrade", "cassandra-ai"]
        ),
    )
...
def test_run_check_disabled_returns_none(monkeypatch):
    monkeypatch.setenv("CASSANDRA_UPDATE_CHECK", "off")
