...
async def test_tracker_start_stop_and_rollover_reset(
    ...
    tracker._state.contribution = CassandraContribution(tokens_submitted=99)
...
async def test_maybe_poll_handles_inactive_and_none_snapshot(
    ...
    monkeypatch.setattr(
        "cassandra.subscription.client.read_cached_oauth_token", lambda: "cached-token"
    )
...
async def test_maybe_poll_success_updates_state_and_metrics(
    ...
    monkeypatch.setitem(
        sys.modules,
        "cassandra.observability.metrics",
        SimpleNamespace(
