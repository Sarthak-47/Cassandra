...
- ``cassandra._core.SmartCrusher.with_compaction_format`` renders uniform
  arrays as Markdown-KV when the lossless gate passes.
- The high-level ``SmartCrusher`` exposes the knob via the
  ``compaction_format`` kwarg and the ``CASSANDRA_COMPACTION_FORMAT`` env
...
from cassandra._core import SmartCrusher as RustSmartCrusher
from cassandra._core import SmartCrusherConfig as RustSmartCrusherConfig
from cassandra.transforms.smart_crusher import SmartCrusher
from cassandra.transforms.smart_crusher import SmartCrusherConfig as PySmartCrusherConfig
...
def test_env_var_opts_in(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("CASSANDRA_COMPACTION_FORMAT", "markdown-kv")
    crusher = SmartCrusher()
    assert crusher._compaction_format == "markdown-kv"


def test_kwarg_overrides_env_var(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("CASSANDRA_COMPACTION_FORMAT", "json")
    crusher = SmartCrusher(compaction_format="markdown-kv")
    assert crusher._compaction_format == "markdown-kv"
