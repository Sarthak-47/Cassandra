...
from cassandra.mcp_registry.base import RegisterResult, RegisterStatus
from cassandra.mcp_registry.display import (
    any_succeeded,
    format_result,
    format_results,
)
...
def test_mismatch_emits_overwrite_hint() -> None:
    line = format_result(
        ...
        overwrite_hint="cassandra mcp install --force",
    )
    assert line is not None
    assert "differs" in line
    assert "cassandra mcp install --force" in line
...
def test_no_sdk_points_at_pip_extras() -> None:
    ...
    assert "cassandra-ai[mcp]" in line
