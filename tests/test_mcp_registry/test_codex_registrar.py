...
from cassandra.mcp_registry.base import RegisterStatus, ServerSpec
from cassandra.mcp_registry.codex import CodexRegistrar
...
_RESOLVED_COMMAND = ("/usr/bin/python", "-m", "cassandra.cli")
_RESOLVED_ARGS = ("-m", "cassandra.cli", "mcp", "serve")
...
def _install_spec(monkeypatch: pytest.MonkeyPatch) -> ServerSpec:
    monkeypatch.setattr(
        "cassandra.mcp_registry.install.resolve_cassandra_command",
        lambda: list(_RESOLVED_COMMAND),
    )
    return build_cassandra_spec()
...
def test_register_uses_codex_home_env(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert "[mcp_servers.cassandra]" in text
...
def test_get_server_returns_none_when_config_missing(tmp_path: Path) -> None:
    assert _make_registrar(tmp_path).get_server("cassandra") is None
...
def test_get_server_returns_spec_when_table_present(tmp_path: Path) -> None:
    ...
    got = _make_registrar(tmp_path).get_server("cassandra")
    assert got is not None
    assert got.command == _RESOLVED_COMMAND[0]
    assert got.args == _RESOLVED_ARGS
    assert got.env == {"CASSANDRA_PROXY_URL": "http://127.0.0.1:9000"}
...
def test_register_creates_config_when_missing(
    ...
    assert cfg.exists()
    text = cfg.read_text()
    assert "# --- Cassandra MCP server ---" in text
    assert "[mcp_servers.cassandra]" in text
    parsed = tomllib.loads(text)
    assert parsed["mcp_servers"]["cassandra"]["command"] == _RESOLVED_COMMAND[0]
    assert parsed["mcp_servers"]["cassandra"]["args"] == list(_RESOLVED_ARGS)
...
def test_register_includes_env_subtable(tmp_path: Path) -> None:
    spec = _spec(env={"CASSANDRA_PROXY_URL": "http://127.0.0.1:9000"})
    _make_registrar(tmp_path).register_server(spec)
    text = _config_path(tmp_path).read_text()
    assert "[mcp_servers.cassandra.env]" in text
    parsed = tomllib.loads(text)
    assert parsed["mcp_servers"]["cassandra"]["env"] == {
        "CASSANDRA_PROXY_URL": "http://127.0.0.1:9000"
    }


def test_register_cassandra_and_serena_coexist(tmp_path: Path) -> None:
    reg = _make_registrar(tmp_path)
    ...
    assert "# --- Cassandra MCP server ---" in text
    assert "# --- Cassandra MCP server: serena ---" in text
...
def test_register_omits_env_subtable_when_env_empty(tmp_path: Path) -> None:
    ...
    text = _config_path(tmp_path).read_text()
    assert "[mcp_servers.cassandra.env]" not in text
...
def test_register_mismatch_when_block_differs_no_force(tmp_path: Path) -> None:
    reg = _make_registrar(tmp_path)
    reg.register_server(_spec(env={"CASSANDRA_PROXY_URL": "http://127.0.0.1:9999"}))
    text_before = _config_path(tmp_path).read_text()
...
def test_register_force_preserves_user_managed_entry(tmp_path: Path) -> None:
    ...
    cfg.parent.mkdir()
    cfg.write_text(
        '[mcp_servers.cassandra]\ncommand = "/usr/local/bin/custom-cassandra"\nargs = ["serve"]\n'
    )
    ...
    assert "/usr/local/bin/custom-cassandra" in cfg.read_text()
    assert cfg.read_text().count("[mcp_servers.cassandra]") == 1


def test_register_mismatch_when_user_managed_outside_markers(tmp_path: Path) -> None:
    cfg = _config_path(tmp_path)
    cfg.parent.mkdir()
    # User has manually put [mcp_servers.cassandra] with different config — no markers.
    cfg.write_text(
        '[mcp_servers.cassandra]\ncommand = "/usr/local/bin/custom-cassandra"\nargs = ["serve"]\n'
    )
    result = _make_registrar(tmp_path).register_server(_spec())
...
def test_unregister_removes_marker_block(tmp_path: Path) -> None:
    ...
    assert "[mcp_servers.cassandra]" in cfg.read_text()

    assert reg.unregister_server("cassandra") is True
    text = cfg.read_text()
    assert "[mcp_servers.cassandra]" not in text
    assert "# --- Cassandra MCP server ---" not in text
    # Surrounding content survives.
    assert "[other_section]" in text


def test_unregister_serena_preserves_cassandra_block(tmp_path: Path) -> None:
    reg = _make_registrar(tmp_path)
    reg.register_server(_spec())
    ...
    assert "# --- Cassandra MCP server ---" in text
    assert "# --- Cassandra MCP server: serena ---" not in text
...
def test_unregister_returns_false_when_no_block(tmp_path: Path) -> None:
    ...
    cfg.parent.mkdir()
    cfg.write_text('model = "gpt-4o"\n')
    assert _make_registrar(tmp_path).unregister_server("cassandra") is False


def test_unregister_preserves_user_managed_entry(tmp_path: Path) -> None:
    """User-managed [mcp_servers.cassandra] without our markers stays put."""
    cfg = _config_path(tmp_path)
    cfg.parent.mkdir()
    cfg.write_text('[mcp_servers.cassandra]\ncommand = "/custom/cassandra"\n')
    # No markers => unregister is a no-op.
    assert _make_registrar(tmp_path).unregister_server("cassandra") is False
    assert "/custom/cassandra" in cfg.read_text()
...
@pytest.mark.parametrize(
    "spec",
    [
        ServerSpec(name="cassandra", command=_RESOLVED_COMMAND[0], args=_RESOLVED_ARGS),
        ServerSpec(
            name="cassandra",
            command=_RESOLVED_COMMAND[0],
            args=_RESOLVED_ARGS,
            env={"CASSANDRA_PROXY_URL": "http://127.0.0.1:9000"},
        ),
        ServerSpec(name="cassandra", command="/usr/bin/cassandra", args=()),
    ],
)
def test_round_trip(tmp_path: Path, spec: ServerSpec) -> None:
    reg = _make_registrar(tmp_path)
    reg.register_server(spec)
    got = reg.get_server("cassandra")
    assert got is not None
...
def test_register_preserves_non_ascii_values(tmp_path: Path) -> None:
    ...
    assert "cassandra" in data.get("mcp_servers", {})
