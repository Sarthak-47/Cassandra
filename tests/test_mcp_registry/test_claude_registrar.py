...
from cassandra.mcp_registry.claude import ClaudeRegistrar
...
def _spec() -> ServerSpec:
    return ServerSpec(
        ...
        args=("-m", "cassandra.cli", "mcp", "serve"),
        env={},
    )
...
def test_get_server_returns_none_when_unregistered(tmp_path: Path) -> None:
    reg = _make_registrar(tmp_path, cli=None)
    assert reg.get_server("cassandra") is None
...
def test_register_via_cli_includes_env(tmp_path: Path) -> None:
    ...
    assert add_cmd[e_idx + 1] == "CASSANDRA_PROXY_URL=http://127.0.0.1:9000"
...
def test_register_writes_file_when_no_cli(tmp_path: Path) -> None:
    ...
    assert "cassandra" in data["mcpServers"]
    assert data["mcpServers"]["cassandra"]["command"] == _RESOLVED_COMMAND[0]
    assert data["mcpServers"]["cassandra"]["args"] == list(_RESOLVED_ARGS)
        ...
        json.dumps(
            {
                "mcpServers": {
                    "cassandra": {
                        "command": _RESOLVED_COMMAND[0],
                        "args": list(_RESOLVED_ARGS),
                        "env": {"CASSANDRA_PROXY_URL": "http://127.0.0.1:9999"},
                    }
                }
            }
        )
...
def test_unregister_via_cli(tmp_path: Path) -> None:
    ...
    assert cmd[:5] == ["/usr/local/bin/claude", "mcp", "remove", "cassandra", "-s"]
    assert cmd[5] == "user"
...
def test_unregister_via_file_when_no_cli(tmp_path: Path) -> None:
    ...
    cfg.write_text(
        json.dumps(
            {
                "mcpServers": {
                    "cassandra": {"command": _RESOLVED_COMMAND[0], "args": list(_RESOLVED_ARGS)},
                    "other": {"command": "other"},
                }
            }
        )
    ...
    assert reg.unregister_server("cassandra") is True
    data = json.loads(cfg.read_text())
    assert "cassandra" not in data["mcpServers"]
    assert "other" in data["mcpServers"]


def test_unregister_returns_false_when_absent(tmp_path: Path) -> None:
    reg = _make_registrar(tmp_path, cli=None)
    assert reg.unregister_server("cassandra") is False
