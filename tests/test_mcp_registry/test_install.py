...
from cassandra.mcp_registry.base import (
    MCPRegistrar,
    RegisterResult,
    RegisterStatus,
    ServerSpec,
...
def test_build_spec_default_proxy_no_env(monkeypatch) -> None:
    monkeypatch.setattr(
        ...
        lambda: ["/opt/cassandra/bin/cassandra"],
    )
    spec = build_cassandra_spec()
    assert spec.name == "cassandra"
    assert spec.command == "/opt/cassandra/bin/cassandra"
    assert spec.args == ("mcp", "serve")
    assert spec.env == {}


def test_build_spec_custom_proxy_sets_env() -> None:
    spec = build_cassandra_spec("http://127.0.0.1:9999")
    assert spec.env == {"CASSANDRA_PROXY_URL": "http://127.0.0.1:9999"}


def test_build_spec_default_url_omits_env() -> None:
    spec = build_cassandra_spec(DEFAULT_PROXY_URL)
    assert spec.env == {}


def test_build_spec_falls_back_to_python_module_when_no_binary(monkeypatch) -> None:
    monkeypatch.setattr("cassandra.install.runtime.shutil.which", lambda name: None)
    monkeypatch.setattr("cassandra.install.runtime.sys.executable", "/usr/bin/python")

    spec = build_cassandra_spec()

    assert spec.command == "/usr/bin/python"
    assert spec.args == ("-m", "cassandra.cli", "mcp", "serve")
    assert spec.env == {}
...
def test_build_serena_spec_disables_dashboard_popup_by_default() -> None:
    # Cassandra installs Serena by default; the dashboard browser tab must not
...
def test_install_everywhere_passes_proxy_url_into_spec() -> None:
    ...
    assert captured[0].env == {"CASSANDRA_PROXY_URL": "http://localhost:9000"}
