...
from cassandra.subscription.client import (
    _BETA_HEADER,
    _USAGE_URL,
    SubscriptionClient,
    _credentials_path,
...
def test_read_cached_oauth_token_prefers_env_and_checks_expiry(
    ...
    monkeypatch.setattr("cassandra.subscription.client._load_credentials_file", lambda: None)
    assert read_cached_oauth_token() == "env-token"

    monkeypatch.delenv("CLAUDE_CODE_OAUTH_TOKEN", raising=False)
    monkeypatch.setattr(
        "cassandra.subscription.client._load_credentials_file",
...
async def test_subscription_client_fetch_handles_success_and_status_codes(
    ...
    record: dict = {}
    monkeypatch.setattr(
        "cassandra.subscription.client.httpx.AsyncClient",
    ...
    monkeypatch.setattr(
        "cassandra.subscription.client.SubscriptionSnapshot.from_api_response",
...
async def test_subscription_client_fetch_uses_cached_token_and_handles_exceptions(
    ...
    monkeypatch.setattr("cassandra.subscription.client.read_cached_oauth_token", lambda: None)
    assert await client.fetch() is None

    monkeypatch.setattr(
        "cassandra.subscription.client.read_cached_oauth_token",
        lambda: "cached-token",
    )
    monkeypatch.setattr(
        "cassandra.subscription.client.httpx.AsyncClient",
