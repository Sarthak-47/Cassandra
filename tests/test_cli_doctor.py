...
import cassandra.cli.doctor as doctor_mod
from cassandra.cli.doctor import (
    FAIL,
    PASS,
    SKIP,
    WARN,
...
class TestProxyLiveness:
    def test_down_is_fail_with_hint(self):
        ...
        assert "cassandra proxy" in (result.hint or "")
...
class TestClaudeRouting:
    ...
    def test_non_cassandra_url_warns(self, tmp_path):
        path = tmp_path / "settings.json"
        path.write_text(
...
class TestDoctorCommand:
    ...
    def isolated(self, tmp_path, monkeypatch):
        ...
        for var in ("ANTHROPIC_BASE_URL", "OPENAI_BASE_URL", "CASSANDRA_PORT"):
    ...
    def test_port_env_var_respected(self, runner, isolated, monkeypatch):
        ...
        monkeypatch.setattr(doctor_mod, "probe_json", recording_probe)
        runner.invoke(main, ["doctor"], env={"CASSANDRA_PORT": "9999"})
        assert "http://127.0.0.1:9999/livez" in seen
