...
from cassandra.memory import storage_router as sr_mod
...
def patch_backend(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    import cassandra.memory.backends.local as _local_mod
...
# (x-cassandra-project-id / x-cassandra-cwd / system-prompt cwd:) must
...
def test_unresolved_project_returns_no_context(tmp_path: Path) -> None:
    ...
    async def run() -> None:
        ...
        ctx_unresolved = sr_mod.RequestContext(
            headers={},  # No x-cassandra-* headers.
