"""Tests for cassandra.update_check (PyPI probe, cache, banner notice)."""
...
def test_disabled_by_env(monkeypatch, val):
    monkeypatch.setenv("CASSANDRA_UPDATE_CHECK", val)
    assert uc.is_update_check_enabled() is False


def test_disabled_in_stateless(monkeypatch):
    monkeypatch.setenv("CASSANDRA_STATELESS", "1")
    assert uc.is_update_check_enabled() is False
...
def test_notice_when_newer(monkeypatch):
    ...
    assert notice and "0.27.0" in notice and "cassandra update" in notice
