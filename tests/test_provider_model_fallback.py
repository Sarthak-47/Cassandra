...
from cassandra.providers.anthropic import (
    AnthropicProvider,
    _infer_model_tier,
)
from cassandra.providers.anthropic import (
...
from cassandra.providers.google import GeminiTokenCounter, GoogleProvider
from cassandra.providers.openai import (
    OpenAIProvider,
    _infer_model_family,
)
from cassandra.providers.openai import (
...
class TestGoogleModelFallback:
    ...
    def test_future_gemini_model_uses_registry_family_fallback(self):
        ...
        with patch("cassandra.models.registry.get_model_pricing", return_value=None):
...
class TestAnthropicConfigLoading:
    ...
    def test_load_from_env_var_json(self):
        ...
        with patch.dict(os.environ, {"CASSANDRA_MODEL_LIMITS": json.dumps(config)}):
    ...
    def test_load_from_env_var_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.dict(os.environ, {"CASSANDRA_MODEL_LIMITS": str(config_path)}):
                loaded = anthropic_load_config()
                assert loaded["context_limits"]["file-model"] == 400000

    def test_load_from_config_file(self):
        """Test loading from ~/.cassandra/models.json."""
        config = {
            "anthropic": {
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            config_dir = Path(tmpdir) / ".cassandra"
            config_dir.mkdir()
    ...
    def test_env_var_overrides_config_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.object(Path, "home", return_value=Path(tmpdir)):
                with patch.dict(os.environ, {"CASSANDRA_MODEL_LIMITS": json.dumps(env_config)}):
                    loaded = anthropic_load_config()
                    # Env var should win
...
class TestCrossProviderConsistency:
    ...
    def test_both_providers_use_same_env_var(self):
        """Test that both providers use CASSANDRA_MODEL_LIMITS."""
        config = {
    ...
    def test_both_providers_warn_for_unknown_models(self):
        ...
        from cassandra.providers import anthropic as anthropic_module
        from cassandra.providers import openai as openai_module
