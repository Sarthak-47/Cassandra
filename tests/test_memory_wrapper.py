...
from cassandra.memory.wrapper import MemoryWrapper, _MemoryAPI, with_memory
...
def test_memory_wrapper_lazy_initialization_and_factory(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("cassandra.memory.wrapper.HierarchicalMemory.create", fake_create)
...
def test_wrapped_completions_create_injects_parses_and_stores(
    ...
    monkeypatch.setattr(
        "cassandra.memory.wrapper.inject_memory_instruction",
        lambda messages, short=True: (
            messages + [{"role": "system", "content": "memory-instruction"}]
        ),
    )
    monkeypatch.setattr(
        "cassandra.memory.wrapper.parse_response_with_memory",
