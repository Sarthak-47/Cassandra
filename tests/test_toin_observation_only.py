...
def test_compression_hint_is_not_publicly_exported():
    """`CompressionHint` is no longer re-exported from `cassandra.telemetry`."""
    import cassandra.telemetry as telemetry_pkg

    assert not hasattr(telemetry_pkg, "CompressionHint"), (
        "PR-B5: CompressionHint was retired and must not be importable from cassandra.telemetry."
    )
...
def test_record_does_not_alter_compression_decision():
    ...
    """
    smart_crusher_module = pytest.importorskip("cassandra.transforms.smart_crusher")
...
def test_smart_crusher_determinism_property():
    ...
    Cassandra). The parametrized test above covers the deterministic
