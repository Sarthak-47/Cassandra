...
GREP_OUTPUT = (
    'cassandra/transforms/content_router.py:1375:            if role in ("tool", "assistant"):\n'
    'cassandra/transforms/smart_crusher.py:1010:            if msg.get("role") == "tool":\n'
    'cassandra/transforms/content_router.py:2653:            if role == "tool":\n'
    'cassandra/proxy/handlers/anthropic.py:44:                elif block.get("type") == "tool_result":\n'
    'cassandra/cache/prefix_tracker.py:88:            if message.get("role") == "tool":\n'
    'cassandra/proxy/helpers.py:102:        if msg.get("role") == "tool":\n'
    "cassandra/transforms/pipeline.py:132:        transforms.append(ContentRouter())\n"
    "cassandra/transforms/kompress_compressor.py:1376:            result = self.compress(content)\n"
    'cassandra/transforms/content_router.py:1483:                    compressor_name = "KompressCompressor"\n'
    'cassandra/transforms/content_router.py:1568:                compressor_name = "KompressCompressor"\n'
    "cassandra/transforms/content_router.py:2667:                bias = self._get_tool_bias(tool_name)\n"
    "cassandra/transforms/content_router.py:3317:        result = self.compress(content, context=context)\n"
    "cassandra/transforms/content_router.py:3331:                and not CCR_RETRIEVAL_MARKER_RE.search(result.compressed)\n"
    "cassandra/proxy/handlers/openai.py:697:    def _compress_openai_responses_live_text_units(self)\n"
    "cassandra/transforms/compression_units.py:204:    def compress_unit_with_router(self, unit)\n"
    "cassandra/config.py:676:class TransformResult:  # messages, tokens_before, tokens_after\n"
)
