...
class TestCassandraMode:
    """Tests for CassandraMode enum."""

    def test_enum_values(self):
        """All expected enum values exist with correct string values."""
        assert CassandraMode.AUDIT.value == "audit"
        assert CassandraMode.OPTIMIZE.value == "optimize"
        assert CassandraMode.SIMULATE.value == "simulate"

    def test_string_conversion(self):
        """CassandraMode inherits from str for string compatibility."""
        # Enum value access works as string
        assert CassandraMode.AUDIT.value == "audit"
        assert CassandraMode.OPTIMIZE.value == "optimize"
        assert CassandraMode.SIMULATE.value == "simulate"
        # Can compare directly with strings since it inherits from str
        assert CassandraMode.AUDIT == "audit"
        assert CassandraMode.OPTIMIZE == "optimize"
        assert CassandraMode.SIMULATE == "simulate"
        # isinstance check confirms str inheritance
        assert isinstance(CassandraMode.AUDIT, str)
...
class TestCassandraConfig:
    """Tests for CassandraConfig main configuration class."""

    def test_default_values(self):
        """Default values are correctly set."""
        config = CassandraConfig()
        assert config.store_url == "sqlite:///cassandra.db"
        assert config.default_mode == CassandraMode.AUDIT
    ...
    def test_get_context_limit_direct_match(self):
        ...
        config = CassandraConfig(model_context_limits={"gpt-4o": 128000, "claude-3-opus": 200000})
    ...
    def test_get_context_limit_prefix_match(self):
        ...
        config = CassandraConfig(model_context_limits={"gpt-4": 128000, "claude-3": 200000})
        # Prefix matches
    ...
    def test_get_context_limit_not_found(self):
        ...
        config = CassandraConfig(model_context_limits={"gpt-4": 128000})
    ...
    def test_model_context_limits_isolation(self):
        ...
        config1 = CassandraConfig()
        config2 = CassandraConfig()
