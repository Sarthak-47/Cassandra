...
from cassandra.storage import JSONLStorage, SQLiteStorage, Storage, create_storage
...
def test_create_storage_builtin_entrypoint_and_fallback(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda group: [FakeEntryPoint()] if group == "cassandra.storage_backend" else [],
    )
    ...
    monkeypatch.setattr("cassandra.storage.SQLiteStorage", FakeSQLiteStorage)
