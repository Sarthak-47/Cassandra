...
from cassandra.proxy.handlers.openai import (
    OpenAIHandlerMixin,
    _decode_openai_bearer_payload,
    _passthrough_usage_from_json,
    _prefers_http1_passthrough,
)
from cassandra.proxy.helpers import _cassandra_bypass_enabled
from cassandra.proxy.server import CassandraProxy
...
def test_cassandra_bypass_helper_is_transport_neutral() -> None:
    assert _cassandra_bypass_enabled({"x-cassandra-bypass": "true"}) is True
    assert _cassandra_bypass_enabled({"x-cassandra-bypass": " TRUE "}) is True
    assert _cassandra_bypass_enabled({"x-cassandra-mode": "passthrough"}) is True
    assert _cassandra_bypass_enabled({"x-cassandra-mode": " PASSTHROUGH "}) is True
    assert _cassandra_bypass_enabled({"x-cassandra-bypass": "false"}) is False
    assert _cassandra_bypass_enabled({}) is False
    assert _cassandra_bypass_enabled(None) is False
    assert OpenAIHandlerMixin._cassandra_bypass_enabled({"x-cassandra-bypass": "true"}) is True
...
def test_vertex_passthrough_records_usage_metadata_for_dashboard() -> None:
    handler = object.__new__(CassandraProxy)
...
def test_vertex_gemini_non_text_generate_records_dashboard_outcome() -> None:
    ...
    assert response.headers["x-cassandra-tokens-before"] == "31"
    assert response.headers["x-cassandra-tokens-after"] == "31"
    assert response.headers["x-cassandra-tokens-saved"] == "0"
    assert len(outcomes) == 1
    outcome = outcomes[0]
    assert outcome.provider == "vertex:google"
    assert outcome.model == "gemini-2.0-flash"
...
def test_retry_request_retries_connect_timeout() -> None:
    proxy = object.__new__(CassandraProxy)
...
def test_proxy_helper_creates_fresh_image_compressors(monkeypatch) -> None:
    from cassandra.proxy import helpers

    monkeypatch.setattr(helpers, "_image_compressor_available", None)
    _FreshCompressor.instances = 0

    with patch("cassandra.image.ImageCompressor", _FreshCompressor):
...
# subsystem's ProjectResolver: x-cassandra-project-id → x-cassandra-cwd →
...
def test_resolve_ccr_workspace_explicit_project_id_wins() -> None:
    """x-cassandra-project-id is the highest-priority signal."""
    request = _fake_request({"x-cassandra-project-id": "my-cool-project"})
    body = {}
...
def test_resolve_ccr_workspace_cwd_header() -> None:
    """x-cassandra-cwd produces a stable per-cwd key + basename label."""
    request = _fake_request({"x-cassandra-cwd": "/home/user/code/daphni-rails"})
    body = {}
    key, label = AnthropicHandlerMixin._resolve_ccr_workspace(request, body)
...
def test_resolve_ccr_workspace_two_cwds_get_distinct_keys() -> None:
    ...
    key_a, _ = AnthropicHandlerMixin._resolve_ccr_workspace(
        _fake_request({"x-cassandra-cwd": "/home/user/code/daphni-rails"}), {}
    )
    key_b, _ = AnthropicHandlerMixin._resolve_ccr_workspace(
        _fake_request({"x-cassandra-cwd": "/home/user/code/tamag0"}), {}
    )
