...
class TestGlobalTelemetryCollector:
    ...
    def test_env_var_disables_telemetry(self, monkeypatch):
        """CASSANDRA_TELEMETRY_DISABLED environment variable disables telemetry."""
        reset_telemetry_collector()
        monkeypatch.setenv("CASSANDRA_TELEMETRY_DISABLED", "1")
    ...
    def test_cassandra_telemetry_off_disables_collector(self, monkeypatch, off_value):
        """CASSANDRA_TELEMETRY=off (and other documented opt-out values) disables
        the collector — closes #390.

        Pre-#390 the collector only honoured CASSANDRA_TELEMETRY_DISABLED, which
        is undocumented. Users following the docs set CASSANDRA_TELEMETRY=off and
        ...
        """
        reset_telemetry_collector()
        monkeypatch.delenv("CASSANDRA_TELEMETRY_DISABLED", raising=False)
        monkeypatch.setenv("CASSANDRA_TELEMETRY", off_value)

        collector = get_telemetry_collector()

        assert collector._config.enabled is False, (
            f"CASSANDRA_TELEMETRY={off_value!r} must disable the collector — "
    ...
    def test_cassandra_telemetry_on_keeps_collector_enabled(self, monkeypatch):
        """Sanity check: the explicit opt-in path (CASSANDRA_TELEMETRY=on) leaves
