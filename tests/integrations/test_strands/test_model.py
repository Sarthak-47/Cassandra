"""Real-world integration tests for Strands CassandraStrandsModel.
...
# ============================================================================
# Test Class for CassandraStrandsModel
# ============================================================================


@pytest.mark.skipif(SKIP_BEDROCK, reason="AWS credentials not available")
@pytest.mark.skipif(not STRANDS_AVAILABLE, reason="strands-agents not installed")
class TestCassandraStrandsModelReal:
    """Real-world integration tests for CassandraStrandsModel with Bedrock."""
    ...
    def wrapped_model(self, base_bedrock_model):
        """Create a CassandraStrandsModel wrapping the Bedrock model."""
    ...
    def test_model_wrapper_basic_response(self, wrapped_model):
        ...
        result = agent("Say 'Hello, Cassandra!' and nothing else.")
    ...
    def test_model_wrapper_custom_config(self, base_bedrock_model):
        """Test that custom CassandraConfig is applied."""
        from cassandra import CassandraConfig
        from cassandra.integrations.strands import CassandraStrandsModel

        custom_config = CassandraConfig()
        ...
        assert wrapped.cassandra_config is custom_config
        assert wrapped.cassandra_config.smart_crusher.min_tokens_to_crush == 50
    ...
    def test_model_wrapper_provider_detection(self, base_bedrock_model):
        ...
        assert wrapped._cassandra_provider is not None
        assert isinstance(wrapped._cassandra_provider, AnthropicProvider)
