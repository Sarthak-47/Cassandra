"""Unit tests for Strands CassandraHookProvider.

These tests use mocks and do NOT require AWS credentials or strands-agents.
They test the internal logic of CassandraHookProvider in isolation.
...
class TestCassandraHookProviderInit:
    """Tests for CassandraHookProvider initialization."""

    def test_init_with_defaults(self):
        """Initialize with default settings."""
        from cassandra.integrations.strands import CassandraHookProvider

        hook = CassandraHookProvider()
    ...
    def test_init_creates_default_config_if_none(self):
        """Initialize creates a default CassandraConfig if none provided."""
        from cassandra import CassandraConfig
        from cassandra.integrations.strands import CassandraHookProvider
        ...
        assert isinstance(hook.config, CassandraConfig)


class TestRegisterHooks:
    """Tests for CassandraHookProvider.register_hooks method."""

    def test_register_hooks_adds_callback_to_registry(self):
        """register_hooks adds AfterToolCallEvent callback to registry."""
        from cassandra.integrations.strands import CassandraHookProvider

        hook = CassandraHookProvider(compress_tool_outputs=True)
    ...
    def test_register_hooks_skips_when_compression_disabled(self):
        ...
        hook = CassandraHookProvider(compress_tool_outputs=False)
        mock_registry = MagicMock()
...
class TestCrusherLazyInit:
    ...
    def test_crusher_uses_configured_min_tokens(self):
        ...
        hook = CassandraHookProvider(min_tokens_to_compress=250)
...
class TestShouldSkipCompression:
    ...
    def test_skip_error_results_when_preserve_errors_true(self):
        ...
        hook = CassandraHookProvider(preserve_errors=True)
    ...
    def test_allow_error_results_when_preserve_errors_false(self):
        ...
        hook = CassandraHookProvider(preserve_errors=False)
        result = {"status": "error", "content": [{"text": "Error message"}]}
...
class TestGetSavingsSummary:
    ...
    def test_summary_with_compressions(self):
        ...
        from cassandra.integrations.strands import CassandraHookProvider
        from cassandra.integrations.strands.hooks import CompressionMetrics
