"""Tests for TOIN feedback loop: cassandra_retrieve calls flow back to TOIN."""
...
from cassandra.cache.compression_store import get_compression_store, reset_compression_store
from cassandra.telemetry import (
    TOINConfig,
...
def reset_globals(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("CASSANDRA_TOIN_PATH", temp_toin_path)
    reset_toin()
...
def test_kompress_ccr_retrieval_updates_toin():
    ...
    original = "\n".join(
        [
            "CASSANDRA_MODE=debug PATH=/tmp/cassandra",
            "ordinary line without the target token",
            "another ordinary line",
        ]
    )
    compressed = "CASSANDRA_MODE=debug"
    ...
    assert "CASSANDRA" in entry.original_content
...
class TestCCRFeedbackExtraction:
    """Bug 2: _record_ccr_feedback_from_response extracts cassandra_retrieve calls."""

    def test_extract_cassandra_retrieve_from_response(self):
        """Should detect cassandra_retrieve tool_use blocks in response content."""
        response = {
            "content": [
        ...
        for block in content:
            if not isinstance(block, dict):
                continue
            if block.get("type") == "tool_use" and block.get("name") == "cassandra_retrieve":
    ...
    def test_ignore_non_retrieve_tool_calls(self):
        """Should ignore tool_use blocks that are not cassandra_retrieve."""
        response = {
            "content": [
                {
                    "type": "tool_use",
    ...
    def test_missing_hash_skipped(self):
        """Should skip cassandra_retrieve calls without a hash."""
        response = {
            "content": [
                {
                    "type": "tool_use",
...
class TestStreamingFeedbackIntegration:
    """Bug 2: Full feedback loop — streaming cassandra_retrieve reaches TOIN."""
    ...
    def test_record_ccr_feedback_calls_store_retrieve(self):
        ...
        with patch(
            "cassandra.cache.compression_store.get_compression_store",
            return_value=mock_store,
        ):
            # Create a minimal proxy to test the method
            proxy = CassandraProxy.__new__(CassandraProxy)
...
class TestParseSSEToolUse:
    ...
    def test_parse_sse_extracts_tool_use(self):
        ...
        sse_data = (
            ...
            "\n"
            'data: {"type":"content_block_start","index":1,"content_block":{"type":"tool_use","id":"toolu_abc","name":"cassandra_retrieve"}}\n'
            "\n"
        ...
        assert tool_block["name"] == "cassandra_retrieve"
