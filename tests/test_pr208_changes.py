...
- _cassandra_log_dir lazy resolution via paths module
...
class TestJitterDelayMs:
    """Tests for cassandra.proxy.helpers.jitter_delay_ms."""

    def test_attempt_zero_returns_within_base_range(self) -> None:
        from cassandra.proxy.helpers import jitter_delay_ms
...
# ---------------------------------------------------------------------------
# _cassandra_log_dir lazy resolution tests
# ---------------------------------------------------------------------------


class TestCassandraLogDir:
    """Tests for _cassandra_log_dir using cassandra.paths.log_dir."""

    def test_log_dir_respects_workspace_env_var(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
        from cassandra.proxy.helpers import _cassandra_log_dir

        monkeypatch.setenv("CASSANDRA_WORKSPACE_DIR", str(tmp_path))
        result = _cassandra_log_dir()
...
class TestIsAnthropicAuth:
    """Tests for cassandra.proxy.helpers.is_anthropic_auth."""

    def test_detects_x_api_key(self) -> None:
        from cassandra.proxy.helpers import is_anthropic_auth
...
class TestSetupFileLogging:
    """Tests for _setup_file_logging using the new _cassandra_log_dir path."""

    def test_setup_file_logging_creates_log_dir(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
        from cassandra.proxy.helpers import _setup_file_logging
        ...
        cassandra_logger.handlers = [
            h for h in cassandra_logger.handlers if not isinstance(h, RotatingFileHandler)
        ]

        _setup_file_logging()

        # Verify a RotatingFileHandler was added
        has_rotating = any(isinstance(h, RotatingFileHandler) for h in cassandra_logger.handlers)
    ...
    def test_setup_file_logging_handles_oserror(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        # Monkey-patch _cassandra_log_dir to return a path that will cause OSError
        def _bad_log_dir():
            return Path("/nonexistent/deeply/nested/path/that/cannot/exist/___test___")

        import cassandra.proxy.helpers as helpers_mod

        monkeypatch.setattr(helpers_mod, "_cassandra_log_dir", _bad_log_dir)
        # Should not raise
        _setup_file_logging()

    def test_importing_server_does_not_install_file_handler(self, tmp_path: Path) -> None:
        """Importing cassandra.proxy.server must NOT attach a RotatingFileHandler
        ...
        script = textwrap.dedent(
            ...
            import logging
            from logging.handlers import RotatingFileHandler
            import cassandra.proxy.server  # noqa: F401
            hr = logging.getLogger("cassandra")
            installed = any(isinstance(h, RotatingFileHandler) for h in hr.handlers)
            print("INSTALLED" if installed else "CLEAN")
            """
        ).strip()
        env = {**os.environ, "CASSANDRA_WORKSPACE_DIR": str(tmp_path)}
        ...
        assert result.stdout.strip() == "CLEAN", (
            f"Importing cassandra.proxy.server attached a file handler: {result.stdout!r}"
        )
...
class TestWrapGetLogPath:
    """Tests for _get_log_path in wrap.py using cassandra.paths."""

    def test_get_log_path_returns_proxy_log(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
        from cassandra.cli.wrap import _get_log_path
    ...
    def test_get_log_path_creates_directory(
        ...
        monkeypatch.setenv("CASSANDRA_WORKSPACE_DIR", str(log_subdir))
        result = _get_log_path()
