...
PyO3 (`cassandra._core`). These tests guard against regressions by
...
Skipped automatically when the `cassandra._core` wheel isn't installed
(e.g. CI lane without the maturin step). The Rust crate's own
`cargo run -p cassandra-parity` covers the same fixtures from the Rust
...
def _has_core() -> bool:
    try:
        import cassandra._core  # noqa: F401
...
def test_rust_backend_matches_recorded_output(fixture_path: Path):
    ...
    real Rust regression (cross-check with `cargo run -p cassandra-parity`
    to localize).
    """
    from cassandra.transforms.diff_compressor import DiffCompressor, DiffCompressorConfig
...
def test_content_router_uses_rust_backend():
    ...
    from cassandra.transforms.diff_compressor import DiffCompressor
