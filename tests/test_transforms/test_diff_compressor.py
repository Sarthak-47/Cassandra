...
was retired in favor of the Rust-backed shim (`cassandra._core` via PyO3).
...
`crates/cassandra-core/tests`. Public-API tests (anything calling
...
def _cfg_below_threshold():
    ...
    from cassandra.transforms.diff_compressor import DiffCompressorConfig
...
class TestRoutingGapDetectorScanWindow:
    ...
    def test_detect_picks_up_diff_after_long_commit_message(self):
        from cassandra.transforms.content_detector import (
            ContentType,
            detect_content_type,
        )
