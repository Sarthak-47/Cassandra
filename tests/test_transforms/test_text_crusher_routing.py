...
when CASSANDRA_TEXT_CRUSHER is enabled (real prose savings), instead of the
...
def test_gate_routes_to_text_crusher_when_enabled(monkeypatch):
    monkeypatch.setenv("CASSANDRA_TEXT_CRUSHER", "1")
...
def test_text_crusher_disabled_by_default(monkeypatch):
    monkeypatch.delenv("CASSANDRA_TEXT_CRUSHER", raising=False)
    router = ContentRouter()
