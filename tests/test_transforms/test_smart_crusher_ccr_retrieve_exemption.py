"""Regression tests for #1077: SmartCrusher must not re-compress cassandra_retrieve
tool results.

When the proxy's CCR path returns content via cassandra_retrieve, the client sends
...
from cassandra import OpenAIProvider, Tokenizer
...
def _build_extension() -> None:
    try:
        ...
        pytest.skip(
            "cassandra._core not built — run `bash scripts/build_rust_extension.sh`",
...
from cassandra.transforms.smart_crusher import SmartCrusher  # noqa: E402
...
class TestCassandraRetrieveExemptionOpenAI:
    """OpenAI-style role=tool messages from cassandra_retrieve must not be crushed."""

    def test_retrieve_result_not_compressed(self):
        """cassandra_retrieve tool result is skipped even when content is large."""
        content = _big_content()
        messages = [
            # The assistant turn that called cassandra_retrieve
            {
                "role": "assistant",
                "tool_calls": [
                    {
        ...
        assert tool_msg["content"] == content, (
            "cassandra_retrieve tool result was re-compressed (infinite loop bug #1077)"
        )
...
class TestCassandraRetrieveExemptionAnthropic:
    """Anthropic-style tool_result content blocks from cassandra_retrieve must not be crushed."""

    def test_retrieve_result_not_compressed(self):
        """Anthropic tool_result block for cassandra_retrieve is skipped."""
        content = _big_content()
        messages = [
            # assistant turn with tool_use block calling cassandra_retrieve
            {
                "role": "assistant",
                "content": [
                    {
        ...
        assert tool_result_block["content"] == content, (
            "cassandra_retrieve Anthropic tool_result was re-compressed (#1077)"
        )
        assert not any("smart_crush" in t for t in result.transforms_applied)
    ...
    def test_mixed_retrieve_and_normal_only_normal_compressed(self):
        ...
        # cassandra_retrieve result must be untouched
        assert ccr_block["content"] == content, (
            "cassandra_retrieve result was compressed — infinite loop bug #1077"
        )
