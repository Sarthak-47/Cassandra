...
from cassandra.transforms import content_router as cr

# Patch the native detector via its string target ("cassandra._core.detect_content_type")
# rather than a module alias captured at import time. content_router._detect_content does a
# fresh `from cassandra._core import detect_content_type` on every call, and other tests pop
# cassandra._core out of sys.modules (e.g. test_rust_core_smoke), which rebuilds the module
...
def test_falls_back_on_rust_exception(monkeypatch):
    ...
    monkeypatch.setattr("cassandra._core.detect_content_type", _boom)
...
def test_falls_back_on_baseexception_panic(monkeypatch):
    ...
    monkeypatch.setenv("CASSANDRA_DETECT_BACKEND", "rust")
    monkeypatch.setattr("cassandra._core.detect_content_type", _panic)
    monkeypatch.setattr(cr, "_detect_panic_warned", False, raising=False)
...
def test_control_flow_exceptions_propagate(monkeypatch):
    ...
    monkeypatch.setenv("CASSANDRA_DETECT_BACKEND", "rust")
    monkeypatch.setattr("cassandra._core.detect_content_type", _interrupt)
    monkeypatch.setattr(cr, "_detect_panic_warned", False, raising=False)
...
def test_cancelled_error_propagates(monkeypatch):
    ...
    monkeypatch.setenv("CASSANDRA_DETECT_BACKEND", "rust")
    monkeypatch.setattr("cassandra._core.detect_content_type", _cancel)
    monkeypatch.setattr(cr, "_detect_panic_warned", False, raising=False)
