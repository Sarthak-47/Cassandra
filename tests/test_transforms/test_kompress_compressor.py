...
class TestLazyImports:
    ...
    def test_is_kompress_available_importable(self) -> None:
        ...
        from cassandra.transforms.kompress_compressor import is_kompress_available
    ...
    def test_module_import_without_torch(self) -> None:
        ...
        with patch.dict(
            ...
            from cassandra.transforms.kompress_compressor import (
...
class TestKompressBackendSelection:
    def test_selected_backend_aliases(self, monkeypatch) -> None:
        import cassandra.transforms.kompress_compressor as kmod

        monkeypatch.setenv("CASSANDRA_KOMPRESS_BACKEND", "mps")
        assert kmod._selected_backend() == "pytorch_mps"

        monkeypatch.setenv("CASSANDRA_KOMPRESS_BACKEND", "coreml")
        assert kmod._selected_backend() == "onnx_coreml"

        monkeypatch.setenv("CASSANDRA_KOMPRESS_BACKEND", "cpu")
        assert kmod._selected_backend() == "onnx_cpu"

        monkeypatch.setenv("CASSANDRA_KOMPRESS_BACKEND", "unknown")
    ...
    def test_unrecognized_backend_warns_and_falls_back_to_auto(self, monkeypatch, caplog) -> None:
        ...
        monkeypatch.setenv("CASSANDRA_KOMPRESS_BACKEND", "tpu")
    ...
    def test_valid_backend_values_do_not_warn(self, monkeypatch, caplog) -> None:
        ...
        with caplog.at_level(logging.WARNING, logger=kmod.logger.name):
            for value in ("auto", "onnx", "cpu", "coreml", "mps", "torch", "ONNX-CPU"):
                monkeypatch.setenv("CASSANDRA_KOMPRESS_BACKEND", value)
                kmod._selected_backend()
            monkeypatch.delenv("CASSANDRA_KOMPRESS_BACKEND", raising=False)
            kmod._selected_backend()
    ...
    def test_forced_pytorch_mps_backend_uses_mps_device(self, monkeypatch) -> None:
        ...
        monkeypatch.setenv("CASSANDRA_KOMPRESS_BACKEND", "pytorch_mps")
    ...
    def test_forced_coreml_backend_uses_onnx_coreml(self, monkeypatch) -> None:
        ...
        monkeypatch.setenv("CASSANDRA_KOMPRESS_BACKEND", "onnx_coreml")
        monkeypatch.setattr(kmod, "_kompress_cache", {})
        monkeypatch.setattr(
            kmod,
            "_load_kompress_onnx",
    ...
    def test_onnx_session_options_read_thread_caps(self, monkeypatch) -> None:
        ...
        monkeypatch.setenv("CASSANDRA_KOMPRESS_ONNX_INTRA_THREADS", "2")
        monkeypatch.setenv("CASSANDRA_KOMPRESS_ONNX_INTER_THREADS", "1")
...
class TestKompressResult:
    def test_tokens_saved(self) -> None:
        from cassandra.transforms.kompress_compressor import KompressResult
    ...
    def test_default_model(self) -> None:
        from cassandra.transforms.kompress_compressor import HF_MODEL_ID, KompressResult
...
class TestKompressCompressorPassthrough:
    ...
    def test_fallback_on_model_error(self) -> None:
        ...
        with patch(
            "cassandra.transforms.kompress_compressor._load_kompress",
...
class TestUnloadKompressModel:
    def test_unload_when_no_model(self) -> None:
        import cassandra.transforms.kompress_compressor as kmod
        from cassandra.transforms.kompress_compressor import unload_kompress_model
