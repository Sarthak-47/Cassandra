...
`tests/parity/fixtures/smart_crusher/` through `cassandra._core.SmartCrusher`
...
same fixtures via `cargo run -p cassandra-parity --bin parity-run --
...
def test_rust_backend_matches_recorded_output(fixture_path: Path):
    ...
    Rust regression — cross-check with `cargo run -p cassandra-parity`.
    """
