...
The Rust core integration test (`crates/cassandra-core/tests/ccr_roundtrip.rs`)
...
def _force_lossy_config():
    ...
    from cassandra._core import SmartCrusherConfig
...
def test_native_default_crusher_has_a_store() -> None:
    ...
    from cassandra._core import SmartCrusher
...
def test_shim_exposes_ccr_get_and_ccr_len() -> None:
    from cassandra.config import SmartCrusherConfig as PyConfig
...
def test_shim_lossy_crush_populates_store() -> None:
    ...
    `cassandra.transforms.smart_crusher.SmartCrusher` shim — the path
...
def test_explicit_before_after_roundtrip_native() -> None:
    ...
    from cassandra._core import SmartCrusher, SmartCrusherConfig
