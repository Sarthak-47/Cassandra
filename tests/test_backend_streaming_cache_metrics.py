...
* ``StreamingMixin._stream_openai_via_backend`` (Azure/LiteLLM/AnyLLM OpenAI
  ...
  for this path, so ``cassandra perf`` shows 0 cache writes for every
...
Both surface to the user as "Cache write: 0 tokens" in ``cassandra perf``.
"""
...
from cassandra.backends.base import StreamEvent  # noqa: E402
...
class _ListHandler(logging.Handler):
    ...
    ``caplog`` attaches to root; ``cassandra.proxy.helpers._setup_file_logging``
    flips ``logging.getLogger("cassandra").propagate = False`` once a proxy
    instance is constructed in the test, after which root-attached handlers
    stop receiving cassandra-namespaced records. Attaching directly to
    ``cassandra.proxy`` sidesteps that.
    """
...
def test_openai_backend_streaming_emits_perf_with_cache_read_and_inferred_write() -> None:
    ...
    ``cassandra perf`` must report both.
    """
    config = ProxyConfig(
        optimize=False,
        cache_enabled=False,
    ...
    try:
        with patch("cassandra.proxy.server.AnyLLMBackend", return_value=backend):
...
def test_openai_backend_streaming_perf_zeros_when_upstream_omits_usage() -> None:
    ...
    final usage chunk) the PERF line still has to emit so ``cassandra perf``
    counts the request.
    """
    config = ProxyConfig(
        optimize=False,
...
def test_streaming_perf_log_has_no_hardcoded_cache_zeros() -> None:
    ...
    src = Path(__file__).resolve().parents[1] / "cassandra" / "proxy" / "handlers" / "streaming.py"
    text = src.read_text()
