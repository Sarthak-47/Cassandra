...
import cassandra._version as version_module


def test_cassandra_import_stays_lazy() -> None:
    script = textwrap.dedent(
        ...
        print(json.dumps({
            ...
            "cache_loaded": "cassandra.cache" in sys.modules,
            "models_registry_loaded": "cassandra.models.registry" in sys.modules,
            "memory_loaded": "cassandra.memory" in sys.modules,
        }))
        """
    )
...
def test_version_prefers_installed_distribution_metadata() -> None:
    ...
    package_version.assert_called_once_with("cassandra-ai")
...
def test_proxy_package_import_does_not_eagerly_load_server() -> None:
    script = textwrap.dedent(
        ...
        import cassandra.proxy

        print(json.dumps({
            "server_loaded": "cassandra.proxy.server" in sys.modules,
        }))
        """
    )
...
def test_proxy_server_import_skips_litellm_backend() -> None:
    script = textwrap.dedent(
        ...
        import cassandra.proxy.server

        print(json.dumps({
            "litellm_backend_loaded": "cassandra.backends.litellm" in sys.modules,
            "anyllm_backend_loaded": "cassandra.backends.anyllm" in sys.modules,
...
def test_dynamic_detector_import_skips_optional_ml_dependencies(tmp_path: Path) -> None:
    ...
    script = textwrap.dedent(
        ...
        import cassandra.cache.dynamic_detector
...
def test_compress_spreadsheet_public_import_survives_ort_pin() -> None:
    """`from cassandra import compress_spreadsheet` stays eagerly exported, and the
    ...
    script = textwrap.dedent(
        ...
        import cassandra
        from cassandra import compress_spreadsheet

        print(json.dumps({
            "eager": "compress_spreadsheet" in cassandra.__dict__,
