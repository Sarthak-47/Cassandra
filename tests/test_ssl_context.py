"""Unit tests for cassandra.proxy.ssl_context.find_ca_bundle.
...
from cassandra.proxy import ssl_context
from cassandra.proxy.ssl_context import (
...
# ---------------------------------------------------------------------------
# CASSANDRA_TLS_STRICT toggle (issue #1308): corporate TLS-inspection roots
...
class TestTlsStrictDisabled:
    ...
    def test_off_values_disable_strict(self, monkeypatch, val):
        _clean_env(monkeypatch)
        monkeypatch.setenv("CASSANDRA_TLS_STRICT", val)
...
class TestBuildHttpxVerify:
    ...
    def test_toggle_off_returns_relaxed_context(self, monkeypatch):
        ...
        _clean_env(monkeypatch)
        monkeypatch.setenv("CASSANDRA_TLS_STRICT", "0")
...
class TestApplyGlobalTlsRelaxation:
    ...
    def test_patches_urllib3_when_toggle_off(self, monkeypatch):
        ...
        try:
            ...
            assert apply_global_tls_relaxation() is True
            assert getattr(u3ssl.create_urllib3_context, "_cassandra_strict_relaxed", False)
        finally:
