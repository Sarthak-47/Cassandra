"""PR-B5 acceptance tests for ``cassandra.cli.toin_publish``.
...
3. Rows include ``auth_mode``, ``model_family``, ``structure_hash``,
   ...
   ``crates/cassandra-core/src/transforms/recommendations.rs`` consumes.
...
from cassandra.cli.toin_publish import main as publish_main
from cassandra.cli.toin_publish import publish
...
def test_cli_entrypoint_writes_to_output_arg(tmp_path: Path, monkeypatch) -> None:
    """`python -m cassandra.cli.toin_publish --output X --min-observations N`."""
    storage = tmp_path / "toin.json"
    monkeypatch.setenv("CASSANDRA_TOIN_PATH", str(storage))

    # Prime the global TOIN singleton with eligible data.
    from cassandra.telemetry.toin import get_toin, reset_toin
