#!/usr/bin/env python3
"""End-to-end token-savings test for Cortex Code (CoCo) + Cassandra.
...
emit.  Cassandra's SmartCrusher compresses JSON natively without any ML
...
Usage:
    ...
    cd cassandra && uv run python tests/test_cortex_code_compression.py

    # Pytest (CI-friendly assertions):
    cd cassandra && uv run --with pytest pytest tests/test_cortex_code_compression.py -v -s
"""
...
def test_cortex_code_cassandra_compression_saves_tokens() -> None:
    """Cassandra must compress a realistic multi-turn CoCo session."""
...
if __name__ == "__main__":
    ...
    print()
    print("=" * 65)
    print("  Cortex Code × Cassandra  —  token savings benchmark")
    ...
    if total_saved > 0:
        print(
            f"  PASS  cassandra saved {total_saved:,} tokens ({total_pct:.0f}%) across all CoCo payload types"
        )
    else:
        print("  FAIL  no compression — run: pip install 'cassandra-ai[all]'")
    print()
