"""Adversarial backward-compatibility stress tests for ``cassandra.paths``.
...
1. A legacy-only user (only ``CASSANDRA_SAVINGS_PATH`` set) must keep getting
   their legacy path, byte-for-byte, with the new canonical vars unset.
2. A canonical-only user (only ``CASSANDRA_WORKSPACE_DIR`` set) must see every
...
def test_legacy_only_user_savings_unchanged(
    ...
    Only ``CASSANDRA_SAVINGS_PATH`` is set. The canonical workspace/config
    vars are unset. The helper must return the exact legacy value as
    supplied.
    """

    legacy_value = str(tmp_path / "oldstyle" / "savings.json")
    clean_env.setenv(paths.CASSANDRA_SAVINGS_PATH_ENV, legacy_value)
...
def test_canonical_only_user_workspace_bucket_relocates(
    ...
    clean_env.setenv(paths.CASSANDRA_WORKSPACE_DIR_ENV, str(alt_ws))
...
def test_both_set_legacy_wins_over_canonical(
    ...
    When both ``CASSANDRA_SAVINGS_PATH=/old/...`` and
    ``CASSANDRA_WORKSPACE_DIR=/new/...`` are set, ``savings_path()`` must
    ...
    clean_env.setenv(paths.CASSANDRA_SAVINGS_PATH_ENV, str(legacy))
    clean_env.setenv(paths.CASSANDRA_WORKSPACE_DIR_ENV, str(new_ws))
...
def test_all_three_legacy_vars_win_simultaneously(
    ...
    clean_env.setenv(paths.CASSANDRA_WORKSPACE_DIR_ENV, str(new_ws))
    clean_env.setenv(paths.CASSANDRA_SAVINGS_PATH_ENV, str(savings_legacy))
    clean_env.setenv(paths.CASSANDRA_TOIN_PATH_ENV, str(toin_legacy))
    clean_env.setenv(paths.CASSANDRA_SUBSCRIPTION_STATE_PATH_ENV, str(sub_legacy))
