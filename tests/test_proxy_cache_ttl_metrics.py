...
from cassandra.observability import reset_cassandra_tracing, reset_otel_metrics
from cassandra.proxy.cost import CostTracker, build_prefix_cache_stats
...
def test_prometheus_metrics_export_includes_extended_fields() -> None:
    ...
    assert "cassandra_latency_ms_count 1" in exported
    assert 'cassandra_transform_timing_ms_sum{transform="router"} 4.5' in exported
    assert 'cassandra_waste_signal_tokens_total{signal="json_bloat"} 7' in exported
    assert 'cassandra_cache_write_ttl_tokens_total{provider="anthropic",ttl="5m"} 10' in exported
    assert 'cassandra_provider_cache_hit_requests_total{provider="anthropic"} 1' in exported
    assert "cassandra_cache_bust_tokens_lost_total 11" in exported


def test_streaming_parser_extracts_anthropic_ttl_bucket_usage() -> None:
    from cassandra.proxy.server import CassandraProxy, ProxyConfig

    proxy = CassandraProxy(
        ProxyConfig(
            optimize=False,
            cache_enabled=False,
...
def test_stats_endpoint_reports_otel_configuration(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    reset_otel_metrics()
    monkeypatch.setenv("CASSANDRA_OTEL_METRICS_ENABLED", "1")
    monkeypatch.setenv("CASSANDRA_OTEL_METRICS_EXPORTER", "console")
    ...
    assert otel["service_name"] == "cassandra-proxy"
...
def test_stats_endpoint_reports_langfuse_configuration(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("CASSANDRA_LANGFUSE_ENABLED", "1")
    ...
    assert langfuse["service_name"] == "cassandra-proxy"
...
def test_prometheus_export_includes_miss_attribution() -> None:
    ...
    assert (
        'cassandra_cache_miss_attribution_total{provider="anthropic",reason="ttl_expiry"} 1'
        in exported
    )
    assert (
        'cassandra_cache_miss_attribution_total{provider="anthropic",reason="prefix_change"} 1'
        in exported
    )
