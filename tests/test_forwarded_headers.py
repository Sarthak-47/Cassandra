"""Tests for ``cassandra.proxy.forwarded_headers`` — Phase F PR-F4.
...
(``CASSANDRA_PROXY_TRUSTED_GATEWAY_CIDRS`` unset / empty) is
...
from cassandra.proxy.forwarded_headers import (
...
def test_non_allowlisted_peer_ignores_forwarded_and_logs(
    ...
    with caplog.at_level(logging.WARNING, logger="cassandra.proxy.forwarded_headers"):
