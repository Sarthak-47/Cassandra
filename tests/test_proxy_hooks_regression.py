...
from cassandra.proxy.savings_tracker import CASSANDRA_SAVINGS_PATH_ENV_VAR
...
def test_anthropic_hooks_do_not_break_extract_user_query_lookup(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv(
        CASSANDRA_SAVINGS_PATH_ENV_VAR,
    ...
    with TestClient(app) as client:
        ...
        # `x-cassandra-tokens-after` reflects the proxy-side recount of the
        ...
        # the inflation guard.
        from cassandra.tokenizers.registry import get_tokenizer

        expected_after = get_tokenizer("claude-sonnet-4-6").count_messages(compressed_messages)
        assert response.headers["x-cassandra-tokens-after"] == str(expected_after)
        tokens_before = int(response.headers["x-cassandra-tokens-before"])
        tokens_after = int(response.headers["x-cassandra-tokens-after"])
        assert tokens_before > tokens_after, (
            f"compression should reduce tokens: before={tokens_before} after={tokens_after}"
        )
        assert int(response.headers["x-cassandra-tokens-saved"]) > 0
