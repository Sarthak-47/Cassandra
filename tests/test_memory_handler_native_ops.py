...
from cassandra.proxy import memory_handler as memory_handler_module
...
async def test_execute_memory_tool_save_and_background_dedup(
    ...
    monkeypatch.setattr("cassandra.proxy.memory_handler.asyncio.create_task", fake_create_task)
    backend.search_results = [
        make_result(
            "other",
...
async def test_init_backend_locked_local_and_bridge_import(
    ...
    monkeypatch.setitem(
        sys.modules,
        "cassandra.memory.backends.local",
        SimpleNamespace(
...
async def test_init_and_import_bridge_success_and_failure(
    ...
    monkeypatch.setitem(
        sys.modules,
        "cassandra.memory.bridge",
        SimpleNamespace(MemoryBridge=FakeMemoryBridge),
    )
    monkeypatch.setitem(
        sys.modules,
        "cassandra.memory.bridge_config",
        SimpleNamespace(
...
def test_memory_handler_init_defaults_and_tool_injection_edges(
    ...
    monkeypatch.setattr(
        "cassandra.paths.native_memory_dir",
        lambda: native_dir,
    )
...
async def test_ensure_initialized_fast_paths_and_qdrant_variants(
    ...
    monkeypatch.setitem(
        sys.modules,
        "cassandra.memory.backends.direct_mem0",
    ...
    monkeypatch.setitem(sys.modules, "cassandra.memory.backends.direct_mem0", None)
    import builtins

    real_import = builtins.__import__

    def fake_import(name, globals=None, locals=None, fromlist=(), level=0):
        if name == "cassandra.memory.backends.direct_mem0":
