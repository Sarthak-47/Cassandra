...
from cassandra.release_version import (
    CommitInfo,
...
def test_list_release_commits_parses_empty_body_entries(
    ...
    monkeypatch.setattr("cassandra.release_version.run", run)
...
def test_release_version_script_runs_directly_without_importing_cassandra_package(
    tmp_path: Path,
) -> None:
    ...
    result = subprocess.run(
        [sys.executable, str(ROOT / "cassandra" / "release_version.py")],
        cwd=ROOT,
        capture_output=True,
        text=True,
        env=env,
