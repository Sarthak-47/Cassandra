"""Tests for :class:`cassandra.mcp_registry.opencode.OpencodeRegistrar`."""
...
from cassandra.mcp_registry.opencode import (
    OpencodeRegistrar,
    _diff_specs,
    _entry_to_spec,
    _spec_to_entry,
...
def test_get_server_returns_none_when_absent(tmp_path: Path) -> None:
    ...
    registrar = _registrar(tmp_path)
    assert registrar.get_server("cassandra") is None
...
def test_get_server_returns_spec_when_present(tmp_path: Path) -> None:
    ...
    registrar = _registrar(tmp_path)
    spec = registrar.get_server("cassandra")
    assert spec is not None
    assert spec.name == "cassandra"
    assert spec.command == "cassandra"
...
def test_register_server_creates_config_when_missing(tmp_path: Path) -> None:
    ...
    spec = ServerSpec(name="cassandra", command="cassandra", args=("mcp", "serve"))
    result = registrar.register_server(spec)
    assert result.status == RegisterStatus.REGISTERED
    config_path = tmp_path / "opencode.json"
    assert config_path.exists()
    data = json.loads(config_path.read_text())
    assert data["mcp"]["cassandra"] == {
        "type": "local",
        "command": ["cassandra", "mcp", "serve"],
        "enabled": True,
    }
...
def test_unregister_server_removes_entry(tmp_path: Path) -> None:
    ...
    spec = ServerSpec(name="cassandra", command="cassandra", args=("mcp", "serve"))
    registrar.register_server(spec)
    assert registrar.unregister_server("cassandra") is True
    assert registrar.get_server("cassandra") is None
...
def test_unregister_server_returns_false_when_absent(tmp_path: Path) -> None:
    ...
    registrar = _registrar(tmp_path)
    assert registrar.unregister_server("cassandra") is False


def test_specs_equivalent_true() -> None:
    from cassandra.mcp_registry.base import ServerSpec

    a = ServerSpec(name="cassandra", command="cassandra", args=("mcp", "serve"))
    b = ServerSpec(name="cassandra", command="cassandra", args=("mcp", "serve"))
...
def test_specs_equivalent_false() -> None:
    ...
    a = ServerSpec(name="cassandra", command="cassandra", args=("mcp", "serve"))
    b = ServerSpec(name="cassandra", command="other", args=("mcp", "serve"))
...
def test_register_server_force_overwrites_mismatch(tmp_path: Path) -> None:
    ...
    spec_a = ServerSpec(name="cassandra", command="old", args=("serve",))
    spec_b = ServerSpec(name="cassandra", command="cassandra", args=("mcp", "serve"))
    ...
    updated = registrar.get_server("cassandra")
    assert updated is not None
    assert updated.command == "cassandra"
...
def test_unregister_preserves_other_mcp_servers(tmp_path: Path) -> None:
    ...
    spec = ServerSpec(name="cassandra", command="cassandra", args=("mcp", "serve"))
    registrar.register_server(spec)
    registrar.unregister_server("cassandra")

    data = json.loads((tmp_path / "opencode.json").read_text())
    assert "cassandra" not in data["mcp"]
    assert "existing-server" in data["mcp"]
...
def test_register_server_with_env_vars(tmp_path: Path) -> None:
    ...
    spec = ServerSpec(
        ...
        command="cassandra",
        args=("mcp", "serve"),
        env={"CASSANDRA_PROXY_URL": "http://127.0.0.1:9090"},
    )
    registrar.register_server(spec)
...
def test_register_then_re_register_with_different_env_returns_mismatch(tmp_path: Path) -> None:
    ...
    spec_a = ServerSpec(name="cassandra", command="cassandra", args=("mcp", "serve"))
    spec_b = ServerSpec(
        name="cassandra",
        command="cassandra",
        args=("mcp", "serve"),
...
def test_entry_to_spec_reads_opencode_environment() -> None:
    ...
    entry = {
        ...
        "command": ["cassandra", "mcp", "serve"],
        "enabled": True,
        "environment": {"CASSANDRA_PROXY_URL": "http://127.0.0.1:9090"},
    }
    spec = _entry_to_spec("cassandra", entry)
    assert spec.env == {"CASSANDRA_PROXY_URL": "http://127.0.0.1:9090"}
...
def test_unregister_server_handles_oserror(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("cassandra.mcp_registry.opencode._write_json", _fail_write)
    ok = r.unregister_server("bad-unregister")
    assert ok is False


def test_get_all_registrars_includes_opencode() -> None:
    from cassandra.mcp_registry.install import get_all_registrars
