...
def _isolate_tracker(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv("CASSANDRA_TOOL_INJECTION_STICKY", raising=False)
    monkeypatch.delenv("CASSANDRA_TOOL_TRACKER_MAX_SESSIONS", raising=False)
    _reset_session_tool_tracker_for_test()
    yield
    _reset_session_tool_tracker_for_test()
...
def test_max_sessions_env_var_custom(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("CASSANDRA_TOOL_TRACKER_MAX_SESSIONS", "42")
    assert get_tool_tracker_max_sessions() == 42


def test_max_sessions_env_var_invalid_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("CASSANDRA_TOOL_TRACKER_MAX_SESSIONS", "0")
    with pytest.raises(ValueError):
        get_tool_tracker_max_sessions()
    monkeypatch.setenv("CASSANDRA_TOOL_TRACKER_MAX_SESSIONS", "-3")
    with pytest.raises(ValueError):
        get_tool_tracker_max_sessions()
    monkeypatch.setenv("CASSANDRA_TOOL_TRACKER_MAX_SESSIONS", "not-int")
    with pytest.raises(ValueError):
        get_tool_tracker_max_sessions()
...
def test_sticky_mode_disabled(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("CASSANDRA_TOOL_INJECTION_STICKY", "disabled")
    assert get_tool_injection_sticky_mode() == "disabled"


def test_sticky_mode_invalid_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("CASSANDRA_TOOL_INJECTION_STICKY", "yolo")
    with pytest.raises(ValueError, match="CASSANDRA_TOOL_INJECTION_STICKY"):
        get_tool_injection_sticky_mode()
...
def test_disabled_mode_passes_through_per_turn_decision(
    ...
    """`CASSANDRA_TOOL_INJECTION_STICKY=disabled` reverts to per-turn behavior.
