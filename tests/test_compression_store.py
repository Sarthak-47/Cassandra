...
@contextmanager
def _capture_cassandra_retrieve_events():
    events: list[dict[str, Any]] = []
    prefix = "event=cassandra_retrieve "
    ...
    logger = logging.getLogger("cassandra.cache.compression_store")
...
def test_retrieve_logs_payload_preview():
    ...
    with _capture_cassandra_retrieve_events() as events:
...
class TestCompressionStoreFeedback:
    ...
    @patch("cassandra.cache.compression_feedback.get_compression_feedback")
    @patch("cassandra.telemetry.get_telemetry_collector")
    @patch("cassandra.telemetry.toin.get_toin")
