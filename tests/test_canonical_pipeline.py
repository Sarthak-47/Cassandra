...
from cassandra.config import CassandraConfig, CassandraMode, TransformResult
from cassandra.hooks import CompressionHooks
from cassandra.pipeline import (
...
from cassandra.transforms import ContentRouter, TransformPipeline
from cassandra.transforms.content_router import CompressionStrategy
...
def test_pipeline_extension_manager_replaces_events_and_ignores_failures(caplog):
    ...
    with caplog.at_level("WARNING", logger="cassandra.pipeline"):
        event = manager.emit(
...
def test_discover_pipeline_extensions_handles_load_and_init_failures(monkeypatch):
    pipeline_module = importlib.import_module("cassandra.pipeline")
...
def test_cassandra_client_emits_canonical_pipeline_events(tmp_path):
    recorder = RecordingExtension()
    original = DummyOriginalClient()
    config = CassandraConfig(
        store_url=f"jsonl://{tmp_path / 'cassandra.jsonl'}",
    ...
    client = CassandraClient(
        ...
        store_url=f"jsonl://{tmp_path / 'cassandra-client.jsonl'}",
