//! PyO3 bindings for cassandra-core. Exposed to Python as `cassandra._core`.
//!
...
//! `cassandra.transforms._rust_diff_compressor.RustBackedDiffCompressor`,
...
/// Mirror of `cassandra.transforms.diff_compressor.DiffCompressorConfig`.
...
/// Mirror of `cassandra.transforms.diff_compressor.DiffCompressionResult`.
...
#[pyclass(name = "DiffCompressionResult", module = "cassandra._core")]
...
#[pyclass(name = "DiffCompressorStats", module = "cassandra._core")]
...
/// Mirror of `cassandra.transforms.diff_compressor.DiffCompressor`. The
/// Python adapter wraps this in `RustBackedDiffCompressor` so
/// `ContentRouter` can swap backends transparently.
#[pyclass(name = "DiffCompressor", module = "cassandra._core")]
...
/// Mirror of `cassandra.transforms.smart_crusher.SmartCrusherConfig`.
...
#[pyclass(name = "SmartCrusherConfig", module = "cassandra._core", from_py_object)]
#[derive(Clone)]
...
/// Mirror of `cassandra.transforms.smart_crusher.CrushResult`. Read-only;
...
#[pyclass(name = "CrushResult", module = "cassandra._core")]
...
/// Mirror of `cassandra.transforms.smart_crusher.SmartCrusher`.
///
...
#[pyclass(name = "SmartCrusher", module = "cassandra._core")]
    ...
    fn with_compaction_format(
        ...
        match RustSmartCrusher::with_compaction_format(cfg, format_name) {
            ...
            None => Err(pyo3::exceptions::PyValueError::new_err(format!(
                ...
                cassandra_core::transforms::smart_crusher::compaction::CompactionStage::SUPPORTED_FORMAT_NAMES.join(", ")
            ))),
        }
    }
...
/// Mirror of `cassandra.transforms.content_detector.DetectionResult`.
///
...
#[pyclass(name = "DetectionResult", module = "cassandra._core", from_py_object)]
#[derive(Clone)]
...
/// `cassandra/transforms/content_router.py`).
///
...
/// Score a line against the default Cassandra keyword detector.
///
...
//
// Mirrors `cassandra.transforms.search_compressor.SearchCompressor` so the
...
#[pyclass(name = "SearchCompressionResult", module = "cassandra._core")]
...
#[pyclass(name = "SearchCompressor", module = "cassandra._core")]
...
impl PySearchCompressor {
    ...
    fn compress(
        ...
        let (result, stats) = py.detach(move || {
            let store = cassandra_core::ccr::InMemoryCcrStore::new();
            let (r, s) = self
                .inner
...
//
// Mirrors `cassandra.transforms.log_compressor.LogCompressor`. Same CCR
...
#[pyclass(name = "LogCompressionResult", module = "cassandra._core")]
...
#[pyclass(name = "LogCompressor", module = "cassandra._core")]
...
//
// Mirrors `cassandra.transforms.tag_protector.{protect_tags,restore_tags,
...
/// `crates/cassandra-proxy` binary would sit in front of Python; that
/// binary is not deployed by the CLI (`cassandra proxy`,
/// `cassandra wrap codex`). This binding lets the Python proxy call
...
///   to `cassandra_core::transforms::live_zone::DEFAULT_MODEL`.
///
/// # Returns
/// `(body, modified)`:
...
fn compress_openai_responses_live_zone(
    ...
    let model_str = if model.is_empty() {
        cassandra_core::transforms::live_zone::DEFAULT_MODEL
    } else {
        model
    };
...
#[pyclass(name = "TextCrusherConfig", module = "cassandra._core", from_py_object)]
#[derive(Clone)]
...
#[pyclass(name = "TextCrusherResult", module = "cassandra._core")]
...
#[pyclass(name = "TextCrusher", module = "cassandra._core")]
...
fn _core(m: &Bound<'_, PyModule>) -> PyResult<()> {
    // Bridge Rust diagnostics into Python's `logging`. cassandra-core emits
    ...
    // `cassandra_core.transforms.magika_detector`), which is what lands in
