...
///    `cassandra_retrieve(hash="HASH")` to recover the original bytes.
///
...
///    `cassandra/ccr/tool_injection.py` keeps working — it matches
...
/// `cassandra-core` is independent of the proxy crate.
...
pub fn compress_openai_responses_live_zone(
    ...
    let mut cassandra_retrieve_call_ids: HashSet<&str> = HashSet::new();
    for item in items {
        if item.get("type").and_then(Value::as_str) != Some("function_call") {
            continue;
        }
        let name = item.get("name").and_then(Value::as_str).unwrap_or("");
        if name == "cassandra_retrieve" || name.ends_with("__cassandra_retrieve") {
            if let Some(call_id) = item.get("call_id").and_then(Value::as_str) {
                cassandra_retrieve_call_ids.insert(call_id);
            }
        }
    }
    ...
    for (idx, item) in items.iter().enumerate() {
        ...
        match type_tag {
            "function_call_output" | "local_shell_call_output" | "apply_patch_call_output" => {
                ...
                if call_id.is_some_and(|id| cassandra_retrieve_call_ids.contains(id)) {
                    continue;
                }
...
mod openai_responses_tests {
    ...
    #[test]
    fn cassandra_retrieve_output_not_in_live_zone() {
