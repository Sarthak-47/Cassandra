...
//! `<tool_call>`, `<thinking>`, `<cassandra:tool_digest>`, etc.) that
...
//!   `{{CASSANDRA_TAG_…}}` substring, Python silently let the collision
...
const DEFAULT_PREFIX: &str = "{{CASSANDRA_TAG_";
...
pub struct ProtectStats {
    ...
    /// input contained a literal `{{CASSANDRA_TAG_` substring.
...
/// `text`. We try `{{CASSANDRA_TAG_` first; if the input contains it
...
fn pick_placeholder_prefix(text: &str) -> (String, bool) {
    ...
    for salt in 0u32..16 {
        let candidate = format!("{{{{CASSANDRA_TAG_{salt}_");
    ...
    let prefix = FALLBACK
        .get_or_init(|| "{{CASSANDRA_TAG_FALLBACK_a4f1c7e2_".to_string())
        .clone();
    (prefix, true)
}
...
fn tag_lost_error(original: &str, compressed_length: usize, request_id: Option<&str>) {
    ...
    match request_id {
        Some(rid) => tracing::error!(
            target: "cassandra::tag_protector",
...
mod tests {
    ...
    fn restore_lost_placeholder_idempotent_when_all_missing() {
        ...
        // byte-for-byte unchanged.
        let blocks = vec![
            ("{{CASSANDRA_TAG_0}}".to_string(), "<a>1</a>".to_string()),
            ("{{CASSANDRA_TAG_1}}".to_string(), "<b>2</b>".to_string()),
            ("{{CASSANDRA_TAG_2}}".to_string(), "<c>3</c>".to_string()),
        ];
    ...
    fn restore_partial_loss_keeps_present_drops_lost() {
        ...
        let blocks = vec![
            ("{{CASSANDRA_TAG_0}}".to_string(), "<a>1</a>".to_string()),
            (
                "{{CASSANDRA_TAG_1}}".to_string(),
                "<lost>x</lost>".to_string(),
            ),
        ];
        let compressed = "head {{CASSANDRA_TAG_0}} tail";
        let restored = restore_tags(compressed, &blocks);
    ...
    fn fixed_in_3e4_placeholder_collision_is_avoided() {
        // Bug #5: input contains literal `{{CASSANDRA_TAG_…}}`. The
        // walker should pick a salted prefix and report the collision
        // in stats.
        let text = "User wrote {{CASSANDRA_TAG_0}} on purpose. \
        ...
        assert_ne!(blocks[0].0, "{{CASSANDRA_TAG_0}}");
    }
