...
//! and maps its 200+ labels onto Cassandra's existing
...
///
/// The real fix is `cassandra/_ort.py`, which pins `ORT_DYLIB_PATH` to the
...
/// Override with `CASSANDRA_MAGIKA_INIT_TIMEOUT_SECS`.
const MAGIKA_INIT_TIMEOUT_SECS_DEFAULT: u64 = 5;

fn magika_init_timeout() -> Duration {
    let secs = std::env::var("CASSANDRA_MAGIKA_INIT_TIMEOUT_SECS")
        .ok()
...
/// Classify `content` and return the mapped Cassandra [`ContentType`].
///
...
/// Map a magika label string to Cassandra's [`ContentType`] enum.
///
