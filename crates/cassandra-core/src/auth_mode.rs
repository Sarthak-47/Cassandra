...
//!   `User-Agent`, never inject `X-Cassandra-*`, never strip
//!   `accept-encoding`).
//!
...
pub enum AuthMode {
    ...
    /// `X-Cassandra-*`; never mutate `User-Agent`.
    Subscription,
}
...
/// `crates/cassandra-core/benches/auth_mode.rs` asserts <10us / call.
...
mod inline_tests {
    ...
    //! cassandra-core --lib` exercises the helper without pulling in
    //! the integration-test binary. The exhaustive test matrix lives
    //! in `crates/cassandra-core/tests/auth_mode.rs`.
