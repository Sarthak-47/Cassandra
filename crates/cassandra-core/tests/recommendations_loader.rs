...
//!    Python `cassandra.cli.toin_publish` CLI emits.
...
use cassandra_core::transforms::recommendations::{
    AuthMode, RecommendationStore, RecommendationsError,
};

/// Minimal tempdir helper. Project convention is to avoid a
/// dev-dependency on `tempfile` (see `crates/cassandra-parity` for the
...
fn tempdir() -> TempDir {
    ...
    let p = std::env::temp_dir().join(format!(
        "cassandra-recommendations-{nanos}-{:?}",
