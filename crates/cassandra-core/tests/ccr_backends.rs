...
use cassandra_core::ccr::backends::{
    from_config, CcrBackendConfig, InMemoryCcrStore, SqliteCcrStore,
};
use cassandra_core::ccr::{compute_key, CcrStore};
...
fn from_config_redis_unsupported_when_feature_off() {
    use cassandra_core::ccr::backends::CcrBackendInitError;
...
mod redis_tests {
    use super::*;
    use cassandra_core::ccr::backends::RedisCcrStore;

    /// Reads `CASSANDRA_TEST_REDIS_URL` from the environment — when the
    /// feature is on but no URL is configured we silently no-op. CI
    /// runs the redis test in a docker-compose'd matrix.
    fn redis_url() -> Option<String> {
        std::env::var("CASSANDRA_TEST_REDIS_URL").ok()
    }

    #[test]
    fn redis_round_trip() {
        let Some(url) = redis_url() else {
            eprintln!("skipping redis_round_trip: CASSANDRA_TEST_REDIS_URL not set");
            return;
        };
