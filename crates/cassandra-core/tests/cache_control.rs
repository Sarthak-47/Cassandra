...
//!
//! These tests exercise [`cassandra_core::compute_frozen_count`] in
//! isolation — no proxy, no upstream, no I/O. Integration tests that
//! drive the walker through the proxy live in
//! `crates/cassandra-proxy/tests/integration_cache_control.rs`.

use cassandra_core::compute_frozen_count;
