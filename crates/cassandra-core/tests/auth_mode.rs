//! Integration tests for `cassandra_core::auth_mode::classify`.
//!
...
use cassandra_core::auth_mode::{classify, AuthMode};
...
fn subscription_takes_precedence_over_oauth_token() {
    ...
    // request count, never identify Cassandra). UA wins.
    let h = headers(&[
...
/// `crates/cassandra-core/benches/auth_mode.rs`. This in-test loop
...
fn classify_under_10us_per_call() {
    ...
    // 10us = 10_000 ns. Asserting 10x cassandra guards against perf
