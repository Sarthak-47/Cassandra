...
use cassandra_core::auth_mode::{classify as classify_auth_mode, AuthMode};
use cassandra_core::compression_policy::CompressionPolicy;
...
pub(crate) async fn forward_http(
    ...
    if strip_internal && pre_strip_internal_count > 0 {
        tracing::info!(
            ...
            request_id = %request_id,
            "stripped internal x-cassandra-* headers from upstream-bound request"
        );
        ...
        tracing::warn!(
            ...
            request_id = %request_id,
            "CASSANDRA_PROXY_STRIP_INTERNAL_HEADERS=disabled; \
             internal x-cassandra-* headers forwarded to upstream"
        );
    }
    ...
    // `cassandra-request-id` header so callers can correlate proxy
    ...
    let mut response = Response::builder().status(status);
    {
        ...
        if let Some(uid) = upstream_request_id.as_deref() {
            if let Ok(v) = http::HeaderValue::from_str(uid) {
                h.insert(HeaderName::from_static("cassandra-upstream-request-id"), v);
            }
        }
    }
