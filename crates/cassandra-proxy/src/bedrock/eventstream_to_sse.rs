...
//! CRC-checksummed framing format. Most clients of Cassandra (Claude
//! Code, the Anthropic SDK in non-Bedrock mode, the Cassandra Python
    ...
    match v {
        HeaderValue::String(s) => {
            if s.len() <= 64 {
                ...
                // was only stabilised in Rust 1.91; cassandra's MSRV is 1.80.)
                let mut end = 64;
