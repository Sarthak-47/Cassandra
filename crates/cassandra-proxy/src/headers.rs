...
///
/// Response-side `X-Cassandra-*` injection (e.g. `x-cassandra-tokens-saved`)
...
pub const INTERNAL_HEADER_PREFIX: &str = "x-cassandra-";
...
/// Returns true when `name` matches the internal `x-cassandra-*` prefix
...
///   - When `strip_internal == true`, `x-cassandra-*` headers stripped
...
mod tests {
    ...
    fn internal_header_detected_case_insensitive() {
        ...
        assert!(is_internal_header(
            &HeaderName::from_bytes(b"X-Cassandra-Mode").unwrap()
        ));
        assert!(is_internal_header(
            &HeaderName::from_bytes(b"X-CASSANDRA-FOO").unwrap()
        ));
        assert!(!is_internal_header(&HeaderName::from_static(
            "x-request-id"
        )));
    ...
    fn strip_internal_headers_removes_only_internal_prefix() {
        ...
        h.insert("x-cassandra-bypass", HeaderValue::from_static("true"));
        h.insert("x-cassandra-mode", HeaderValue::from_static("passthrough"));
        h.insert("x-request-id", HeaderValue::from_static("req-1"));
        let removed = strip_internal_headers(&mut h);
        assert_eq!(removed, 2);
        assert!(h.get("x-cassandra-bypass").is_none());
        assert!(h.get("x-cassandra-mode").is_none());
    ...
    fn build_forward_strips_internal_when_enabled() {
        ...
        incoming.insert("x-cassandra-bypass", HeaderValue::from_static("true"));
