...
/// Policy for stripping internal `x-cassandra-*` headers from upstream-bound
/// requests (PR-A5, fixes P5-49).
///
/// When `enabled` (default), every header whose name starts with
/// `x-cassandra-` is dropped before the upstream call. Stops fingerprinting
/// of the proxy via subscription-revocation flags (`x-cassandra-bypass`,
/// `x-cassandra-mode`, etc.) and prevents leakage of internal user-id /
...
///
/// Source priority: CLI flag → `CASSANDRA_PROXY_STRIP_INTERNAL_HEADERS`
/// env var → default (`enabled`).
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
#[clap(rename_all = "snake_case")]
pub enum StripInternalHeaders {
    /// Strip every `x-cassandra-*` header from upstream-bound requests.
    /// Default. Operationally safe.
    Enabled,
    /// Forward `x-cassandra-*` to upstream verbatim. Diagnostic-only;
...
///
/// Source priority: CLI flag → `CASSANDRA_PROXY_CACHE_CONTROL_AUTO_FROZEN`
/// env var → default (`enabled`).
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
#[clap(rename_all = "snake_case")]
...
/// `CASSANDRA_PROXY_AUTH_MODE_POLICY_ENFORCEMENT` env var →
/// default (`disabled`).
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
#[clap(rename_all = "snake_case")]
...
#[command(
    ...
    version,
    about = "Cassandra transparent reverse proxy"
)]
pub struct CliArgs {
    /// Address the proxy listens on (e.g. 0.0.0.0:8787).
    #[arg(long, env = "CASSANDRA_PROXY_LISTEN", default_value = "0.0.0.0:8787")]
    ...
    #[arg(long, env = "CASSANDRA_PROXY_UPSTREAM")]
    pub upstream: Url,
    ...
    /// Enable Cassandra compression on LLM-shaped requests
    ...
    /// `--compression` (or set `CASSANDRA_PROXY_COMPRESSION=1`).
    #[arg(
        long = "compression",
        env = "CASSANDRA_PROXY_COMPRESSION",
    ...
    ///
    /// Source priority: CLI flag → `CASSANDRA_PROXY_COMPRESSION_MODE`
    /// env var → default (`off`).
    #[arg(
        long = "compression-mode",
        env = "CASSANDRA_PROXY_COMPRESSION_MODE",
        value_enum,
    ...
    #[arg(
        ...
        env = "CASSANDRA_PROXY_CACHE_CONTROL_AUTO_FROZEN",
        value_enum,
    ...
    #[arg(
        ...
        env = "CASSANDRA_PROXY_STRIP_INTERNAL_HEADERS",
        value_enum,
    ...
    ///
    /// Source priority: CLI flag → `CASSANDRA_PROXY_ENABLE_RESPONSES_STREAMING`
    /// env var → default (`true`).
    #[arg(
        long = "enable-responses-streaming",
        env = "CASSANDRA_PROXY_ENABLE_RESPONSES_STREAMING",
    ...
    ///
    /// Source priority: CLI flag → `CASSANDRA_PROXY_ENABLE_CONVERSATIONS_PASSTHROUGH`
    /// env var → default (`true`).
    #[arg(
        long = "enable-conversations-passthrough",
        env = "CASSANDRA_PROXY_ENABLE_CONVERSATIONS_PASSTHROUGH",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
    ...
    ///
    /// Source priority: CLI flag → `CASSANDRA_PROXY_ENABLE_BEDROCK_NATIVE`
    /// env var → default (`true`).
    #[arg(
        long = "enable-bedrock-native",
        env = "CASSANDRA_PROXY_ENABLE_BEDROCK_NATIVE",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
    ...
    ///
    /// Source priority: CLI flag → `CASSANDRA_PROXY_BEDROCK_REGION`
    /// env var → `AWS_REGION` env var → default (`us-east-1`).
    #[arg(
        long = "bedrock-region",
        env = "CASSANDRA_PROXY_BEDROCK_REGION",
    ...
    ///
    /// Source priority: CLI flag → `CASSANDRA_PROXY_BEDROCK_ENDPOINT`
    /// env var → derived-from-region.
    #[arg(long = "bedrock-endpoint", env = "CASSANDRA_PROXY_BEDROCK_ENDPOINT")]
    ...
    ///
    /// Source priority: CLI flag → `CASSANDRA_PROXY_AWS_PROFILE`
    /// env var → `AWS_PROFILE` env var → default chain.
    #[arg(long = "aws-profile", env = "CASSANDRA_PROXY_AWS_PROFILE")]
    ...
    ///
    /// Source priority: CLI flag → `CASSANDRA_PROXY_BEDROCK_VALIDATE_EVENTSTREAM_CRC`
    /// env var → default (`true`).
    #[arg(
        long = "bedrock-validate-eventstream-crc",
        env = "CASSANDRA_PROXY_BEDROCK_VALIDATE_EVENTSTREAM_CRC",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
    ...
    ///
    /// Source priority: CLI flag → `CASSANDRA_PROXY_VERTEX_REGION`
    /// env var → default (`us-central1`).
    #[arg(
        long = "vertex-region",
        env = "CASSANDRA_PROXY_VERTEX_REGION",
    ...
    ///
    /// Source priority: CLI flag → `CASSANDRA_PROXY_VERTEX_ADC_SCOPE`
    /// env var → default (`cloud-platform`).
    #[arg(
        long = "vertex-adc-scope",
        env = "CASSANDRA_PROXY_VERTEX_ADC_SCOPE",
...
pub struct Config {
    ...
    pub auth_mode_policy_enforcement: AuthModePolicyEnforcement,
    /// Whether to strip internal `x-cassandra-*` headers from
...
impl Config {
    ...
    pub fn for_test(upstream: Url) -> Self {
        Self {
            ...
            // Production default: strip internal `x-cassandra-*` headers
