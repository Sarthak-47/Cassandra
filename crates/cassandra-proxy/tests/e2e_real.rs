//! Real end-to-end tests: Rust proxy → Python Cassandra proxy → real LLM API.
//!
//! These spawn the actual Python proxy as a subprocess and route real requests
//! to Anthropic / OpenAI through the full chain. Skipped unless CASSANDRA_E2E=1
//! to keep `cargo test` fast and free.
//!
//! Run with:
//!     CASSANDRA_E2E=1 cargo test -p cassandra-proxy --test e2e_real -- --nocapture
//!
...
const E2E_GUARD: &str = "CASSANDRA_E2E";
...
impl PythonProxy {
    /// Spawn `cassandra proxy --port <ephemeral> --no-optimize` in passthrough
    ...
    async fn spawn() -> Self {
        ...
        let root = repo_root();
        let venv_python = root.join(".venv/bin/cassandra");
        assert!(
            venv_python.exists(),
...
async fn e2e_health_through_full_chain() {
    ...
    assert_eq!(json["service"], "cassandra-proxy");
