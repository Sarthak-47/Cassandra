...
//! (PR-A4).
//!
//! The core walker `cassandra_core::compute_frozen_count` is unit-
//! tested in `crates/cassandra-core/tests/cache_control.rs`. This file
//! exercises the proxy wrapper [`resolve_frozen_count`] which adds
//! the configurability gate (`CASSANDRA_PROXY_CACHE_CONTROL_AUTO_FROZEN`)
//! and the structured-log emission tested via in-memory tracing
//! capture.

use cassandra_proxy::compression::resolve_frozen_count;
use cassandra_proxy::config::CacheControlAutoFrozen;
use serde_json::json;
...
fn cache_control_disabled_via_config_returns_zero_even_with_markers() {
    ...
    // `CASSANDRA_PROXY_CACHE_CONTROL_AUTO_FROZEN=disabled`.
    let body = json!({
        "messages": [
            {"role": "user", "content": [
                {"type": "text", "text": "m0", "cache_control": {"type": "ephemeral"}},
