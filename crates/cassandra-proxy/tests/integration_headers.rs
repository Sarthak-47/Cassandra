...
//! internal `x-cassandra-*` strip (PR-A5, fixes P5-49).
...
#[tokio::test]
async fn x_cassandra_request_headers_stripped() {
    let upstream = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/v1/messages"))
        .respond_with(move |req: &wiremock::Request| {
            // PR-A5: internal x-cassandra-* must NOT reach upstream.
            assert!(
                req.headers.get("x-cassandra-bypass").is_none(),
                "x-cassandra-bypass leaked upstream"
            );
            assert!(
                req.headers.get("x-cassandra-mode").is_none(),
                "x-cassandra-mode leaked upstream"
            );
            assert!(
                req.headers.get("x-cassandra-user-id").is_none(),
                "x-cassandra-user-id leaked upstream"
            );
    ...
    let resp = reqwest::Client::new()
        ...
        .header("x-cassandra-bypass", "true")
        .header("x-cassandra-mode", "passthrough")
        .header("x-cassandra-user-id", "alice")
        .body("{}")
        .send()
        .await
        .unwrap();
...
#[tokio::test]
async fn x_cassandra_case_insensitive_stripped() {
    let upstream = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/v1/messages"))
        .respond_with(move |req: &wiremock::Request| {
    ...
    let resp = reqwest::Client::new()
        .post(format!("{}/v1/messages", proxy.url()))
        .header("X-Cassandra-Foo", "1")
        .header("x-Cassandra-Bar", "2")
        .header("X-CASSANDRA-BAZ", "3")
        .body("{}")
        .send()
        .await
        .unwrap();
...
async fn legitimate_headers_passthrough_with_strip_enabled() {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(move |req: &wiremock::Request| {
            ...
            assert!(req.headers.get("x-cassandra-bypass").is_none());
            ResponseTemplate::new(200)
        })
        .mount(&upstream)
        .await;
...
async fn disabled_mode_passes_internal_headers_through() {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(move |req: &wiremock::Request| {
            ...
            assert_eq!(req.headers.get("x-cassandra-bypass").unwrap(), "true");
            assert_eq!(req.headers.get("x-cassandra-mode").unwrap(), "passthrough");
            ResponseTemplate::new(200).set_body_string("{}")
        })
        .mount(&upstream)
        .await;
