...
Sources: 10 parallel deep-audit subagents (Rust proxy passthrough; Rust compression correctness; Python proxy + bridges; prefix cache safety; streaming + wire-format; RTK + tests/parity; over-engineering; OpenAI long-tail + Bedrock; Cassandra-side injections; auth-mode handling).
...
- **File:** `cassandra/proxy/server.py:1050-1058`; `cassandra/proxy/handlers/openai.py:1212`
...
- **File:** `cassandra/proxy/server.py:1088, 1090`; `cassandra/proxy/handlers/streaming.py:651`; `cassandra/proxy/handlers/openai.py:2392-2397`; `cassandra/proxy/handlers/batch.py:344`
...
- **File:** `crates/cassandra-proxy/src/compression/anthropic.rs:151-156`
...
- **File:** `crates/cassandra-proxy/src/compression/anthropic.rs:146-157`; `crates/cassandra-core/src/context/strategy/drop_by_score.rs:64-80`; `crates/cassandra-core/src/context/manager.rs`; `cassandra/transforms/intelligent_context.py:354-450`
...
- **File:** `crates/cassandra-proxy/src/compression/anthropic.rs:91, 172`
...
- **File:** `cassandra/proxy/memory_handler.py:389-398`; `cassandra/proxy/handlers/anthropic.py:1147-1171`
...
- **File:** `cassandra/proxy/responses_converter.py:94, 221-256`
...
- **File:** `cassandra/proxy/handlers/streaming.py:58, 772`; `cassandra/ccr/response_handler.py:672`
...
- **File:** `cassandra/proxy/handlers/streaming.py:213-298`
...
- **File:** `cassandra/proxy/handlers/streaming.py:300-391` (`_response_to_sse`)
...
- **File:** `cassandra/backends/litellm.py:860`
...
- **File:** `cassandra/proxy/handlers/openai.py:2422-2447`
...
- **File:** `crates/cassandra-proxy/src/compression/anthropic.rs:90-188`
...
- **File:** `cassandra/proxy/handlers/streaming.py:160-211, 213-298`
- **Evidence:** No `event_type == "error"` arm. The wire passthrough is byte-faithful (good) but Cassandra's bookkeeping (`stream_state.input_tokens` etc.) silently doesn't reflect the failure; `_finalize_stream_response` reports a clean PERF line for an errored stream.
- **Guide:** §1.7, §2.21.
- **Fix:** Add `error` handling to telemetry.
- **Phase C → PR-C1 (Rust SSE).**

### P1-15. Connection drop without `message_stop`/`[DONE]` not surfaced
- **File:** `cassandra/proxy/handlers/streaming.py:899` (`finally: await self._finalize_stream_response`)
...
- **File:** `cassandra/proxy/handlers/streaming.py:227, 700`
...
- **Files:** `cassandra/transforms/intelligent_context.py`; `crates/cassandra-core/src/context/manager.rs`; `crates/cassandra-proxy/src/compression/icm.rs`
- **Status:** Delete in Phase B (PR-B1).

### P2-19. `RollingWindow`, `ProgressiveSummarizer` (head-truncation strategies)
- **Files:** `cassandra/transforms/rolling_window.py` (395 LOC); `cassandra/transforms/progressive_summarizer.py` (508 LOC)
...
- **Files:** `crates/cassandra-core/src/scoring/{scorer,score,weights,traits,mod}.rs` (~1500 LOC); `crates/cassandra-core/src/relevance/{embedding,bm25,hybrid,base,mod}.rs` (~1600 LOC); `cassandra/transforms/scoring.py` (459 LOC)
...
### P2-21. `crates/cassandra-core/src/context/` — except `safety.rs`
- **Files:** `crates/cassandra-core/src/context/{config,workspace,candidate,ccr_drop,manager,strategy/}.rs` (~1500 LOC)
- **Status:** Delete in Phase B (PR-B1). `safety.rs` (tool-pair atomicity) is moved to `crates/cassandra-core/src/transforms/safety.rs` and kept.

### P2-22. `ToolCrusher` operates without `frozen_message_count`
- **File:** `cassandra/transforms/tool_crusher.py:106`
...
- **File:** `cassandra/transforms/cache_aligner.py:160-262`
...
- **File:** `cassandra/proxy/memory_handler.py:498-510`; `cassandra/proxy/handlers/openai.py:535-540`
...
- **File:** `cassandra/ccr/tool_injection.py:302-328`
...
- **File:** `crates/cassandra-core/src/context/manager.rs:172-185`; `crates/cassandra-proxy/src/proxy.rs:285`
...
- **File:** `cassandra/telemetry/toin.py:853-927`
...
- **File:** Missing entirely in `crates/cassandra-proxy/`
...
- **Evidence:** Compression acceptance is `bytes_saved > 0` (`crates/cassandra-core/src/transforms/pipeline/orchestrator.rs:158-165`); ICM aggregate-checks tokens (`anthropic.rs:162-168`) but per-block transforms don't.
...
- **File:** `cassandra/backends/litellm.py:486-628`
...
- **File:** Same — `cassandra/backends/litellm.py`
- **Phase D → PR-D4 builds native Vertex.**

### P4-39. No native Bedrock/Vertex paths in Rust
- **File:** `crates/cassandra-proxy/src/compression/mod.rs:50` only matches `/v1/messages`.
- **Phase D → PR-D1-D4.**

### P4-40. `/v1/conversations` blind spot (§4.14)
- **Evidence:** Zero references. Server-side prepended items invisible to Cassandra; tokenizer count over-reports.
- **Phase C → PR-C4.**
...
- **File:** `cassandra/backends/litellm.py:600`; `cassandra/learn/plugins/codex.py:283`
- **Guide:** §4.4.
...
- **File:** `cassandra/proxy/responses_converter.py:94, 235`
...
- **File:** `cassandra/proxy/request_logger.py` — no base64/image redaction
- **Guide:** §11.6.
...
- **File:** `cassandra/proxy/responses_converter.py:99` — "Unknown item type: preserve"
...
### P5-49. `X-Cassandra-*` request headers leak upstream
- **File:** `cassandra/proxy/handlers/anthropic.py:526` — `dict(request.headers.items())` captured unmodified, no strip step before `httpx.post(headers=headers)`.
- **Risk:** Subscription-revocation fingerprint.
- **Phase A → PR-A5.**

### P5-50. `anthropic-beta` mutated when memory enabled, not session-sticky
- **File:** `cassandra/proxy/handlers/anthropic.py:1162-1168`
- **Status:** Already covered by P0-6. **Phase A → PR-A6, A7.**

### P5-51. `OpenAI-Beta` auto-injection on WS path
- **File:** `cassandra/proxy/handlers/openai.py:1566-1567`
...
- **File:** `cassandra/telemetry/toin.py:477, 496`
...
- **File:** `crates/cassandra-proxy/src/proxy.rs:355-358, 377-383`
- **Guide:** §11.10.
...
- **Files:** `cassandra/cli/wrap.py` — only Claude/Codex/Aider/Copilot/Cursor today.
- **Phase G → PR-G1.**
...
- **File:** `crates/cassandra-parity/src/lib.rs:172-174`
...
- **File:** `cassandra/proxy/prometheus_metrics.py` — only aggregate by provider
- **Phase G → PR-G3.**
