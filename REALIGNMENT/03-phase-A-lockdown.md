...
**Worktree:** `~/claude-projects/cassandra-worktrees/realign-A1-icm-passthrough`
...
**Delete:**
- `crates/cassandra-proxy/src/compression/icm.rs`

**Modify:**
- `crates/cassandra-proxy/src/compression/mod.rs` — remove `pub mod icm;`, remove ICM dispatch in `maybe_compress`. The `is_compressible_path` check still matches `/v1/messages` but `compress_anthropic_request` becomes a no-op stub returning `Outcome::NoCompression`.
- `crates/cassandra-proxy/src/compression/anthropic.rs` — replace function body with `Ok(Outcome::NoCompression)`. Keep the function signature so callers compile; subsequent PRs in Phase B replace this with the live-zone block dispatcher.
- `crates/cassandra-proxy/src/proxy.rs` — confirm the `Outcome::NoCompression` branch forwards original bytes (already does at line 296-298; just verify with the new test).

**Tests added:**
- `crates/cassandra-proxy/tests/integration_compression.rs::compression_on_message_passes_body_unchanged_sha256` — record a real Anthropic request body to a fixture; send through proxy; assert SHA-256 of upstream-received body equals SHA-256 of inbound body.
...
- This PR does NOT delete ICM the module yet — `crates/cassandra-core/src/context/manager.rs` still compiles. PR-B1 deletes the modules. Splitting keeps the diff scoped.
...
**Worktree:** `~/claude-projects/cassandra-worktrees/realign-A2-system-prompt-immutable`
...
**Modify:**
- `cassandra/proxy/server.py:1026-1071` — delete `_inject_system_context`. Memory context handling routes exclusively through `_append_context_to_latest_non_frozen_user_turn` (already exists at `handlers/anthropic.py:1117-1135` for cache mode; promote to default).
- `cassandra/proxy/handlers/openai.py:1212` — same: delete `body["instructions"] = f"{existing_instructions}\n\n{memory_context}"`. Replace with append-to-latest-user-message-tail.
- `cassandra/transforms/cache_aligner.py` — delete the rewrite path (lines 160-262). Keep the volatile-content detector and the `cache_aligner_warnings` callback that surfaces detected dynamic content (UUIDs, dates, tokens) to a customer-visible log line.
- `cassandra/proxy/server.py:299` — `cache_aligner.enabled` flag stays default-False; document that turning it on now only affects warnings.
...
**Worktree:** `~/claude-projects/cassandra-worktrees/realign-A3-byte-faithful-forwarders`
...
**Modify:**
- `cassandra/proxy/server.py:1073-1124` — `_retry_request`: track whether body was mutated; if not, forward `original_body_bytes`; if yes, re-serialize with the canonical settings. Switch `await self.http_client.post(url, json=body)` to `await self.http_client.post(url, content=outbound_bytes, headers={**headers, "content-type": "application/json"})`.
- `cassandra/proxy/handlers/streaming.py:617-660` — same pattern in `_send_streaming_request`.
- `cassandra/proxy/handlers/openai.py:2392-2410` — WS→HTTP fallback: same pattern.
- `cassandra/proxy/handlers/batch.py:340-360` — batch endpoint: same pattern.
- `cassandra/proxy/helpers.py` — add `serialize_body_canonical(body: dict) -> bytes` helper using `json.dumps(body, separators=(",", ":"), ensure_ascii=False).encode("utf-8")`.
...
**Worktree:** `~/claude-projects/cassandra-worktrees/realign-A4-honor-cache-control`
...
- `crates/cassandra-proxy/src/compression/anthropic.rs` — add `pub fn compute_frozen_count(parsed: &serde_json::Value) -> usize` that walks `messages[*].content[*].cache_control`, `system[*].cache_control`, `tools[*].cache_control` and returns the highest message index whose content contains a marker. (Used by Phase B; currently called only by tests.)
- `crates/cassandra-core/src/lib.rs` — re-export `compute_frozen_count` for use in Phase B.

**Add:**
- `crates/cassandra-proxy/tests/integration_cache_control.rs::cache_control_marker_at_message_3_yields_frozen_count_3`
- `crates/cassandra-proxy/tests/integration_cache_control.rs::cache_control_in_system_blocks_yields_frozen_count_full_history`
- `crates/cassandra-proxy/tests/integration_cache_control.rs::cache_control_ttl_1h_before_5m_passes`
- `crates/cassandra-proxy/tests/integration_cache_control.rs::cache_control_ttl_5m_before_1h_warns_and_passes` (we don't reject, but log per §2.19 ordering rule)

### Acceptance criteria

- `cargo build -p cassandra-proxy` works with new features.
- `cargo test -p cassandra-proxy` green.
...
## PR-A5 — Strip `x-cassandra-*` from upstream-bound headers

**Branch:** `realign-A5-strip-cassandra-headers`
**Worktree:** `~/claude-projects/cassandra-worktrees/realign-A5-strip-cassandra-headers`
**Risk:** **LOW**
**LOC:** -10 / +50
...
**Modify:**
- `cassandra/proxy/handlers/anthropic.py:526` — wrap `dict(request.headers.items())` with `_strip_internal_headers(headers)` (new helper).
- `cassandra/proxy/handlers/openai.py:232-264` — same.
- `cassandra/proxy/handlers/streaming.py:617` — same.
- `cassandra/proxy/handlers/batch.py:340` — same.
- `cassandra/proxy/handlers/gemini.py:31` — same.
- `cassandra/proxy/helpers.py` — add `_strip_internal_headers` helper. Default strip list: `x-cassandra-*` (case-insensitive prefix), plus a hardcoded set of internal flags.
- `crates/cassandra-proxy/src/headers.rs` — add `strip_internal_headers` to the request-side filter. Document that response-side `X-Cassandra-*` injection (which is fine) is unrelated.

**Tests added:**
- `tests/test_header_isolation.py::test_x_cassandra_bypass_not_forwarded`
- `tests/test_header_isolation.py::test_x_cassandra_mode_not_forwarded`
- `tests/test_header_isolation.py::test_x_cassandra_user_id_not_forwarded`
- `crates/cassandra-proxy/tests/integration_headers.rs::x_cassandra_request_headers_stripped`

### Acceptance criteria

- New tests pass.
- Existing client-driven `x-cassandra-bypass: true` flow still works (proxy reads it; just doesn't forward).
- No legitimate header is stripped (whitelist `x-request-id`, `x-trace-id`, etc. by default — though they aren't `x-cassandra-*` so they're untouched).
...
**Worktree:** `~/claude-projects/cassandra-worktrees/realign-A6-anthropic-beta-stable`
...
**Modify:**
- `cassandra/proxy/handlers/anthropic.py:1162-1168` — replace the ad-hoc concat with a helper `merge_anthropic_beta(client: str, cassandra: list[str]) -> str` that splits client's value on `,`, lowercases each token, deduplicates, appends Cassandra-required tokens (sorted within the appended group), and rejoins.
- `cassandra/proxy/server.py` — extend `session_state` (already exists for memory) to track `betas_seen: set[str]` per session. Update on every request; merge into outbound `anthropic-beta` for follow-up requests.
- `cassandra/proxy/helpers.py` — add `betas_seen_lock` and `update_session_betas` helpers.
...
- New tests pass.
- Session ID is keyed off the existing session detection (per `cassandra/proxy/handlers/anthropic.py:1417`).
...
**Worktree:** `~/claude-projects/cassandra-worktrees/realign-A7-memory-tool-sticky`
**Risk:** **MEDIUM**
**LOC:** -30 / +150
...
**Modify:**
- `cassandra/proxy/memory_tool_adapter.py:625-657` — make injection session-state-aware. The session-state object grows a `memory_tools_injected: bool` and `memory_tools_definition_bytes: bytes` (golden form). On every request: if previously injected, inject again with byte-equal definition.
- `cassandra/proxy/memory_handler.py:389-398` — same: native-tool path becomes session-sticky.
- `cassandra/proxy/handlers/anthropic.py:1147-1171` — read session state; either inject all (if previously injected or memory enabled this turn) or none.
...
**Worktree:** `~/claude-projects/cassandra-worktrees/realign-A8-python-wire-hotfix`
**Risk:** **MEDIUM**
**LOC:** -100 / +400
...
**Modify:**
- `cassandra/proxy/handlers/streaming.py:213-298` — rewrite `_parse_sse_to_response` to handle all delta types per guide §5.1. Add index-keyed block map. Bytes-level SSE buffer.
- `cassandra/proxy/handlers/streaming.py:58, 772` — switch `chunk.decode("utf-8", errors="ignore")` to a bytes-buffer + decode-after-`\n\n` pattern.
- `cassandra/ccr/response_handler.py:665-686` — same pattern.
- `cassandra/proxy/responses_converter.py:94, 235` — preserve `phase` explicitly. Fix multi-text-part rebuild: rebuild parts by index, replacing each part's text in place.
- `cassandra/proxy/responses_converter.py:99` — add `logger.warning(f"unknown responses item type: {item.get('type')}")`.
- `crates/cassandra-proxy/src/proxy.rs:355-358` — capture upstream `request-id` (Anthropic) and `x-request-id` (OpenAI) into the tracing field.
- `crates/cassandra-proxy/src/proxy.rs:243-263` — return 413 on body-too-large; return 400 only on actual parse error.
...
- `crates/cassandra-proxy/tests/integration_request_id.rs::upstream_request_id_captured`
...
- ✅ `x-cassandra-*` stripped from upstream
