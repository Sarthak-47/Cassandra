...
**Worktree:** `~/claude-projects/cassandra-worktrees/realign-B1-delete-icm-and-deps`
...
**Delete (Python):**
- `cassandra/transforms/intelligent_context.py` (1077 LOC)
- `cassandra/transforms/rolling_window.py` (395 LOC)
- `cassandra/transforms/progressive_summarizer.py` (508 LOC)
- `cassandra/transforms/scoring.py` (459 LOC)
- `cassandra/transforms/tool_crusher.py` (338 LOC)

**Delete (Rust):**
- `crates/cassandra-core/src/context/manager.rs`
- `crates/cassandra-core/src/context/config.rs`
- `crates/cassandra-core/src/context/workspace.rs`
- `crates/cassandra-core/src/context/candidate.rs`
- `crates/cassandra-core/src/context/ccr_drop.rs`
- `crates/cassandra-core/src/context/strategy/mod.rs`
- `crates/cassandra-core/src/context/strategy/drop_by_score.rs`
- All of `crates/cassandra-core/src/scoring/*.rs` (~1500 LOC)
- All of `crates/cassandra-core/src/relevance/*.rs` (~1600 LOC)
- `crates/cassandra-core/.fastembed_cache/` directory and its `bge-small-en-v1.5` ONNX artifacts (~50 MB)

**Move:**
- `crates/cassandra-core/src/context/safety.rs` → `crates/cassandra-core/src/transforms/safety.rs`. Update all callers' `use` paths. The tool-pair atomicity logic is preserved verbatim (it's correct and live-zone code needs it).

**Modify:**
- `crates/cassandra-core/src/lib.rs` — remove `pub mod context;`, `pub mod scoring;`, `pub mod relevance;`. Add `pub use transforms::safety;`.
- `crates/cassandra-core/src/context/mod.rs` — delete (empty after move).
- `crates/cassandra-proxy/src/lib.rs` — no changes (already doesn't reach into deleted modules after PR-A1).
- `crates/cassandra-py/src/lib.rs` — remove any PyO3 exports of `MessageScorer`, `IntelligentContextManager`, etc. (per agent reports, MessageScorer was exposed in PR #338/#343).
- `cassandra/transforms/__init__.py` — remove imports of deleted modules.
- `cassandra/proxy/handlers/anthropic.py` — remove all imports / call sites of `IntelligentContextManager`. (Agent C found these at multiple locations; track via `grep -n IntelligentContextManager cassandra/`.)
- `cassandra/proxy/server.py` — remove ICM import and instantiation.
...
- `crates/cassandra-core/tests/scoring_*.rs`, `relevance_*.rs`, `context_*.rs` (other than safety)
...
- `git grep -i "IntelligentContextManager\|MessageScorer\|RollingWindow\|ProgressiveSummarizer\|ToolCrusher\|DropByScoreStrategy"` returns nothing in `crates/`, `cassandra/`, `tests/` except comments referencing the deletion.
...
- **`anchor_selector.py`** — Agent G suspected it might be ICM-only. Check: `git grep AnchorSelector cassandra/ crates/`. If only consumed by ICM/scoring/SmartCrusher, delete; if consumed by SmartCrusher's anchor logic, keep. Current Rust has `crates/cassandra-core/src/transforms/anchor_selector.rs` which is consumed by SmartCrusher — keep that one, delete the Python one if ICM was its only caller.
...
**Worktree:** `~/claude-projects/cassandra-worktrees/realign-B2-live-zone-dispatcher`
...
**Add:**
- `crates/cassandra-core/src/transforms/live_zone.rs` — the dispatcher. Public API:
  ```rust
...
**Modify:**
- `crates/cassandra-proxy/src/compression/mod.rs` — add `pub mod live_zone_anthropic;` and route `/v1/messages` to it (replacing the no-op stub from PR-A1).
- `crates/cassandra-proxy/src/compression/live_zone_anthropic.rs` (new) — calls `compress_live_zone` with `frozen_count = compute_frozen_count(parsed)` (from PR-A4).
- `crates/cassandra-proxy/src/compression/anthropic.rs` — delete (replaced by `live_zone_anthropic.rs`).

**Tests added:**
- `crates/cassandra-core/tests/live_zone_skeleton.rs::dispatches_only_to_latest_user_message`
- `crates/cassandra-core/tests/live_zone_skeleton.rs::respects_frozen_message_count`
- `crates/cassandra-core/tests/live_zone_skeleton.rs::no_change_when_no_block_mutated_returns_original`
- `crates/cassandra-core/tests/live_zone_skeleton.rs::modified_messages_byte_equal_outside_block`
- `crates/cassandra-core/tests/live_zone_skeleton.rs::system_and_tools_byte_equal_always`
- `crates/cassandra-proxy/tests/integration_live_zone.rs::end_to_end_live_zone_passthrough`

### Acceptance criteria

- `cargo test -p cassandra-core` green.
- `cargo test -p cassandra-proxy` green.
...
**Worktree:** `~/claude-projects/cassandra-worktrees/realign-B3-wire-type-aware-compressors`
...
**Modify:**
- `crates/cassandra-core/src/transforms/live_zone.rs` — replace no-op compressors with real dispatch:
  ```rust
  ...
  ```
- `crates/cassandra-core/src/transforms/content_detector.rs` — extend `ContentType` enum with the variants above. Use existing `Magika` + `unidiff-rs` + heuristic detectors.

**Tests added:**
- `crates/cassandra-core/tests/live_zone_dispatch.rs::json_tool_result_routes_to_smart_crusher`
- `crates/cassandra-core/tests/live_zone_dispatch.rs::log_tool_result_routes_to_log_compressor`
- `crates/cassandra-core/tests/live_zone_dispatch.rs::diff_tool_result_routes_to_diff_compressor`
- `crates/cassandra-core/tests/live_zone_dispatch.rs::source_code_tool_result_routes_to_code_compressor`
- `crates/cassandra-core/tests/live_zone_dispatch.rs::unknown_content_type_no_op`
...
**Worktree:** `~/claude-projects/cassandra-worktrees/realign-B4-token-validation-gate`
**Risk:** **LOW**
**LOC:** +250
...
**Modify:**
- `crates/cassandra-core/src/transforms/live_zone.rs` — wrap each compressor call with:
  ```rust
  ...
  ```
- `crates/cassandra-core/src/transforms/live_zone.rs::compress_block` — gate on byte threshold per content type:
  ```rust
...
**Tests added:**
- `crates/cassandra-core/tests/live_zone_thresholds.rs::below_threshold_no_compression_attempted`
- `crates/cassandra-core/tests/live_zone_thresholds.rs::above_threshold_compression_attempted`
- `crates/cassandra-core/tests/live_zone_token_validation.rs::compressed_more_tokens_falls_back`
- `crates/cassandra-core/tests/live_zone_token_validation.rs::compressed_fewer_tokens_accepted`
...
**Worktree:** `~/claude-projects/cassandra-worktrees/realign-B5-toin-observation-only`
...
**Modify:**
- `cassandra/telemetry/toin.py:853-927` — remove `get_recommendation()` and `CompressionHint`. Replace with a no-op stub that returns `None`; deprecation warning in docstring.
- `cassandra/telemetry/toin.py:103` — `Pattern` adds `auth_mode: str`, `model_family: str` fields.
- `cassandra/telemetry/toin.py:477, 496, 727, 729, 1248, 1256` — change aggregation key from `sig_hash` to `(auth_mode, model_family, sig_hash)` tuple. Update all dict-key uses.
- `cassandra/telemetry/toin.py:1596` — keep `tenant_prefix` for storage but document it's now redundant with the aggregation key.
- `cassandra/transforms/smart_crusher.py:446` — remove the `get_recommendation()` call site. SmartCrusher is now deterministic; TOIN observes outcomes only.
- New CLI: `cassandra/cli/toin_publish.py` — aggregates the on-disk TOIN store and produces `recommendations.toml`. Run as part of the deploy pipeline.
- New: `crates/cassandra-core/src/transforms/recommendations.rs` — loads `recommendations.toml` at startup. Provides API like `recommendations::get(auth_mode, model, structure_hash) -> Option<Recommendation>`. Used to bias which compressor variants to try first (deterministic; no per-request mutation).
...
**Worktree:** `~/claude-projects/cassandra-worktrees/realign-B6-memory-live-zone-tail`
...
**Modify:**
- `cassandra/proxy/memory_handler.py:498-510` — delete `_inject_to_system_or_instructions`. Replace with `_append_to_latest_user_tail`.
- `cassandra/proxy/handlers/openai.py:535-540` — same.
- `cassandra/proxy/handlers/anthropic.py:1117-1135` — promote the existing `_append_context_to_latest_non_frozen_user_turn` to be the default path.
- `cassandra/proxy/server.py:1050-1058` — already deleted in PR-A2; verify nothing reintroduces it.
- `cassandra/proxy/memory_handler.py` — add `MemoryMode` enum: `AutoTail | Tool`. Default `AutoTail`. `Tool` mode skips auto-injection entirely.
...
**Worktree:** `~/claude-projects/cassandra-worktrees/realign-B7-ccr-hardening`
**Risk:** **MEDIUM**
**LOC:** -150 / +600
...
**Add:**
- `crates/cassandra-core/src/ccr/backends/sqlite.rs` — SQLite-backed `CcrStore`. Schema: `ccr_entries(hash TEXT PRIMARY KEY, original BLOB, created_at INTEGER, ttl_seconds INTEGER)`. Auto-purge on read (`WHERE created_at + ttl_seconds > now`).
- `crates/cassandra-core/src/ccr/backends/redis.rs` — Redis-backed `CcrStore`. `SETEX hash ttl_seconds original`.
- `crates/cassandra-core/src/ccr/backends/mod.rs` — `pub trait CcrStore` (already exists at `ccr.rs`); `pub fn from_config(config: &CcrConfig) -> Box<dyn CcrStore>`.

**Modify:**
- `crates/cassandra-core/src/ccr.rs` — extract `InMemoryCcrStore` to its own file; rest stays.
- `crates/cassandra-core/src/transforms/live_zone.rs` — when a compressor returns a `CompressionResult` with original bytes, store original bytes in CCR backend keyed by `BLAKE3(original_bytes)`. Append `<<ccr:HASH>>` marker to compressed block content.
- `cassandra/proxy/handlers/anthropic.py` — `inject_ccr_retrieve_tool`: always add the tool when `session.has_done_ccr` is true; never toggle off.
- `cassandra/proxy/handlers/openai.py` — same for OpenAI Chat / Responses.
- `cassandra/ccr/tool_injection.py:302-328` — change `if has_compressed_content:` to `if session.has_done_ccr:`.
...
**Tests added:**
- `crates/cassandra-core/tests/ccr_backends.rs::sqlite_round_trip`
- `crates/cassandra-core/tests/ccr_backends.rs::sqlite_ttl_purge`
- `crates/cassandra-core/tests/ccr_backends.rs::redis_round_trip` (gated behind `cfg(feature = "redis")`)
- `crates/cassandra-core/tests/ccr_backends.rs::backend_swap_byte_equal_keys`
...
After Phase B, Cassandra's compression value is **back online** — and now it's correct.
