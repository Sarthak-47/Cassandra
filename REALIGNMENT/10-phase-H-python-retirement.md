...
**Worktree:** `~/claude-projects/cassandra-worktrees/realign-H1-retire-python-proxy-request-path`
**Risk:** **HIGH** (production-facing change; canary deploy mandatory)
**LOC:** **-15,000 / +500**

### Scope
Delete the Python FastAPI server, all handlers, the responses converter (already gone in Phase C PR-C5), memory subsystem (replaced by live-zone tail injection from Phase A PR-A2 and Phase B PR-B6), semantic cache, batch handler, etc. The Rust proxy becomes the canonical request-path implementation. Operators run only `cassandra-proxy` (Rust binary).

### Files

**Delete:**
- `cassandra/proxy/server.py` (2864 LOC)
- `cassandra/proxy/handlers/anthropic.py` (2423 LOC)
- `cassandra/proxy/handlers/openai.py` (2742 LOC)
- `cassandra/proxy/handlers/streaming.py` (1131 LOC)
- `cassandra/proxy/handlers/gemini.py` (839 LOC)
- `cassandra/proxy/handlers/batch.py` (1010 LOC)
- `cassandra/proxy/responses_converter.py` — already deleted in PR-C5; verify gone.
- `cassandra/proxy/memory_handler.py` (1756 LOC)
- `cassandra/proxy/memory_tool_adapter.py` (1273 LOC)
- `cassandra/proxy/semantic_cache.py` (142 LOC)
- `cassandra/proxy/savings_tracker.py` (934 LOC) — re-implemented in Rust as part of `observability/`.
- `cassandra/proxy/loopback_guard.py` (92 LOC) — Rust equivalent in `crates/cassandra-proxy/src/handlers/debug.rs`.
- `cassandra/proxy/ws_session_registry.py` (226 LOC) — Rust equivalent.
- `cassandra/proxy/interceptors/` — all of this directory.
- `cassandra/proxy/cost.py`, `helpers.py`, `rate_limiter.py`, `request_logger.py` — re-implemented in Rust as part of compression dispatch / observability.
- `cassandra/proxy/prometheus_metrics.py` — re-implemented in Rust as `observability/prometheus.rs`.
- `cassandra/proxy/extensions.py`, `models.py`, `modes.py`, `stage_timer.py`, `warmup.py`, `responses_converter.py`, `debug_introspection.py`.
- `cassandra/transforms/cache_aligner.py` — already gutted in Phase A PR-A2; delete the remaining stub.

**Move:**
- `cassandra/proxy/loopback_guard.py` test logic → `crates/cassandra-proxy/tests/integration_loopback_guard.rs`.

**Modify:**
- `cassandra/cli/proxy.py` — `cassandra proxy start` now spawns the Rust binary (`./target/release/cassandra-proxy`) instead of `uvicorn cassandra.proxy.server:app`.
- `cassandra/cli/wrap/*.py` — same: env-var setup remains, but `proxy_url` points at the Rust binary's listen address.
...
- Manual test: `cassandra proxy start` boots the Rust binary; `curl -s http://127.0.0.1:8787/healthz` returns OK.
...
**Worktree:** `~/claude-projects/cassandra-worktrees/realign-H2-retire-litellm-backends`
**Risk:** **MEDIUM**
**LOC:** **-3,000 / +100**
...
**Delete:**
- `cassandra/backends/litellm.py` (~1500 LOC, the lossy converter)
- `cassandra/backends/__init__.py` if the only contents was the LiteLLM backend.

**Modify:**
- `pyproject.toml` — remove `litellm` from dependencies. (Saves ~50 MB of installed-deps size.)
- `cassandra/providers/registry.py` — delete `litellm-bedrock`, `litellm-vertex` provider entries.
- `cassandra/cli/wrap/*` — verify no wrap CLIs route to LiteLLM (they shouldn't; they go through the proxy).
...
**Worktree:** `~/claude-projects/cassandra-worktrees/realign-H3-final-cleanup`
**Risk:** **LOW**
**LOC:** -2,000 / +500
...
- `cassandra/__init__.py` — drop unused module imports.
...
- `crates/cassandra-parity/` — the parity harness itself becomes irrelevant after Python side is gone. **Decision needed** (see `12-decisions-needed.md` Q3): keep parity-run as a "prior-version-vs-current-version" harness, or delete?

**Add:**
- `CHANGELOG.md` entry: "**Breaking**: Python proxy retired. Operators must use the Rust binary `cassandra-proxy`. See migration guide at `docs/operations/python-to-rust-migration.md`."
- `docs/operations/python-to-rust-migration.md` — operator migration guide.

### Acceptance criteria

- `git grep -i "uvicorn\|fastapi" cassandra/` returns nothing in non-test code.
...
| Module | Role | Reason |
|---|---|---|
| `cassandra/cli/wrap/*.py` | Agent launchers | Off-path; orchestrates filesystem + subprocess. Python is the right tool. |
| `cassandra/cli/{evals,init,install,learn,memory,perf,proxy,tools}.py` | CLI admin commands | Click-based; off-path. |
| `cassandra/rtk/installer.py` | RTK binary downloader | Off-path; filesystem operations. |
| `cassandra/providers/codex/install.py`, `claude/install.py` | Client config installation | Off-path; filesystem. |
| `cassandra/evals/`, `learn/`, `memory/` writers | Research / batch tooling | Off-path; long-running batch. |
| `cassandra/tokenizers/` | Parity backstop | Used only by parity harness if H3 keeps it. |
| `cassandra/telemetry/toin.py` | TOIN learning loop | Off-path; observation-only after Phase B PR-B5. |
| `cassandra/subscription/tracker.py`, `client.py` | Subscription usage poller | Off-path. |
| `cassandra/copilot_auth.py` | Copilot OAuth refresh | Off-path; specific to Copilot integration. |
...
- ✅ Operators run only the Rust `cassandra-proxy` binary
