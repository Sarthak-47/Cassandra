...
- **Worktree path:** `~/claude-projects/cassandra-worktrees/realign-<phase><num>-<slug>`. Use `git worktree add` so each PR is an isolated checkout.
...
10. Auth mode (PAYG / OAuth / subscription) gates compression policy; subscription mode runs in stealth (no `X-Cassandra-*` upstream, no beta drift, no UA mutation, no `accept-encoding` strip).
...
- `crates/cassandra-core/src/scoring/`, `relevance/`, most of `context/` (Rust)
- `crates/cassandra-proxy/src/compression/icm.rs`
- `cassandra/transforms/cache_aligner.py` rewrite path (keep detector + warning)
- `cassandra/proxy/server.py`, `handlers/anthropic.py`, `handlers/openai.py`, `handlers/streaming.py`, `handlers/gemini.py`, `responses_converter.py`, `memory_handler.py`, `memory_tool_adapter.py`, `semantic_cache.py`, `batch.py` — once Rust hits parity (Phase H)
- `cassandra/backends/litellm.py` Bedrock/Vertex converter — replaced by native envelopes (Phase D)
