...
```
                                Client request
                                      ...
                                      ▼
            ┌──────────────────────────────────────────────┐
            │ cassandra-proxy (axum)                        │
            │                                              │
            │  1. classify_auth_mode(headers)              │  ← Phase F
            │     → "payg" | "oauth" | "subscription"      │
            │                                              │
            │  2. strip x-cassandra-* from upstream-bound   │  ← Phase A (PR-A5)
            │                                              │
...
The following are never mutated by Cassandra:
...
PAYG: aggressive (full live-zone compression, CCR, tool injection, Phase 3 stabilization). OAuth: passthrough-prefer (live-zone lossless only, no auto-`cache_control`, no auto-`prompt_cache_key`, no `X-Forwarded-*`). Subscription: stealth-prefer (everything OAuth does PLUS preserve `accept-encoding`, never inject `X-Cassandra-*` upstream, never mutate `User-Agent`).
...
//   $ cargo run -p cassandra-toin-publish -- --auth-mode payg --model claude-3-7-sonnet
...
- Does NOT inject Cassandra's own tools into customer prompts unless CCR has already fired in this session (and then always, never toggling).
...
- Does NOT rewrite request headers except to strip `x-cassandra-*` from upstream-bound headers and add conditional `X-Forwarded-*` (PAYG/OAuth only).
