...
**Worktree:** `~/claude-projects/cassandra-worktrees/realign-G1-wrap-more-agents`
**Risk:** **LOW**
**LOC:** +800

### Scope
Eliminate P5-62. Add `cassandra wrap cline`, `cassandra wrap continue`, `cassandra wrap goose`, `cassandra wrap openhands` (extending the existing pattern from `wrap claude` / `wrap codex` / `wrap aider` / `wrap copilot` / `wrap cursor`). Each wrap subcommand:
1. Ensures the RTK binary is installed (`_ensure_rtk_binary()`).
2. Injects the `<!-- cassandra:rtk-instructions -->` block into the agent's instruction file (AGENTS.md / .cursorrules / etc.).
...
**Add:**
- `cassandra/cli/wrap/cline.py` — wrap implementation for Cline (agent that lives in VS Code; instruction file is `.clinerules`).
- `cassandra/cli/wrap/continue_dev.py` — Continue agent (`.continue/config.json` configuration; system message injection).
- `cassandra/cli/wrap/goose.py` — Goose agent (Block's CLI; `.goose/config.yaml`).
- `cassandra/cli/wrap/openhands.py` — OpenHands (instruction injection via `OPENHANDS_INSTRUCTIONS` env var).

**Modify:**
- `cassandra/cli/wrap/__init__.py` — register new subcommands.
- `cassandra/cli/main.py` — `cassandra wrap --help` lists new agents.
...
- Tests pass.
- Manual test: `cassandra wrap cline -- claude-3-7-sonnet` launches a Cline session with the proxy in-front and RTK instructions in `.clinerules`.
...
**Worktree:** `~/claude-projects/cassandra-worktrees/realign-G2-tokens-saved-rtk`
**Risk:** **LOW**
**LOC:** +200

### Scope
Eliminate P5-60. The `tokens_saved_rtk` field on `SubscriptionContribution` (`cassandra/subscription/models.py:260`) exists but is never populated. Wire it: poll `rtk gain --format json` periodically (already done by `_get_rtk_stats` in `helpers.py:132`), diff the cumulative `tokens_saved` since last snapshot, and feed into `tracker.update_session_savings(tokens_saved_rtk=delta)`.

### Files

**Modify:**
- `cassandra/subscription/tracker.py` — add `_last_rtk_tokens_saved: int = 0` state; on every `update_session_savings` call, fetch `_get_rtk_stats()`, compute `delta = current.tokens_saved - self._last_rtk_tokens_saved`, set `tokens_saved_rtk=delta`, update state.
- `cassandra/proxy/helpers.py:132` — `_get_rtk_stats` returns `RtkStats { invocations: int, tokens_saved: int, last_run_at: datetime }`. Memoization stays at 5s.
...
**Worktree:** `~/claude-projects/cassandra-worktrees/realign-G3-rtk-metrics-and-obs`
**Risk:** **LOW**
**LOC:** +600
...
**Modify:**
- `crates/cassandra-proxy/src/observability/prometheus.rs` — add all new metrics.
- `crates/cassandra-proxy/src/sse/anthropic.rs` — emit `proxy_cache_hit_rate_per_session` from `usage.cache_read_input_tokens / total_input_tokens` on `message_delta`.
- `crates/cassandra-proxy/src/sse/openai_responses.rs` — emit on `response.completed`.
- `crates/cassandra-proxy/src/sse/openai_chat.rs` — emit on final usage chunk.
- `crates/cassandra-proxy/src/handlers/responses.rs` — extract and log `service_tier`.
- `crates/cassandra-proxy/src/handlers/responses.rs` — log `incomplete_details.reason` when `status == incomplete`.
- `cassandra/proxy/request_logger.py` — redact base64 strings >1024 bytes; replace with `<base64 truncated, X bytes>`.
- `crates/cassandra-proxy/src/observability/cache_hit_rate.rs` — new module.
- `crates/cassandra-proxy/src/observability/compression_ratio.rs` — new module.
...
**Tests added:**
- `crates/cassandra-proxy/tests/integration_metrics.rs::cache_hit_rate_emitted_per_session`
- `crates/cassandra-proxy/tests/integration_metrics.rs::compression_ratio_emitted_per_strategy`
- `crates/cassandra-proxy/tests/integration_metrics.rs::passthrough_bytes_modified_zero_when_no_compression`
- `crates/cassandra-proxy/tests/integration_metrics.rs::service_tier_logged`
- `crates/cassandra-proxy/tests/integration_metrics.rs::incomplete_status_logged_with_reason`
