...
about: Report results of testing `cassandra wrap copilot --subscription` on Linux/Windows/macOS
...
- **How you installed cassandra**: (pipx/pip `--pre` wheel · Docker install.sh/ps1 · built from source)
...
- **Command run**:
  ```
  cassandra wrap copilot --subscription -- --model gpt-4o -p "Reply with exactly: CASSANDRA_OK"
  ```
- **Did it print `CASSANDRA_OK`?**: yes / no
...
(logs from `~/.cassandra/logs/proxy.log`, surprises, etc.)
