...
BEGIN
  ...
  WITH instance_totals AS (
    ...
    FROM (
      SELECT
        ...
        MAX(os) AS os,
        MAX(cassandra_version) AS version
      FROM proxy_telemetry_v2
      GROUP BY instance_id, created_at::date
    ) sub
  ...
  -- Version breakdown
  SELECT COALESCE(jsonb_object_agg(COALESCE(cassandra_version, '?'), cnt), '{}'::jsonb)
  INTO _versions
  FROM (
    SELECT cassandra_version, COUNT(*) AS cnt
    FROM proxy_telemetry_v2
    GROUP BY cassandra_version
  ) sub;
