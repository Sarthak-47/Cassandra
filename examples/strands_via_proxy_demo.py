#!/usr/bin/env python3
"""End-to-end demo: Strands -> Cassandra proxy -> Bedrock.
...
---------------------
1. Spawns the Cassandra proxy as a subprocess (backend=bedrock).
2. Waits for /readyz.
3. Sends two requests in the same session via Strands' OpenAIModel
   ...
   ``cache_control: {type: "ephemeral"}`` via Cassandra's CacheAligner.
4. Reports:
...
LARGE_SYSTEM_PROMPT = (
    ...
    + "Cassandra is an open-source context compression layer for LLM "
...
def start_proxy(port: int, region: str) -> subprocess.Popen[bytes]:
    """Spawn `cassandra proxy --backend bedrock` as a subprocess."""
    env = os.environ.copy()
    ...
    env.setdefault("CASSANDRA_LOG", "INFO")
    cmd = [
        sys.executable,
        "-m",
        "cassandra.cli",
    ...
    model = OpenAIModel(
        ...
        client_args={
            ...
            "default_headers": {
                ...
                "x-cassandra-session-id": SESSION_ID,
    ...
    body = json.dumps(
        {
            ...
            "messages": [
                ...
                {"role": "user", "content": "In one short sentence, what is Cassandra?"},
            ],
            "max_tokens": 60,
            "temperature": 0.2,
        }
...
async def run_demo(port: int, region: str, model_id: str) -> int:
    print("=" * 76)
    print(" Cassandra Path-B E2E: Strands -> Cassandra proxy -> Bedrock")
    print("=" * 76)
    print(f" port={port} region={region} model={model_id}")
    print(f" session_id={SESSION_ID}")
    print()

    print("[1/4] Spawning Cassandra proxy ...")
    ...
    try:
        ...
        try:
            ...
            stream_req = urllib.request.Request(  # noqa: S310
                ...
                headers={
                    ...
                    "x-cassandra-session-id": smoke_session,
                    "X-Client": "strands",
                },
                method="POST",
            )
        ...
        # ContentRouter defaults (cassandra/transforms/content_router.py):
        ...
        tool_req = urllib.request.Request(  # noqa: S310
            ...
            headers={
                "Content-Type": "application/json",
                "Authorization": "Bearer dummy",
                "x-cassandra-session-id": "compression-probe-session",
                "X-Client": "strands",
            },
            method="POST",
        )
        ...
        r1 = agent("In one short sentence, what is Cassandra?")
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Strands -> Cassandra proxy -> Bedrock E2E")
