#!/usr/bin/env python3
"""Comprehensive Strands + Bedrock Demo for Cassandra SDK.

This demo showcases two Cassandra integration patterns for AWS Strands Agents:

1. **CassandraHookProvider** - Compresses tool outputs as they happen
...
2. **CassandraStrandsModel** - Optimizes entire conversation context
...
Requirements:
    ...
    - pip install strands-agents cassandra-ai[strands]
"""
...
def check_dependencies() -> bool:
    ...
    # Check cassandra
    try:
        from cassandra.integrations.strands import (  # noqa: F401
            CassandraHookProvider,
            CassandraStrandsModel,
        )
    except ImportError:
        missing.append("cassandra-ai[strands]")
...
# ============================================================================
# Demo 1: CassandraHookProvider
# ============================================================================


def run_hook_provider_demo(region: str = "us-west-2") -> dict[str, Any]:
    """Demonstrate CassandraHookProvider for tool output compression.
    ...
    from cassandra.integrations.strands import CassandraHookProvider

    print_box(
        "Demo 1: CassandraHookProvider",
        [
            "The CassandraHookProvider intercepts tool outputs and compresses",
    ...
    # Create CassandraHookProvider
    hook_provider = CassandraHookProvider(
...
# ============================================================================
# Demo 2: CassandraStrandsModel
# ============================================================================


def run_model_wrapper_demo(region: str = "us-west-2") -> dict[str, Any]:
    """Demonstrate CassandraStrandsModel for conversation optimization.
    ...
    print_box(
        "Demo 2: CassandraStrandsModel",
        [
            "CassandraStrandsModel wraps the Bedrock model to optimize the",
            ...
            "",
            "Using: Claude 3 Haiku wrapped with CassandraStrandsModel",
        ],
    )
    ...
    # Wrap with CassandraStrandsModel
    optimized_model = CassandraStrandsModel(
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Cassandra + Strands Bedrock Demo",
...
Environment Variables:
  ...
  AWS_PROFILE             AWS profile name (alternative to keys)
    ...
    parser.add_argument(
        "--hook",
        action="store_true",
        help="Run only the CassandraHookProvider demo",
    )
    parser.add_argument(
        "--model",
        action="store_true",
        help="Run only the CassandraStrandsModel demo",
    )
    parser.add_argument(
        "--region",
    ...
    # Print header
    print_box(
        "Cassandra + Strands Bedrock Demo",
        [
            "This demo showcases Cassandra's integration with AWS Strands Agents.",
            "",
            "Cassandra provides two integration patterns:",
            "  1. CassandraHookProvider - Compress tool outputs in real-time",
            "  2. CassandraStrandsModel - Optimize entire conversation context",
            "",
        ...
        if run_hook and run_model:
            ...
            print_box(
                ...
                [
                    ...
                    "  [x] CassandraHookProvider - Real-time tool output compression",
                    "  [x] CassandraStrandsModel - Full context optimization",
                ],
                style="success",
            )
