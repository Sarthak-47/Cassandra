#!/usr/bin/env python3
"""Verify Strands' MCP dispatcher can reach Cassandra's MCP server.

This is the focused 15-min test that proves the MCP-everywhere
architecture for Path B works end-to-end:

  1. Cassandra proxy is running (started separately, port 8787).
  2. Build a Strands MCPClient that stdio-spawns `cassandra mcp serve`.
  3. List MCP tools — confirm `cassandra_retrieve` is exposed.
  4. Round-trip: stash content via the proxy's /v1/compress endpoint
     to get a hash, then call `cassandra_retrieve(hash)` via MCP,
...
Strands Agent receives a model-emitted `cassandra_retrieve` tool_call
...
def start_proxy() -> subprocess.Popen[bytes]:
    ...
    log = Path("/tmp/cassandra_proxy_mcp_test.log").open("wb")
    proc = subprocess.Popen(cmd, stdout=log, stderr=subprocess.STDOUT)  # noqa: S603
    print(f"  proxy spawned (pid={proc.pid}); log → /tmp/cassandra_proxy_mcp_test.log")
    return proc
...
def main() -> int:
    print("=" * 72)
    print(" Strands MCPClient → Cassandra MCP server  end-to-end probe")
    print("=" * 72)

    print("\n[1/5] Starting Cassandra proxy ...")
    proxy = start_proxy()
    try:
        wait_for_proxy()
        print("  proxy ready.\n")
        ...
        print("\n[3/5] Building Strands MCPClient pointed at `cassandra mcp serve` ...")
        ...
        with MCPClient(lambda: stdio_client(server_params)) as mcp:
            ...
            print(f"  tools advertised by Cassandra MCP: {tool_names}")
            if "cassandra_retrieve" not in tool_names:
                print("  ! cassandra_retrieve NOT in tool list — abort.")
                return 4
            ...
            # `cassandra_retrieve` tool_call in a real conversation.
            print("\n[5a/5] Stashing content via MCP cassandra_compress (same dispatcher) ...")
            try:
                comp = mcp.call_tool_sync(
                    tool_use_id="probe-compress-1",
                    name="cassandra_compress",
                ...
                # Find the hash. Cassandra MCP compress is documented to emit
            ...
            print("\n[5b/5] Calling cassandra_retrieve via Strands MCP dispatcher ...")
            try:
        ...
        print(" Strands receives a cassandra_retrieve tool_call → MCP dispatcher")
        print(" → Cassandra MCP server → CompressionStore → original content back.")
        print("=" * 72)
        return 0
    finally:
        print("\n  stopping proxy ...")
