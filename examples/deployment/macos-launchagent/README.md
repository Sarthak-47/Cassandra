...
This directory contains templates and scripts for running the cassandra proxy server as a persistent background service on macOS using LaunchAgent.
...
- **com.cassandra.proxy.plist.template**: LaunchAgent plist template
...
- **Standard Logging**: Logs to `~/Library/Logs/cassandra/`
- **Shell Integration**: Automatically sets `ANTHROPIC_BASE_URL` for Claude clients

## Requirements

- macOS 10.13+ (High Sierra or later)
- cassandra-ai installed with proxy support: `pip install cassandra-ai[proxy]`
...
- Port already in use
- cassandra not installed with proxy support
...
2. Edit the plist file:
   - Replace `__CASSANDRA_PATH__` with output of `command -v cassandra`
