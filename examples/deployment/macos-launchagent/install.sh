#!/usr/bin/env bash
#
# Cassandra Proxy LaunchAgent Installer for macOS
#
# This script installs the cassandra proxy as a macOS LaunchAgent for automatic
...
info "Checking for cassandra installation..."
CASSANDRA_PATH=$(command -v cassandra || true)
if [[ -z "${CASSANDRA_PATH}" ]]; then
  fatal "cassandra not found in PATH. Please install it first: pip install cassandra-ai[proxy]"
fi
success "Found cassandra at: ${CASSANDRA_PATH}"

# Verify proxy support
info "Verifying proxy support..."
if ! "${CASSANDRA_PATH}" proxy --help >/dev/null 2>&1; then
  fatal "cassandra proxy command not available. Install with: pip install cassandra-ai[proxy]"
fi
...
sed -e "s|__CASSANDRA_PATH__|${CASSANDRA_PATH}|g" \
  -e "s|__PORT__|${PORT}|g" \
  -e "s|__HOME__|${HOME}|g" \
...
echo ""
echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${GREEN}✓ Cassandra proxy installed successfully!${NC}"
echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
...
echo ""
echo "    export CASSANDRA_PROXY_PORT=${PORT}"
