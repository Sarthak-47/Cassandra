...
Generates representative sample data and runs it through Cassandra's tabular
...
def _run_xlsx(label: str, path: Path) -> None:
    res = cassandra.compress_spreadsheet(str(path))
    pct = 100 * res.tokens_saved / res.tokens_before if res.tokens_before else 0.0
    print(
        f"{label:24s} tokens    {res.tokens_before:6d} -> "
        f"{res.tokens_after:6d}  ({pct:5.1f}% saved)"
...
def main() -> None:
    ...
    if not _HAS_OPENPYXL:
        print("  skipped — install: pip install cassandra-ai[spreadsheet]")
    else:
