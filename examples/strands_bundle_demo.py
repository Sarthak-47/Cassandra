#!/usr/bin/env python3
"""Strands agent + Cassandra — drop-in compression demo.

This is what a real Strands user writes. The ONLY Cassandra-specific
...
The demo:
  ...
  2. Builds a normal Strands `Agent` with that tool + the bundle's MCP
     tools (cassandra_compress / cassandra_retrieve / cassandra_stats).
  3. Sends ONE user query.
  4. Lets the agent loop autonomously.
  5. Prints the answer + the proxy /stats summary so you can SEE that
     Cassandra compressed the verbose tool output on the way to Bedrock.

The Cassandra proxy is started as a background process here so the
...
# ============================================================================
# Boilerplate: start the Cassandra proxy as a child process for the demo.
...
def _start_proxy() -> subprocess.Popen[bytes]:
    ...
    log_path = Path("/tmp/cassandra_bundle_demo_proxy.log")
...
def _print_stats_panel() -> None:
    ...
    # MCP-side work (cassandra_compress / cassandra_retrieve called by the LLM
...
# Cassandra-specific bits are the `CassandraBundle` import and one
...
from cassandra.integrations.strands import CassandraBundle  # noqa: E402
...
def search_documentation(query: str) -> str:
    ...
    # Cassandra is designed to shrink.
    articles = [
        {
            "id": f"doc-{i:04d}",
...
def run_agent_demo() -> None:
    ...
    # === The only Cassandra-specific code in your app ===
    bundle = CassandraBundle(
        proxy_url=PROXY_URL,
    ...
    agent = Agent(
        model=model,
        # bundle.tools = [Cassandra MCP client]; we also add our local tool
...
def main() -> int:
    print("=" * 72)
    print(" Strands agent + Cassandra (drop-in compression)")
    print("=" * 72)
    print(f" proxy: {PROXY_URL}    region: {os.environ.get('AWS_REGION', 'us-west-2')}")
    print()
    print(" Starting Cassandra proxy (in production this is a long-lived service) ...")
    proxy = _start_proxy()
    try:
        _wait_for_proxy()
        print("  → proxy ready.")
        ...
        _print_stats_panel()
        print("\n" + "=" * 72)
        print(" Done. If requests_compressed > 0 above, Cassandra shrunk the")
