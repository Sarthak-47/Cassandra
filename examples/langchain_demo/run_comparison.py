"""Real-world LangChain Agent: Before/After Cassandra Comparison.

This script demonstrates the impact of Cassandra optimization on a realistic
...
class AgentRun:
    ...
    scenario: str
    mode: str  # "baseline" or "cassandra"
...
def run_agent_baseline(scenario: dict, api_key: str) -> AgentRun:
    """Run agent WITHOUT Cassandra (baseline)."""
...
def run_agent_cassandra(scenario: dict, api_key: str) -> AgentRun:
    """Run agent WITH Cassandra optimization."""

    # Import Cassandra integration
    ...
    cassandra_model = CassandraChatModel(
    ...
    for _ in range(5):
        ...
        # Call model (Cassandra optimizes internally)
        response = cassandra_model.invoke(messages)
        messages.append(response)
    ...
    tokens_saved = cassandra_model.get_total_tokens_saved()
...
def print_comparison(baseline: AgentRun, cassandra: AgentRun):
    """Print comparison between baseline and cassandra runs."""
    ...
    # Token comparison
    input_saved = baseline.total_input_tokens - cassandra.total_input_tokens
    input_pct = (
        (input_saved / baseline.total_input_tokens * 100) if baseline.total_input_tokens > 0 else 0
    )

    print(f"\n{'METRIC':<30} {'BASELINE':>15} {'CASSANDRA':>15} {'SAVINGS':>15}")
    print("-" * 75)
    print(
        f"{'Input Tokens':<30} {baseline.total_input_tokens:>15,} {cassandra.total_input_tokens:>15,} {input_saved:>14,} ({input_pct:.1f}%)"
    )
    print(
        f"{'Output Tokens':<30} {baseline.total_output_tokens:>15,} {cassandra.total_output_tokens:>15,} {'N/A':>15}"
    )
    print(
        f"{'Tool Output Tokens':<30} {baseline.tool_output_tokens:>15,} {cassandra.tool_output_tokens:>15,} {'(raw)':>15}"
    )
    print(f"{'Tool Calls':<30} {baseline.tool_calls:>15} {cassandra.tool_calls:>15} {'':>15}")
    print(f"{'Messages':<30} {baseline.messages_count:>15} {cassandra.messages_count:>15} {'':>15}")
    print(
        f"{'Duration (ms)':<30} {baseline.duration_ms:>15.0f} {cassandra.duration_ms:>15.0f} {'':>15}"
    )
    ...
    cassandra_cost = (
        cassandra.total_input_tokens * input_cost_per_1m
        + cassandra.total_output_tokens * output_cost_per_1m
    ) / 1_000_000
    cost_saved = baseline_cost - cassandra_cost
    cost_pct = (cost_saved / baseline_cost * 100) if baseline_cost > 0 else 0

    print(
        f"\n{'Estimated Cost (USD)':<30} ${baseline_cost:>14.6f} ${cassandra_cost:>14.6f} ${cost_saved:>13.6f} ({cost_pct:.1f}%)"
    )
...
def main():
    ...
    print("\n" + "=" * 70)
    print("LANGCHAIN AGENT: BEFORE/AFTER CASSANDRA COMPARISON")
    print("=" * 70)
    ...
    for scenario in SCENARIOS:
        ...
        # Run with Cassandra
        print("  - Running with Cassandra optimization...")
        cassandra = run_agent_cassandra(scenario, api_key)
        all_cassandra.append(cassandra)

        # Print comparison
        print_comparison(baseline, cassandra)

    # Print summary
    print_summary(all_baseline, all_cassandra)
...
def run_simulation():
    ...
    for scenario in SCENARIOS:
        ...
        print(f"  With Cassandra (20 items max), estimated: ~{total_tool_tokens // 5:,} tokens")
        print(
            f"  Estimated savings: ~{total_tool_tokens * 2 - total_tool_tokens // 5:,} tokens (~80%)"
        )


def print_summary(baseline_runs: list[AgentRun], cassandra_runs: list[AgentRun]):
    ...
    total_cassandra_input = sum(r.total_input_tokens for r in cassandra_runs)
    total_saved = total_baseline_input - total_cassandra_input
    pct_saved = (total_saved / total_baseline_input * 100) if total_baseline_input > 0 else 0

    print(f"\n{'Metric':<30} {'Baseline':>15} {'Cassandra':>15} {'Savings':>15}")
    print("-" * 75)
    print(
        f"{'Total Input Tokens':<30} {total_baseline_input:>15,} {total_cassandra_input:>15,} {total_saved:>14,}"
    )
    ...
    cassandra_cost = total_cassandra_input * input_cost
    cost_saved = baseline_cost - cassandra_cost

    print(
        f"\n{'Est. Input Cost (USD)':<30} ${baseline_cost:>14.4f} ${cassandra_cost:>14.4f} ${cost_saved:>13.4f}"
    )
    ...
    print("=" * 70)
    print(f"""
Cassandra reduced input tokens by {pct_saved:.1f}% across all scenarios.
