...
This demonstrates the minimal code change needed to add Cassandra
...
def main():
    print("\n" + "=" * 70)
    print("CASSANDRA MCP INTEGRATION - DEVELOPER EXPERIENCE")
    print("=" * 70)
...
"""
    ...
    print("\nAFTER (with Cassandra compression):")
    print("-" * 40)
    after_standalone = """
from cassandra.integrations.mcp import compress_tool_result  # ADD THIS
...
"""
    ...
    after_wrapper = """
from mcp import Client
from cassandra.integrations.mcp import CassandraMCPClientWrapper  # ADD THIS

# Create MCP client
base_client = Client(transport)
client = CassandraMCPClientWrapper(base_client)  # WRAP IT (1 line)
...
"""
    ...
    print("-" * 40)
    with_metrics = """
from cassandra.integrations.mcp import compress_tool_result_with_metrics
