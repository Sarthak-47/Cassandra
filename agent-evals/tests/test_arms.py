...
skips unless ANTHROPIC_API_KEY (or an explicit CASSANDRA_LIVE) is present.
"""
...
def test_build_proxy_command_b_token() -> None:
    ...
    spec = _spec(ArmName.B_CASSANDRA, Provider.ANTHROPIC, proxy_mode=ProxyMode.TOKEN)
...
def test_build_proxy_command_honors_custom_cmd() -> None:
    ...
    settings = _settings()
    settings.proxy.cassandra_cmd = ["python", "-m", "cassandra", "proxy"]
    spec = _spec(ArmName.A1_PASSTHROUGH, Provider.ANTHROPIC, proxy_mode=ProxyMode.OFF)
    assert build_proxy_command(spec, settings, port=99)[:4] == ["python", "-m", "cassandra", "proxy"]
...
def test_build_arm_env_openai_appends_v1() -> None:
    ...
    spec = _spec(ArmName.B_CASSANDRA, Provider.OPENAI, proxy_mode=ProxyMode.TOKEN)
    env = build_arm_env(spec, _settings(), "http://127.0.0.1:18800")
    assert env == {
...
# --------------------------------------------------------------------------------------------
# CassandraArm.__aenter__ / __aexit__
# --------------------------------------------------------------------------------------------
...
async def test_aenter_a0_spawns_no_subprocess(
    ...
    spec = _spec(ArmName.A0_DIRECT, Provider.ANTHROPIC, proxy_mode=None)
    arm = CassandraArm(spec, settings, tmp_path)
...
def test_cassandra_arm_satisfies_protocol(tmp_path: Path) -> None:
    spec = _spec(ArmName.A0_DIRECT, Provider.ANTHROPIC, proxy_mode=None)
    arm = CassandraArm(spec, _settings(), tmp_path)
    assert isinstance(arm, ArmProto)


# --------------------------------------------------------------------------------------------
# Live test (opt-in): spawn a real cassandra proxy in passthrough and hit /readyz.
# --------------------------------------------------------------------------------------------


@pytest.mark.live
@pytest.mark.skipif(
    not os.environ.get("ANTHROPIC_API_KEY") and not os.environ.get("CASSANDRA_LIVE"),
    reason="requires ANTHROPIC_API_KEY (or CASSANDRA_LIVE=1) and an installed cassandra",
)
async def test_live_real_proxy_readyz(tmp_path: Path) -> None:
    """Spawn `cassandra proxy --no-optimize`, confirm /readyz, then tear down."""
