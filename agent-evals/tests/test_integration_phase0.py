...
``x-cassandra-*`` headers), the real ``Orchestrator``/``Journal``, the real ``ArmHandle``
...
class FakeArm:
    ...
    async def __aenter__(self) -> ArmHandle:
        ...
        self._active.emits_headers = self.spec.name == ArmName.B_CASSANDRA
        return self.handle
...
class FakeHarness:
    ...
    REAL response hook with a synthetic httpx.Response carrying real x-cassandra headers."""
...
async def test_phase0_pipeline_composes(tmp_path: Path) -> None:
    ...
    arms = [
        ...
        _arm(ArmName.B_CASSANDRA, ProxyMode.TOKEN, pricing, active),
    ]
    ...
    assert set(by_arm) == {ArmName.A0_DIRECT, ArmName.A1_PASSTHROUGH, ArmName.B_CASSANDRA}
    # Resolved rates reflect the graders.
    assert by_arm[ArmName.B_CASSANDRA].resolved_rate == pytest.approx(1.0)
    assert by_arm[ArmName.A1_PASSTHROUGH].resolved_rate == pytest.approx(0.5)
    # Only the compression arm captured savings (the others produced no x-cassandra headers).
    assert by_arm[ArmName.B_CASSANDRA].median_savings_percent == pytest.approx(60.0)
    ...
    assert "b_cassandra" in rendered
...
async def test_savings_attributed_per_task_via_real_handle(tmp_path: Path) -> None:
    ...
    pricing = Pricing(input_usd_per_1m=2.0)
    active = _Active()
    b = _arm(ArmName.B_CASSANDRA, ProxyMode.TOKEN, pricing, active)
