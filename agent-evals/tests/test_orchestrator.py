...
@pytest.fixture
def cassandra_spec() -> ArmSpec:
    return ArmSpec(
        name=ArmName.B_CASSANDRA,
...
def test_journal_append_and_load(tmp_path: Path, pricing: Pricing) -> None:
    journal = Journal(tmp_path)
    r = TaskResult(task_id="t1", arm=ArmName.B_CASSANDRA, run_index=0, resolved=True)
    journal.append(r)
    journal.append(TaskResult(task_id="t2", arm=ArmName.B_CASSANDRA, run_index=1, resolved=False))
...
async def test_full_run(tmp_path: Path, pricing: Pricing, cassandra_spec: ArmSpec) -> None:
    tasks = _tasks("t1", "t2")
    arm = FakeArm(cassandra_spec, pricing)
    harness = FakeHarness()
...
async def test_resume_skips_completed_cells(
    tmp_path: Path, pricing: Pricing, cassandra_spec: ArmSpec
) -> None:
    tasks = _tasks("t1", "t2")
    journal = Journal(tmp_path)
    # Pre-write 2 of the 4 cells (run_index 0 for both tasks).
    journal.append(TaskResult(task_id="t1", arm=ArmName.B_CASSANDRA, run_index=0, resolved=True))
    journal.append(TaskResult(task_id="t2", arm=ArmName.B_CASSANDRA, run_index=0, resolved=False))
...
async def test_error_capture(tmp_path: Path, pricing: Pricing, cassandra_spec: ArmSpec) -> None:
    tasks = _tasks("t1", "t2")
    arm = FakeArm(cassandra_spec, pricing)
...
async def test_timeout_capture(tmp_path: Path, pricing: Pricing, cassandra_spec: ArmSpec) -> None:
    tasks = _tasks("t1")
    arm = FakeArm(cassandra_spec, pricing)
