"""Opt-in LIVE tests (``-m live``): spawn a real ``cassandra proxy`` and hit a real upstream.
...
* ``test_b_arm_captures_savings_live`` — the B arm emits ``x-cassandra-*`` headers that the client
...
from agent_evals.arms import CassandraArm
...
def _arm(
    ...
    return CassandraArm(spec, settings, run_dir, **kw)  # type: ignore[arg-type]
