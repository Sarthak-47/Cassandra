...
def test_task_result_cell_key() -> None:
    tr = TaskResult(task_id="t1", arm=ArmName.B_CASSANDRA, run_index=3, resolved=True)
    assert tr.cell_key == ("t1", "b_cassandra", 3)
...
def test_arm_name_values_are_stable() -> None:
    ...
    assert ArmName.B_CASSANDRA.value == "b_cassandra"
