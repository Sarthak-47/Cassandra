...
def results(pricing: Pricing) -> list[TaskResult]:
    ...
    # B_CASSANDRA — with savings; latency 10ms each so mean_added_latency_ms == 10.0.
    b_cells = [
        ("t1", 0, True, 1000, 500),
        ("t1", 1, True, 1000, 500),
        ("t2", 0, True, 1000, 400),
...
def test_resolved_rates_per_arm(results: list[TaskResult]) -> None:
    ...
    assert _arm(sc, ArmName.B_CASSANDRA).resolved_rate == pytest.approx(0.75)


def test_cell_and_task_counts(results: list[TaskResult]) -> None:
    sc = build_scorecard(results, experiment_id="exp-1")
    b = _arm(sc, ArmName.B_CASSANDRA)
    assert b.n_cells == 4
    assert b.n_tasks == 2
...
def test_accuracy_delta_none_when_a1_missing(pricing: Pricing) -> None:
    only_treatment = [
        TaskResult(task_id="t1", arm=ArmName.B_CASSANDRA, run_index=0, resolved=True),
    ]
...
def test_arms_emitted_in_declaration_order(results: list[TaskResult]) -> None:
    ...
    assert emitted == [ArmName.A0_DIRECT, ArmName.A1_PASSTHROUGH, ArmName.B_CASSANDRA]
...
def test_render_scorecard_returns_text(results: list[TaskResult]) -> None:
    ...
    for arm in (ArmName.A0_DIRECT, ArmName.A1_PASSTHROUGH, ArmName.B_CASSANDRA):
