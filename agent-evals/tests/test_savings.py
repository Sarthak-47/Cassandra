...
def test_parse_headers_mixed_case_keys(pricing: Pricing) -> None:
    headers = {
        "X-Cassandra-Tokens-Before": "800",
        "X-CASSANDRA-TOKENS-AFTER": "200",
        "X-Cassandra-Cached": "TRUE",
    }
    s = parse_savings_headers(headers, pricing)
...
def test_response_hook_noop_when_no_cassandra_headers(pricing: Pricing) -> None:
    store = SavingsStore()
