...
class ArmName(str, Enum):
    """The experiment arms. Headline accuracy claim = B_CASSANDRA vs A1_PASSTHROUGH."""

    A0_DIRECT = "a0_direct"
    A1_PASSTHROUGH = "a1_passthrough"
    B_CASSANDRA = "b_cassandra"
    B_ABLATE = "b_ablate"
...
class ProxyMode(str, Enum):
    """How the Cassandra proxy is launched for an arm. An ArmSpec with ``proxy_mode=None``
    means no proxy at all (A0 direct)."""

    OFF = "off"  # `cassandra proxy --no-optimize` — proxy in path, compression disabled (A1)
    TOKEN = "token"  # `cassandra proxy --mode token` — compression enabled (B)
...
class ArmSpec(BaseModel):
    ...
    # Extra flags appended to the `cassandra proxy` command for ablation arms,
...
class TaskSavings(BaseModel):
    """Per-task Layer-1 savings, parsed from the per-response ``x-cassandra-*`` headers.

    Per spec §4.1/§5: ``tokens_before/after/saved`` come from the per-request response headers
    (``x-cassandra-tokens-before``/``-after``/``-saved``). ``savings_percent`` and ``ratio`` are
    DERIVED (``x-cassandra-savings-percent`` is batch-path-only). ``cost_*`` is DERIVED
