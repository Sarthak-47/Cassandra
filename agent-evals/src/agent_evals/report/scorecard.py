...
(``B_CASSANDRA`` resolved_rate minus ``A1_PASSTHROUGH`` resolved_rate) that is explicitly
...
# The headline accuracy claim is B_CASSANDRA vs A1_PASSTHROUGH (see ArmName docstring).
_HEADLINE_TREATMENT = ArmName.B_CASSANDRA
...
class Scorecard(BaseModel):
    ...
    # B_CASSANDRA resolved_rate - A1_PASSTHROUGH resolved_rate. None if either arm is absent.
...
def render_scorecard(scorecard: Scorecard) -> str:
    ...
    if scorecard.accuracy_delta_b_vs_a1 is None:
        delta_text = "n/a (B_CASSANDRA or A1_PASSTHROUGH arm missing)"
    else:
        delta_pp = scorecard.accuracy_delta_b_vs_a1 * 100.0
        delta_text = f"{delta_pp:+.1f}pp (B_CASSANDRA - A1_PASSTHROUGH resolved rate)"
