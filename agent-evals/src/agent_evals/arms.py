...
This module owns the lifecycle of one experiment arm. ``CassandraArm`` is an async context
manager that — depending on the spec — either resolves a provider-default ``base_url`` (A0
direct, no proxy) or spawns a ``cassandra proxy`` subprocess, waits for ``/readyz``, and tears
it down on exit.
...
against ``cassandra/cli/proxy.py`` (``--port``, ``--no-optimize``, ``--mode token``, and ablation
...
# Mode flags. Verified against cassandra/cli/proxy.py:
#   --no-optimize  -> passthrough (optimize disabled) for ProxyMode.OFF (A1).
#   --mode token   -> compression enabled for ProxyMode.TOKEN (B).
# These two literals are the *protocol* the cassandra CLI exposes; they are not tunable knobs,
# so they live here as the single source of truth for the off/token mapping. The command head
# (``cassandra proxy``) and everything else come from Settings.
_PASSTHROUGH_FLAG = "--no-optimize"
_MODE_OPTION = "--mode"


def build_proxy_command(spec: ArmSpec, settings: Settings, port: int) -> list[str]:
    """Build the ``cassandra proxy`` argv for an arm. PURE — no I/O.

    Layout: ``settings.proxy.cassandra_cmd`` + ``--port <port>`` + mode flag + ``spec.proxy_flags``.
    ...
    cmd: list[str] = list(settings.proxy.cassandra_cmd)
    cmd += ["--port", str(port)]
...
def build_arm_env(spec: ArmSpec, settings: Settings, base_url: str) -> dict[str, str]:
    ...
    proxy's ``http://127.0.0.1:<port>``). Per the cassandra CLI usage contract:
...
class CassandraArm:
    ...
    settings is used. Otherwise a ``cassandra proxy`` subprocess is spawned, probed at ``/readyz``,
