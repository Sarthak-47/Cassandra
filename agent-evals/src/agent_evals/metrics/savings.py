...
Two attribution surfaces, matching the Cassandra proxy's two reporting channels:

* **Per-request** — every optimized response carries ``x-cassandra-*`` headers
...
``cassandra/proxy/handlers/{openai,anthropic}.py``). ``x-cassandra-savings-percent`` is
...
# --- Per-response header names (verified against cassandra proxy handlers) -------------------
# Required token headers — absent => no Cassandra optimization happened on this response.
HEADER_TOKENS_BEFORE = "x-cassandra-tokens-before"
HEADER_TOKENS_AFTER = "x-cassandra-tokens-after"
...
HEADER_TOKENS_SAVED = "x-cassandra-tokens-saved"
HEADER_MODEL = "x-cassandra-model"
# Conditional headers — only present when their condition holds.
HEADER_TRANSFORMS = "x-cassandra-transforms"
HEADER_CACHED = "x-cassandra-cached"
HEADER_COMPRESSION_FAILED = "x-cassandra-compression-failed"

# String the proxy writes for a true boolean header (e.g. ``x-cassandra-cached: "true"``).
_HEADER_TRUE = "true"
...
def parse_savings_headers(
    ...
    """Parse the per-response ``x-cassandra-*`` headers into a :class:`TaskSavings`.

    Returns ``None`` (never a fabricated zero) when the required token headers are absent —
    that signals "no Cassandra optimization on this response" (e.g. a pass-through / A1 arm, or
    a non-optimized request). ``savings_percent``/``ratio``/``cost_*`` are DERIVED by
    :meth:`TaskSavings.from_token_counts`; ``x-cassandra-savings-percent`` is never read (it is
    batch-path-only).
    """
    ...
    try:
        ...
        logger.warning(
            "malformed cassandra token headers; skipping savings capture",
            extra={
                "fields": {
...
def make_response_hook(
    ...
    On each response it reads the ``x-cassandra-*`` headers; if ``task_id_getter()`` resolves to
    ...
    Cassandra headers, nothing is recorded — savings are never fabricated.
    """
