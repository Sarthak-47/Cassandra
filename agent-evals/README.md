...
End-to-end accuracy A/B framework for **Cassandra**: run trusted coding-agent benchmarks
**WITH vs WITHOUT** Cassandra's context-compression proxy and produce a statistically
defensible verdict — *does compression preserve what the agent can solve, and how much
does it save?*

This is a self-contained nested project inside the `cassandra` repo. It consumes Cassandra
only as the system-under-test (via `base_url`); it is **not** part of the `cassandra-ai`
wheel and is **not** wired into cassandra's `make ci-precheck`.

## The clean A/B (why a proxy helps)

Cassandra sits in the request path, so the only variable between arms is `base_url`:

| Arm | base_url | Cassandra mode | Isolates |
|-----|----------|---------------|----------|
| `A0_DIRECT` | provider API | none | native agent score |
| `A1_PASSTHROUGH` | `localhost:N` | `--no-optimize` | proxy-hop cost only |
| `B_CASSANDRA` | `localhost:M` | `--mode token` | compression cost (vs A1) |
